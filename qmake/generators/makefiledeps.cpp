...
parameter_list|)
block|{
comment|// Join physical lines to make logical lines, as in the C preprocessor
while|while
condition|(
name|offset
operator|+
...
return|;
block|}
comment|// That also skipped any remaining BSNLs immediately after the match.
comment|// Tell caller how long the match was:
operator|*
name|matchlen
operator|=
name|x
...
end_function
begin_comment
comment|/* Advance from an opening quote at buffer[offset] to the matching close quote. */
end_comment
begin_function
...
parameter_list|)
block|{
comment|// It might be a C++11 raw string.
name|bool
name|israw
init|=
literal|false
...
name|pos
parameter_list|)
value|skipEscapedLineEnds(buffer, buffer_len, (pos), lines)
name|offset
operator|=
name|SKIP_BSNL
argument_list|(
...
condition|)
block|{
comment|// skip, but keep track of lines
if|if
condition|(
name|qmake_endOfLine
argument_list|(
...
name|extralines
expr_stmt|;
comment|// from the match
comment|// buffer[offset] is ')'
name|offset
operator|+=
literal|1
operator|+
...
block|{
comment|// Traditional string or char literal:
specifier|const
name|char
name|term
init|=
...
enum|enum
block|{
comment|/*           States of C preprocessing (for TYPE_C only), after backslash-newline           elimination and skipping comments and spaces (i.e. in ANSI X3.159-1989           section 2.1.1.2's phase 4).  We're about to study buffer[x] to decide           on which transition to do.          */
name|AtStart
block|,
comment|// start of logical line; a # may start a preprocessor directive
name|HadHash
block|,
comment|// saw a # at start, looking for preprocessor keyword
name|WantName
block|,
comment|// saw #include or #import, waiting for name
name|InCode
comment|// after directive, parsing non-#include directive or in actual code
block|}
name|cpp_state
init|=
name|AtStart
...
name|buffer
decl_stmt|;
comment|// skip UTF-8 BOM, if present
if|if
condition|(
name|p
index|[
...
condition|)
block|{
comment|// We've studied all buffer[i] for i< x
for|for
control|(
init|;
name|x
...
control|)
block|{
comment|// How to handle backslash-newline (BSNL) pairs:
DECL|macro|SKIP_BSNL
define|#
directive|define
name|SKIP_BSNL
...
name|pos
parameter_list|)
value|skipEscapedLineEnds(buffer, buffer_len, (pos),&line_count)
...
name|y
expr_stmt|;
comment|// for loop shall step past this
break|break;
block|}
block|}
elseif|else
...
block|{
comment|// buffer[x] is the division operator
break|break;
block|}
block|}
elseif|else
...
block|{
comment|/* Drop out of phases 1, 2, 3, into phase 4 */
break|break;
block|}
block|}
comment|// Phase 4 study of buffer[x]:
if|if
condition|(
name|x
operator|>=
...
case|:
block|{
comment|// Read keyword; buffer[x] starts first preprocessing token after #
specifier|const
name|char
modifier|*
specifier|const
...
condition|)
block|{
comment|// skip over keyword, consolidating it if it contains BSNLs
comment|// (see WantName's similar code consolidating inc, below)
if|if
condition|(
name|clean
operator|<
...
operator|--
expr_stmt|;
comment|// Still need to study buffer[x] next time round for loop.
name|cpp_state
operator|=
operator|(
operator|(
...
condition|)
block|{
comment|/*                           Possibly malformed, but this may be something like:                           #include IDENTIFIER                           which does work, if #define IDENTIFIER "filename" is                           in effect.  This is beyond this noddy preprocessor's                           powers of tracking.  So give up and resume searching                           for a directive.  We haven't made sense of buffer[x],                           so back up to ensure we do study it (now as code) next                           time round the loop.                         */
name|x
operator|--
expr_stmt|;
name|cpp_state
...
name|x
decl_stmt|;
comment|// offset if we need to clear \-newlines
for|for
control|(
init|;
name|x
...
break|break;
block|}
comment|/*                           If we do skip any BSNLs, we need to consolidate the                           surviving text by copying to lower indices.  For that                           to be possible, we also have to keep 'clean' advanced                           in step with x even when we've yet to see any BSNLs.                         */
if|if
condition|(
name|clean
operator|<
...
name|InCode
expr_stmt|;
comment|// ... and fall through to handle buffer[x] as such.
case|case
name|InCode
case|:
comment|// matching quotes (string literals and character literals)
if|if
condition|(
name|buffer
index|[
...
argument_list|)
expr_stmt|;
comment|// for loop's ++x shall step over the closing quote.
block|}
comment|// else: buffer[x] is just some code; move on.
break|break;
block|}
if|if
condition|(
name|inc
condition|)
comment|// We were in WantName and found a name.
break|break;
DECL|macro|SKIP_BSNL
undef|#
directive|undef
...
condition|)
block|{
comment|// If comment, advance to the character that ends it:
if|if
condition|(
name|buffer
index|[
...
block|}
block|}
comment|// else: don't update x, buffer[x] is just the division operator.
block|}
block|}
elseif|else
if|if
...
argument_list|)
expr_stmt|;
comment|// Leaves us on closing quote; for loop's x++ steps us past it.
block|}
if|if
condition|(
name|x
