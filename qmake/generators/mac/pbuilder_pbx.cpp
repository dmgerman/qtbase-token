...
condition|)
block|{
comment|// The resource copy phase doesn't support paths, so we have to use
comment|// a regular file copy phase (which doesn't optimize the resources).
name|QString
name|phase_key
init|=
name|keyFor
...
block|{
comment|// Otherwise we leave it to the resource copy phase below
name|bundle_resources_files
operator|+=
name|bundle_files
expr_stmt|;
