...
name|v
index|[
literal|"QMAKE_BUILTIN_COMPILERS"
index|]
operator|=
name|ProStringList
argument_list|()
...
argument_list|)
decl_stmt|;
comment|// If the file lives in the output dir we treat it as 'owned' by
comment|// the current project, so it should be distcleaned by it as well.
if|if
condition|(
name|fi
operator|.
...
name|stderr
argument_list|,
literal|"Error: %s does not define QMAKE_PRL_TARGET\n"
argument_list|,
name|meta_file
operator|.
name|toLatin1
...
name|stderr
argument_list|,
literal|"Error: %s defines QMAKE_PRL_TARGET without extension\n"
argument_list|,
name|meta_file
operator|.
name|toLatin1
...
literal|1
argument_list|,
literal|"  Replacing library reference %s with %s"
argument_list|,
name|file
operator|.
name|mid
...
control|)
block|{
comment|// Unfortunately we were not consistent about the C++ naming
name|ProString
...
condition|)
block|{
comment|// ### This is basically fubar. Add 'lines' flag to CONFIG?
name|QStringList
name|dep_cmd_deps
init|=
name|indeps
...
name|qFatal
argument_list|(
literal|"MakefileGenerator::fixLibFlag() called"
argument_list|)
expr_stmt|;
return|return
name|ProString
...
name|first
argument_list|(
literal|"QMAKE_PKGCONFIG_VERSION"
argument_list|)
decl_stmt|;
if|if
condition|(
...
parameter_list|)
block|{
comment|// The paths are escaped in prl files, so every slash needs to turn into two backslashes.
comment|// Then each backslash needs to be escaped for sed. And another level for C quoting here.
return|return
name|QString
argument_list|(
name|str
