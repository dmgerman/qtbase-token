...
name|Q_OS_BSD4
argument_list|)
comment|// FreeBSD, OpenBSD, NetBSD, BSD/OS, OS X
name|size_t
name|len
init|=
sizeof|sizeof
...
condition|)
block|{
comment|// The termination conditions wouldn't be evaluated, so we must skip it.
name|traceMsg
argument_list|(
literal|"skipping forever loop in cumulative mode"
argument_list|)
expr_stmt|;
return|return
name|ReturnFalse
...
condition|)
block|{
comment|// Such a loop is unlikely to contribute something useful to the
comment|// file collection, and may cause considerable delay.
name|traceMsg
argument_list|(
literal|"skipping excessive loop in cumulative mode"
argument_list|)
expr_stmt|;
return|return
name|ReturnFalse
