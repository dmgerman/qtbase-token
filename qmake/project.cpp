...
directive|if
literal|0
comment|// This is way too noisy
block|if(!varlist.isEmpty()) {                 bool send_warning = false;                 if(var != "TEMPLATE"&& var != "TARGET") {                     QSet<QString> incoming_vals = vallist.toSet();                     for(int i = 0; i< varlist.size(); ++i) {                         const QString var = varlist.at(i).trimmed();                         if(!var.isEmpty()&& !incoming_vals.contains(var)) {                             send_warning = true;                             break;                         }                     }                 }                 if(send_warning)                     warn_msg(WarnParser, "Operator=(%s) clears variables previously set: %s:%d",                              var.toLatin1().constData(), parser.file.toLatin1().constData(), parser.line_no);             }
endif|#
directive|endif
name|varlist
operator|.
...
argument_list|)
expr_stmt|;
comment|// The spec extends the feature search path, so invalidate the cache.
name|invalidateFeatureRoots
argument_list|()
expr_stmt|;
...
name|stderr
argument_list|,
literal|"%s:%d system_path(path) requires one argument.\n"
argument_list|,
name|parser
operator|.
name|file
...
name|stderr
argument_list|,
literal|"%s:%d shell_path(path) requires one argument.\n"
argument_list|,
name|parser
operator|.
name|file
...
name|stderr
argument_list|,
literal|"%s:%d system_quote(args) requires one argument.\n"
argument_list|,
name|parser
operator|.
name|file
