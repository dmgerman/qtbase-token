...
block|}
block|}
comment|// make patch_capabilities and createpackage scripts for Symbian that can be used from the shadow build
name|QFile
name|patch_capabilities
argument_list|(
name|buildPath
operator|+
literal|"/bin/patch_capabilities"
argument_list|)
decl_stmt|;
if|if
condition|(
...
block|}
name|QFile
name|patch_capabilities_bat
argument_list|(
name|buildPath
operator|+
literal|"/bin/patch_capabilities.bat"
argument_list|)
decl_stmt|;
if|if
condition|(
...
block|}
name|QFile
name|createpackage_bat
argument_list|(
name|buildPath
operator|+
literal|"/bin/createpackage.bat"
argument_list|)
decl_stmt|;
if|if
condition|(
...
name|QString
argument_list|(
literal|"\\resource\\qt%1\\translations"
argument_list|)
operator|.
name|arg
argument_list|(
...
name|QString
argument_list|(
literal|"make debug-winscw|debug-armv5|release-armv5"
argument_list|)
expr_stmt|;
block|}
elseif|else
...
literal|"yes"
expr_stmt|;
comment|// iconv makes makes apps start and run ridiculously slowly in symbian emulator (HW not tested)
comment|// iconv_open seems to return -1 always, so something is probably missing from the platform.
name|dictionary
index|[
literal|"QT_ICONV"
index|]
...
index|]
operator|=
literal|"\\resource\\qt\\translations"
expr_stmt|;
name|dictionary
index|[
literal|"ARM_FPU_TYPE"
...
argument_list|)
expr_stmt|;
comment|// Qt\Symbian only options go below here -----------------------------------------------------------------------------
name|desc
argument_list|(
literal|"Qt for Symbian OS only:\n\n"
argument_list|)
expr_stmt|;
name|desc
argument_list|(
...
literal|"-fpu<flags>"
argument_list|,
literal|"VFP type on ARM, supported options: softvfp(default) | vfpv2 | softvfp+vfpv2"
argument_list|)
expr_stmt|;
name|desc
argument_list|(
...
literal|"-no-s60"
argument_list|,
literal|"Do not compile in S60 support."
argument_list|)
expr_stmt|;
name|desc
argument_list|(
...
literal|"-s60"
argument_list|,
literal|"Compile with support for the S60 UI Framework"
argument_list|)
expr_stmt|;
name|desc
argument_list|(
...
literal|"-no-usedeffiles"
argument_list|,
literal|"Disable the usage of DEF files."
argument_list|)
expr_stmt|;
name|desc
argument_list|(
...
literal|"-usedeffiles"
argument_list|,
literal|"Enable the usage of DEF files.\n"
argument_list|)
expr_stmt|;
return|return
literal|true
...
literal|false
expr_stmt|;
comment|// In Symbian we only support system sqlite option
else|else
name|available
operator|=
literal|true
...
condition|)
block|{
comment|// Symbian has multiple .lib/.dll files we need to find
if|if
condition|(
name|dictionary
operator|.
...
literal|true
expr_stmt|;
comment|// There is sqlite_symbian plugin which exports the necessary stuff
name|dictionary
index|[
...
name|paths
operator|<<
literal|"epoc32/release/armv5/lib/mmfdevsound.dso"
operator|<<
literal|"epoc32/release/armv5/lib/mmfdevsound.lib"
operator|<<
literal|"epoc32/release/winscw/udeb/mmfdevsound.dll"
operator|<<
literal|"epoc32/release/winscw/udeb/mmfdevsound.lib"
operator|<<
literal|"epoc32/include/mmf/server/sounddevice.h"
expr_stmt|;
name|QStringList
operator|::
name|iterator
...
name|endl
operator|<<
literal|"The QtMultimedia audio backend will not be built because required"
operator|<<
name|endl
operator|<<
literal|"support for CMMFDevSound was not found in the SDK."
operator|<<
name|endl
operator|<<
literal|"The SDK which was examined was located at the following path:"
operator|<<
name|endl
operator|<<
literal|"    "
...
name|endl
operator|<<
literal|"The following required files were missing from the SDK:"
operator|<<
name|endl
expr_stmt|;
name|QString
...
name|endl
operator|<<
literal|"The SDK root was determined to be '"
operator|<<
name|epocRoot
operator|<<
literal|"'."
...
name|endl
operator|<<
literal|"This directory was not found, so the SDK could not be checked for"
operator|<<
name|endl
operator|<<
literal|"CMMFDevSound support.  The QtMultimedia audio backend will therefore"
operator|<<
name|endl
operator|<<
literal|"not be built."
operator|<<
name|endl
operator|<<
name|endl
...
name|QString
argument_list|(
literal|"OPENSSL_LIBS    = -llibssl -llibcrypto"
argument_list|)
expr_stmt|;
else|else
name|qmakeVars
...
name|cacheStream
operator|<<
literal|"QMAKE_UIC3      = $$QT_BUILD_TREE"
operator|<<
name|fixSeparators
argument_list|(
literal|"/bin/uic3.exe"
argument_list|,
literal|true
argument_list|)
operator|<<
...
name|qconfigList
operator|+=
literal|"QT_NO_PRINTER"
expr_stmt|;
name|qconfigList
operator|+=
literal|"QT_NO_SYSTEMTRAYICON"
expr_stmt|;
if|if
condition|(
name|dictionary
...
name|cout
operator|<<
literal|"Symbian DEF files enabled..."
operator|<<
name|dictionary
index|[
literal|"SYMBIAN_DEFFILES"
...
name|cout
operator|<<
literal|"WARNING: Disabling DEF files will mean that Qt is NOT binary compatible with previous versions."
operator|<<
name|endl
expr_stmt|;
name|cout
operator|<<
literal|"         This feature is only intended for use during development, NEVER for release builds."
operator|<<
name|endl
expr_stmt|;
block|}
...
block|{
comment|// Compiling for Symbian OS
name|cout
operator|<<
name|endl
operator|<<
