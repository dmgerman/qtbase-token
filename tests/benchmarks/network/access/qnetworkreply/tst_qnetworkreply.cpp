...
include|#
directive|include
file|<QtNetwork/private/qhostinfo_p.h>
end_include
begin_include
include|#
directive|include
...
argument_list|)
expr_stmt|;
comment|// make sure we have a full request including
comment|// DNS lookup, TCP and SSL handshakes
ifdef|#
directive|ifdef
name|QT_BUILD_INTERNAL
...
name|qWarning
argument_list|(
literal|"no internal build, could not clear DNS cache. Results may not be representative."
argument_list|)
expr_stmt|;
endif|#
directive|endif
comment|// first, benchmark a normal request
name|QPair
argument_list|<
name|QNetworkReply
modifier|*
...
name|second
decl_stmt|;
comment|// clear all caches again
ifdef|#
directive|ifdef
name|QT_BUILD_INTERNAL
name|qt_qhostinfo_clear_cache
...
argument_list|()
operator|<<
literal|"full request:"
operator|<<
name|normalElapsed
operator|<<
literal|"ms, pre-connect request:"
operator|<<
name|preConnectElapsed
operator|<<
literal|"ms, difference:"
operator|<<
operator|(
name|normalElapsed
operator|-
...
argument_list|>
argument_list|(
literal|"sslConfiguration"
argument_list|)
expr_stmt|;
comment|// start a new normal request after preconnecting is done
name|QTest
operator|::
name|newRow
argument_list|(
...
argument_list|()
expr_stmt|;
comment|// start a new normal request while preconnecting is in-flight
name|QTest
operator|::
name|newRow
argument_list|(
...
argument_list|)
comment|// start a new SPDY request while preconnecting is done
name|QTest
operator|::
name|newRow
argument_list|(
...
name|spdySslConf
expr_stmt|;
comment|// start a new SPDY request while preconnecting is in-flight
name|QTest
operator|::
name|newRow
argument_list|(
...
argument_list|)
expr_stmt|;
comment|//    for (int a = 0; a< 10; ++a)
comment|//        runHttpsUploadRequest(data, request); // to warmup all TCP connections
name|QBENCHMARK
block|{
name|runHttpsUploadRequest
argument_list|(
...
decl_stmt|;
comment|// make sure we have a full request including
comment|// DNS lookup and TCP handshake
ifdef|#
directive|ifdef
name|QT_BUILD_INTERNAL
name|qt_qhostinfo_clear_cache
