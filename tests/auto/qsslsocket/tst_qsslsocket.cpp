...
directive|ifdef
name|Q_OS_SYMBIAN
comment|// I don't understand how socket->waitForDisconnected can work on other platforms
comment|// since socket->write will end to:
comment|//   QMetaObject::invokeMethod(this, "_q_flushWriteBuffer", Qt::QueuedConnection);
comment|// In order that _q_flushWriteBuffer will be called the eventloop need to run
comment|// If we just call waitForDisconnected, which blocks the whole thread how that can happen?
name|connect
argument_list|(
name|socket
argument_list|,
