...
literal|"native"
argument_list|,
literal|"Default settings on Mac/WinRT are valid, despite organization domain, name, and app name being null"
argument_list|,
name|Continue
argument_list|)
expr_stmt|;
...
name|QSKIP
argument_list|(
literal|"This test is specific to the Windows registry."
argument_list|,
name|SkipAll
argument_list|)
expr_stmt|;
...
name|key
argument_list|(
literal|"HKEY_LOCAL_MACHINE\\Software"
argument_list|)
decl_stmt|;
specifier|const
name|QString
name|keyWow
argument_list|(
literal|"HKEY_LOCAL_MACHINE\\Software\\Wow6432Node"
argument_list|)
decl_stmt|;
ifndef|#
directive|ifndef
name|Q_OS_WIN64
comment|// This branch is taken at compile time if targeting 32-bit; it does not
comment|// necessarily mean that the OS running the test is 32-bit (it could be
comment|// e.g. 64-bit).
name|QCOMPARE
argument_list|(
name|QSettings
...
argument_list|)
expr_stmt|;
comment|// Detect whether we are running under 64-bit Windows.
typedef|typedef
name|BOOL
function_decl|(
name|WINAPI
modifier|*
name|IsWow64ProcessPtr
function_decl|)
parameter_list|(
name|HANDLE
name|hProcess
...
condition|)
block|{
comment|// The 64-bit registry's "Wow6432Node" key should match the 32-bit registry.
comment|// If we are not on 32-bit Windows, these should never be the same,
comment|// because the 64-bit registry has a "Wow6432Node" key.
name|QCOMPARE
argument_list|(
name|QSettings
argument_list|(
...
directive|else
comment|// This branch is taken at compile time if targeting 64-bit; it does not
comment|// necessarily mean that the OS running the test is 64-bit (it could be
comment|// e.g. 128-bit).
name|QCOMPARE
argument_list|(
name|QSettings
...
name|s
argument_list|(
literal|"tst_QSettings_trailingWhitespace"
argument_list|)
decl_stmt|;
name|s
operator|.
...
argument_list|)
expr_stmt|;
comment|// Sandboxed WinRT applications cannot write into the
comment|// application directory. Hence reset the current
comment|// directory
ifdef|#
directive|ifdef
name|Q_OS_WINRT
