...
function_decl|;
name|void
name|setStandardOutputFileNullDevice
parameter_list|()
function_decl|;
name|void
name|setStandardOutputFileAndWaitForBytesWritten
parameter_list|()
function_decl|;
name|void
...
function_decl|;
name|void
name|startStopStartStop
parameter_list|()
function_decl|;
name|void
name|startStopStartStopBuffers_data
parameter_list|()
function_decl|;
name|void
name|startStopStartStopBuffers
parameter_list|()
function_decl|;
name|void
name|processEventsInAReadyReadSlot_data
parameter_list|()
function_decl|;
name|void
name|processEventsInAReadyReadSlot
parameter_list|()
function_decl|;
...
name|Q_OS_QNX
argument_list|)
comment|// QNX may lose the kill if it's delivered while the forked process
comment|// is doing the exec that morphs it into testProcessEcho.  It's very
comment|// unlikely that a normal application would do such a thing.  Make
comment|// sure the test doesn't accidentally try to do it.
name|proc
operator|.
name|write
argument_list|(
...
specifier|const
name|QString
name|reversedCommand
init|=
literal|"lamroNssecorPtset/lamroNssecorPtset"
decl_stmt|;
name|QProcess
name|process
decl_stmt|;
...
name|newRow
argument_list|(
literal|"separate with waitForBytesWritten"
argument_list|)
operator|<<
literal|false
operator|<<
...
parameter_list|()
block|{
comment|// we actually do start-stop x 3 :-)
name|QProcess
name|process
decl_stmt|;
name|process
...
name|start
argument_list|(
literal|"testProcessHang/testProcessHang"
argument_list|)
expr_stmt|;
name|QVERIFY2
argument_list|(
...
expr_stmt|;
block|}
comment|// We want to test that the write buffer still has bytes after the child
comment|// exiting. We do that by writing to a child process that never reads. We
comment|// just have to write more data than a pipe can hold, so that even if
comment|// QProcess finds the pipe writable (during waitForFinished() or in the
comment|// QWindowsPipeWriter thread), some data will remain. The worst case I know
comment|// of is Linux, which defaults to 64 kB of buffer.
name|process
operator|.
name|write
argument_list|(
...
directive|ifndef
name|Q_OS_WIN
comment|// confirm that our buffers are still full
comment|// Note: this doesn't work on Windows because our buffers are drained into
comment|// QWindowsPipeWriter before being sent to the child process.
name|QVERIFY
argument_list|(
name|process
operator|.
...
argument_list|)
expr_stmt|;
comment|// channelMode1 is not ForwardedChannels
if|if
condition|(
name|channelMode1
operator|==
...
argument_list|)
expr_stmt|;
comment|// the buffers should now be empty
name|QCOMPARE
argument_list|(
name|process
operator|.
...
parameter_list|()
block|{
comment|// Test whether processing events in a readyReadXXX slot crashes. (QTBUG-48697)
name|QFETCH
argument_list|(
name|bool
argument_list|,
name|callWaitForReadyRead
argument_list|)
expr_stmt|;
name|QProcess
name|process
