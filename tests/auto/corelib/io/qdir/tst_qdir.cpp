...
name|QT_BUILD_INTERNAL
name|void
name|normalizePathSegments_data
parameter_list|()
function_decl|;
name|void
...
name|qWarning
argument_list|(
literal|"Couldn't set current path to %s"
argument_list|,
name|qPrintable
argument_list|(
name|m_dataPath
...
name|qPrintable
argument_list|(
literal|"Did not find testdata. Is this builtin?"
argument_list|)
argument_list|)
expr_stmt|;
name|m_dataPath
...
directive|ifdef
name|BUILTIN_TESTDATA
comment|// We need to reset the current directory outside of QTemporaryDir for successful deletion
name|QDir
operator|::
name|setCurrent
argument_list|(
...
comment|// Remove the directory and create a file with the same path
name|QDir
operator|::
name|current
argument_list|()
...
argument_list|)
expr_stmt|;
comment|// calling mkdir on an existing file will fail.
name|QVERIFY
argument_list|(
operator|!
name|QDir
...
argument_list|)
expr_stmt|;
comment|// calling mkpath on an existing file will fail.
name|f
operator|.
name|remove
argument_list|()
...
name|newRow
argument_list|(
literal|"cdUp non existent (relative dir)"
argument_list|)
operator|<<
...
name|newRow
argument_list|(
literal|"cdUp non existent (absolute dir)"
argument_list|)
operator|<<
name|m_dataPath
operator|+
...
literal|"ab/a"
expr_stmt|;
comment|// Path item with length of 2
ifdef|#
directive|ifdef
name|Q_OS_WIN
name|QTest
...
literal|"c:/foo"
expr_stmt|;
comment|// Drive letters and unc path in one string
ifndef|#
directive|ifndef
name|Q_OS_WINRT
ifdef|#
...
expr_stmt|;
comment|// Path item with length of 2
comment|// Drive letters and unc path in one string. The drive letter isn't handled as a drive letter
comment|// but as a host name in this case (even though Windows host names can't contain a ':')
name|QTest
operator|::
name|newRow
argument_list|(
...
argument_list|)
argument_list|,
literal|"Strings are same but data is not shared"
argument_list|)
expr_stmt|;
block|}
end_function
...
name|_WRS_KERNEL
argument_list|)
comment|// unsetenv is not available on VxWorks DKM mode
name|unsetenv
argument_list|(
literal|"HOME"
argument_list|)
...
name|QSKIP
argument_list|(
literal|"WinRT has no concept of drives"
argument_list|)
expr_stmt|;
endif|#
directive|endif
...
name|QSKIP
argument_list|(
literal|"WinRT has no concept of system root"
argument_list|)
expr_stmt|;
endif|#
directive|endif
