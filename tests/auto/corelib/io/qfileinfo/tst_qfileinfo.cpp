...
argument_list|)
expr_stmt|;
comment|// Release temporary directory so that it can be deleted on Windows
block|}
end_function
begin_comment
...
literal|true
expr_stmt|;
comment|// Skip for the WinRT case, as GetFileAttributesEx removes _any_
comment|// trailing whitespace and "." is a valid entry as seen in data10
ifndef|#
directive|ifndef
name|Q_OS_WINRT
name|QTest
...
literal|""
argument_list|,
literal|"WinRT does not allow timestamp handling change in the filesystem due to sandboxing"
argument_list|,
name|Continue
argument_list|)
expr_stmt|;
...
literal|""
argument_list|,
literal|"Android may use relatime or noatime on mounts"
argument_list|,
name|Continue
argument_list|)
expr_stmt|;
...
argument_list|()
decl_stmt|;
comment|// Ensure that junctions, mountpoints are removed. If this fails, do not remove
comment|// temporary directory to prevent it from trashing the system.
if|if
condition|(
name|fi
operator|.
...
name|qWarning
argument_list|(
literal|"Unable to remove NTFS junction '%s'', keeping '%s'."
argument_list|,
name|qPrintable
argument_list|(
name|fi
