...
argument_list|()
expr_stmt|;
comment|// QTBUG-39976, file mappings should be cleared as well.
name|QVERIFY
argument_list|(
name|file
operator|.
...
directive|endif
comment|// File might not exist locally in case of sandboxing or remote testing
if|if
condition|(
operator|!
name|nativeFilePath
...
name|takenFileName
decl_stmt|;
comment|// First pass. See which filename QTemporaryFile will try first.
block|{
comment|// Fix the random seed.
name|qsrand
argument_list|(
literal|1135
argument_list|)
...
argument_list|)
expr_stmt|;
comment|// Create a directory with same name.
name|QVERIFY
argument_list|(
name|dir
operator|.
...
argument_list|)
expr_stmt|;
comment|// Second pass, now we have blocked its first attempt with a directory.
block|{
comment|// Fix the random seed.
name|qsrand
argument_list|(
