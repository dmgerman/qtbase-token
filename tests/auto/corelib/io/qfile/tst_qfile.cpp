...
parameter_list|)
block|{
comment|//connect file server first time
if|if
condition|(
operator|!
name|rfs_
...
literal|false
return|;
comment|//symbian does not like ./ in filenames, so open by absolute path
name|QString
name|fileName
argument_list|(
name|QDir
...
parameter_list|()
block|{
comment|// clean up the files we created
name|QFile
operator|::
name|remove
argument_list|(
...
name|QSKIP
argument_list|(
literal|"Symbian does not support non-readable files"
argument_list|,
name|SkipSingle
argument_list|)
expr_stmt|;
...
name|QSKIP
argument_list|(
literal|"Currently no stdin/out supported for Windows CE or Symbian"
argument_list|,
name|SkipAll
argument_list|)
expr_stmt|;
...
name|QSKIP
argument_list|(
literal|"Symbian does not have execution permissions"
argument_list|,
name|SkipSingle
argument_list|)
expr_stmt|;
...
name|QSKIP
argument_list|(
literal|"Symbian does not support links"
argument_list|,
name|SkipAll
argument_list|)
expr_stmt|;
...
name|QSKIP
argument_list|(
literal|"Symbian does not support linking to directories"
argument_list|,
name|SkipAll
argument_list|)
expr_stmt|;
...
literal|""
argument_list|,
literal|"Symlinking using relative paths is currently different on Windows and Unix/Symbian"
argument_list|,
name|Continue
argument_list|)
expr_stmt|;
...
name|Q_OS_SYMBIAN
argument_list|)
comment|// In Symbian can't link to nonexisting file directly, so create the file temporarily
name|QFile
name|tempFile
argument_list|(
literal|"file12"
...
literal|1024
decl_stmt|;
comment|// WinCE and Symbian do not have much space
else|#
directive|else
name|int
name|resizeSize
...
argument_list|()
expr_stmt|;
comment|/* Guess what, we have to rename it back, otherwise we'll fail on second      * invocation. */
name|QVERIFY
argument_list|(
name|QFile
operator|::
...
name|QSKIP
argument_list|(
literal|"test assumptions invalid for symbian"
argument_list|,
name|SkipAll
argument_list|)
expr_stmt|;
...
literal|false
condition|)
comment|// No permissions for user makes no sense in Symbian
elif|#
directive|elif
name|defined
argument_list|(
...
name|owndrive
operator|+
literal|"/resource/apps/tst_qfile.rsc"
operator|<<
literal|true
operator|<<
name|amiprivileged
...
name|newRow
argument_list|(
literal|"other private"
argument_list|)
operator|<<
name|owndrive
operator|+
literal|"/private/10003a3f/import/apps/tst_qfile_reg.rsc"
operator|<<
name|amiprivileged
operator|<<
name|amiprivileged
...
expr_stmt|;
block|}
comment|//append mode used to avoid truncating the files.
block|{
name|QFile
name|f
argument_list|(
...
directive|ifdef
name|Q_OS_SYMBIAN
comment|// these tests are a bit different, because using a closed file handle results in a panic rather than error
block|{
name|QFile
name|file
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// file is opened with mandatory lock, so opening again should fail
name|QVERIFY
argument_list|(
operator|!
name|file2
...
argument_list|()
expr_stmt|;
comment|// opening again should now succeed (because handle is closed)
name|QVERIFY
argument_list|(
name|file2
operator|.
...
argument_list|()
expr_stmt|;
comment|// opening again should still fail (because handle is not auto closed)
name|QVERIFY
argument_list|(
operator|!
name|file2
