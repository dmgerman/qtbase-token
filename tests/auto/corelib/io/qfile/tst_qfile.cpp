...
class|class
name|StdinReaderProcessGuard
block|{
comment|// Ensure the stdin reader process is stopped on destruction.
name|Q_DISABLE_COPY
parameter_list|(
name|StdinReaderProcessGuard
parameter_list|)
...
literal|"ReadOnly + MapPrivate"
argument_list|,
literal|"Windows CE does not support MapPrivateOption."
argument_list|,
name|Abort
argument_list|)
expr_stmt|;
...
index|]
init|=
literal|"Some data padded with nulls\n"
decl_stmt|;
name|QFETCH
argument_list|(
name|int
...
argument_list|()
expr_stmt|;
comment|// test that we can read the data we've just written, without closing the file
name|uchar
modifier|*
name|memory
init|=
