...
function_decl|;
name|void
name|tryLockNegative_data
parameter_list|()
function_decl|;
name|void
name|tryLockNegative
parameter_list|()
function_decl|;
name|void
name|moreStress
...
parameter_list|()
block|{
comment|// the documentation says tryLock() with a negative number is the same as lock()
struct|struct
name|TrylockThread
super|:
name|QThread
...
argument_list|)
expr_stmt|;
comment|// after we unlock the mutex, the thread should succeed in locking, then
comment|// unlock and exit. Do this before more tests to avoid deadlocking due to
comment|// ~QThread waiting forever on a thread that won't exit.
name|mutex
operator|.
name|unlock
argument_list|()
