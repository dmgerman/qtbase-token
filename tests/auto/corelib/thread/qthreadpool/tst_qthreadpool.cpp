...
argument_list|)
expr_stmt|;
comment|//one which will run and one which will not
name|threadPool
operator|.
name|cancel
argument_list|(
...
argument_list|)
expr_stmt|;
comment|//verify NOOP for jobs not in the queue
name|threadPool
operator|.
name|start
argument_list|(
...
literal|0
expr_stmt|;
comment|//valgrind will catch this if cancel() is crazy enough to delete currently running jobs
name|runnables
index|[
name|runs
operator|-
...
index|]
expr_stmt|;
comment|//if the pool deletes them then we'll get double-free crash
operator|delete
name|runnables
index|[
name|runs
