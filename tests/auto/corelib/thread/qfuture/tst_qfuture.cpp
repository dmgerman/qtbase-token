...
argument_list|)
expr_stmt|;
comment|// pool is busy (meaning our tasks did end up executing there)
name|QVERIFY
argument_list|(
name|pool
operator|.
...
argument_list|)
expr_stmt|;
comment|// max sleep time in UselessTask::run is 150ms, so 10s should be enough
comment|// (and the call returns as soon as all tasks finished anyway, so the
comment|// maximum wait time only matters when the test fails)
name|Q_FOREACH
argument_list|(
argument|const QFuture<int>&future
argument_list|,
argument|futures
argument_list|)
block|{
