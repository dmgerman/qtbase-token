...
parameter_list|)
define|\
value|numSeen = 0; \         containerIter = intList.begin(); \         for (QVariant v : listIter) { \             QVERIFY(ContainerAPI<CONTAINER<VALUE_TYPE>>::compare(v, *containerIter)); \             ++containerIter; \             ++numSeen; \         } \         QCOMPARE(numSeen, (int)std::distance(intList.begin(), intList.end()));
else|#
directive|else
define|#
directive|define
...
parameter_list|)
define|\
value|{ \         int numSeen = 0; \         CONTAINER<VALUE_TYPE> intList; \         ContainerAPI<CONTAINER<VALUE_TYPE>>::insert(intList, 1); \         ContainerAPI<CONTAINER<VALUE_TYPE>>::insert(intList, 2); \         ContainerAPI<CONTAINER<VALUE_TYPE>>::insert(intList, 3); \         \         QVariant listVariant = QVariant::fromValue(intList); \         QVERIFY(listVariant.canConvert<QVariantList>()); \         QSequentialIterable listIter = listVariant.value<QSequentialIterable>(); \         \         CONTAINER<VALUE_TYPE>::iterator containerIter = intList.begin(); \         const CONTAINER<VALUE_TYPE>::iterator containerEnd = intList.end(); \         for (int i = 0; i< listIter.size(); ++i, ++containerIter, ++numSeen) \         { \             QVERIFY(ContainerAPI<CONTAINER<VALUE_TYPE>>::compare(listIter.at(i), *containerIter)); \         } \         QCOMPARE(numSeen, (int)std::distance(intList.begin(), intList.end())); \         QCOMPARE(containerIter, containerEnd); \         \         containerIter = intList.begin(); \         numSeen = 0; \         Q_FOREACH (const QVariant&v, listIter) { \             QVERIFY(ContainerAPI<CONTAINER<VALUE_TYPE>>::compare(v, *containerIter)); \             ++containerIter; \             ++numSeen; \         } \         QCOMPARE(numSeen, (int)std::distance(intList.begin(), intList.end())); \         TEST_RANGE_FOR(CONTAINER, VALUE_TYPE) \     }
