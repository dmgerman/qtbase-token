...
condition|)
block|{
comment|// This code is currently unreachable because it's only used by the
comment|// template-based versions of connect() and disconnect(), which don't
comment|// work with dynamically generated meta-objects (see test).
name|int
modifier|*
name|result
init|=
...
name|qFatal
argument_list|(
literal|"You forgot to add one or more IndexOfMethod cases"
argument_list|)
expr_stmt|;
block|}
block|}
...
name|QtWarningMsg
argument_list|,
literal|"QObject::connect: signal not found in QObject"
argument_list|)
expr_stmt|;
name|QMetaObject
operator|::
...
literal|""
argument_list|,
literal|"template-based connect() fails because meta-object is deduced at compile-time"
argument_list|,
name|Abort
argument_list|)
expr_stmt|;
