...
define|#
directive|define
name|TEST_RUNNING_CHANGED
parameter_list|(
name|RUNNING
parameter_list|)
define|\
value|{ \     QTRY_COMPARE(runningSpy.count(), 1); \     QList<QVariant> runningArgs = runningSpy.takeFirst(); \     QVERIFY(runningArgs.at(0).type() == QVariant::Bool); \     QVERIFY(runningArgs.at(0).toBool() == RUNNING); \     QCOMPARE(machine.isRunning(), runningArgs.at(0).toBool()); \ }
end_define
begin_define
DECL|macro|TEST_RUNNING_CHANGED_STARTED_STOPPED
define|#
directive|define
name|TEST_RUNNING_CHANGED_STARTED_STOPPED
define|\
value|{ \     QTRY_COMPARE(runningSpy.count(), 2); \     QList<QVariant> runningArgs = runningSpy.takeFirst(); \     QVERIFY(runningArgs.at(0).type() == QVariant::Bool); \     QVERIFY(runningArgs.at(0).toBool() == true); \     runningArgs = runningSpy.takeFirst(); \     QVERIFY(runningArgs.at(0).type() == QVariant::Bool); \     QVERIFY(runningArgs.at(0).toBool() == false); \     QCOMPARE(machine.isRunning(), runningArgs.at(0).toBool()); \ }
end_define
begin_define
DECL|macro|DEFINE_ACTIVE_SPY
define|#
directive|define
name|DEFINE_ACTIVE_SPY
parameter_list|(
name|VAR
parameter_list|)
define|\
value|QSignalSpy VAR##_activeSpy(VAR,&QState::activeChanged); \     QVERIFY(VAR##_activeSpy.isValid());
end_define
begin_define
DECL|macro|TEST_ACTIVE_CHANGED
define|#
directive|define
name|TEST_ACTIVE_CHANGED
parameter_list|(
name|VAR
parameter_list|,
name|COUNT
parameter_list|)
define|\
value|{ \     QTRY_COMPARE(VAR##_activeSpy.count(), COUNT); \     bool active = true; \     foreach (const QList<QVariant>&activeArgs, static_cast<QList<QList<QVariant>>>(VAR##_activeSpy)) { \         QVERIFY(activeArgs.at(0).type() == QVariant::Bool); \         QVERIFY(activeArgs.at(0).toBool() == active); \         active = !active; \     } \     QCOMPARE(VAR->active(), !active); \ }
end_define
begin_class
...
name|void
name|createPointerToMemberSignalTransition
parameter_list|()
function_decl|;
endif|#
directive|endif
...
function_decl|;
name|void
name|conflictingTransition
parameter_list|()
function_decl|;
name|void
name|conflictingTransition2
parameter_list|()
function_decl|;
name|void
name|qtbug_46059
...
function_decl|;
name|void
name|postEventFromBeginSelectTransitions
parameter_list|()
function_decl|;
block|}
class|;
...
name|setObjectName
argument_list|(
literal|"initialState->historyState"
argument_list|)
expr_stmt|;
name|initialState
operator|->
...
parameter_list|()
block|{
comment|// This test checks if the parallel state is exited and re-entered if one compound state
comment|// is exited and subsequently re-entered. When the parallel state is exited, the other compound
comment|// state in the parallel state has to be exited too. When the parallel state is re-entered, the
comment|// other state also needs to be re-entered.
name|QStateMachine
name|machine
decl_stmt|;
name|QState
...
argument_list|)
expr_stmt|;
comment|// The following transition will exit s1 (which means that parallelState is also exited), and
comment|// subsequently re-entered (which means that parallelState is also re-entered).
name|EventTransition
modifier|*
name|et
init|=
...
argument_list|()
expr_stmt|;
comment|// Initial entrance of the parallel state and its sub-states:
name|TEST_ACTIVE_CHANGED
argument_list|(
name|parallelState
argument_list|,
...
argument_list|)
expr_stmt|;
comment|// initial + exit due to parent exit + entry due to parent re-entry
name|TEST_ACTIVE_CHANGED
argument_list|(
name|s2InitialChild
argument_list|,
...
argument_list|)
expr_stmt|;
comment|// initial + exit due to parent exit + re-entry due to parent re-entry
name|TEST_ACTIVE_CHANGED
argument_list|(
name|s1OtherChild
argument_list|,
...
argument_list|)
expr_stmt|;
comment|// entry due to transition
name|QVERIFY
argument_list|(
name|machine
operator|.
...
argument_list|)
expr_stmt|;
comment|// Check that s1InitialChild is not in the configuration, because although s1 is re-entered,
comment|// another child state (s1OtherChild) is active, so the initial child should not be activated.
name|QVERIFY
argument_list|(
name|machine
operator|.
...
argument_list|()
expr_stmt|;
comment|// Only one of the following two can be true, because the two possible transitions conflict.
if|if
condition|(
name|machine
operator|.
...
condition|)
block|{
comment|// the transition p2 -> s1 was taken, not p1_1 -> p1_2, so:
comment|// the parallel state exited, so none of the states inside it are active
name|QTRY_COMPARE
argument_list|(
name|machine
operator|.
...
block|{
comment|// the transition p1_1 -> p1_2 was taken, not p2 -> s1, so:
comment|// the parallel state was not exited and the state is the same as the start state with one
comment|// difference: p1_1 inactive and p1_2 active:
name|QTRY_COMPARE
argument_list|(
name|machine
operator|.
...
argument_list|)
expr_stmt|;
comment|// external transitions will return 3, internal transitions should return 1.
name|TEST_ACTIVE_CHANGED
argument_list|(
name|s
argument_list|,
...
argument_list|)
expr_stmt|;
comment|// t3 got preempted by t2
name|QVERIFY
argument_list|(
name|machine
operator|.
