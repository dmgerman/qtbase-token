...
block|,
literal|"text-x-objcsrc.xml"
block|,
literal|"invalid-magic1.xml"
block|,
literal|"invalid-magic2.xml"
block|,
literal|"invalid-magic3.xml"
block|,
literal|0
block|}
decl_stmt|;
...
ifdef|#
directive|ifdef
name|USE_XDG_DATA_DIRS
comment|// Create a temporary "global" XDG data dir for later use
comment|// It will initially contain a copy of freedesktop.org.xml
name|QVERIFY2
argument_list|(
name|m_temporaryDir
operator|.
...
parameter_list|()
block|{
comment|// clean up local data from previous runs
name|QDir
argument_list|(
name|m_localMimeDir
argument_list|)
...
name|QUrl
argument_list|(
literal|"mailto:something@example.com"
argument_list|)
argument_list|)
operator|.
name|name
...
name|QUrl
argument_list|(
literal|"mailto:something@polish.pl"
argument_list|)
argument_list|)
operator|.
name|name
...
argument_list|)
expr_stmt|;
comment|// Now make a symlink to the fifo
specifier|const
name|QString
name|link
init|=
...
name|QSKIP
argument_list|(
literal|"This test requires pipes and symlinks"
argument_list|)
expr_stmt|;
endif|#
directive|endif
...
specifier|static
name|void
name|ignoreInvalidMimetypeWarnings
parameter_list|(
specifier|const
name|QString
modifier|&
...
argument_list|,
operator|(
literal|"QMimeDatabase: Error parsing "
operator|+
name|basePath
operator|+
literal|"invalid-magic1.xml\nInvalid magic rule value \"foo\""
operator|)
operator|.
name|constData
argument_list|()
...
name|basePath
operator|+
literal|"invalid-magic3.xml\nInvalid magic rule mask size \"0xffff\""
operator|)
operator|.
name|constData
argument_list|()
...
name|QSKIP
argument_list|(
literal|"This test requires XDG_DATA_DIRS"
argument_list|)
expr_stmt|;
endif|#
directive|endif
...
argument_list|)
expr_stmt|;
comment|// ensure we can access the empty glob list
block|{
name|QMimeType
name|objcsrc
init|=
...
expr_stmt|;
block|}
comment|// Now test removing the mimetype definitions again
for|for
control|(
name|int
name|i
...
argument_list|)
expr_stmt|;
comment|// These mimetypes have invalid magic, but still do exist.
name|QVERIFY
argument_list|(
name|db
operator|.
