...
function_decl|;
name|void
name|changeSourceLayoutFilteredOut
parameter_list|()
function_decl|;
name|void
...
function_decl|;
name|void
name|changeSourceDataKeepsStableSorting_qtbug1548
parameter_list|()
function_decl|;
name|void
name|changeSourceDataForwardsRoles_qtbug35440
parameter_list|()
function_decl|;
name|void
name|resortingDoesNotBreakTreeModels
parameter_list|()
function_decl|;
name|void
name|sortFilterRole
...
name|Q_DECLARE_METATYPE
argument_list|(
argument|QAbstractItemModel::LayoutChangeHint
argument_list|)
end_macro
begin_comment
...
argument_list|)
expr_stmt|;
comment|// make sure the filter does not match any entry
name|proxy
operator|.
name|setFilterRegExp
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// without fix for QTBUG-30662 this will make all entries visible - but unsorted
name|proxy
operator|.
name|setFilterRegExp
argument_list|(
...
argument_list|)
decl_stmt|;
comment|// Remove filter; we expect an insert
name|proxy
operator|.
name|setFilterRegExp
argument_list|(
...
end_function
begin_comment
comment|// Checks that the model is a table, and that each and every row is like this:
end_comment
begin_comment
comment|// i-th row:   ( rows.at(i), i )
end_comment
begin_function
DECL|function|checkSortedTableModel
specifier|static
name|void
name|checkSortedTableModel
parameter_list|(
specifier|const
name|QAbstractItemModel
modifier|*
...
name|QSKIP
argument_list|(
literal|"This test will fail, see QTBUG-1548"
argument_list|)
expr_stmt|;
comment|// Check that emitting dataChanged from the source model
comment|// for a change of a role which is not the sorting role
comment|// doesn't alter the sorting. In this case, we sort on the DisplayRole,
comment|// and play with other roles.
specifier|static
specifier|const
name|QStringList
name|rows
...
literal|"x"
decl_stmt|;
comment|// Build a table of pairs (string, #row) in each row
name|QStandardItemModel
name|model
argument_list|(
literal|0
...
argument_list|)
expr_stmt|;
comment|// The proxy is now sorted by the first column, check that the sorting
comment|// * is correct (the input is already sorted, so it must not have changed)
comment|// * was stable (by looking at the second column)
name|checkSortedTableModel
argument_list|(
operator|&
name|model
...
argument_list|)
expr_stmt|;
comment|// Change the check status of an item. That must not break the stable sorting
comment|// changes the middle "b"
name|model
operator|.
name|item
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// changes the starting "a"
name|model
operator|.
name|item
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// change the background color of the first "c"
name|model
operator|.
name|item
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// change the background color of the second "c"
name|model
operator|.
name|item
argument_list|(
...
name|items
decl_stmt|;
comment|// Stable sorting: Items with invalid data should move to the end
name|items
operator|<<
operator|new
name|QStandardItem
...
end_function
begin_comment
comment|// A source model with ABABAB rows, where only A rows accept drops.
end_comment
begin_comment
comment|// It will then be sorted by a QSFPM.
end_comment
begin_class
...
end_function
begin_comment
comment|// QTBUG-39549, test whether canDropMimeData(), dropMimeData() are proxied as well
end_comment
begin_comment
comment|// by invoking them on a QSortFilterProxyModel proxying a QStandardItemModel that allows drops
end_comment
begin_comment
comment|// on row #1, filtering for that row.
end_comment
begin_class
...
parameter_list|()
block|{
comment|// Given a source model which only supports dropping on even rows
name|DropOnOddRows
name|sourceModel
decl_stmt|;
name|QCOMPARE
...
argument_list|)
expr_stmt|;
comment|// and a proxy model that sorts the rows
name|QSortFilterProxyModel
name|proxy
decl_stmt|;
name|proxy
...
argument_list|)
expr_stmt|;
comment|// the proxy should correctly map canDropMimeData to the source model,
comment|// i.e. accept drops on the first 5 rows and refuse drops on the next 5.
for|for
control|(
name|int
name|row
