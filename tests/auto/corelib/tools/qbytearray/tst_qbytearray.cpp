...
end_class
begin_comment
comment|// Except for QByteArrays initialized with fromRawData(), QByteArray ensures
end_comment
begin_comment
comment|// that data() is null-terminated. VERIFY_ZERO_TERMINATION checks that
end_comment
begin_comment
comment|// invariant and goes further by testing that the null-character is in writable
end_comment
begin_comment
comment|// memory allocated by QByteArray. If the invariant is broken, tools like
end_comment
begin_comment
comment|// valgrind should be able to detect this.
end_comment
begin_define
DECL|macro|VERIFY_ZERO_TERMINATION
define|#
directive|define
name|VERIFY_ZERO_TERMINATION
parameter_list|(
name|ba
parameter_list|)
define|\
value|do {                                                                        \
comment|/* Statics could be anything, as can fromRawData() strings */
value|\         QByteArray::DataPtr baDataPtr = ba.data_ptr();                          \         if (!baDataPtr->ref.isStatic()                                          \&& baDataPtr->offset == QByteArray().data_ptr()->offset) {      \             int baSize = ba.size();                                             \             QCOMPARE(ba.constData()[baSize], '\0');                             \                                                                                 \             QByteArray baCopy(ba.constData(), baSize);                          \             if (!baDataPtr->ref.isShared()) {                                   \
comment|/* Don't detach, assumes no setSharable support */
value|\                 char *baData = ba.data();                                       \                 baData[baSize] = 'x';                                           \                                                                                 \                 QCOMPARE(ba.constData()[baSize], 'x');                          \                 QCOMPARE(ba, baCopy);                                           \                                                                                 \                 baData[baSize] = '\0';
comment|/* Sanity restored */
value|\             }                                                                   \         }                                                                       \     } while (false)
end_define
begin_comment
unit|\
