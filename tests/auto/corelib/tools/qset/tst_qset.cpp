...
argument_list|)
expr_stmt|;
comment|// first inserted is kept
name|QCOMPARE
argument_list|(
name|set
operator|.
...
argument_list|)
expr_stmt|;
comment|// check _which_ of the equal elements gets inserted (in the QHash/QMap case, it's the last):
specifier|const
name|QSet
argument_list|<
name|IdentityTracker
...
parameter_list|()
block|{
comment|// only restore when no-one else changed the seed in the meantime:
name|qt_qhash_seed
operator|.
...
block|{
comment|//
comment|// check that sets containing the same elements hash to the same value
comment|//
block|{
comment|// create some deterministic initial state:
specifier|const
name|QtQHashSeedSaver
name|seed1
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// provoke different bucket counts
name|s2
operator|<<
literal|100
operator|<<
...
literal|400
expr_stmt|;
comment|// and insert elements in different order, too
name|QVERIFY
argument_list|(
name|s1
operator|.
...
argument_list|)
expr_stmt|;
comment|// verify that the order _is_ different
name|QCOMPARE
argument_list|(
name|qHash
argument_list|(
...
block|}
comment|//
comment|// check that sets of sets work:
comment|//
block|{
ifdef|#
directive|ifdef
