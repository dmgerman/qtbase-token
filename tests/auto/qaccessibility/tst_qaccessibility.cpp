...
argument_list|()
operator|<<
literal|"Enable test again when child and parent functions are implemented"
expr_stmt|;
if|#
directive|if
literal|0
block|QVERIFY(interface->state(1)& QAccessible::Invisible);      const int lots = 10;     for (int i = 0; i< lots; ++i)         tabBar->addTab("Foo");      QVERIFY((interface->state(1)& QAccessible::Invisible) == false);     tabBar->hide();     QVERIFY(interface->state(1)& QAccessible::Invisible);      tabBar->show();     tabBar->setCurrentIndex(0);
comment|// Test that sending a focus action to a tab does not select it.
block|interface->doAction(QAccessible::Focus, 2, QVariantList());     QCOMPARE(tabBar->currentIndex(), 0);
comment|// Test that sending a press action to a tab selects it.
block|interface->doAction(QAccessible::Press, 2, QVariantList());     QCOMPARE(tabBar->currentIndex(), 1);
endif|#
directive|endif
operator|delete
name|tabBar
...
directive|endif
block|QCOMPARE(parent->role(0), QAccessible::LayeredPane);     delete parent;     delete stackChild1Interface;     delete stackChild2Interface;     delete stackWidgetInterface;
endif|#
directive|endif
operator|delete
name|tabBarInterface
