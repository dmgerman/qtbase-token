...
end_include
begin_comment
comment|/*! \class QSystemLocker      \brief The QSystemLocker class is a convenience class that simplifies     locking and unlocking system locks.      The purpose of QSystemLocker is to simplify QSystemLock locking and     unlocking.  Locking and unlocking a QSystemLock in complex functions and     statements or in exception handling code is error-prone and difficult to     debug.  QSystemLocker can be used in such situations to ensure that the     state of the locks is always well-defined.      QSystemLocker should be created within a function where a QSystemLock needs     to be locked. The system lock is locked when QSystemLocker is created.  If     locked, the system lock will be unlocked when the QSystemLocker is     destroyed.  QSystemLocker can be unlocked with unlock() and relocked with     relock().      \sa QSystemLock  */
end_comment
begin_comment
comment|/*! \fn QSystemLocker::QSystemLocker()      Constructs a QSystemLocker and locks \a lock. The \a lock will be     unlocked when the QSystemLocker is destroyed. If lock is zero,     QSystemLocker does nothing.      \sa QSystemLock::lock()   */
end_comment
begin_comment
comment|/*! \fn QSystemLocker::~QSystemLocker()      Destroys the QSystemLocker and unlocks it if it was     locked in the constructor.      \sa QSystemLock::unlock()  */
end_comment
begin_comment
comment|/*! \fn QSystemLocker::systemLock()      Returns a pointer to the lock that was locked in the constructor.  */
end_comment
begin_comment
comment|/*! \fn QSystemLocker::relock()      Relocks an unlocked locker.      \sa unlock()   */
end_comment
begin_comment
comment|/*! \fn QSystemLocker::unlock()      Unlocks this locker. You can use relock() to lock it again.     It does not need to be locked when destroyed.      \sa relock()  */
end_comment
begin_comment
comment|/*! \class QSystemLock      \brief The QSystemLock class provides a system wide lock     that can be used between threads or processes.      The purpose of a QSystemLocker is to protect an object that can be     accessed by multiple threads or processes such as shared memory or a file.      For example, say there is a method which prints a message to a log file:      void log(const QString&logText)     {         QSystemLock systemLock(QLatin1String("logfile"));         systemLock.lock();         QFile file(QDir::temp() + QLatin1String("/log"));         if (file.open(QIODevice::Append)) {             QTextStream out(&file);             out<< logText;         }         systemLock.unlock();     }      If this is called from two seperate processes the resulting log file is     guaranteed to contain both lines.      When you call lock(), other threads or processes that try to call lock()     with the same key will block until the thread or process that got the lock     calls unlock().      A non-blocking alternative to lock() is tryLock().  */
end_comment
begin_comment
comment|/*!     Constructs a new system lock with \a key. The lock is created in an     unlocked state.      \sa lock(), key().  */
end_comment
begin_constructor
DECL|function|QSystemLock
name|QSystemLock
operator|::
name|QSystemLock
parameter_list|(
...
end_constructor
begin_comment
comment|/*!     Destroys a system lock.      warning: This will not unlock the system lock if it has been locked. */
end_comment
begin_destructor
DECL|function|~QSystemLock
name|QSystemLock
...
end_destructor
begin_comment
comment|/*!     Sets a new key to this system lock.      \sa key()  */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!     Returns the key assigned to this system lock      \sa setKey()  */
end_comment
begin_function
DECL|function|key
name|QString
...
argument_list|()
operator|<<
literal|"QSystemLock::lock readwrite lock on top of readonly lock."
expr_stmt|;
return|return
literal|false
return|;
...
end_function
begin_comment
comment|/*!     Unlocks the system lock.     Returns true on success; otherwise returns false.      \sa lock()   */
end_comment
begin_function
...
argument_list|()
operator|<<
literal|"QSystemLock::unlock: unlock with no lock."
expr_stmt|;
return|return
literal|false
return|;
