...
directive|ifdef
name|Q_OS_SYMBIAN
comment|// Can't deploy empty test dir, so create it here
name|QDir
name|dir
argument_list|(
name|SRCDIR
...
directive|ifdef
name|Q_OS_SYMBIAN
comment|// Remove created test dir
name|QDir
name|dir
argument_list|(
name|SRCDIR
...
name|newRow
argument_list|(
literal|"QDir::All | QDir::Hidden | QDir::System"
argument_list|)
operator|<<
name|SRCDIR
literal|"entrylist/"
...
name|newRow
argument_list|(
literal|"QDir::All | QDir::Readable"
argument_list|)
operator|<<
name|SRCDIR
literal|"entrylist/"
...
name|newRow
argument_list|(
literal|"QDir::All | QDir::Writable"
argument_list|)
operator|<<
name|SRCDIR
literal|"entrylist/"
...
argument_list|(
name|SRCDIR
literal|"entrylist/linktodirectory"
argument_list|)
expr_stmt|;
name|QFile
operator|::
...
argument_list|(
name|SRCDIR
literal|"entrylist/brokenlink"
argument_list|)
expr_stmt|;
comment|// WinCE/Symbian does not have . and .. in the directory listing
if|#
directive|if
name|defined
argument_list|(
...
name|Q_OS_SYMBIAN
argument_list|)
comment|// Symbian doesn't support links to directories
name|expected
operator|.
name|removeAll
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// Expecting failures from a couple of OpenC bugs. Do checks only once.
specifier|static
name|int
name|xFailChecked
init|=
...
condition|)
block|{
comment|// Can't create link if file doesn't exist in symbian, so create file temporarily,
comment|// But only if testing for
name|QFile
name|tempFile
argument_list|(
name|SRCDIR
...
condition|)
block|{
comment|// ### Until OpenC supports stat correctly for symbolic links, expect them to fail.
name|expectedFail1
operator|=
name|QFileInfo
argument_list|(
...
literal|""
argument_list|,
literal|"OpenC bug, stat for broken links returns normally, when it should return error."
argument_list|,
name|Continue
argument_list|)
expr_stmt|;
...
literal|""
argument_list|,
literal|"OpenC bug, stat for file links doesn't indicate them as such."
argument_list|,
name|Continue
argument_list|)
expr_stmt|;
...
name|Q_OS_SYMBIAN
argument_list|)
comment|// Test cleanup on device requires setting the permissions back to normal
name|QFile
argument_list|(
name|SRCDIR
literal|"entrylist/file"
...
literal|""
argument_list|,
literal|"OpenC stat for symlinks is buggy."
argument_list|,
name|Continue
argument_list|)
expr_stmt|;
...
argument_list|)
expr_stmt|;
comment|// remove the ending slash
if|#
directive|if
name|defined
name|Q_WS_WIN
...
literal|"rename-test"
argument_list|,
literal|"/resource/rename-test-renamed"
argument_list|)
argument_list|)
expr_stmt|;
elif|#
...
name|QSKIP
argument_list|(
literal|"WinCE and Symbian do not have . nor .."
argument_list|,
name|SkipAll
argument_list|)
expr_stmt|;
...
end_ifdef
begin_comment
comment|/*     Tets that the setMatchAllDirs setting survies a call to setFilter. */
end_comment
begin_function
