...
function_decl|;
name|void
name|QTBUG36933_brokenPseudoClassLookup
parameter_list|()
function_decl|;
name|void
name|styleSheetChangeBeforePolish
parameter_list|()
function_decl|;
...
parameter_list|()
block|{
comment|/* qApp->setStyleSheet("");     // qApp style can never be 0     QVERIFY(QApplication::style() != 0);     QPointer<QStyle> style1 = QStyleFactory::create("Windows");     QPointer<QStyle> style2 = QStyleFactory::create("Windows");     qApp->setStyle(style1);     // Basic sanity     QVERIFY(qApp->style() == style1);     qApp->setStyle(style2);     QVERIFY(style1.isNull()); // qApp must have taken ownership and deleted it     // Setting null should not crash     qApp->setStyle(0);     QVERIFY(qApp->style() == style2);      // Set the stylesheet     qApp->setStyleSheet("whatever");     QPointer<QStyleSheetStyle> sss = (QStyleSheetStyle *)qApp->style();     QVERIFY(!sss.isNull());     QCOMPARE(sss->metaObject()->className(), "QStyleSheetStyle"); // must be our proxy now     QVERIFY(!style2.isNull()); // this should exist as it is the base of the proxy     QVERIFY(sss->baseStyle() == style2);     style1 = QStyleFactory::create("Windows");     qApp->setStyle(style1);     QVERIFY(style2.isNull()); // should disappear automatically     QVERIFY(sss.isNull()); // should disappear automatically      // Update the stylesheet and check nothing changes     sss = (QStyleSheetStyle *)qApp->style();     qApp->setStyleSheet("whatever2");     QVERIFY(qApp->style() == sss);     QVERIFY(sss->baseStyle() == style1);      // Revert the stylesheet     qApp->setStyleSheet("");     QVERIFY(sss.isNull()); // should have disappeared     QVERIFY(qApp->style() == style1);      qApp->setStyleSheet("");     QVERIFY(qApp->style() == style1);*/
block|}
end_function
begin_function
...
literal|" (for example, QTBUG-33959)."
...
name|QSKIP
argument_list|(
literal|"This test is fragile on Mac, most likely due to QTBUG-33959."
argument_list|)
expr_stmt|;
endif|#
directive|endif
...
name|w
decl_stmt|;
comment|// Use "Fusion" to prevent the Vista style from clobbering the tooltip palette in polish().
name|QStyle
modifier|*
name|fusionStyle
init|=
...
argument_list|)
expr_stmt|;
comment|// Wait until Roll-Effect is finished (Windows Vista)
name|QCOMPARE
argument_list|(
name|tooltip
operator|->
...
argument_list|)
expr_stmt|;
comment|// put no visible text for the vertical headers, but still put some text or they will collapse
name|widget
operator|.
...
expr_stmt|;
block|}
comment|// parsing of this stylesheet must not crash, and it must be correctly applied
name|widget
operator|.
name|setStyleSheet
argument_list|(
name|QStringLiteral
argument_list|(
literal|"QHeaderView::section:vertical { background-color: #FF0000 }"
argument_list|)
argument_list|)
expr_stmt|;
name|centerOnScreen
...
name|setStyleSheet
argument_list|(
literal|"background-color: #FF0000;"
argument_list|)
expr_stmt|;
name|frame
operator|->
name|setStyleSheet
argument_list|(
literal|"background-color: #00FF00;"
argument_list|)
expr_stmt|;
name|vbox
operator|->
