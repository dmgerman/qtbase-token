...
function_decl|;
name|void
name|task_QTBUG_31146_popupCompletion
parameter_list|()
function_decl|;
name|void
name|task_QTBUG_41288_completerChangesCurrentIndex
parameter_list|()
function_decl|;
name|void
...
function_decl|;
name|void
name|setCustomModelAndView
parameter_list|()
function_decl|;
name|void
name|updateDelegateOnEditableChange
parameter_list|()
function_decl|;
name|void
name|respectChangedOwnershipOfItemView
parameter_list|()
function_decl|;
name|void
name|task_QTBUG_39088_inputMethodHints
parameter_list|()
function_decl|;
name|void
name|task_QTBUG_49831_scrollerNotActivated
parameter_list|()
function_decl|;
block|}
class|;
...
argument_list|)
expr_stmt|;
comment|// check AdjustToContents changes when model changes
name|content
operator|=
name|testWidget
operator|->
...
argument_list|)
expr_stmt|;
comment|// check AdjustToContents changes when a row is inserted into the model
name|content
operator|=
name|testWidget
operator|->
...
argument_list|)
expr_stmt|;
comment|// check AdjustToContents changes when model is reset
name|content
operator|=
name|testWidget
operator|->
...
argument_list|)
expr_stmt|;
comment|// change the model, ensure that the root index gets reset
name|oldModel
operator|=
name|box
operator|.
...
argument_list|)
expr_stmt|;
comment|// check that setting the very same model doesn't move the current item
name|box
operator|.
name|setCurrentIndex
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// check that setting the very same model doesn't move the root index
name|rootModelIndex
operator|=
name|box
operator|.
...
parameter_list|()
block|{
comment|// QTBUG-27597, ensure the correct text is returned when using custom view and a tree model.
name|QComboBox
name|combo
decl_stmt|;
name|combo
...
name|setWindowTitle
argument_list|(
literal|"QTBUG-27597, setCustomModelAndView"
argument_list|)
expr_stmt|;
name|combo
operator|.
...
parameter_list|()
block|{
comment|// QTBUG-40656, QTBUG-42731 combo and other popups should not be affected by wheel events.
name|QScrollArea
name|scrollArea
decl_stmt|;
name|scrollArea
...
argument_list|)
expr_stmt|;
comment|// QTBUG-33973, cursor needs to be within view from start on Mac.
name|QApplication
operator|::
name|setActiveWindow
argument_list|(
...
literal|10
decl_stmt|;
comment|// ensure that the currentText(), the DisplayRole and the EditRole
comment|// stay in sync when using QComboBox's default model
for|for
control|(
name|int
name|i
...
control|)
block|{
comment|// now change by changing the DisplayRole's data
name|QString
name|itemText
init|=
name|QString
...
control|)
block|{
comment|// now change by changing the EditRole's data
name|QString
name|itemText
init|=
name|QString
...
argument_list|()
expr_stmt|;
comment|// set additional user data in the addItem call
for|for
control|(
name|int
name|i
...
argument_list|()
expr_stmt|;
comment|// additional roles, setItemData
comment|// UserRole + 0 -> string
comment|// UserRole + 1 -> double
comment|// UserRole + 2 -> icon
name|QString
name|qtlogoPath
init|=
name|QFINDTESTDATA
...
expr_stmt|;
block|{
comment|// change currentIndex() by keyboard
name|comboBox
operator|.
name|lineEdit
argument_list|()
...
block|}
block|{
comment|// change currentIndex() programmatically
name|comboBox
operator|.
name|lineEdit
argument_list|()
...
argument_list|)
expr_stmt|;
comment|// Clear any remaining keyboard input from previous tests.
name|QTest
operator|::
name|qWait
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// Ownership should now be transferred to box2
name|QTableView
modifier|*
name|v2
init|=
...
argument_list|)
expr_stmt|;
comment|// Here we do not expect v1 to be deleted
name|QApplication
operator|::
name|processEvents
argument_list|()
...
argument_list|()
decl_stmt|;
comment|// Not all styles support scrollers. We rely only on those platforms that do to catch any regression.
if|if
condition|(
operator|!
name|scrollers
...
name|Q_FOREACH
argument_list|(
argument|QComboBoxPrivateScroller *scroller
argument_list|,
argument|scrollers
argument_list|)
block|{
