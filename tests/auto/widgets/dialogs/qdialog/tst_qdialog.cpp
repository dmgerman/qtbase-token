...
name|QSKIP
argument_list|(
literal|"Test relies on there being a cursor"
argument_list|)
expr_stmt|;
else|#
directive|else
...
argument_list|)
expr_stmt|;
comment|// Transient parent should always be the top level, also when using
...
parameter_list|()
block|{
comment|// QTBUG-49124: A dialog embedded into QGraphicsView has Qt::WA_DontShowOnScreen
comment|// set (as has a native dialog). It must not trigger the modal handling though
comment|// as not to lock up.
name|QGraphicsScene
name|scene
decl_stmt|;
name|QGraphicsView
