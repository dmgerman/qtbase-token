...
function_decl|;
name|void
name|selectFileWrongCaseSaveAs
parameter_list|()
function_decl|;
name|void
name|selectFilter
...
function_decl|;
name|void
name|widgetlessNativeDialog
parameter_list|()
function_decl|;
name|void
name|selectedFilesWithoutWidgets
parameter_list|()
function_decl|;
name|void
name|trailingDotsAndSpaces
parameter_list|()
function_decl|;
ifdef|#
directive|ifdef
...
directive|endif
name|void
name|rejectModalDialogs
parameter_list|()
function_decl|;
name|void
name|QTBUG49600_nativeIconProviderCrash
parameter_list|()
function_decl|;
private|private:
name|void
...
name|QSKIP
argument_list|(
literal|"This test requires at least 2 side bar entries."
argument_list|)
expr_stmt|;
name|fd
operator|.
...
condition|)
block|{
comment|// Try to find a suitable directory under root that does not
comment|// start with 'C' to avoid issues with completing to "C:" drives on Windows.
specifier|const
name|QString
name|rootPath
init|=
...
argument_list|()
argument_list|,
literal|"Unable to find a suitable input directory"
argument_list|)
expr_stmt|;
block|}
...
expr_stmt|;
block|}
comment|// The temporary dir may create a node in QFileSystemModel
comment|// which will bypass filters. If the path to the temporary
comment|// dir contains an element which should be a subdirectory
comment|// of x dir, but which is not listed, then take it into
comment|// accont.
if|if
condition|(
operator|!
...
argument_list|()
expr_stmt|;
comment|// Ensure the file dialog let's go of the temporary file for "temp".
block|}
end_function
begin_function
...
name|QSKIP
argument_list|(
literal|"This test is intended for case-insensitive file systems only."
argument_list|)
expr_stmt|;
comment|// QTBUG-38162: when passing a wrongly capitalized path to selectFile()
comment|// on a case-insensitive file system, the line edit should only
comment|// contain the file name ("c:\PRogram files\foo.txt" -> "foo.txt").
specifier|const
name|QString
name|fileName
init|=
...
name|QLatin1String
argument_list|(
literal|"/qfiledialog_auto_test_not_pres_"
argument_list|)
operator|+
name|QString
operator|::
...
name|QSKIP
argument_list|(
literal|"This platform always uses widgets to realize its QFileDialog, instead of the native file dialog."
argument_list|)
expr_stmt|;
name|QFileDialog
name|fd
...
parameter_list|()
block|{
comment|// Test for a crash when widgets are not instantiated yet.
name|QFileDialog
name|fd
decl_stmt|;
name|fd
...
name|QSKIP
argument_list|(
literal|"This is only tested on Windows"
argument_list|)
expr_stmt|;
endif|#
directive|endif
...
parameter_list|()
block|{
comment|// QTBUG-38672 , static functions should return empty Urls
specifier|const
name|QFileDialog
operator|::
name|Options
...
argument_list|)
expr_stmt|;
comment|// Same test with local files
name|QString
name|file
init|=
name|QFileDialog
