...
function_decl|;
name|void
name|tst_paintEventOnResize_QTBUG50796
parameter_list|()
function_decl|;
ifndef|#
directive|ifndef
...
function_decl|;
name|void
name|tst_eventfilter_on_toplevel
parameter_list|()
function_decl|;
name|void
name|QTBUG_50561_QCocoaBackingStore_paintDevice_crash
parameter_list|()
function_decl|;
block|}
class|;
...
name|QSKIP
argument_list|(
literal|"Cocoa: This fails. Figure out why."
argument_list|)
expr_stmt|;
elseif|else
if|if
...
name|QSKIP
argument_list|(
literal|"Qt::WA_ShowWithoutActivating is currently supported only on xcb, windows, and ios platforms."
argument_list|)
expr_stmt|;
name|QWidget
name|w1
...
argument_list|()
expr_stmt|;
comment|// We're testing native widgets
name|native
operator|->
name|setGeometry
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// Only one paint event must occur
block|}
end_function
begin_ifndef
ifndef|#
...
argument_list|)
expr_stmt|;
comment|// send an event not handled in a special way by QWidgetWindow::event,
comment|// and check that it's received by the event filter
...
parameter_list|()
block|{
comment|// Keep application state clean if testcase fails
name|ApplicationStateSaver
name|as
decl_stmt|;
name|QMainWindow
...
argument_list|)
expr_stmt|;
comment|// Simulate window system close
name|QCloseEvent
modifier|*
name|e
init|=
