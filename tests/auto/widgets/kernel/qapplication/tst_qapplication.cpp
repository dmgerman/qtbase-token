...
argument_list|)
decl_stmt|;
comment|// Even though We create a new widget and show it,
comment|// the ref count does not go up because it is a child of
comment|// w1, which is the top-level, and what we are actually
comment|// refcounting.
name|subWidget1
operator|->
name|show
...
argument_list|)
expr_stmt|;
comment|// When we use setParent(0) and re-show, the
comment|// ref count does increase:
name|QCOMPARE
argument_list|(
name|subWidget1
operator|->
...
argument_list|)
expr_stmt|;
comment|// Run a job before showing a window, and only enable the refcounting
comment|// after doing so.
comment|// Although the job brings the refcount to zero, the app does not exit
comment|// until setQuitLockEnabled is called and the feature re-enabled.
name|JobBeforeWindowRunner
modifier|*
name|eventRunner
init|=
...
argument_list|)
decl_stmt|;
comment|// A job runs, and while it is running, a window is shown and closed,
comment|// then the job ends, which causes the quit.
name|JobDuringWindowRunner
modifier|*
name|eventRunner
init|=
...
argument_list|)
decl_stmt|;
comment|// A job runs, and while it is running, a window is shown
comment|// then the job ends, then the window is closed, which causes the quit.
name|JobWindowJobWindowRunner
modifier|*
name|eventRunner
init|=
