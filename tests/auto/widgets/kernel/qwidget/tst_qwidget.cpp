...
specifier|inline
name|void
name|setWindowsAnimationsEnabled
parameter_list|(
name|bool
name|enabled
parameter_list|)
...
function_decl|;
name|void
name|showHideEventWhileMinimize
parameter_list|()
function_decl|;
name|void
...
function_decl|;
name|void
name|touchUpdateOnNewTouch
parameter_list|()
function_decl|;
name|void
name|touchEventsForGesturePendingWidgets
parameter_list|()
function_decl|;
name|void
...
function_decl|;
name|void
name|resizeStaticContentsChildWidget_QTBUG35282
parameter_list|()
function_decl|;
name|void
name|qmlSetParentHelper
parameter_list|()
function_decl|;
name|void
name|testForOutsideWSRangeFlag
parameter_list|()
function_decl|;
private|private:
name|bool
...
name|QPoint
name|m_safeCursorPos
decl_stmt|;
DECL|member|m_windowsAnimationsEnabled
specifier|const
name|bool
name|m_windowsAnimationsEnabled
decl_stmt|;
block|}
class|;
end_class
...
name|m_windowsAnimationsEnabled
condition|)
comment|// Disable animations which can interfere with screen grabbing in moveChild(), showAndMoveChild()
name|setWindowsAnimationsEnabled
argument_list|(
literal|false
argument_list|)
...
directive|endif
comment|// Size of reference widget, 200 for< 2000, scale up for larger screens
...
argument_list|()
expr_stmt|;
comment|// XCB: Determine "safe" cursor position at bottom/right corner of screen.
comment|// Pushing the mouse rapidly to the top left corner can trigger KDE / KWin's
comment|// "Present all Windows" (Ctrl+F9) feature also programmatically.
if|if
condition|(
name|m_platform
operator|==
...
name|QSKIP
argument_list|(
literal|"QTBUG-52974: Cocoa has no Z-Order for views, we hack it, but it results in paint events."
argument_list|)
expr_stmt|;
endif|#
directive|endif
...
block|}
class|;
comment|// Scale to remove devicePixelRatio should scaling be active.
...
name|QSKIP
argument_list|(
literal|"This test does not work with scaling."
argument_list|)
expr_stmt|;
name|ContentsPropagationWidget
name|widget
...
name|QSKIP
argument_list|(
literal|"QTBUG-52974: this test will most probably be deleted - to be investigated."
argument_list|)
expr_stmt|;
name|QWidget
name|tmpWindow
...
name|QSKIP
argument_list|(
literal|"Skip due to rounding errors in the regions."
argument_list|)
expr_stmt|;
name|StaticWidget
name|topLevel
...
argument_list|)
decl_stmt|;
comment|// QTBUG-41898: Use top level as transient parent for native widgets as well.
name|QToolButton
modifier|*
name|toolButton
init|=
...
name|QSKIP
argument_list|(
literal|"This test requires window management capabilities"
argument_list|)
expr_stmt|;
block|}
comment|// QTBUG-41312, hide, show events should be received during minimized.
name|ShowHideEventWidget
name|widget
decl_stmt|;
name|widget
...
block|{
comment|// in case of an invalid rect, wait for the geometry to become
comment|// adjusted to the actual (valid) value.
name|QApplication
operator|::
name|processEvents
argument_list|()
...
argument_list|)
expr_stmt|;
comment|// Windows: Minimum size of decorated windows.
specifier|const
name|bool
name|expectResizeFail
init|=
operator|(
operator|!
name|windowFlags
...
control|)
block|{
comment|// XCB: First resize after show of zero-sized gets wrong win_gravity.
specifier|const
name|bool
name|expectMoveFail
init|=
...
condition|)
block|{
comment|/* Normal run.                    If it succeeds: return success                    If it fails:    do not return, but wait a bit and reiterate (retry)                 */
if|if
condition|(
name|firstPixel
operator|==
...
block|{
comment|// Last run, report failure if it still fails
if|if
condition|(
operator|!
name|QTest
...
directive|ifndef
name|QT_NO_CURSOR
comment|// Try to make sure the cursor is not in a taskbar area to prevent tooltips or window highlighting
name|QCursor
operator|::
name|setPos
argument_list|(
...
name|QSKIP
argument_list|(
literal|"QTBUG-52974: Cocoa only has rect granularity."
argument_list|)
expr_stmt|;
endif|#
directive|endif
...
specifier|const
name|int
name|lastWidgetWindow
init|=
literal|1
decl_stmt|;
comment|// 0 and 1 are qwidgetwindow, 2 is a pure qwindow
...
name|WindowEventSpyPtr
argument_list|>
name|appWindowEventSpies
decl_stmt|;
name|QList
argument_list|<
name|WindowEventSpyPtr
argument_list|>
name|windowEventSpies
decl_stmt|;
foreach|foreach
control|(
name|QWindow
...
control|)
block|{
comment|// Check QEvent::ApplicationWindowIconChange (sent to QWindow)
comment|// QWidgetWindows don't get this event, since the widget takes care of changing the icon
name|WindowEventSpyPtr
name|spy
init|=
name|appWindowEventSpies
...
argument_list|()
expr_stmt|;
comment|// Check QEvent::WindowIconChange (sent to QWindow)
name|spy
operator|=
name|windowEventSpies
operator|.
...
argument_list|)
member_init_list|,
name|m_touchBeginCount
argument_list|(
literal|0
argument_list|)
member_init_list|,
name|m_touchUpdateCount
argument_list|(
literal|0
argument_list|)
member_init_list|,
name|m_touchEndCount
argument_list|(
literal|0
argument_list|)
member_init_list|,
...
argument_list|)
expr_stmt|;
comment|// needed for QApplication::topLevelAt(), which is used by QGestureManager
name|QCOMPARE
argument_list|(
name|child
operator|.
...
argument_list|)
expr_stmt|;
comment|// QTapAndHoldGestureRecognizer::create() sets Qt::WA_AcceptTouchEvents
name|QCOMPARE
argument_list|(
name|parent
operator|.
...
name|QSKIP
argument_list|(
literal|"QTBUG-52974: this test can crash!"
argument_list|)
expr_stmt|;
endif|#
directive|endif
...
argument_list|)
expr_stmt|;
comment|// The window should not be visible
name|QVERIFY
argument_list|(
name|widget
operator|.
...
argument_list|)
expr_stmt|;
comment|// The widget should be in visible state
block|}
block|{
name|QWidget
name|widget
...
argument_list|()
expr_stmt|;
comment|// The layout should change the size, so the widget must be visible!
name|QVERIFY
argument_list|(
name|QTest
operator|::
