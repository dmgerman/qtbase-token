...
parameter_list|)
block|{
comment|//qDebug()<< "incomingConnection"<< socketDescriptor;
if|if
condition|(
operator|!
name|doSsl
...
literal|"empty-scheme-host"
argument_list|,
literal|"this is expected to fail on Windows and Symbian, QTBUG-17731"
argument_list|,
name|Abort
argument_list|)
expr_stmt|;
...
parameter_list|()
block|{
comment|// FIXME: Implement a test that aborts a big HTTP reply
comment|// a) after the first readyRead()
comment|// b) immediatly after the get()
comment|// c) after the finished()
comment|// The goal is no crash and no irrelevant signals after the abort
