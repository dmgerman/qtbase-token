...
parameter_list|)
block|{
comment|// For the plain text logger, we'll read straight from standard output.
comment|// For all other loggers (XML), we'll tell testlib to redirect to a file.
comment|// The reason is that tests are allowed to print to standard output, and
comment|// that means the test log is no longer guaranteed to be valid XML.
name|QStringList
name|extraArguments
decl_stmt|;
name|QString
