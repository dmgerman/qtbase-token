...
argument_list|)
expr_stmt|;
comment|// The second call to setQuery() clears the model by removing it's rows.
comment|// Only 256 rows because that is what was cached.
name|QCOMPARE
argument_list|(
name|modelRowsRemovedSpy
operator|.
...
argument_list|)
expr_stmt|;
comment|// The call to scrollToBottom() forces the model to fetch all rows,
comment|// which will be done in two steps.
name|QCOMPARE
argument_list|(
name|modelRowsInsertedSpy
operator|.
...
decl_stmt|;
name|QSignalSpy
name|modelRowsAboutToBeRemovedSpy
argument_list|(
operator|&
name|model
argument_list|,
...
argument_list|)
decl_stmt|;
comment|// First select, the model was empty and no rows had to be removed!
name|model
operator|.
name|setQuery
argument_list|(
