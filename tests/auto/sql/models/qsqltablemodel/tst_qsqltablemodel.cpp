...
name|void
name|insertWithAutoColumn_data
parameter_list|()
block|{
name|generic_data_with_strategies
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// ignore unchanged 0 to 0
name|idx
operator|=
name|model
operator|.
...
argument_list|)
expr_stmt|;
comment|// unchanged value, but causes column to be included in INSERT
name|QVERIFY_SQL
argument_list|(
name|model
argument_list|,
...
name|tbl
operator|+
literal|"(id INTEGER PRIMARY KEY AUTOINCREMENT, val TEXT)"
argument_list|)
argument_list|)
expr_stmt|;
name|QSqlTableModel
...
argument_list|)
expr_stmt|;
comment|// For insertRow/insertRows, we have to touch at least one column
comment|// or else the generated flag won't be set, which would lead to
comment|// an empty column list in the INSERT statement, which generally
...
condition|)
block|{
comment|// Rows updated in original positions after previous submits.
name|QCOMPARE
argument_list|(
name|model
operator|.
...
block|{
comment|// Manual submit is followed by requery.
name|QVERIFY_SQL
argument_list|(
name|model
argument_list|,
...
name|tblB
operator|+
literal|"(b INT PRIMARY KEY, FOREIGN KEY (b) REFERENCES "
operator|+
name|tblA
operator|+
literal|" (a))"
...
name|QSKIP
argument_list|(
literal|"database does not enforce foreign key constraints, skipping test"
argument_list|)
expr_stmt|;
name|QSqlTableModel
name|model
...
argument_list|)
expr_stmt|;
comment|// don't crash if there is no change
name|model
operator|.
name|revert
argument_list|()
expr_stmt|;
comment|// UPDATE
comment|// invalid value makes submit fail leaving pending update in cache
specifier|const
name|QModelIndex
name|idx
init|=
...
argument_list|)
expr_stmt|;
comment|// insertRow() does not submit leaving pending insert in cache
name|QVERIFY_SQL
argument_list|(
name|model
argument_list|,
...
argument_list|)
expr_stmt|;
comment|// foreign key makes submit fail leaving pending delete in cache
if|if
condition|(
name|submitpolicy
operator|==
