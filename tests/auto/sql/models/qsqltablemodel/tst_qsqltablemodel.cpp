...
block|{
comment|// dataChanged() is not emitted when submitAll() is called
name|QCOMPARE
argument_list|(
name|spy
operator|.
...
argument_list|)
expr_stmt|;
comment|// FieldChange updates immediately and resorts
comment|// Row/Manual submit does not resort
name|QVERIFY
argument_list|(
name|model
operator|.
...
argument_list|)
expr_stmt|;
comment|// See comment above setRecord
if|if
condition|(
name|submitpolicy
operator|==
...
argument_list|)
expr_stmt|;
comment|// headerDataChanged must be emitted by the model when the edit strategy is OnManualSubmit,
comment|// when OnFieldChange or OnRowChange it's not needed because the model will re-select.
name|qRegisterMetaType
argument_list|<
name|Qt
operator|::
...
argument_list|)
expr_stmt|;
comment|// QSqlTableModel emits 10 signals for its 10 inserted rows
name|QCOMPARE
argument_list|(
name|spyRowsRemoved
operator|.
...
argument_list|)
expr_stmt|;
comment|// QSqlQueryModel emits 1 signal for its 3 rows
name|QCOMPARE
argument_list|(
name|spyRowsRemoved
operator|.
