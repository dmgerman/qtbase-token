...
argument_list|)
decl_stmt|;
comment|// nothing should happen on an empty query
name|QSqlRecord
name|rec
init|=
name|db
...
argument_list|)
expr_stmt|;
comment|// query without result set should return empty record
name|rec
operator|=
name|db
operator|.
...
end_comment
begin_comment
unit|void tst_QSqlDatabase::record() {     QFETCH(QString, dbName);     QSqlDatabase db = QSqlDatabase::database(dbName);     CHECK_DATABASE(db);      static const FieldDef fieldDefs[] = {         FieldDef("char(20)", QVariant::String,         QString("blah1"), false),         FieldDef("varchar(20)", QVariant::String,      QString("blah2"), false),         FieldDef()     };      const int fieldCount = createFieldTable(fieldDefs, db);     QVERIFY(fieldCount> 0);
comment|// doesn't work with oracle:   checkNullValues(fieldDefs, db);
end_comment
begin_endif
unit|commonFieldTest(fieldDefs, db, fieldCount);     for (int i = 0; i< ITERATION_COUNT; ++i) {         checkValues(fieldDefs, db);     } }
endif|#
directive|endif
end_endif
begin_ifdef
...
name|QString
argument_list|(
literal|" Expected: '%1' Received: '%2' for field %3 in testRecordInfo"
argument_list|)
operator|.
name|arg
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// check whether recordInfo returns the right types
ifdef|#
directive|ifdef
name|QT3_SUPPORT
name|Q3SqlRecordInfo
...
end_function
begin_comment
comment|// inserts testdata into the testtable, fetches and compares them
end_comment
begin_function
...
argument_list|)
expr_stmt|;
comment|//     qDebug(QString("inserting %1 into %2").arg(fieldDefs[ i ].val.toString()).arg(fieldDefs[ i ].fieldName()));
block|}
name|QVERIFY_SQL
argument_list|(
name|cur
...
condition|)
block|{
comment|// we don't care about precision here, we just want to know whether
comment|// we can insert/fetch the right values
name|ok
operator|=
operator|(
name|val1
...
name|QString
argument_list|(
literal|" Expected: '%1' Received: '%2' for field %3 (etype %4 rtype %5) in checkValues"
argument_list|)
operator|.
name|arg
argument_list|(
...
end_function
begin_comment
comment|// inserts a NULL value for each nullable field in testdata, fetches and checks whether
end_comment
begin_comment
comment|// we get back NULL
end_comment
begin_function
...
condition|)
continue|continue;
comment|// multiple inheritance sucks so much
name|QVERIFY2
argument_list|(
operator|(
operator|(
...
name|QString
argument_list|(
literal|"QVariant is not null for NULL-Value in Field '%1'"
argument_list|)
operator|.
name|arg
argument_list|(
...
directive|ifdef
name|QT3_SUPPORT
comment|//     FieldDef("clob(20)", QVariant::CString,	QString("Blah7")),
comment|//     FieldDef("dbclob(20)", QVariant::CString,	QString("Blah8")),
comment|//     FieldDef("blob(20)", QVariant::ByteArray,	QByteArray(Q3CString("Blah9"))),
endif|#
directive|endif
