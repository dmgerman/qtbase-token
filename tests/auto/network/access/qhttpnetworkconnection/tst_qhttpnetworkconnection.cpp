...
condition|)
return|return;
comment|// close socket instantly!
name|connect
argument_list|(
name|socket
argument_list|,
...
parameter_list|()
block|{
comment|// server accepts connections, but closes the socket instantly
name|TestTcpServer
name|server
decl_stmt|;
...
name|accessManager
decl_stmt|;
comment|// ten requests are scheduled. All should result in an RemoteHostClosed...
name|QUrl
name|url
decl_stmt|;
name|url
...
argument_list|)
decl_stmt|;
comment|// Not using Qt5 connection syntax here because of overly baroque syntax to discern between
comment|// different error() methods.
name|QObject
operator|::
name|connect
argument_list|(
