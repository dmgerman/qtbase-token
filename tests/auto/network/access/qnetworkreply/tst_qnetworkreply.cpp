...
define|#
directive|define
name|FLUKE_CERTIFICATE_ERROR
value|QSslError::SelfSignedCertificate
end_define
begin_else
else|#
directive|else
...
directive|define
name|FLUKE_CERTIFICATE_ERROR
value|QSslError::CertificateUntrusted
end_define
begin_endif
endif|#
directive|endif
...
function_decl|;
name|void
name|qtbug45581WrongReplyStatusCode
parameter_list|()
function_decl|;
name|void
...
function_decl|;
name|void
name|ioHttpSingleRedirect
parameter_list|()
function_decl|;
name|void
name|ioHttpChangeMaxRedirects
parameter_list|()
function_decl|;
name|void
name|ioHttpRedirectErrors_data
parameter_list|()
function_decl|;
name|void
name|ioHttpRedirectErrors
parameter_list|()
function_decl|;
ifndef|#
directive|ifndef
...
name|bool
name|notEnoughDataForFastSender
decl_stmt|;
block|}
class|;
end_class
...
argument_list|)
expr_stmt|;
comment|// FIXME: For SSL connections, if we don't flush the socket, the
comment|// client never receives the data and since we're doing a disconnect
comment|// immediately afterwards, it causes a RemoteHostClosedError for the
comment|// client
name|client
operator|->
name|flush
...
name|QSKIP
argument_list|(
literal|"kernel socket buffers are too big for this test to work"
argument_list|)
expr_stmt|;
block|}
name|QVERIFY2
...
argument_list|()
expr_stmt|;
comment|// verify that the normal request emitted less signals than the one emitting all signals
name|QVERIFY2
argument_list|(
name|signalCount
operator|.
...
argument_list|)
argument_list|,
literal|"no upload signal was suppressed"
argument_list|)
expr_stmt|;
block|}
end_function
...
literal|"HTTP/1.1 200 OK\r\nContent-Length: 0\r\n\r\n"
decl_stmt|;
comment|// Setup server to which the second server will redirect to
name|MiniHttpServer
name|server2
argument_list|(
name|http200Reply
...
argument_list|)
expr_stmt|;
comment|// Redirected and finished should be emitted exactly once
name|QCOMPARE
argument_list|(
name|redSpy
operator|.
...
argument_list|)
expr_stmt|;
comment|// Original URL should not be changed after redirect
name|QCOMPARE
argument_list|(
name|request
operator|.
...
argument_list|)
expr_stmt|;
comment|// Reply url is set to the redirect url
name|QCOMPARE
argument_list|(
name|reply
operator|->
...
argument_list|)
expr_stmt|;
comment|// Set Max redirects to 1. This will cause TooManyRedirectsError
name|request
operator|.
...
argument_list|)
expr_stmt|;
comment|// Increase max redirects to allow successful completion
name|request
operator|.
name|setMaximumRedirectsAllowed
argument_list|(
...
literal|"HTTP/1.1 307 Temporary Redirect\r\n"
literal|"Content-Type: text/plain\r\n"
literal|"location: http://localhost:%1\r\n\r\n"
argument_list|)
decl_stmt|;
name|QTest
operator|::
...
name|QNetworkReply
operator|::
name|InsecureRedirectError
expr_stmt|;
name|QTest
operator|::
name|newRow
argument_list|(
literal|"unknown-redirect"
argument_list|)
operator|<<
literal|"http://localhost"
operator|<<
