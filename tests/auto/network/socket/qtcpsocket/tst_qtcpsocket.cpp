...
function_decl|;
name|void
name|bindThenResolveHost_data
parameter_list|()
function_decl|;
name|void
name|bindThenResolveHost
parameter_list|()
function_decl|;
name|void
...
function_decl|;
name|void
name|increaseReadBufferSizeFromSlot
parameter_list|()
function_decl|;
name|void
...
function_decl|;
name|void
name|clientSendDataOnDelayedDisconnect
parameter_list|()
function_decl|;
name|void
...
function_decl|;
name|void
name|socketDiscardDataInWriteMode
parameter_list|()
function_decl|;
name|void
name|writeOnReadBufferOverflow
parameter_list|()
function_decl|;
name|void
name|readNotificationsAfterBind
parameter_list|()
function_decl|;
protected|protected
name|slots
...
function_decl|;
name|void
name|slotIncreaseReadBufferSizeReadyRead
parameter_list|()
function_decl|;
private|private:
name|QByteArray
...
name|firstFailName
argument_list|(
literal|"qt-test-server-first-fail"
argument_list|)
block|{
name|qsrand
argument_list|(
...
block|}
block|}
comment|// test binding to localhost
name|QTest
operator|::
name|newRow
argument_list|(
...
literal|"::"
expr_stmt|;
comment|// and binding with a port number...
comment|// Since we want to test that we got the port number we asked for, we need a random port number.
comment|// We use random in case a previous run of the test left the port lingering open.
comment|// -1 indicates "random port"
name|QTest
operator|::
name|newRow
argument_list|(
literal|"0.0.0.0:randomport"
argument_list|)
operator|<<
literal|"0.0.0.0"
operator|<<
...
directive|ifdef
name|Q_OS_UNIX
comment|// try to bind to a privileged ports
comment|// we should fail if we're not root (unless the ports are in use!)
name|QTest
operator|::
name|newRow
argument_list|(
...
name|dummySocket
decl_stmt|;
comment|// used only to "use up" a file descriptor
name|dummySocket
operator|.
name|bind
argument_list|()
...
literal|5
decl_stmt|;
comment|// only used with randomPort
do|do
block|{
if|if
condition|(
...
condition|)
block|{
comment|// try to get a random port number
comment|// we do this to ensure we're not trying to bind to the same port as we've just used in
comment|// a previous run - race condition with the OS actually freeing the port
...
condition|)
block|{
comment|// we may have been unlucky and hit an already open port, so try another
operator|--
name|attemptsLeft
expr_stmt|;
continue|continue;
...
condition|)
block|{
comment|// try to use the socket and expect it to remain working
name|QTcpServer
name|server
decl_stmt|;
name|QVERIFY
...
argument_list|)
expr_stmt|;
comment|// free up the file descriptor
name|dummySocket
operator|.
name|close
argument_list|()
...
end_function
begin_comment
comment|// similar to the previous test, but we'll connect to a host name that needs resolving
end_comment
begin_function
...
condition|)
return|return;
comment|// doesn't make sense to test binding locally with proxies
name|QFETCH
argument_list|(
name|QString
argument_list|,
...
name|increaseReadBufferSizeFromSlot
parameter_list|()
comment|// like KIO's socketconnectionbackend
block|{
name|QFETCH_GLOBAL
argument_list|(
name|bool
...
end_function
begin_comment
comment|// Test buffered socket properly send data on delayed disconnect
end_comment
begin_function
...
argument_list|)
expr_stmt|;
comment|// Connect to server, write data and close socket
specifier|const
name|QByteArray
name|sendData
argument_list|(
...
comment|// Check data on server side
name|QByteArray
name|recData
decl_stmt|;
name|QVERIFY
...
end_function
begin_comment
comment|// Test buffered sockets discard input when opened in WriteOnly mode
end_comment
begin_function
...
end_function
begin_comment
comment|// Test waitForBytesWritten() does not fail on read buffer overflow
end_comment
begin_function
...
argument_list|)
expr_stmt|;
comment|// Wait for buffer overflow
name|QVERIFY
argument_list|(
name|socket
operator|->
...
argument_list|)
expr_stmt|;
comment|// Write data and wait for successful send
name|QVERIFY
argument_list|(
name|socket
operator|->
...
end_function
begin_comment
comment|// Test that the socket does not enable the read notifications in bind()
end_comment
begin_function
