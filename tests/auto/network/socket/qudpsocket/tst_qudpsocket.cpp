...
function_decl|;
name|void
name|readyReadForEmptyDatagram
parameter_list|()
function_decl|;
name|void
name|asyncReadDatagram
parameter_list|()
function_decl|;
name|void
name|writeInHostLookupState
parameter_list|()
function_decl|;
protected|protected
name|slots
...
parameter_list|()
block|{
comment|// hack: we only enable the Socks5 over UDP tests on the old
comment|// test server, because they fail on the new one. See QTBUG-35490
name|bool
name|newTestServer
init|=
literal|true
...
directive|endif
comment|// we use the dummy socket to use up a file descriptor
name|dummysocket
operator|.
name|bind
argument_list|()
...
name|QFAIL
argument_list|(
literal|"Could not find test server address"
argument_list|)
expr_stmt|;
name|QFETCH
argument_list|(
...
argument_list|)
decl_stmt|;
comment|// send a datagram to that port
name|sender
operator|.
name|writeDatagram
argument_list|(
...
comment|// if QTBUG-43857 is still going, we'll live-lock on socket notifications from receiver's socket
name|QTest
operator|::
name|qWait
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// make sure only one signal was emitted
name|QCOMPARE
argument_list|(
name|spy
operator|.
...
argument_list|)
expr_stmt|;
comment|// write another datagram
name|sender
operator|.
name|writeDatagram
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// no new signal should be emitted because we haven't read the first datagram yet
name|QTest
operator|::
name|qWait
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// most likely is 1, but it could be 1 + 2 in the future
name|QCOMPARE
argument_list|(
name|receiver
operator|.
...
argument_list|)
expr_stmt|;
comment|// read all the datagrams (we could read one only, but we can't be sure the OS is queueing)
while|while
condition|(
name|receiver
operator|.
...
argument_list|)
expr_stmt|;
comment|// write a new datagram and ensure the signal is emitted now
name|sender
operator|.
name|writeDatagram
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// send an empty datagram to that port
name|sender
operator|.
name|writeDatagram
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// ensure that we got a readyRead, despite bytesAvailable() == 0
name|QTestEventLoop
operator|::
name|instance
argument_list|()
...
argument_list|)
expr_stmt|;
comment|// wait a little to ensure that the datagram we've just sent
comment|// will be delivered on receiver side.
name|QTest
operator|::
name|qSleep
argument_list|(
