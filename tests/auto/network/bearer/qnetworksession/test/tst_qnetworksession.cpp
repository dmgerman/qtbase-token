...
argument_list|)
expr_stmt|;
comment|// If you wish to skip tests, set value as false. This is often very convinient because tests are so lengthy.
comment|// Better way still would be to make this readable from a file.
name|testsToRun
index|[
literal|"robustnessBombing"
index|]
operator|=
literal|true
expr_stmt|;
...
name|QSKIP
argument_list|(
literal|"Temporary skip due to value set false (or it is missing) in testsToRun map"
argument_list|,
name|SkipAll
argument_list|)
expr_stmt|;
...
directive|ifdef
name|Q_OS_SYMBIAN
comment|// On symbian, the connection opening is tried even with defined state.
name|qDebug
argument_list|(
literal|"Waiting for 10 seconds to all signals to propagate."
argument_list|)
expr_stmt|;
name|QTest
operator|::
...
argument_list|)
operator|)
comment|// On Symbian emulator, the support for data bearers is limited
name|QCOMPARE
argument_list|(
name|session
operator|.
...
name|QSKIP
argument_list|(
literal|"Roaming supported on Symbian."
argument_list|,
name|SkipAll
argument_list|)
expr_stmt|;
...
name|QSKIP
argument_list|(
literal|"No WLAN IAP accessible, skipping test."
argument_list|,
name|SkipAll
argument_list|)
expr_stmt|;
block|}
comment|// Check that opening and closing two sessions on same config work gracefully:
name|QNetworkSession
name|iapSession
argument_list|(
name|wlanIapConfig
...
argument_list|)
expr_stmt|;
comment|// false == not a last session based on the configuration
name|QVERIFY
argument_list|(
name|closeSession
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// Open configurations again, force close bearer and check that errors are emitted correctly
comment|// on the other session
name|QVERIFY
argument_list|(
name|openSession
argument_list|(
...
argument_list|()
expr_stmt|;
comment|// requires NetworkControl capabilities
name|QTRY_VERIFY
argument_list|(
operator|!
name|errorSpy
...
argument_list|)
expr_stmt|;
comment|// wait for error signals
name|QNetworkSession
operator|::
name|SessionError
name|error
...
argument_list|)
expr_stmt|;
comment|// Wait for a moment to all platform signals to propagate
name|QVERIFY
argument_list|(
name|error
operator|==
...
name|QSKIP
argument_list|(
literal|"Testcase contains mainly Symbian specific checks, because it is only platform to really support interface (IAP-level) Stop."
argument_list|,
name|SkipAll
argument_list|)
expr_stmt|;
...
name|QSKIP
argument_list|(
literal|"No suitable configurations, skipping this round of session stop test."
argument_list|,
name|SkipSingle
argument_list|)
expr_stmt|;
...
argument_list|()
operator|<<
literal|"Using following configuration to open and stop a session: "
operator|<<
name|config
operator|.
name|name
...
decl_stmt|;
name|QSignalSpy
name|closedSessionStateChangedSpy
argument_list|(
operator|&
name|closedSession
argument_list|,
...
decl_stmt|;
name|QSignalSpy
name|openedSessionStateChangedSpy
argument_list|(
operator|&
name|openedSession
argument_list|,
...
decl_stmt|;
name|QSignalSpy
name|innocentSessionStateChangedSpy
argument_list|(
operator|&
name|innocentSession
argument_list|,
...
name|sessionError
decl_stmt|;
comment|// 1. Verify that stopping an opened session works (the simplest usecase).
name|qDebug
argument_list|(
literal|"----------1. Verify that stopping an opened session works (the simplest usecase)"
argument_list|)
expr_stmt|;
name|QSignalSpy
...
name|qDebug
argument_list|(
literal|"Waiting for %d ms to get all configurationChange signals from platform."
argument_list|,
name|configWaitdelayInMs
argument_list|)
expr_stmt|;
comment|// Clear signals caused by opening
name|closedSessionOpenedSpy
operator|.
name|clear
argument_list|()
...
argument_list|)
expr_stmt|;
comment|// Wait to get all relevant configurationChange() signals
comment|// First to closing, then to disconnected
name|QVERIFY
argument_list|(
name|openedSessionStateChangedSpy
operator|.
...
argument_list|)
expr_stmt|;
comment|// 2. Verify that stopping a session based on non-connected configuration does nothing
name|qDebug
argument_list|(
literal|"----------2. Verify that stopping a session based on non-connected configuration does nothing"
argument_list|)
expr_stmt|;
name|QNetworkSession
operator|::
name|State
name|closedSessionOriginalState
init|=
name|closedSession
operator|.
name|state
argument_list|()
decl_stmt|;
comment|// Clear all possible signals
name|configChangeSpy
operator|.
name|clear
argument_list|()
...
argument_list|)
expr_stmt|;
comment|// State remains
comment|// 3. Check that stopping a opened session affects also other opened session based on the same configuration.
if|if
condition|(
name|config
operator|.
...
name|qDebug
argument_list|(
literal|"----------3. Check that stopping a opened session affects also other opened session based on the same configuration."
argument_list|)
expr_stmt|;
name|QVERIFY
argument_list|(
...
name|qDebug
argument_list|(
literal|"----------3. Skip for SNAP configuration."
argument_list|)
expr_stmt|;
block|}
comment|// 4. Check that stopping a non-opened session stops the other session based on the
comment|// same configuration if configuration is IAP. Stopping closed SNAP session has no impact on other opened SNAP session.
if|if
condition|(
name|config
operator|.
...
name|qDebug
argument_list|(
literal|"----------4. Skip for SNAP configuration."
argument_list|)
expr_stmt|;
block|}
elseif|else
...
name|qDebug
argument_list|(
literal|"----------4.   Check that stopping a non-opened session stops the other session based on the same configuration"
argument_list|)
expr_stmt|;
name|qDebug
argument_list|(
literal|"----------4.1 Opening innocent session"
argument_list|)
expr_stmt|;
name|QVERIFY
argument_list|(
...
name|qDebug
argument_list|(
literal|"Waiting for %d ms after open to make sure all platform indications are propagated"
argument_list|,
name|configWaitdelayInMs
argument_list|)
expr_stmt|;
...
name|qDebug
argument_list|(
literal|"----------4.2 Calling closedSession.stop()"
argument_list|)
expr_stmt|;
name|closedSession
operator|.
...
expr_stmt|;
block|}
comment|// 5. Sanity check that stopping invalid session does not crash
name|qDebug
argument_list|(
literal|"----------5. Sanity check that stopping invalid session does not crash"
argument_list|)
expr_stmt|;
name|QNetworkSession
name|invalidSession
...
name|Q_OS_SYMBIAN
argument_list|)
comment|// Opening& closing multiple connections in a row sometimes
comment|// results hanging of connection opening on Symbian devices
comment|// => If first open fails, wait a moment and try again.
if|if
condition|(
operator|!
name|session
...
name|qDebug
argument_list|(
literal|"**** Session open Timeout - Wait 5 seconds and try once again ****"
argument_list|)
expr_stmt|;
name|session
operator|.
...
argument_list|)
expr_stmt|;
comment|// Wait a while before trying to open session again
name|session
operator|.
name|open
argument_list|()
...
directive|ifdef
name|Q_CC_NOKIAX86
comment|// For S60 emulator builds: RConnection::Stop does not work on all Emulators
name|bool
name|expectStateChange
init|=
literal|false
...
argument_list|)
decl_stmt|;
comment|// Symbian version dependant.
name|QVERIFY
argument_list|(
name|state
operator|==
...
directive|ifdef
name|Q_CC_NOKIAX86
comment|// For S60 emulator builds: RConnection::Close does not actually
comment|//                          close network connection on all Emulators
name|bool
name|expectStateChange
init|=
literal|false
...
name|QSKIP
argument_list|(
literal|"Symbian emulator does not support two [QR]PRocesses linking a dll (QtBearer.dll) with global writeable static data."
argument_list|,
name|SkipAll
argument_list|)
expr_stmt|;
