...
argument_list|)
expr_stmt|;
comment|// setting empty SSL configuration explicitly -> depends on on-demand loading
name|QSslSocketPtr
name|socket4
init|=
name|newSocket
...
comment|// when we allow on demand loading, it is enabled by default,
comment|// so on Unix it will work without setting any certificates. Otherwise,
comment|// the configuration contains an empty set of certificates
comment|// and will fail.
name|bool
name|works
decl_stmt|;
if|#
