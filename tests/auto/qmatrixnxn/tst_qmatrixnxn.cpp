...
end_function
begin_comment
comment|// QVector2/3/4D use float internally, which can sometimes lead
end_comment
begin_comment
comment|// to precision issues when converting to and from qreal during
end_comment
begin_comment
comment|// operations involving QMatrix4x4.  This fuzzy compare is slightly
end_comment
begin_comment
comment|// "fuzzier" than the default qFuzzyCompare for qreal to compensate.
end_comment
begin_function
...
argument_list|)
expr_stmt|;
comment|// Use optimize() to drop the internal flags that
...
parameter_list|)
define|\
value|do { \         xout = ((const qreal *)resultValues)[0] * xin + \                ((const qreal *)resultValues)[1] * yin + \                ((const qreal *)resultValues)[2] * zin + \                ((const qreal *)resultValues)[3] * win; \         yout = ((const qreal *)resultValues)[4] * xin + \                ((const qreal *)resultValues)[5] * yin + \                ((const qreal *)resultValues)[6] * zin + \                ((const qreal *)resultValues)[7] * win; \         zout = ((const qreal *)resultValues)[8] * xin + \                ((const qreal *)resultValues)[9] * yin + \                ((const qreal *)resultValues)[10] * zin + \                ((const qreal *)resultValues)[11] * win; \         wout = ((const qreal *)resultValues)[12] * xin + \                ((const qreal *)resultValues)[13] * yin + \                ((const qreal *)resultValues)[14] * zin + \                ((const qreal *)resultValues)[15] * win; \     } while (0)
...
literal|0x0004
block|,
comment|// Contains a simple translation
DECL|enumerator|Scale
name|Scale
init|=
literal|0x0008
block|,
comment|// Contains a simple scale
DECL|enumerator|Rotation
name|Rotation
init|=
literal|0x0010
comment|// Contains a simple rotation
block|}
enum|;
end_enum
begin_comment
