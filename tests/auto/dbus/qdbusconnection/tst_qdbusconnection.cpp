...
name|QSKIP
argument_list|(
literal|"Test requires a QCoreApplication"
argument_list|)
expr_stmt|;
comment|// because of the qWait()
name|QDBusSpy
name|spy
decl_stmt|;
name|QDBusConnection
...
condition|)
return|return;
comment|// cannot receive signals in this thread without QCoreApplication
name|con
operator|.
name|connect
argument_list|(
...
argument_list|)
argument_list|,
literal|"Daemon didn't print its address in time; error: \""
operator|+
name|daemon
operator|.
name|errorString
...
argument_list|)
expr_stmt|;
comment|// confirm we're connected and we're alone in this bus
name|QCOMPARE
argument_list|(
name|con
operator|.
...
argument_list|()
expr_stmt|;
comment|// send something, which we should get an error with
name|QDBusMessage
name|message
init|=
name|QDBusMessage
