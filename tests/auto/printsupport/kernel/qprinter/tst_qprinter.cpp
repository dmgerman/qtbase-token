...
argument_list|)
decl_stmt|;
comment|// Due to the different calculations, the sizes may be off by a fraction so we have to check it manually
comment|// instead of relying on QSizeF comparison
name|QVERIFY2
argument_list|(
name|sqrt
argument_list|(
...
name|QSKIP
argument_list|(
literal|"Currently this triggers a problem on non Windows platforms, this will be fixed separately - QTBUG-34521"
argument_list|)
expr_stmt|;
endif|#
directive|endif
...
argument_list|)
expr_stmt|;
comment|// TODO Bypass direct compare until QPageSize used in QPrinter for consistency
comment|//QCOMPARE(sizes.at(i).second, printer.paperSize(QPrinter::Millimeter));
name|QCOMPARE
argument_list|(
name|qRound
argument_list|(
...
comment|// So we iterate up to the current position to check
name|QSizeF
name|paperSize
init|=
name|sizes
...
expr_stmt|;
comment|// TODO Bypass direct compare until QPageSize used in QPrinter for consistency
comment|//QCOMPARE(printer.paperSize(QPrinter::Millimeter), customSize);
name|QCOMPARE
argument_list|(
name|qRound
argument_list|(
...
literal|""
argument_list|,
literal|"Win paper size doesn't persist over format change"
argument_list|,
name|Continue
argument_list|)
expr_stmt|;
...
name|QSKIP
argument_list|(
literal|"Skip tests until new backends pass"
argument_list|)
expr_stmt|;
name|QFETCH
argument_list|(
...
literal|""
argument_list|,
literal|"Rect calculation lacks required precision"
argument_list|,
name|Continue
argument_list|)
expr_stmt|;
