...
end_if
begin_comment
comment|// In Symbian, the PluginsPath doesn't specify the only absolute path; just the dir that can be found on any drive
end_comment
begin_function
DECL|function|addExpectedSymbianPluginsPath
specifier|static
name|void
name|addExpectedSymbianPluginsPath
parameter_list|(
name|QStringList
modifier|&
name|expected
...
function_decl|;
name|void
name|symbianNoApplicationPanes
parameter_list|()
function_decl|;
name|void
name|symbianNeedForTraps
parameter_list|()
function_decl|;
name|void
name|symbianLeaveThroughMain
parameter_list|()
function_decl|;
name|void
name|qtbug_12673
...
name|MyInputContext
decl_stmt|;
comment|// QApplication takes ownership, so check for reparenting:
name|obj1
operator|.
...
argument_list|)
expr_stmt|;
comment|// Test for self-assignment:
name|obj1
operator|.
name|setInputContext
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// Resetting the input context to 0 is not allowed:
name|QTest
operator|::
name|ignoreMessage
argument_list|(
...
name|void
name|processEventsWithDeferredDeletion
parameter_list|()
block|{
name|QApplication
operator|::
...
name|void
name|deleteLaterAndProcessEvents1
parameter_list|()
block|{
name|QEventLoop
name|eventLoop
...
name|this
argument_list|,
literal|"processEventsWithDeferredDeletion"
argument_list|,
name|Qt
operator|::
name|QueuedConnection
...
name|void
name|deleteLaterAndProcessEvents2
parameter_list|()
block|{
name|QEventLoop
name|eventLoop
...
comment|// QEventLoop::DeferredDeletion is passed.
name|object
operator|=
operator|new
name|QObject
...
argument_list|)
expr_stmt|;
comment|// 0x10 == QEventLoop::DeferredDeletion
name|QVERIFY
argument_list|(
operator|!
name|p
argument_list|)
expr_stmt|;
comment|// sendPostedEvents(0, DeferredDelete); also works
name|object
operator|=
operator|new
name|QObject
...
name|start
argument_list|(
literal|"desktopsettingsaware/debug/desktopsettingsaware"
argument_list|)
expr_stmt|;
elif|#
directive|elif
...
name|start
argument_list|(
literal|"desktopsettingsaware/release/desktopsettingsaware"
argument_list|)
expr_stmt|;
elif|#
directive|elif
...
literal|""
argument_list|,
literal|"QProcess on Q_CC_NOKIAX86 cannot launch another Qt application, due to DLL conflicts."
argument_list|,
name|Abort
argument_list|)
expr_stmt|;
comment|// TODO: Remove XFAIL, as soon as we can launch Qt applications from within Qt applications on Symbian
name|QVERIFY
argument_list|(
name|testProcess
operator|.
...
name|start
argument_list|(
literal|"wincmdline/debug/wincmdline"
argument_list|,
name|QStringList
argument_list|(
name|args
...
name|start
argument_list|(
literal|"wincmdline/release/wincmdline"
argument_list|,
name|QStringList
argument_list|(
name|args
...
argument_list|)
expr_stmt|;
comment|// Run in a block so that QApplication is destroyed before resetting the attribute.
block|{
comment|// Actually I wasn't able to get the forced orientation change to work properly,
comment|// but I'll leave the code here for the future in case we manage to test that
comment|// later. If someone knows how to force an orientation switch in an autotest, do
comment|// feel free to fix this testcase.
name|int
name|argc
init|=
literal|0
...
name|w
expr_stmt|;
comment|// These will have no effect, since there is no status pane, but they shouldn't
comment|// crash either.
name|w
operator|=
operator|new
name|QWidget
...
argument_list|)
expr_stmt|;
comment|// No other error condition. Program will crash if unsuccessful.
endif|#
directive|endif
block|}
end_function
...
parameter_list|)
block|{
comment|// Push a few objects, just so that the cleanup stack has something to clean up.
name|CleanupStack
operator|::
name|PushL
argument_list|(
...
literal|0
decl_stmt|;
comment|// This next part should not require a trap. If it does, the test will crash.
name|CleanupStack
operator|::
name|PushL
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// No other failure condition. The program will crash if it does not pass.
endif|#
directive|endif
block|}
end_function
...
name|QSKIP
argument_list|(
literal|"This might not make sense in Symbian, but since I do not know how to test it I'll just skip it for now."
argument_list|,
name|SkipAll
argument_list|)
expr_stmt|;
