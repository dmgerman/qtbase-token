...
include|#
directive|include
file|<QtGui/QOpenGLFunctions_4_2_Core>
end_include
begin_include
include|#
directive|include
...
function_decl|;
name|void
name|fboRenderingRGB30_data
parameter_list|()
function_decl|;
name|void
name|fboRenderingRGB30
parameter_list|()
function_decl|;
name|void
...
function_decl|;
name|void
name|imageFormatPainting
parameter_list|()
function_decl|;
name|void
name|nullTextureInitializtion
parameter_list|()
function_decl|;
ifdef|#
directive|ifdef
...
argument_list|)
expr_stmt|;
comment|// Uncomplicate things by using POT:
specifier|const
name|QSize
name|size
argument_list|(
...
name|QSKIP
argument_list|(
literal|"An internal RGB30_A2 format is not guaranteed on this platform"
argument_list|)
expr_stmt|;
comment|// No multisample with combined depth/stencil attachment:
name|QOpenGLFramebufferObjectFormat
...
argument_list|)
expr_stmt|;
comment|// Check rendering can handle color values below 1/256.
name|fboPainter
operator|.
name|begin
argument_list|(
...
name|QSKIP
argument_list|(
literal|"Multiple render targets not supported on this platform"
argument_list|)
expr_stmt|;
...
decl_stmt|;
block|{
comment|// 3 color attachments, different sizes, same internal format, no depth/stencil.
name|QVector
argument_list|<
name|QSize
argument_list|>
...
argument_list|)
expr_stmt|;
comment|// Clear the three buffers to red, green and blue.
name|GLenum
name|drawBuf
init|=
...
argument_list|)
expr_stmt|;
comment|// Verify, keeping in mind that only a 128x123 area is touched in the buffers.
comment|// Some drivers do not get this right, unfortunately, so do not rely on it.
specifier|const
name|char
modifier|*
name|vendor
...
block|}
block|{
comment|// 2 color attachments, same size, same internal format, depth/stencil.
name|QVector
argument_list|<
name|QSize
argument_list|>
...
name|QSKIP
argument_list|(
literal|"The tested formats may not be supported on this platform"
argument_list|)
expr_stmt|;
if|if
condition|(
...
name|QSKIP
argument_list|(
literal|"RGB10_A2 not supported on this platform"
argument_list|)
expr_stmt|;
comment|// 3 color attachments, same size, different internal format, depth/stencil.
name|QVector
argument_list|<
name|QSize
argument_list|>
...
name|newRow
argument_list|(
literal|"Using QOffscreenSurface - RGB32"
argument_list|)
operator|<<
name|int
argument_list|(
...
name|newRow
argument_list|(
literal|"Using QOffscreenSurface - RGBx8888"
argument_list|)
operator|<<
name|int
argument_list|(
...
name|newRow
argument_list|(
literal|"Using QOffscreenSurface - RGB888"
argument_list|)
operator|<<
name|int
argument_list|(
...
name|newRow
argument_list|(
literal|"Using QOffscreenSurface - RGB16"
argument_list|)
operator|<<
name|int
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// Now do a makeCurrent() do make sure the pixel format on the native
comment|// window (the HWND we are going to retrieve below) is set.
name|QVERIFY
argument_list|(
name|ctx
operator|->
...
argument_list|)
expr_stmt|;
comment|// This tests two things: that a regular, non-adopted QOpenGLContext is
comment|// able to return a QSurfaceFormat containing the real values after
comment|// create(), and that the adopted context got the correct pixel format from
comment|// window and was able to update its format accordingly.
name|QCOMPARE
argument_list|(
name|adopted
operator|->
...
argument_list|)
expr_stmt|;
comment|// This must work since we are using the exact same window.
name|QVERIFY
argument_list|(
name|adopted
operator|->
...
end_endif
begin_comment
comment|// Q_OS_WIN32&& !QT_OPENGL_ES_2
end_comment
begin_function
...
name|QSKIP
argument_list|(
literal|"glMapBufferRange not supported"
argument_list|)
expr_stmt|;
name|QOpenGLBuffer
name|buf
