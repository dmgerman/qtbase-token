...
function_decl|;
name|void
name|qrgba64MemoryLayout
parameter_list|()
function_decl|;
name|void
name|qrgba64Premultiply
parameter_list|()
function_decl|;
name|void
name|qrgba64Equivalence
parameter_list|()
function_decl|;
ifdef|#
directive|ifdef
...
argument_list|)
expr_stmt|;
comment|// name should be case insensitive
name|color
operator|.
name|setNamedColor
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// spaces should be ignored
name|color
operator|.
name|setNamedColor
argument_list|(
...
parameter_list|()
block|{
comment|// Tests that qPremultiply(qUnpremultiply(x)) returns x.
for|for
control|(
name|uint
name|a
...
parameter_list|()
block|{
comment|// Tests that qUnpremultiply_sse4 returns the same as qUnpremultiply.
if|#
directive|if
...
name|QSKIP
argument_list|(
literal|"SSE4 not supported on this CPU."
argument_list|)
expr_stmt|;
block|}
end_function
...
end_function
begin_function
DECL|function|qrgba64MemoryLayout
name|void
name|tst_QColor
operator|::
name|qrgba64MemoryLayout
...
argument_list|)
expr_stmt|;
comment|// Check in-memory order, so it can be used by things like SSE
...
parameter_list|()
block|{
comment|// Tests that qPremultiply(qUnpremultiply(rgba64)) returns rgba64.
for|for
control|(
name|uint
name|a
...
parameter_list|()
block|{
comment|// Any ARGB32 converted back and forth.
for|for
control|(
name|uint
name|a
...
block|}
block|}
comment|// Any unpremultiplied ARGB32 value premultipled in RGB64 (except alpha 0).
for|for
control|(
name|uint
name|a
...
argument_list|()
decl_stmt|;
comment|// 64bit premultiplied is more accurate than 32bit, so allow slight difference.
name|QCOMPARE
argument_list|(
name|qAlpha
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// But verify the added accuracy means we can return to accurate unpremultiplied ARGB32.
name|QRgba64
name|pu64
init|=
name|qUnpremultiply
...
block|}
block|}
comment|// Any premultiplied ARGB32 value unpremultipled in RGB64.
for|for
control|(
name|uint
name|a
