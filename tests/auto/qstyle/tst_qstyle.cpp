...
end_comment
begin_comment
comment|//TESTED_FILES=gui/styles/qstyle.h gui/styles/qstyle.cpp gui/styles/qplastiquestyle.cpp gui/styles/qwindowsstyle.cpp gui/styles/qwindowsxpstyle.cpp gui/styles/qwindowsvistastyle.cpp gui/styles/qmotifstyle.cpp gui/styles/qs60style.cpp
end_comment
begin_class
...
directive|ifdef
name|Q_OS_SYMBIAN
comment|// QPixmap cannot be assumed to be exactly same, unless it is created from exactly same content.
comment|// In Symbian, pixmap format might get "optimized" depending on how QPixmap is created.
comment|// Therefore, force the content to specific format and compare QImages.
comment|// Then re-create the QPixmaps and compare those.
name|QImage
name|i1
init|=
name|p
...
directive|ifdef
name|Q_WS_S60
comment|// in s60style add line and sub line have been removed.
if|if
condition|(
name|subControl
operator|==
...
name|QSKIP
argument_list|(
literal|"Either style Plastique or Windows or S60 missing"
argument_list|,
name|SkipAll
argument_list|)
expr_stmt|;
