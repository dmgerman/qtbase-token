...
end_if
begin_comment
comment|// Unused in the Qt configuration
end_comment
begin_endif
unit|static void fromBytesToWord(UINT64 *word, const UINT8 *bytes) {     unsigned int i;      *word = 0;     for(i=0; i<(64/8); i++)         *word |= (UINT64)(bytes[i])<< (8*i); }
endif|#
directive|endif
end_endif
begin_ifdef
...
end_comment
begin_endif
unit|static void fromWordToBytes(UINT8 *bytes, const UINT64 word) {     unsigned int i;      for(i=0; i<(64/8); i++)         bytes[i] = (word>> (8*i))& 0xFF; }
endif|#
directive|endif
end_endif
begin_ifdef
