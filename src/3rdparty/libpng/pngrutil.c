...
name|msg
operator|=
literal|"Buffer error in compressed datastream"
expr_stmt|;
break|break;
case|case
name|Z_DATA_ERROR
...
name|msg
operator|=
literal|"Data error in compressed datastream"
expr_stmt|;
break|break;
default|default:
name|msg
operator|=
literal|"Incomplete compressed datastream"
expr_stmt|;
break|break;
block|}
name|png_chunk_warning
...
end_endif
begin_comment
comment|/* PNG_READ_COMPRESSED_TEXT_SUPPORTED */
end_comment
begin_comment
...
name|p
argument_list|,
literal|"Ignoring incorrect gAMA value @1 when sRGB is also present"
argument_list|)
expr_stmt|;
return|return;
block|}
...
name|png_ptr
argument_list|,
literal|"Out of place cHRM chunk"
argument_list|)
expr_stmt|;
if|if
condition|(
...
name|p
argument_list|,
literal|"Ignoring incorrect cHRM white(@1,@2) r(@3,@4)g(@5,@6)b(@7,@8) "
literal|"when sRGB is also present"
argument_list|)
expr_stmt|;
block|}
return|return;
...
comment|/* Store the _white values as default coefficients for the rgb to gray     * operation if it is supported.  Check if the transform is already set to     * avoid destroying the transform values.     */
if|if
condition|(
operator|!
name|png_ptr
...
condition|)
block|{
comment|/* png_set_background has not been called and we haven't seen an sRGB        * chunk yet.  Find the XYZ of the three end points.        */
name|png_XYZ
name|XYZ
decl_stmt|;
name|png_xy
...
condition|)
block|{
comment|/* The success case, because XYZ_from_xy normalises to a reference           * white Y of 1.0 we just need to scale the numbers.  This should           * always work just fine. It is an internal error if this overflows.           */
block|{
name|png_fixed_point
name|r
decl_stmt|,
...
comment|/* This is recorded for use when handling the cHRM chunk above.  An sRGB     * chunk unconditionally overwrites the coefficients for grayscale conversion     * too.     */
name|png_ptr
operator|->
name|is_sRGB
operator|=
...
directive|ifdef
name|PNG_READ_RGB_TO_GRAY_SUPPORTED
comment|/* Don't overwrite user supplied values: */
if|if
condition|(
operator|!
name|png_ptr
...
literal|23434
expr_stmt|;
comment|/* 0.715168678767756 */
comment|/* png_ptr->rgb_to_gray_blue_coeff  =  2366; 0.072192315360734	*/
comment|/* The following keeps the cHRM chunk from destroying the           * coefficients again in the event that it follows the sRGB chunk.           */
name|png_ptr
operator|->
name|rgb_to_gray_coefficients_set
operator|=
...
name|p
argument_list|,
literal|"Ignoring iCCP chunk with declared size = @1 and actual length = @2"
argument_list|)
expr_stmt|;
return|return;
block|}
...
name|png_ptr
argument_list|,
literal|"Invalid sCAL chunk ignored: non-positive width"
argument_list|)
expr_stmt|;
else|else
block|{
...
name|png_ptr
argument_list|,
literal|"Invalid sCAL chunk ignored: non-positive height"
argument_list|)
expr_stmt|;
else|else
...
name|png_ptr
argument_list|,
literal|"Unknown iTXt compression type or method"
argument_list|)
expr_stmt|;
name|png_free
argument_list|(
...
directive|endif
comment|/* TODO: this code is very close to the unknown handling in pngpread.c,        * maybe it can be put into a common utility routine?        * png_struct::unknown_chunk is just used as a temporary variable, along        * with the data into which the chunk is read.  These can be eliminated.        */
...
block|}
comment|/* For non-interlaced images this reduces to a png_memcpy(). A png_memcpy()     * will also happen if interlacing isn't supported or if the application     * does not call png_set_interlace_handling().  In the latter cases the     * caller just gets a sequence of the unexpanded rows from each interlace     * pass.     */
ifdef|#
directive|ifdef
...
name|s
parameter_list|)
value|{ B_MASK(1,d,s), S_MASK(3,d,s), S_MASK(5,d,s) }
...
operator|!=
name|PNG_ALIGN_NONE
comment|/* Check for double byte alignment and, if possible, use a                 * 16-bit copy.  Don't attempt this for narrow images - ones that                 * are less than an interlace panel wide.  Don't attempt it for                 * wide bytes_to_copy either - use the png_memcpy there.                 */
if|if
condition|(
name|bytes_to_copy
operator|<
literal|16
comment|/*else use png_memcpy*/
operator|&&
name|png_isaligned
argument_list|(
name|dp
...
directive|endif
comment|/* PNG_ALIGN_ code */
comment|/* The true default - use a png_memcpy: */
for|for
control|(
init|;
condition|;
...
directive|endif
comment|/* If here then the switch above wasn't used so just png_memcpy the whole row     * from the temporary row buffer (notice that this overwrites the end of the     * destination row if it is a partial byte.)     */
name|png_memcpy
argument_list|(
name|dp
argument_list|,
