...
expr_stmt|;
block|}
comment|/* New API, make sure apps call the correct initializers: */
name|png_ptr
operator|->
name|flags
operator||=
...
operator|>=
literal|10600
comment|/* Checking the gamma values for being>0 was added in 1.5.4 along with the     * premultiplied alpha support; this actually hides an undocumented feature     * of the previous implementation which allowed gamma processing to be     * disabled in background handling.  There is no evidence (so far) that this     * was being used; however, png_set_background itself accepted and must still     * accept '0' for the gamma value it takes, because it isn't always used.     *     * Since this is an API change (albeit a very minor one that removes an     * undocumented API feature) it will not be made until libpng-1.6.0.     */
if|if
condition|(
name|file_gamma
operator|<=
...
condition|)
block|{
comment|/* Any alpha means background and associative alpha processing is        * required, however if the alpha is 0 or 1 throughout OPTIIMIZE_ALPHA        * and ENCODE_ALPHA are irrelevant.        */
name|png_ptr
operator|->
name|transformations
operator|&=
...
directive|endif
comment|/* PNG_READ_INVERT_ALPHA_SUPPORTED */
block|}
block|}
comment|/* background expand and (therefore) no alpha association. */
endif|#
directive|endif
comment|/* PNG_READ_EXPAND_SUPPORTED&& PNG_READ_BACKGROUND_SUPPORTED */
block|}
end_function
begin_function
specifier|static
...
literal|8
case|:
comment|/* Already 8 bits, fall through */
case|case
literal|16
case|:
...
directive|endif
comment|/* PNG_READ_GRAY_TO_RGB_SUPPORTED (etc) */
...
name|x
parameter_list|)
value|(x)=((png_uint_16)(((png_uint_32)(x)*255+32895)>> 16))
name|CHOP
argument_list|(
name|png_ptr
operator|->
...
directive|endif
comment|/* PNG_READ_BACKGROUND_SUPPORTED&& PNG_READ_EXPAND_16_SUPPORTED */
if|#
directive|if
name|defined
argument_list|(
...
directive|ifdef
name|PNG_READ_GAMMA_SUPPORTED
comment|/* The following used to be conditional on PNG_GAMMA (prior to 1.5.4),     * however it seems that the code in png_init_read_transformations, which has     * been called before this from png_read_update_info->png_read_start_row     * sometimes does the gamma transform and cancels the flag.     */
name|info_ptr
operator|->
name|gamma
operator|=
...
expr_stmt|;
block|}
comment|/* The following is debugging; prior to 1.5.4 the code was never compiled in;     * in 1.5.4 PNG_FLAG_DETECT_UNINITIALIZED was added and the macro     * PNG_WARN_UNINITIALIZED_ROW removed.  In 1.5 the new flag is set only for     * selected new APIs to ensure that there is no API change.     */
if|if
condition|(
operator|(
name|png_ptr
...
end_endif
begin_comment
comment|/* PNG_READ_TRANSFORMS_SUPPORTED */
end_comment
begin_ifdef
ifdef|#
directive|ifdef
...
end_endif
begin_comment
comment|/* PNG_READ_BACKGROUND_SUPPORTED || PNG_READ_ALPHA_MODE_SUPPORTED */
end_comment
begin_ifdef
ifdef|#
directive|ifdef
