...
name|buf_size
decl_stmt|;
comment|/* Repeat the checks in png_write_start_row; 1 pixel high or wide           * images cannot benefit from certain filters.  If this isn't done here           * the check below will fire on 1 pixel high images.           */
if|if
condition|(
name|png_ptr
operator|->
...
condition|)
block|{
comment|/* This is the error case, however it is benign - the previous row              * is not available so the filter can't be used.  Just warn here.              */
name|png_app_warning
argument_list|(
name|png_ptr
argument_list|,
literal|"png_set_filter: UP/AVG/PAETH cannot be added after start"
argument_list|)
expr_stmt|;
name|filters
operator|&=
...
operator|++
expr_stmt|;
comment|/* Allocate needed row buffers if they have not already been           * allocated.           */
name|buf_size
operator|=
name|PNG_ROWBYTES
argument_list|(
...
name|PNG_UNUSED
argument_list|(
argument|heuristic_method
argument_list|)
name|PNG_UNUSED
argument_list|(
argument|num_weights
argument_list|)
name|PNG_UNUSED
argument_list|(
argument|filter_weights
argument_list|)
name|PNG_UNUSED
argument_list|(
argument|filter_costs
argument_list|)
block|}
end_function
begin_endif
...
directive|endif
comment|/* Swap bits of 1-bit, 2-bit, 4-bit packed pixel formats */
if|if
condition|(
operator|(
name|transforms
