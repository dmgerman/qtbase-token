begin_unit
begin_comment
comment|/* pngpriv.h - private declarations for use inside libpng  *  * For conditions of distribution and use, see copyright notice in png.h  * Copyright (c) 1998-2011 Glenn Randers-Pehrson  * (Version 0.96 Copyright (c) 1996, 1997 Andreas Dilger)  * (Version 0.88 Copyright (c) 1995, 1996 Guy Eric Schalnat, Group 42, Inc.)  *  * Last changed in libpng 1.5.0 [January 6, 2011]  *  * This code is released under the libpng license.  * For conditions of distribution and use, see the disclaimer  * and license in png.h  */
end_comment
begin_comment
comment|/* The symbols declared in this file (including the functions declared  * as PNG_EXTERN) are PRIVATE.  They are not part of the libpng public  * interface, and are not recommended for use by regular applications.  * Some of them may become public in the future; others may stay private,  * change in an incompatible way, or even disappear.  * Although the libpng users are not forbidden to include this header,  * they should be well aware of the issues that may arise from doing so.  */
end_comment
begin_ifndef
ifndef|#
directive|ifndef
name|PNGPRIV_H
end_ifndef
begin_define
DECL|macro|PNGPRIV_H
define|#
directive|define
name|PNGPRIV_H
end_define
begin_ifdef
ifdef|#
directive|ifdef
name|_MSC_VER
end_ifdef
begin_define
DECL|macro|_CRT_SECURE_NO_DEPRECATE
define|#
directive|define
name|_CRT_SECURE_NO_DEPRECATE
end_define
begin_endif
endif|#
directive|endif
end_endif
begin_comment
comment|/* This is required for the definition of abort(), used as a last ditch  * error handler when all else fails.  */
end_comment
begin_include
include|#
directive|include
file|<stdlib.h>
end_include
begin_define
DECL|macro|PNGLIB_BUILD
define|#
directive|define
name|PNGLIB_BUILD
end_define
begin_ifdef
ifdef|#
directive|ifdef
name|PNG_USER_CONFIG
end_ifdef
begin_include
include|#
directive|include
file|"pngusr.h"
end_include
begin_comment
comment|/* These should have been defined in pngusr.h */
end_comment
begin_ifndef
ifndef|#
directive|ifndef
name|PNG_USER_PRIVATEBUILD
end_ifndef
begin_define
DECL|macro|PNG_USER_PRIVATEBUILD
define|#
directive|define
name|PNG_USER_PRIVATEBUILD
value|"Custom libpng build"
end_define
begin_endif
endif|#
directive|endif
end_endif
begin_ifndef
ifndef|#
directive|ifndef
name|PNG_USER_DLLFNAME_POSTFIX
end_ifndef
begin_define
DECL|macro|PNG_USER_DLLFNAME_POSTFIX
define|#
directive|define
name|PNG_USER_DLLFNAME_POSTFIX
value|"Cb"
end_define
begin_endif
endif|#
directive|endif
end_endif
begin_endif
endif|#
directive|endif
end_endif
begin_include
include|#
directive|include
file|"png.h"
end_include
begin_include
include|#
directive|include
file|"pnginfo.h"
end_include
begin_include
include|#
directive|include
file|"pngstruct.h"
end_include
begin_comment
comment|/* This is used for 16 bit gamma tables - only the top level pointers are const,  * this could be changed:  */
end_comment
begin_typedef
DECL|typedef|png_const_uint_16pp
typedef|typedef
name|PNG_CONST
name|png_uint_16p
name|FAR
modifier|*
name|png_const_uint_16pp
typedef|;
end_typedef
begin_comment
comment|/* Added at libpng-1.2.9 */
end_comment
begin_comment
comment|/* Moved to pngpriv.h at libpng-1.5.0 */
end_comment
begin_comment
comment|/* config.h is created by and PNG_CONFIGURE_LIBPNG is set by the "configure"  * script.  We may need it here to get the correct configuration on things  * like limits.  */
end_comment
begin_ifdef
ifdef|#
directive|ifdef
name|PNG_CONFIGURE_LIBPNG
end_ifdef
begin_ifdef
ifdef|#
directive|ifdef
name|HAVE_CONFIG_H
end_ifdef
begin_include
include|#
directive|include
file|"config.h"
end_include
begin_endif
endif|#
directive|endif
end_endif
begin_endif
endif|#
directive|endif
end_endif
begin_comment
comment|/* Moved to pngpriv.h at libpng-1.5.0 */
end_comment
begin_comment
comment|/* NOTE: some of these may have been used in external applications as  * these definitions were exposed in pngconf.h prior to 1.5.  */
end_comment
begin_comment
comment|/* If you are running on a machine where you cannot allocate more  * than 64K of memory at once, uncomment this.  While libpng will not  * normally need that much memory in a chunk (unless you load up a very  * large file), zlib needs to know how big of a chunk it can use, and  * libpng thus makes sure to check any memory allocation to verify it  * will fit into memory.  *  * zlib provides 'MAXSEG_64K' which, if defined, indicates the  * same limit and pngconf.h (already included) sets the limit  * if certain operating systems are detected.  */
end_comment
begin_if
if|#
directive|if
name|defined
argument_list|(
name|MAXSEG_64K
argument_list|)
operator|&&
operator|!
name|defined
argument_list|(
name|PNG_MAX_MALLOC_64K
argument_list|)
end_if
begin_define
DECL|macro|PNG_MAX_MALLOC_64K
define|#
directive|define
name|PNG_MAX_MALLOC_64K
end_define
begin_endif
endif|#
directive|endif
end_endif
begin_comment
comment|/* Unused formal parameter warnings are silenced using the following macro  * which is expected to have no bad effects on performance (optimizing  * compilers will probably remove it entirely).  Note that if you replace  * it with something other than whitespace, you must include the terminating  * semicolon.  */
end_comment
begin_define
DECL|macro|PNG_UNUSED
define|#
directive|define
name|PNG_UNUSED
parameter_list|(
name|param
parameter_list|)
value|(void)param;
end_define
begin_comment
comment|/* Just a little check that someone hasn't tried to define something  * contradictory.  */
end_comment
begin_if
if|#
directive|if
operator|(
name|PNG_ZBUF_SIZE
operator|>
literal|65536L
operator|)
operator|&&
name|defined
argument_list|(
name|PNG_MAX_MALLOC_64K
argument_list|)
end_if
begin_undef
DECL|macro|PNG_ZBUF_SIZE
undef|#
directive|undef
name|PNG_ZBUF_SIZE
end_undef
begin_define
DECL|macro|PNG_ZBUF_SIZE
define|#
directive|define
name|PNG_ZBUF_SIZE
value|65536L
end_define
begin_endif
endif|#
directive|endif
end_endif
begin_comment
comment|/* If warnings or errors are turned off the code is disabled  * or redirected here.  */
end_comment
begin_ifndef
ifndef|#
directive|ifndef
name|PNG_WARNINGS_SUPPORTED
end_ifndef
begin_define
DECL|macro|png_warning
define|#
directive|define
name|png_warning
parameter_list|(
name|s1
parameter_list|,
name|s2
parameter_list|)
value|((void)0)
end_define
begin_define
DECL|macro|png_chunk_warning
define|#
directive|define
name|png_chunk_warning
parameter_list|(
name|s1
parameter_list|,
name|s2
parameter_list|)
value|((void)0)
end_define
begin_endif
endif|#
directive|endif
end_endif
begin_ifndef
ifndef|#
directive|ifndef
name|PNG_ERROR_TEXT_SUPPORTED
end_ifndef
begin_define
DECL|macro|png_error
define|#
directive|define
name|png_error
parameter_list|(
name|s1
parameter_list|,
name|s2
parameter_list|)
value|png_err(s1)
end_define
begin_define
DECL|macro|png_chunk_error
define|#
directive|define
name|png_chunk_error
parameter_list|(
name|s1
parameter_list|,
name|s2
parameter_list|)
value|png_err(s1)
end_define
begin_define
DECL|macro|png_fixed_error
define|#
directive|define
name|png_fixed_error
parameter_list|(
name|s1
parameter_list|,
name|s2
parameter_list|)
value|png_err(s1)
end_define
begin_endif
endif|#
directive|endif
end_endif
begin_ifndef
ifndef|#
directive|ifndef
name|PNG_EXTERN
end_ifndef
begin_comment
comment|/* The functions exported by PNG_EXTERN are internal functions, which  * aren't usually used outside the library (as far as I know), so it is  * debatable if they should be exported at all.  In the future, when it  * is possible to have run-time registry of chunk-handling functions,  * some of these might be made available again. #  define PNG_EXTERN extern  */
end_comment
begin_define
DECL|macro|PNG_EXTERN
define|#
directive|define
name|PNG_EXTERN
end_define
begin_endif
endif|#
directive|endif
end_endif
begin_comment
comment|/* Some fixed point APIs are still required even if not exported because  * they get used by the corresponding floating point APIs.  This magic  * deals with this:  */
end_comment
begin_ifdef
ifdef|#
directive|ifdef
name|PNG_FIXED_POINT_SUPPORTED
end_ifdef
begin_define
DECL|macro|PNGFAPI
define|#
directive|define
name|PNGFAPI
value|PNGAPI
end_define
begin_else
else|#
directive|else
end_else
begin_define
DECL|macro|PNGFAPI
define|#
directive|define
name|PNGFAPI
end_define
begin_comment
DECL|macro|PNGFAPI
comment|/* PRIVATE */
end_comment
begin_endif
endif|#
directive|endif
end_endif
begin_comment
comment|/* Other defines specific to compilers can go here.  Try to keep  * them inside an appropriate ifdef/endif pair for portability.  */
end_comment
begin_if
if|#
directive|if
name|defined
argument_list|(
name|PNG_FLOATING_POINT_SUPPORTED
argument_list|)
operator|||
expr|\
name|defined
argument_list|(
name|PNG_FLOATING_ARITHMETIC_SUPPORTED
argument_list|)
end_if
begin_comment
comment|/* png.c requires the following ANSI-C constants if the conversion of     * floating point to ASCII is implemented therein:     *     *  DBL_DIG  Maximum number of decimal digits (can be set to any constant)     *  DBL_MIN  Smallest normalized fp number (can be set to an arbitrary value)     *  DBL_MAX  Maximum floating point number (can be set to an arbitrary value)     */
end_comment
begin_include
include|#
directive|include
file|<float.h>
end_include
begin_if
if|#
directive|if
operator|(
name|defined
argument_list|(
name|__MWERKS__
argument_list|)
operator|&&
name|defined
argument_list|(
name|macintosh
argument_list|)
operator|)
operator|||
name|defined
argument_list|(
name|applec
argument_list|)
operator|||
expr|\
name|defined
argument_list|(
name|THINK_C
argument_list|)
operator|||
name|defined
argument_list|(
name|__SC__
argument_list|)
operator|||
name|defined
argument_list|(
name|TARGET_OS_MAC
argument_list|)
end_if
begin_comment
comment|/* We need to check that<math.h> hasn't already been included earlier       * as it seems it doesn't agree with<fp.h>, yet we should really use       *<fp.h> if possible.       */
end_comment
begin_if
if|#
directive|if
operator|!
name|defined
argument_list|(
name|__MATH_H__
argument_list|)
operator|&&
operator|!
name|defined
argument_list|(
name|__MATH_H
argument_list|)
operator|&&
operator|!
name|defined
argument_list|(
name|__cmath__
argument_list|)
end_if
begin_include
include|#
directive|include
file|<fp.h>
end_include
begin_endif
endif|#
directive|endif
end_endif
begin_else
else|#
directive|else
end_else
begin_include
include|#
directive|include
file|<math.h>
end_include
begin_endif
endif|#
directive|endif
end_endif
begin_if
if|#
directive|if
name|defined
argument_list|(
name|_AMIGA
argument_list|)
operator|&&
name|defined
argument_list|(
name|__SASC
argument_list|)
operator|&&
name|defined
argument_list|(
name|_M68881
argument_list|)
end_if
begin_comment
comment|/* Amiga SAS/C: We must include builtin FPU functions when compiling using       * MATH=68881       */
end_comment
begin_include
include|#
directive|include
file|<m68881.h>
end_include
begin_endif
endif|#
directive|endif
end_endif
begin_endif
endif|#
directive|endif
end_endif
begin_comment
comment|/* This provides the non-ANSI (far) memory allocation routines. */
end_comment
begin_if
if|#
directive|if
name|defined
argument_list|(
name|__TURBOC__
argument_list|)
operator|&&
name|defined
argument_list|(
name|__MSDOS__
argument_list|)
end_if
begin_include
include|#
directive|include
file|<mem.h>
end_include
begin_include
include|#
directive|include
file|<alloc.h>
end_include
begin_endif
endif|#
directive|endif
end_endif
begin_if
if|#
directive|if
name|defined
argument_list|(
name|WIN32
argument_list|)
operator|||
name|defined
argument_list|(
name|_Windows
argument_list|)
operator|||
name|defined
argument_list|(
name|_WINDOWS
argument_list|)
operator|||
expr|\
name|defined
argument_list|(
name|_WIN32
argument_list|)
operator|||
name|defined
argument_list|(
name|__WIN32__
argument_list|)
end_if
begin_if
if|#
directive|if
operator|!
name|defined
argument_list|(
name|__SYMBIAN32__
argument_list|)
end_if
begin_include
include|#
directive|include
file|<windows.h>
end_include
begin_comment
comment|/* defines _WINDOWS_ macro */
end_comment
begin_endif
endif|#
directive|endif
end_endif
begin_endif
endif|#
directive|endif
end_endif
begin_comment
comment|/* Moved here around 1.5.0beta36 from pngconf.h */
end_comment
begin_comment
comment|/* Users may want to use these so they are not private.  Any library  * functions that are passed far data must be model-independent.  */
end_comment
begin_comment
comment|/* Memory model/platform independent fns */
end_comment
begin_ifndef
ifndef|#
directive|ifndef
name|PNG_ABORT
end_ifndef
begin_if
if|#
directive|if
name|defined
argument_list|(
name|_WINDOWS_
argument_list|)
operator|||
name|defined
argument_list|(
name|_WIN32_WCE
argument_list|)
end_if
begin_define
DECL|macro|PNG_ABORT
define|#
directive|define
name|PNG_ABORT
parameter_list|()
value|ExitProcess(0)
end_define
begin_else
else|#
directive|else
end_else
begin_define
DECL|macro|PNG_ABORT
define|#
directive|define
name|PNG_ABORT
parameter_list|()
value|abort()
end_define
begin_endif
endif|#
directive|endif
end_endif
begin_endif
endif|#
directive|endif
end_endif
begin_ifdef
ifdef|#
directive|ifdef
name|USE_FAR_KEYWORD
end_ifdef
begin_comment
comment|/* Use this to make far-to-near assignments */
end_comment
begin_define
DECL|macro|CHECK
define|#
directive|define
name|CHECK
value|1
end_define
begin_define
DECL|macro|NOCHECK
define|#
directive|define
name|NOCHECK
value|0
end_define
begin_define
DECL|macro|CVT_PTR
define|#
directive|define
name|CVT_PTR
parameter_list|(
name|ptr
parameter_list|)
value|(png_far_to_near(png_ptr,ptr,CHECK))
end_define
begin_define
DECL|macro|CVT_PTR_NOCHECK
define|#
directive|define
name|CVT_PTR_NOCHECK
parameter_list|(
name|ptr
parameter_list|)
value|(png_far_to_near(png_ptr,ptr,NOCHECK))
end_define
begin_define
DECL|macro|png_strcpy
define|#
directive|define
name|png_strcpy
value|_fstrcpy
end_define
begin_define
DECL|macro|png_strncpy
define|#
directive|define
name|png_strncpy
value|_fstrncpy
end_define
begin_comment
DECL|macro|png_strncpy
comment|/* Added to v 1.2.6 */
end_comment
begin_define
DECL|macro|png_strlen
define|#
directive|define
name|png_strlen
value|_fstrlen
end_define
begin_define
DECL|macro|png_memcmp
define|#
directive|define
name|png_memcmp
value|_fmemcmp
end_define
begin_comment
DECL|macro|png_memcmp
comment|/* SJT: added */
end_comment
begin_define
DECL|macro|png_memcpy
define|#
directive|define
name|png_memcpy
value|_fmemcpy
end_define
begin_define
DECL|macro|png_memset
define|#
directive|define
name|png_memset
value|_fmemset
end_define
begin_define
DECL|macro|png_sprintf
define|#
directive|define
name|png_sprintf
value|sprintf
end_define
begin_else
else|#
directive|else
end_else
begin_ifdef
ifdef|#
directive|ifdef
name|_WINDOWS_
end_ifdef
begin_comment
comment|/* Favor Windows over C runtime fns */
end_comment
begin_define
DECL|macro|CVT_PTR
define|#
directive|define
name|CVT_PTR
parameter_list|(
name|ptr
parameter_list|)
value|(ptr)
end_define
begin_define
DECL|macro|CVT_PTR_NOCHECK
define|#
directive|define
name|CVT_PTR_NOCHECK
parameter_list|(
name|ptr
parameter_list|)
value|(ptr)
end_define
begin_define
DECL|macro|png_strcpy
define|#
directive|define
name|png_strcpy
value|lstrcpyA
end_define
begin_define
DECL|macro|png_strncpy
define|#
directive|define
name|png_strncpy
value|lstrcpynA
end_define
begin_define
DECL|macro|png_strlen
define|#
directive|define
name|png_strlen
value|lstrlenA
end_define
begin_define
DECL|macro|png_memcmp
define|#
directive|define
name|png_memcmp
value|memcmp
end_define
begin_define
DECL|macro|png_memcpy
define|#
directive|define
name|png_memcpy
value|CopyMemory
end_define
begin_define
DECL|macro|png_memset
define|#
directive|define
name|png_memset
value|memset
end_define
begin_define
DECL|macro|png_sprintf
define|#
directive|define
name|png_sprintf
value|wsprintfA
end_define
begin_else
else|#
directive|else
end_else
begin_define
DECL|macro|CVT_PTR
define|#
directive|define
name|CVT_PTR
parameter_list|(
name|ptr
parameter_list|)
value|(ptr)
end_define
begin_define
DECL|macro|CVT_PTR_NOCHECK
define|#
directive|define
name|CVT_PTR_NOCHECK
parameter_list|(
name|ptr
parameter_list|)
value|(ptr)
end_define
begin_define
DECL|macro|png_strcpy
define|#
directive|define
name|png_strcpy
value|strcpy
end_define
begin_define
DECL|macro|png_strncpy
define|#
directive|define
name|png_strncpy
value|strncpy
end_define
begin_comment
DECL|macro|png_strncpy
comment|/* Added to v 1.2.6 */
end_comment
begin_define
DECL|macro|png_strlen
define|#
directive|define
name|png_strlen
value|strlen
end_define
begin_define
DECL|macro|png_memcmp
define|#
directive|define
name|png_memcmp
value|memcmp
end_define
begin_comment
DECL|macro|png_memcmp
comment|/* SJT: added */
end_comment
begin_define
DECL|macro|png_memcpy
define|#
directive|define
name|png_memcpy
value|memcpy
end_define
begin_define
DECL|macro|png_memset
define|#
directive|define
name|png_memset
value|memset
end_define
begin_define
DECL|macro|png_sprintf
define|#
directive|define
name|png_sprintf
value|sprintf
end_define
begin_endif
endif|#
directive|endif
end_endif
begin_endif
endif|#
directive|endif
end_endif
begin_comment
comment|/* End of memory model/platform independent support */
end_comment
begin_ifndef
ifndef|#
directive|ifndef
name|PNG_NO_SNPRINTF
end_ifndef
begin_ifdef
ifdef|#
directive|ifdef
name|_MSC_VER
end_ifdef
begin_define
DECL|macro|png_snprintf
define|#
directive|define
name|png_snprintf
value|_snprintf
end_define
begin_comment
DECL|macro|png_snprintf
comment|/* Added to v 1.2.19 */
end_comment
begin_define
DECL|macro|png_snprintf2
define|#
directive|define
name|png_snprintf2
value|_snprintf
end_define
begin_define
DECL|macro|png_snprintf6
define|#
directive|define
name|png_snprintf6
value|_snprintf
end_define
begin_else
else|#
directive|else
end_else
begin_define
DECL|macro|png_snprintf
define|#
directive|define
name|png_snprintf
value|snprintf
end_define
begin_comment
DECL|macro|png_snprintf
comment|/* Added to v 1.2.19 */
end_comment
begin_define
DECL|macro|png_snprintf2
define|#
directive|define
name|png_snprintf2
value|snprintf
end_define
begin_define
DECL|macro|png_snprintf6
define|#
directive|define
name|png_snprintf6
value|snprintf
end_define
begin_endif
endif|#
directive|endif
end_endif
begin_else
else|#
directive|else
end_else
begin_comment
comment|/* You don't have or don't want to use snprintf().  Caution: Using    * sprintf instead of snprintf exposes your application to accidental    * or malevolent buffer overflows.  If you don't have snprintf()    * as a general rule you should provide one (you can get one from    * Portable OpenSSH).    */
end_comment
begin_define
DECL|macro|png_snprintf
define|#
directive|define
name|png_snprintf
parameter_list|(
name|s1
parameter_list|,
name|n
parameter_list|,
name|fmt
parameter_list|,
name|x1
parameter_list|)
value|png_sprintf(s1,fmt,x1)
end_define
begin_define
DECL|macro|png_snprintf2
define|#
directive|define
name|png_snprintf2
parameter_list|(
name|s1
parameter_list|,
name|n
parameter_list|,
name|fmt
parameter_list|,
name|x1
parameter_list|,
name|x2
parameter_list|)
value|png_sprintf(s1,fmt,x1,x2)
end_define
begin_define
DECL|macro|png_snprintf6
define|#
directive|define
name|png_snprintf6
parameter_list|(
name|s1
parameter_list|,
name|n
parameter_list|,
name|fmt
parameter_list|,
name|x1
parameter_list|,
name|x2
parameter_list|,
name|x3
parameter_list|,
name|x4
parameter_list|,
name|x5
parameter_list|,
name|x6
parameter_list|)
define|\
value|png_sprintf(s1,fmt,x1,x2,x3,x4,x5,x6)
end_define
begin_endif
endif|#
directive|endif
end_endif
begin_comment
comment|/* End of 1.5.0beta36 move from pngconf.h */
end_comment
begin_comment
comment|/* CONSTANTS and UTILITY MACROS  * These are used internally by libpng and not exposed in the API  */
end_comment
begin_comment
comment|/* Various modes of operation.  Note that after an init, mode is set to  * zero automatically when the structure is created.  */
end_comment
begin_define
DECL|macro|PNG_HAVE_IHDR
define|#
directive|define
name|PNG_HAVE_IHDR
value|0x01
end_define
begin_define
DECL|macro|PNG_HAVE_PLTE
define|#
directive|define
name|PNG_HAVE_PLTE
value|0x02
end_define
begin_define
DECL|macro|PNG_HAVE_IDAT
define|#
directive|define
name|PNG_HAVE_IDAT
value|0x04
end_define
begin_define
DECL|macro|PNG_AFTER_IDAT
define|#
directive|define
name|PNG_AFTER_IDAT
value|0x08
end_define
begin_comment
DECL|macro|PNG_AFTER_IDAT
comment|/* Have complete zlib datastream */
end_comment
begin_define
DECL|macro|PNG_HAVE_IEND
define|#
directive|define
name|PNG_HAVE_IEND
value|0x10
end_define
begin_define
DECL|macro|PNG_HAVE_gAMA
define|#
directive|define
name|PNG_HAVE_gAMA
value|0x20
end_define
begin_define
DECL|macro|PNG_HAVE_cHRM
define|#
directive|define
name|PNG_HAVE_cHRM
value|0x40
end_define
begin_define
DECL|macro|PNG_HAVE_sRGB
define|#
directive|define
name|PNG_HAVE_sRGB
value|0x80
end_define
begin_define
DECL|macro|PNG_HAVE_CHUNK_HEADER
define|#
directive|define
name|PNG_HAVE_CHUNK_HEADER
value|0x100
end_define
begin_define
DECL|macro|PNG_WROTE_tIME
define|#
directive|define
name|PNG_WROTE_tIME
value|0x200
end_define
begin_define
DECL|macro|PNG_WROTE_INFO_BEFORE_PLTE
define|#
directive|define
name|PNG_WROTE_INFO_BEFORE_PLTE
value|0x400
end_define
begin_define
DECL|macro|PNG_BACKGROUND_IS_GRAY
define|#
directive|define
name|PNG_BACKGROUND_IS_GRAY
value|0x800
end_define
begin_define
DECL|macro|PNG_HAVE_PNG_SIGNATURE
define|#
directive|define
name|PNG_HAVE_PNG_SIGNATURE
value|0x1000
end_define
begin_define
DECL|macro|PNG_HAVE_CHUNK_AFTER_IDAT
define|#
directive|define
name|PNG_HAVE_CHUNK_AFTER_IDAT
value|0x2000
end_define
begin_comment
DECL|macro|PNG_HAVE_CHUNK_AFTER_IDAT
comment|/* Have another chunk after IDAT */
end_comment
begin_comment
comment|/* Flags for the transformations the PNG library does on the image data */
end_comment
begin_define
DECL|macro|PNG_BGR
define|#
directive|define
name|PNG_BGR
value|0x0001
end_define
begin_define
DECL|macro|PNG_INTERLACE
define|#
directive|define
name|PNG_INTERLACE
value|0x0002
end_define
begin_define
DECL|macro|PNG_PACK
define|#
directive|define
name|PNG_PACK
value|0x0004
end_define
begin_define
DECL|macro|PNG_SHIFT
define|#
directive|define
name|PNG_SHIFT
value|0x0008
end_define
begin_define
DECL|macro|PNG_SWAP_BYTES
define|#
directive|define
name|PNG_SWAP_BYTES
value|0x0010
end_define
begin_define
DECL|macro|PNG_INVERT_MONO
define|#
directive|define
name|PNG_INVERT_MONO
value|0x0020
end_define
begin_define
DECL|macro|PNG_QUANTIZE
define|#
directive|define
name|PNG_QUANTIZE
value|0x0040
end_define
begin_define
DECL|macro|PNG_BACKGROUND
define|#
directive|define
name|PNG_BACKGROUND
value|0x0080
end_define
begin_define
DECL|macro|PNG_BACKGROUND_EXPAND
define|#
directive|define
name|PNG_BACKGROUND_EXPAND
value|0x0100
end_define
begin_comment
comment|/*    0x0200 unused */
end_comment
begin_define
DECL|macro|PNG_16_TO_8
define|#
directive|define
name|PNG_16_TO_8
value|0x0400
end_define
begin_define
DECL|macro|PNG_RGBA
define|#
directive|define
name|PNG_RGBA
value|0x0800
end_define
begin_define
DECL|macro|PNG_EXPAND
define|#
directive|define
name|PNG_EXPAND
value|0x1000
end_define
begin_define
DECL|macro|PNG_GAMMA
define|#
directive|define
name|PNG_GAMMA
value|0x2000
end_define
begin_define
DECL|macro|PNG_GRAY_TO_RGB
define|#
directive|define
name|PNG_GRAY_TO_RGB
value|0x4000
end_define
begin_define
DECL|macro|PNG_FILLER
define|#
directive|define
name|PNG_FILLER
value|0x8000L
end_define
begin_define
DECL|macro|PNG_PACKSWAP
define|#
directive|define
name|PNG_PACKSWAP
value|0x10000L
end_define
begin_define
DECL|macro|PNG_SWAP_ALPHA
define|#
directive|define
name|PNG_SWAP_ALPHA
value|0x20000L
end_define
begin_define
DECL|macro|PNG_STRIP_ALPHA
define|#
directive|define
name|PNG_STRIP_ALPHA
value|0x40000L
end_define
begin_define
DECL|macro|PNG_INVERT_ALPHA
define|#
directive|define
name|PNG_INVERT_ALPHA
value|0x80000L
end_define
begin_define
DECL|macro|PNG_USER_TRANSFORM
define|#
directive|define
name|PNG_USER_TRANSFORM
value|0x100000L
end_define
begin_define
DECL|macro|PNG_RGB_TO_GRAY_ERR
define|#
directive|define
name|PNG_RGB_TO_GRAY_ERR
value|0x200000L
end_define
begin_define
DECL|macro|PNG_RGB_TO_GRAY_WARN
define|#
directive|define
name|PNG_RGB_TO_GRAY_WARN
value|0x400000L
end_define
begin_define
DECL|macro|PNG_RGB_TO_GRAY
define|#
directive|define
name|PNG_RGB_TO_GRAY
value|0x600000L
end_define
begin_comment
DECL|macro|PNG_RGB_TO_GRAY
comment|/* two bits, RGB_TO_GRAY_ERR|WARN */
end_comment
begin_comment
comment|/*     0x800000L     Unused */
end_comment
begin_define
DECL|macro|PNG_ADD_ALPHA
define|#
directive|define
name|PNG_ADD_ALPHA
value|0x1000000L
end_define
begin_comment
DECL|macro|PNG_ADD_ALPHA
comment|/* Added to libpng-1.2.7 */
end_comment
begin_define
DECL|macro|PNG_EXPAND_tRNS
define|#
directive|define
name|PNG_EXPAND_tRNS
value|0x2000000L
end_define
begin_comment
DECL|macro|PNG_EXPAND_tRNS
comment|/* Added to libpng-1.2.9 */
end_comment
begin_comment
comment|/*   0x4000000L  unused */
end_comment
begin_comment
comment|/*   0x8000000L  unused */
end_comment
begin_comment
comment|/*  0x10000000L  unused */
end_comment
begin_comment
comment|/*  0x20000000L  unused */
end_comment
begin_comment
comment|/*  0x40000000L  unused */
end_comment
begin_comment
comment|/* Flags for png_create_struct */
end_comment
begin_define
DECL|macro|PNG_STRUCT_PNG
define|#
directive|define
name|PNG_STRUCT_PNG
value|0x0001
end_define
begin_define
DECL|macro|PNG_STRUCT_INFO
define|#
directive|define
name|PNG_STRUCT_INFO
value|0x0002
end_define
begin_comment
comment|/* Scaling factor for filter heuristic weighting calculations */
end_comment
begin_define
DECL|macro|PNG_WEIGHT_FACTOR
define|#
directive|define
name|PNG_WEIGHT_FACTOR
value|(1<<(PNG_WEIGHT_SHIFT))
end_define
begin_define
DECL|macro|PNG_COST_FACTOR
define|#
directive|define
name|PNG_COST_FACTOR
value|(1<<(PNG_COST_SHIFT))
end_define
begin_comment
comment|/* Flags for the png_ptr->flags rather than declaring a byte for each one */
end_comment
begin_define
DECL|macro|PNG_FLAG_ZLIB_CUSTOM_STRATEGY
define|#
directive|define
name|PNG_FLAG_ZLIB_CUSTOM_STRATEGY
value|0x0001
end_define
begin_define
DECL|macro|PNG_FLAG_ZLIB_CUSTOM_LEVEL
define|#
directive|define
name|PNG_FLAG_ZLIB_CUSTOM_LEVEL
value|0x0002
end_define
begin_define
DECL|macro|PNG_FLAG_ZLIB_CUSTOM_MEM_LEVEL
define|#
directive|define
name|PNG_FLAG_ZLIB_CUSTOM_MEM_LEVEL
value|0x0004
end_define
begin_define
DECL|macro|PNG_FLAG_ZLIB_CUSTOM_WINDOW_BITS
define|#
directive|define
name|PNG_FLAG_ZLIB_CUSTOM_WINDOW_BITS
value|0x0008
end_define
begin_define
DECL|macro|PNG_FLAG_ZLIB_CUSTOM_METHOD
define|#
directive|define
name|PNG_FLAG_ZLIB_CUSTOM_METHOD
value|0x0010
end_define
begin_define
DECL|macro|PNG_FLAG_ZLIB_FINISHED
define|#
directive|define
name|PNG_FLAG_ZLIB_FINISHED
value|0x0020
end_define
begin_define
DECL|macro|PNG_FLAG_ROW_INIT
define|#
directive|define
name|PNG_FLAG_ROW_INIT
value|0x0040
end_define
begin_define
DECL|macro|PNG_FLAG_FILLER_AFTER
define|#
directive|define
name|PNG_FLAG_FILLER_AFTER
value|0x0080
end_define
begin_define
DECL|macro|PNG_FLAG_CRC_ANCILLARY_USE
define|#
directive|define
name|PNG_FLAG_CRC_ANCILLARY_USE
value|0x0100
end_define
begin_define
DECL|macro|PNG_FLAG_CRC_ANCILLARY_NOWARN
define|#
directive|define
name|PNG_FLAG_CRC_ANCILLARY_NOWARN
value|0x0200
end_define
begin_define
DECL|macro|PNG_FLAG_CRC_CRITICAL_USE
define|#
directive|define
name|PNG_FLAG_CRC_CRITICAL_USE
value|0x0400
end_define
begin_define
DECL|macro|PNG_FLAG_CRC_CRITICAL_IGNORE
define|#
directive|define
name|PNG_FLAG_CRC_CRITICAL_IGNORE
value|0x0800
end_define
begin_comment
comment|/*        0x1000  unused */
end_comment
begin_comment
comment|/*        0x2000  unused */
end_comment
begin_comment
comment|/*        0x4000  unused */
end_comment
begin_define
DECL|macro|PNG_FLAG_KEEP_UNKNOWN_CHUNKS
define|#
directive|define
name|PNG_FLAG_KEEP_UNKNOWN_CHUNKS
value|0x8000L
end_define
begin_define
DECL|macro|PNG_FLAG_KEEP_UNSAFE_CHUNKS
define|#
directive|define
name|PNG_FLAG_KEEP_UNSAFE_CHUNKS
value|0x10000L
end_define
begin_define
DECL|macro|PNG_FLAG_LIBRARY_MISMATCH
define|#
directive|define
name|PNG_FLAG_LIBRARY_MISMATCH
value|0x20000L
end_define
begin_define
DECL|macro|PNG_FLAG_STRIP_ERROR_NUMBERS
define|#
directive|define
name|PNG_FLAG_STRIP_ERROR_NUMBERS
value|0x40000L
end_define
begin_define
DECL|macro|PNG_FLAG_STRIP_ERROR_TEXT
define|#
directive|define
name|PNG_FLAG_STRIP_ERROR_TEXT
value|0x80000L
end_define
begin_define
DECL|macro|PNG_FLAG_MALLOC_NULL_MEM_OK
define|#
directive|define
name|PNG_FLAG_MALLOC_NULL_MEM_OK
value|0x100000L
end_define
begin_define
DECL|macro|PNG_FLAG_ADD_ALPHA
define|#
directive|define
name|PNG_FLAG_ADD_ALPHA
value|0x200000L
end_define
begin_comment
DECL|macro|PNG_FLAG_ADD_ALPHA
comment|/* Added to libpng-1.2.8 */
end_comment
begin_define
DECL|macro|PNG_FLAG_STRIP_ALPHA
define|#
directive|define
name|PNG_FLAG_STRIP_ALPHA
value|0x400000L
end_define
begin_comment
DECL|macro|PNG_FLAG_STRIP_ALPHA
comment|/* Added to libpng-1.2.8 */
end_comment
begin_define
DECL|macro|PNG_FLAG_BENIGN_ERRORS_WARN
define|#
directive|define
name|PNG_FLAG_BENIGN_ERRORS_WARN
value|0x800000L
end_define
begin_comment
DECL|macro|PNG_FLAG_BENIGN_ERRORS_WARN
comment|/* Added to libpng-1.4.0 */
end_comment
begin_comment
comment|/*     0x1000000L  unused */
end_comment
begin_comment
comment|/*     0x2000000L  unused */
end_comment
begin_comment
comment|/*     0x4000000L  unused */
end_comment
begin_comment
comment|/*     0x8000000L  unused */
end_comment
begin_comment
comment|/*    0x10000000L  unused */
end_comment
begin_comment
comment|/*    0x20000000L  unused */
end_comment
begin_comment
comment|/*    0x40000000L  unused */
end_comment
begin_define
DECL|macro|PNG_FLAG_CRC_ANCILLARY_MASK
define|#
directive|define
name|PNG_FLAG_CRC_ANCILLARY_MASK
value|(PNG_FLAG_CRC_ANCILLARY_USE | \                                      PNG_FLAG_CRC_ANCILLARY_NOWARN)
end_define
begin_define
DECL|macro|PNG_FLAG_CRC_CRITICAL_MASK
define|#
directive|define
name|PNG_FLAG_CRC_CRITICAL_MASK
value|(PNG_FLAG_CRC_CRITICAL_USE | \                                      PNG_FLAG_CRC_CRITICAL_IGNORE)
end_define
begin_define
DECL|macro|PNG_FLAG_CRC_MASK
define|#
directive|define
name|PNG_FLAG_CRC_MASK
value|(PNG_FLAG_CRC_ANCILLARY_MASK | \                                      PNG_FLAG_CRC_CRITICAL_MASK)
end_define
begin_comment
comment|/* zlib.h declares a magic type 'uInt' that limits the amount of data that zlib  * can handle at once.  This type need be no larger than 16 bits (so maximum of  * 65535), this define allows us to discover how big it is, but limited by the  * maximuum for png_size_t.  The value can be overriden in a library build  * (pngusr.h, or set it in CPPFLAGS) and it works to set it to a considerably  * lower value (e.g. 255 works).  A lower value may help memory usage (slightly)  * and may even improve performance on some systems (and degrade it on others.)  */
end_comment
begin_ifndef
ifndef|#
directive|ifndef
name|ZLIB_IO_MAX
end_ifndef
begin_define
DECL|macro|ZLIB_IO_MAX
define|#
directive|define
name|ZLIB_IO_MAX
value|((uInt)-1)
end_define
begin_endif
endif|#
directive|endif
end_endif
begin_comment
comment|/* Save typing and make code easier to understand */
end_comment
begin_define
DECL|macro|PNG_COLOR_DIST
define|#
directive|define
name|PNG_COLOR_DIST
parameter_list|(
name|c1
parameter_list|,
name|c2
parameter_list|)
value|(abs((int)((c1).red) - (int)((c2).red)) + \    abs((int)((c1).green) - (int)((c2).green)) + \    abs((int)((c1).blue) - (int)((c2).blue)))
end_define
begin_comment
comment|/* Added to libpng-1.2.6 JB */
end_comment
begin_define
DECL|macro|PNG_ROWBYTES
define|#
directive|define
name|PNG_ROWBYTES
parameter_list|(
name|pixel_bits
parameter_list|,
name|width
parameter_list|)
define|\
value|((pixel_bits)>= 8 ? \     ((png_size_t)(width) * (((png_size_t)(pixel_bits))>> 3)) : \     (( ((png_size_t)(width) * ((png_size_t)(pixel_bits))) + 7)>> 3) )
end_define
begin_comment
comment|/* PNG_OUT_OF_RANGE returns true if value is outside the range  * ideal-delta..ideal+delta.  Each argument is evaluated twice.  * "ideal" and "delta" should be constants, normally simple  * integers, "value" a variable. Added to libpng-1.2.6 JB  */
end_comment
begin_define
DECL|macro|PNG_OUT_OF_RANGE
define|#
directive|define
name|PNG_OUT_OF_RANGE
parameter_list|(
name|value
parameter_list|,
name|ideal
parameter_list|,
name|delta
parameter_list|)
define|\
value|( (value)< (ideal)-(delta) || (value)> (ideal)+(delta) )
end_define
begin_comment
comment|/* Conversions between fixed and floating point, only defined if  * required (to make sure the code doesn't accidentally use float  * when it is supposedly disabled.)  */
end_comment
begin_ifdef
ifdef|#
directive|ifdef
name|PNG_FLOATING_POINT_SUPPORTED
end_ifdef
begin_comment
comment|/* The floating point conversion can't overflow, though it can and  * does lose accuracy relative to the original fixed point value.  * In practice this doesn't matter because png_fixed_point only  * stores numbers with very low precision.  The png_ptr and s  * arguments are unused by default but are there in case error  * checking becomes a requirement.  */
end_comment
begin_define
DECL|macro|png_float
define|#
directive|define
name|png_float
parameter_list|(
name|png_ptr
parameter_list|,
name|fixed
parameter_list|,
name|s
parameter_list|)
value|(.00001 * (fixed))
end_define
begin_comment
comment|/* The fixed point conversion performs range checking and evaluates  * its argument multiple times, so must be used with care.  The  * range checking uses the PNG specification values for a signed  * 32 bit fixed point value except that the values are deliberately  * rounded-to-zero to an integral value - 21474 (21474.83 is roughly  * (2^31-1) * 100000). 's' is a string that describes the value being  * converted.  *  * NOTE: this macro will raise a png_error if the range check fails,  * therefore it is normally only appropriate to use this on values  * that come from API calls or other sources where an out of range  * error indicates a programming error, not a data error!  *  * NOTE: by default this is off - the macro is not used - because the  * function call saves a lot of code.  */
end_comment
begin_ifdef
ifdef|#
directive|ifdef
name|PNG_FIXED_POINT_MACRO_SUPPORTED
end_ifdef
begin_define
DECL|macro|png_fixed
define|#
directive|define
name|png_fixed
parameter_list|(
name|png_ptr
parameter_list|,
name|fp
parameter_list|,
name|s
parameter_list|)
value|((fp)<= 21474&& (fp)>= -21474 ?\     ((png_fixed_point)(100000 * (fp))) : (png_fixed_error(png_ptr, s),0))
end_define
begin_else
else|#
directive|else
end_else
begin_decl_stmt
name|PNG_EXTERN
name|png_fixed_point
name|png_fixed
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|double
name|fp
operator|,
name|png_const_charp
name|text
operator|)
argument_list|)
decl_stmt|;
end_decl_stmt
begin_endif
endif|#
directive|endif
end_endif
begin_endif
endif|#
directive|endif
end_endif
begin_comment
comment|/* Constant strings for known chunk types.  If you need to add a chunk,  * define the name here, and add an invocation of the macro wherever it's  * needed.  */
end_comment
begin_define
DECL|macro|PNG_IHDR
define|#
directive|define
name|PNG_IHDR
value|PNG_CONST png_byte png_IHDR[5] = { 73,  72,  68,  82, '\0'}
end_define
begin_define
DECL|macro|PNG_IDAT
define|#
directive|define
name|PNG_IDAT
value|PNG_CONST png_byte png_IDAT[5] = { 73,  68,  65,  84, '\0'}
end_define
begin_define
DECL|macro|PNG_IEND
define|#
directive|define
name|PNG_IEND
value|PNG_CONST png_byte png_IEND[5] = { 73,  69,  78,  68, '\0'}
end_define
begin_define
DECL|macro|PNG_PLTE
define|#
directive|define
name|PNG_PLTE
value|PNG_CONST png_byte png_PLTE[5] = { 80,  76,  84,  69, '\0'}
end_define
begin_define
DECL|macro|PNG_bKGD
define|#
directive|define
name|PNG_bKGD
value|PNG_CONST png_byte png_bKGD[5] = { 98,  75,  71,  68, '\0'}
end_define
begin_define
DECL|macro|PNG_cHRM
define|#
directive|define
name|PNG_cHRM
value|PNG_CONST png_byte png_cHRM[5] = { 99,  72,  82,  77, '\0'}
end_define
begin_define
DECL|macro|PNG_gAMA
define|#
directive|define
name|PNG_gAMA
value|PNG_CONST png_byte png_gAMA[5] = {103,  65,  77,  65, '\0'}
end_define
begin_define
DECL|macro|PNG_hIST
define|#
directive|define
name|PNG_hIST
value|PNG_CONST png_byte png_hIST[5] = {104,  73,  83,  84, '\0'}
end_define
begin_define
DECL|macro|PNG_iCCP
define|#
directive|define
name|PNG_iCCP
value|PNG_CONST png_byte png_iCCP[5] = {105,  67,  67,  80, '\0'}
end_define
begin_define
DECL|macro|PNG_iTXt
define|#
directive|define
name|PNG_iTXt
value|PNG_CONST png_byte png_iTXt[5] = {105,  84,  88, 116, '\0'}
end_define
begin_define
DECL|macro|PNG_oFFs
define|#
directive|define
name|PNG_oFFs
value|PNG_CONST png_byte png_oFFs[5] = {111,  70,  70, 115, '\0'}
end_define
begin_define
DECL|macro|PNG_pCAL
define|#
directive|define
name|PNG_pCAL
value|PNG_CONST png_byte png_pCAL[5] = {112,  67,  65,  76, '\0'}
end_define
begin_define
DECL|macro|PNG_sCAL
define|#
directive|define
name|PNG_sCAL
value|PNG_CONST png_byte png_sCAL[5] = {115,  67,  65,  76, '\0'}
end_define
begin_define
DECL|macro|PNG_pHYs
define|#
directive|define
name|PNG_pHYs
value|PNG_CONST png_byte png_pHYs[5] = {112,  72,  89, 115, '\0'}
end_define
begin_define
DECL|macro|PNG_sBIT
define|#
directive|define
name|PNG_sBIT
value|PNG_CONST png_byte png_sBIT[5] = {115,  66,  73,  84, '\0'}
end_define
begin_define
DECL|macro|PNG_sPLT
define|#
directive|define
name|PNG_sPLT
value|PNG_CONST png_byte png_sPLT[5] = {115,  80,  76,  84, '\0'}
end_define
begin_define
DECL|macro|PNG_sRGB
define|#
directive|define
name|PNG_sRGB
value|PNG_CONST png_byte png_sRGB[5] = {115,  82,  71,  66, '\0'}
end_define
begin_define
DECL|macro|PNG_sTER
define|#
directive|define
name|PNG_sTER
value|PNG_CONST png_byte png_sTER[5] = {115,  84,  69,  82, '\0'}
end_define
begin_define
DECL|macro|PNG_tEXt
define|#
directive|define
name|PNG_tEXt
value|PNG_CONST png_byte png_tEXt[5] = {116,  69,  88, 116, '\0'}
end_define
begin_define
DECL|macro|PNG_tIME
define|#
directive|define
name|PNG_tIME
value|PNG_CONST png_byte png_tIME[5] = {116,  73,  77,  69, '\0'}
end_define
begin_define
DECL|macro|PNG_tRNS
define|#
directive|define
name|PNG_tRNS
value|PNG_CONST png_byte png_tRNS[5] = {116,  82,  78,  83, '\0'}
end_define
begin_define
DECL|macro|PNG_zTXt
define|#
directive|define
name|PNG_zTXt
value|PNG_CONST png_byte png_zTXt[5] = {122,  84,  88, 116, '\0'}
end_define
begin_comment
comment|/* Inhibit C++ name-mangling for libpng functions but not for system calls. */
end_comment
begin_ifdef
ifdef|#
directive|ifdef
name|__cplusplus
end_ifdef
begin_extern
extern|extern
literal|"C"
block|{
endif|#
directive|endif
comment|/* __cplusplus */
comment|/* These functions are used internally in the code.  They generally  * shouldn't be used unless you are writing code to add or replace some  * functionality in libpng.  More information about most functions can  * be found in the files where the functions are located.  */
comment|/* Allocate memory for an internal libpng struct */
name|PNG_EXTERN
name|PNG_FUNCTION
argument_list|(
name|png_voidp
argument_list|,
name|png_create_struct
argument_list|,
name|PNGARG
argument_list|(
operator|(
name|int
name|type
operator|)
argument_list|)
argument_list|,
name|PNG_ALLOCATED
argument_list|)
decl_stmt|;
comment|/* Free memory from internal libpng struct */
name|PNG_EXTERN
name|void
name|png_destroy_struct
name|PNGARG
argument_list|(
operator|(
name|png_voidp
name|struct_ptr
operator|)
argument_list|)
decl_stmt|;
name|PNG_EXTERN
name|PNG_FUNCTION
argument_list|(
name|png_voidp
argument_list|,
name|png_create_struct_2
argument_list|,
name|PNGARG
argument_list|(
operator|(
name|int
name|type
operator|,
name|png_malloc_ptr
name|malloc_fn
operator|,
name|png_voidp
name|mem_ptr
operator|)
argument_list|)
argument_list|,
name|PNG_ALLOCATED
argument_list|)
decl_stmt|;
name|PNG_EXTERN
name|void
name|png_destroy_struct_2
name|PNGARG
argument_list|(
operator|(
name|png_voidp
name|struct_ptr
operator|,
name|png_free_ptr
name|free_fn
operator|,
name|png_voidp
name|mem_ptr
operator|)
argument_list|)
decl_stmt|;
comment|/* Free any memory that info_ptr points to and reset struct. */
name|PNG_EXTERN
name|void
name|png_info_destroy
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|png_infop
name|info_ptr
operator|)
argument_list|)
decl_stmt|;
comment|/* Function to allocate memory for zlib.  PNGAPI is disallowed. */
name|PNG_EXTERN
name|PNG_FUNCTION
argument_list|(
name|voidpf
argument_list|,
name|png_zalloc
argument_list|,
name|PNGARG
argument_list|(
operator|(
name|voidpf
name|png_ptr
operator|,
name|uInt
name|items
operator|,
name|uInt
name|size
operator|)
argument_list|)
argument_list|,
name|PNG_ALLOCATED
argument_list|)
decl_stmt|;
comment|/* Function to free memory for zlib.  PNGAPI is disallowed. */
name|PNG_EXTERN
name|void
name|png_zfree
name|PNGARG
argument_list|(
operator|(
name|voidpf
name|png_ptr
operator|,
name|voidpf
name|ptr
operator|)
argument_list|)
decl_stmt|;
comment|/* Next four functions are used internally as callbacks.  PNGCBAPI is required  * but not PNG_EXPORT.  PNGAPI added at libpng version 1.2.3, changed to  * PNGCBAPI at 1.5.0  */
name|PNG_EXTERN
name|void
name|PNGCBAPI
name|png_default_read_data
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|png_bytep
name|data
operator|,
name|png_size_t
name|length
operator|)
argument_list|)
decl_stmt|;
ifdef|#
directive|ifdef
name|PNG_PROGRESSIVE_READ_SUPPORTED
name|PNG_EXTERN
name|void
name|PNGCBAPI
name|png_push_fill_buffer
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|png_bytep
name|buffer
operator|,
name|png_size_t
name|length
operator|)
argument_list|)
decl_stmt|;
endif|#
directive|endif
name|PNG_EXTERN
name|void
name|PNGCBAPI
name|png_default_write_data
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|png_bytep
name|data
operator|,
name|png_size_t
name|length
operator|)
argument_list|)
decl_stmt|;
ifdef|#
directive|ifdef
name|PNG_WRITE_FLUSH_SUPPORTED
ifdef|#
directive|ifdef
name|PNG_STDIO_SUPPORTED
name|PNG_EXTERN
name|void
name|PNGCBAPI
name|png_default_flush
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|)
argument_list|)
decl_stmt|;
endif|#
directive|endif
endif|#
directive|endif
comment|/* Reset the CRC variable */
name|PNG_EXTERN
name|void
name|png_reset_crc
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|)
argument_list|)
decl_stmt|;
comment|/* Write the "data" buffer to whatever output you are using */
name|PNG_EXTERN
name|void
name|png_write_data
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|png_const_bytep
name|data
operator|,
name|png_size_t
name|length
operator|)
argument_list|)
decl_stmt|;
comment|/* Read and check the PNG file signature */
name|PNG_EXTERN
name|void
name|png_read_sig
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|png_infop
name|info_ptr
operator|)
argument_list|)
decl_stmt|;
comment|/* Read the chunk header (length + type name) */
name|PNG_EXTERN
name|png_uint_32
name|png_read_chunk_header
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|)
argument_list|)
decl_stmt|;
comment|/* Read data from whatever input you are using into the "data" buffer */
name|PNG_EXTERN
name|void
name|png_read_data
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|png_bytep
name|data
operator|,
name|png_size_t
name|length
operator|)
argument_list|)
decl_stmt|;
comment|/* Read bytes into buf, and update png_ptr->crc */
name|PNG_EXTERN
name|void
name|png_crc_read
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|png_bytep
name|buf
operator|,
name|png_size_t
name|length
operator|)
argument_list|)
decl_stmt|;
comment|/* Decompress data in a chunk that uses compression */
if|#
directive|if
name|defined
argument_list|(
name|PNG_zTXt_SUPPORTED
argument_list|)
operator|||
name|defined
argument_list|(
name|PNG_iTXt_SUPPORTED
argument_list|)
operator|||
expr|\
name|defined
argument_list|(
name|PNG_iCCP_SUPPORTED
argument_list|)
operator|||
name|defined
argument_list|(
name|PNG_sPLT_SUPPORTED
argument_list|)
name|PNG_EXTERN
name|void
name|png_decompress_chunk
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|int
name|comp_type
operator|,
name|png_size_t
name|chunklength
operator|,
name|png_size_t
name|prefix_length
operator|,
name|png_size_t
operator|*
name|data_length
operator|)
argument_list|)
decl_stmt|;
endif|#
directive|endif
comment|/* Read "skip" bytes, read the file crc, and (optionally) verify png_ptr->crc */
name|PNG_EXTERN
name|int
name|png_crc_finish
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|png_uint_32
name|skip
operator|)
argument_list|)
decl_stmt|;
comment|/* Read the CRC from the file and compare it to the libpng calculated CRC */
name|PNG_EXTERN
name|int
name|png_crc_error
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|)
argument_list|)
decl_stmt|;
comment|/* Calculate the CRC over a section of data.  Note that we are only  * passing a maximum of 64K on systems that have this as a memory limit,  * since this is the maximum buffer size we can specify.  */
name|PNG_EXTERN
name|void
name|png_calculate_crc
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|png_const_bytep
name|ptr
operator|,
name|png_size_t
name|length
operator|)
argument_list|)
decl_stmt|;
ifdef|#
directive|ifdef
name|PNG_WRITE_FLUSH_SUPPORTED
name|PNG_EXTERN
name|void
name|png_flush
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|)
argument_list|)
decl_stmt|;
endif|#
directive|endif
comment|/* Write various chunks */
comment|/* Write the IHDR chunk, and update the png_struct with the necessary  * information.  */
name|PNG_EXTERN
name|void
name|png_write_IHDR
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|png_uint_32
name|width
operator|,
name|png_uint_32
name|height
operator|,
name|int
name|bit_depth
operator|,
name|int
name|color_type
operator|,
name|int
name|compression_method
operator|,
name|int
name|filter_method
operator|,
name|int
name|interlace_method
operator|)
argument_list|)
decl_stmt|;
name|PNG_EXTERN
name|void
name|png_write_PLTE
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|png_const_colorp
name|palette
operator|,
name|png_uint_32
name|num_pal
operator|)
argument_list|)
decl_stmt|;
name|PNG_EXTERN
name|void
name|png_write_IDAT
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|png_bytep
name|data
operator|,
name|png_size_t
name|length
operator|)
argument_list|)
decl_stmt|;
name|PNG_EXTERN
name|void
name|png_write_IEND
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|)
argument_list|)
decl_stmt|;
ifdef|#
directive|ifdef
name|PNG_WRITE_gAMA_SUPPORTED
ifdef|#
directive|ifdef
name|PNG_FLOATING_POINT_SUPPORTED
name|PNG_EXTERN
name|void
name|png_write_gAMA
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|double
name|file_gamma
operator|)
argument_list|)
decl_stmt|;
endif|#
directive|endif
ifdef|#
directive|ifdef
name|PNG_FIXED_POINT_SUPPORTED
name|PNG_EXTERN
name|void
name|png_write_gAMA_fixed
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|png_fixed_point
name|file_gamma
operator|)
argument_list|)
decl_stmt|;
endif|#
directive|endif
endif|#
directive|endif
ifdef|#
directive|ifdef
name|PNG_WRITE_sBIT_SUPPORTED
name|PNG_EXTERN
name|void
name|png_write_sBIT
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|png_const_color_8p
name|sbit
operator|,
name|int
name|color_type
operator|)
argument_list|)
decl_stmt|;
endif|#
directive|endif
ifdef|#
directive|ifdef
name|PNG_WRITE_cHRM_SUPPORTED
ifdef|#
directive|ifdef
name|PNG_FLOATING_POINT_SUPPORTED
name|PNG_EXTERN
name|void
name|png_write_cHRM
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|double
name|white_x
operator|,
name|double
name|white_y
operator|,
name|double
name|red_x
operator|,
name|double
name|red_y
operator|,
name|double
name|green_x
operator|,
name|double
name|green_y
operator|,
name|double
name|blue_x
operator|,
name|double
name|blue_y
operator|)
argument_list|)
decl_stmt|;
endif|#
directive|endif
name|PNG_EXTERN
name|void
name|png_write_cHRM_fixed
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|png_fixed_point
name|int_white_x
operator|,
name|png_fixed_point
name|int_white_y
operator|,
name|png_fixed_point
name|int_red_x
operator|,
name|png_fixed_point
name|int_red_y
operator|,
name|png_fixed_point
name|int_green_x
operator|,
name|png_fixed_point
name|int_green_y
operator|,
name|png_fixed_point
name|int_blue_x
operator|,
name|png_fixed_point
name|int_blue_y
operator|)
argument_list|)
decl_stmt|;
endif|#
directive|endif
ifdef|#
directive|ifdef
name|PNG_WRITE_sRGB_SUPPORTED
name|PNG_EXTERN
name|void
name|png_write_sRGB
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|int
name|intent
operator|)
argument_list|)
decl_stmt|;
endif|#
directive|endif
ifdef|#
directive|ifdef
name|PNG_WRITE_iCCP_SUPPORTED
name|PNG_EXTERN
name|void
name|png_write_iCCP
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|png_const_charp
name|name
operator|,
name|int
name|compression_type
operator|,
name|png_const_charp
name|profile
operator|,
name|int
name|proflen
operator|)
argument_list|)
decl_stmt|;
comment|/* Note to maintainer: profile should be png_bytep */
endif|#
directive|endif
ifdef|#
directive|ifdef
name|PNG_WRITE_sPLT_SUPPORTED
name|PNG_EXTERN
name|void
name|png_write_sPLT
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|png_const_sPLT_tp
name|palette
operator|)
argument_list|)
decl_stmt|;
endif|#
directive|endif
ifdef|#
directive|ifdef
name|PNG_WRITE_tRNS_SUPPORTED
name|PNG_EXTERN
name|void
name|png_write_tRNS
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|png_const_bytep
name|trans
operator|,
name|png_const_color_16p
name|values
operator|,
name|int
name|number
operator|,
name|int
name|color_type
operator|)
argument_list|)
decl_stmt|;
endif|#
directive|endif
ifdef|#
directive|ifdef
name|PNG_WRITE_bKGD_SUPPORTED
name|PNG_EXTERN
name|void
name|png_write_bKGD
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|png_const_color_16p
name|values
operator|,
name|int
name|color_type
operator|)
argument_list|)
decl_stmt|;
endif|#
directive|endif
ifdef|#
directive|ifdef
name|PNG_WRITE_hIST_SUPPORTED
name|PNG_EXTERN
name|void
name|png_write_hIST
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|png_const_uint_16p
name|hist
operator|,
name|int
name|num_hist
operator|)
argument_list|)
decl_stmt|;
endif|#
directive|endif
if|#
directive|if
name|defined
argument_list|(
name|PNG_WRITE_TEXT_SUPPORTED
argument_list|)
operator|||
name|defined
argument_list|(
name|PNG_WRITE_pCAL_SUPPORTED
argument_list|)
operator|||
expr|\
name|defined
argument_list|(
name|PNG_WRITE_iCCP_SUPPORTED
argument_list|)
operator|||
name|defined
argument_list|(
name|PNG_WRITE_sPLT_SUPPORTED
argument_list|)
name|PNG_EXTERN
name|png_size_t
name|png_check_keyword
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|png_const_charp
name|key
operator|,
name|png_charpp
name|new_key
operator|)
argument_list|)
decl_stmt|;
endif|#
directive|endif
ifdef|#
directive|ifdef
name|PNG_WRITE_tEXt_SUPPORTED
name|PNG_EXTERN
name|void
name|png_write_tEXt
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|png_const_charp
name|key
operator|,
name|png_const_charp
name|text
operator|,
name|png_size_t
name|text_len
operator|)
argument_list|)
decl_stmt|;
endif|#
directive|endif
ifdef|#
directive|ifdef
name|PNG_WRITE_zTXt_SUPPORTED
name|PNG_EXTERN
name|void
name|png_write_zTXt
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|png_const_charp
name|key
operator|,
name|png_const_charp
name|text
operator|,
name|png_size_t
name|text_len
operator|,
name|int
name|compression
operator|)
argument_list|)
decl_stmt|;
endif|#
directive|endif
ifdef|#
directive|ifdef
name|PNG_WRITE_iTXt_SUPPORTED
name|PNG_EXTERN
name|void
name|png_write_iTXt
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|int
name|compression
operator|,
name|png_const_charp
name|key
operator|,
name|png_const_charp
name|lang
operator|,
name|png_const_charp
name|lang_key
operator|,
name|png_const_charp
name|text
operator|)
argument_list|)
decl_stmt|;
endif|#
directive|endif
ifdef|#
directive|ifdef
name|PNG_TEXT_SUPPORTED
comment|/* Added at version 1.0.14 and 1.2.4 */
name|PNG_EXTERN
name|int
name|png_set_text_2
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|png_infop
name|info_ptr
operator|,
name|png_const_textp
name|text_ptr
operator|,
name|int
name|num_text
operator|)
argument_list|)
decl_stmt|;
endif|#
directive|endif
ifdef|#
directive|ifdef
name|PNG_WRITE_oFFs_SUPPORTED
name|PNG_EXTERN
name|void
name|png_write_oFFs
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|png_int_32
name|x_offset
operator|,
name|png_int_32
name|y_offset
operator|,
name|int
name|unit_type
operator|)
argument_list|)
decl_stmt|;
endif|#
directive|endif
ifdef|#
directive|ifdef
name|PNG_WRITE_pCAL_SUPPORTED
name|PNG_EXTERN
name|void
name|png_write_pCAL
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|png_charp
name|purpose
operator|,
name|png_int_32
name|X0
operator|,
name|png_int_32
name|X1
operator|,
name|int
name|type
operator|,
name|int
name|nparams
operator|,
name|png_const_charp
name|units
operator|,
name|png_charpp
name|params
operator|)
argument_list|)
decl_stmt|;
endif|#
directive|endif
ifdef|#
directive|ifdef
name|PNG_WRITE_pHYs_SUPPORTED
name|PNG_EXTERN
name|void
name|png_write_pHYs
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|png_uint_32
name|x_pixels_per_unit
operator|,
name|png_uint_32
name|y_pixels_per_unit
operator|,
name|int
name|unit_type
operator|)
argument_list|)
decl_stmt|;
endif|#
directive|endif
ifdef|#
directive|ifdef
name|PNG_WRITE_tIME_SUPPORTED
name|PNG_EXTERN
name|void
name|png_write_tIME
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|png_const_timep
name|mod_time
operator|)
argument_list|)
decl_stmt|;
endif|#
directive|endif
ifdef|#
directive|ifdef
name|PNG_WRITE_sCAL_SUPPORTED
name|PNG_EXTERN
name|void
name|png_write_sCAL_s
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|int
name|unit
operator|,
name|png_const_charp
name|width
operator|,
name|png_const_charp
name|height
operator|)
argument_list|)
decl_stmt|;
endif|#
directive|endif
comment|/* Called when finished processing a row of data */
name|PNG_EXTERN
name|void
name|png_write_finish_row
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|)
argument_list|)
decl_stmt|;
comment|/* Internal use only.   Called before first row of data */
name|PNG_EXTERN
name|void
name|png_write_start_row
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|)
argument_list|)
decl_stmt|;
comment|/* Combine a row of data, dealing with alpha, etc. if requested */
name|PNG_EXTERN
name|void
name|png_combine_row
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|png_bytep
name|row
operator|,
name|int
name|mask
operator|)
argument_list|)
decl_stmt|;
ifdef|#
directive|ifdef
name|PNG_READ_INTERLACING_SUPPORTED
comment|/* Expand an interlaced row */
comment|/* OLD pre-1.0.9 interface: PNG_EXTERN void png_do_read_interlace PNGARG((png_row_infop row_info,     png_bytep row, int pass, png_uint_32 transformations));  */
name|PNG_EXTERN
name|void
name|png_do_read_interlace
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|)
argument_list|)
decl_stmt|;
endif|#
directive|endif
comment|/* GRR TO DO (2.0 or whenever):  simplify other internal calling interfaces */
ifdef|#
directive|ifdef
name|PNG_WRITE_INTERLACING_SUPPORTED
comment|/* Grab pixels out of a row for an interlaced pass */
name|PNG_EXTERN
name|void
name|png_do_write_interlace
name|PNGARG
argument_list|(
operator|(
name|png_row_infop
name|row_info
operator|,
name|png_bytep
name|row
operator|,
name|int
name|pass
operator|)
argument_list|)
decl_stmt|;
endif|#
directive|endif
comment|/* Unfilter a row */
name|PNG_EXTERN
name|void
name|png_read_filter_row
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|png_row_infop
name|row_info
operator|,
name|png_bytep
name|row
operator|,
name|png_const_bytep
name|prev_row
operator|,
name|int
name|filter
operator|)
argument_list|)
decl_stmt|;
comment|/* Choose the best filter to use and filter the row data */
name|PNG_EXTERN
name|void
name|png_write_find_filter
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|png_row_infop
name|row_info
operator|)
argument_list|)
decl_stmt|;
comment|/* Write out the filtered row. */
name|PNG_EXTERN
name|void
name|png_write_filtered_row
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|png_bytep
name|filtered_row
operator|)
argument_list|)
decl_stmt|;
comment|/* Finish a row while reading, dealing with interlacing passes, etc. */
name|PNG_EXTERN
name|void
name|png_read_finish_row
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|)
argument_list|)
decl_stmt|;
comment|/* Initialize the row buffers, etc. */
name|PNG_EXTERN
name|void
name|png_read_start_row
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|)
argument_list|)
decl_stmt|;
comment|/* Optional call to update the users info structure */
name|PNG_EXTERN
name|void
name|png_read_transform_info
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|png_infop
name|info_ptr
operator|)
argument_list|)
decl_stmt|;
comment|/* These are the functions that do the transformations */
ifdef|#
directive|ifdef
name|PNG_READ_FILLER_SUPPORTED
name|PNG_EXTERN
name|void
name|png_do_read_filler
name|PNGARG
argument_list|(
operator|(
name|png_row_infop
name|row_info
operator|,
name|png_bytep
name|row
operator|,
name|png_uint_32
name|filler
operator|,
name|png_uint_32
name|flags
operator|)
argument_list|)
decl_stmt|;
endif|#
directive|endif
ifdef|#
directive|ifdef
name|PNG_READ_SWAP_ALPHA_SUPPORTED
name|PNG_EXTERN
name|void
name|png_do_read_swap_alpha
name|PNGARG
argument_list|(
operator|(
name|png_row_infop
name|row_info
operator|,
name|png_bytep
name|row
operator|)
argument_list|)
decl_stmt|;
endif|#
directive|endif
ifdef|#
directive|ifdef
name|PNG_WRITE_SWAP_ALPHA_SUPPORTED
name|PNG_EXTERN
name|void
name|png_do_write_swap_alpha
name|PNGARG
argument_list|(
operator|(
name|png_row_infop
name|row_info
operator|,
name|png_bytep
name|row
operator|)
argument_list|)
decl_stmt|;
endif|#
directive|endif
ifdef|#
directive|ifdef
name|PNG_READ_INVERT_ALPHA_SUPPORTED
name|PNG_EXTERN
name|void
name|png_do_read_invert_alpha
name|PNGARG
argument_list|(
operator|(
name|png_row_infop
name|row_info
operator|,
name|png_bytep
name|row
operator|)
argument_list|)
decl_stmt|;
endif|#
directive|endif
ifdef|#
directive|ifdef
name|PNG_WRITE_INVERT_ALPHA_SUPPORTED
name|PNG_EXTERN
name|void
name|png_do_write_invert_alpha
name|PNGARG
argument_list|(
operator|(
name|png_row_infop
name|row_info
operator|,
name|png_bytep
name|row
operator|)
argument_list|)
decl_stmt|;
endif|#
directive|endif
if|#
directive|if
name|defined
argument_list|(
name|PNG_WRITE_FILLER_SUPPORTED
argument_list|)
operator|||
expr|\
name|defined
argument_list|(
name|PNG_READ_STRIP_ALPHA_SUPPORTED
argument_list|)
name|PNG_EXTERN
name|void
name|png_do_strip_filler
name|PNGARG
argument_list|(
operator|(
name|png_row_infop
name|row_info
operator|,
name|png_bytep
name|row
operator|,
name|png_uint_32
name|flags
operator|)
argument_list|)
decl_stmt|;
endif|#
directive|endif
ifdef|#
directive|ifdef
name|PNG_16BIT_SUPPORTED
if|#
directive|if
name|defined
argument_list|(
name|PNG_READ_SWAP_SUPPORTED
argument_list|)
operator|||
name|defined
argument_list|(
name|PNG_WRITE_SWAP_SUPPORTED
argument_list|)
name|PNG_EXTERN
name|void
name|png_do_swap
name|PNGARG
argument_list|(
operator|(
name|png_row_infop
name|row_info
operator|,
name|png_bytep
name|row
operator|)
argument_list|)
decl_stmt|;
endif|#
directive|endif
endif|#
directive|endif
if|#
directive|if
name|defined
argument_list|(
name|PNG_READ_PACKSWAP_SUPPORTED
argument_list|)
operator|||
expr|\
name|defined
argument_list|(
name|PNG_WRITE_PACKSWAP_SUPPORTED
argument_list|)
name|PNG_EXTERN
name|void
name|png_do_packswap
name|PNGARG
argument_list|(
operator|(
name|png_row_infop
name|row_info
operator|,
name|png_bytep
name|row
operator|)
argument_list|)
decl_stmt|;
endif|#
directive|endif
ifdef|#
directive|ifdef
name|PNG_READ_RGB_TO_GRAY_SUPPORTED
name|PNG_EXTERN
name|int
name|png_do_rgb_to_gray
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|png_row_infop
name|row_info
operator|,
name|png_bytep
name|row
operator|)
argument_list|)
decl_stmt|;
endif|#
directive|endif
ifdef|#
directive|ifdef
name|PNG_READ_GRAY_TO_RGB_SUPPORTED
name|PNG_EXTERN
name|void
name|png_do_gray_to_rgb
name|PNGARG
argument_list|(
operator|(
name|png_row_infop
name|row_info
operator|,
name|png_bytep
name|row
operator|)
argument_list|)
decl_stmt|;
endif|#
directive|endif
ifdef|#
directive|ifdef
name|PNG_READ_PACK_SUPPORTED
name|PNG_EXTERN
name|void
name|png_do_unpack
name|PNGARG
argument_list|(
operator|(
name|png_row_infop
name|row_info
operator|,
name|png_bytep
name|row
operator|)
argument_list|)
decl_stmt|;
endif|#
directive|endif
ifdef|#
directive|ifdef
name|PNG_READ_SHIFT_SUPPORTED
name|PNG_EXTERN
name|void
name|png_do_unshift
name|PNGARG
argument_list|(
operator|(
name|png_row_infop
name|row_info
operator|,
name|png_bytep
name|row
operator|,
name|png_const_color_8p
name|sig_bits
operator|)
argument_list|)
decl_stmt|;
endif|#
directive|endif
if|#
directive|if
name|defined
argument_list|(
name|PNG_READ_INVERT_SUPPORTED
argument_list|)
operator|||
name|defined
argument_list|(
name|PNG_WRITE_INVERT_SUPPORTED
argument_list|)
name|PNG_EXTERN
name|void
name|png_do_invert
name|PNGARG
argument_list|(
operator|(
name|png_row_infop
name|row_info
operator|,
name|png_bytep
name|row
operator|)
argument_list|)
decl_stmt|;
endif|#
directive|endif
ifdef|#
directive|ifdef
name|PNG_READ_16_TO_8_SUPPORTED
name|PNG_EXTERN
name|void
name|png_do_chop
name|PNGARG
argument_list|(
operator|(
name|png_row_infop
name|row_info
operator|,
name|png_bytep
name|row
operator|)
argument_list|)
decl_stmt|;
endif|#
directive|endif
ifdef|#
directive|ifdef
name|PNG_READ_QUANTIZE_SUPPORTED
name|PNG_EXTERN
name|void
name|png_do_quantize
name|PNGARG
argument_list|(
operator|(
name|png_row_infop
name|row_info
operator|,
name|png_bytep
name|row
operator|,
name|png_const_bytep
name|palette_lookup
operator|,
name|png_const_bytep
name|quantize_lookup
operator|)
argument_list|)
decl_stmt|;
ifdef|#
directive|ifdef
name|PNG_CORRECT_PALETTE_SUPPORTED
name|PNG_EXTERN
name|void
name|png_correct_palette
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|png_colorp
name|palette
operator|,
name|int
name|num_palette
operator|)
argument_list|)
decl_stmt|;
endif|#
directive|endif
endif|#
directive|endif
if|#
directive|if
name|defined
argument_list|(
name|PNG_READ_BGR_SUPPORTED
argument_list|)
operator|||
name|defined
argument_list|(
name|PNG_WRITE_BGR_SUPPORTED
argument_list|)
name|PNG_EXTERN
name|void
name|png_do_bgr
name|PNGARG
argument_list|(
operator|(
name|png_row_infop
name|row_info
operator|,
name|png_bytep
name|row
operator|)
argument_list|)
decl_stmt|;
endif|#
directive|endif
ifdef|#
directive|ifdef
name|PNG_WRITE_PACK_SUPPORTED
name|PNG_EXTERN
name|void
name|png_do_pack
name|PNGARG
argument_list|(
operator|(
name|png_row_infop
name|row_info
operator|,
name|png_bytep
name|row
operator|,
name|png_uint_32
name|bit_depth
operator|)
argument_list|)
decl_stmt|;
endif|#
directive|endif
ifdef|#
directive|ifdef
name|PNG_WRITE_SHIFT_SUPPORTED
name|PNG_EXTERN
name|void
name|png_do_shift
name|PNGARG
argument_list|(
operator|(
name|png_row_infop
name|row_info
operator|,
name|png_bytep
name|row
operator|,
name|png_const_color_8p
name|bit_depth
operator|)
argument_list|)
decl_stmt|;
endif|#
directive|endif
ifdef|#
directive|ifdef
name|PNG_READ_BACKGROUND_SUPPORTED
ifdef|#
directive|ifdef
name|PNG_READ_GAMMA_SUPPORTED
name|PNG_EXTERN
name|void
name|png_do_background
name|PNGARG
argument_list|(
operator|(
name|png_row_infop
name|row_info
operator|,
name|png_bytep
name|row
operator|,
name|png_const_color_16p
name|trans_color
operator|,
name|png_const_color_16p
name|background
operator|,
name|png_const_color_16p
name|background_1
operator|,
name|png_const_bytep
name|gamma_table
operator|,
name|png_const_bytep
name|gamma_from_1
operator|,
name|png_const_bytep
name|gamma_to_1
operator|,
name|png_const_uint_16pp
name|gamma_16
operator|,
name|png_const_uint_16pp
name|gamma_16_from_1
operator|,
name|png_const_uint_16pp
name|gamma_16_to_1
operator|,
name|int
name|gamma_shift
operator|)
argument_list|)
decl_stmt|;
else|#
directive|else
name|PNG_EXTERN
name|void
name|png_do_background
name|PNGARG
argument_list|(
operator|(
name|png_row_infop
name|row_info
operator|,
name|png_bytep
name|row
operator|,
name|png_const_color_16p
name|trans_color
operator|,
name|png_const_color_16p
name|background
operator|)
argument_list|)
decl_stmt|;
endif|#
directive|endif
endif|#
directive|endif
ifdef|#
directive|ifdef
name|PNG_READ_GAMMA_SUPPORTED
name|PNG_EXTERN
name|void
name|png_do_gamma
name|PNGARG
argument_list|(
operator|(
name|png_row_infop
name|row_info
operator|,
name|png_bytep
name|row
operator|,
name|png_const_bytep
name|gamma_table
operator|,
name|png_const_uint_16pp
name|gamma_16_table
operator|,
name|int
name|gamma_shift
operator|)
argument_list|)
decl_stmt|;
endif|#
directive|endif
ifdef|#
directive|ifdef
name|PNG_READ_EXPAND_SUPPORTED
name|PNG_EXTERN
name|void
name|png_do_expand_palette
name|PNGARG
argument_list|(
operator|(
name|png_row_infop
name|row_info
operator|,
name|png_bytep
name|row
operator|,
name|png_const_colorp
name|palette
operator|,
name|png_const_bytep
name|trans
operator|,
name|int
name|num_trans
operator|)
argument_list|)
decl_stmt|;
name|PNG_EXTERN
name|void
name|png_do_expand
name|PNGARG
argument_list|(
operator|(
name|png_row_infop
name|row_info
operator|,
name|png_bytep
name|row
operator|,
name|png_const_color_16p
name|trans_color
operator|)
argument_list|)
decl_stmt|;
endif|#
directive|endif
comment|/* The following decodes the appropriate chunks, and does error correction,  * then calls the appropriate callback for the chunk if it is valid.  */
comment|/* Decode the IHDR chunk */
name|PNG_EXTERN
name|void
name|png_handle_IHDR
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|png_infop
name|info_ptr
operator|,
name|png_uint_32
name|length
operator|)
argument_list|)
decl_stmt|;
name|PNG_EXTERN
name|void
name|png_handle_PLTE
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|png_infop
name|info_ptr
operator|,
name|png_uint_32
name|length
operator|)
argument_list|)
decl_stmt|;
name|PNG_EXTERN
name|void
name|png_handle_IEND
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|png_infop
name|info_ptr
operator|,
name|png_uint_32
name|length
operator|)
argument_list|)
decl_stmt|;
ifdef|#
directive|ifdef
name|PNG_READ_bKGD_SUPPORTED
name|PNG_EXTERN
name|void
name|png_handle_bKGD
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|png_infop
name|info_ptr
operator|,
name|png_uint_32
name|length
operator|)
argument_list|)
decl_stmt|;
endif|#
directive|endif
ifdef|#
directive|ifdef
name|PNG_READ_cHRM_SUPPORTED
name|PNG_EXTERN
name|void
name|png_handle_cHRM
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|png_infop
name|info_ptr
operator|,
name|png_uint_32
name|length
operator|)
argument_list|)
decl_stmt|;
endif|#
directive|endif
ifdef|#
directive|ifdef
name|PNG_READ_gAMA_SUPPORTED
name|PNG_EXTERN
name|void
name|png_handle_gAMA
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|png_infop
name|info_ptr
operator|,
name|png_uint_32
name|length
operator|)
argument_list|)
decl_stmt|;
endif|#
directive|endif
ifdef|#
directive|ifdef
name|PNG_READ_hIST_SUPPORTED
name|PNG_EXTERN
name|void
name|png_handle_hIST
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|png_infop
name|info_ptr
operator|,
name|png_uint_32
name|length
operator|)
argument_list|)
decl_stmt|;
endif|#
directive|endif
ifdef|#
directive|ifdef
name|PNG_READ_iCCP_SUPPORTED
name|PNG_EXTERN
name|void
name|png_handle_iCCP
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|png_infop
name|info_ptr
operator|,
name|png_uint_32
name|length
operator|)
argument_list|)
decl_stmt|;
endif|#
directive|endif
comment|/* PNG_READ_iCCP_SUPPORTED */
ifdef|#
directive|ifdef
name|PNG_READ_iTXt_SUPPORTED
name|PNG_EXTERN
name|void
name|png_handle_iTXt
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|png_infop
name|info_ptr
operator|,
name|png_uint_32
name|length
operator|)
argument_list|)
decl_stmt|;
endif|#
directive|endif
ifdef|#
directive|ifdef
name|PNG_READ_oFFs_SUPPORTED
name|PNG_EXTERN
name|void
name|png_handle_oFFs
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|png_infop
name|info_ptr
operator|,
name|png_uint_32
name|length
operator|)
argument_list|)
decl_stmt|;
endif|#
directive|endif
ifdef|#
directive|ifdef
name|PNG_READ_pCAL_SUPPORTED
name|PNG_EXTERN
name|void
name|png_handle_pCAL
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|png_infop
name|info_ptr
operator|,
name|png_uint_32
name|length
operator|)
argument_list|)
decl_stmt|;
endif|#
directive|endif
ifdef|#
directive|ifdef
name|PNG_READ_pHYs_SUPPORTED
name|PNG_EXTERN
name|void
name|png_handle_pHYs
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|png_infop
name|info_ptr
operator|,
name|png_uint_32
name|length
operator|)
argument_list|)
decl_stmt|;
endif|#
directive|endif
ifdef|#
directive|ifdef
name|PNG_READ_sBIT_SUPPORTED
name|PNG_EXTERN
name|void
name|png_handle_sBIT
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|png_infop
name|info_ptr
operator|,
name|png_uint_32
name|length
operator|)
argument_list|)
decl_stmt|;
endif|#
directive|endif
ifdef|#
directive|ifdef
name|PNG_READ_sCAL_SUPPORTED
name|PNG_EXTERN
name|void
name|png_handle_sCAL
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|png_infop
name|info_ptr
operator|,
name|png_uint_32
name|length
operator|)
argument_list|)
decl_stmt|;
endif|#
directive|endif
ifdef|#
directive|ifdef
name|PNG_READ_sPLT_SUPPORTED
name|PNG_EXTERN
name|void
name|png_handle_sPLT
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|png_infop
name|info_ptr
operator|,
name|png_uint_32
name|length
operator|)
argument_list|)
decl_stmt|;
endif|#
directive|endif
comment|/* PNG_READ_sPLT_SUPPORTED */
ifdef|#
directive|ifdef
name|PNG_READ_sRGB_SUPPORTED
name|PNG_EXTERN
name|void
name|png_handle_sRGB
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|png_infop
name|info_ptr
operator|,
name|png_uint_32
name|length
operator|)
argument_list|)
decl_stmt|;
endif|#
directive|endif
ifdef|#
directive|ifdef
name|PNG_READ_tEXt_SUPPORTED
name|PNG_EXTERN
name|void
name|png_handle_tEXt
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|png_infop
name|info_ptr
operator|,
name|png_uint_32
name|length
operator|)
argument_list|)
decl_stmt|;
endif|#
directive|endif
ifdef|#
directive|ifdef
name|PNG_READ_tIME_SUPPORTED
name|PNG_EXTERN
name|void
name|png_handle_tIME
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|png_infop
name|info_ptr
operator|,
name|png_uint_32
name|length
operator|)
argument_list|)
decl_stmt|;
endif|#
directive|endif
ifdef|#
directive|ifdef
name|PNG_READ_tRNS_SUPPORTED
name|PNG_EXTERN
name|void
name|png_handle_tRNS
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|png_infop
name|info_ptr
operator|,
name|png_uint_32
name|length
operator|)
argument_list|)
decl_stmt|;
endif|#
directive|endif
ifdef|#
directive|ifdef
name|PNG_READ_zTXt_SUPPORTED
name|PNG_EXTERN
name|void
name|png_handle_zTXt
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|png_infop
name|info_ptr
operator|,
name|png_uint_32
name|length
operator|)
argument_list|)
decl_stmt|;
endif|#
directive|endif
name|PNG_EXTERN
name|void
name|png_handle_unknown
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|png_infop
name|info_ptr
operator|,
name|png_uint_32
name|length
operator|)
argument_list|)
decl_stmt|;
name|PNG_EXTERN
name|void
name|png_check_chunk_name
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|png_const_bytep
name|chunk_name
operator|)
argument_list|)
decl_stmt|;
comment|/* Handle the transformations for reading and writing */
name|PNG_EXTERN
name|void
name|png_do_read_transformations
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|)
argument_list|)
decl_stmt|;
name|PNG_EXTERN
name|void
name|png_do_write_transformations
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|)
argument_list|)
decl_stmt|;
name|PNG_EXTERN
name|void
name|png_init_read_transformations
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|)
argument_list|)
decl_stmt|;
ifdef|#
directive|ifdef
name|PNG_PROGRESSIVE_READ_SUPPORTED
name|PNG_EXTERN
name|void
name|png_push_read_chunk
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|png_infop
name|info_ptr
operator|)
argument_list|)
decl_stmt|;
name|PNG_EXTERN
name|void
name|png_push_read_sig
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|png_infop
name|info_ptr
operator|)
argument_list|)
decl_stmt|;
name|PNG_EXTERN
name|void
name|png_push_check_crc
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|)
argument_list|)
decl_stmt|;
name|PNG_EXTERN
name|void
name|png_push_crc_skip
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|png_uint_32
name|length
operator|)
argument_list|)
decl_stmt|;
name|PNG_EXTERN
name|void
name|png_push_crc_finish
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|)
argument_list|)
decl_stmt|;
name|PNG_EXTERN
name|void
name|png_push_save_buffer
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|)
argument_list|)
decl_stmt|;
name|PNG_EXTERN
name|void
name|png_push_restore_buffer
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|png_bytep
name|buffer
operator|,
name|png_size_t
name|buffer_length
operator|)
argument_list|)
decl_stmt|;
name|PNG_EXTERN
name|void
name|png_push_read_IDAT
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|)
argument_list|)
decl_stmt|;
name|PNG_EXTERN
name|void
name|png_process_IDAT_data
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|png_bytep
name|buffer
operator|,
name|png_size_t
name|buffer_length
operator|)
argument_list|)
decl_stmt|;
name|PNG_EXTERN
name|void
name|png_push_process_row
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|)
argument_list|)
decl_stmt|;
name|PNG_EXTERN
name|void
name|png_push_handle_unknown
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|png_infop
name|info_ptr
operator|,
name|png_uint_32
name|length
operator|)
argument_list|)
decl_stmt|;
name|PNG_EXTERN
name|void
name|png_push_have_info
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|png_infop
name|info_ptr
operator|)
argument_list|)
decl_stmt|;
name|PNG_EXTERN
name|void
name|png_push_have_end
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|png_infop
name|info_ptr
operator|)
argument_list|)
decl_stmt|;
name|PNG_EXTERN
name|void
name|png_push_have_row
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|png_bytep
name|row
operator|)
argument_list|)
decl_stmt|;
name|PNG_EXTERN
name|void
name|png_push_read_end
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|png_infop
name|info_ptr
operator|)
argument_list|)
decl_stmt|;
name|PNG_EXTERN
name|void
name|png_process_some_data
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|png_infop
name|info_ptr
operator|)
argument_list|)
decl_stmt|;
name|PNG_EXTERN
name|void
name|png_read_push_finish_row
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|)
argument_list|)
decl_stmt|;
ifdef|#
directive|ifdef
name|PNG_READ_tEXt_SUPPORTED
name|PNG_EXTERN
name|void
name|png_push_handle_tEXt
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|png_infop
name|info_ptr
operator|,
name|png_uint_32
name|length
operator|)
argument_list|)
decl_stmt|;
name|PNG_EXTERN
name|void
name|png_push_read_tEXt
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|png_infop
name|info_ptr
operator|)
argument_list|)
decl_stmt|;
endif|#
directive|endif
ifdef|#
directive|ifdef
name|PNG_READ_zTXt_SUPPORTED
name|PNG_EXTERN
name|void
name|png_push_handle_zTXt
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|png_infop
name|info_ptr
operator|,
name|png_uint_32
name|length
operator|)
argument_list|)
decl_stmt|;
name|PNG_EXTERN
name|void
name|png_push_read_zTXt
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|png_infop
name|info_ptr
operator|)
argument_list|)
decl_stmt|;
endif|#
directive|endif
ifdef|#
directive|ifdef
name|PNG_READ_iTXt_SUPPORTED
name|PNG_EXTERN
name|void
name|png_push_handle_iTXt
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|png_infop
name|info_ptr
operator|,
name|png_uint_32
name|length
operator|)
argument_list|)
decl_stmt|;
name|PNG_EXTERN
name|void
name|png_push_read_iTXt
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|png_infop
name|info_ptr
operator|)
argument_list|)
decl_stmt|;
endif|#
directive|endif
endif|#
directive|endif
comment|/* PNG_PROGRESSIVE_READ_SUPPORTED */
ifdef|#
directive|ifdef
name|PNG_MNG_FEATURES_SUPPORTED
name|PNG_EXTERN
name|void
name|png_do_read_intrapixel
name|PNGARG
argument_list|(
operator|(
name|png_row_infop
name|row_info
operator|,
name|png_bytep
name|row
operator|)
argument_list|)
decl_stmt|;
name|PNG_EXTERN
name|void
name|png_do_write_intrapixel
name|PNGARG
argument_list|(
operator|(
name|png_row_infop
name|row_info
operator|,
name|png_bytep
name|row
operator|)
argument_list|)
decl_stmt|;
endif|#
directive|endif
comment|/* Added at libpng version 1.4.0 */
ifdef|#
directive|ifdef
name|PNG_CHECK_cHRM_SUPPORTED
name|PNG_EXTERN
name|int
name|png_check_cHRM_fixed
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|png_fixed_point
name|int_white_x
operator|,
name|png_fixed_point
name|int_white_y
operator|,
name|png_fixed_point
name|int_red_x
operator|,
name|png_fixed_point
name|int_red_y
operator|,
name|png_fixed_point
name|int_green_x
operator|,
name|png_fixed_point
name|int_green_y
operator|,
name|png_fixed_point
name|int_blue_x
operator|,
name|png_fixed_point
name|int_blue_y
operator|)
argument_list|)
decl_stmt|;
endif|#
directive|endif
ifdef|#
directive|ifdef
name|PNG_CHECK_cHRM_SUPPORTED
comment|/* Added at libpng version 1.2.34 and 1.4.0 */
comment|/* Currently only used by png_check_cHRM_fixed */
name|PNG_EXTERN
name|void
name|png_64bit_product
name|PNGARG
argument_list|(
operator|(
name|long
name|v1
operator|,
name|long
name|v2
operator|,
name|unsigned
name|long
operator|*
name|hi_product
operator|,
name|unsigned
name|long
operator|*
name|lo_product
operator|)
argument_list|)
decl_stmt|;
endif|#
directive|endif
comment|/* Added at libpng version 1.4.0 */
name|PNG_EXTERN
name|void
name|png_check_IHDR
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|png_uint_32
name|width
operator|,
name|png_uint_32
name|height
operator|,
name|int
name|bit_depth
operator|,
name|int
name|color_type
operator|,
name|int
name|interlace_type
operator|,
name|int
name|compression_type
operator|,
name|int
name|filter_type
operator|)
argument_list|)
decl_stmt|;
comment|/* Free all memory used by the read (old method - NOT DLL EXPORTED) */
name|PNG_EXTERN
name|void
name|png_read_destroy
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|png_infop
name|info_ptr
operator|,
name|png_infop
name|end_info_ptr
operator|)
argument_list|)
decl_stmt|;
comment|/* Free any memory used in png_ptr struct (old method - NOT DLL EXPORTED) */
name|PNG_EXTERN
name|void
name|png_write_destroy
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|)
argument_list|)
decl_stmt|;
ifdef|#
directive|ifdef
name|USE_FAR_KEYWORD
comment|/* memory model conversion function */
name|PNG_EXTERN
name|void
modifier|*
name|png_far_to_near
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|png_voidp
name|ptr
operator|,
name|int
name|check
operator|)
argument_list|)
decl_stmt|;
endif|#
directive|endif
comment|/* USE_FAR_KEYWORD */
if|#
directive|if
name|defined
argument_list|(
name|PNG_FLOATING_POINT_SUPPORTED
argument_list|)
operator|&&
name|defined
argument_list|(
name|PNG_ERROR_TEXT_SUPPORTED
argument_list|)
name|PNG_EXTERN
name|PNG_FUNCTION
argument_list|(
name|void
argument_list|,
name|png_fixed_error
argument_list|,
operator|(
name|png_structp
name|png_ptr
operator|,
name|png_const_charp
name|name
operator|)
argument_list|,
name|PNG_NORETURN
argument_list|)
decl_stmt|;
endif|#
directive|endif
comment|/* ASCII to FP interfaces, currently only implemented if sCAL  * support is required.  */
if|#
directive|if
name|defined
argument_list|(
name|PNG_READ_sCAL_SUPPORTED
argument_list|)
comment|/* MAX_DIGITS is actually the maximum number of characters in an sCAL  * width or height, derived from the precision (number of significant  * digits - a build time settable option) and assumpitions about the  * maximum ridiculous exponent.  */
DECL|macro|PNG_sCAL_MAX_DIGITS
define|#
directive|define
name|PNG_sCAL_MAX_DIGITS
value|(PNG_sCAL_PRECISION+1
comment|/*.*/
value|+1
comment|/*E*/
value|+10
comment|/*exponent*/
value|)
ifdef|#
directive|ifdef
name|PNG_FLOATING_POINT_SUPPORTED
name|PNG_EXTERN
name|void
name|png_ascii_from_fp
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|png_charp
name|ascii
operator|,
name|png_size_t
name|size
operator|,
name|double
name|fp
operator|,
name|unsigned
name|int
name|precision
operator|)
argument_list|)
decl_stmt|;
endif|#
directive|endif
comment|/* FLOATING_POINT */
ifdef|#
directive|ifdef
name|PNG_FIXED_POINT_SUPPORTED
name|PNG_EXTERN
name|void
name|png_ascii_from_fixed
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|png_charp
name|ascii
operator|,
name|png_size_t
name|size
operator|,
name|png_fixed_point
name|fp
operator|)
argument_list|)
decl_stmt|;
endif|#
directive|endif
comment|/* FIXED_POINT */
endif|#
directive|endif
comment|/* READ_sCAL */
if|#
directive|if
name|defined
argument_list|(
name|PNG_sCAL_SUPPORTED
argument_list|)
operator|||
name|defined
argument_list|(
name|PNG_pCAL_SUPPORTED
argument_list|)
comment|/* An internal API to validate the format of a floating point number.  * The result is the index of the next character.  If the number is  * not valid it will be the index of a character in the supposed number.  *  * The format of a number is defined in the PNG extensions specification  * and this API is strictly conformant to that spec, not anyone elses!  *  * The format as a regular expression is:  *  * [+-]?[0-9]+.?([Ee][+-]?[0-9]+)?  *  * or:  *  * [+-]?.[0-9]+(.[0-9]+)?([Ee][+-]?[0-9]+)?  *  * The complexity is that either integer or fraction must be present and the  * fraction is permitted to have no digits only if the integer is present.  *  * NOTE: The dangling E problem.  *   There is a PNG valid floating point number in the following:  *  *       PNG floating point numb1.ers are not greedy.  *  *   Working this out requires *TWO* character lookahead (because of the  *   sign), the parser does not do this - it will fail at the 'r' - this  *   doesn't matter for PNG sCAL chunk values, but it requires more care  *   if the value were ever to be embedded in something more complex.  Use  *   ANSI-C strtod if you need the lookahead.  */
comment|/* State table for the parser. */
DECL|macro|PNG_FP_INTEGER
define|#
directive|define
name|PNG_FP_INTEGER
value|0
comment|/* before or in integer */
DECL|macro|PNG_FP_FRACTION
define|#
directive|define
name|PNG_FP_FRACTION
value|1
comment|/* before or in fraction */
DECL|macro|PNG_FP_EXPONENT
define|#
directive|define
name|PNG_FP_EXPONENT
value|2
comment|/* before or in exponent */
DECL|macro|PNG_FP_STATE
define|#
directive|define
name|PNG_FP_STATE
value|3
comment|/* mask for the above */
DECL|macro|PNG_FP_SAW_SIGN
define|#
directive|define
name|PNG_FP_SAW_SIGN
value|4
comment|/* Saw +/- in current state */
DECL|macro|PNG_FP_SAW_DIGIT
define|#
directive|define
name|PNG_FP_SAW_DIGIT
value|8
comment|/* Saw a digit in current state */
DECL|macro|PNG_FP_SAW_DOT
define|#
directive|define
name|PNG_FP_SAW_DOT
value|16
comment|/* Saw a dot in current state */
DECL|macro|PNG_FP_SAW_E
define|#
directive|define
name|PNG_FP_SAW_E
value|32
comment|/* Saw an E (or e) in current state */
DECL|macro|PNG_FP_SAW_ANY
define|#
directive|define
name|PNG_FP_SAW_ANY
value|60
comment|/* Saw any of the above 4 */
DECL|macro|PNG_FP_WAS_VALID
define|#
directive|define
name|PNG_FP_WAS_VALID
value|64
comment|/* Preceding substring is a valid fp number */
DECL|macro|PNG_FP_INVALID
define|#
directive|define
name|PNG_FP_INVALID
value|128
comment|/* Available for callers as a distinct value */
comment|/* Result codes for the parser (boolean - true meants ok, false means  * not ok yet.)  */
DECL|macro|PNG_FP_MAYBE
define|#
directive|define
name|PNG_FP_MAYBE
value|0
comment|/* The number may be valid in the future */
DECL|macro|PNG_FP_OK
define|#
directive|define
name|PNG_FP_OK
value|1
comment|/* The number is valid */
comment|/* The actual parser.  This can be called repeatedly, it updates  * the index into the string and the state variable (which must  * be initialzed to 0).  It returns a result code, as above.  There  * is no point calling the parser any more if it fails to advance to  * the end of the string - it is stuck on an invalid character (or  * terminated by '\0').  *  * Note that the pointer will consume an E or even an E+ then leave  * a 'maybe' state even though a preceding integer.fraction is valid.  * The PNG_FP_WAS_VALID flag indicates that a preceding substring was  * a valid number.  It's possible to recover from this by calling  * the parser again (from the start, with state 0) but with a string  * that omits the last character (i.e. set the size to the index of  * the problem character.)  This has not been tested within libpng.  */
name|PNG_EXTERN
name|int
name|png_check_fp_number
name|PNGARG
argument_list|(
operator|(
name|png_const_charp
name|string
operator|,
name|png_size_t
name|size
operator|,
name|int
operator|*
name|statep
operator|,
name|png_size_tp
name|whereami
operator|)
argument_list|)
decl_stmt|;
comment|/* This is the same but it checks a complete string and returns true  * only if it just contains a floating point number.  */
name|PNG_EXTERN
name|int
name|png_check_fp_string
name|PNGARG
argument_list|(
operator|(
name|png_const_charp
name|string
operator|,
name|png_size_t
name|size
operator|)
argument_list|)
decl_stmt|;
endif|#
directive|endif
comment|/* pCAL || sCAL */
if|#
directive|if
name|defined
argument_list|(
name|PNG_READ_GAMMA_SUPPORTED
argument_list|)
operator|||
expr|\
name|defined
argument_list|(
name|PNG_INCH_CONVERSIONS_SUPPORTED
argument_list|)
operator|||
name|defined
argument_list|(
name|PNG_READ_pHYs_SUPPORTED
argument_list|)
comment|/* Added at libpng version 1.5.0 */
comment|/* This is a utility to provide a*times/div (rounded) and indicate  * if there is an overflow.  The result is a boolean - false (0)  * for overflow, true (1) if no overflow, in which case *res  * holds the result.  */
name|PNG_EXTERN
name|int
name|png_muldiv
name|PNGARG
argument_list|(
operator|(
name|png_fixed_point_p
name|res
operator|,
name|png_fixed_point
name|a
operator|,
name|png_int_32
name|times
operator|,
name|png_int_32
name|div
operator|)
argument_list|)
decl_stmt|;
endif|#
directive|endif
if|#
directive|if
name|defined
argument_list|(
name|PNG_READ_GAMMA_SUPPORTED
argument_list|)
operator|||
name|defined
argument_list|(
name|PNG_INCH_CONVERSIONS_SUPPORTED
argument_list|)
comment|/* Same deal, but issue a warning on overflow and return 0. */
name|PNG_EXTERN
name|png_fixed_point
name|png_muldiv_warn
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|png_fixed_point
name|a
operator|,
name|png_int_32
name|times
operator|,
name|png_int_32
name|div
operator|)
argument_list|)
decl_stmt|;
endif|#
directive|endif
ifdef|#
directive|ifdef
name|PNG_READ_GAMMA_SUPPORTED
comment|/* Calculate a reciprocal - used for gamma values.  This returns  * 0 if the argument is 0 in order to maintain an undefined value,  * there are no warnings.  */
name|PNG_EXTERN
name|png_fixed_point
name|png_reciprocal
name|PNGARG
argument_list|(
operator|(
name|png_fixed_point
name|a
operator|)
argument_list|)
decl_stmt|;
comment|/* The same but gives a reciprocal of the product of two fixed point  * values.  Accuracy is suitable for gamma calculations but this is  * not exact - use png_muldiv for that.  */
name|PNG_EXTERN
name|png_fixed_point
name|png_reciprocal2
name|PNGARG
argument_list|(
operator|(
name|png_fixed_point
name|a
operator|,
name|png_fixed_point
name|b
operator|)
argument_list|)
decl_stmt|;
endif|#
directive|endif
ifdef|#
directive|ifdef
name|PNG_READ_GAMMA_SUPPORTED
comment|/* Internal fixed point gamma correction.  These APIs are called as  * required to convert single values - they don't need to be fast,  * they are not used when processing image pixel values.  *  * While the input is an 'unsigned' value it must actually be the  * correct bit value - 0..255 or 0..65535 as required.  */
name|PNG_EXTERN
name|png_uint_16
name|png_gamma_correct
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|unsigned
name|int
name|value
operator|,
name|png_fixed_point
name|gamma
operator|)
argument_list|)
decl_stmt|;
name|PNG_EXTERN
name|int
name|png_gamma_significant
name|PNGARG
argument_list|(
operator|(
name|png_fixed_point
name|gamma
operator|)
argument_list|)
decl_stmt|;
name|PNG_EXTERN
name|png_uint_16
name|png_gamma_16bit_correct
name|PNGARG
argument_list|(
operator|(
name|unsigned
name|int
name|value
operator|,
name|png_fixed_point
name|gamma
operator|)
argument_list|)
decl_stmt|;
name|PNG_EXTERN
name|png_byte
name|png_gamma_8bit_correct
name|PNGARG
argument_list|(
operator|(
name|unsigned
name|int
name|value
operator|,
name|png_fixed_point
name|gamma
operator|)
argument_list|)
decl_stmt|;
name|PNG_EXTERN
name|void
name|png_build_gamma_table
name|PNGARG
argument_list|(
operator|(
name|png_structp
name|png_ptr
operator|,
name|int
name|bit_depth
operator|)
argument_list|)
decl_stmt|;
endif|#
directive|endif
comment|/* Maintainer: Put new private prototypes here ^ and in libpngpf.3 */
include|#
directive|include
file|"pngdebug.h"
ifdef|#
directive|ifdef
name|__cplusplus
block|}
end_extern
begin_endif
endif|#
directive|endif
end_endif
begin_endif
endif|#
directive|endif
end_endif
begin_comment
comment|/* PNGPRIV_H */
end_comment
end_unit
