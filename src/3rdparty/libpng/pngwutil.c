...
expr_stmt|;
block|}
comment|/* We only need to keep the previous row if we are using one of the following     * filters.     */
if|if
condition|(
operator|(
name|filters
...
operator|-
literal|256
comment|/* so we can detect potential overflow of the                                running sum */
expr_stmt|;
...
comment|/* Overflow is not possible */
block|{
for|for
control|(
name|i
