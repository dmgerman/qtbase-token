...
end_function
begin_comment
comment|/* get a top bucket for specified hash from cache,    * body for FTC_NODE__TOP_FOR_HASH( cache, hash )    */
end_comment
begin_macro
name|FT_LOCAL_DEF
argument_list|(
...
expr_stmt|;
block|}
comment|/* otherwise, the hash table is balanced */
else|else
break|break;
block|}
block|}
...
argument_list|)
expr_stmt|;
comment|/* Lookup a node with exactly same hash and queried properties.  */
...
condition|)
block|{
comment|/* Update bucket by modified linked list */
name|bucket
operator|=
name|pnode
operator|=
...
argument_list|)
expr_stmt|;
comment|/* Update pnode by modified linked list */
while|while
condition|(
operator|*
name|pnode
...
argument_list|(
operator|(
literal|"FTC_Cache_Lookup: oops!!!  node missing\n"
operator|)
argument_list|)
expr_stmt|;
goto|goto
