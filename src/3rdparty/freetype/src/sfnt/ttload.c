...
end_comment
begin_comment
comment|/*   also return the number of valid table entries                  */
end_comment
begin_comment
comment|/*                                                                  */
end_comment
...
condition|)
block|{
comment|/* Some tables have such a simple structure that clipping its     */
comment|/* contents is harmless.  This also makes FreeType less sensitive */
comment|/* to invalid table lengths (which programs like Acroread seem to */
comment|/* ignore in general).                                            */
if|if
condition|(
name|table
operator|.
...
argument_list|(
operator|(
literal|"check_table_dir: no valid tables found\n"
operator|)
argument_list|)
expr_stmt|;
name|error
...
argument_list|()
expr_stmt|;
comment|/* ignore invalid tables that can't be sanitized */
if|if
condition|(
name|entry
operator|.
...
directive|endif
comment|/* make metrics table length a multiple of 4 */
name|entry
operator|.
name|Length
operator|=
...
literal|" (sanitized; original length %08lx)"
operator|,
call|(
name|FT_Char
call|)
...
directive|endif
comment|/* ignore duplicate tables â the first one wins */
name|duplicate
operator|=
literal|0
expr_stmt|;
...
argument_list|)
expr_stmt|;
comment|/* we finally have a valid entry */
name|face
operator|->
name|dir_tables
index|[
...
block|}
block|}
comment|/* final adjustment to number of tables */
name|face
operator|->
name|num_tables
operator|=
