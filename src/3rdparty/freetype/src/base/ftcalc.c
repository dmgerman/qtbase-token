...
name|b_
decl_stmt|;
comment|/* this requires arithmetic right shift of signed numbers */
return|return
call|(
name|FT_Long
call|)
...
return|;
block|}
comment|/* Estimate length and prenormalize by shifting so that */
comment|/* the new approximate length is between 2/3 and 4/3.   */
comment|/* The magic constant 0xAAAAAAAAUL (2/3 of 2^32) helps  */
comment|/* achieve this in 16.16 fixed-point representation.    */
name|l
operator|=
name|x
operator|>
...
name|shift
expr_stmt|;
comment|/* re-estimate length for tiny vectors */
name|l
operator|=
name|x
operator|>
...
expr_stmt|;
block|}
comment|/* lower linear approximation for reciprocal length minus one */
name|b
operator|=
literal|0x10000
operator|-
...
argument_list|)
expr_stmt|;
comment|/* Normalized squared length in the parentheses approaches 2^32. */
comment|/* On two's complement systems, converting to signed gives the   */
comment|/* difference with 2^32 even if the expression wraps around.     */
name|z
operator|=
operator|-
call|(
...
name|v
expr_stmt|;
comment|/* Conversion to signed helps to recover from likely wrap around */
comment|/* in calculating the prenormalized length, because it gives the */
comment|/* correct difference with 2^32 on two's complement systems.     */
name|l
operator|=
call|(
name|FT_UInt32
...
end_comment
begin_endif
unit|FT_BASE_DEF( FT_Int32 )   FT_SqrtFixed( FT_Int32  x )   {     FT_UInt32  root, rem_hi, rem_lo, test_div;     FT_Int     count;       root = 0;      if ( x> 0 )     {       rem_hi = 0;       rem_lo = (FT_UInt32)x;       count  = 24;       do       {         rem_hi   = ( rem_hi<< 2 ) | ( rem_lo>> 30 );         rem_lo<<= 2;         root<<= 1;         test_div = ( root<< 1 ) + 1;          if ( rem_hi>= test_div )         {           rem_hi -= test_div;           root   += 1;         }       } while ( --count );     }      return (FT_Int32)root;   }
endif|#
directive|endif
end_endif
begin_comment
...
comment|/* products might overflow 32 bits */
block|{
name|FT_Int64
name|z1
decl_stmt|,
