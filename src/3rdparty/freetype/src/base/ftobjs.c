...
end_comment
begin_comment
comment|/* the maximum level of warnings: `md5.c' is non-FreeType  */
end_comment
begin_comment
comment|/* code, and it gets used during development builds only.  */
end_comment
begin_pragma
pragma|#
directive|pragma
...
end_comment
begin_comment
comment|/* it's easiest to include `md5.c' directly */
end_comment
begin_include
include|#
directive|include
...
argument_list|)
decl_stmt|;
comment|/* the check for `num_locations' assures that we actually    */
comment|/* test for instructions in a TTF and not in a CFF-based OTF */
comment|/*                                                           */
comment|/* since `maxSizeOfInstructions' might be unreliable, we     */
comment|/* check the size of the `fpgm' and `prep' tables, too --    */
comment|/* the assumption is that there don't exist real TTFs where  */
comment|/* both `fpgm' and `prep' tables are missing                 */
if|if
condition|(
name|mode
operator|==
...
name|stream
expr_stmt|;
comment|/* Stream may have been changed. */
if|if
condition|(
name|error
condition|)
...
name|Exit
goto|;
comment|/* FT2 allocator takes signed long buffer length,        * too large value causing overflow should be checked        */
name|FT_TRACE4
argument_list|(
operator|(
...
argument_list|(
operator|(
literal|"             total buffer size to concatenate %d"
literal|" POST fragments: 0x%08x\n"
operator|,
name|resource_cnt
operator|,
name|pfb_len
...
operator|(
literal|"             too long fragment length makes"
literal|" pfb_len confused: pfb_len=0x%08x\n"
operator|,
name|pfb_len
operator|)
argument_list|)
...
name|Exit2
goto|;
comment|/* FT2 allocator takes signed long buffer length,        * too large fragment length causing overflow should be checked        */
if|if
condition|(
literal|0x7FFFFFFFUL
operator|<
...
argument_list|(
operator|(
literal|"POST fragment[%d]: offsets=0x%08x, rlen=0x%08x, flags=0x%04x\n"
operator|,
name|i
operator|,
name|offsets
...
argument_list|)
expr_stmt|;
comment|/* postpone the check of rlen longer than buffer until FT_Stream_Read() */
if|if
condition|(
operator|(
name|flags
...
literal|0
condition|)
comment|/* Comment, should not be loaded */
block|{
name|FT_TRACE3
argument_list|(
operator|(
literal|"    Skip POST fragment #%d because it is a comment\n"
operator|,
name|i
operator|)
argument_list|)
...
continue|continue;
block|}
comment|/* the flags are part of the resource, so rlen>= 2.  */
comment|/* but some fonts declare rlen = 0 for empty fragment */
if|if
condition|(
name|rlen
operator|>
...
argument_list|(
operator|(
literal|"  Abort due to too-short buffer to store"
literal|" all POST fragments\n"
operator|)
argument_list|)
expr_stmt|;
elseif|else
...
argument_list|(
operator|(
literal|"  Abort due to invalid offset in a POST fragment\n"
operator|)
argument_list|)
expr_stmt|;
if|if
...
name|error
return|;
comment|/* POST resources must be sorted to concatenate properly */
name|error
operator|=
...
return|;
block|}
comment|/* sfnt resources should not be sorted to preserve the face order by        QuickDraw API */
name|error
operator|=
name|FT_Raccess_Get_DataOffsets
argument_list|(
...
argument_list|(
operator|(
literal|"Skip rule %d: darwin vfs resource fork"
literal|" is already checked and"
literal|" no font is found\n"
operator|,
name|i
operator|)
argument_list|)
...
name|limit
decl_stmt|;
comment|/* test for valid `library' delayed to `FT_Stream_New' */
if|if
condition|(
operator|(
operator|!
...
name|Fail3
label|:
comment|/* If we are on the mac, and we get an                          */
comment|/* FT_Err_Invalid_Stream_Operation it may be because we have an */
comment|/* empty data fork, so we need to check the resource fork.      */
if|if
condition|(
name|FT_ERR_NEQ
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* face must be in the driver's list */
elseif|else
if|if
condition|(
name|face
...
argument_list|(
operator|(
literal|"FT_Match_Size: bitmap strike %d matches\n"
operator|,
name|i
operator|)
argument_list|)
...
argument_list|(
operator|(
literal|"FT_Select_Size (font driver's `select_size'):\n"
operator|)
argument_list|)
expr_stmt|;
name|FT_TRACE5
...
argument_list|(
operator|(
literal|"FT_Request_Size (font driver's `request_size'):\n"
operator|)
argument_list|)
expr_stmt|;
name|FT_TRACE5
...
name|req
decl_stmt|;
comment|/* check of `face' delayed to `FT_Request_Size' */
if|if
condition|(
operator|!
name|char_width
...
literal|0
decl_stmt|;
comment|/* only do something if we have a charmap, and we have glyphs at all */
if|if
condition|(
name|face
operator|&&
...
name|offset
decl_stmt|;
comment|/* test for valid `length' delayed to `service->table_info' */
if|if
condition|(
operator|!
name|face
...
name|FT_COMPONENT
value|trace_bitmap
comment|/* we convert to a single bitmap format for computing the checksum */
if|if
condition|(
operator|!
name|error
...
argument_list|)
expr_stmt|;
comment|/* this also converts the bitmap flow to `down' (i.e., pitch> 0) */
name|err
operator|=
...
return|;
block|}
comment|/* check whether we have a service interface */
if|if
condition|(
operator|!
name|cur
...
argument_list|(
operator|(
literal|"%s: module `%s' doesn't support properties\n"
operator|,
name|func_name
operator|,
name|module_name
...
return|;
block|}
comment|/* search property service */
name|interface
operator|=
name|cur
index|[
...
argument_list|(
operator|(
literal|"%s: property service of module `%s' is broken\n"
operator|,
name|func_name
operator|,
name|module_name
...
end_macro
begin_macro
name|FT_Property_Set
argument_list|(
argument|FT_Library        library
argument_list|,
argument|const FT_String*  module_name
argument_list|,
argument|const FT_String*  property_name
argument_list|,
...
argument_list|(
operator|(
literal|"FT_Done_Library: close faces for %s\n"
operator|,
name|module_name
operator|)
argument_list|)
