...
comment|/* do nothing to retain original direction */
block|}
else|else
block|{
operator|*
...
end_function
begin_comment
comment|/* Return the average of `angle1' and `angle2'.            */
end_comment
begin_comment
comment|/* This gives correct result even if `angle1' and `angle2' */
end_comment
begin_comment
comment|/* have opposite signs.                                    */
end_comment
begin_function
specifier|static
name|FT_Angle
...
name|movable
decl_stmt|;
comment|/* TRUE for ends of lineto borders */
DECL|member|start
name|FT_Int
name|start
decl_stmt|;
...
block|{
comment|/* don't add zero-length lineto */
if|if
condition|(
name|border
operator|->
...
name|angle_in
decl_stmt|;
comment|/* direction into curr join */
DECL|member|angle_out
name|FT_Angle
name|angle_out
decl_stmt|;
comment|/* direction out of join  */
DECL|member|center
name|FT_Vector
name|center
decl_stmt|;
...
name|line_length
decl_stmt|;
comment|/* length of last lineto */
DECL|member|first_point
name|FT_Bool
name|first_point
decl_stmt|;
comment|/* is this the start? */
DECL|member|subpath_open
name|FT_Bool
name|subpath_open
decl_stmt|;
comment|/* is the subpath open? */
DECL|member|subpath_angle
name|FT_Angle
name|subpath_angle
decl_stmt|;
comment|/* subpath start direction */
DECL|member|subpath_start
name|FT_Vector
name|subpath_start
decl_stmt|;
comment|/* subpath start position */
DECL|member|subpath_line_length
name|FT_Fixed
name|subpath_line_length
decl_stmt|;
comment|/* subpath start lineto len */
DECL|member|handle_wide_strokes
name|FT_Bool
name|handle_wide_strokes
decl_stmt|;
comment|/* use wide strokes logic? */
...
name|miter_limit
expr_stmt|;
comment|/* ensure miter limit has sensible value */
if|if
condition|(
name|stroker
operator|->
...
literal|0x10000L
expr_stmt|;
comment|/* save line join style:                                           */
comment|/* line join style can be temporarily changed when stroking curves */
name|stroker
operator|->
name|line_join_saved
operator|=
...
end_block
begin_comment
comment|/* create a circular arc at a corner or cap */
end_comment
begin_function
specifier|static
name|FT_Error
...
end_function
begin_comment
comment|/* add a cap at the end of an opened path */
end_comment
begin_function
specifier|static
name|FT_Error
...
name|intersect
decl_stmt|;
comment|/* use intersection of lines? */
name|rotate
operator|=
name|FT_SIDE_TO_ROTATE
argument_list|(
...
block|{
comment|/* compute minimum required length of lines */
name|FT_Fixed
name|min_length
init|=
name|ft_pos_abs
...
block|{
comment|/* this is a mitered (pointed) or beveled (truncated) corner */
name|FT_Fixed
name|sigma
init|=
literal|0
...
argument_list|)
expr_stmt|;
comment|/* is miter limit exceeded? */
if|if
condition|(
name|sigma
operator|<
...
condition|)
block|{
comment|/* don't create variable bevels for very small deviations; */
...
name|bevel
condition|)
comment|/* this is a bevel (broken angle) */
block|{
if|if
condition|(
name|fixed_bevel
condition|)
block|{
comment|/* the outer corners are simply joined together */
name|FT_Vector
name|delta
decl_stmt|;
comment|/* add bevel */
...
comment|/* variable bevel */
block|{
comment|/* the miter is truncated */
name|FT_Vector
name|middle
decl_stmt|,
name|delta
...
name|Exit
goto|;
comment|/* finally, add an end point; only needed if not lineto */
comment|/* (line_length is zero for curves)                     */
if|if
condition|(
name|line_length
operator|==
...
comment|/* this is a miter (intersection) */
block|{
name|FT_Fixed
name|length
decl_stmt|;
...
name|Exit
goto|;
comment|/* now add an end point; only needed if not lineto */
comment|/* (line_length is zero for curves)                */
if|if
condition|(
name|line_length
...
argument_list|)
expr_stmt|;
comment|/* save angle, position, and line length for last join */
comment|/* (line_length is zero for curves)                    */
name|stroker
operator|->
name|subpath_angle
...
name|y
expr_stmt|;
comment|/* a zero-length lineto is a no-op; avoid creating a spurious corner */
if|if
condition|(
name|delta
operator|.
...
name|Exit
goto|;
comment|/* compute length of line */
name|line_length
operator|=
...
name|y
expr_stmt|;
comment|/* the ends of lineto borders are movable */
name|error
operator|=
name|ft_stroke_border_lineto
argument_list|(
...
goto|;
block|}
comment|/* if all control points are coincident, this is a no-op; */
comment|/* avoid creating a spurious corner                       */
if|if
condition|(
name|FT_IS_SMALL
argument_list|(
...
name|angle_out
decl_stmt|;
comment|/* initialize with current direction */
name|angle_in
operator|=
name|angle_out
operator|=
...
condition|)
block|{
comment|/* if the deviation from one arc to the next is too great, */
comment|/* add a round corner                                      */
name|stroker
operator|->
name|center
operator|=
...
argument_list|)
expr_stmt|;
comment|/* reinstate line join style */
name|stroker
operator|->
name|line_join
operator|=
...
argument_list|)
expr_stmt|;
comment|/* compute direction of original arc */
if|if
condition|(
name|stroker
operator|->
...
name|alpha1
decl_stmt|;
comment|/* determine whether the border radius is greater than the */
comment|/* radius of curvature of the original arc                 */
name|start
operator|=
name|border
operator|->
...
argument_list|)
expr_stmt|;
comment|/* is the direction of the border arc opposite to */
comment|/* that of the original arc? */
if|if
condition|(
name|ft_pos_abs
argument_list|(
...
name|alen
decl_stmt|;
comment|/* use the sine rule to find the intersection point */
name|beta
operator|=
name|FT_Atan2
argument_list|(
...
name|y
expr_stmt|;
comment|/* circumnavigate the negative sector backwards */
name|border
operator|->
name|movable
operator|=
...
name|Exit
goto|;
comment|/* and then move to the endpoint */
name|error
operator|=
name|ft_stroke_border_lineto
argument_list|(
...
name|open
expr_stmt|;
comment|/* Determine if we need to check whether the border radius is greater */
comment|/* than the radius of curvature of a curve, to handle this case       */
comment|/* specially.  This is only required if bevel joins or butt caps may  */
comment|/* be created, because round& miter joins and round& square caps    */
comment|/* cover the negative sector created with wide strokes.               */
name|stroker
operator|->
name|handle_wide_strokes
operator|=
...
name|Exit
goto|;
comment|/* don't try to end the path if no segments have been generated */
if|if
condition|(
operator|!
name|stroker
...
name|NULL
decl_stmt|;
comment|/* for FT_OUTLINE_GLYPH_CLASS_GET (in PIC mode) */
name|FT_Library
name|library
init|=
name|stroker
