...
name|x
parameter_list|)
value|( ( (x)< 0 ? -( -(x)<< shift )             \                                  :  (  (x)<< shift ) ) - delta )
name|FT_Vector
name|v_last
decl_stmt|;
name|FT_Vector
...
index|]
expr_stmt|;
comment|/* Counter j cycles though the points; counter i advances only  */
comment|/* when points are moved; anchor k marks the first moved point. */
for|for
control|(
name|i
operator|=
...
literal|0x10000L
expr_stmt|;
comment|/* shift components along lateral bisector in proper orientation */
name|shift
operator|.
name|x
operator|=
...
comment|/* only that is spanned up by the control points.                 */
name|FT_Outline_Get_CBox
argument_list|(
name|outline
argument_list|,
