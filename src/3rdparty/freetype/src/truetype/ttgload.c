...
comment|/* Note that we have to unconditionally load the tweaks since */
comment|/* it is possible that glyphs individually switch ClearType's */
comment|/* backwards compatibility mode on and off.                   */
name|sph_set_tweaks
argument_list|(
name|loader
argument_list|,
...
condition|)
block|{
comment|/* a small outline structure with four elements for */
comment|/* communication with `TT_Vary_Apply_Glyph_Deltas'  */
name|FT_Vector
name|points
index|[
literal|4
...
name|num_subglyphs
expr_stmt|;
comment|/* construct an outline structure for              */
comment|/* communication with `TT_Vary_Apply_Glyph_Deltas' */
name|outline
operator|.
name|n_points
...
control|)
block|{
comment|/* applying deltas for anchor points doesn't make sense, */
comment|/* but we don't have to specially check this since       */
comment|/* unused delta values are zero anyways                  */
name|points
index|[
name|i
index|]
...
name|contours
expr_stmt|;
comment|/* this call provides additional offsets */
...
comment|/*      Deltas must be within signed 16-bit,              */
comment|/*      but the restriction of summed deltas is not clear */
name|subglyph
operator|->
name|arg1
operator|=
...
comment|/* unless FT_LOAD_COMPUTE_METRICS is set                         */
if|if
condition|(
operator|!
name|face
...
block|FT_Bool  compatible_widths;       FT_Bool  symmetrical_smoothing;       FT_Bool  bgr;       FT_Bool  vertical_lcd;       FT_Bool  subpixel_positioned;       FT_Bool  gray_cleartype;
endif|#
directive|endif
endif|#
directive|endif
...
return|;
block|}
comment|/* check whether the cvt program has disabled hinting */
if|if
condition|(
name|exec
operator|->
...
directive|ifdef
name|TT_CONFIG_OPTION_SUBPIXEL_HINTING
comment|/* check whether we have a font hinted for ClearType --           */
comment|/* note that this flag can also be modified in a glyph's bytecode */
if|if
condition|(
name|exec
operator|->
