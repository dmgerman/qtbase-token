...
index|]
decl_stmt|;
comment|/* Record separator in MODE_Csv */
DECL|member|colWidth
name|int
name|colWidth
index|[
...
name|normalMode
decl_stmt|;
comment|/* Holds the mode just before .explain ON */
DECL|member|outfile
name|char
name|outfile
index|[
...
end_decl_stmt
begin_comment
comment|/* ** Output a single term of CSV.  Actually, p->separator is used for ** the separator, which may or may not be a comma.  p->nullvalue is ** the null value.  Strings are quoted if necessary.  The separator ** is only issued if bSep is true. */
end_comment
begin_function
...
literal|"                         LIKE pattern TABLE.\n"
literal|".separator STRING ?NL? Change separator used by output mode and .import\n"
literal|"                         NL is the end-of-line mark for CSV\n"
...
name|stderr
argument_list|,
literal|"Usage: .separator SEPARATOR ?NEWLINE?\n"
argument_list|)
expr_stmt|;
name|rc
operator|=
...
directive|endif
literal|"   -newline SEP         set newline character(s) for CSV\n"
