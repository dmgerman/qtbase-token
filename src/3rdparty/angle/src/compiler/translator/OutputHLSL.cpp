...
literal|"gl_texture"
decl_stmt|;
comment|// We need to include full the sampler type in the function name to make the signature unique
comment|// on D3D11, where samplers are passed to texture functions as indices.
name|name
operator|+=
...
argument_list|)
expr_stmt|;
comment|// Now that we are done changing the AST, do the analyses need for HLSL generation
name|CallDAG
operator|::
name|InitResult
name|success
...
condition|)
block|{
comment|// dx_ViewScale is only used in the fragment shader to correct
comment|// the value for glFragCoord if necessary
name|out
operator|<<
literal|"    float2 dx_ViewScale : packoffset(c3);\n"
expr_stmt|;
block|}
name|out
operator|<<
...
expr_stmt|;
block|}
comment|// dx_ViewAdjust and dx_ViewCoords will only be used in Feature Level 9
comment|// shaders. However, we declare it for all shaders (including Feature Level 10+).
comment|// The bytecode is the same whether we declare it or not, since D3DCompiler removes it
comment|// if it's unused.
name|out
operator|<<
...
name|out
operator|<<
literal|"const uint samplerIndex"
expr_stmt|;
block|}
block|}
if|if
...
literal|"{\n"
expr_stmt|;
comment|// In some cases we use a variable to store the texture/sampler objects, but to work around
comment|// a D3D11 compiler bug related to discard inside a loop that is conditional on texture
comment|// sampling we need to call the function directly on a reference to the array. The bug was
comment|// found using dEQP-GLES3.functional.shaders.discard*loop_texture* tests.
name|TString
...
name|out
operator|<<
literal|"    const uint textureIndex = samplerIndex - textureIndexOffset"
operator|<<
name|suffix
operator|<<
literal|";\n"
...
name|suffix
operator|+
literal|"[textureIndex]"
expr_stmt|;
name|out
operator|<<
literal|"    const uint samplerArrayIndex = samplerIndex - samplerIndexOffset"
operator|<<
name|suffix
operator|<<
literal|";\n"
...
name|textureReference
operator|<<
literal|".GetDimensions(lod, width, height, layers, numberOfLevels);\n"
expr_stmt|;
block|}
else|else
block|{
...
name|textureReference
operator|<<
literal|".GetDimensions(lod, width, height, numberOfLevels);\n"
expr_stmt|;
block|}
block|}
elseif|else
...
name|textureReference
operator|<<
literal|".GetDimensions(lod, width, height, depth, numberOfLevels);\n"
expr_stmt|;
block|}
else|else
name|UNREACHABLE
...
name|textureReference
operator|<<
literal|".GetDimensions(mip, width, height, layers, levels);\n"
expr_stmt|;
name|out
operator|<<
...
literal|"    float2 dx = ddx(tSized);\n"
literal|"    float2 dy = ddy(tSized);\n"
literal|"    float lod = 0.5f * log2(max(dot(dx, dx), dot(dy, dy)));\n"
literal|"    mip = uint(min(max(round(lod), 0), levels - 1));\n"
operator|<<
literal|"    "
operator|<<
name|textureReference
...
name|textureReference
operator|<<
literal|".GetDimensions(0, width, height, layers, levels);\n"
expr_stmt|;
if|if
condition|(
name|textureFunction
...
name|textureReference
operator|<<
literal|".GetDimensions(0, width, height, levels);\n"
expr_stmt|;
if|if
condition|(
name|textureFunction
...
name|textureReference
operator|<<
literal|".GetDimensions(mip, width, height, levels);\n"
expr_stmt|;
block|}
block|}
elseif|else
...
name|textureReference
operator|<<
literal|".GetDimensions(0, width, height, depth, levels);\n"
expr_stmt|;
if|if
condition|(
name|textureFunction
...
name|out
operator|<<
literal|"    float3 tSized = float3(t.x * width, t.y * height, t.z * "
literal|"depth);\n"
literal|"    float dx = length(ddx(tSized));\n"
literal|"    float dy = length(ddy(tSized));\n"
literal|"    float lod = log2(max(dx, dy));\n"
...
name|textureReference
operator|<<
literal|".GetDimensions(mip, width, height, depth, levels);\n"
expr_stmt|;
block|}
else|else
name|UNREACHABLE
...
return|;
block|}
comment|// ArrayReturnValueToOutParameter should have eliminated expressions where a function call is assigned.
name|ASSERT
argument_list|(
name|rightAgg
operator|==
...
comment|// after we initialize uniforms.
name|TIntermBinary
modifier|*
name|deferredInit
init|=
...
name|EOpLogicalOr
case|:
comment|// HLSL doesn't short-circuit ||, so we assume that || affected by short-circuiting have been unfolded.
name|ASSERT
argument_list|(
operator|!
name|node
...
comment|// Also no need to output ; after selection (if) statements or sequences. This is done just
comment|// for code clarity.
...
argument_list|)
decl_stmt|;
comment|// Skip the prototype if it is not implemented (and thus not used)
if|if
condition|(
name|index
operator|==
...
index|]
decl_stmt|;
comment|// The function body node will output braces.
name|ASSERT
argument_list|(
name|IsSequence
argument_list|(
...
condition|)
block|{
comment|// There is no HLSL equivalent for ESSL3 built-in "genType mix (genType x, genType y, genBType a)",
comment|// so use emulated version.
name|ASSERT
argument_list|(
name|node
operator|->
...
condition|)
block|{
comment|// The trueBlock child node will output braces.
name|ASSERT
argument_list|(
name|IsSequence
argument_list|(
...
block|{
comment|// TODO(oetuaho): Check if the semicolon inside is necessary.
comment|// It's there as a result of conservative refactoring of the output.
name|out
operator|<<
literal|"{;}\n"
expr_stmt|;
...
argument_list|)
expr_stmt|;
comment|// Either this is "else if" or the falseBlock child node will output braces.
name|ASSERT
argument_list|(
name|IsSequence
argument_list|(
...
condition|)
block|{
comment|// This is part of unfolded global initialization.
name|mDeferredGlobalInitializers
operator|.
name|push_back
argument_list|(
...
condition|)
block|{
comment|// The loop body node will output braces.
name|ASSERT
argument_list|(
name|IsSequence
argument_list|(
...
condition|)
block|{
comment|// Samplers are passed as indices to the sampler array.
name|ASSERT
argument_list|(
name|qualifier
operator|!=
...
parameter_list|)
block|{
comment|// We support writing constant unions and constructors that only take constant unions as
comment|// parameters as HLSL literals.
if|if
condition|(
name|expression
operator|->
