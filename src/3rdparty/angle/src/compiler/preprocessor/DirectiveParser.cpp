...
return|return;
block|}
comment|// Using double underscores is allowed, but may result in unintended
comment|// behavior, so a warning is issued. At the time of writing this was
comment|// specified in ESSL 3.10, but the intent judging from Khronos
comment|// discussions and dEQP tests was that double underscores should be
comment|// allowed in earlier ESSL versions too.
if|if
condition|(
name|hasDoubleUnderscores
argument_list|(
...
literal|true
expr_stmt|;
comment|// Check if there are extra tokens after #else.
name|mTokenizer
operator|->
name|lex
argument_list|(
...
argument_list|()
expr_stmt|;
comment|// Check if there are tokens after #endif.
name|mTokenizer
operator|->
name|lex
argument_list|(
...
argument_list|)
decl_stmt|;
comment|// Lex the first token after "#line" so we can check it for EOD.
name|macroExpander
operator|.
name|lex
argument_list|(
...
name|errorSettings
decl_stmt|;
comment|// See GLES3 section 12.42
name|errorSettings
operator|.
...
expr_stmt|;
comment|// The first token was lexed earlier to check if it was EOD. Include
comment|// the token in parsing for a second time by setting the
comment|// parsePresetToken flag to true.
name|expressionParser
operator|.
name|parse
argument_list|(
...
expr_stmt|;
comment|// After parsing the line expression expressionParser has also
comment|// advanced to the first token of the file expression - this is the
comment|// token that makes the parser reduce the "input" rule for the line
comment|// expression and stop. So we're using parsePresetToken = true here
comment|// as well.
name|expressionParser
operator|.
name|parse
...
argument_list|)
expr_stmt|;
comment|// Check if there are tokens after #if expression.
if|if
condition|(
operator|!
name|isEOD
...
literal|0
decl_stmt|;
comment|// Check if there are tokens after #ifdef expression.
name|mTokenizer
operator|->
name|lex
argument_list|(
