begin_unit
begin_comment
comment|//
end_comment
begin_comment
comment|// Copyright (c) 2014 The ANGLE Project Authors. All rights reserved.
end_comment
begin_comment
comment|// Use of this source code is governed by a BSD-style license that can be
end_comment
begin_comment
comment|// found in the LICENSE file.
end_comment
begin_comment
comment|//
end_comment
begin_include
include|#
directive|include
file|"libANGLE/Caps.h"
end_include
begin_include
include|#
directive|include
file|"common/debug.h"
end_include
begin_include
include|#
directive|include
file|"common/angleutils.h"
end_include
begin_include
include|#
directive|include
file|"angle_gl.h"
end_include
begin_include
include|#
directive|include
file|<algorithm>
end_include
begin_include
include|#
directive|include
file|<sstream>
end_include
begin_function
DECL|function|InsertExtensionString
specifier|static
name|void
name|InsertExtensionString
parameter_list|(
specifier|const
name|std
operator|::
name|string
modifier|&
name|extension
parameter_list|,
name|bool
name|supported
parameter_list|,
name|std
operator|::
name|vector
argument_list|<
name|std
operator|::
name|string
argument_list|>
modifier|*
name|extensionVector
parameter_list|)
block|{
if|if
condition|(
name|supported
condition|)
block|{
name|extensionVector
operator|->
name|push_back
argument_list|(
name|extension
argument_list|)
expr_stmt|;
block|}
block|}
end_function
begin_namespace
DECL|namespace|gl
namespace|namespace
name|gl
block|{
DECL|function|TextureCaps
name|TextureCaps
operator|::
name|TextureCaps
parameter_list|()
member_init_list|:
name|texturable
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|filterable
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|renderable
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|sampleCounts
argument_list|()
block|{ }
DECL|function|getMaxSamples
name|GLuint
name|TextureCaps
operator|::
name|getMaxSamples
parameter_list|()
specifier|const
block|{
return|return
operator|!
name|sampleCounts
operator|.
name|empty
argument_list|()
condition|?
operator|*
name|sampleCounts
operator|.
name|rbegin
argument_list|()
else|:
literal|0
return|;
block|}
DECL|function|getNearestSamples
name|GLuint
name|TextureCaps
operator|::
name|getNearestSamples
parameter_list|(
name|GLuint
name|requestedSamples
parameter_list|)
specifier|const
block|{
if|if
condition|(
name|requestedSamples
operator|==
literal|0
condition|)
block|{
return|return
literal|0
return|;
block|}
for|for
control|(
name|SupportedSampleSet
operator|::
name|const_iterator
name|i
init|=
name|sampleCounts
operator|.
name|begin
argument_list|()
init|;
name|i
operator|!=
name|sampleCounts
operator|.
name|end
argument_list|()
condition|;
name|i
operator|++
control|)
block|{
name|GLuint
name|samples
init|=
operator|*
name|i
decl_stmt|;
if|if
condition|(
name|samples
operator|>=
name|requestedSamples
condition|)
block|{
return|return
name|samples
return|;
block|}
block|}
return|return
literal|0
return|;
block|}
DECL|function|insert
name|void
name|TextureCapsMap
operator|::
name|insert
parameter_list|(
name|GLenum
name|internalFormat
parameter_list|,
specifier|const
name|TextureCaps
modifier|&
name|caps
parameter_list|)
block|{
name|mCapsMap
index|[
name|internalFormat
index|]
operator|=
name|caps
expr_stmt|;
block|}
DECL|function|remove
name|void
name|TextureCapsMap
operator|::
name|remove
parameter_list|(
name|GLenum
name|internalFormat
parameter_list|)
block|{
name|InternalFormatToCapsMap
operator|::
name|iterator
name|i
init|=
name|mCapsMap
operator|.
name|find
argument_list|(
name|internalFormat
argument_list|)
decl_stmt|;
if|if
condition|(
name|i
operator|!=
name|mCapsMap
operator|.
name|end
argument_list|()
condition|)
block|{
name|mCapsMap
operator|.
name|erase
argument_list|(
name|i
argument_list|)
expr_stmt|;
block|}
block|}
DECL|function|get
specifier|const
name|TextureCaps
modifier|&
name|TextureCapsMap
operator|::
name|get
parameter_list|(
name|GLenum
name|internalFormat
parameter_list|)
specifier|const
block|{
specifier|static
name|TextureCaps
name|defaultUnsupportedTexture
decl_stmt|;
name|InternalFormatToCapsMap
operator|::
name|const_iterator
name|iter
init|=
name|mCapsMap
operator|.
name|find
argument_list|(
name|internalFormat
argument_list|)
decl_stmt|;
return|return
operator|(
name|iter
operator|!=
name|mCapsMap
operator|.
name|end
argument_list|()
operator|)
condition|?
name|iter
operator|->
name|second
else|:
name|defaultUnsupportedTexture
return|;
block|}
DECL|function|begin
name|TextureCapsMap
operator|::
name|const_iterator
name|TextureCapsMap
operator|::
name|begin
parameter_list|()
specifier|const
block|{
return|return
name|mCapsMap
operator|.
name|begin
argument_list|()
return|;
block|}
DECL|function|end
name|TextureCapsMap
operator|::
name|const_iterator
name|TextureCapsMap
operator|::
name|end
parameter_list|()
specifier|const
block|{
return|return
name|mCapsMap
operator|.
name|end
argument_list|()
return|;
block|}
DECL|function|size
name|size_t
name|TextureCapsMap
operator|::
name|size
parameter_list|()
specifier|const
block|{
return|return
name|mCapsMap
operator|.
name|size
argument_list|()
return|;
block|}
DECL|function|Extensions
name|Extensions
operator|::
name|Extensions
parameter_list|()
member_init_list|:
name|elementIndexUint
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|packedDepthStencil
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|getProgramBinary
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|rgb8rgba8
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|textureFormatBGRA8888
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|readFormatBGRA
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|pixelBufferObject
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|mapBuffer
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|mapBufferRange
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|colorBufferHalfFloat
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|textureHalfFloat
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|textureHalfFloatLinear
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|textureFloat
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|textureFloatLinear
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|textureRG
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|textureCompressionDXT1
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|textureCompressionDXT3
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|textureCompressionDXT5
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|textureCompressionASTCHDR
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|textureCompressionASTCLDR
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|compressedETC1RGB8Texture
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|depthTextures
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|depth32
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|textureStorage
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|textureNPOT
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|drawBuffers
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|textureFilterAnisotropic
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|maxTextureAnisotropy
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|occlusionQueryBoolean
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|fence
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|timerQuery
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|disjointTimerQuery
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|queryCounterBitsTimeElapsed
argument_list|(
literal|0
argument_list|)
member_init_list|,
name|queryCounterBitsTimestamp
argument_list|(
literal|0
argument_list|)
member_init_list|,
name|robustness
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|blendMinMax
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|framebufferBlit
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|framebufferMultisample
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|instancedArrays
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|packReverseRowOrder
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|standardDerivatives
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|shaderTextureLOD
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|shaderFramebufferFetch
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|ARMshaderFramebufferFetch
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|NVshaderFramebufferFetch
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|fragDepth
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|textureUsage
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|translatedShaderSource
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|fboRenderMipmap
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|discardFramebuffer
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|debugMarker
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|eglImage
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|eglImageExternal
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|eglImageExternalEssl3
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|unpackSubimage
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|packSubimage
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|vertexArrayObject
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|debug
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|maxDebugMessageLength
argument_list|(
literal|0
argument_list|)
member_init_list|,
name|maxDebugLoggedMessages
argument_list|(
literal|0
argument_list|)
member_init_list|,
name|maxDebugGroupStackDepth
argument_list|(
literal|0
argument_list|)
member_init_list|,
name|maxLabelLength
argument_list|(
literal|0
argument_list|)
member_init_list|,
name|noError
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|lossyETCDecode
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|colorBufferFloat
argument_list|(
literal|false
argument_list|)
block|{ }
DECL|function|getStrings
name|std
operator|::
name|vector
argument_list|<
name|std
operator|::
name|string
argument_list|>
name|Extensions
operator|::
name|getStrings
parameter_list|()
specifier|const
block|{
name|std
operator|::
name|vector
argument_list|<
name|std
operator|::
name|string
argument_list|>
name|extensionStrings
decl_stmt|;
comment|// clang-format off
comment|//                   | Extension name                       | Supported flag           | Output vector   |
name|InsertExtensionString
argument_list|(
literal|"GL_OES_element_index_uint"
argument_list|,
name|elementIndexUint
argument_list|,
operator|&
name|extensionStrings
argument_list|)
expr_stmt|;
name|InsertExtensionString
argument_list|(
literal|"GL_OES_packed_depth_stencil"
argument_list|,
name|packedDepthStencil
argument_list|,
operator|&
name|extensionStrings
argument_list|)
expr_stmt|;
name|InsertExtensionString
argument_list|(
literal|"GL_OES_get_program_binary"
argument_list|,
name|getProgramBinary
argument_list|,
operator|&
name|extensionStrings
argument_list|)
expr_stmt|;
name|InsertExtensionString
argument_list|(
literal|"GL_OES_rgb8_rgba8"
argument_list|,
name|rgb8rgba8
argument_list|,
operator|&
name|extensionStrings
argument_list|)
expr_stmt|;
name|InsertExtensionString
argument_list|(
literal|"GL_EXT_texture_format_BGRA8888"
argument_list|,
name|textureFormatBGRA8888
argument_list|,
operator|&
name|extensionStrings
argument_list|)
expr_stmt|;
name|InsertExtensionString
argument_list|(
literal|"GL_EXT_read_format_bgra"
argument_list|,
name|readFormatBGRA
argument_list|,
operator|&
name|extensionStrings
argument_list|)
expr_stmt|;
name|InsertExtensionString
argument_list|(
literal|"GL_NV_pixel_buffer_object"
argument_list|,
name|pixelBufferObject
argument_list|,
operator|&
name|extensionStrings
argument_list|)
expr_stmt|;
name|InsertExtensionString
argument_list|(
literal|"GL_OES_mapbuffer"
argument_list|,
name|mapBuffer
argument_list|,
operator|&
name|extensionStrings
argument_list|)
expr_stmt|;
name|InsertExtensionString
argument_list|(
literal|"GL_EXT_map_buffer_range"
argument_list|,
name|mapBufferRange
argument_list|,
operator|&
name|extensionStrings
argument_list|)
expr_stmt|;
name|InsertExtensionString
argument_list|(
literal|"GL_EXT_color_buffer_half_float"
argument_list|,
name|colorBufferHalfFloat
argument_list|,
operator|&
name|extensionStrings
argument_list|)
expr_stmt|;
name|InsertExtensionString
argument_list|(
literal|"GL_OES_texture_half_float"
argument_list|,
name|textureHalfFloat
argument_list|,
operator|&
name|extensionStrings
argument_list|)
expr_stmt|;
name|InsertExtensionString
argument_list|(
literal|"GL_OES_texture_half_float_linear"
argument_list|,
name|textureHalfFloatLinear
argument_list|,
operator|&
name|extensionStrings
argument_list|)
expr_stmt|;
name|InsertExtensionString
argument_list|(
literal|"GL_OES_texture_float"
argument_list|,
name|textureFloat
argument_list|,
operator|&
name|extensionStrings
argument_list|)
expr_stmt|;
name|InsertExtensionString
argument_list|(
literal|"GL_OES_texture_float_linear"
argument_list|,
name|textureFloatLinear
argument_list|,
operator|&
name|extensionStrings
argument_list|)
expr_stmt|;
name|InsertExtensionString
argument_list|(
literal|"GL_EXT_texture_rg"
argument_list|,
name|textureRG
argument_list|,
operator|&
name|extensionStrings
argument_list|)
expr_stmt|;
name|InsertExtensionString
argument_list|(
literal|"GL_EXT_texture_compression_dxt1"
argument_list|,
name|textureCompressionDXT1
argument_list|,
operator|&
name|extensionStrings
argument_list|)
expr_stmt|;
name|InsertExtensionString
argument_list|(
literal|"GL_ANGLE_texture_compression_dxt3"
argument_list|,
name|textureCompressionDXT3
argument_list|,
operator|&
name|extensionStrings
argument_list|)
expr_stmt|;
name|InsertExtensionString
argument_list|(
literal|"GL_ANGLE_texture_compression_dxt5"
argument_list|,
name|textureCompressionDXT5
argument_list|,
operator|&
name|extensionStrings
argument_list|)
expr_stmt|;
name|InsertExtensionString
argument_list|(
literal|"GL_KHR_texture_compression_astc_hdr"
argument_list|,
name|textureCompressionASTCHDR
argument_list|,
operator|&
name|extensionStrings
argument_list|)
expr_stmt|;
name|InsertExtensionString
argument_list|(
literal|"GL_KHR_texture_compression_astc_ldr"
argument_list|,
name|textureCompressionASTCLDR
argument_list|,
operator|&
name|extensionStrings
argument_list|)
expr_stmt|;
name|InsertExtensionString
argument_list|(
literal|"GL_OES_compressed_ETC1_RGB8_texture"
argument_list|,
name|compressedETC1RGB8Texture
argument_list|,
operator|&
name|extensionStrings
argument_list|)
expr_stmt|;
name|InsertExtensionString
argument_list|(
literal|"GL_EXT_sRGB"
argument_list|,
name|sRGB
argument_list|,
operator|&
name|extensionStrings
argument_list|)
expr_stmt|;
name|InsertExtensionString
argument_list|(
literal|"GL_ANGLE_depth_texture"
argument_list|,
name|depthTextures
argument_list|,
operator|&
name|extensionStrings
argument_list|)
expr_stmt|;
name|InsertExtensionString
argument_list|(
literal|"GL_OES_depth32"
argument_list|,
name|depth32
argument_list|,
operator|&
name|extensionStrings
argument_list|)
expr_stmt|;
name|InsertExtensionString
argument_list|(
literal|"GL_EXT_texture_storage"
argument_list|,
name|textureStorage
argument_list|,
operator|&
name|extensionStrings
argument_list|)
expr_stmt|;
name|InsertExtensionString
argument_list|(
literal|"GL_OES_texture_npot"
argument_list|,
name|textureNPOT
argument_list|,
operator|&
name|extensionStrings
argument_list|)
expr_stmt|;
name|InsertExtensionString
argument_list|(
literal|"GL_EXT_draw_buffers"
argument_list|,
name|drawBuffers
argument_list|,
operator|&
name|extensionStrings
argument_list|)
expr_stmt|;
name|InsertExtensionString
argument_list|(
literal|"GL_EXT_texture_filter_anisotropic"
argument_list|,
name|textureFilterAnisotropic
argument_list|,
operator|&
name|extensionStrings
argument_list|)
expr_stmt|;
name|InsertExtensionString
argument_list|(
literal|"GL_EXT_occlusion_query_boolean"
argument_list|,
name|occlusionQueryBoolean
argument_list|,
operator|&
name|extensionStrings
argument_list|)
expr_stmt|;
name|InsertExtensionString
argument_list|(
literal|"GL_NV_fence"
argument_list|,
name|fence
argument_list|,
operator|&
name|extensionStrings
argument_list|)
expr_stmt|;
name|InsertExtensionString
argument_list|(
literal|"GL_ANGLE_timer_query"
argument_list|,
name|timerQuery
argument_list|,
operator|&
name|extensionStrings
argument_list|)
expr_stmt|;
name|InsertExtensionString
argument_list|(
literal|"GL_EXT_disjoint_timer_query"
argument_list|,
name|disjointTimerQuery
argument_list|,
operator|&
name|extensionStrings
argument_list|)
expr_stmt|;
name|InsertExtensionString
argument_list|(
literal|"GL_EXT_robustness"
argument_list|,
name|robustness
argument_list|,
operator|&
name|extensionStrings
argument_list|)
expr_stmt|;
name|InsertExtensionString
argument_list|(
literal|"GL_EXT_blend_minmax"
argument_list|,
name|blendMinMax
argument_list|,
operator|&
name|extensionStrings
argument_list|)
expr_stmt|;
name|InsertExtensionString
argument_list|(
literal|"GL_ANGLE_framebuffer_blit"
argument_list|,
name|framebufferBlit
argument_list|,
operator|&
name|extensionStrings
argument_list|)
expr_stmt|;
name|InsertExtensionString
argument_list|(
literal|"GL_ANGLE_framebuffer_multisample"
argument_list|,
name|framebufferMultisample
argument_list|,
operator|&
name|extensionStrings
argument_list|)
expr_stmt|;
name|InsertExtensionString
argument_list|(
literal|"GL_ANGLE_instanced_arrays"
argument_list|,
name|instancedArrays
argument_list|,
operator|&
name|extensionStrings
argument_list|)
expr_stmt|;
name|InsertExtensionString
argument_list|(
literal|"GL_ANGLE_pack_reverse_row_order"
argument_list|,
name|packReverseRowOrder
argument_list|,
operator|&
name|extensionStrings
argument_list|)
expr_stmt|;
name|InsertExtensionString
argument_list|(
literal|"GL_OES_standard_derivatives"
argument_list|,
name|standardDerivatives
argument_list|,
operator|&
name|extensionStrings
argument_list|)
expr_stmt|;
name|InsertExtensionString
argument_list|(
literal|"GL_EXT_shader_texture_lod"
argument_list|,
name|shaderTextureLOD
argument_list|,
operator|&
name|extensionStrings
argument_list|)
expr_stmt|;
name|InsertExtensionString
argument_list|(
literal|"GL_NV_shader_framebuffer_fetch"
argument_list|,
name|NVshaderFramebufferFetch
argument_list|,
operator|&
name|extensionStrings
argument_list|)
expr_stmt|;
name|InsertExtensionString
argument_list|(
literal|"GL_ARM_shader_framebuffer_fetch"
argument_list|,
name|ARMshaderFramebufferFetch
argument_list|,
operator|&
name|extensionStrings
argument_list|)
expr_stmt|;
name|InsertExtensionString
argument_list|(
literal|"GL_EXT_shader_framebuffer_fetch"
argument_list|,
name|shaderFramebufferFetch
argument_list|,
operator|&
name|extensionStrings
argument_list|)
expr_stmt|;
name|InsertExtensionString
argument_list|(
literal|"GL_EXT_frag_depth"
argument_list|,
name|fragDepth
argument_list|,
operator|&
name|extensionStrings
argument_list|)
expr_stmt|;
name|InsertExtensionString
argument_list|(
literal|"GL_ANGLE_texture_usage"
argument_list|,
name|textureUsage
argument_list|,
operator|&
name|extensionStrings
argument_list|)
expr_stmt|;
name|InsertExtensionString
argument_list|(
literal|"GL_ANGLE_translated_shader_source"
argument_list|,
name|translatedShaderSource
argument_list|,
operator|&
name|extensionStrings
argument_list|)
expr_stmt|;
name|InsertExtensionString
argument_list|(
literal|"GL_OES_fbo_render_mipmap"
argument_list|,
name|fboRenderMipmap
argument_list|,
operator|&
name|extensionStrings
argument_list|)
expr_stmt|;
name|InsertExtensionString
argument_list|(
literal|"GL_EXT_discard_framebuffer"
argument_list|,
name|discardFramebuffer
argument_list|,
operator|&
name|extensionStrings
argument_list|)
expr_stmt|;
name|InsertExtensionString
argument_list|(
literal|"GL_EXT_debug_marker"
argument_list|,
name|debugMarker
argument_list|,
operator|&
name|extensionStrings
argument_list|)
expr_stmt|;
name|InsertExtensionString
argument_list|(
literal|"GL_OES_EGL_image"
argument_list|,
name|eglImage
argument_list|,
operator|&
name|extensionStrings
argument_list|)
expr_stmt|;
name|InsertExtensionString
argument_list|(
literal|"GL_OES_EGL_image_external"
argument_list|,
name|eglImageExternal
argument_list|,
operator|&
name|extensionStrings
argument_list|)
expr_stmt|;
name|InsertExtensionString
argument_list|(
literal|"GL_OES_EGL_image_external_essl3"
argument_list|,
name|eglImageExternalEssl3
argument_list|,
operator|&
name|extensionStrings
argument_list|)
expr_stmt|;
name|InsertExtensionString
argument_list|(
literal|"GL_EXT_unpack_subimage"
argument_list|,
name|unpackSubimage
argument_list|,
operator|&
name|extensionStrings
argument_list|)
expr_stmt|;
name|InsertExtensionString
argument_list|(
literal|"GL_NV_pack_subimage"
argument_list|,
name|packSubimage
argument_list|,
operator|&
name|extensionStrings
argument_list|)
expr_stmt|;
name|InsertExtensionString
argument_list|(
literal|"GL_EXT_color_buffer_float"
argument_list|,
name|colorBufferFloat
argument_list|,
operator|&
name|extensionStrings
argument_list|)
expr_stmt|;
name|InsertExtensionString
argument_list|(
literal|"GL_OES_vertex_array_object"
argument_list|,
name|vertexArrayObject
argument_list|,
operator|&
name|extensionStrings
argument_list|)
expr_stmt|;
name|InsertExtensionString
argument_list|(
literal|"GL_KHR_debug"
argument_list|,
name|debug
argument_list|,
operator|&
name|extensionStrings
argument_list|)
expr_stmt|;
comment|// TODO(jmadill): Enable this when complete.
comment|//InsertExtensionString("GL_KHR_no_error",                     noError,&extensionStrings);
name|InsertExtensionString
argument_list|(
literal|"GL_ANGLE_lossy_etc_decode"
argument_list|,
name|lossyETCDecode
argument_list|,
operator|&
name|extensionStrings
argument_list|)
expr_stmt|;
comment|// clang-format on
return|return
name|extensionStrings
return|;
block|}
DECL|function|Limitations
name|Limitations
operator|::
name|Limitations
parameter_list|()
member_init_list|:
name|noFrontFacingSupport
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|noSampleAlphaToCoverageSupport
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|attributeZeroRequiresZeroDivisorInEXT
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|noSeparateStencilRefsAndMasks
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|shadersRequireIndexedLoopValidation
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|noSimultaneousConstantColorAndAlphaBlendFunc
argument_list|(
literal|false
argument_list|)
block|{ }
DECL|function|GetFormatSupport
specifier|static
name|bool
name|GetFormatSupport
parameter_list|(
specifier|const
name|TextureCapsMap
modifier|&
name|textureCaps
parameter_list|,
specifier|const
name|std
operator|::
name|vector
argument_list|<
name|GLenum
argument_list|>
modifier|&
name|requiredFormats
parameter_list|,
name|bool
name|requiresTexturing
parameter_list|,
name|bool
name|requiresFiltering
parameter_list|,
name|bool
name|requiresRendering
parameter_list|)
block|{
for|for
control|(
name|size_t
name|i
init|=
literal|0
init|;
name|i
operator|<
name|requiredFormats
operator|.
name|size
argument_list|()
condition|;
name|i
operator|++
control|)
block|{
specifier|const
name|TextureCaps
modifier|&
name|cap
init|=
name|textureCaps
operator|.
name|get
argument_list|(
name|requiredFormats
index|[
name|i
index|]
argument_list|)
decl_stmt|;
if|if
condition|(
name|requiresTexturing
operator|&&
operator|!
name|cap
operator|.
name|texturable
condition|)
block|{
return|return
literal|false
return|;
block|}
if|if
condition|(
name|requiresFiltering
operator|&&
operator|!
name|cap
operator|.
name|filterable
condition|)
block|{
return|return
literal|false
return|;
block|}
if|if
condition|(
name|requiresRendering
operator|&&
operator|!
name|cap
operator|.
name|renderable
condition|)
block|{
return|return
literal|false
return|;
block|}
block|}
return|return
literal|true
return|;
block|}
comment|// Check for GL_OES_packed_depth_stencil
DECL|function|DeterminePackedDepthStencilSupport
specifier|static
name|bool
name|DeterminePackedDepthStencilSupport
parameter_list|(
specifier|const
name|TextureCapsMap
modifier|&
name|textureCaps
parameter_list|)
block|{
name|std
operator|::
name|vector
argument_list|<
name|GLenum
argument_list|>
name|requiredFormats
decl_stmt|;
name|requiredFormats
operator|.
name|push_back
argument_list|(
name|GL_DEPTH24_STENCIL8
argument_list|)
expr_stmt|;
return|return
name|GetFormatSupport
argument_list|(
name|textureCaps
argument_list|,
name|requiredFormats
argument_list|,
literal|false
argument_list|,
literal|false
argument_list|,
literal|true
argument_list|)
return|;
block|}
comment|// Checks for GL_OES_rgb8_rgba8 support
DECL|function|DetermineRGB8AndRGBA8TextureSupport
specifier|static
name|bool
name|DetermineRGB8AndRGBA8TextureSupport
parameter_list|(
specifier|const
name|TextureCapsMap
modifier|&
name|textureCaps
parameter_list|)
block|{
name|std
operator|::
name|vector
argument_list|<
name|GLenum
argument_list|>
name|requiredFormats
decl_stmt|;
name|requiredFormats
operator|.
name|push_back
argument_list|(
name|GL_RGB8
argument_list|)
expr_stmt|;
name|requiredFormats
operator|.
name|push_back
argument_list|(
name|GL_RGBA8
argument_list|)
expr_stmt|;
return|return
name|GetFormatSupport
argument_list|(
name|textureCaps
argument_list|,
name|requiredFormats
argument_list|,
literal|true
argument_list|,
literal|true
argument_list|,
literal|true
argument_list|)
return|;
block|}
comment|// Checks for GL_EXT_texture_format_BGRA8888 support
DECL|function|DetermineBGRA8TextureSupport
specifier|static
name|bool
name|DetermineBGRA8TextureSupport
parameter_list|(
specifier|const
name|TextureCapsMap
modifier|&
name|textureCaps
parameter_list|)
block|{
name|std
operator|::
name|vector
argument_list|<
name|GLenum
argument_list|>
name|requiredFormats
decl_stmt|;
name|requiredFormats
operator|.
name|push_back
argument_list|(
name|GL_BGRA8_EXT
argument_list|)
expr_stmt|;
return|return
name|GetFormatSupport
argument_list|(
name|textureCaps
argument_list|,
name|requiredFormats
argument_list|,
literal|true
argument_list|,
literal|true
argument_list|,
literal|true
argument_list|)
return|;
block|}
comment|// Checks for GL_OES_color_buffer_half_float support
DECL|function|DetermineColorBufferHalfFloatSupport
specifier|static
name|bool
name|DetermineColorBufferHalfFloatSupport
parameter_list|(
specifier|const
name|TextureCapsMap
modifier|&
name|textureCaps
parameter_list|)
block|{
name|std
operator|::
name|vector
argument_list|<
name|GLenum
argument_list|>
name|requiredFormats
decl_stmt|;
name|requiredFormats
operator|.
name|push_back
argument_list|(
name|GL_RGBA16F
argument_list|)
expr_stmt|;
name|requiredFormats
operator|.
name|push_back
argument_list|(
name|GL_RGB16F
argument_list|)
expr_stmt|;
name|requiredFormats
operator|.
name|push_back
argument_list|(
name|GL_RG16F
argument_list|)
expr_stmt|;
name|requiredFormats
operator|.
name|push_back
argument_list|(
name|GL_R16F
argument_list|)
expr_stmt|;
return|return
name|GetFormatSupport
argument_list|(
name|textureCaps
argument_list|,
name|requiredFormats
argument_list|,
literal|true
argument_list|,
literal|false
argument_list|,
literal|true
argument_list|)
return|;
block|}
comment|// Checks for GL_OES_texture_half_float support
DECL|function|DetermineHalfFloatTextureSupport
specifier|static
name|bool
name|DetermineHalfFloatTextureSupport
parameter_list|(
specifier|const
name|TextureCapsMap
modifier|&
name|textureCaps
parameter_list|)
block|{
name|std
operator|::
name|vector
argument_list|<
name|GLenum
argument_list|>
name|requiredFormats
decl_stmt|;
name|requiredFormats
operator|.
name|push_back
argument_list|(
name|GL_RGB16F
argument_list|)
expr_stmt|;
name|requiredFormats
operator|.
name|push_back
argument_list|(
name|GL_RGBA16F
argument_list|)
expr_stmt|;
return|return
name|GetFormatSupport
argument_list|(
name|textureCaps
argument_list|,
name|requiredFormats
argument_list|,
literal|true
argument_list|,
literal|false
argument_list|,
literal|true
argument_list|)
return|;
block|}
comment|// Checks for GL_OES_texture_half_float_linear support
DECL|function|DetermineHalfFloatTextureFilteringSupport
specifier|static
name|bool
name|DetermineHalfFloatTextureFilteringSupport
parameter_list|(
specifier|const
name|TextureCapsMap
modifier|&
name|textureCaps
parameter_list|)
block|{
name|std
operator|::
name|vector
argument_list|<
name|GLenum
argument_list|>
name|requiredFormats
decl_stmt|;
name|requiredFormats
operator|.
name|push_back
argument_list|(
name|GL_RGB16F
argument_list|)
expr_stmt|;
name|requiredFormats
operator|.
name|push_back
argument_list|(
name|GL_RGBA16F
argument_list|)
expr_stmt|;
return|return
name|DetermineHalfFloatTextureSupport
argument_list|(
name|textureCaps
argument_list|)
operator|&&
name|GetFormatSupport
argument_list|(
name|textureCaps
argument_list|,
name|requiredFormats
argument_list|,
literal|true
argument_list|,
literal|true
argument_list|,
literal|false
argument_list|)
return|;
block|}
comment|// Checks for GL_OES_texture_float support
DECL|function|DetermineFloatTextureSupport
specifier|static
name|bool
name|DetermineFloatTextureSupport
parameter_list|(
specifier|const
name|TextureCapsMap
modifier|&
name|textureCaps
parameter_list|)
block|{
name|std
operator|::
name|vector
argument_list|<
name|GLenum
argument_list|>
name|requiredFormats
decl_stmt|;
name|requiredFormats
operator|.
name|push_back
argument_list|(
name|GL_RGB32F
argument_list|)
expr_stmt|;
name|requiredFormats
operator|.
name|push_back
argument_list|(
name|GL_RGBA32F
argument_list|)
expr_stmt|;
return|return
name|GetFormatSupport
argument_list|(
name|textureCaps
argument_list|,
name|requiredFormats
argument_list|,
literal|true
argument_list|,
literal|false
argument_list|,
literal|true
argument_list|)
return|;
block|}
comment|// Checks for GL_OES_texture_float_linear support
DECL|function|DetermineFloatTextureFilteringSupport
specifier|static
name|bool
name|DetermineFloatTextureFilteringSupport
parameter_list|(
specifier|const
name|TextureCapsMap
modifier|&
name|textureCaps
parameter_list|)
block|{
name|std
operator|::
name|vector
argument_list|<
name|GLenum
argument_list|>
name|requiredFormats
decl_stmt|;
name|requiredFormats
operator|.
name|push_back
argument_list|(
name|GL_RGB32F
argument_list|)
expr_stmt|;
name|requiredFormats
operator|.
name|push_back
argument_list|(
name|GL_RGBA32F
argument_list|)
expr_stmt|;
return|return
name|DetermineFloatTextureSupport
argument_list|(
name|textureCaps
argument_list|)
operator|&&
name|GetFormatSupport
argument_list|(
name|textureCaps
argument_list|,
name|requiredFormats
argument_list|,
literal|true
argument_list|,
literal|true
argument_list|,
literal|false
argument_list|)
return|;
block|}
comment|// Checks for GL_EXT_texture_rg support
DECL|function|DetermineRGTextureSupport
specifier|static
name|bool
name|DetermineRGTextureSupport
parameter_list|(
specifier|const
name|TextureCapsMap
modifier|&
name|textureCaps
parameter_list|,
name|bool
name|checkHalfFloatFormats
parameter_list|,
name|bool
name|checkFloatFormats
parameter_list|)
block|{
name|std
operator|::
name|vector
argument_list|<
name|GLenum
argument_list|>
name|requiredFormats
decl_stmt|;
name|requiredFormats
operator|.
name|push_back
argument_list|(
name|GL_R8
argument_list|)
expr_stmt|;
name|requiredFormats
operator|.
name|push_back
argument_list|(
name|GL_RG8
argument_list|)
expr_stmt|;
if|if
condition|(
name|checkHalfFloatFormats
condition|)
block|{
name|requiredFormats
operator|.
name|push_back
argument_list|(
name|GL_R16F
argument_list|)
expr_stmt|;
name|requiredFormats
operator|.
name|push_back
argument_list|(
name|GL_RG16F
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|checkFloatFormats
condition|)
block|{
name|requiredFormats
operator|.
name|push_back
argument_list|(
name|GL_R32F
argument_list|)
expr_stmt|;
name|requiredFormats
operator|.
name|push_back
argument_list|(
name|GL_RG32F
argument_list|)
expr_stmt|;
block|}
return|return
name|GetFormatSupport
argument_list|(
name|textureCaps
argument_list|,
name|requiredFormats
argument_list|,
literal|true
argument_list|,
literal|true
argument_list|,
literal|false
argument_list|)
return|;
block|}
comment|// Check for GL_EXT_texture_compression_dxt1
DECL|function|DetermineDXT1TextureSupport
specifier|static
name|bool
name|DetermineDXT1TextureSupport
parameter_list|(
specifier|const
name|TextureCapsMap
modifier|&
name|textureCaps
parameter_list|)
block|{
name|std
operator|::
name|vector
argument_list|<
name|GLenum
argument_list|>
name|requiredFormats
decl_stmt|;
name|requiredFormats
operator|.
name|push_back
argument_list|(
name|GL_COMPRESSED_RGB_S3TC_DXT1_EXT
argument_list|)
expr_stmt|;
name|requiredFormats
operator|.
name|push_back
argument_list|(
name|GL_COMPRESSED_RGBA_S3TC_DXT1_EXT
argument_list|)
expr_stmt|;
return|return
name|GetFormatSupport
argument_list|(
name|textureCaps
argument_list|,
name|requiredFormats
argument_list|,
literal|true
argument_list|,
literal|true
argument_list|,
literal|false
argument_list|)
return|;
block|}
comment|// Check for GL_ANGLE_texture_compression_dxt3
DECL|function|DetermineDXT3TextureSupport
specifier|static
name|bool
name|DetermineDXT3TextureSupport
parameter_list|(
specifier|const
name|TextureCapsMap
modifier|&
name|textureCaps
parameter_list|)
block|{
name|std
operator|::
name|vector
argument_list|<
name|GLenum
argument_list|>
name|requiredFormats
decl_stmt|;
name|requiredFormats
operator|.
name|push_back
argument_list|(
name|GL_COMPRESSED_RGBA_S3TC_DXT3_ANGLE
argument_list|)
expr_stmt|;
return|return
name|GetFormatSupport
argument_list|(
name|textureCaps
argument_list|,
name|requiredFormats
argument_list|,
literal|true
argument_list|,
literal|true
argument_list|,
literal|false
argument_list|)
return|;
block|}
comment|// Check for GL_ANGLE_texture_compression_dxt5
DECL|function|DetermineDXT5TextureSupport
specifier|static
name|bool
name|DetermineDXT5TextureSupport
parameter_list|(
specifier|const
name|TextureCapsMap
modifier|&
name|textureCaps
parameter_list|)
block|{
name|std
operator|::
name|vector
argument_list|<
name|GLenum
argument_list|>
name|requiredFormats
decl_stmt|;
name|requiredFormats
operator|.
name|push_back
argument_list|(
name|GL_COMPRESSED_RGBA_S3TC_DXT5_ANGLE
argument_list|)
expr_stmt|;
return|return
name|GetFormatSupport
argument_list|(
name|textureCaps
argument_list|,
name|requiredFormats
argument_list|,
literal|true
argument_list|,
literal|true
argument_list|,
literal|false
argument_list|)
return|;
block|}
comment|// Check for GL_KHR_texture_compression_astc_hdr and GL_KHR_texture_compression_astc_ldr
DECL|function|DetermineASTCTextureSupport
specifier|static
name|bool
name|DetermineASTCTextureSupport
parameter_list|(
specifier|const
name|TextureCapsMap
modifier|&
name|textureCaps
parameter_list|)
block|{
name|std
operator|::
name|vector
argument_list|<
name|GLenum
argument_list|>
name|requiredFormats
decl_stmt|;
name|requiredFormats
operator|.
name|push_back
argument_list|(
name|GL_COMPRESSED_RGBA_ASTC_4x4_KHR
argument_list|)
expr_stmt|;
name|requiredFormats
operator|.
name|push_back
argument_list|(
name|GL_COMPRESSED_RGBA_ASTC_5x4_KHR
argument_list|)
expr_stmt|;
name|requiredFormats
operator|.
name|push_back
argument_list|(
name|GL_COMPRESSED_RGBA_ASTC_5x5_KHR
argument_list|)
expr_stmt|;
name|requiredFormats
operator|.
name|push_back
argument_list|(
name|GL_COMPRESSED_RGBA_ASTC_6x5_KHR
argument_list|)
expr_stmt|;
name|requiredFormats
operator|.
name|push_back
argument_list|(
name|GL_COMPRESSED_RGBA_ASTC_6x6_KHR
argument_list|)
expr_stmt|;
name|requiredFormats
operator|.
name|push_back
argument_list|(
name|GL_COMPRESSED_RGBA_ASTC_8x5_KHR
argument_list|)
expr_stmt|;
name|requiredFormats
operator|.
name|push_back
argument_list|(
name|GL_COMPRESSED_RGBA_ASTC_8x6_KHR
argument_list|)
expr_stmt|;
name|requiredFormats
operator|.
name|push_back
argument_list|(
name|GL_COMPRESSED_RGBA_ASTC_8x8_KHR
argument_list|)
expr_stmt|;
name|requiredFormats
operator|.
name|push_back
argument_list|(
name|GL_COMPRESSED_RGBA_ASTC_10x5_KHR
argument_list|)
expr_stmt|;
name|requiredFormats
operator|.
name|push_back
argument_list|(
name|GL_COMPRESSED_RGBA_ASTC_10x6_KHR
argument_list|)
expr_stmt|;
name|requiredFormats
operator|.
name|push_back
argument_list|(
name|GL_COMPRESSED_RGBA_ASTC_10x8_KHR
argument_list|)
expr_stmt|;
name|requiredFormats
operator|.
name|push_back
argument_list|(
name|GL_COMPRESSED_RGBA_ASTC_10x10_KHR
argument_list|)
expr_stmt|;
name|requiredFormats
operator|.
name|push_back
argument_list|(
name|GL_COMPRESSED_RGBA_ASTC_12x10_KHR
argument_list|)
expr_stmt|;
name|requiredFormats
operator|.
name|push_back
argument_list|(
name|GL_COMPRESSED_RGBA_ASTC_12x12_KHR
argument_list|)
expr_stmt|;
name|requiredFormats
operator|.
name|push_back
argument_list|(
name|GL_COMPRESSED_SRGB8_ALPHA8_ASTC_4x4_KHR
argument_list|)
expr_stmt|;
name|requiredFormats
operator|.
name|push_back
argument_list|(
name|GL_COMPRESSED_SRGB8_ALPHA8_ASTC_5x4_KHR
argument_list|)
expr_stmt|;
name|requiredFormats
operator|.
name|push_back
argument_list|(
name|GL_COMPRESSED_SRGB8_ALPHA8_ASTC_5x5_KHR
argument_list|)
expr_stmt|;
name|requiredFormats
operator|.
name|push_back
argument_list|(
name|GL_COMPRESSED_SRGB8_ALPHA8_ASTC_6x5_KHR
argument_list|)
expr_stmt|;
name|requiredFormats
operator|.
name|push_back
argument_list|(
name|GL_COMPRESSED_SRGB8_ALPHA8_ASTC_6x6_KHR
argument_list|)
expr_stmt|;
name|requiredFormats
operator|.
name|push_back
argument_list|(
name|GL_COMPRESSED_SRGB8_ALPHA8_ASTC_8x5_KHR
argument_list|)
expr_stmt|;
name|requiredFormats
operator|.
name|push_back
argument_list|(
name|GL_COMPRESSED_SRGB8_ALPHA8_ASTC_8x6_KHR
argument_list|)
expr_stmt|;
name|requiredFormats
operator|.
name|push_back
argument_list|(
name|GL_COMPRESSED_SRGB8_ALPHA8_ASTC_8x8_KHR
argument_list|)
expr_stmt|;
name|requiredFormats
operator|.
name|push_back
argument_list|(
name|GL_COMPRESSED_SRGB8_ALPHA8_ASTC_10x5_KHR
argument_list|)
expr_stmt|;
name|requiredFormats
operator|.
name|push_back
argument_list|(
name|GL_COMPRESSED_SRGB8_ALPHA8_ASTC_10x6_KHR
argument_list|)
expr_stmt|;
name|requiredFormats
operator|.
name|push_back
argument_list|(
name|GL_COMPRESSED_SRGB8_ALPHA8_ASTC_10x8_KHR
argument_list|)
expr_stmt|;
name|requiredFormats
operator|.
name|push_back
argument_list|(
name|GL_COMPRESSED_SRGB8_ALPHA8_ASTC_10x10_KHR
argument_list|)
expr_stmt|;
name|requiredFormats
operator|.
name|push_back
argument_list|(
name|GL_COMPRESSED_SRGB8_ALPHA8_ASTC_12x10_KHR
argument_list|)
expr_stmt|;
name|requiredFormats
operator|.
name|push_back
argument_list|(
name|GL_COMPRESSED_SRGB8_ALPHA8_ASTC_12x12_KHR
argument_list|)
expr_stmt|;
return|return
name|GetFormatSupport
argument_list|(
name|textureCaps
argument_list|,
name|requiredFormats
argument_list|,
literal|true
argument_list|,
literal|true
argument_list|,
literal|false
argument_list|)
return|;
block|}
comment|// Check for GL_ETC1_RGB8_OES
DECL|function|DetermineETC1RGB8TextureSupport
specifier|static
name|bool
name|DetermineETC1RGB8TextureSupport
parameter_list|(
specifier|const
name|TextureCapsMap
modifier|&
name|textureCaps
parameter_list|)
block|{
name|std
operator|::
name|vector
argument_list|<
name|GLenum
argument_list|>
name|requiredFormats
decl_stmt|;
name|requiredFormats
operator|.
name|push_back
argument_list|(
name|GL_ETC1_RGB8_OES
argument_list|)
expr_stmt|;
return|return
name|GetFormatSupport
argument_list|(
name|textureCaps
argument_list|,
name|requiredFormats
argument_list|,
literal|true
argument_list|,
literal|true
argument_list|,
literal|false
argument_list|)
return|;
block|}
comment|// Check for GL_ANGLE_texture_compression_dxt5
DECL|function|DetermineSRGBTextureSupport
specifier|static
name|bool
name|DetermineSRGBTextureSupport
parameter_list|(
specifier|const
name|TextureCapsMap
modifier|&
name|textureCaps
parameter_list|)
block|{
name|std
operator|::
name|vector
argument_list|<
name|GLenum
argument_list|>
name|requiredFilterFormats
decl_stmt|;
name|requiredFilterFormats
operator|.
name|push_back
argument_list|(
name|GL_SRGB8
argument_list|)
expr_stmt|;
name|requiredFilterFormats
operator|.
name|push_back
argument_list|(
name|GL_SRGB8_ALPHA8
argument_list|)
expr_stmt|;
name|std
operator|::
name|vector
argument_list|<
name|GLenum
argument_list|>
name|requiredRenderFormats
decl_stmt|;
name|requiredRenderFormats
operator|.
name|push_back
argument_list|(
name|GL_SRGB8_ALPHA8
argument_list|)
expr_stmt|;
return|return
name|GetFormatSupport
argument_list|(
name|textureCaps
argument_list|,
name|requiredFilterFormats
argument_list|,
literal|true
argument_list|,
literal|true
argument_list|,
literal|false
argument_list|)
operator|&&
name|GetFormatSupport
argument_list|(
name|textureCaps
argument_list|,
name|requiredRenderFormats
argument_list|,
literal|true
argument_list|,
literal|false
argument_list|,
literal|true
argument_list|)
return|;
block|}
comment|// Check for GL_ANGLE_depth_texture
DECL|function|DetermineDepthTextureSupport
specifier|static
name|bool
name|DetermineDepthTextureSupport
parameter_list|(
specifier|const
name|TextureCapsMap
modifier|&
name|textureCaps
parameter_list|)
block|{
name|std
operator|::
name|vector
argument_list|<
name|GLenum
argument_list|>
name|requiredFormats
decl_stmt|;
name|requiredFormats
operator|.
name|push_back
argument_list|(
name|GL_DEPTH_COMPONENT16
argument_list|)
expr_stmt|;
name|requiredFormats
operator|.
name|push_back
argument_list|(
name|GL_DEPTH_COMPONENT32_OES
argument_list|)
expr_stmt|;
name|requiredFormats
operator|.
name|push_back
argument_list|(
name|GL_DEPTH24_STENCIL8_OES
argument_list|)
expr_stmt|;
return|return
name|GetFormatSupport
argument_list|(
name|textureCaps
argument_list|,
name|requiredFormats
argument_list|,
literal|true
argument_list|,
literal|true
argument_list|,
literal|true
argument_list|)
return|;
block|}
comment|// Check for GL_OES_depth32
DECL|function|DetermineDepth32Support
specifier|static
name|bool
name|DetermineDepth32Support
parameter_list|(
specifier|const
name|TextureCapsMap
modifier|&
name|textureCaps
parameter_list|)
block|{
name|std
operator|::
name|vector
argument_list|<
name|GLenum
argument_list|>
name|requiredFormats
decl_stmt|;
name|requiredFormats
operator|.
name|push_back
argument_list|(
name|GL_DEPTH_COMPONENT32_OES
argument_list|)
expr_stmt|;
return|return
name|GetFormatSupport
argument_list|(
name|textureCaps
argument_list|,
name|requiredFormats
argument_list|,
literal|false
argument_list|,
literal|false
argument_list|,
literal|true
argument_list|)
return|;
block|}
comment|// Check for GL_EXT_color_buffer_float
DECL|function|DetermineColorBufferFloatSupport
specifier|static
name|bool
name|DetermineColorBufferFloatSupport
parameter_list|(
specifier|const
name|TextureCapsMap
modifier|&
name|textureCaps
parameter_list|)
block|{
name|std
operator|::
name|vector
argument_list|<
name|GLenum
argument_list|>
name|requiredFormats
decl_stmt|;
name|requiredFormats
operator|.
name|push_back
argument_list|(
name|GL_R16F
argument_list|)
expr_stmt|;
name|requiredFormats
operator|.
name|push_back
argument_list|(
name|GL_RG16F
argument_list|)
expr_stmt|;
name|requiredFormats
operator|.
name|push_back
argument_list|(
name|GL_RGBA16F
argument_list|)
expr_stmt|;
name|requiredFormats
operator|.
name|push_back
argument_list|(
name|GL_R32F
argument_list|)
expr_stmt|;
name|requiredFormats
operator|.
name|push_back
argument_list|(
name|GL_RG32F
argument_list|)
expr_stmt|;
name|requiredFormats
operator|.
name|push_back
argument_list|(
name|GL_RGBA32F
argument_list|)
expr_stmt|;
name|requiredFormats
operator|.
name|push_back
argument_list|(
name|GL_R11F_G11F_B10F
argument_list|)
expr_stmt|;
return|return
name|GetFormatSupport
argument_list|(
name|textureCaps
argument_list|,
name|requiredFormats
argument_list|,
literal|true
argument_list|,
literal|false
argument_list|,
literal|true
argument_list|)
return|;
block|}
DECL|function|setTextureExtensionSupport
name|void
name|Extensions
operator|::
name|setTextureExtensionSupport
parameter_list|(
specifier|const
name|TextureCapsMap
modifier|&
name|textureCaps
parameter_list|)
block|{
name|packedDepthStencil
operator|=
name|DeterminePackedDepthStencilSupport
argument_list|(
name|textureCaps
argument_list|)
expr_stmt|;
name|rgb8rgba8
operator|=
name|DetermineRGB8AndRGBA8TextureSupport
argument_list|(
name|textureCaps
argument_list|)
expr_stmt|;
name|textureFormatBGRA8888
operator|=
name|DetermineBGRA8TextureSupport
argument_list|(
name|textureCaps
argument_list|)
expr_stmt|;
name|colorBufferHalfFloat
operator|=
name|DetermineColorBufferHalfFloatSupport
argument_list|(
name|textureCaps
argument_list|)
expr_stmt|;
name|textureHalfFloat
operator|=
name|DetermineHalfFloatTextureSupport
argument_list|(
name|textureCaps
argument_list|)
expr_stmt|;
name|textureHalfFloatLinear
operator|=
name|DetermineHalfFloatTextureFilteringSupport
argument_list|(
name|textureCaps
argument_list|)
expr_stmt|;
name|textureFloat
operator|=
name|DetermineFloatTextureSupport
argument_list|(
name|textureCaps
argument_list|)
expr_stmt|;
name|textureFloatLinear
operator|=
name|DetermineFloatTextureFilteringSupport
argument_list|(
name|textureCaps
argument_list|)
expr_stmt|;
name|textureRG
operator|=
name|DetermineRGTextureSupport
argument_list|(
name|textureCaps
argument_list|,
name|textureHalfFloat
argument_list|,
name|textureFloat
argument_list|)
expr_stmt|;
name|textureCompressionDXT1
operator|=
name|DetermineDXT1TextureSupport
argument_list|(
name|textureCaps
argument_list|)
expr_stmt|;
name|textureCompressionDXT3
operator|=
name|DetermineDXT3TextureSupport
argument_list|(
name|textureCaps
argument_list|)
expr_stmt|;
name|textureCompressionDXT5
operator|=
name|DetermineDXT5TextureSupport
argument_list|(
name|textureCaps
argument_list|)
expr_stmt|;
name|textureCompressionASTCHDR
operator|=
name|DetermineASTCTextureSupport
argument_list|(
name|textureCaps
argument_list|)
expr_stmt|;
name|textureCompressionASTCLDR
operator|=
name|textureCompressionASTCHDR
expr_stmt|;
name|compressedETC1RGB8Texture
operator|=
name|DetermineETC1RGB8TextureSupport
argument_list|(
name|textureCaps
argument_list|)
expr_stmt|;
name|sRGB
operator|=
name|DetermineSRGBTextureSupport
argument_list|(
name|textureCaps
argument_list|)
expr_stmt|;
name|depthTextures
operator|=
name|DetermineDepthTextureSupport
argument_list|(
name|textureCaps
argument_list|)
expr_stmt|;
name|depth32
operator|=
name|DetermineDepth32Support
argument_list|(
name|textureCaps
argument_list|)
expr_stmt|;
name|colorBufferFloat
operator|=
name|DetermineColorBufferFloatSupport
argument_list|(
name|textureCaps
argument_list|)
expr_stmt|;
block|}
DECL|function|TypePrecision
name|TypePrecision
operator|::
name|TypePrecision
parameter_list|()
block|{
name|range
index|[
literal|0
index|]
operator|=
literal|0
expr_stmt|;
name|range
index|[
literal|1
index|]
operator|=
literal|0
expr_stmt|;
name|precision
operator|=
literal|0
expr_stmt|;
block|}
DECL|function|setIEEEFloat
name|void
name|TypePrecision
operator|::
name|setIEEEFloat
parameter_list|()
block|{
name|range
index|[
literal|0
index|]
operator|=
literal|127
expr_stmt|;
name|range
index|[
literal|1
index|]
operator|=
literal|127
expr_stmt|;
name|precision
operator|=
literal|23
expr_stmt|;
block|}
DECL|function|setTwosComplementInt
name|void
name|TypePrecision
operator|::
name|setTwosComplementInt
parameter_list|(
name|unsigned
name|int
name|bits
parameter_list|)
block|{
name|range
index|[
literal|0
index|]
operator|=
name|GLint
argument_list|(
name|bits
argument_list|)
operator|-
literal|1
expr_stmt|;
name|range
index|[
literal|1
index|]
operator|=
name|GLint
argument_list|(
name|bits
argument_list|)
operator|-
literal|2
expr_stmt|;
name|precision
operator|=
literal|0
expr_stmt|;
block|}
DECL|function|setSimulatedInt
name|void
name|TypePrecision
operator|::
name|setSimulatedInt
parameter_list|(
name|unsigned
name|int
name|r
parameter_list|)
block|{
name|range
index|[
literal|0
index|]
operator|=
name|GLint
argument_list|(
name|r
argument_list|)
expr_stmt|;
name|range
index|[
literal|1
index|]
operator|=
name|GLint
argument_list|(
name|r
argument_list|)
expr_stmt|;
name|precision
operator|=
literal|0
expr_stmt|;
block|}
DECL|function|get
name|void
name|TypePrecision
operator|::
name|get
parameter_list|(
name|GLint
modifier|*
name|returnRange
parameter_list|,
name|GLint
modifier|*
name|returnPrecision
parameter_list|)
specifier|const
block|{
name|returnRange
index|[
literal|0
index|]
operator|=
name|range
index|[
literal|0
index|]
expr_stmt|;
name|returnRange
index|[
literal|1
index|]
operator|=
name|range
index|[
literal|1
index|]
expr_stmt|;
operator|*
name|returnPrecision
operator|=
name|precision
expr_stmt|;
block|}
DECL|function|Caps
name|Caps
operator|::
name|Caps
parameter_list|()
member_init_list|:
name|maxElementIndex
argument_list|(
literal|0
argument_list|)
member_init_list|,
name|max3DTextureSize
argument_list|(
literal|0
argument_list|)
member_init_list|,
name|max2DTextureSize
argument_list|(
literal|0
argument_list|)
member_init_list|,
name|maxArrayTextureLayers
argument_list|(
literal|0
argument_list|)
member_init_list|,
name|maxLODBias
argument_list|(
literal|0
argument_list|)
member_init_list|,
name|maxCubeMapTextureSize
argument_list|(
literal|0
argument_list|)
member_init_list|,
name|maxRenderbufferSize
argument_list|(
literal|0
argument_list|)
member_init_list|,
name|maxDrawBuffers
argument_list|(
literal|0
argument_list|)
member_init_list|,
name|maxColorAttachments
argument_list|(
literal|0
argument_list|)
member_init_list|,
name|maxViewportWidth
argument_list|(
literal|0
argument_list|)
member_init_list|,
name|maxViewportHeight
argument_list|(
literal|0
argument_list|)
member_init_list|,
name|minAliasedPointSize
argument_list|(
literal|0
argument_list|)
member_init_list|,
name|maxAliasedPointSize
argument_list|(
literal|0
argument_list|)
member_init_list|,
name|minAliasedLineWidth
argument_list|(
literal|0
argument_list|)
member_init_list|,
comment|// Table 6.29
name|maxElementsIndices
argument_list|(
literal|0
argument_list|)
member_init_list|,
name|maxElementsVertices
argument_list|(
literal|0
argument_list|)
member_init_list|,
name|maxServerWaitTimeout
argument_list|(
literal|0
argument_list|)
member_init_list|,
comment|// Table 6.31
name|maxVertexAttributes
argument_list|(
literal|0
argument_list|)
member_init_list|,
name|maxVertexUniformComponents
argument_list|(
literal|0
argument_list|)
member_init_list|,
name|maxVertexUniformVectors
argument_list|(
literal|0
argument_list|)
member_init_list|,
name|maxVertexUniformBlocks
argument_list|(
literal|0
argument_list|)
member_init_list|,
name|maxVertexOutputComponents
argument_list|(
literal|0
argument_list|)
member_init_list|,
name|maxVertexTextureImageUnits
argument_list|(
literal|0
argument_list|)
member_init_list|,
comment|// Table 6.32
name|maxFragmentUniformComponents
argument_list|(
literal|0
argument_list|)
member_init_list|,
name|maxFragmentUniformVectors
argument_list|(
literal|0
argument_list|)
member_init_list|,
name|maxFragmentUniformBlocks
argument_list|(
literal|0
argument_list|)
member_init_list|,
name|maxFragmentInputComponents
argument_list|(
literal|0
argument_list|)
member_init_list|,
name|maxTextureImageUnits
argument_list|(
literal|0
argument_list|)
member_init_list|,
name|minProgramTexelOffset
argument_list|(
literal|0
argument_list|)
member_init_list|,
name|maxProgramTexelOffset
argument_list|(
literal|0
argument_list|)
member_init_list|,
comment|// Table 6.33
name|maxUniformBufferBindings
argument_list|(
literal|0
argument_list|)
member_init_list|,
name|maxUniformBlockSize
argument_list|(
literal|0
argument_list|)
member_init_list|,
name|uniformBufferOffsetAlignment
argument_list|(
literal|0
argument_list|)
member_init_list|,
name|maxCombinedUniformBlocks
argument_list|(
literal|0
argument_list|)
member_init_list|,
name|maxCombinedVertexUniformComponents
argument_list|(
literal|0
argument_list|)
member_init_list|,
name|maxCombinedFragmentUniformComponents
argument_list|(
literal|0
argument_list|)
member_init_list|,
name|maxVaryingComponents
argument_list|(
literal|0
argument_list|)
member_init_list|,
name|maxVaryingVectors
argument_list|(
literal|0
argument_list|)
member_init_list|,
name|maxCombinedTextureImageUnits
argument_list|(
literal|0
argument_list|)
member_init_list|,
comment|// Table 6.34
name|maxTransformFeedbackInterleavedComponents
argument_list|(
literal|0
argument_list|)
member_init_list|,
name|maxTransformFeedbackSeparateAttributes
argument_list|(
literal|0
argument_list|)
member_init_list|,
name|maxTransformFeedbackSeparateComponents
argument_list|(
literal|0
argument_list|)
member_init_list|,
comment|// Table 6.35
name|maxSamples
argument_list|(
literal|0
argument_list|)
block|{ }
block|}
end_namespace
begin_namespace
DECL|namespace|egl
namespace|namespace
name|egl
block|{
DECL|function|Caps
name|Caps
operator|::
name|Caps
parameter_list|()
member_init_list|:
name|textureNPOT
argument_list|(
literal|false
argument_list|)
block|{ }
DECL|function|DisplayExtensions
name|DisplayExtensions
operator|::
name|DisplayExtensions
parameter_list|()
member_init_list|:
name|createContextRobustness
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|d3dShareHandleClientBuffer
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|surfaceD3DTexture2DShareHandle
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|querySurfacePointer
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|windowFixedSize
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|keyedMutex
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|surfaceOrientation
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|postSubBuffer
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|createContext
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|deviceQuery
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|image
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|imageBase
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|imagePixmap
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|glTexture2DImage
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|glTextureCubemapImage
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|glTexture3DImage
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|glRenderbufferImage
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|getAllProcAddresses
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|flexibleSurfaceCompatibility
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|directComposition
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|createContextNoError
argument_list|(
literal|false
argument_list|)
block|{ }
DECL|function|getStrings
name|std
operator|::
name|vector
argument_list|<
name|std
operator|::
name|string
argument_list|>
name|DisplayExtensions
operator|::
name|getStrings
parameter_list|()
specifier|const
block|{
name|std
operator|::
name|vector
argument_list|<
name|std
operator|::
name|string
argument_list|>
name|extensionStrings
decl_stmt|;
comment|// clang-format off
comment|//                   | Extension name                                 | Supported flag                | Output vector   |
name|InsertExtensionString
argument_list|(
literal|"EGL_EXT_create_context_robustness"
argument_list|,
name|createContextRobustness
argument_list|,
operator|&
name|extensionStrings
argument_list|)
expr_stmt|;
name|InsertExtensionString
argument_list|(
literal|"EGL_ANGLE_d3d_share_handle_client_buffer"
argument_list|,
name|d3dShareHandleClientBuffer
argument_list|,
operator|&
name|extensionStrings
argument_list|)
expr_stmt|;
name|InsertExtensionString
argument_list|(
literal|"EGL_ANGLE_surface_d3d_texture_2d_share_handle"
argument_list|,
name|surfaceD3DTexture2DShareHandle
argument_list|,
operator|&
name|extensionStrings
argument_list|)
expr_stmt|;
name|InsertExtensionString
argument_list|(
literal|"EGL_ANGLE_query_surface_pointer"
argument_list|,
name|querySurfacePointer
argument_list|,
operator|&
name|extensionStrings
argument_list|)
expr_stmt|;
name|InsertExtensionString
argument_list|(
literal|"EGL_ANGLE_window_fixed_size"
argument_list|,
name|windowFixedSize
argument_list|,
operator|&
name|extensionStrings
argument_list|)
expr_stmt|;
name|InsertExtensionString
argument_list|(
literal|"EGL_ANGLE_keyed_mutex"
argument_list|,
name|keyedMutex
argument_list|,
operator|&
name|extensionStrings
argument_list|)
expr_stmt|;
name|InsertExtensionString
argument_list|(
literal|"EGL_ANGLE_surface_orientation"
argument_list|,
name|surfaceOrientation
argument_list|,
operator|&
name|extensionStrings
argument_list|)
expr_stmt|;
name|InsertExtensionString
argument_list|(
literal|"EGL_ANGLE_direct_composition"
argument_list|,
name|directComposition
argument_list|,
operator|&
name|extensionStrings
argument_list|)
expr_stmt|;
name|InsertExtensionString
argument_list|(
literal|"EGL_NV_post_sub_buffer"
argument_list|,
name|postSubBuffer
argument_list|,
operator|&
name|extensionStrings
argument_list|)
expr_stmt|;
name|InsertExtensionString
argument_list|(
literal|"EGL_KHR_create_context"
argument_list|,
name|createContext
argument_list|,
operator|&
name|extensionStrings
argument_list|)
expr_stmt|;
name|InsertExtensionString
argument_list|(
literal|"EGL_EXT_device_query"
argument_list|,
name|deviceQuery
argument_list|,
operator|&
name|extensionStrings
argument_list|)
expr_stmt|;
name|InsertExtensionString
argument_list|(
literal|"EGL_KHR_image"
argument_list|,
name|image
argument_list|,
operator|&
name|extensionStrings
argument_list|)
expr_stmt|;
name|InsertExtensionString
argument_list|(
literal|"EGL_KHR_image_base"
argument_list|,
name|imageBase
argument_list|,
operator|&
name|extensionStrings
argument_list|)
expr_stmt|;
name|InsertExtensionString
argument_list|(
literal|"EGL_KHR_image_pixmap"
argument_list|,
name|imagePixmap
argument_list|,
operator|&
name|extensionStrings
argument_list|)
expr_stmt|;
name|InsertExtensionString
argument_list|(
literal|"EGL_KHR_gl_texture_2D_image"
argument_list|,
name|glTexture2DImage
argument_list|,
operator|&
name|extensionStrings
argument_list|)
expr_stmt|;
name|InsertExtensionString
argument_list|(
literal|"EGL_KHR_gl_texture_cubemap_image"
argument_list|,
name|glTextureCubemapImage
argument_list|,
operator|&
name|extensionStrings
argument_list|)
expr_stmt|;
name|InsertExtensionString
argument_list|(
literal|"EGL_KHR_gl_texture_3D_image"
argument_list|,
name|glTexture3DImage
argument_list|,
operator|&
name|extensionStrings
argument_list|)
expr_stmt|;
name|InsertExtensionString
argument_list|(
literal|"EGL_KHR_gl_renderbuffer_image"
argument_list|,
name|glRenderbufferImage
argument_list|,
operator|&
name|extensionStrings
argument_list|)
expr_stmt|;
name|InsertExtensionString
argument_list|(
literal|"EGL_KHR_get_all_proc_addresses"
argument_list|,
name|getAllProcAddresses
argument_list|,
operator|&
name|extensionStrings
argument_list|)
expr_stmt|;
name|InsertExtensionString
argument_list|(
literal|"EGL_ANGLE_flexible_surface_compatibility"
argument_list|,
name|flexibleSurfaceCompatibility
argument_list|,
operator|&
name|extensionStrings
argument_list|)
expr_stmt|;
comment|// TODO(jmadill): Enable this when complete.
comment|//InsertExtensionString("KHR_create_context_no_error",                   createContextNoError,&extensionStrings);
comment|// clang-format on
return|return
name|extensionStrings
return|;
block|}
DECL|function|DeviceExtensions
name|DeviceExtensions
operator|::
name|DeviceExtensions
parameter_list|()
member_init_list|:
name|deviceD3D
argument_list|(
literal|false
argument_list|)
block|{ }
DECL|function|getStrings
name|std
operator|::
name|vector
argument_list|<
name|std
operator|::
name|string
argument_list|>
name|DeviceExtensions
operator|::
name|getStrings
parameter_list|()
specifier|const
block|{
name|std
operator|::
name|vector
argument_list|<
name|std
operator|::
name|string
argument_list|>
name|extensionStrings
decl_stmt|;
comment|//                   | Extension name                                 | Supported flag                | Output vector   |
name|InsertExtensionString
argument_list|(
literal|"EGL_ANGLE_device_d3d"
argument_list|,
name|deviceD3D
argument_list|,
operator|&
name|extensionStrings
argument_list|)
expr_stmt|;
return|return
name|extensionStrings
return|;
block|}
DECL|function|ClientExtensions
name|ClientExtensions
operator|::
name|ClientExtensions
parameter_list|()
member_init_list|:
name|clientExtensions
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|platformBase
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|platformDevice
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|platformANGLE
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|platformANGLED3D
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|platformANGLEOpenGL
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|deviceCreation
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|deviceCreationD3D11
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|x11Visual
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|experimentalPresentPath
argument_list|(
literal|false
argument_list|)
member_init_list|,
name|clientGetAllProcAddresses
argument_list|(
literal|false
argument_list|)
block|{ }
DECL|function|getStrings
name|std
operator|::
name|vector
argument_list|<
name|std
operator|::
name|string
argument_list|>
name|ClientExtensions
operator|::
name|getStrings
parameter_list|()
specifier|const
block|{
name|std
operator|::
name|vector
argument_list|<
name|std
operator|::
name|string
argument_list|>
name|extensionStrings
decl_stmt|;
comment|// clang-format off
comment|//                   | Extension name                         | Supported flag           | Output vector   |
name|InsertExtensionString
argument_list|(
literal|"EGL_EXT_client_extensions"
argument_list|,
name|clientExtensions
argument_list|,
operator|&
name|extensionStrings
argument_list|)
expr_stmt|;
name|InsertExtensionString
argument_list|(
literal|"EGL_EXT_platform_base"
argument_list|,
name|platformBase
argument_list|,
operator|&
name|extensionStrings
argument_list|)
expr_stmt|;
name|InsertExtensionString
argument_list|(
literal|"EGL_EXT_platform_device"
argument_list|,
name|platformDevice
argument_list|,
operator|&
name|extensionStrings
argument_list|)
expr_stmt|;
name|InsertExtensionString
argument_list|(
literal|"EGL_ANGLE_platform_angle"
argument_list|,
name|platformANGLE
argument_list|,
operator|&
name|extensionStrings
argument_list|)
expr_stmt|;
name|InsertExtensionString
argument_list|(
literal|"EGL_ANGLE_platform_angle_d3d"
argument_list|,
name|platformANGLED3D
argument_list|,
operator|&
name|extensionStrings
argument_list|)
expr_stmt|;
name|InsertExtensionString
argument_list|(
literal|"EGL_ANGLE_platform_angle_opengl"
argument_list|,
name|platformANGLEOpenGL
argument_list|,
operator|&
name|extensionStrings
argument_list|)
expr_stmt|;
name|InsertExtensionString
argument_list|(
literal|"EGL_ANGLE_device_creation"
argument_list|,
name|deviceCreation
argument_list|,
operator|&
name|extensionStrings
argument_list|)
expr_stmt|;
name|InsertExtensionString
argument_list|(
literal|"EGL_ANGLE_device_creation_d3d11"
argument_list|,
name|deviceCreationD3D11
argument_list|,
operator|&
name|extensionStrings
argument_list|)
expr_stmt|;
name|InsertExtensionString
argument_list|(
literal|"EGL_ANGLE_x11_visual"
argument_list|,
name|x11Visual
argument_list|,
operator|&
name|extensionStrings
argument_list|)
expr_stmt|;
name|InsertExtensionString
argument_list|(
literal|"EGL_ANGLE_experimental_present_path"
argument_list|,
name|experimentalPresentPath
argument_list|,
operator|&
name|extensionStrings
argument_list|)
expr_stmt|;
name|InsertExtensionString
argument_list|(
literal|"EGL_KHR_client_get_all_proc_addresses"
argument_list|,
name|clientGetAllProcAddresses
argument_list|,
operator|&
name|extensionStrings
argument_list|)
expr_stmt|;
comment|// clang-format on
return|return
name|extensionStrings
return|;
block|}
block|}
end_namespace
end_unit
