...
condition|)
block|{
comment|// TODO(jmadill): pass back translated name from the shader translator
name|typeString
operator|=
...
condition|)
block|{
comment|// The input type of the instance ID in HLSL (uint) differs from the one in ESSL (int).
name|structHLSL
operator|+=
literal|" uint"
expr_stmt|;
...
comment|// If [...] not all user-defined output variables are written, the values of fragment colors
...
expr_stmt|;
block|}
comment|// Do this after glPointSize, to potentially combine gl_PointCoord and gl_PointSize into the same register.
name|linkHLSL
operator|+=
name|varyingHLSL
expr_stmt|;
...
operator|)
decl_stmt|;
comment|// gl_Position only needs to be outputted from the vertex shader if transform feedback is active.
comment|// This isn't supported on D3D11 Feature Level 9_3, so we don't output gl_Position from the vertex shader in this case.
comment|// This saves us 1 output vector.
name|bool
name|outputPositionFromVS
init|=
operator|!
operator|(
name|shaderModel
...
argument_list|)
decl_stmt|;
comment|// Instanced PointSprite emulation requires that gl_PointCoord is present in the vertex shader VS_OUTPUT
comment|// structure to ensure compatibility with the generated PS_INPUT of the pixel shader.
comment|// GeometryShader PointSprite emulation does not require this additional entry because the
comment|// GS_OUTPUT of the Geometry shader contains the pointCoord value and already matches the PS_INPUT of the
comment|// generated pixel shader.
comment|// The Geometry Shader point sprite implementation needs gl_PointSize to be in VS_OUTPUT and GS_INPUT.
comment|// Instanced point sprites doesn't need gl_PointSize in VS_OUTPUT.
specifier|const
name|SemanticInfo
modifier|&
...
literal|"    output.dx_Position.xyz += float3(input.spriteVertexPos.x * gl_PointSize / (dx_ViewCoords.x*2), input.spriteVertexPos.y * gl_PointSize / (dx_ViewCoords.y*2), input.spriteVertexPos.z) * output.dx_Position.w;\n"
expr_stmt|;
if|if
condition|(
name|usesPointCoord
...
comment|// Other Shader Models (4_0_level_9_3 and 2_x) don't support this, so we emulate it using dx_ViewCoords.
if|if
condition|(
name|shaderModel
operator|>=
