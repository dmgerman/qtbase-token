...
return|;
block|}
comment|// Retrieve an optional property from a property set
...
return|;
block|}
comment|// Attempts to read an optional float property value that is assumed to be in the form of
comment|// an ABI::Windows::Foundation::Single.  This function validates the Single value before returning
comment|// it to the caller.
comment|//
comment|// Possible return values are:
comment|// S_OK, valueExists == true - optional Single value was successfully retrieved and validated
comment|// S_OK, valueExists == false - optional Single value was not found
comment|// E_INVALIDARG, valueExists = false - optional Single value was malformed in the property set.
comment|//    * Incorrect property type ( must be PropertyType_Single)
comment|//    * Invalid property value (must be> 0)
comment|// Additional errors may be returned from IMap or IPropertyValue
comment|//
...
argument_list|)
expr_stmt|;
comment|// Check if the expected Scale property is of PropertyType_Single type.
if|if
condition|(
name|SUCCEEDED
argument_list|(
...
block|{
comment|// An invalid scale was set
name|result
operator|=
name|E_INVALIDARG
expr_stmt|;
...
block|{
comment|// An invalid property type was detected. Size property must be of PropertyType_Single
name|result
operator|=
name|E_INVALIDARG
expr_stmt|;
