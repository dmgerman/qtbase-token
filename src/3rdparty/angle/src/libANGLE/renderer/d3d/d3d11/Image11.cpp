...
argument_list|)
block|{
comment|// mRenderer should remain unchanged during the lifetime of the Image11 object.
comment|// This lets us safely use mRenderer (and its Feature Level) in Image11's methods.
name|mFeatureLevel
operator|=
name|renderer
operator|->
...
name|dim
operator|==
name|D3D11_RESOURCE_DIMENSION_TEXTURE2D
condition|)
block|{
...
name|dim
operator|==
name|D3D11_RESOURCE_DIMENSION_TEXTURE3D
condition|)
block|{
...
argument_list|)
decl_stmt|;
comment|// Currently in ANGLE, the source data may only need to be converted if the source is the current framebuffer
comment|// and OpenGL ES framebuffers must be 2D textures therefore we should not need to convert 3D textures between different formats.
name|ASSERT
argument_list|(
name|dim
operator|==
