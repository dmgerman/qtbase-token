...
name|ERR
argument_list|(
literal|"It is undefined behaviour to clear a render buffer which is not normalized "
literal|"fixed point or floating-"
literal|"point to floating point values (color attachment %u has internal format "
literal|"0x%X)."
argument_list|,
name|colorAttachmentIndex
argument_list|,
...
condition|)
block|{
comment|// Some drivers do not correctly handle calling Clear() on a format with 1-bit alpha.
comment|// They can incorrectly round all non-zero values up to 1.0f. Note that WARP does not do this.
comment|// We should handle the rounding for them instead.
name|clearValues
index|[
literal|3
index|]
