...
condition|)
block|{
comment|// If a divisor is set, it still applies even if an instanced draw was not used, so treat
comment|// as a single-instance draw.
name|instances
operator|=
literal|1
expr_stmt|;
...
block|}
block|}
comment|// The validation layer checks that there is at least one active attribute with a zero divisor as per
comment|// the GL_ANGLE_instanced_arrays spec.
name|ASSERT
argument_list|(
name|indexedAttribute
operator|!=
...
argument_list|,
literal|"Failed to create internal vertex declaration, result: 0x%X."
argument_list|,
name|result
argument_list|)
return|;
