...
expr_stmt|;
block|}
comment|// Disabling mipmapping will always sample from level 0 of the texture. It is possible to work
comment|// around this by modifying D3DSAMP_MAXMIPLEVEL to force a specific mip level to become the
comment|// lowest sampled mip level and using a large negative value for D3DSAMP_MIPMAPLODBIAS to
comment|// ensure that only the base mip level is sampled.
if|if
condition|(
name|baseLevel
operator|>
...
literal|3
return|;
comment|// dx_ViewCoords, dx_ViewAdjust and dx_DepthRange.
block|}
...
literal|false
expr_stmt|;
comment|// textureRG is emulated and not performant.
name|extensions
operator|->
name|textureRG
operator|=
...
operator|)
expr_stmt|;
comment|// Disable depth texture support on AMD cards (See ANGLE issue 839)
if|if
condition|(
name|adapterId
operator|.
...
literal|false
expr_stmt|;
comment|// It would be valid to set this to true, since glDiscardFramebufferEXT is just a hint
name|extensions
operator|->
...
literal|true
expr_stmt|;
comment|// D3D9 has no concept of separate masks and refs for front and back faces in the depth stencil
comment|// state.
name|limitations
operator|->
...
literal|true
expr_stmt|;
comment|// D3D9 shader models have limited support for looping, so the Appendix A
comment|// index/loop limitations are necessary. Workarounds that are needed to
comment|// support dynamic indexing of vectors on HLSL also don't work on D3D9.
name|limitations
operator|->
...
literal|true
expr_stmt|;
comment|// D3D9 cannot support constant color and alpha blend funcs together
name|limitations
operator|->
