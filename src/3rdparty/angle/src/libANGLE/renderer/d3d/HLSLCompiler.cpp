...
end_endif
begin_comment
comment|// ANGLE_APPEND_ASSEMBLY_TO_SHADER_DEBUG_INFO == ANGLE_ENABLED
end_comment
begin_namespace
DECL|namespace|rx
namespace|namespace
...
literal|"gpu.angle"
argument_list|,
literal|"HLSLCompiler::initialize"
argument_list|)
expr_stmt|;
if|#
directive|if
...
name|GL_INVALID_OPERATION
argument_list|,
literal|"Error finding D3DCompile entry point"
argument_list|)
return|;
block|}
name|mInitialized
...
argument_list|(
literal|"GPU.ANGLE.D3DCompileMS"
argument_list|)
expr_stmt|;
name|result
operator|=
...
operator|)
operator|&&
comment|// "cannot have gradient operations inside loops with divergent flow control",
comment|// even though it is counter-intuitive to disable unrolling for this error,
comment|// some very long shaders have trouble deciding which loops to unroll and
comment|// turning off forced unrolls allows them to compile properly.
name|macros
operator|!=
literal|nullptr
condition|)
