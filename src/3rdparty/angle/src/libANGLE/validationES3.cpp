...
parameter_list|)
block|{
comment|// For historical reasons, glTexImage2D and glTexImage3D pass in their internal format as a
comment|// GLint instead of a GLenum. Therefor an invalid internal format gives a GL_INVALID_VALUE
comment|// error instead of a GL_INVALID_ENUM error. As this validation function is only called in
comment|// the validation codepaths for glTexImage2D/3D, we record a GL_INVALID_VALUE error.
specifier|const
name|gl
operator|::
name|InternalFormat
...
name|GL_INVALID_OPERATION
argument_list|,
literal|"Operation only supported on ES 3.0 and above"
argument_list|)
argument_list|)
expr_stmt|;
return|return
...
return|;
block|}
comment|// 3D texture target validation
if|if
condition|(
name|target
operator|!=
...
name|GL_INVALID_ENUM
argument_list|,
literal|"Must specify a valid 3D texture destination target"
argument_list|)
argument_list|)
expr_stmt|;
return|return
...
name|GL_INVALID_OPERATION
argument_list|,
literal|"Context does not support GLES3."
argument_list|)
argument_list|)
expr_stmt|;
return|return
