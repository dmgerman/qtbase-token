...
return|;
block|}
comment|// Windows Phone works around the rotation limitation by using negative values for the swap chain size
if|#
directive|if
name|defined
argument_list|(
...
literal|0
expr_stmt|;
comment|// Landscape/InvertedLandscape
name|mRotateR
operator|=
name|backbufferHeight
operator|<
literal|0
expr_stmt|;
comment|// InvertedPortrait/InvertedLandscape
name|backbufferWidth
operator|=
name|abs
argument_list|(
...
operator|)
comment|// The swap chain is not directly resized on Windows Phone
name|SafeRelease
argument_list|(
name|mBackBufferTexture
argument_list|)
