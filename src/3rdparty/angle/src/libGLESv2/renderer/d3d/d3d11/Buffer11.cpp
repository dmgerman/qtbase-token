...
argument_list|,
literal|"Failed to get internal staging buffer."
argument_list|)
return|;
block|}
if|if
...
name|GL_OUT_OF_MEMORY
argument_list|,
literal|"Failed to resize data resolve buffer."
argument_list|)
return|;
block|}
block|}
...
name|GL_OUT_OF_MEMORY
argument_list|,
literal|"Failed to map internal buffer, result: 0x%X."
argument_list|,
name|result
argument_list|)
return|;
...
literal|0
expr_stmt|;
comment|// Only happens if we initialized the buffer with no data (NULL)
if|if
condition|(
name|mResolvedData
operator|.
