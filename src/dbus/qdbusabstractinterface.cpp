...
namespace|namespace
block|{
comment|// ### Qt6: change to a regular QEvent (customEvent)
comment|// We need to use a QMetaCallEvent here because we can't override customEvent() in
comment|// Qt 5. Since QDBusAbstractInterface is meant to be derived from, the vtables of
comment|// classes in generated code will have a pointer to QObject::customEvent instead
comment|// of to QDBusAbstractInterface::customEvent.
comment|// See solution in Patch Set 1 of this change in the Qt Gerrit servers.
comment|// (https://codereview.qt-project.org/#/c/126384/1)
...
argument_list|)
decl_stmt|;
comment|// Caller supports QVariant returns? Then we can also report errors
comment|// by storing an invalid variant.
if|if
condition|(
operator|!
name|readStatus
...
condition|)
return|return;
comment|// disconnection is just resource freeing, so it can be delayed;
comment|// let's do that later, after all the QObject mutexes have been unlocked.
...
end_function
begin_comment
comment|/*!     \internal     Continues the disconnect notification from above. */
end_comment
begin_function
