...
parameter_list|)
block|{
comment|//    qDebug()<< "exitStates("<< enabledTransitions<< ')';
name|QSet
argument_list|<
name|QAbstractState
modifier|*
...
directive|endif
comment|//    qDebug()<< "enterStates("<< enabledTransitions<< ')';
name|QSet
argument_list|<
name|QAbstractState
modifier|*
...
directive|ifndef
name|QT_NO_ANIMATION
comment|// Gracefully terminate playing animations for states that are exited.
for|for
control|(
name|int
name|i
...
block|}
block|}
comment|// Find the animations to use for the state change.
name|QList
argument_list|<
name|QAbstractAnimation
modifier|*
...
expr_stmt|;
block|}
comment|// Initialize animations from property assignments.
for|for
control|(
name|int
name|i
...
block|}
block|}
comment|// Emit propertiesAssigned signal for entered states that have no animated properties.
for|for
control|(
name|int
name|i
...
end_function
begin_comment
comment|/*!     \internal    Returns true if the variable with the given \a id has been registered for restoration. */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!    \internal     Unregisters the variable identified by \a id */
end_comment
begin_function
...
parameter_list|)
block|{
comment|//    qDebug()<< "unregisterRestorable("<< object<< propertyName<< ')';
name|RestorableId
name|id
argument_list|(
name|object
...
argument_list|()
decl_stmt|;
comment|// If a transition has already been added, then we skip this step, as the
comment|// initial transition in that case has been overridden.
if|if
condition|(
name|transitions
operator|.
...
end_function
begin_comment
comment|/*!    Sets the restore policy of the state machine to \a restorePolicy. The default     restore policy is QAbstractState::DontRestoreProperties.        \sa globalRestorePolicy() */
end_comment
begin_function
...
init|=
block|{
literal|"QSignalEventGenerator\0\0execute()\0"
block|}
decl_stmt|;
end_decl_stmt
begin_decl_stmt
...
case|:
block|{
comment|// ### in Qt 4.6 we can use QObject::senderSignalIndex()
name|QObjectPrivate
modifier|*
name|d
init|=
