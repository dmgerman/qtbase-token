...
end_comment
begin_comment
comment|/*!     \fn QDir&QDir::operator=(QDir&&other)      Move-assigns \a other to this QDir instance.      \since 5.2 */
end_comment
begin_comment
comment|/*!     \internal */
end_comment
...
condition|)
block|{
comment|// Read-only files prevent directory deletion on Windows, retry with Write permission.
specifier|const
name|QFile
operator|::
name|Permissions
...
end_comment
begin_comment
comment|/*!     Returns \a path with redundant directory separators removed,     and "."s and ".."s resolved (as far as possible).      This method is shared with QUrl, so it doesn't deal with QDir::separator(),     nor does it remove the trailing slash, if any. */
end_comment
begin_function
...
condition|)
block|{
comment|// starts with double slash
name|prefixLength
operator|=
literal|2
expr_stmt|;
...
condition|)
block|{
comment|// remember the drive letter
name|prefixLength
operator|=
operator|(
name|len
...
name|prefixLength
expr_stmt|;
comment|// replicate trailing slash (i> 0 checks for emptiness of input string p)
if|if
condition|(
name|i
operator|>
...
continue|continue;
block|}
comment|// remove current directory
if|if
condition|(
name|p
index|[
...
continue|continue;
block|}
comment|// prepend a slash before copying when not empty
if|if
condition|(
operator|!
name|up
...
expr_stmt|;
block|}
comment|// decrement up after copying/skipping
if|if
condition|(
name|up
condition|)
...
literal|'/'
condition|)
comment|// is not empty and there isn't already a '/'
name|out
index|[
operator|--
name|used
...
condition|)
block|{
comment|// Eventhough there is a prefix the out string is a slash. This happens, if the input
comment|// string only consists of a prefix followed by one or more slashes. Just skip the slash.
operator|++
name|used
expr_stmt|;
block|}
...
condition|)
block|{
comment|// After resolving the input path, the resulting string is empty (e.g. "foo/.."). Return
comment|// a dot in that case.
name|out
index|[
operator|--
name|used
...
condition|)
block|{
comment|// After parsing the input string, out only contains a slash. That happens whenever all
comment|// parts are resolved and there is a trailing slash ("./" or "foo/../" for example).
comment|// Prepend a dot to have the correct return value.
name|out
index|[
operator|--
name|used
...
block|}
block|}
comment|// If path was not modified return the original value
name|QString
name|ret
init|=
operator|(
...
end_function
begin_comment
comment|/*!     \macro void Q_INIT_RESOURCE(name)     \relates QDir      Initializes the resources specified by the \c .qrc file with the     specified base \a name. Normally, when resources are built as part     of the application, the resources are loaded automatically at     startup. The Q_INIT_RESOURCE() macro is necessary on some platforms     for resources stored in a static library.      For example, if your application's resources are listed in a file     called \c myapp.qrc, you can ensure that the resources are     initialized at startup by adding this line to your \c main()     function:      \snippet code/src_corelib_io_qdir.cpp 13      If the file name contains characters that cannot be part of a valid C++ function name     (such as '-'), they have to be replaced by the underscore character ('_').      Note: This macro cannot be used in a namespace. It should be called from     main(). If that is not possible, the following workaround can be used     to init the resource \c myapp from the function \c{MyNamespace::myFunction}:      \snippet code/src_corelib_io_qdir.cpp 14      \sa Q_CLEANUP_RESOURCE(), {The Qt Resource System} */
end_comment
begin_comment
