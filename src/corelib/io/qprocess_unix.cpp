...
parameter_list|()
block|{
comment|// can only be called from main thread
name|Q_ASSERT
argument_list|(
operator|!
name|qApp
...
end_function
begin_comment
comment|// under QNX RTOS we have to use vfork() when multithreading
end_comment
begin_function
...
directive|ifdef
name|Q_OS_MAC
comment|// Mac OS X uses UTF8 for exec, regardless of the system locale.
name|argv
index|[
name|i
operator|+
...
condition|)
block|{
comment|// The process manager must be initialized in the main thread
comment|// Note: The call below will re-enter this function, but in the right thread,
comment|// so the else statement below will be executed.
name|QMetaObject
operator|::
name|invokeMethod
argument_list|(
name|qApp
argument_list|,
literal|"_q_initializeProcessManager"
argument_list|,
name|Qt
operator|::
