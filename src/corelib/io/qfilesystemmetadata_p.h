...
block|{
return|return
name|qt_symbian_TTime_To_QDateTime
argument_list|(
name|modificationTime_
argument_list|)
return|;
...
argument|bool isDriveRoot
argument_list|,
argument|const QString&nativeFullFilePath
argument_list|)
block|{
name|fillFromFileAttribute
argument_list|(
...
condition|)
block|{
comment|// Junctions and mount points are implemented as NTFS reparse points.
comment|// But mount points cannot be treated as symlinks because they might
comment|// not have a link target.
name|wchar_t
name|buf
index|[
literal|50
