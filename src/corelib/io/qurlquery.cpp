...
end_comment
begin_comment
comment|/*!     \fn QUrlQuery&QUrlQuery::operator=(QUrlQuery&&other)      Move-assigns \a other to this QUrlQuery instance.      \since 5.2 */
end_comment
begin_typedef
DECL|typedef|Map
typedef|typedef
...
name|d
condition|)
comment|// keep in sync with qHash(QUrlQuery):
return|return
name|d
operator|->
name|valueDelimiter
...
end_function
begin_comment
comment|/*!     \since 5.6     \relates QUrlQuery      Returns the hash value for \a key,     using \a seed to seed the calculation. */
end_comment
begin_function
name|uint
name|qHash
...
name|hash
decl_stmt|;
comment|// keep in sync with operator==:
name|seed
operator|=
name|hash
argument_list|(
