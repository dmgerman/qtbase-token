...
end_comment
begin_comment
comment|/*!     \fn QIODevice::channelBytesWritten(int channel, qint64 bytes)     \since 5.7      This signal is emitted every time a payload of data has been written to     the device. The \a bytes argument is set to the number of bytes that were     written in this payload, while \a channel is the channel they were written     to. Unlike bytesWritten(), it is emitted regardless of the     \l{currentWriteChannel()}{current write channel}.      channelBytesWritten() can be emitted recursively - even for the same     channel.      \sa bytesWritten(), channelReadyRead() */
end_comment
begin_comment
comment|/*!     \fn QIODevice::readyRead()      This signal is emitted once every time new data is available for     reading from the device's current read channel. It will only be emitted     again once new data is available, such as when a new payload of network     data has arrived on your network socket, or when a new block of data has     been appended to your device.      readyRead() is not emitted recursively; if you reenter the event loop or     call waitForReadyRead() inside a slot connected to the readyRead() signal,     the signal will not be reemitted (although waitForReadyRead() may still     return true).      Note for developers implementing classes derived from QIODevice:     you should always emit readyRead() when new data has arrived (do not     emit it only because there's data still to be read in your     buffers). Do not emit readyRead() in other conditions.      \sa bytesWritten() */
end_comment
begin_comment
comment|/*!     \fn QIODevice::channelReadyRead(int channel)     \since 5.7      This signal is emitted when new data is available for reading from the     device. The \a channel argument is set to the index of the read channel on     which the data has arrived. Unlike readyRead(), it is emitted regardless of     the \l{currentReadChannel()}{current read channel}.      channelReadyRead() can be emitted recursively - even for the same channel.      \sa readyRead(), channelBytesWritten() */
end_comment
begin_comment
...
end_function
begin_comment
comment|/*!     \since 5.7      Returns the number of available read channels if the device is open;     otherwise returns 0.      \sa writeChannelCount(), QProcess */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!     \since 5.7      Returns the number of available write channels if the device is open;     otherwise returns 0.      \sa readChannelCount() */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!     \since 5.7      Returns the index of the current read channel.      \sa setCurrentReadChannel(), readChannelCount(), QProcess */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!     \since 5.7      Sets the current read channel of the QIODevice to the given \a     channel. The current input channel is used by the functions     read(), readAll(), readLine(), and getChar(). It also determines     which channel triggers QIODevice to emit readyRead().      \sa currentReadChannel(), readChannelCount(), QProcess */
end_comment
begin_function
...
name|this
argument_list|,
literal|"setReadChannel"
argument_list|,
literal|"Failed due to read transaction being in progress"
argument_list|)
expr_stmt|;
return|return;
block|}
...
name|qDebug
argument_list|(
literal|"%p QIODevice::setCurrentReadChannel(%d), d->currentReadChannel = %d, d->readChannelCount = %d\n"
argument_list|,
name|this
argument_list|,
name|channel
...
end_function
begin_comment
comment|/*!     \since 5.7      Returns the the index of the current write channel.      \sa setCurrentWriteChannel(), writeChannelCount() */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!     \since 5.7      Sets the current write channel of the QIODevice to the given \a     channel. The current output channel is used by the functions     write(), putChar(). It also determines  which channel triggers     QIODevice to emit bytesWritten().      \sa currentWriteChannel(), writeChannelCount() */
end_comment
begin_function
...
name|qDebug
argument_list|(
literal|"%p QIODevice::setCurrentWriteChannel(%d), d->currentWriteChannel = %d, d->writeChannelCount = %d\n"
argument_list|,
name|this
argument_list|,
name|channel
...
condition|)
block|{
comment|// If writeBufferChunkSize is zero (default value), we don't use
comment|// QIODevice's write buffers.
if|if
condition|(
name|writeBufferChunkSize
operator|!=
...
argument_list|)
expr_stmt|;
comment|// Do not clear write buffers to allow delayed close in sockets
name|d
operator|->
name|writeChannelCount
operator|=
...
comment|// operation will then refill the buffer.
name|buffer
operator|.
name|clear
argument_list|()
...
block|{
comment|// Do not read more than maxSize on unbuffered devices
specifier|const
name|qint64
name|bytesToBuffer
init|=
...
condition|)
block|{
comment|// Peek line from the specified position
specifier|const
name|qint64
name|i
init|=
...
condition|)
block|{
comment|// QRingBuffer::readLine() terminates the line with '\0'
name|readSoFar
operator|=
name|d
operator|->
...
literal|false
expr_stmt|;
comment|// Force base implementation for transaction on sequential device
comment|// as it stores the data in internal buffer automatically.
name|qint64
name|readBytes
init|=
name|keepDataInBuffer
...
name|this
argument_list|,
literal|"startTransaction"
argument_list|,
literal|"Called while transaction already in progress"
argument_list|)
expr_stmt|;
return|return;
block|}
...
end_function
begin_comment
comment|/*!     \since 5.7      Completes a read transaction.      For sequential devices, all data recorded in the internal buffer during     the transaction will be discarded.      \sa startTransaction(), rollbackTransaction() */
end_comment
begin_function
...
argument_list|,
literal|"Called while no transaction in progress"
argument_list|)
expr_stmt|;
return|return;
block|}
...
end_function
begin_comment
comment|/*!     \since 5.7      Rolls back a read transaction.      Restores the input stream to the point of the startTransaction() call.     This function is commonly used to rollback the transaction when an     incomplete read was detected prior to committing the transaction.      \sa startTransaction(), commitTransaction() */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!     \since 5.7      Returns \c true if a transaction is in progress on the device, otherwise     \c false.      \sa startTransaction() */
end_comment
begin_function
...
literal|"ungetChar"
argument_list|,
literal|"Called while transaction is in progress"
argument_list|)
expr_stmt|;
return|return;
block|}
...
argument_list|)
decl_stmt|;
comment|// Restore initial position
if|if
condition|(
name|isSequential
argument_list|()
