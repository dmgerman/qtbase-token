...
argument_list|)
expr_stmt|;
comment|// calling padding() when no padding is needed is an error
comment|// Do NOT break NRVO in this function or kittens will die!
name|PaddingResult
name|result
decl_stmt|;
specifier|const
...
condition|)
block|{
comment|// write the sign before the padding, then skip it later
name|write
argument_list|(
operator|&
name|sign
...
end_function
begin_comment
comment|/*!     Reads the entire content of the stream, and returns it as a     QString. Avoid this function when working on large files, as it     will consume a significant amount of memory.      Calling \l {QTextStream::readLine()}{readLine()} is better if you do not know how much data is     available.      \sa readLine() */
end_comment
begin_function
...
argument_list|)
expr_stmt|;
comment|// keep in sync with CHECK_VALID_STREAM
if|if
condition|(
operator|!
name|d
...
end_function
begin_comment
comment|/*!     \overload      Writes \a string to the stream, and returns a reference to the     QTextStream. */
end_comment
begin_function
DECL|function|operator <<
name|QTextStream
...
end_function
begin_comment
comment|/*!     \since 5.6     \overload      Writes \a string to the stream, and returns a reference to the     QTextStream. */
end_comment
begin_function
DECL|function|operator <<
name|QTextStream
