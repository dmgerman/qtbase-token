...
end_ifdef
begin_comment
comment|// on Windows, read() and write() use int and unsigned int
end_comment
begin_typedef
...
argument_list|)
argument_list|,
literal|"Unsupported: read/write return a type with different size as the len parameter"
argument_list|)
expr_stmt|;
end_expr_stmt
begin_endif
...
argument_list|()
decl_stmt|;
comment|// Make a copy since unmap() modifies the map.
for|for
control|(
name|int
name|i
...
do|do
block|{
comment|// calculate the chunk size
comment|// on Windows or 32-bit no-largefile Unix, we'll need to read in chunks
comment|// we limit to the size of the signed type, otherwise we could get a negative number as a result
name|quint64
name|wantedBytes
init|=
name|quint64
