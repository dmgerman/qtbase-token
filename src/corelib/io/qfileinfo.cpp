...
argument_list|)
decl_stmt|;
comment|// Expensive fallback to non-QFileSystemEngine implementation
if|if
condition|(
name|engine
condition|)
...
end_function
begin_comment
comment|/*!     \since 4.3     Returns the name of the bundle.      On OS X and iOS this returns the proper localized name for a bundle if the     path isBundle(). On all other platforms an empty QString is returned.      Example:     \snippet code/src_corelib_io_qfileinfo.cpp 4      \sa isBundle(), filePath(), baseName(), suffix() */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!     Returns \c true if the user can read the file; otherwise returns \c false.      \note If the \l{NTFS permissions} check has not been enabled, the result     on Windows will merely reflect whether the file exists.      \sa isWritable(), isExecutable(), permission() */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!     Returns \c true if the user can write to the file; otherwise returns \c false.      \note If the \l{NTFS permissions} check has not been enabled, the result on     Windows will merely reflect whether the file is marked as Read Only.      \sa isReadable(), isExecutable(), permission() */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!     \since 4.3     Returns \c true if this object points to a bundle or to a symbolic     link to a bundle on OS X and iOS; otherwise returns \c false.      \sa isDir(), isSymLink(), isFile() */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!     Returns \c true if this object points to a symbolic link (or to a     shortcut on Windows); otherwise returns \c false.      On Unix (including OS X and iOS), opening a symlink effectively opens     the \l{symLinkTarget()}{link's target}. On Windows, it opens the \c     .lnk file itself.      Example:      \snippet code/src_corelib_io_qfileinfo.cpp 9      \note If the symlink points to a non existing file, exists() returns      false.      \sa isFile(), isDir(), symLinkTarget() */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!     Returns the owner of the file. On systems where files     do not have owners, or if an error occurs, an empty string is     returned.      This function can be time consuming under Unix (in the order of     milliseconds). On Windows, it will return an empty string unless     the \l{NTFS permissions} check has been enabled.      \sa ownerId(), group(), groupId() */
end_comment
begin_function
DECL|function|owner
name|QString
...
end_function
begin_comment
comment|/*!     Tests for file permissions. The \a permissions argument can be     several flags of type QFile::Permissions OR-ed together to check     for permission combinations.      On systems where files do not have permissions this function     always returns \c true.      \note The result might be inaccurate on Windows if the     \l{NTFS permissions} check has not been enabled.      Example:     \snippet code/src_corelib_io_qfileinfo.cpp 10      \sa isReadable(), isWritable(), isExecutable() */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!     Returns the complete OR-ed together combination of     QFile::Permissions for the file.      \note The result might be inaccurate on Windows if the     \l{NTFS permissions} check has not been enabled. */
end_comment
begin_function
