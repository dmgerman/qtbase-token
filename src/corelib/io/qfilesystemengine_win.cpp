...
directive|else
comment|// Some WinRT APIs do not support absolute paths (due to sandboxing).
comment|// Thus the port uses the executable's directory as its root directory
comment|// and treats paths relative to that as absolute paths.
name|ret
operator|=
name|QDir
operator|::
...
name|lastError
condition|)
comment|// Capture lastError before any QString is freed since custom allocators might change it.
operator|*
name|lastError
operator|=
name|GetLastError
...
argument_list|)
expr_stmt|;
comment|// We try to create something outside the sandbox, which is forbidden
comment|// However we could still try to pass into the sandbox
if|if
condition|(
name|dataLocation
operator|.
...
name|Q_OS_WINRT
argument_list|)
comment|// We specify the package root as root directory
name|QString
name|ret
init|=
name|QLatin1String
...
argument_list|)
decl_stmt|;
comment|// Get package location
name|ComPtr
argument_list|<
name|IPackageStatics
argument_list|>
name|statics
decl_stmt|;
if|if
...
argument_list|(
name|RuntimeClass_Windows_ApplicationModel_Package
argument_list|)
operator|.
name|Get
argument_list|()
...
directive|else
comment|// !Q_OS_WINRT
comment|// According to http://msdn.microsoft.com/en-us/library/windows/apps/windows.storage.applicationdata.temporaryfolder.aspx
comment|// the API is not available on winphone which should cause one of the functions
comment|// below to fail
name|ComPtr
argument_list|<
...
directive|else
comment|// !Q_OS_WINRT
name|COPYFILE2_EXTENDED_PARAMETERS
name|copyParams
init|=
block|{
sizeof|sizeof
