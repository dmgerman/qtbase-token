...
end_endif
begin_comment
comment|// enable gcc warnings for printf-style functions
end_comment
begin_if
if|#
directive|if
...
parameter_list|)
define|\
value|__attribute__((format(gnu_printf, (A), (B))))
end_define
begin_else
else|#
directive|else
...
begin_comment
DECL|macro|QT_NO_SYSTEMSEMAPHORE
comment|// not needed at all in a single AddressSpace
end_comment
begin_define
DECL|macro|QT_NO_MULTIPROCESS
define|#
directive|define
name|QT_NO_MULTIPROCESS
end_define
begin_comment
DECL|macro|QT_NO_MULTIPROCESS
comment|// no system
...
name|string
parameter_list|)
value|QString(string).toUtf8().constData()
end_define
begin_endif
endif|#
directive|endif
...
block|{
name|namespace
name|SwapExceptionTester
block|{
comment|// insulate users from the "using std::swap" below
name|using
name|std
operator|::
name|swap
expr_stmt|;
comment|// import std::swap
name|template
operator|<
name|typename
name|T
...
argument_list|)
expr_stmt|;
comment|// declared, but not implemented (only to be used in unevaluated contexts (noexcept operator))
block|}
block|}
end_decl_stmt
begin_comment
...
name|Q_DECL_NOEXCEPT_EXPR
argument_list|(
argument|noexcept(QtPrivate::SwapExceptionTester::checkSwap(value1))
argument_list|)
block|{
name|using
name|std
...
end_decl_stmt
begin_comment
comment|// this adds const to non-const objects (like std::as_const)
end_comment
begin_expr_stmt
name|template
operator|<
...
end_expr_stmt
begin_comment
comment|// prevent rvalue arguments:
end_comment
begin_expr_stmt
name|template
operator|<
