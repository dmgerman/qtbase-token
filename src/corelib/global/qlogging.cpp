...
end_if
begin_comment
comment|// make sure the function has "Message" in the name so the function is removed
end_comment
begin_if
if|#
directive|if
...
operator|)
operator|||
name|QT_HAS_ATTRIBUTE
argument_list|(
name|optimize
argument_list|)
end_if
begin_comment
comment|// force skipping the frame pointer, to save the backtrace() function some work
end_comment
begin_macro
name|__attribute__
argument_list|(
...
comment|// This regexp tries to extract the library name (without the path) and the function name.
...
condition|)
block|{
comment|// innermost, unknown frames are usually the logging framework itself
continue|continue;
block|}
name|result
operator|.
...
argument_list|()
return|;
comment|// if the first frame is unknown, replace it with the context function
if|if
condition|(
name|function
operator|&&
...
end_endif
begin_comment
comment|// QLOGGING_HAVE_BACKTRACE&& !QT_BOOTSTRAPPED
end_comment
begin_if
if|#
directive|if
