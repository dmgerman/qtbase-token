...
end_ifdef
begin_comment
comment|// Do we have stderr for QDebug? - Either there is a console or we are running
end_comment
begin_comment
comment|// with redirected stderr.
end_comment
begin_if
if|#
directive|if
...
parameter_list|)
block|{
comment|// to determine logging destination and marking logging environment variable as deprecated
comment|// ### remove when deprecated
struct|struct
name|LogDestination
block|{
name|LogDestination
...
name|stderr
argument_list|,
literal|"Warning: Environment variable %s is deprecated, "
literal|"use %s instead.\n"
argument_list|,
name|deprecated
argument_list|,
name|replacement
...
name|QT_BOOTSTRAPPED
argument_list|)
comment|// We use isatty to catch the obvious case of someone running something interactively.
comment|// We also support environment variables for Qt Creator use, or more complicated cases
comment|// like subprocesses.
specifier|static
specifier|const
name|LogDestination
name|logdest
argument_list|(
literal|"QT_NO_JOURNALD_LOG"
argument_list|,
name|isatty
argument_list|(
name|fileno
...
end_endif
begin_comment
comment|// (Q_COMPILER_THREAD_LOCAL) || ((Q_CC_MSVC)&& !(Q_OS_WINCE))
end_comment
begin_function
