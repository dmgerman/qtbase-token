...
comment|/*  * QGlobalStatic internals:  *  * The pointer is initialized to 0.  * The guard is initialized to 0.  * The guard can assume the following values:  *   -2:      object initialized and already destroyed  *   -1:      object initialized and is still valid  *    0:      not initialized, the value of the pointer should be null  *   +1:      initializing, must wait until a state change happens  *            (not used in the current implementation)  */
name|namespace
name|QtGlobalStatic
block|{
enum|enum
...
parameter_list|)
define|\
value|Q_DECL_HIDDEN inline Type *innerFunction()                  \     {                                                           \         struct HolderBase {                                     \             ~HolderBase() Q_DECL_NOTHROW                        \             { guard.store(QtGlobalStatic::Destroyed); }         \         };                                                      \         static struct Holder : public HolderBase {              \             Type value;                                         \             Holder()                                            \                 Q_DECL_NOEXCEPT_EXPR(noexcept(Type ARGS))       \                 : value ARGS                                    \             { guard.store(QtGlobalStatic::Initialized); }       \         } holder;                                               \         return&holder.value;                                   \     }
end_define
begin_else
else|#
directive|else
