...
argument_list|)
argument_list|,
literal|"QT_POINTER_SIZE defined incorrectly"
argument_list|)
expr_stmt|;
end_expr_stmt
begin_comment
...
end_comment
begin_comment
comment|/*!     \fn QFlags QFlags::setFlag(Enum flag, bool on) const     \since 5.7      Sets the indicated \a flag if \a on is \c true or unsets it if     it if \a on is \c false. Returns a reference to this object. */
end_comment
begin_comment
...
end_comment
begin_comment
comment|/*!     \class QSysInfo     \inmodule QtCore     \brief The QSysInfo class provides information about the system.      \list     \li \l WordSize specifies the size of a pointer for the platform        on which the application is compiled.     \li \l ByteOrder specifies whether the platform is big-endian or        little-endian.     \li \l WindowsVersion specifies the version of the Windows operating        system on which the application is run.     \li \l MacintoshVersion specifies the version of the Macintosh        operating system on which the application is run.     \endlist      Some constants are defined only on certain platforms. You can use     the preprocessor symbols Q_OS_WIN and Q_OS_OSX to test that     the application is compiled under Windows or OS X.      \sa QLibraryInfo */
end_comment
begin_comment
...
end_comment
begin_comment
comment|/*!     \variable QSysInfo::WindowsVersion     \brief the version of the Windows operating system on which the            application is run. */
end_comment
begin_comment
comment|/*!     \fn QSysInfo::WindowsVersion QSysInfo::windowsVersion()     \since 4.4      Returns the version of the Windows operating system on which the     application is run, or WV_None if the operating system is not     Windows. */
end_comment
begin_comment
comment|/*!     \variable QSysInfo::MacintoshVersion     \brief the version of the Macintosh operating system on which            the application is run. */
end_comment
begin_comment
comment|/*!     \fn QSysInfo::MacVersion QSysInfo::macVersion()      Returns the version of Darwin (OS X or iOS) on which the     application is run, or MV_None if the operating system     is not a version of Darwin. */
end_comment
begin_comment
...
argument_list|(
argument|QWindowsSockInit
argument_list|,
argument|winsockInit
argument_list|)
end_macro
begin_endif
endif|#
...
specifier|inline
name|HMODULE
name|moduleHandleForFunction
parameter_list|(
name|LPCVOID
name|address
parameter_list|)
block|{
comment|// This is a widely used, decades-old technique for retrieving the handle
comment|// of a module and is effectively equivalent to GetModuleHandleEx
comment|// (which is unavailable on WinRT)
...
directive|endif
comment|// GetModuleHandle is not supported in WinRT and linking to it at load time
comment|// will not pass the Windows App Certification Kit... but it exists and is functional,
comment|// so use some unusual but widely used techniques to get a pointer to it
ifdef|#
directive|ifdef
name|Q_OS_WINRT
comment|// 1. Get HMODULE of kernel32.dll, using the address of some function exported by that DLL
name|HMODULE
name|kernelModule
init|=
name|moduleHandleForFunction
...
name|result
return|;
comment|// 2. Get pointer to GetModuleHandle so we can then load other arbitrary modules (DLLs)
typedef|typedef
name|HMODULE
function_decl|(
name|WINAPI
modifier|*
name|GetModuleHandleFunction
function_decl|)
parameter_list|(
name|LPCWSTR
parameter_list|)
...
name|result
return|;
comment|// NTSTATUS is not defined on WinRT
typedef|typedef
name|LONG
name|NTSTATUS
typedef|;
...
name|NTAPI
modifier|*
name|RtlGetVersionFunction
function_decl|)
parameter_list|(
name|LPOSVERSIONINFO
parameter_list|)
function_decl|;
comment|// RtlGetVersion is documented public API but we must load it dynamically
comment|// because linking to it at load time will not pass the Windows App Certification Kit
comment|// https://msdn.microsoft.com/en-us/library/windows/hardware/ff561910.aspx
name|RtlGetVersionFunction
name|pRtlGetVersion
init|=
cast|reinterpret_cast
...
comment|// only Windows 8 from that version on, so use the kernel API function.
name|pRtlGetVersion
argument_list|(
operator|(
name|LPOSVERSIONINFO
...
name|QUnixOSVersion
block|{
comment|// from /etc/os-release         older /etc/lsb-release         // redhat /etc/redhat-release         // debian /etc/debian_version
DECL|member|productType
name|QString
name|productType
decl_stmt|;
comment|// $ID                          $DISTRIB_ID                    // single line file containing:       // Debian
DECL|member|productVersion
name|QString
name|productVersion
decl_stmt|;
comment|// $VERSION_ID                  $DISTRIB_RELEASE               //<Vendor_ID release Version_ID>     // single line file<Release_ID/sid>
DECL|member|prettyName
name|QString
name|prettyName
decl_stmt|;
comment|// $PRETTY_NAME                 $DISTRIB_DESCRIPTION
block|}
struct|;
end_struct
begin_function
...
name|QByteArrayLiteral
argument_list|(
literal|"DISTRIB_RELEASE="
argument_list|)
argument_list|,
name|QByteArrayLiteral
argument_list|(
literal|"DISTRIB_DESCRIPTION="
argument_list|)
argument_list|)
decl_stmt|;
if|if
...
condition|)
block|{
comment|// some distributions have redundant information for the pretty name,
comment|// so try /etc/<lowercasename>-release
comment|// we're still avoiding QFile here
name|QByteArray
name|distrorelease
init|=
literal|"/etc/"
...
condition|)
block|{
comment|// file apparently contains interesting information
name|QByteArray
name|buffer
argument_list|(
name|sbuf
...
block|}
block|}
comment|// some distributions have a /etc/lsb-release file that does not provide the values
comment|// we are looking for, i.e. DISTRIB_ID, DISTRIB_RELEASE and DISTRIB_DESCRIPTION.
comment|// Assuming that neither DISTRIB_ID nor DISTRIB_RELEASE were found, or contained valid values,
comment|// returning false for readEtcLsbRelease will allow further /etc/<lowercasename>-release parsing.
return|return
name|ok
operator|&&
operator|!
...
parameter_list|)
block|{
comment|// /etc/redhat-release analysed should be a one line file
comment|// the format of its content is<Vendor_ID release Version>
comment|// i.e. "Red Hat Enterprise Linux Workstation release 6.5 (Santiago)"
name|QByteArray
name|line
init|=
name|getEtcFileFirstLine
...
parameter_list|)
block|{
comment|// /etc/debian_version analysed should be a one line file
comment|// the format of its content is<Release_ID/sid>
comment|// i.e. "jessie/sid"
name|QByteArray
name|line
init|=
name|getEtcFileFirstLine
...
name|Q_OS_WINCE
argument_list|)
comment|// We don't need to catch all the CPU architectures in this function;
comment|// only those where the host CPU might be different than the build target
comment|// (usually, 64-bit platforms).
name|SYSTEM_INFO
name|info
decl_stmt|;
...
condition|)
block|{
comment|// the use of QT_BUILD_INTERNAL here is simply to ensure all branches build
comment|// as we don't often build on some of the less common platforms
if|#
directive|if
name|defined
argument_list|(
...
name|QT_BUILD_INTERNAL
argument_list|)
comment|// harmonize "powerpc" and "ppc" to "power"
if|if
condition|(
name|strncmp
argument_list|(
...
name|QT_BUILD_INTERNAL
argument_list|)
comment|// harmonize all "i?86" to "i386"
if|if
condition|(
name|strlen
argument_list|(
...
end_function
begin_comment
comment|/*!     \since 5.4      Returns a prettier form of productType() and productVersion(), containing     other tokens like the operating system type, codenames and other     information. The result of this function is suitable for displaying to the     user, but not for long-term storage, as the string may change with updates     to Qt.      If productType() is "unknown", this function will instead use the     kernelType() and kernelVersion() functions.      \sa kernelType(), kernelVersion(), productType(), productVersion() */
end_comment
begin_function
...
end_ifndef
begin_comment
comment|/*!     \since 5.6      Returns this machine's host name, if one is configured. Note that hostnames     are not guaranteed to be globally unique, especially if they were     configured automatically.      This function does not guarantee the returned host name is a Fully     Qualified Domain Name (FQDN). For that, use QHostInfo to resolve the     returned name to an FQDN.      This function returns the same as QHostInfo::localHostName().      \sa QHostInfo::localDomainName  */
end_comment
begin_function
...
name|Q_OS_LINUX
argument_list|)
comment|// gethostname(3) on Linux just calls uname(2), so do it ourselves
comment|// and avoid a memcpy
name|struct
name|utsname
name|u
decl_stmt|;
...
directive|ifdef
name|Q_OS_WIN
comment|// Important: QtNetwork depends on machineHostName() initializing ws2_32.dll
name|winsockInit
argument_list|()
expr_stmt|;
endif|#
...
end_function
begin_comment
comment|/*!     \relates<QtGlobal>     \since 5.5      Returns the numerical value of the environment variable \a varName.     If \a ok is not null, sets \c{*ok} to \c true or \c false depending     on the success of the conversion.      Equivalent to     \code     qgetenv(varName).toInt()     \endcode     except that it's much faster, and can't throw exceptions.      \sa qgetenv(), qEnvironmentVariableIsSet() */
end_comment
begin_function
name|int
...
operator|>=
literal|1400
comment|// we provide a buffer that can hold any int value:
specifier|static
specifier|const
name|int
name|NumBinaryDigitsPerOctalDigit
init|=
literal|3
decl_stmt|;
specifier|static
...
index|]
decl_stmt|;
comment|// +1 for NUL +1 for optional '-'
name|size_t
name|dummy
decl_stmt|;
if|if
...
condition|)
block|{
comment|// this is the check in QByteArray::toInt(), keep it in sync
if|if
condition|(
name|ok
condition|)
