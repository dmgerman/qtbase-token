begin_unit
begin_comment
comment|/**************************************************************************** ** ** Copyright (C) 2013 Digia Plc and/or its subsidiary(-ies). ** Contact: http://www.qt-project.org/legal ** ** This file is part of the QtCore module of the Qt Toolkit. ** ** $QT_BEGIN_LICENSE:LGPL$ ** Commercial License Usage ** Licensees holding valid commercial Qt licenses may use this file in ** accordance with the commercial license agreement provided with the ** Software or, alternatively, in accordance with the terms contained in ** a written agreement between you and Digia.  For licensing terms and ** conditions see http://qt.digia.com/licensing.  For further information ** use the contact form at http://qt.digia.com/contact-us. ** ** GNU Lesser General Public License Usage ** Alternatively, this file may be used under the terms of the GNU Lesser ** General Public License version 2.1 as published by the Free Software ** Foundation and appearing in the file LICENSE.LGPL included in the ** packaging of this file.  Please review the following information to ** ensure the GNU Lesser General Public License version 2.1 requirements ** will be met: http://www.gnu.org/licenses/old-licenses/lgpl-2.1.html. ** ** In addition, as a special exception, Digia gives you certain additional ** rights.  These rights are described in the Digia Qt LGPL Exception ** version 1.1, included in the file LGPL_EXCEPTION.txt in this package. ** ** GNU General Public License Usage ** Alternatively, this file may be used under the terms of the GNU ** General Public License version 3.0 as published by the Free Software ** Foundation and appearing in the file LICENSE.GPL included in the ** packaging of this file.  Please review the following information to ** ensure the GNU General Public License version 3.0 requirements will be ** met: http://www.gnu.org/copyleft/gpl.html. ** ** ** $QT_END_LICENSE$ ** ****************************************************************************/
end_comment
begin_ifndef
ifndef|#
directive|ifndef
name|QATOMIC_UNIX_H
end_ifndef
begin_define
DECL|macro|QATOMIC_UNIX_H
define|#
directive|define
name|QATOMIC_UNIX_H
end_define
begin_include
include|#
directive|include
file|<QtCore/qgenericatomic.h>
end_include
begin_expr_stmt
name|QT_BEGIN_NAMESPACE
if|#
directive|if
literal|0
comment|// silence syncqt warnings
expr|QT_END_NAMESPACE
pragma|#
directive|pragma
name|qt_sync_skip_header_check
pragma|#
directive|pragma
name|qt_sync_stop_processing
endif|#
directive|endif
DECL|macro|Q_ATOMIC_INT_REFERENCE_COUNTING_IS_NOT_NATIVE
define|#
directive|define
name|Q_ATOMIC_INT_REFERENCE_COUNTING_IS_NOT_NATIVE
DECL|macro|Q_ATOMIC_INT_TEST_AND_SET_IS_NOT_NATIVE
define|#
directive|define
name|Q_ATOMIC_INT_TEST_AND_SET_IS_NOT_NATIVE
DECL|macro|Q_ATOMIC_INT_FETCH_AND_STORE_IS_NOT_NATIVE
define|#
directive|define
name|Q_ATOMIC_INT_FETCH_AND_STORE_IS_NOT_NATIVE
DECL|macro|Q_ATOMIC_INT_FETCH_AND_ADD_IS_NOT_NATIVE
define|#
directive|define
name|Q_ATOMIC_INT_FETCH_AND_ADD_IS_NOT_NATIVE
DECL|macro|Q_ATOMIC_INT32_IS_SUPPORTED
define|#
directive|define
name|Q_ATOMIC_INT32_IS_SUPPORTED
DECL|macro|Q_ATOMIC_INT32_REFERENCE_COUNTING_IS_NOT_NATIVE
define|#
directive|define
name|Q_ATOMIC_INT32_REFERENCE_COUNTING_IS_NOT_NATIVE
DECL|macro|Q_ATOMIC_INT32_TEST_AND_SET_IS_NOT_NATIVE
define|#
directive|define
name|Q_ATOMIC_INT32_TEST_AND_SET_IS_NOT_NATIVE
DECL|macro|Q_ATOMIC_INT32_FETCH_AND_STORE_IS_NOT_NATIVE
define|#
directive|define
name|Q_ATOMIC_INT32_FETCH_AND_STORE_IS_NOT_NATIVE
DECL|macro|Q_ATOMIC_INT32_FETCH_AND_ADD_IS_NOT_NATIVE
define|#
directive|define
name|Q_ATOMIC_INT32_FETCH_AND_ADD_IS_NOT_NATIVE
DECL|macro|Q_ATOMIC_INT64_IS_SUPPORTED
define|#
directive|define
name|Q_ATOMIC_INT64_IS_SUPPORTED
DECL|macro|Q_ATOMIC_INT64_REFERENCE_COUNTING_IS_NOT_NATIVE
define|#
directive|define
name|Q_ATOMIC_INT64_REFERENCE_COUNTING_IS_NOT_NATIVE
DECL|macro|Q_ATOMIC_INT64_TEST_AND_SET_IS_NOT_NATIVE
define|#
directive|define
name|Q_ATOMIC_INT64_TEST_AND_SET_IS_NOT_NATIVE
DECL|macro|Q_ATOMIC_INT64_FETCH_AND_STORE_IS_NOT_NATIVE
define|#
directive|define
name|Q_ATOMIC_INT64_FETCH_AND_STORE_IS_NOT_NATIVE
DECL|macro|Q_ATOMIC_INT64_FETCH_AND_ADD_IS_NOT_NATIVE
define|#
directive|define
name|Q_ATOMIC_INT64_FETCH_AND_ADD_IS_NOT_NATIVE
DECL|macro|Q_ATOMIC_POINTER_TEST_AND_SET_IS_NOT_NATIVE
define|#
directive|define
name|Q_ATOMIC_POINTER_TEST_AND_SET_IS_NOT_NATIVE
DECL|macro|Q_ATOMIC_POINTER_FETCH_AND_STORE_IS_NOT_NATIVE
define|#
directive|define
name|Q_ATOMIC_POINTER_FETCH_AND_STORE_IS_NOT_NATIVE
DECL|macro|Q_ATOMIC_POINTER_FETCH_AND_ADD_IS_NOT_NATIVE
define|#
directive|define
name|Q_ATOMIC_POINTER_FETCH_AND_ADD_IS_NOT_NATIVE
name|template
operator|<
operator|>
expr|struct
name|QAtomicIntegerTraits
operator|<
name|long
name|long
operator|>
block|{ enum
block|{
name|IsInteger
operator|=
literal|1
block|}
block|; }
expr_stmt|;
end_expr_stmt
begin_expr_stmt
name|template
operator|<
operator|>
expr|struct
name|QAtomicIntegerTraits
operator|<
name|unsigned
name|long
name|long
operator|>
block|{ enum
block|{
name|IsInteger
operator|=
literal|1
block|}
block|; }
expr_stmt|;
end_expr_stmt
begin_comment
comment|// No definition, needs specialization
end_comment
begin_expr_stmt
name|template
operator|<
name|typename
name|T
operator|>
expr|struct
name|QAtomicOps
expr_stmt|;
end_expr_stmt
begin_comment
comment|// 32-bit version
end_comment
begin_expr_stmt
name|template
operator|<
operator|>
expr|struct
name|QAtomicOps
operator|<
name|int
operator|>
operator|:
name|QGenericAtomicOps
operator|<
name|QAtomicOps
operator|<
name|int
operator|>
expr|>
block|{
typedef|typedef
name|int
name|Type
typedef|;
specifier|static
specifier|inline
name|Q_DECL_CONSTEXPR
name|bool
name|isTestAndSetNative
argument_list|()
name|Q_DECL_NOTHROW
block|{
return|return
name|false
return|;
block|}
specifier|static
specifier|inline
name|Q_DECL_CONSTEXPR
name|bool
name|isTestAndSetWaitFree
argument_list|()
name|Q_DECL_NOTHROW
block|{
return|return
name|false
return|;
block|}
name|Q_CORE_EXPORT
specifier|static
name|bool
name|testAndSetRelaxed
argument_list|(
argument|int&_q_value
argument_list|,
argument|int expectedValue
argument_list|,
argument|int newValue
argument_list|)
name|Q_DECL_NOTHROW
expr_stmt|;
end_expr_stmt
begin_comment
unit|};
comment|// 64-bit version
end_comment
begin_expr_stmt
name|template
operator|<
operator|>
expr|struct
name|QAtomicOps
operator|<
name|long
name|long
operator|>
operator|:
name|QGenericAtomicOps
operator|<
name|QAtomicOps
operator|<
name|long
name|long
operator|>
expr|>
block|{
typedef|typedef
name|long
name|long
name|Type
typedef|;
specifier|static
specifier|inline
name|Q_DECL_CONSTEXPR
name|bool
name|isTestAndSetNative
argument_list|()
name|Q_DECL_NOTHROW
block|{
return|return
name|false
return|;
block|}
specifier|static
specifier|inline
name|Q_DECL_CONSTEXPR
name|bool
name|isTestAndSetWaitFree
argument_list|()
name|Q_DECL_NOTHROW
block|{
return|return
name|false
return|;
block|}
name|Q_CORE_EXPORT
specifier|static
name|bool
name|testAndSetRelaxed
argument_list|(
argument|Type&_q_value
argument_list|,
argument|Type expectedValue
argument_list|,
argument|Type newValue
argument_list|)
name|Q_DECL_NOTHROW
expr_stmt|;
end_expr_stmt
begin_comment
unit|};
comment|// pointer version
end_comment
begin_expr_stmt
name|template
operator|<
operator|>
expr|struct
name|QAtomicOps
operator|<
name|void
operator|*
operator|>
operator|:
name|QGenericAtomicOps
operator|<
name|QAtomicOps
operator|<
name|void
operator|*
operator|>
expr|>
block|{
typedef|typedef
name|void
modifier|*
name|Type
typedef|;
specifier|static
specifier|inline
name|Q_DECL_CONSTEXPR
name|bool
name|isTestAndSetNative
argument_list|()
name|Q_DECL_NOTHROW
block|{
return|return
name|false
return|;
block|}
specifier|static
specifier|inline
name|Q_DECL_CONSTEXPR
name|bool
name|isTestAndSetWaitFree
argument_list|()
name|Q_DECL_NOTHROW
block|{
return|return
name|false
return|;
block|}
name|Q_CORE_EXPORT
specifier|static
name|bool
name|testAndSetRelaxed
argument_list|(
argument|void *&_q_value
argument_list|,
argument|void *expectedValue
argument_list|,
argument|void *newValue
argument_list|)
name|Q_DECL_NOTHROW
expr_stmt|;
end_expr_stmt
begin_expr_stmt
unit|};
name|template
operator|<
name|typename
name|T
operator|>
expr|struct
DECL|struct|T
name|QAtomicOps
operator|<
name|T
operator|*
operator|>
operator|:
name|QGenericAtomicOps
operator|<
name|QAtomicOps
operator|<
name|T
operator|*
operator|>
expr|>
block|{
DECL|typedef|Type
typedef|typedef
name|T
modifier|*
name|Type
typedef|;
comment|// helper to strip cv qualifiers
DECL|function|nocv
specifier|static
specifier|inline
name|void
operator|*
name|nocv
argument_list|(
argument|const T *p
argument_list|)
block|{
return|return
name|const_cast
operator|<
name|void
operator|*
operator|>
operator|(
name|static_cast
operator|<
specifier|const
specifier|volatile
name|void
operator|*
operator|>
operator|(
name|p
operator|)
operator|)
return|;
block|}
specifier|static
specifier|inline
name|Q_DECL_CONSTEXPR
name|bool
name|isTestAndSetNative
argument_list|()
name|Q_DECL_NOTHROW
block|{
return|return
name|false
return|;
block|}
specifier|static
specifier|inline
name|Q_DECL_CONSTEXPR
name|bool
name|isTestAndSetWaitFree
argument_list|()
name|Q_DECL_NOTHROW
block|{
return|return
name|false
return|;
block|}
specifier|static
specifier|inline
name|bool
name|testAndSetRelaxed
argument_list|(
argument|T *&_q_value
argument_list|,
argument|T *expectedValue
argument_list|,
argument|T *newValue
argument_list|)
name|Q_DECL_NOTHROW
block|{
comment|// forward to the void* specialization
name|void
operator|*
name|voidp
operator|=
name|nocv
argument_list|(
name|_q_value
argument_list|)
block|;
name|bool
name|returnValue
operator|=
name|QAtomicOps
operator|<
name|void
operator|*
operator|>
operator|::
name|testAndSetRelaxed
argument_list|(
name|voidp
argument_list|,
name|nocv
argument_list|(
name|expectedValue
argument_list|)
argument_list|,
name|nocv
argument_list|(
name|newValue
argument_list|)
argument_list|)
block|;
name|_q_value
operator|=
name|reinterpret_cast
operator|<
name|T
operator|*
operator|>
operator|(
name|voidp
operator|)
block|;
return|return
name|returnValue
return|;
block|}
block|}
end_expr_stmt
begin_empty_stmt
empty_stmt|;
end_empty_stmt
begin_comment
comment|// 32- and 64-bit unsigned versions
end_comment
begin_expr_stmt
name|template
operator|<
operator|>
expr|struct
name|QAtomicOps
operator|<
name|unsigned
operator|>
operator|:
name|QAtomicOps
operator|<
name|int
operator|>
block|{
typedef|typedef
name|unsigned
name|Type
typedef|;
name|Q_CORE_EXPORT
specifier|static
name|bool
name|testAndSetRelaxed
argument_list|(
argument|Type&_q_value
argument_list|,
argument|Type expectedValue
argument_list|,
argument|Type newValue
argument_list|)
name|Q_DECL_NOTHROW
block|{
return|return
name|QAtomicOps
operator|<
name|int
operator|>
operator|::
name|testAndSetRelaxed
argument_list|(
name|reinterpret_cast
operator|<
name|int
operator|&
operator|>
operator|(
name|_q_value
operator|)
argument_list|,
name|int
argument_list|(
name|expectedValue
argument_list|)
argument_list|,
name|int
argument_list|(
name|newValue
argument_list|)
argument_list|)
return|;
block|}
block|}
end_expr_stmt
begin_empty_stmt
empty_stmt|;
end_empty_stmt
begin_expr_stmt
name|template
operator|<
operator|>
expr|struct
name|QAtomicOps
operator|<
name|unsigned
name|long
name|long
operator|>
operator|:
name|QAtomicOps
operator|<
name|long
name|long
operator|>
block|{
typedef|typedef
name|unsigned
name|long
name|longType
typedef|;
name|Q_CORE_EXPORT
specifier|static
name|bool
name|testAndSetRelaxed
argument_list|(
argument|Type&_q_value
argument_list|,
argument|Type expectedValue
argument_list|,
argument|Type newValue
argument_list|)
name|Q_DECL_NOTHROW
block|{
return|return
name|QAtomicOps
operator|<
name|long
name|long
operator|>
operator|::
name|testAndSetRelaxed
argument_list|(
argument|reinterpret_cast<long long&>(_q_value)
argument_list|,
argument|int(expectedValue)
argument_list|,
argument|int(newValue)
argument_list|)
return|;
block|}
block|}
end_expr_stmt
begin_empty_stmt
empty_stmt|;
end_empty_stmt
begin_macro
name|QT_END_NAMESPACE
end_macro
begin_endif
endif|#
directive|endif
end_endif
begin_comment
comment|// QATOMIC_UNIX_H
end_comment
end_unit
