...
control|)
block|{
comment|// We don't need to keep track of ItemIsSelectable and ItemIsEnabled here. That is
comment|// required in indexesFromRange() because that method is called from public API
comment|// which requires the limitation.
name|result
operator|.
name|push_back
argument_list|(
...
name|SLOT
argument_list|(
argument|_q_rowsAboutToBeRemoved(QModelIndex,int,int)
argument_list|)
block|}
block|,
block|{
...
name|SLOT
argument_list|(
argument|_q_columnsAboutToBeRemoved(QModelIndex,int,int)
argument_list|)
block|}
block|,
block|{
...
name|SLOT
argument_list|(
argument|_q_rowsAboutToBeInserted(QModelIndex,int,int)
argument_list|)
block|}
block|,
block|{
...
name|SLOT
argument_list|(
argument|_q_columnsAboutToBeInserted(QModelIndex,int,int)
argument_list|)
block|}
block|,
block|{
...
name|SLOT
argument_list|(
argument|_q_layoutAboutToBeChanged()
argument_list|)
block|}
block|,
block|{
...
name|SLOT
argument_list|(
argument|_q_layoutAboutToBeChanged(QList<QPersistentModelIndex>,QAbstractItemModel::LayoutChangeHint)
argument_list|)
block|}
block|,
block|{
...
name|SLOT
argument_list|(
argument|_q_layoutChanged(QList<QPersistentModelIndex>,QAbstractItemModel::LayoutChangeHint)
argument_list|)
block|}
block|,
block|{
...
condition|)
block|{
comment|// Special case when we know we're sorting vertically. We can assume that all indexes for columns
comment|// are displaced the same way, and therefore we only need to track an index from one column per
comment|// row with a QPersistentModelIndex together with the length of items to the right of it
comment|// which are displaced the same way.
comment|// An algorithm which contains the same assumption is used to process layoutChanged.
name|savedPersistentRowLengths
operator|=
name|qSelectionPersistentRowLengths
argument_list|(
...
end_comment
begin_comment
comment|/*!     \fn void QItemSelectionModel::modelChanged(QAbstractItemModel *model)     \since 5.5      This signal is emitted when the \a model is successfully set with setModel().      \sa model(), setModel() */
end_comment
begin_comment
...
name|qWarning
argument_list|(
literal|"QItemSelectionModel: Selecting when no model has been set will result in a no-op."
argument_list|)
expr_stmt|;
return|return;
block|}
...
name|qWarning
argument_list|(
literal|"QItemSelectionModel: Setting the current index when no model has been set will result in a no-op."
argument_list|)
expr_stmt|;
return|return;
block|}
...
end_function
begin_comment
comment|/*!     \since 5.5      \property QItemSelectionModel::hasSelection     \internal */
end_comment
begin_comment
comment|/*!     \since 5.5      \property QItemSelectionModel::currentIndex     \internal */
end_comment
begin_comment
...
end_comment
begin_comment
comment|/*!     \since 5.5      Returns the item model operated on by the selection model. */
end_comment
begin_function
DECL|function|model
name|QAbstractItemModel
