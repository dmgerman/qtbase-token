...
end_function
begin_comment
comment|/*!     Returns \c{true} if a model can accept a drop of the \a data. This     default implementation always returns \c{true}.      Reimplement this function in your custom model, if you want to     test whether the \a data can be dropped at \a row, \a column,     \a parent with \a action. If you don't need that test, it is not     necessary to reimplement this function.      \sa dropMimeData(), {Using drag and drop with item views}  */
end_comment
begin_function
