...
end_comment
begin_comment
comment|/*!     \fn void QAbstractItemModel::dataChanged(const QModelIndex&topLeft, const QModelIndex&bottomRight, const QSet<int>&roles = QSet<int>())      This signal is emitted whenever the data in an existing item changes.      If the items are of the same parent, the affected ones are those between     \a topLeft and \a bottomRight inclusive. If the items do not have the same     parent, the behavior is undefined.      When reimplementing the setData() function, this signal must be emitted     explicitly.      The optional roles argument can be used to specify which data roles have actually     been modified. An empty set in the roles argument means that all roles should be     considered modified.      \sa headerDataChanged(), setData(), layoutChanged() */
end_comment
begin_comment
