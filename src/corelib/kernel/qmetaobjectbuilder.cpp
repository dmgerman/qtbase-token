...
end_function
begin_comment
comment|// copied from qmetaobject_p.h
end_comment
begin_comment
comment|// do not touch without touching the moc as well
end_comment
begin_enum
...
end_function
begin_comment
comment|// end of copied lines from qmetaobject.cpp
end_comment
begin_class
...
end_define
begin_comment
comment|// Build a string into a QMetaObject representation.  Returns the
end_comment
begin_comment
comment|// position in the string table where the string was placed.
end_comment
begin_function
...
end_function
begin_comment
comment|// Build the parameter array string for a method.
end_comment
begin_function
DECL|function|buildParameterNames
specifier|static
name|QByteArray
name|buildParameterNames
parameter_list|(
specifier|const
name|QByteArray
modifier|&
...
parameter_list|)
block|{
comment|// If the parameter name list is specified, then concatenate them.
if|if
condition|(
operator|!
name|parameterNames
...
return|;
block|}
comment|// Count commas in the signature, excluding those inside template arguments.
name|int
name|index
init|=
name|signature
...
end_comment
begin_comment
comment|// relocatable is set, but the metaobject contains extradata.
end_comment
begin_function
...
argument_list|)
expr_stmt|;
comment|// Add a common empty string, which is used to indicate "void"
comment|// method returns, empty tag strings, etc.
name|int
name|empty
init|=
name|buildString
...
expr_stmt|;
block|}
comment|// One more empty string to act as a terminator.
name|buildString
argument_list|(
name|buf
argument_list|,
...
literal|0
expr_stmt|;
comment|// Create the extradata block if we need one.
if|if
condition|(
name|d
operator|->
...
end_function
begin_comment
comment|/*!     Sets the return type for this method to \a value.  If \a value     is empty, then the method's return type is \c{void}.  The \a value     will be normalized before it is added to the method.      \sa returnType(), signature() */
end_comment
begin_function
