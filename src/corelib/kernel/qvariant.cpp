...
return|;
default|default:
comment|// includes enum conversion as well as invalid types
return|return
name|qreal
argument_list|(
name|qConvertToNumber
...
return|;
block|}
comment|/*!  \internal  Returns the internal data pointer from \a d.  */
...
directive|ifndef
name|QT_NO_QOBJECT
comment|/*!   \internal   returns a QMetaEnum for a given meta tape type id if possible */
...
end_comment
begin_comment
comment|/*!     \since 5.0     \fn QVariant::QVariant(const QModelIndex&val)      Constructs a new variant with a QModelIndex value, \a val. */
end_comment
begin_comment
comment|/*!     \since 5.5     \fn QVariant::QVariant(const QPersistentModelIndex&val)      Constructs a new variant with a QPersistentModelIndex value, \a val. */
end_comment
begin_comment
...
end_function
begin_comment
comment|/*!     \since 5.0      Returns the variant as a QModelIndex if the variant has userType() \l     QModelIndex; otherwise returns a default constructed QModelIndex.      \sa canConvert(), convert(), toPersistentModelIndex() */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!     \since 5.5      Returns the variant as a QPersistentModelIndex if the variant has userType() \l     QPersistentModelIndex; otherwise returns a default constructed QPersistentModelIndex.      \sa canConvert(), convert(), toModelIndex() */
end_comment
begin_function
...
specifier|const
name|size_t
name|qCanConvertMatrixMaximumTargetType
init|=
literal|8
operator|*
sizeof|sizeof
...
argument_list|)
expr_stmt|;
comment|// C++ integral ranks: (4.13 Integer conversion rank [conv.rank])
comment|//   bool< signed char< short< int< long< long long
comment|//   unsigneds have the same rank as their signed counterparts
comment|// C++ integral promotion rules (4.5 Integral Promotions [conv.prom])
comment|// - any type with rank less than int can be converted to int or unsigned int
comment|// 5 Expressions [expr] paragraph 9:
comment|// - if either operand is double, the other shall be converted to double
comment|// -     "       "        float,   "          "         "         float
comment|// - if both operands have the same type, no further conversion is needed.
comment|// - if both are signed or if both are unsigned, convert to the one with highest rank
comment|// - if the unsigned has higher or same rank, convert the signed to the unsigned one
comment|// - if the signed can represent all values of the unsigned, convert to the signed
comment|// - otherwise, convert to the unsigned corresponding to the rank of the signed
comment|// floating point: we deviate from the C++ standard by always using qreal
if|if
condition|(
name|qIsFloatingPoint
argument_list|(
...
name|QReal
return|;
comment|// integral rules:
comment|// for all platforms we support, int can always hold the values of lower-ranked types
name|t1
operator|=
name|normalizeLowerRanks
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// normalize long / ulong: in all platforms we run, they're either the same as int or as long long
name|t1
operator|=
name|normalizeLong
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// implement the other rules
comment|// the four possibilities are Int, UInt, LongLong and ULongLong
comment|// if any of the two is ULongLong, then it wins (highest rank, unsigned)
comment|// otherwise, if one of the two is LongLong, then the other is either LongLong too or lower-ranked
comment|// otherwise, if one of the two is UInt, then the other is either UInt too or Int
if|if
condition|(
name|t1
operator|==
...
parameter_list|)
block|{
comment|// use toLongLong to retrieve the data, it gets us all the bits
name|bool
name|ok
decl_stmt|;
name|qlonglong
...
specifier|const
block|{
comment|// try numerics first, with C++ type promotion rules (no conversion)
if|if
condition|(
name|qIsNumericType
argument_list|(
...
block|{
comment|// try the opposite conversion, it might work
name|qSwap
argument_list|(
name|v1
argument_list|,
...
argument_list|)
return|;
comment|// check for equality next, as more types implement operator== than operator<
if|if
condition|(
name|cmp
argument_list|(
...
condition|)
block|{
comment|// if both types differ, try to convert
if|if
condition|(
name|v2
operator|.
...
condition|)
block|{
comment|// if conversion fails, default to toString
name|int
name|r
init|=
name|v1
...
condition|)
block|{
comment|// cmp(v) returned false, so we should try to agree with it.
return|return
operator|(
name|v1
operator|.
...
return|;
block|}
comment|// did we end up with two numerics? If so, restart
if|if
condition|(
name|qIsNumericType
argument_list|(
...
argument_list|()
block|; }
comment|/*! \fn QSequentialIterable::const_iterator QSequentialIterable::begin() const      Returns a QSequentialIterable::const_iterator for the beginning of the container. This     can be used in stl-style iteration.      \sa end() */
DECL|function|begin
name|QSequentialIterable
operator|::
name|const_iterator
...
end_expr_stmt
begin_comment
comment|/*!     Returns a QSequentialIterable::const_iterator for the end of the container. This     can be used in stl-style iteration.      \sa begin() */
end_comment
begin_function
DECL|function|end
name|QSequentialIterable
...
end_function
begin_comment
comment|/*!     Returns the element at position \a idx in the container. */
end_comment
begin_function
DECL|function|at
name|QVariant
...
end_function
begin_comment
comment|/*!     Returns whether it is possible to iterate over the container in reverse. This     corresponds to the std::bidirectional_iterator_tag iterator trait of the     const_iterator of the container. */
end_comment
begin_function
...
end_constructor
begin_comment
comment|/*!     Assigns \a other to this. */
end_comment
begin_function
name|QSequentialIterable
operator|::
...
end_function
begin_comment
comment|/*!     Returns the current item, converted to a QVariant. */
end_comment
begin_function
DECL|function|operator *
specifier|const
...
end_function
begin_comment
comment|/*!     The prefix ++ operator (\c{++it}) advances the iterator to the     next item in the container and returns an iterator to the new current     item.      Calling this function on QSequentialIterable::end() leads to undefined results.      \sa operator--() */
end_comment
begin_function
DECL|function|operator ++
name|QSequentialIterable
...
end_function
begin_comment
comment|/*!     \overload      The postfix ++ operator (\c{it++}) advances the iterator to the     next item in the container and returns an iterator to the previously     current item. */
end_comment
begin_function
DECL|function|operator ++
name|QSequentialIterable
...
end_function
begin_comment
comment|/*!     The prefix -- operator (\c{--it}) makes the preceding item     current and returns an iterator to the new current item.      Calling this function on QSequentialIterable::begin() leads to undefined results.      If the container in the QVariant does not support bi-directional iteration, calling this function     leads to undefined results.      \sa operator++(), canReverseIterate() */
end_comment
begin_function
DECL|function|operator --
name|QSequentialIterable
...
end_function
begin_comment
comment|/*!     \overload      The postfix -- operator (\c{it--}) makes the preceding item     current and returns an iterator to the previously current item.      If the container in the QVariant does not support bi-directional iteration, calling this function     leads to undefined results.      \sa canReverseIterate() */
end_comment
begin_function
DECL|function|operator --
name|QSequentialIterable
...
end_function
begin_comment
comment|/*!     Advances the iterator by \a j items.      \sa operator-=(), operator+() */
end_comment
begin_function
DECL|function|operator +=
name|QSequentialIterable
...
end_function
begin_comment
comment|/*!     Makes the iterator go back by \a j items.      If the container in the QVariant does not support bi-directional iteration, calling this function     leads to undefined results.      \sa operator+=(), operator-(), canReverseIterate() */
end_comment
begin_function
DECL|function|operator -=
name|QSequentialIterable
...
end_function
begin_comment
comment|/*!     Returns an iterator to the item at \a j positions forward from     this iterator.      \sa operator-(), operator+=() */
end_comment
begin_function
DECL|function|operator +
name|QSequentialIterable
...
end_function
begin_comment
comment|/*!     Returns an iterator to the item at \a j positions backward from     this iterator.      If the container in the QVariant does not support bi-directional iteration, calling this function     leads to undefined results.      \sa operator+(), operator-=(), canReverseIterate() */
end_comment
begin_function
DECL|function|operator -
name|QSequentialIterable
...
argument_list|)
block|; }
comment|/*!     Returns a QAssociativeIterable::const_iterator for the beginning of the container. This     can be used in stl-style iteration.      \sa end() */
DECL|function|begin
name|QAssociativeIterable
operator|::
name|const_iterator
...
end_expr_stmt
begin_comment
comment|/*!     Returns a QAssociativeIterable::const_iterator for the end of the container. This     can be used in stl-style iteration.      \sa begin() */
end_comment
begin_function
DECL|function|end
name|QAssociativeIterable
...
end_function
begin_comment
comment|/*!     Returns the current value, converted to a QVariant. */
end_comment
begin_function
DECL|function|operator *
specifier|const
...
end_function
begin_comment
comment|/*!     Returns the current key, converted to a QVariant. */
end_comment
begin_function
DECL|function|key
specifier|const
...
end_function
begin_comment
comment|/*!     The prefix ++ operator (\c{++it}) advances the iterator to the     next item in the container and returns an iterator to the new current     item.      Calling this function on QAssociativeIterable::end() leads to undefined results.      \sa operator--() */
end_comment
begin_function
DECL|function|operator ++
name|QAssociativeIterable
...
end_function
begin_comment
comment|/*!     The prefix -- operator (\c{--it}) makes the preceding item     current and returns an iterator to the new current item.      Calling this function on QAssociativeIterable::begin() leads to undefined results.      \sa operator++() */
end_comment
begin_function
DECL|function|operator --
name|QAssociativeIterable
...
end_function
begin_comment
comment|/*!     \overload      The postfix -- operator (\c{it--}) makes the preceding item     current and returns an iterator to the previously current item. */
end_comment
begin_function
DECL|function|operator --
name|QAssociativeIterable
...
end_function
begin_comment
comment|/*!     Makes the iterator go back by \a j items.      \sa operator+=(), operator-() */
end_comment
begin_function
DECL|function|operator -=
name|QAssociativeIterable
...
end_function
begin_comment
comment|/*!     Returns an iterator to the item at \a j positions backward from     this iterator.      \sa operator+(), operator-=() */
end_comment
begin_function
DECL|function|operator -
name|QAssociativeIterable
