...
end_function
begin_comment
comment|/*   \internal   \since 4.4    We cannot use v_cast() for QMetaType's numeric types because they're smaller than QVariant::Private::Data,   which in turns makes v_cast() believe the value is stored in d->data.c. But   it's not, since we're a QMetaType type.  */
end_comment
begin_function
template|template
parameter_list|<
...
operator|)
decl_stmt|;
comment|/* The reason we cannot place this test in a case branch above for the types      * QMetaType::VoidStar, QMetaType::QObjectStar and so forth, is that it wouldn't include      * user defined pointer types. */
specifier|const
name|char
modifier|*
specifier|const
...
name|BitArray
case|:
comment|//dbg.nospace()<< v.toBitArray();
break|break;
default|default:
break|break;
block|}
...
specifier|const
block|{
comment|//we can treat floats as double
comment|//the reason for not doing it the "proper" way is that QMetaType::Float's value is 135,
comment|//which can't be handled by qCanConvertMatrix
comment|//In addition QVariant::Type doesn't have a Float value, so we're using QMetaType::Float
specifier|const
name|uint
name|currentType
init|=
...
name|qWarning
argument_list|(
literal|"This compiler doesn't support streaming QVariant to QDebug"
argument_list|)
expr_stmt|;
return|return
name|dbg
...
name|qWarning
argument_list|(
literal|"This compiler doesn't support streaming QVariant::Type to QDebug"
argument_list|)
expr_stmt|;
return|return
name|dbg
...
end_comment
begin_comment
comment|/*! \fn bool QVariant::canConvert() const      Returns true if the variant can be converted to the template type \c{T},     otherwise false.      Example:      \snippet doc/src/snippets/code/src_corelib_kernel_qvariant.cpp 6      \sa convert() */
end_comment
begin_comment
comment|/*! \fn static QVariant QVariant::fromValue(const T&value)      Returns a QVariant containing a copy of \a value. Behaves     exactly like setValue() otherwise.      Example:      \snippet doc/src/snippets/code/src_corelib_kernel_qvariant.cpp 7      \note If you are working with custom types, you should use     the Q_DECLARE_METATYPE() macro to register your custom type.      \sa setValue(), value() */
end_comment
begin_comment
