...
comment|/*!     \fn bool QBasicTimer::isActive() const      Returns \c true if the timer is running and has not been stopped; otherwise     returns \c false.      \sa start(), stop() */
...
name|qWarning
argument_list|(
literal|"QBasicTimer::start: Timers cannot be started from another thread"
argument_list|)
expr_stmt|;
return|return;
block|}
...
name|qWarning
argument_list|(
literal|"QBasicTimer::start: Stopping previous timer failed. Possibly trying to stop from a different thread"
argument_list|)
expr_stmt|;
block|}
name|id
...
name|qWarning
argument_list|(
literal|"QBasicTimer::start: Timers cannot have negative timeouts"
argument_list|)
expr_stmt|;
return|return;
block|}
...
name|qWarning
argument_list|(
literal|"QBasicTimer::stop: Failed. Possibly trying to stop from a different thread"
argument_list|)
expr_stmt|;
return|return;
block|}
