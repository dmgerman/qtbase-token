...
end_comment
begin_comment
comment|/*!     Sends \a message through the event filters that were set by     installNativeEventFilter().  This function returns true as soon as an     event filter returns true, and false otherwise to indicate that     the processing of the event should continue.      Subclasses of QAbstractEventDispatcher \e must call this function     for \e all messages received from the system to ensure     compatibility with any extensions that may be used in the     application.      Note that the type of \a message is platform dependent. See     QAbstractNativeEventFilter for details.      \sa installNativeEventFilter()     \since 5.0      \internal     This method only exists for the Windows event dispatcher to call the winEventFilter virtual.     Every other platform can just use QAbstractNativeEventFilter::filterNativeEvent directly. */
end_comment
begin_function
