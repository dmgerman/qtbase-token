...
condition|)
block|{
comment|// args is never empty, but the navigator can change behaviour some day
name|QFileInfo
name|fileInfo
argument_list|(
name|arguments
...
condition|)
block|{
comment|// Handle the zygotized case:
name|QCoreApplicationPrivate
operator|::
name|setApplicationFilePath
argument_list|(
...
block|}
block|}
comment|// Handle the non-zygotized case:
specifier|const
name|size_t
name|maximum_path
init|=
...
name|qWarning
argument_list|(
literal|"QCoreApplication::applicationFilePath: _cmdname() failed"
argument_list|)
expr_stmt|;
comment|// _cmdname() won't fail, but just in case, fallback to the old method
name|QDir
name|dir
argument_list|(
name|QStringLiteral
