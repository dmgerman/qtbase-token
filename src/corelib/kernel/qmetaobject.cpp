...
end_function
begin_comment
comment|/*!     Returns the integer value of the given enumeration \a key, or -1     if \a key is not defined.      For flag types, use keysToValue().      \sa valueToKey(), isFlag(), keysToValue() */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!     Returns the value derived from combining together the values of     the \a keys using the OR operator, or -1 if \a keys is not     defined. Note that the strings in \a keys must be '|'-separated.      \sa isFlag(), valueToKey(), valueToKeys() */
end_comment
begin_function
