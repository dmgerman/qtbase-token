...
end_ifdef
begin_comment
comment|/*!     \overload QTranslator()     \obsolete  */
end_comment
begin_constructor
DECL|function|QTranslator
name|QTranslator
...
end_function
begin_comment
comment|/*!     Returns the translation for the key (\a context, \a sourceText,     \a disambiguation). If none is found, also tries (\a context, \a     sourceText, ""). If that still fails, returns an empty string.      If you need to programatically insert translations in to a     QTranslator, this function can be reimplemented.      \sa load() */
end_comment
begin_function
...
argument_list|)
expr_stmt|;
comment|// this step is necessary because the 3-parameter translate() overload is virtual
if|if
condition|(
name|n
operator|==
