...
end_function
begin_comment
comment|/*!     \since 4.8      Disconnects \a signal in object \a sender from \a method in object     \a receiver. Returns true if the connection is successfully broken;     otherwise returns false.      This function provides the same posibilities like     disconnect(const QObject *sender, const char *signal, const QObject *receiver, const char *method)     but uses QMetaMethod to represent the signal and the method to be disconnected.      Additionally this function returnsfalse and no signals and slots disconnected     if:     \list 1          \i \a signal is not a member of sender class or one of its parent classes.          \i \a method is not a member of receiver class or one of its parent classes.          \i \a signal instance represents not a signal.      \endlist      QMetaMethod() may be used as wildcard in the meaning "any signal" or "any slot in receiving object".     In the same way 0 can be used for \a receiver in the meaning "any receiving object". In this case     method shoud also be QMetaMethod(). \a sender parameter should be never 0.      \sa disconnect(const QObject *sender, const char *signal, const QObject *receiver, const char *method)  */
end_comment
begin_function
DECL|function|disconnect
name|bool
