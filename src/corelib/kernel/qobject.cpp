...
name|methodCount
expr_stmt|;
comment|/*Before Qt 4.6 (revision 4), the signalCount information was not generated by moc.            so for compatibility we consider all the method as slot for old moc output*/
name|m
operator|=
name|m
operator|->
...
argument_list|()
decl_stmt|;
comment|/* check that the hash is empty - otherwise we might detach        the shared_null hash, which will alloc, which is not nice */
if|if
condition|(
operator|!
name|hash
...
name|ptr
condition|)
comment|//check again, under the lock
return|return;
name|GuardHash
operator|::
name|iterator
...
literal|false
decl_stmt|;
comment|//if the QObject has more pointer attached to it.
for|for
control|(
init|;
name|it
...
argument_list|)
block|{
comment|// do nothing in case of OOM - code below is safe
block|}
comment|/* check that the hash is empty - otherwise we might detach        the shared_null hash, which will alloc, which is not nice */
if|if
condition|(
...
end_comment
begin_comment
comment|/*!     \relates QObject      Returns a pointer to the object named \a name that inherits \a     type and with a given \a parent.      Returns 0 if there is no such child.      \snippet doc/src/snippets/code/src_corelib_kernel_qobject.cpp 0 */
end_comment
begin_function
...
end_ifdef
begin_comment
comment|/*!     \overload QObject()     \obsolete      Creates a new QObject with the given \a parent and object \a name.  */
end_comment
begin_constructor
DECL|function|QObject
name|QObject
...
condition|)
block|{
comment|// set all QPointers for this object to zero - note that
comment|// ~QWidget() does this for us, so we don't have to do it twice
name|QObjectPrivate
operator|::
name|clearGuards
argument_list|(
...
name|qWarning
argument_list|(
literal|"QObject: Do not delete object, '%s', during its event handler!"
argument_list|,
name|objectName
argument_list|()
operator|.
...
end_ifdef
begin_comment
comment|/*! \internal     QObject::child is compat but needs to call itself recursively,     that's why we need this helper. */
end_comment
begin_function
...
condition|)
block|{
comment|// set inThreadChangeEvent to true to tell the dispatcher not to release out timer ids
comment|// back to the pool (since the timer ids are moving to a new thread).
name|d
operator|->
name|inThreadChangeEvent
operator|=
...
directive|ifdef
name|QT_JAMBI_BUILD
comment|// the current event thread also shouldn't restore the delete watch
name|inEventHandler
operator|=
literal|false
expr_stmt|;
...
end_comment
begin_comment
comment|/*!     \internal     \fn T qFindChild(const QObject *obj, const QString&name = QString(), T dummy = 0)     \relates QObject     \overload qFindChildren()      This function is equivalent to     \a{obj}->\l{QObject::findChild()}{findChild}<T>(\a name).      \note This function was provided as a workaround for MSVC 6     which did not support member template functions. It is advised     to use the other form in new code.      \sa QObject::findChild() */
end_comment
begin_comment
comment|/*!     \internal     \fn QList<T> qFindChildren(const QObject *obj, const QString&name = QString(), T dummy = 0)     \relates QObject     \overload qFindChildren()      This function is equivalent to     \a{obj}->\l{QObject::findChildren()}{findChildren}<T>(\a name).      \note This function was provided as a workaround for MSVC 6     which did not support member template functions. It is advised     to use the other form in new code.      \sa QObject::findChildren() */
end_comment
begin_comment
comment|/*!     \internal */
end_comment
...
end_function
begin_comment
comment|/*!     \internal      This helper function calculates signal and method index for the given     member in the specified class.      \li If member.mobj is 0 then both signalIndex and methodIndex are set to -1.      \li If specified member is not a member of obj instance class (or one of     its parent classes) then both signalIndex and methodIndex are set to -1.      This function is used by QObject::connect and QObject::disconnect which     are working with QMetaMethod.      \param[out] signalIndex is set to the signal index of member. If the member     specified is not signal this variable is set to -1.      \param[out] methodIndex is set to the method index of the member. If the     member is not a method of the object specified by obj param this variable     is set to -1. */
end_comment
begin_function
...
condition|)
block|{
comment|// re-use tmp_signal_name and signal from above
name|smeta
operator|=
name|sender
operator|->
...
end_function
begin_comment
comment|/*!     \since 4.8      Creates a connection of the given \a type from the \a signal in     the \a sender object to the \a method in the \a receiver object.     Returns true if the connection succeeds; otherwise returns false.      This function works in the same way as     connect(const QObject *sender, const char *signal,             const QObject *receiver, const char *method,             Qt::ConnectionType type)     but it uses QMetaMethod to specify signal and method.      \see connect(const QObject *sender, const char *signal,                  const QObject *receiver, const char *method,                  Qt::ConnectionType type)  */
end_comment
begin_function
DECL|function|connect
name|bool
...
return|;
block|}
comment|// Reconstructing SIGNAL() macro result for signal.signature() string
name|QByteArray
...
end_function
begin_comment
comment|/*! \internal    Same as the QMetaObject::connect, but \a signal_index must be the result of QObjectPrivate::signalIndex      method_index is relative to the rmeta metaobject, if rmeta is null, then it is absolute index  */
end_comment
begin_function
DECL|function|connect
name|bool
...
end_function
begin_comment
comment|/*!     \fn void QMetaObject::connectSlotsByName(QObject *object)      Searches recursively for all child objects of the given \a object, and connects     matching signals from them to slots of \a object that follow the following form:      \snippet doc/src/snippets/code/src_corelib_kernel_qobject.cpp 33      Let's assume our object has a child object of type QPushButton with     the \l{QObject::objectName}{object name} \c{button1}. The slot to catch the     button's \c{clicked()} signal would be:      \snippet doc/src/snippets/code/src_corelib_kernel_qobject.cpp 34      \sa QObject::setObjectName()  */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!\internal    \obsolete.    Used to be called from QMetaObject::activate(QObject *, QMetaObject *, int, int, void **) before Qt 4.6  */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!\internal    Obsolete.  (signal_index comes from indexOfMethod()) */
end_comment
begin_function
DECL|function|activate
name|void
...
end_function
begin_comment
comment|/*!\internal    Obsolete, called by moc generated code before Qt 4.6 for cloned signals    But since Qt 4.6, all clones are connected to their original  */
end_comment
begin_function
DECL|function|activate
name|void
...
end_function
begin_comment
comment|/*! \internal     Returns the signal index used in the internal connectionLists vector.      It is different from QMetaObject::indexOfSignal():  indexOfSignal is the same as indexOfMethod     while QObjectPrivate::signalIndex is smaller because it doesn't give index to slots. */
end_comment
begin_function
...
name|qWarning
argument_list|(
literal|"This compiler doesn't support streaming QObject to QDebug"
argument_list|)
expr_stmt|;
return|return
name|dbg
...
end_comment
begin_comment
comment|/*!     \macro Q_PROPERTY(...)     \relates QObject      This macro is used for declaring properties in classes that     inherit QObject. Properties behave like class data members, but     they have additional features accessible through the \l     {Meta-Object System}.      \snippet doc/src/snippets/code/src_corelib_kernel_qobject.cpp 36      The property name and type and the \c READ function are required.     The type can be any type supported by QVariant, or it can be a     user-defined type.  The other items are optional, but a \c WRITE     function is common.  The attributes default to true except \c USER,     which defaults to false.      For example:      \snippet doc/src/snippets/code/src_corelib_kernel_qobject.cpp 37      For more details about how to use this macro, and a more detailed     example of its use, see the discussion on \l {Qt's Property System}.      \sa {Qt's Property System} */
end_comment
begin_comment
comment|/*!     \macro Q_ENUMS(...)     \relates QObject      This macro registers one or several enum types to the meta-object     system.      For example:      \snippet doc/src/snippets/code/src_corelib_kernel_qobject.cpp 38      If you want to register an enum that is declared in another class,     the enum must be fully qualified with the name of the class     defining it. In addition, the class \e defining the enum has to     inherit QObject as well as declare the enum using Q_ENUMS().      \sa {Qt's Property System} */
end_comment
begin_comment
