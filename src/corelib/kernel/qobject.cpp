...
end_function
begin_comment
comment|// ### Qt>= 5.6, remove qt_add/removeObject
end_comment
begin_extern
...
end_function
begin_comment
comment|/*!     \class QSignalBlocker     \brief Exception-safe wrapper around QObject::blockSignals()     \since 5.3     \ingroup objectmodel     \inmodule QtCore      \reentrant      QSignalBlocker can be used whereever you would otherwise use a     pair of calls to blockSignals(). It blocks signals in its     constructor and in the destructor it resets the state to what     it was before the constructor ran.      \code     {     const QSignalBlocker blocker(someQObject);     // no signals here     }     \endcode     is thus equivalent to     \code     const bool wasBlocked = someQObject->blockSignals(true);     // no signals here     someQObject->blockSignals(wasBlocked);     \endcode      except the code using QSignalBlocker is safe in the face of     exceptions.      \sa QMutexLocker, QEventLoopLocker */
end_comment
begin_comment
comment|/*!     \fn QSignalBlocker::QSignalBlocker(QObject *object)      Constructor. Calls \a{object}->blockSignals(true). */
end_comment
begin_comment
comment|/*!     \fn QSignalBlocker::QSignalBlocker(QObject&object)     \overload      Calls \a{object}.blockSignals(true). */
end_comment
begin_comment
...
end_comment
begin_comment
comment|/*!     \fn void QSignalBlocker::reblock()      Re-blocks signals after a previous unblock().      The numbers of reblock() and unblock() calls are not counted, so     every reblock() undoes any number of unblock() calls. */
end_comment
begin_comment
...
name|nextConnectionList
expr_stmt|;
comment|// The destroy operation must happen outside the lock
if|if
condition|(
name|c
operator|->
...
argument_list|()
expr_stmt|;
comment|// we have been disconnected while the mutex was unlocked
for|for
control|(
name|int
name|n
...
name|qDebug
argument_list|(
literal|"<functor or function pointer>"
argument_list|)
expr_stmt|;
name|c
operator|=
...
end_function
begin_comment
comment|/*!     \internal      Internal version of connect used by the template version of QObject::connect (called via connectImpl) and     also used by the QObjectPrivate::connect version used by QML. The signal_index is expected to be relative     to the number of signals.  */
end_comment
begin_function
DECL|function|connectImpl
name|QMetaObject
...
end_function
begin_comment
comment|/*!  \internal  Used by QML to connect a signal by index to a slot implemented in JavaScript (wrapped in a custom QSlotOBjectBase subclass).   The signal_index is an index relative to the number of methods.  */
end_comment
begin_function
DECL|function|connect
name|QMetaObject
...
end_destructor
begin_comment
comment|/*! \internal Returns true if the object is still connected */
end_comment
begin_function
DECL|function|isConnected_helper
name|bool
name|QMetaObject
operator|::
name|Connection
...
argument_list|)
expr_stmt|;
comment|// we're only called from operator RestrictedBool() const
name|QObjectPrivate
operator|::
name|Connection
modifier|*
