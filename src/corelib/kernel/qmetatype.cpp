...
end_comment
begin_comment
comment|/*!     \fn const QMetaObject *QMetaType::metaObject() const     \since 5.5      return a QMetaObject relative to this type.      If the type is a pointer type to a subclass of QObject, flags() contains     QMetaType::PointerToQObject and this function returns the corresponding QMetaObject. This can     be used to in combinaison with QMetaObject::construct to create QObject of this type.      If the type is a Q_GADGET, flags() contains QMetaType::IsGadget, and this function returns its     QMetaObject.  This can be used to retrieve QMetaMethod and QMetaProperty and use them on a     pointer of this type. (given by QVariant::data for example)      If the type is an enumeration, flags() contains QMetaType::IsEnumeration, and this function     returns the QMetaObject of the enclosing object if the enum was registered as a Q_ENUM or 0     otherwise      \sa QMetaType::metaObjectForType(), QMetaType::flags() */
end_comment
begin_comment
...
end_comment
begin_comment
comment|/*!     \fn bool QMetaType::registerComparators()     \since 5.2     Registers comparison operators for the user-registered type T. This requires T to have     both an operator== and an operator<.     Returns \c true if the registration succeeded, otherwise false. */
end_comment
begin_comment
comment|/*!     \fn bool QMetaType::registerEqualsComparator()     \since 5.5     Registers equals operator for the user-registered type T. This requires T to have     an operator==.     Returns \c true if the registration succeeded, otherwise false. */
end_comment
begin_ifndef
ifndef|#
directive|ifndef
...
end_function
begin_comment
comment|/*!     Compares the objects at \a lhs and \a rhs. Both objects need to be of type \a typeId.     \a result is set to less than, equal to or greater than zero, if \a lhs is less than, equal to     or greater than \a rhs. Returns \c true, if the comparison succeeded, otherwise \c false.     \since 5.2 */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!     Compares the objects at \a lhs and \a rhs. Both objects need to be of type \a typeId.     \a result is set to zero, if \a lhs equals to rhs. Returns \c true, if the comparison     succeeded, otherwise \c false.     \since 5.5 */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!     \internal     Similar to QMetaType::type(), but only looks in the custom set of     types, and doesn't lock the mutex.     The extra \a firstInvalidIndex parameter is an easy way to avoid     iterating over customTypes() a second time in registerNormalizedType(). */
end_comment
begin_function
...
literal|false
return|;
comment|// only types without Q_DECLARE_METATYPE can be unregistered
if|if
condition|(
name|ct
operator|->
...
literal|false
return|;
comment|// invalidate type and all its alias entries
for|for
control|(
name|int
name|v
...
end_function
begin_comment
comment|/*!   \internal   \since 5.0   \overload   Don't use, kept for binary compatibility    ### TODO Qt6: remove me */
end_comment
begin_decl_stmt
...
argument_list|)
argument_list|; }
comment|/*!     \internal     \since 5.5      Registers a user type for marshalling, with \a normalizedTypeName,     a \a destructor, a \a constructor, and a \a size. Returns the type's     handle, or -1 if the type could not be registered.      \note normalizedTypeName is not checked for conformance with     Qt's normalized format, so it must already conform.  */
name|int
name|QMetaType
operator|::
name|registerNormalizedType
...
end_endif
begin_comment
comment|/*!     \since 5.0      returns QMetaType::metaObject for \a type      \sa metaObject() */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!     \fn int qRegisterMetaType(const char *typeName)     \relates QMetaType     \threadsafe      Registers the type name \a typeName for the type \c{T}. Returns     the internal ID used by QMetaType. Any class or struct that has a     public default constructor, a public copy constructor and a public     destructor can be registered.      This function requires that \c{T} is a fully defined type at the point     where the function is called. For pointer types, it also requires that the     pointed to type is fully defined. Use Q_DECLARE_OPAQUE_POINTER() to be able     to register pointers to forward declared types.      After a type has been registered, you can create and destroy     objects of that type dynamically at run-time.      This example registers the class \c{MyClass}:      \snippet code/src_corelib_kernel_qmetatype.cpp 4      This function is useful to register typedefs so they can be used     by QMetaProperty, or in QueuedConnections      \snippet code/src_corelib_kernel_qmetatype.cpp 9      \warning This function is useful only for registering an alias (typedef)     for every other use case Q_DECLARE_METATYPE and qMetaTypeId() should be used instead.      \sa {QMetaType::}{qRegisterMetaTypeStreamOperators()}, {QMetaType::}{isRegistered()},         Q_DECLARE_METATYPE() */
end_comment
begin_comment
...
end_function
begin_comment
comment|/*!     \fn void *QMetaType::createExtended(const void *copy) const     \internal      Method used for future binary compatible extensions. The function may be called     during QMetaType::create to force library call from inlined code.      ### TODO Qt6 remove the extension */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!     \fn void QMetaType::destroyExtended(void *data) const     \internal      Method used for future binary compatible extensions. The function may be called     during QMetaType::destroy to force library call from inlined code.      ### TODO Qt6 remove the extension */
end_comment
begin_function
