...
end_function
begin_comment
comment|/*!     Enters the event loop and waits until exit() is called, returning the value     that was passed to exit(). The value returned is 0 if exit() is called via     quit().      This function is meant to be called from within run(). It is necessary to     call this function to start event handling.      \sa quit(), exit() */
end_comment
begin_function
DECL|function|exec
name|int
...
end_function
begin_comment
comment|/*!     \since 5.2      Request the interruption of the thread.     That request is advisory and it is up to code running on the thread to decide     if and how it should act upon such request.     This function does not stop any event loop running on the thread and     does not terminate it in any way.      \sa isInterruptionRequested() */
end_comment
begin_function
...
name|qWarning
argument_list|(
literal|"QThread::requestInterruption has no effect on the main thread"
argument_list|)
expr_stmt|;
return|return;
block|}
...
end_function
begin_comment
comment|/*!     \since 5.2      Return true if the task running on this thread should be stopped.     An interruption can be requested by requestInterruption().      This function can be used to make long running tasks cleanly interruptible.     Never checking or acting on the value returned by this function is safe,     however it is advisable do so regularly in long running functions.     Take care not to call it too often, to keep the overhead low.      \code     void long_task() {          forever {             if ( QThread::currentThread()->isInterruptionRequested() ) {                 return;             }         }     }     \endcode      \sa currentThread() requestInterruption() */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!     \class QDaemonThread     \since 5.5     \brief The QDaemonThread provides a class to manage threads that outlive QCoreApplication     \internal      Note: don't try to deliver events from the started() signal. */
end_comment
begin_function
...
argument_list|)
block|{
comment|// QThread::started() is emitted from the thread we start
name|connect
argument_list|(
name|this
argument_list|,
