...
end_comment
begin_comment
comment|/*!     \fn T QAtomicInteger::load() const      Atomically loads the value of this QAtomicInteger using relaxed memory     ordering. The value is not modified in any way, but note that there's no     guarantee that it remains so.      \sa store(), loadAcquire() */
end_comment
begin_comment
comment|/*!     \fn T QAtomicInteger::loadAcquire() const      Atomically loads the value of this QAtomicInteger using the "Acquire" memory     ordering. The value is not modified in any way, but note that there's no     guarantee that it remains so.      \sa store(), load() */
end_comment
begin_comment
comment|/*!     \fn void QAtomicInteger::store(T newValue)      Atomically stores the \a newValue value into this atomic type, using     relaxed memory ordering.      \sa storeRelease(), load() */
end_comment
begin_comment
comment|/*!     \fn void QAtomicInteger::storeRelease(T newValue)      Atomically stores the \a newValue value into this atomic type, using     the "Release" memory ordering.      \sa store(), load() */
end_comment
begin_comment
comment|/*!     \fn QAtomicInteger::operator T() const     \since 5.3      Atomically loads the value of this QAtomicInteger using a sequentially     consistent memory ordering if possible; or "Acquire" ordering if not. The     value is not modified in any way, but note that there's no guarantee that     it remains so.      \sa load(), loadAcquire() */
end_comment
begin_comment
comment|/*!     \fn QAtomicInteger&QAtomicInteger::operator=(T newValue)     \since 5.3      Atomically stores the \a newValue value into this atomic type using a     sequentially consistent memory ordering if possible; or "Release" ordering     if not. This function returns a reference to this object.      \sa store(), storeRelease() */
end_comment
begin_comment
...
end_comment
begin_comment
comment|/*!     \fn T QAtomicInteger::operator++()     \since 5.3      Atomically pre-increments the value of this QAtomicInteger. Returns the new     value of this atomic.      This function uses a sequentially consistent memory ordering if possible;     or "Ordered" ordering if not.      \sa ref(), operator++(int), operator--() */
end_comment
begin_comment
comment|/*!     \fn T QAtomicInteger::operator++(int)     \since 5.3      Atomically post-increments the value of this QAtomicInteger. Returns the old     value of this atomic.      This function uses a sequentially consistent memory ordering if possible;     or "Ordered" ordering if not.      \sa ref(), operator++(), operator--(int) */
end_comment
begin_comment
...
end_comment
begin_comment
comment|/*!     \fn T QAtomicInteger::operator--()     \since 5.3      Atomically pre-decrements the value of this QAtomicInteger. Returns the new     value of this atomic.      This function uses a sequentially consistent memory ordering if possible;     or "Ordered" ordering if not.      \sa deref(), operator--(int), operator++() */
end_comment
begin_comment
comment|/*!     \fn T QAtomicInteger::operator--(int)     \since 5.3      Atomically post-decrements the value of this QAtomicInteger. Returns the old     value of this atomic.      This function uses a sequentially consistent memory ordering if possible;     or "Ordered" ordering if not.      \sa deref(), operator--(), operator++(int) */
end_comment
begin_comment
...
end_comment
begin_comment
comment|/*! \fn T QAtomicInteger::fetchAndAddRelaxed(T valueToAdd)      Atomic fetch-and-add.      Reads the current value of this QAtomicInteger and then adds     \a valueToAdd to the current value, returning the original value.      This function uses \e relaxed \l {QAtomicInteger#Memory     ordering}{memory ordering} semantics, leaving the compiler and     processor to freely reorder memory accesses.      \sa operator+=(), fetchAndSubRelaxed() */
end_comment
begin_comment
comment|/*! \fn T QAtomicInteger::fetchAndAddAcquire(T valueToAdd)      Atomic fetch-and-add.      Reads the current value of this QAtomicInteger and then adds     \a valueToAdd to the current value, returning the original value.      This function uses \e acquire \l {QAtomicInteger#Memory     ordering}{memory ordering} semantics, which ensures that memory     access following the atomic operation (in program order) may not     be re-ordered before the atomic operation.      \sa operator+=(), fetchAndSubAcquire() */
end_comment
begin_comment
comment|/*! \fn T QAtomicInteger::fetchAndAddRelease(T valueToAdd)      Atomic fetch-and-add.      Reads the current value of this QAtomicInteger and then adds     \a valueToAdd to the current value, returning the original value.      This function uses \e release \l {QAtomicInteger#Memory     ordering}{memory ordering} semantics, which ensures that memory     access before the atomic operation (in program order) may not be     re-ordered after the atomic operation.      \sa operator+=(), fetchAndSubRelease() */
end_comment
begin_comment
comment|/*! \fn T QAtomicInteger::fetchAndAddOrdered(T valueToAdd)      Atomic fetch-and-add.      Reads the current value of this QAtomicInteger and then adds     \a valueToAdd to the current value, returning the original value.      This function uses \e ordered \l {QAtomicInteger#Memory     ordering}{memory ordering} semantics, which ensures that memory     access before and after the atomic operation (in program order)     may not be re-ordered.      \sa operator+=(), fetchAndSubOrdered() */
end_comment
begin_comment
comment|/*! \fn T QAtomicInteger::operator+=(T valueToAdd)     \since 5.3      Atomic add-and-fetch.      Reads the current value of this QAtomicInteger and then adds     \a valueToAdd to the current value, returning the new value value.      This function uses a sequentially consistent memory ordering if possible;     or "Ordered" ordering if not.      \sa fetchAndAddOrdered(), operator-=() */
end_comment
begin_comment
comment|/*! \fn T QAtomicInteger::fetchAndSubRelaxed(T valueToSub)     \since 5.3      Atomic fetch-and-sub.      Reads the current value of this QAtomicInteger and then subtracts     \a valueToSub to the current value, returning the original value.      This function uses \e relaxed \l {QAtomicInteger#Memory     ordering}{memory ordering} semantics, leaving the compiler and     processor to freely reorder memory accesses.      \sa operator-=(), fetchAndAddRelaxed() */
end_comment
begin_comment
comment|/*! \fn T QAtomicInteger::fetchAndSubAcquire(T valueToSub)     \since 5.3      Atomic fetch-and-sub.      Reads the current value of this QAtomicInteger and then subtracts     \a valueToSub to the current value, returning the original value.      This function uses \e acquire \l {QAtomicInteger#Memory     ordering}{memory ordering} semantics, which ensures that memory     access following the atomic operation (in program order) may not     be re-ordered before the atomic operation.      \sa operator-=(), fetchAndAddAcquire() */
end_comment
begin_comment
comment|/*! \fn T QAtomicInteger::fetchAndSubRelease(T valueToSub)     \since 5.3      Atomic fetch-and-sub.      Reads the current value of this QAtomicInteger and then subtracts     \a valueToSub to the current value, returning the original value.      This function uses \e release \l {QAtomicInteger#Memory     ordering}{memory ordering} semantics, which ensures that memory     access before the atomic operation (in program order) may not be     re-ordered after the atomic operation.      \sa operator-=(), fetchAndAddRelease() */
end_comment
begin_comment
comment|/*! \fn T QAtomicInteger::fetchAndSubOrdered(T valueToSub)     \since 5.3      Atomic fetch-and-sub.      Reads the current value of this QAtomicInteger and then subtracts     \a valueToSub to the current value, returning the original value.      This function uses \e ordered \l {QAtomicInteger#Memory     ordering}{memory ordering} semantics, which ensures that memory     access before and after the atomic operation (in program order)     may not be re-ordered.      \sa operator-=(), fetchAndAddOrdered() */
end_comment
begin_comment
comment|/*! \fn T QAtomicInteger::operator-=(T valueToSub)     \since 5.3      Atomic sub-and-fetch.      Reads the current value of this QAtomicInteger and then subtracts     \a valueToSub to the current value, returning the new value value.      This function uses a sequentially consistent memory ordering if possible;     or "Ordered" ordering if not.      \sa fetchAndSubOrdered(), operator+=() */
end_comment
begin_comment
comment|/*! \fn T QAtomicInteger::fetchAndOrRelaxed(T valueToOr)     \since 5.3      Atomic fetch-and-or.      Reads the current value of this QAtomicInteger and then bitwise-ORs     \a valueToOr to the current value, returning the original value.      This function uses \e relaxed \l {QAtomicInteger#Memory     ordering}{memory ordering} semantics, leaving the compiler and     processor to freely reorder memory accesses.      \sa operator|=() */
end_comment
begin_comment
comment|/*! \fn T QAtomicInteger::fetchAndOrAcquire(T valueToOr)     \since 5.3      Atomic fetch-and-or.      Reads the current value of this QAtomicInteger and then bitwise-ORs     \a valueToOr to the current value, returning the original value.      This function uses \e acquire \l {QAtomicInteger#Memory     ordering}{memory ordering} semantics, which ensures that memory     access following the atomic operation (in program order) may not     be re-ordered before the atomic operation.      \sa operator|=() */
end_comment
begin_comment
comment|/*! \fn T QAtomicInteger::fetchAndOrRelease(T valueToOr)     \since 5.3      Atomic fetch-and-or.      Reads the current value of this QAtomicInteger and then bitwise-ORs     \a valueToOr to the current value, returning the original value.      This function uses \e release \l {QAtomicInteger#Memory     ordering}{memory ordering} semantics, which ensures that memory     access before the atomic operation (in program order) may not be     re-ordered after the atomic operation.      \sa operator|=() */
end_comment
begin_comment
comment|/*! \fn T QAtomicInteger::fetchAndOrOrdered(T valueToOr)     \since 5.3      Atomic fetch-and-or.      Reads the current value of this QAtomicInteger and then bitwise-ORs     \a valueToOr to the current value, returning the original value.      This function uses \e ordered \l {QAtomicInteger#Memory     ordering}{memory ordering} semantics, which ensures that memory     access before and after the atomic operation (in program order)     may not be re-ordered.      \sa operator|=() */
end_comment
begin_comment
comment|/*! \fn T QAtomicInteger::operator|=(T valueToOr)     \since 5.3      Atomic or-and-fetch.      Reads the current value of this QAtomicInteger and then bitwise-ORs     \a valueToOr to the current value, returning the new value value.      This function uses a sequentially consistent memory ordering if possible;     or "Ordered" ordering if not.      \sa fetchAndOrOrdered() */
end_comment
begin_comment
comment|/*! \fn T QAtomicInteger::fetchAndXorRelaxed(T valueToXor)     \since 5.3      Atomic fetch-and-xor.      Reads the current value of this QAtomicInteger and then bitwise-XORs     \a valueToXor to the current value, returning the original value.      This function uses \e relaxed \l {QAtomicInteger#Memory     ordering}{memory ordering} semantics, leaving the compiler and     processor to freely reorder memory accesses.      \sa operator^=() */
end_comment
begin_comment
comment|/*! \fn T QAtomicInteger::fetchAndXorAcquire(T valueToXor)     \since 5.3      Atomic fetch-and-xor.      Reads the current value of this QAtomicInteger and then bitwise-XORs     \a valueToXor to the current value, returning the original value.      This function uses \e acquire \l {QAtomicInteger#Memory     ordering}{memory ordering} semantics, which ensures that memory     access following the atomic operation (in program order) may not     be re-ordered before the atomic operation.      \sa operator^=() */
end_comment
begin_comment
comment|/*! \fn T QAtomicInteger::fetchAndXorRelease(T valueToXor)     \since 5.3      Atomic fetch-and-xor.      Reads the current value of this QAtomicInteger and then bitwise-XORs     \a valueToXor to the current value, returning the original value.      This function uses \e release \l {QAtomicInteger#Memory     ordering}{memory ordering} semantics, which ensures that memory     access before the atomic operation (in program order) may not be     re-ordered after the atomic operation.      \sa operator^=() */
end_comment
begin_comment
comment|/*! \fn T QAtomicInteger::fetchAndXorOrdered(T valueToXor)     \since 5.3      Atomic fetch-and-xor.      Reads the current value of this QAtomicInteger and then bitwise-XORs     \a valueToXor to the current value, returning the original value.      This function uses \e ordered \l {QAtomicInteger#Memory     ordering}{memory ordering} semantics, which ensures that memory     access before and after the atomic operation (in program order)     may not be re-ordered.      \sa operator^=() */
end_comment
begin_comment
comment|/*! \fn T QAtomicInteger::operator^=(T valueToXor)     \since 5.3      Atomic xor-and-fetch.      Reads the current value of this QAtomicInteger and then bitwise-XORs     \a valueToXor to the current value, returning the new value value.      This function uses a sequentially consistent memory ordering if possible;     or "Ordered" ordering if not.      \sa fetchAndXorOrdered() */
end_comment
begin_comment
comment|/*! \fn T QAtomicInteger::fetchAndAndRelaxed(T valueToAnd)     \since 5.3      Atomic fetch-and-and.      Reads the current value of this QAtomicInteger and then bitwise-ANDs     \a valueToAnd to the current value, returning the original value.      This function uses \e relaxed \l {QAtomicInteger#Memory     ordering}{memory ordering} semantics, leaving the compiler and     processor to freely reorder memory accesses.      \sa operator&=() */
end_comment
begin_comment
comment|/*! \fn T QAtomicInteger::fetchAndAndAcquire(T valueToAnd)     \since 5.3      Atomic fetch-and-and.      Reads the current value of this QAtomicInteger and then bitwise-ANDs     \a valueToAnd to the current value, returning the original value.      This function uses \e acquire \l {QAtomicInteger#Memory     ordering}{memory ordering} semantics, which ensures that memory     access following the atomic operation (in program order) may not     be re-ordered before the atomic operation.      \sa operator&=() */
end_comment
begin_comment
comment|/*! \fn T QAtomicInteger::fetchAndAndRelease(T valueToAnd)     \since 5.3      Atomic fetch-and-and.      Reads the current value of this QAtomicInteger and then bitwise-ANDs     \a valueToAnd to the current value, returning the original value.      This function uses \e release \l {QAtomicInteger#Memory     ordering}{memory ordering} semantics, which ensures that memory     access before the atomic operation (in program order) may not be     re-ordered after the atomic operation.      \sa operator&=() */
end_comment
begin_comment
comment|/*! \fn T QAtomicInteger::fetchAndAndOrdered(T valueToAnd)     \since 5.3      Atomic fetch-and-and.      Reads the current value of this QAtomicInteger and then bitwise-ANDs     \a valueToAnd to the current value, returning the original value.      This function uses \e ordered \l {QAtomicInteger#Memory     ordering}{memory ordering} semantics, which ensures that memory     access before and after the atomic operation (in program order)     may not be re-ordered.      \sa operator&=() */
end_comment
begin_comment
comment|/*! \fn T QAtomicInteger::operator&=(T valueToAnd)     \since 5.3      Atomic add-and-fetch.      Reads the current value of this QAtomicInteger and then bitwise-ANDs     \a valueToAnd to the current value, returning the new value value.      This function uses a sequentially consistent memory ordering if possible;     or "Ordered" ordering if not.      \sa fetchAndAndOrdered() */
end_comment
begin_comment
