...
comment|// Latin 1 case system, used by QByteArray::to{Upper,Lower}() and qstr(n)icmp():
...
end_decl_stmt
begin_comment
comment|/* #!/usr/bin/perl -l use feature "unicode_strings"; for (0..255) {     $up = lc(chr($_));     $up = chr($_) if ord($up)> 0x100 || length $up> 1;     printf "0x%02x,", ord($up);     print "" if ($_& 0xf) == 0xf; } */
end_comment
begin_decl_stmt
...
end_function_decl
begin_comment
comment|/*  * This pair of functions is declared in qtools_p.h and is used by the Qt  * containers to allocate memory and grow the memory block during append  * operations.  *  * They take size_t parameters and return size_t so they will change sizes  * according to the pointer width. However, knowing Qt containers store the  * container size and element indexes in ints, these functions never return a  * size larger than INT_MAX. This is done by casting the element count and  * memory block size to int in several comparisons: the check for negative is  * very fast on most platforms as the code only needs to check the sign bit.  *  * These functions return SIZE_MAX on overflow, which can be passed to malloc()  * and will surely cause a NULL return (there's no way you can allocate a  * memory block the size of your entire VM space).  */
end_comment
begin_comment
comment|/*!     \internal     \since 5.7      Returns the memory block size for a container containing \a elementCount     elements, each of \a elementSize bytes, plus a header of \a headerSize     bytes. That is, this function returns \c       {elementCount * elementSize + headerSize}      but unlike the simple calculation, it checks for overflows during the     multiplication and the addition.      Both \a elementCount and \a headerSize can be zero, but \a elementSize     cannot.      This function returns SIZE_MAX (~0) on overflow or if the memory block size     would not fit an int. */
end_comment
begin_function
name|size_t
...
end_function
begin_comment
comment|/*!     \internal     \since 5.7      Returns the memory block size and the number of elements that will fit in     that block for a container containing \a elementCount elements, each of \a     elementSize bytes, plus a header of \a headerSize bytes. This function     assumes the container will grow and pre-allocates a growth factor.      Both \a elementCount and \a headerSize can be zero, but \a elementSize     cannot.      This function returns SIZE_MAX (~0) on overflow or if the memory block size     would not fit an int.      \note The memory block may contain up to \a elementSize - 1 bytes more than     needed. */
end_comment
begin_function
...
literal|0
condition|)
comment|// catches std::numeric_limits<size_t>::max()
return|return
name|result
return|;
name|unsigned
...
condition|)
block|{
comment|// catches morebytes == 2GB
comment|// grow by half the difference between bytes and morebytes
name|bytes
operator|+=
operator|(
name|morebytes
...
end_function
begin_comment
comment|/*! \fn QByteArray&QByteArray::prepend(int count, char ch)      \overload     \since 5.7      Prepends \a count copies of character \a ch to this byte array. */
end_comment
begin_comment
...
end_function
begin_comment
comment|/*! \fn QByteArray&QByteArray::append(int count, char ch)      \overload     \since 5.7      Appends \a count copies of character \a ch to this byte     array and returns a reference to this byte array.      If \a count is negative or zero nothing is appended to the byte array. */
end_comment
begin_comment
...
end_function
begin_comment
comment|/*! \fn QByteArray&QByteArray::insert(int i, int count, char ch)      \overload     \since 5.7      Inserts \a count copies of character \a ch at index position \a i in the     byte array.      If \a i is greater than size(), the array is first extended using resize(). */
end_comment
begin_function
DECL|function|insert
name|QByteArray
...
parameter_list|)
block|{
comment|// find the first bad character in input
specifier|const
name|char
modifier|*
name|orig_begin
...
argument_list|)
decl_stmt|;
comment|// will copy if T is const QByteArray
name|char
modifier|*
name|b
init|=
...
end_function
begin_comment
comment|/*!     \fn QByteArray QByteArray::toUpper() const      Returns an uppercase copy of the byte array. The bytearray is     interpreted as a Latin-1 encoded string.      Example:     \snippet code/src_corelib_tools_qbytearray.cpp 31      \sa toLower(), {8-bit Character Comparisons} */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!     \fn QByteArray QByteArray::trimmed() const      Returns a byte array that has whitespace removed from the start     and the end.      Whitespace means any character for which the standard C++     \c isspace() function returns \c true in the C locale. This includes the ASCII     characters '\\t', '\\n', '\\v', '\\f', '\\r', and ' '.      Example:     \snippet code/src_corelib_tools_qbytearray.cpp 33      Unlike simplified(), \l {QByteArray::trimmed()}{trimmed()} leaves internal whitespace alone.      \sa simplified() */
end_comment
begin_function
