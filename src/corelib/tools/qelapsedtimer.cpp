...
comment|/*!     \fn QElapsedTimer::QElapsedTimer()     \since 5.4      Constructs an invalid QElapsedTimer. A timer becomes valid once it has been     started.      \sa isValid(), start() */
comment|/*!     \fn bool QElapsedTimer::operator ==(const QElapsedTimer&other) const      Returns \c true if this object and \a other contain the same time. */
comment|/*!     \fn bool QElapsedTimer::operator !=(const QElapsedTimer&other) const      Returns \c true if this object and \a other contain different times. */
...
end_function
begin_comment
comment|/*!     Returns \c false if the timer has never been started or invalidated by a     call to invalidate().      \sa invalidate(), start(), restart() */
end_comment
begin_function
name|bool
name|QElapsedTimer
...
end_function
begin_comment
comment|/*!     Returns \c true if this QElapsedTimer has already expired by \a timeout     milliseconds (that is, more than \a timeout milliseconds have elapsed).     The value of \a timeout can be -1 to indicate that this timer does not     expire, in which case this function will always return false.      \sa elapsed() */
end_comment
begin_function
name|bool
name|QElapsedTimer
