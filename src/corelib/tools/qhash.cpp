...
parameter_list|)
block|{
comment|// The crc32[whbd] instructions on Aarch64/Aarch32 calculate a 32-bit CRC32 checksum
specifier|const
name|uchar
modifier|*
name|p
...
end_comment
begin_comment
comment|/*!     \fn uint qHash(const std::pair<T1, T2>&key, uint seed = 0)     \since 5.7     \relates QHash      Returns the hash value for the \a key, using \a seed to seed the calculation.      Types \c T1 and \c T2 must be supported by qHash().      \note The return type of this function is \e{not} the same as that of     \code     qHash(qMakePair(key.first, key.second), seed);     \endcode     The two functions use different hashing algorithms; due to binary compatibility     constraints, we cannot change the QPair algorithm to match the std::pair one before Qt 6. */
end_comment
begin_comment
...
end_comment
begin_comment
comment|/*! \fn QPair<iterator, iterator> QHash::equal_range(const Key&key)     \since 5.7      Returns a pair of iterators delimiting the range of values \c{[first, second)}, that     are stored under \a key. If the range is empty then both iterators will be equal to end(). */
end_comment
begin_comment
comment|/*!     \fn QPair<const_iterator, const_iterator> QHash::equal_range(const Key&key) const     \overload     \since 5.7 */
end_comment
begin_comment
