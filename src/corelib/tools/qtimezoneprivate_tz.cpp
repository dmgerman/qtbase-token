...
end_function
begin_comment
comment|// returns the time in seconds, INT_MIN if we failed to parse
end_comment
begin_function
...
literal|0
decl_stmt|;
comment|// Note that the calls to qstrtoll do *not* check the end pointer, which
comment|// means they proceed until they find a non-digit. We check that we're
comment|// still in range at the end, but we may have read from past end. It's the
comment|// caller's responsibility to ensure that begin is part of a
...
block|}
block|}
comment|// we must have consumed everything
if|if
condition|(
name|begin
operator|!=
...
condition|)
block|{
comment|// if we failed to parse, return 02:00
return|return
name|QTime
argument_list|(
literal|2
...
block|{
comment|// Format "[+|-]hh[:mm[:ss]]"
comment|// note that the sign is inverted because POSIX counts in hours West of GMT
name|bool
name|negate
init|=
literal|true
...
literal|0x20
expr_stmt|;
comment|// lowercases if it is a letter, otherwise just corrupts ch
return|return
name|ch
operator|>=
literal|'a'
...
end_function
begin_comment
comment|// Returns the zone name, the offset (in seconds) and advances \a begin to
end_comment
begin_comment
comment|// where the parsing ended. Returns a zone of INT_MIN in case an offset
end_comment
begin_comment
comment|// couldn't be read.
end_comment
begin_function
...
condition|)
block|{
comment|// POSIX says only alphanumeric, but we allow anything
operator|++
name|nameEnd
expr_stmt|;
block|}
...
name|result
return|;
comment|// name must be at least 3 characters long
comment|// zone offset, form [+-]hh:mm:ss
specifier|const
name|char
modifier|*
name|zoneBegin
...
comment|// See the section about TZ at http://pubs.opengroup.org/onlinepubs/9699919799/basedefs/V1_chap08.html
name|QList
argument_list|<
name|QByteArray
argument_list|>
...
literal|0
expr_stmt|;
comment|// reset to UTC if we failed to parse
block|}
elseif|else
if|if
condition|(
...
condition|)
block|{
comment|// if the dst offset isn't provided, it is 1 hour ahead of the standard offset
name|dstZone
operator|.
name|second
operator|=
