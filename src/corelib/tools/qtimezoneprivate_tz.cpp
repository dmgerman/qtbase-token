...
name|hdr
return|;
comment|// Parse Version, 1 byte, before 2005 was '\0', since 2005 a '2'
name|ds
operator|>>
name|ch
expr_stmt|;
...
comment|// tz_abbrind index points to the first char of the abbreviation in the array, not the
comment|// occurrence in the list. By parsing char at a time we can track the char index and convert
comment|// to an occurrence index. By using a map with tz_abbrind as ordered key we get both index
...
name|abbrev
decl_stmt|;
comment|// Track the start position of each abbreviation
name|int
name|tz_abbrind
init|=
literal|0
...
break|break;
block|}
comment|// If after the last transition time then we need to use the posix rule if available
if|if
condition|(
name|tran
operator|>=
...
block|}
block|}
comment|// Otherwise use the transition we found
name|QTimeZonePrivate
operator|::
name|Data
name|data
