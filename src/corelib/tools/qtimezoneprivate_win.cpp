...
end_else
begin_comment
comment|// QT_USE_REGISTRY_TIMEZONE
end_comment
begin_struct
DECL|struct|QWinDynamicTimeZone
struct|struct
name|QWinDynamicTimeZone
block|{
...
name|QWinDynamicTimeZone
argument_list|>
name|QWinRTTimeZoneHash
typedef|;
end_typedef
begin_macro
name|Q_GLOBAL_STATIC
argument_list|(
argument|QWinRTTimeZoneHash
argument_list|,
argument|gTimeZones
argument_list|)
end_macro
...
argument_list|)
expr_stmt|;
comment|// As soon as key name repeats, break. Some systems continue to always
comment|// return the last item independent of index being out of range
if|if
condition|(
name|item
operator|.
...
parameter_list|)
block|{
comment|// If month is 0 then there is no date
if|if
condition|(
name|rule
operator|.
...
return|;
block|}
comment|// If the year is set then is an absolute date
return|return
name|QDate
argument_list|(
name|time
...
argument_list|)
expr_stmt|;
comment|// If there is no dynamic information, you can still query for
comment|// year 0, which helps simplifying following part
for|for
control|(
name|DWORD
name|year
...
specifier|const
block|{
comment|// Convert MSecs to year to get transitions for, assumes no transitions around 31 Dec/1 Jan
name|int
name|year
init|=
name|msecsToDate
...
argument_list|)
expr_stmt|;
comment|// If no transition rules to calculate then no DST, so just use rule for std
if|if
condition|(
name|rule
operator|.
...
name|rule
decl_stmt|;
comment|// If the required year falls after the last rule start year and the last rule has no
comment|// valid future transition calculations then there is no next transition
if|if
condition|(
name|year
operator|>
...
argument_list|)
expr_stmt|;
comment|// If the rules have either a fixed year, or no month, then no future trans
if|if
condition|(
name|rule
operator|.
...
block|}
block|}
comment|// Otherwise we have a valid rule for the required year that can be used
comment|// to calculate this year or next
name|qint64
name|first
decl_stmt|;
name|qint64
...
argument_list|)
expr_stmt|;
comment|// If no transition rules to calculate then no next transition
if|if
condition|(
name|rule
operator|.
...
name|rule
decl_stmt|;
comment|// If the required year falls before the first rule start year and the first rule has no
comment|// valid transition calculations then there is no previous transition
if|if
condition|(
name|year
operator|<
...
argument_list|)
expr_stmt|;
comment|// If the rules have either a fixed year, or no month, then no previous trans
if|if
condition|(
name|rule
operator|.
...
argument_list|)
expr_stmt|;
comment|// If no transition rules to calculate then no previous transition
if|if
condition|(
name|rule
operator|.
