...
end_comment
begin_comment
comment|/*! \fn const T QMap::value(const Key&key) const  */
end_comment
begin_comment
comment|/*! \fn const T QMap::value(const Key&key, const T&defaultValue) const      \overload      Returns the value associated with the key \a key.      If the map contains no item with key \a key, the function returns     \a defaultValue. If no \a defaultValue is specified, the function     returns a \l{default-constructed value}. If there are multiple     items for \a key in the map, the value of the most recently     inserted one is returned.      \sa key(), values(), contains(), operator[]() */
end_comment
begin_comment
