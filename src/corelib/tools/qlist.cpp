...
parameter_list|)
block|{
comment|// dear compiler: don't optimize me out.
specifier|volatile
name|int
name|x
init|=
...
end_function
begin_comment
comment|/*!  *  Detaches the QListData by allocating new memory for a list which will be bigger  *  than the copied one and is expected to grow further.  *  *idx is the desired insertion point and is clamped to the actual size of the list.  *  num is the number of new elements to insert at the insertion point.  *  Returns the old (shared) data, it is up to the caller to deref() and free().  *  For the new data node_copy needs to be called.  *  *  \internal  */
end_comment
begin_function
...
name|alloc
expr_stmt|;
comment|// The space reservation algorithm's optimization is biased towards appending:
comment|// Something which looks like an append will put the data at the beginning,
comment|// while something which looks like a prepend will put it in the middle
comment|// instead of at the end. That's based on the assumption that prepending
comment|// is uncommon and even an initial prepend will eventually be followed by
comment|// at least some appends.
name|int
name|bg
decl_stmt|;
if|if
...
error|#
directive|error
literal|"Remove QListData::detach(), it is only required for binary compatibility for 4.0.x to 4.2.x"
end_error
begin_endif
endif|#
directive|endif
...
end_function
begin_comment
comment|/*!  *  Detaches the QListData by reallocating new memory.  *  Returns the old (shared) data, it is up to the caller to deref() and free()  *  For the new data node_copy needs to be called.  *  *  \internal  */
end_comment
begin_if
if|#
directive|if
...
error|#
directive|error
literal|"Remove QListData::detach2(), it is only required for binary compatibility for 4.3.x to 4.5.x"
end_error
begin_endif
endif|#
directive|endif
...
end_function
begin_comment
comment|/*!  *  Detaches the QListData by allocating new memory for a list which possibly  *  has a different size than the copied one.  *  Returns the old (shared) data, it is up to the caller to deref() and free()  *  For the new data node_copy needs to be called.  *  *  \internal  */
end_comment
begin_function
DECL|function|detach
name|QListData
...
error|#
directive|error
literal|"Remove QListData::detach3(), it is only required for binary compatibility for 4.5.x to 4.6.x"
end_error
begin_endif
endif|#
directive|endif
...
end_function
begin_comment
comment|// ensures that enough space is available to append n elements
end_comment
begin_function
...
condition|)
block|{
comment|// we have enough space. Just not at the end -> move it.
name|e
operator|-=
name|b
expr_stmt|;
...
end_function
begin_comment
comment|// ensures that enough space is available to append one element
end_comment
begin_function
DECL|function|append
name|void
...
error|#
directive|error
literal|"Remove QListData::append(), it is only required for binary compatibility up to 4.5.x"
end_error
begin_endif
endif|#
directive|endif
...
condition|)
block|{
comment|// If the array is full, we expand it and move some items rightward
name|realloc
argument_list|(
name|grow
argument_list|(
...
block|{
comment|// If there is free space at the end of the array, we move some items rightward
block|}
block|}
else|else
block|{
...
condition|)
block|{
comment|// If there is free space at the beginning of the array, we move some items leftward
name|leftward
operator|=
literal|true
expr_stmt|;
...
block|{
comment|// If there is free space at both ends, we move as few items as possible
name|leftward
operator|=
operator|(
name|i
...
end_comment
begin_comment
comment|/*! \fn QList::QList(std::initializer_list<T> args)     \since 4.8      Construct a list from a std::initilizer_list.      This constructor is only enabled if the compiler supports C++0x */
end_comment
begin_comment
...
end_comment
begin_comment
comment|/*! \fn QList<T>&QList::operator=(const QList<T>&other)      Assigns \a other to this list and returns a reference to this     list. */
end_comment
begin_comment
...
end_comment
begin_comment
comment|/*! \fn void QList::detach()      \internal */
end_comment
begin_comment
comment|/*! \fn void QList::detachShared()      \internal      like detach(), but does nothing if we're shared_null.     This prevents needless mallocs, and makes QList more exception safe     in case of cleanup work done in destructors on empty lists. */
end_comment
begin_comment
comment|/*! \fn bool QList::isDetached() const      \internal */
end_comment
begin_comment
comment|/*! \fn void QList::setSharable(bool sharable)      \internal */
end_comment
begin_comment
...
end_comment
begin_comment
comment|/*! \fn void QList::replace(int i, const T&value)      Replaces the item at index position \a i with \a value. \a i must     be a valid index position in the list (i.e., 0<= \a i< size()).      \sa operator[](), removeAt() */
end_comment
begin_comment
comment|/*!     \fn int QList::removeAll(const T&value)      Removes all occurrences of \a value in the list and returns the     number of entries removed.      Example:     \snippet doc/src/snippets/code/src_corelib_tools_qlistdata.cpp 9      This function requires the value type to have an implementation of     \c operator==().      \sa removeOne(), removeAt(), takeAt(), replace() */
end_comment
begin_comment
comment|/*!     \fn bool QList::removeOne(const T&value)     \since 4.4      Removes the first occurrence of \a value in the list and returns     true on success; otherwise returns false.      Example:     \snippet doc/src/snippets/code/src_corelib_tools_qlistdata.cpp 10      This function requires the value type to have an implementation of     \c operator==().      \sa removeAll(), removeAt(), takeAt(), replace() */
end_comment
begin_comment
...
end_comment
begin_comment
comment|/*! \fn void QList::move(int from, int to)      Moves the item at index position \a from to index position \a to.      Example:     \snippet doc/src/snippets/code/src_corelib_tools_qlistdata.cpp 11      This is the same as insert(\a{to}, takeAt(\a{from})).This function     assumes that both \a from and \a to are at least 0 but less than     size(). To avoid failure, test that both \a from and \a to are at     least 0 and less than size().      \sa swap(), insert(), takeAt() */
end_comment
begin_comment
comment|/*! \fn void QList::swap(int i, int j)      Exchange the item at index position \a i with the item at index     position \a j. This function assumes that both \a i and \a j are     at least 0 but less than size(). To avoid failure, test that both     \a i and \a j are at least 0 and less than size().      Example:     \snippet doc/src/snippets/code/src_corelib_tools_qlistdata.cpp 12      \sa move() */
end_comment
begin_comment
comment|/*! \fn int QList::indexOf(const T&value, int from = 0) const      Returns the index position of the first occurrence of \a value in     the list, searching forward from index position \a from. Returns     -1 if no item matched.      Example:     \snippet doc/src/snippets/code/src_corelib_tools_qlistdata.cpp 13      This function requires the value type to have an implementation of     \c operator==().      Note that QList uses 0-based indexes, just like C++ arrays. Negative     indexes are not supported with the exception of the value mentioned     above.      \sa lastIndexOf(), contains() */
end_comment
begin_comment
...
end_comment
begin_comment
comment|/*! \fn QList::iterator QList::erase(iterator begin, iterator end)      \overload      Removes all the items from \a begin up to (but not including) \a     end. Returns an iterator to the same item that \a end referred to     before the call. */
end_comment
begin_comment
comment|/*! \typedef QList::Iterator      Qt-style synonym for QList::iterator. */
end_comment
begin_comment
comment|/*! \typedef QList::ConstIterator      Qt-style synonym for QList::const_iterator. */
end_comment
begin_comment
comment|/*!     \typedef QList::size_type      Typedef for int. Provided for STL compatibility. */
end_comment
begin_comment
comment|/*!     \typedef QList::value_type      Typedef for T. Provided for STL compatibility. */
end_comment
begin_comment
comment|/*!     \typedef QList::difference_type      Typedef for ptrdiff_t. Provided for STL compatibility. */
end_comment
begin_comment
comment|/*!     \typedef QList::pointer      Typedef for T *. Provided for STL compatibility. */
end_comment
begin_comment
comment|/*!     \typedef QList::const_pointer      Typedef for const T *. Provided for STL compatibility. */
end_comment
begin_comment
comment|/*!     \typedef QList::reference      Typedef for T&. Provided for STL compatibility. */
end_comment
begin_comment
...
end_comment
begin_comment
comment|/*! \fn void QList::removeLast()      Removes the last item in the list. Calling this function is     equivalent to calling removeAt(size() - 1). The list must not be     empty. If the list can be empty, call isEmpty() before calling     this function.      \sa removeAt(), takeLast() */
end_comment
begin_comment
comment|/*! \fn T QList::value(int i) const      Returns the value at index position \a i in the list.      If the index \a i is out of bounds, the function returns a     \l{default-constructed value}. If you are certain that the index     is going to be within bounds, you can use at() instead, which is     slightly faster.      \sa at(), operator[]() */
end_comment
begin_comment
comment|/*! \fn T QList::value(int i, const T&defaultValue) const      \overload      If the index \a i is out of bounds, the function returns     \a defaultValue. */
end_comment
begin_comment
comment|/*! \fn void QList::push_back(const T&value)      This function is provided for STL compatibility. It is equivalent     to \l{QList::append()}{append(\a value)}. */
end_comment
begin_comment
comment|/*! \fn void QList::push_front(const T&value)      This function is provided for STL compatibility. It is equivalent     to \l{QList::prepend()}{prepend(\a value)}. */
end_comment
begin_comment
comment|/*! \fn T& QList::front()      This function is provided for STL compatibility. It is equivalent     to first(). The list must not be empty. If the list can be empty,     call isEmpty() before calling this function. */
end_comment
begin_comment
comment|/*! \fn const T& QList::front() const      \overload */
end_comment
begin_comment
comment|/*! \fn T& QList::back()      This function is provided for STL compatibility. It is equivalent     to last(). The list must not be empty. If the list can be empty,     call isEmpty() before calling this function. */
end_comment
begin_comment
comment|/*! \fn const T& QList::back() const      \overload */
end_comment
begin_comment
comment|/*! \fn void QList::pop_front()      This function is provided for STL compatibility. It is equivalent     to removeFirst(). The list must not be empty. If the list can be     empty, call isEmpty() before calling this function. */
end_comment
begin_comment
...
end_comment
begin_comment
comment|/*! \fn QList::iterator QList::iterator::operator++(int)      \overload      The postfix ++ operator (\c{it++}) advances the iterator to the     next item in the list and returns an iterator to the previously     current item. */
end_comment
begin_comment
comment|/*! \fn QList::iterator&QList::iterator::operator--()      The prefix -- operator (\c{--it}) makes the preceding item     current and returns an iterator to the new current item.      Calling this function on QList::begin() leads to undefined results.      \sa operator++() */
end_comment
begin_comment
...
end_comment
begin_comment
comment|/*! \fn QList::const_iterator QList::const_iterator::operator++(int)      \overload      The postfix ++ operator (\c{it++}) advances the iterator to the     next item in the list and returns an iterator to the previously     current item. */
end_comment
begin_comment
comment|/*! \fn QList::const_iterator&QList::const_iterator::operator--()      The prefix -- operator (\c{--it}) makes the preceding item     current and returns an iterator to the new current item.      Calling this function on QList::begin() leads to undefined results.      \sa operator++() */
end_comment
begin_comment
...
end_comment
begin_comment
comment|/*! \fn QVector<T> QList<T>::toVector() const      Returns a QVector object with the data contained in this QList.      Example:      \snippet doc/src/snippets/code/src_corelib_tools_qlistdata.cpp 22      \sa toSet(), fromVector(), QVector::fromList() */
end_comment
begin_comment
comment|/*! \fn QList<T> QList<T>::fromSet(const QSet<T>&set)      Returns a QList object with the data contained in \a set. The     order of the elements in the QList is undefined.      Example:      \snippet doc/src/snippets/code/src_corelib_tools_qlistdata.cpp 23      \sa fromVector(), toSet(), QSet::toList(), qSort() */
end_comment
begin_comment
comment|/*! \fn QSet<T> QList<T>::toSet() const      Returns a QSet object with the data contained in this QList.     Since QSet doesn't allow duplicates, the resulting QSet might be     smaller than the original list was.      Example:      \snippet doc/src/snippets/code/src_corelib_tools_qlistdata.cpp 24      \sa toVector(), fromSet(), QSet::fromList() */
end_comment
begin_comment
comment|/*! \fn QList<T> QList<T>::fromStdList(const std::list<T>&list)      Returns a QList object with the data contained in \a list. The     order of the elements in the QList is the same as in \a list.      Example:      \snippet doc/src/snippets/code/src_corelib_tools_qlistdata.cpp 25      \sa toStdList(), QVector::fromStdVector() */
end_comment
begin_comment
comment|/*! \fn std::list<T> QList<T>::toStdList() const      Returns a std::list object with the data contained in this QList.     Example:      \snippet doc/src/snippets/code/src_corelib_tools_qlistdata.cpp 26      \sa fromStdList(), QVector::toStdVector() */
end_comment
begin_macro
