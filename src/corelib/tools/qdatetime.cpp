...
end_decl_stmt
begin_comment
comment|/*****************************************************************************   QDate static helper functions  *****************************************************************************/
end_comment
begin_function
...
end_function
begin_comment
comment|/*   Division, rounding down (rather than towards zero).    From C++11 onwards, integer division is defined to round towards zero, so we   can rely on that when implementing this.  This is only used with denominator b> 0, so we only have to treat negative numerator, a, specially.  */
end_comment
begin_function
...
end_function
begin_comment
comment|/*****************************************************************************   Date/Time formatting helper functions  *****************************************************************************/
end_comment
begin_decl_stmt
...
literal|0
return|;
comment|// sign will be +1 for a positive and -1 for a negative offset
name|int
name|sign
decl_stmt|;
...
condition|)
block|{
comment|// last week of previous year
operator|--
name|year
expr_stmt|;
name|week
...
condition|)
block|{
comment|// maybe first week of next year
name|int
name|w
init|=
operator|(
...
case|:
block|{
comment|// Semi-strict parsing, must be long enough and have non-numeric separators
if|if
condition|(
name|string
operator|.
...
block|{
comment|// HH:mm:ss or HH:mm:ss.zzz
name|second
operator|=
name|string
operator|.
...
end_function
begin_comment
comment|/*****************************************************************************   QDateTime static helper functions  *****************************************************************************/
end_comment
begin_comment
...
end_function
begin_comment
comment|// Calls the platform variant of mktime for the given date, time and daylightStatus,
end_comment
begin_comment
...
argument_list|)
expr_stmt|;
comment|// tm_[wy]day plus any non-standard fields
name|local
operator|.
name|tm_sec
operator|=
...
name|TimeZone
case|:
comment|// Use system time zone instead
name|m_spec
operator|=
name|Qt
operator|::
...
end_function
begin_comment
comment|// Set the Daylight Status if LocalTime set via msecs
end_comment
begin_function
...
name|UTC
case|:
comment|// for these, a valid date and a valid time imply a valid QDateTime
if|if
condition|(
name|isValidDate
argument_list|()
...
name|LocalTime
case|:
comment|// for these, we need to check whether the timezone is valid and whether
comment|// the time is valid in that timezone. Expensive, but no other option.
...
end_ifndef
begin_comment
comment|// Convert a TimeZone time expressed in zone msecs encoding into a UTC epoch msecs
end_comment
begin_function
...
parameter_list|)
block|{
comment|// Get the effective data from QTimeZone
...
end_ifndef
begin_comment
comment|/*!     \since 5.2      Constructs a datetime with the given \a date and \a time, using     the Time Zone specified by \a timeZone.      If \a date is valid and \a time is not, the time will be set to 00:00:00.      If \a timeZone is invalid then the datetime will be invalid. */
end_comment
begin_constructor
DECL|function|QDateTime
name|QDateTime
...
end_ifndef
begin_comment
comment|/*!     \since 5.2      Returns the time zone of the datetime.      If the timeSpec() is Qt::LocalTime then an instance of the current system     time zone will be returned. Note however that if you copy this time zone     the instance will not remain in sync if the system time zone changes.      \sa setTimeZone(), Qt::TimeSpec */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!     Sets the date part of this datetime to \a date. If no time is set yet, it     is set to midnight. If \a date is invalid, this QDateTime becomes invalid.      \sa date(), setTime(), setTimeSpec() */
end_comment
begin_function
DECL|function|setDate
name|void
...
end_function
begin_comment
comment|/*!     Sets the time part of this datetime to \a time. If \a time is not valid,     this function sets it to midnight. Therefore, it's possible to clear any     set time in a QDateTime by setting it to a default QTime:      \code         QDateTime dt = QDateTime::currentDateTime();         dt.setTime(QTime());     \endcode      \sa time(), setDate(), setTimeSpec() */
end_comment
begin_function
...
end_ifndef
begin_comment
comment|/*!     \since 5.2      Sets the time zone used in this datetime to \a toZone.     The datetime will refer to a different point in time.      If \a toZone is invalid then the datetime will be invalid.      \sa timeZone(), Qt::TimeSpec */
end_comment
begin_function
...
end_ifdef
begin_comment
comment|// Avoid duplicate #if-ery in uses.
end_comment
begin_define
...
name|t
parameter_list|)
value|massageAdjustedDateTime(s, d, t)
end_define
begin_else
else|#
directive|else
...
name|t
parameter_list|)
value|massageAdjustedDateTime(s, z, d, t)
end_define
begin_endif
endif|#
directive|endif
...
end_function
begin_comment
comment|/*!     \fn QDateTime QDateTime::toTimeSpec(Qt::TimeSpec spec) const      Returns a copy of this datetime converted to the given time     \a spec.      If \a spec is Qt::OffsetFromUTC then it is set to Qt::UTC.  To set to a     spec of Qt::OffsetFromUTC use toOffsetFromUtc().      If \a spec is Qt::TimeZone then it is set to Qt::LocalTime,     i.e. the local Time Zone.      Example:     \snippet code/src_corelib_tools_qdatetime.cpp 16      \sa timeSpec(), toTimeZone(), toUTC(), toLocalTime() */
end_comment
begin_function
...
end_ifndef
begin_comment
comment|/*!     \since 5.2      Returns a copy of this datetime converted to the given \a timeZone      \sa timeZone(), toTimeSpec() */
end_comment
begin_function
...
end_ifndef
begin_comment
comment|/*!     \since 5.2      Returns a datetime whose date and time are the number of \a seconds     that have passed since 1970-01-01T00:00:00, Coordinated Universal     Time (Qt::UTC) and with the given \a timeZone.      \sa toTime_t(), setTime_t() */
end_comment
begin_function
DECL|function|fromTime_t
name|QDateTime
...
end_ifndef
begin_comment
comment|/*!     \since 5.2      Returns a datetime whose date and time are the number of milliseconds \a msecs     that have passed since 1970-01-01T00:00:00.000, Coordinated Universal     Time (Qt::UTC) and with the given \a timeZone.      \sa fromTime_t() */
end_comment
begin_function
DECL|function|fromMSecsSinceEpoch
name|QDateTime
...
block|{
comment|// Qt3 had no support for reading -1, QTime() was valid and serialized as 0
return|return
name|out
operator|<<
name|quint32
...
block|{
comment|// Qt3 would write 0 for a null time
name|time
operator|.
name|mds
operator|=
...
condition|)
block|{
comment|// In 5.2 we switched to using Qt::TimeSpec and added offset support
name|dateAndTime
operator|=
name|dateTime
operator|.
...
condition|)
block|{
comment|// In Qt 5.0 we incorrectly serialised all datetimes as UTC.
...
condition|)
block|{
comment|// From 4.0 to 5.1 (except 5.0) we used QDateTimePrivate::Spec
name|dateAndTime
operator|=
name|dateTime
operator|.
...
block|{
comment|// version< QDataStream::Qt_4_0
comment|// Before 4.0 there was no TimeSpec, only Qt::LocalTime was supported
name|dateAndTime
operator|=
name|dateTime
operator|.
...
end_comment
begin_comment
comment|/*****************************************************************************   Date / Time Debug Streams *****************************************************************************/
end_comment
begin_if
if|#
directive|if
