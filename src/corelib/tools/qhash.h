...
condition|)
block|{
comment|// save 'it' across the detach:
name|int
name|bucketNum
init|=
operator|(
...
condition|)
block|{
comment|// equal keys must hash to the same value and so they all
comment|// end up in the same bucket. So we can use node->next,
comment|// which only works within a bucket, instead of (out-of-line)
comment|// QHashData::nextNode()
while|while
condition|(
name|node
operator|->
...
name|next
expr_stmt|;
comment|// 'node' may be the last node in the bucket. To produce the end iterator, we'd
comment|// need to enter the next bucket in this case, so we need to use
comment|// QHashData::nextNode() here, which, unlike node->next above, can move between
comment|// buckets.
name|node
operator|=
name|concrete
