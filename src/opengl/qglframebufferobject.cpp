...
value|const QGLContext *ctx = d_ptr->fbo_guard.context();
end_define
begin_define
...
value|const QGLContext *ctx = fbo_guard.context();
end_define
begin_ifndef
ifndef|#
directive|ifndef
...
end_function
begin_comment
comment|/*!     \since 4.8      Enables or disables mipmapping. Mipmapping is disabled by default.     If mipmapping is enabled, additional memory will be allocated for     the mipmap levels. The mipmap levels can be updated by binding the     texture and calling glGenerateMipmap(). Mipmapping cannot be enabled     for multisampled framebuffer objects.      \sa mipmap(), texture() */
end_comment
begin_function
...
name|Q_GLOBAL_STATIC
argument_list|(
argument|QGLEngineThreadStorage<QOpenGLPaintEngine>
argument_list|,
argument|qt_buffer_engine
argument_list|)
end_macro
begin_endif
endif|#
