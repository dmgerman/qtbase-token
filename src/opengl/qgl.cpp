...
operator|::
name|Type
name|preferredPaintEngine
parameter_list|()
block|{
ifdef|#
directive|ifdef
name|Q_WS_MAC
comment|// The ATI X1600 driver for Mac OS X does not support return
comment|// values from functions in GLSL. Since working around this in
comment|// the GL2 engine would require a big, ugly rewrite, we're
comment|// falling back to the GL 1 engine..
specifier|static
name|bool
name|mac_x1600_check_done
init|=
literal|false
decl_stmt|;
if|if
...
condition|)
block|{
comment|// No user-set engine - use the defaults
if|#
directive|if
name|defined
argument_list|(
...
directive|else
comment|// We can't do this in the constructor for this object because it
comment|// needs to be called *before* the QApplication constructor.
comment|// Also check for the FragmentShader extension in conjunction with
comment|// the 2.0 version flag, to cover the case where we export the display
comment|// from an old GL 1.1 server to a GL 2.x client. In that case we can't
comment|// use GL 2.0.
if|if
condition|(
operator|(
...
name|qgetenv
argument_list|(
literal|"QT_GL_USE_OPENGL1ENGINE"
argument_list|)
operator|.
name|isEmpty
argument_list|()
...
name|Q_GLOBAL_STATIC
argument_list|(
argument|QGLEngineSelector
argument_list|,
argument|qgl_engine_selector
argument_list|)
end_macro
begin_function
...
end_function
begin_comment
comment|/*!     \namespace QGL     \inmodule QtOpenGL      \brief The QGL namespace specifies miscellaneous identifiers used     in the Qt OpenGL module.      \ingroup painting-3D */
end_comment
begin_comment
...
end_function
begin_comment
comment|/*!     Sets a new default QGLFormat for the application to \a f. For     example, to set single buffering as the default instead of double     buffering, your main() might contain code like this:     \snippet doc/src/snippets/code/src_opengl_qgl.cpp 4      \sa defaultFormat() */
end_comment
begin_function
...
parameter_list|()
block|{
comment|// Clear any remaining QGLSharedResourceGuard objects on the group.
...
argument_list|)
expr_stmt|;
comment|// Make sure reference counting isn't broken
name|QGLContextPrivate
operator|::
...
argument_list|()
expr_stmt|;
comment|// clean up resources belonging to this context's group
name|d_ptr
operator|->
name|group
operator|->
...
name|Q_WS_X11
argument_list|)
comment|// Try to use texture_from_pixmap
specifier|const
name|QX11Info
modifier|*
name|xinfo
...
end_comment
begin_comment
comment|/*!     Sets a \a format for this context. The context is \link reset()     reset\endlink.      Call create() to create a new GL context that tries to match the     new format.      \snippet doc/src/snippets/code/src_opengl_qgl.cpp 7      \sa format(), reset(), create() */
end_comment
begin_function
...
directive|ifndef
name|Q_WS_QPA
comment|//We do this in choose context->setupSharing()
if|if
condition|(
name|d
operator|->
name|sharing
condition|)
comment|// ok, we managed to share
name|QGLContextGroup
operator|::
name|addShare
argument_list|(
...
condition|)
block|{
comment|// We don't have a current QThread, so just set the static.
name|QGLContext
operator|::
name|currentCtx
operator|=
...
name|context
expr_stmt|;
comment|// XXX: backwards-compat, not thread-safe
endif|#
directive|endif
block|}
end_function
...
end_comment
begin_comment
comment|/*! \fn void *QGLContext::chooseVisual()    \bold{X11 only:} This virtual function tries to find a visual that   matches the format, reducing the demands if the original request   cannot be met.    The algorithm for reducing the demands of the format is quite   simple-minded, so override this method in your subclass if your   application has spcific requirements on visual selection.    \sa chooseContext() */
end_comment
begin_comment
comment|/*! \fn void *QGLContext::tryVisual(const QGLFormat& f, int bufDepth)   \internal    \bold{X11 only:} This virtual function chooses a visual that matches   the OpenGL \link format() format\endlink. Reimplement this function   in a subclass if you need a custom visual.    \sa chooseContext() */
end_comment
begin_comment
comment|/*!     \fn void QGLContext::reset()      Resets the context and makes it invalid.      \sa create(), isValid() */
end_comment
begin_comment
comment|/*!     \fn void QGLContext::makeCurrent()      Makes this context the current OpenGL rendering context. All GL     functions you call operate on this context until another context     is made current.      In some very rare cases the underlying call may fail. If this     occurs an error message is output to stderr. */
end_comment
begin_comment
comment|/*!     \fn void QGLContext::swapBuffers() const      Swaps the screen contents with an off-screen buffer. Only works if     the context is in double buffer mode.      \sa QGLFormat::setDoubleBuffer() */
end_comment
begin_comment
...
end_comment
begin_comment
comment|/*!     \obsolete     \fn void QGLContext::generateFontDisplayLists(const QFont& font, int listBase)      Generates a set of 256 display lists for the 256 first characters     in the font \a font. The first list will start at index \a listBase.      \sa QGLWidget::renderText() */
end_comment
begin_comment
...
end_constructor
begin_comment
comment|/*!     Constructs an OpenGL widget with parent \a parent.      The \a context argument is a pointer to the QGLContext that     you wish to be bound to this widget. This allows you to pass in     your own QGLContext sub-classes.      The widget will be \link isValid() invalid\endlink if the system     has no \link QGLFormat::hasOpenGL() OpenGL support\endlink.      The \a parent and widget flag, \a f, arguments are passed     to the QWidget constructor.      If \a shareWidget is a valid QGLWidget, this widget will share     OpenGL display lists and texture objects with \a shareWidget. But     if \a shareWidget and this widget have different \l {format()}     {formats}, sharing might not be possible. You can check whether     sharing is in effect by calling isSharing().      The initialization of OpenGL rendering state, etc. should be done     by overriding the initializeGL() function, rather than in the     constructor of your QGLWidget subclass.      \sa QGLFormat::defaultFormat(), isValid() */
end_comment
begin_constructor
DECL|function|QGLWidget
name|QGLWidget
...
end_function
begin_comment
comment|/*!     \fn const QGLContext *QGLWidget::context() const      Returns the context of this widget.      It is possible that the context is not valid (see isValid()), for     example, if the underlying hardware does not support the format     attributes that were requested. */
end_comment
begin_comment
...
end_function
begin_comment
comment|/*! \fn bool QGLWidget::event(QEvent *e)   \reimp */
end_comment
begin_if
if|#
directive|if
...
condition|)
block|{
comment|// if we've reparented a window that has the current context
comment|// bound, we need to rebind that context to the new window id
if|if
condition|(
name|d
operator|->
...
argument_list|)
expr_stmt|;
comment|// ### recreating the overlay isn't supported atm
block|}
block|}
ifndef|#
directive|ifndef
name|QT_NO_EGL
comment|// A re-parent is likely to destroy the X11 window and re-create it. It is important
comment|// that we free the EGL surface _before_ the winID changes - otherwise we can leak.
if|if
condition|(
name|e
operator|->
...
condition|)
block|{
comment|// The window may have been re-created during re-parent or state change - if so, the EGL
comment|// surface will need to be re-created.
name|d
operator|->
...
condition|&& ((QSysInfo::MacintoshVersion>= QSysInfo::MV_10_5&& isWindow())             || QSysInfo::MacintoshVersion<= QSysInfo::MV_10_4)
endif|#
directive|endif
condition|)
block|{
...
name|Q_OS_SYMBIAN
argument_list|)
comment|// prevents errors on some systems, where we get a flush to a
comment|// hidden widget
if|if
condition|(
name|e
operator|->
...
expr_stmt|;
block|}
comment|// A re-parent is likely to destroy the Symbian window and re-create it. It is important
comment|// that we free the EGL surface _before_ the winID changes - otherwise we can leak.
if|if
condition|(
name|e
...
argument_list|()
decl_stmt|;
comment|// make sure we use a pixmap with the same depth/visual as the widget
if|if
condition|(
name|xinfo
operator|.
...
directive|ifdef
name|Q_WS_MAC
comment|// crash prevention on the Mac - it's unlikely to work anyway
name|fmt
operator|.
name|setSampleBuffers
argument_list|(
...
directive|ifdef
name|Q_OS_SYMBIAN
comment|// Crashes on Symbian if trying to render to invisible surfaces
if|if
condition|(
operator|!
name|isVisible
...
end_comment
begin_comment
comment|/*!     \obsolete      Returns the value of the first display list that is generated for     the characters in the given \a font. \a listBase indicates the base     value used when generating the display lists for the font. The     default value is 2000.      \note This function is not supported on OpenGL/ES systems. */
end_comment
begin_function
...
condition|)
block|{
comment|// this can't happen unless we run out of mem
return|return
literal|0
return|;
block|}
comment|// always regenerate font disp. lists for pixmaps - hw accelerated
comment|// contexts can't handle this otherwise
name|bool
name|regenerate
init|=
name|d
...
comment|// font color needs to be part of the font cache key when using
comment|// antialiased fonts since one set of glyphs needs to be generated
comment|// for each font color
name|QString
name|color_key
decl_stmt|;
if|if
...
end_function
begin_comment
comment|/*!     \internal      Returns the GL widget's paint engine. This is normally a     QOpenGLPaintEngine. */
end_comment
begin_function
...
name|match
argument_list|(
literal|"GL_ARB_fragment_program"
argument_list|)
condition|)
name|glExtensions
operator||=
...
condition|)
block|{
comment|// ARB_framebuffer_object also includes EXT_framebuffer_blit.
name|glExtensions
operator||=
name|FramebufferObject
expr_stmt|;
...
end_macro
begin_comment
comment|/*     Returns the GL extensions for the current QGLContext. If there is no     current QGLContext, a default context will be created and the extensions     for that context will be returned instead. */
end_comment
begin_function
...
name|QLatin1String
argument_list|(
literal|"/System/Library/Frameworks/OpenGL.framework/Versions/A/Libraries/libGL.dylib"
argument_list|)
return|;
else|#
directive|else
...
parameter_list|)
block|{
comment|// If there are still shares, then no cleanup to be done yet.
if|if
condition|(
name|m_shares
operator|.
...
condition|)
return|return;
comment|// Iterate over all resources and free each in turn.
name|QHash
argument_list|<
name|QGLContextGroupResourceBase
modifier|*
...
name|qWarning
argument_list|(
literal|"QGLContext::bindTexture(): The GL implementation does "
literal|"not support texture compression extensions."
argument_list|)
expr_stmt|;
return|return
name|QSize
...
name|qWarning
argument_list|(
literal|"QGLContext::bindTexture(): could not resolve "
literal|"glCompressedTexImage2DARB."
argument_list|)
expr_stmt|;
return|return
