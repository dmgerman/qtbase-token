...
argument_list|()
return|;
comment|// QOpenGLExtensions is just a subclass of QOpenGLFunctions
block|}
end_function
begin_ifndef
ifndef|#
...
literal|0
decl_stmt|;
comment|// if QOpenGLFramebufferObjects have been used in the mean-time, we've lost our cached value
if|if
condition|(
name|guiGlContextPrivate
operator|&&
...
name|printf
argument_list|(
literal|" - converted RGBA8888 -> RGBA8888_Premultiplied (%d ms) \n"
argument_list|,
name|time
operator|.
name|elapsed
...
name|printf
argument_list|(
literal|" - converted RGBA8888_Premultiplied -> RGBA8888 (%d ms) \n"
argument_list|,
name|time
operator|.
name|elapsed
...
break|break;
default|default:
comment|// Ideally more formats would be converted directly to an RGBA8888 format,
comment|// but we are only guaranteed to have a fast conversion to an ARGB format.
if|if
condition|(
name|img
operator|.
...
comment|// 32 in the switch above is for the RGB16 case, where we do
comment|// not need a byteswap.
name|Q_ASSERT
argument_list|(
name|img
operator|.
...
end_function
begin_comment
comment|/*!     \fn bool QGLContext::deviceIsPixmap() const      Returns \c true if the paint device of this context is a pixmap;     otherwise returns \c false.      Since Qt 5 the paint device is never actually a pixmap. renderPixmap() is     however still simulated using framebuffer objects and readbacks, and this     function will return \c true in this case. */
end_comment
begin_comment
...
argument_list|)
expr_stmt|;
comment|// The only option in Qt 5 is the shader-based OpenGL 2 paint engine.
comment|// Setting fixed pipeline transformations is futile. Instead, pass the
comment|// extra values directly and let the engine figure the matrices out.
cast|static_cast
argument_list|<
name|QGL2PaintEngineEx
operator|*
