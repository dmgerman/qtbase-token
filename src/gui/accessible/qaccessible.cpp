...
end_function
begin_comment
comment|/*!     \fn int QAccessibleInterface::childAt(int x, int y) const      Returns the 1-based index of the child that contains the screen     coordinates (\a x, \a y). This function returns 0 if the point is     positioned on the object itself. If the tested point is outside     the boundaries of the object this function returns -1.      This function is only relyable for visible objects (invisible     object might not be laid out correctly).      All visual objects provide this information.      \sa rect() */
end_comment
begin_comment
comment|/*!     \fn QAccessibleInterface *parent() const      Returns the QAccessibleInterface of the parent in the accessible object hierarchy.      Returns 0 if no parent exists (e.g. for the top level application object).      \sa child() */
end_comment
begin_comment
comment|/*!     \fn QAccessibleInterface *child(int index) const      Returns the accessible child with index \a index.     0-based index. The number of children of an object can be checked with childCount.      Returns 0 when asking for an invalid child (e.g. when the child became invalid in the meantime).      \sa childCount(), parent() */
end_comment
begin_comment
...
end_comment
begin_comment
comment|/*!     \fn int QAccessibleInterface::userActionCount(int child) const      Returns the number of custom actions of the object, or of the     object's child if \a child is not 0.      The \c Action type enumerates predefined actions: these     are not included in the returned value.      \sa actionText(), doAction() */
end_comment
begin_comment
comment|/*!     \fn QString QAccessibleInterface::actionText(int action, Text t, int child) const      Returns the text property \a t of the action \a action supported by     the object, or of the object's child if \a child is not 0.      \sa text(), userActionCount() */
end_comment
begin_comment
comment|/*!     \fn bool QAccessibleInterface::doAction(int action, int child, const QVariantList&params)      Asks the object, or the object's \a child if \a child is not 0, to     execute \a action using the parameters, \a params. Returns true if     the action could be executed; otherwise returns false.      \a action can be a predefined or a custom action.      \sa userActionCount(), actionText() */
end_comment
begin_comment
...
end_comment
begin_comment
comment|/*!     \fn QAccessibleEvent::QAccessibleEvent(Type type, int child)      Constructs an accessibility event of the given \a type, which     must be QEvent::AccessibilityDescription or     QEvent::AccessibilityHelp.      \a child is the (1-based) index of the child to which the request     applies. If \a child is 0, the request is for the widget itself.      \sa child() */
end_comment
begin_comment
