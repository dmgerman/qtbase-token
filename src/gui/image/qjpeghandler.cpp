...
literal|5
case|:
comment|// mirror horizontal and rotate 270 CW
return|return
name|QImageIOHandler
operator|::
...
name|qWarning
argument_list|(
literal|"Invalid EXIF orientation"
argument_list|)
expr_stmt|;
return|return
name|QImageIOHandler
...
argument_list|)
comment|// from qimage_ssse3.cpps
if|if
condition|(
name|qCpuHasFeature
argument_list|(
...
condition|)
block|{
comment|// We don't support writing EXIF headers so apply the transform to the data.
name|QImage
name|img
init|=
name|image
