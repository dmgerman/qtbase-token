...
end_function
begin_comment
comment|/*!     \fn bool QPixmap::operator!() const      Returns \c true if this is a null pixmap; otherwise returns \c false.      \sa isNull() */
end_comment
begin_comment
...
end_function
begin_comment
comment|/*!     \fn bool QPixmap::isQBitmap() const      Returns \c true if this is a QBitmap; otherwise returns \c false. */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!     \fn bool QPixmap::isNull() const      Returns \c true if this is a null pixmap; otherwise returns \c false.      A null pixmap has zero width, zero height and no contents. You     cannot draw in a null pixmap. */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!     Returns the device pixel ratio for the pixmap. This is the     ratio between \e{device pixels} and \e{device independent pixels}.      Use this function when calculating layout geometry based on     the pixmap size: QSize layoutSize = image.size() / image.devicePixelRatio()      The default value is 1.0.      \sa setDevicePixelRatio(), QImageReader */
end_comment
begin_function
...
end_comment
begin_comment
comment|/*!     Saves the pixmap to the file with the given \a fileName using the     specified image file \a format and \a quality factor. Returns \c true     if successful; otherwise returns \c false.      The \a quality factor must be in the range [0,100] or -1. Specify     0 to obtain small compressed files, 100 for large uncompressed     files, and -1 to use the default settings.      If \a format is 0, an image format will be chosen from \a fileName's     suffix.      \sa {QPixmap#Reading and Writing Image Files}{Reading and Writing     Image Files} */
end_comment
begin_function
DECL|function|save
name|bool
...
end_comment
begin_comment
comment|/*!     Returns \c true if this pixmap has an alpha channel, \e or has a     mask, otherwise returns \c false.      \sa hasAlphaChannel(), mask() */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!     Returns \c true if the pixmap has a format that respects the alpha     channel, otherwise returns \c false.      \sa hasAlpha() */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!     \fn QPixmap QPixmap::fromImage(QImage&&image, Qt::ImageConversionFlags flags)     \since 5.3     \overload      Converts the given \a image to a pixmap without copying if possible. */
end_comment
begin_comment
comment|/*!     \internal */
end_comment
