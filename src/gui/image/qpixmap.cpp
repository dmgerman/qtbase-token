...
argument_list|()
operator|<<
literal|"QPixmap::grabWidget is deprecated, use QWidget::render() instead"
expr_stmt|;
return|return
name|QPixmap
argument_list|()
...
directive|if
literal|0
block|if (!widget)         return QPixmap();      if (widget->testAttribute(Qt::WA_PendingResizeEvent) || !widget->testAttribute(Qt::WA_WState_Created))         sendResizeEvents(widget);      widget->d_func()->prepareToRender(QRegion(),         QWidget::DrawWindowBackground | QWidget::DrawChildren | QWidget::IgnoreMask);      QRect r(rect);     if (r.width()< 0)         r.setWidth(widget->width() - rect.x());     if (r.height()< 0)         r.setHeight(widget->height() - rect.y());      if (!r.intersects(widget->rect()))         return QPixmap();      QPixmap res(r.size());     if (!qt_widget_private(widget)->isOpaque)         res.fill(Qt::transparent);      widget->d_func()->render(&res, QPoint(), r, QWidget::DrawWindowBackground                              | QWidget::DrawChildren | QWidget::IgnoreMask, true);     return res;
endif|#
directive|endif
block|}
end_function
