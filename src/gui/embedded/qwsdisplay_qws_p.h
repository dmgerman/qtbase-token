begin_unit
begin_comment
comment|/**************************************************************************** ** ** Copyright (C) 2011 Nokia Corporation and/or its subsidiary(-ies). ** All rights reserved. ** Contact: Nokia Corporation (qt-info@nokia.com) ** ** This file is part of the QtGui module of the Qt Toolkit. ** ** $QT_BEGIN_LICENSE:LGPL$ ** No Commercial Usage ** This file contains pre-release code and may not be distributed. ** You may use this file in accordance with the terms and conditions ** contained in the Technology Preview License Agreement accompanying ** this package. ** ** GNU Lesser General Public License Usage ** Alternatively, this file may be used under the terms of the GNU Lesser ** General Public License version 2.1 as published by the Free Software ** Foundation and appearing in the file LICENSE.LGPL included in the ** packaging of this file.  Please review the following information to ** ensure the GNU Lesser General Public License version 2.1 requirements ** will be met: http://www.gnu.org/licenses/old-licenses/lgpl-2.1.html. ** ** In addition, as a special exception, Nokia gives you certain additional ** rights.  These rights are described in the Nokia Qt LGPL Exception ** version 1.1, included in the file LGPL_EXCEPTION.txt in this package. ** ** If you have questions regarding the use of this file, please contact ** Nokia at qt-info@nokia.com. ** ** ** ** ** ** ** ** ** $QT_END_LICENSE$ ** ****************************************************************************/
end_comment
begin_ifndef
ifndef|#
directive|ifndef
name|QWSDISPLAY_QWS_P_H
end_ifndef
begin_define
DECL|macro|QWSDISPLAY_QWS_P_H
define|#
directive|define
name|QWSDISPLAY_QWS_P_H
end_define
begin_comment
comment|//
end_comment
begin_comment
comment|//  W A R N I N G
end_comment
begin_comment
comment|//  -------------
end_comment
begin_comment
comment|//
end_comment
begin_comment
comment|// This file is not part of the Qt API.  It exists purely as an
end_comment
begin_comment
comment|// implementation detail.  This header file may change from version to
end_comment
begin_comment
comment|// version without notice, or even be removed.
end_comment
begin_comment
comment|//
end_comment
begin_comment
comment|// We mean it.
end_comment
begin_comment
comment|//
end_comment
begin_include
include|#
directive|include
file|"qwsdisplay_qws.h"
end_include
begin_include
include|#
directive|include
file|"qwssocket_qws.h"
end_include
begin_include
include|#
directive|include
file|"qwsevent_qws.h"
end_include
begin_include
include|#
directive|include
file|<private/qwssharedmemory_p.h>
end_include
begin_include
include|#
directive|include
file|"qwscommand_qws_p.h"
end_include
begin_include
include|#
directive|include
file|"qwslock_p.h"
end_include
begin_expr_stmt
name|QT_BEGIN_NAMESPACE
name|class
name|QWSDisplay
operator|::
name|Data
block|{
name|public
operator|:
name|Data
argument_list|(
argument|QObject* parent
argument_list|,
argument|bool singleProcess = false
argument_list|)
block|;
operator|~
name|Data
argument_list|()
block|;
name|void
name|flush
argument_list|()
block|;
name|bool
name|queueNotEmpty
argument_list|()
block|;
name|QWSEvent
operator|*
name|dequeue
argument_list|()
block|;
name|QWSEvent
operator|*
name|peek
argument_list|()
block|;
name|bool
name|directServerConnection
argument_list|()
block|;
name|void
name|fillQueue
argument_list|()
block|;
ifndef|#
directive|ifndef
name|QT_NO_QWS_MULTIPROCESS
name|void
name|connectToPipe
argument_list|()
block|;
name|void
name|waitForConnection
argument_list|()
block|;
name|void
name|waitForPropertyReply
argument_list|()
block|;
name|void
name|waitForRegionAck
argument_list|(
argument|int winId
argument_list|)
block|;
name|void
name|waitForRegionEvents
argument_list|(
argument|int winId
argument_list|,
argument|bool ungrabDisplay
argument_list|)
block|;
name|bool
name|hasPendingRegionEvents
argument_list|()
specifier|const
block|;
endif|#
directive|endif
name|void
name|waitForCreation
argument_list|()
block|;
ifndef|#
directive|ifndef
name|QT_NO_COP
name|void
name|waitForQCopResponse
argument_list|()
block|;
endif|#
directive|endif
name|void
name|init
argument_list|()
block|;
name|void
name|reinit
argument_list|(
specifier|const
name|QString
operator|&
name|newAppName
argument_list|)
block|;
name|void
name|create
argument_list|(
argument|int n =
literal|1
argument_list|)
block|;
name|void
name|flushCommands
argument_list|()
block|;
name|void
name|sendCommand
argument_list|(
name|QWSCommand
operator|&
name|cmd
argument_list|)
block|;
name|void
name|sendSynchronousCommand
argument_list|(
name|QWSCommand
operator|&
name|cmd
argument_list|)
block|;
name|QWSEvent
operator|*
name|readMore
argument_list|()
block|;
name|int
name|takeId
argument_list|()
block|;
name|void
name|setMouseFilter
argument_list|(
name|void
argument_list|(
operator|*
name|filter
argument_list|)
argument_list|(
name|QWSMouseEvent
operator|*
argument_list|)
argument_list|)
block|;
comment|//####public data members
comment|//    QWSRegionManager *rgnMan;
name|uchar
operator|*
name|sharedRam
block|;
ifndef|#
directive|ifndef
name|QT_NO_QWS_MULTIPROCESS
name|QWSSharedMemory
name|shm
block|;
endif|#
directive|endif
name|int
name|sharedRamSize
block|;
ifndef|#
directive|ifndef
name|QT_NO_QWS_MULTIPROCESS
specifier|static
name|QWSLock
operator|*
name|clientLock
block|;
specifier|static
name|bool
name|lockClient
argument_list|(
argument|QWSLock::LockType
argument_list|,
argument|int timeout = -
literal|1
argument_list|)
block|;
specifier|static
name|void
name|unlockClient
argument_list|(
name|QWSLock
operator|::
name|LockType
argument_list|)
block|;
specifier|static
name|bool
name|waitClient
argument_list|(
argument|QWSLock::LockType
argument_list|,
argument|int timeout = -
literal|1
argument_list|)
block|;
specifier|static
name|QWSLock
operator|*
name|getClientLock
argument_list|()
block|;
endif|#
directive|endif
comment|// QT_NO_QWS_MULTIPROCESS
name|private
operator|:
ifndef|#
directive|ifndef
name|QT_NO_QWS_MULTIPROCESS
name|QWSSocket
operator|*
name|csocket
block|;
endif|#
directive|endif
name|QList
operator|<
name|QWSEvent
operator|*
operator|>
name|queue
block|;
if|#
directive|if
literal|0
block|void debugQueue() {             for (int i = 0; i< queue.size(); ++i) {                 QWSEvent *e = queue.at(i);                 qDebug( "   ev %d type %d sl %d rl %d", i, e->type, e->simpleLen, e->rawLen);             }     }
endif|#
directive|endif
name|QWSConnectedEvent
operator|*
name|connected_event
block|;
name|QWSMouseEvent
operator|*
name|mouse_event
block|;
name|int
name|region_events_count
block|;
name|int
name|mouse_state
block|;
name|int
name|mouse_winid
block|;
name|QPoint
name|region_offset
block|;
name|int
name|region_offset_window
block|;
ifndef|#
directive|ifndef
name|QT_NO_COP
name|QWSQCopMessageEvent
operator|*
name|qcop_response
block|;
endif|#
directive|endif
name|QWSEvent
operator|*
name|current_event
block|;
name|QList
operator|<
name|int
operator|>
name|unused_identifiers
block|;
ifdef|#
directive|ifdef
name|QAPPLICATION_EXTRA_DEBUG
name|int
name|mouse_event_count
block|;
endif|#
directive|endif
name|void
argument_list|(
operator|*
name|mouseFilter
argument_list|)
argument_list|(
name|QWSMouseEvent
operator|*
argument_list|)
block|;      enum
block|{
name|VariableEvent
operator|=
operator|-
literal|1
block|}
block|;  }
expr_stmt|;
end_expr_stmt
begin_macro
name|QT_END_NAMESPACE
end_macro
begin_endif
endif|#
directive|endif
end_endif
begin_comment
comment|// QWSDISPLAY_QWS_P_H
end_comment
end_unit
