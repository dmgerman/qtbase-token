...
end_comment
begin_comment
comment|/*!     \fn void QHeaderView::sectionPressed(int logicalIndex)      This signal is emitted when a section is pressed. The section's logical     index is specified by \a logicalIndex.      \sa setClickable() */
end_comment
begin_comment
comment|/*!     \fn void QHeaderView::sectionClicked(int logicalIndex)      This signal is emitted when a section is clicked. The section's logical     index is specified by \a logicalIndex.      Note that the sectionPressed signal will also be emitted.      \sa setClickable(), sectionPressed() */
end_comment
begin_comment
comment|/*!     \fn void QHeaderView::sectionEntered(int logicalIndex)     \since 4.3      This signal is emitted when the cursor moves over the section and the left     mouse button is pressed. The section's logical index is specified by     \a logicalIndex.      \sa setClickable(), sectionPressed() */
end_comment
begin_comment
comment|/*!     \fn void QHeaderView::sectionDoubleClicked(int logicalIndex)      This signal is emitted when a section is double-clicked. The section's     logical index is specified by \a logicalIndex.      \sa setClickable() */
end_comment
begin_comment
...
end_comment
begin_comment
comment|/*!     \fn void QHeaderView::sectionHandleDoubleClicked(int logicalIndex)      This signal is emitted when a section is double-clicked. The section's     logical index is specified by \a logicalIndex.      \sa setClickable() */
end_comment
begin_comment
...
end_comment
begin_comment
comment|/*!     \fn void QHeaderView::sectionAutoResize(int logicalIndex,     QHeaderView::ResizeMode mode)      This signal is emitted when a section is automatically resized. The     section's logical index is specified by \a logicalIndex, and the resize     mode by \a mode.      \sa setResizeMode(), stretchLastSection() */
end_comment
begin_comment
comment|// ### Qt 5: change to sectionAutoResized()
end_comment
begin_comment
...
end_function
begin_comment
comment|/*!     Returns the length along the orientation of the header.      \sa sizeHint(), setResizeMode(), offset() */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!     Returns the width (or height for vertical headers) of the given     \a logicalIndex.      \sa length(), setResizeMode(), defaultSectionSize() */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!     If \a movable is true, the header may be moved by the user; otherwise it     is fixed in place.      \sa isMovable(), sectionMoved() */
end_comment
begin_comment
comment|// ### Qt 5: change to setSectionsMovable()
end_comment
begin_function
...
end_function
begin_comment
comment|/*!     Returns true if the header can be moved by the user; otherwise returns     false.      \sa setMovable() */
end_comment
begin_comment
comment|// ### Qt 5: change to sectionsMovable()
end_comment
begin_function
...
end_function
begin_comment
comment|/*!     If \a clickable is true, the header will respond to single clicks.      \sa isClickable(), sectionClicked(), sectionPressed(),     setSortIndicatorShown() */
end_comment
begin_comment
comment|// ### Qt 5: change to setSectionsClickable()
end_comment
begin_function
...
end_function
begin_comment
comment|/*!     Returns true if the header is clickable; otherwise returns false. A     clickable header could be set up to allow the user to change the     representation of the data in the view related to the header.      \sa setClickable() */
end_comment
begin_comment
comment|// ### Qt 5: change to sectionsClickable()
end_comment
begin_function
...
end_function
begin_comment
comment|/*!     Sets the constraints on how the header can be resized to those described     by the given \a mode.      \sa resizeMode(), length(), sectionResized(), sectionAutoResize() */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!     \overload      Sets the constraints on how the section specified by \a logicalIndex in     the header can be resized to those described by the given \a mode. The logical     index should exist at the time this function is called.      \note This setting will be ignored for the last section if the stretchLastSection     property is set to true. This is the default for the horizontal headers provided     by QTreeView.      \sa setStretchLastSection() */
end_comment
begin_comment
comment|// ### Qt 5: change to setSectionResizeMode()
end_comment
begin_function
DECL|function|setResizeMode
name|void
...
end_function
begin_comment
comment|/*!     Returns the resize mode that applies to the section specified by the given     \a logicalIndex.      \sa setResizeMode() */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!   \property QHeaderView::showSortIndicator   \brief whether the sort indicator is shown    By default, this property is false.    \sa setClickable() */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!     \property QHeaderView::stretchLastSection     \brief whether the last visible section in the header takes up all the     available space      The default value is false.      \note The horizontal headers provided by QTreeView are configured with this     property set to true, ensuring that the view does not waste any of the     space assigned to it for its header. If this value is set to true, this     property will override the resize mode set on the last section in the     header.      \sa setResizeMode() */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!     \property QHeaderView::defaultSectionSize     \brief the default size of the header sections before resizing.      This property only affects sections that have \l Interactive or \l Fixed     as their resize mode.      \sa setResizeMode() minimumSectionSize */
end_comment
begin_function
...
condition|)
block|{
comment|// add the new sections to an existing span
name|int
name|insertLength
init|=
name|d
...
block|{
comment|// separate them out into their own span
name|int
name|insertLength
init|=
name|d
...
expr_stmt|;
block|}
comment|// ### handle sectionSelection, sectionHidden
block|}
name|sectionCount
operator|-=
name|changeCount
...
parameter_list|)
block|{
comment|// ### the code for merging spans does not merge at all opertuneties
comment|// ### what if the number of sections is reduced ?
name|SectionSpan
name|span
argument_list|(
name|size
...
condition|)
block|{
comment|// the existing span is entirely coveded by the new span
name|spansToRemove
operator|.
name|append
argument_list|(
...
condition|)
block|{
comment|// the new span is covered by an existin span
name|length
operator|-=
name|sectionSpans
operator|.
...
name|mode
expr_stmt|;
comment|// ### check if we can merge the section with any of its neighbours
name|removeSpans
argument_list|(
name|spansToRemove
argument_list|)
...
condition|)
block|{
comment|// the new span covers the last part of the existing span
name|length
operator|-=
name|sectionSpans
operator|.
...
condition|)
block|{
comment|// the new span covers the first part of the existing span
name|length
operator|-=
name|sectionSpans
operator|.
...
expr_stmt|;
block|}
comment|// ### adding and removing _ sections_  in addition to spans
comment|// ### add some more checks here
if|if
condition|(
name|spansToRemove
operator|.
...
argument_list|)
expr_stmt|;
comment|//Q_ASSERT(initial_section_count == headerSectionCount());
block|}
block|}
end_function
begin_function
...
condition|)
block|{
comment|// the change covers the entire span
name|spansToRemove
operator|.
name|append
argument_list|(
...
condition|)
block|{
comment|// all the removed sections are inside the span
name|int
name|change
init|=
operator|(
...
condition|)
block|{
comment|// the some of the removed sections are inside the span,at the end
name|int
name|change
init|=
name|qMin
...
operator|)
expr_stmt|;
comment|// the change affects several spans
block|}
elseif|else
if|if
condition|(
...
condition|)
block|{
comment|// the some of the removed sections are inside the span, at the beginning
name|int
name|change
init|=
name|qMin
