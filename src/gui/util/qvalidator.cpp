...
name|origRe
decl_stmt|;
comment|// the one set by the user
DECL|member|usedRe
name|QRegularExpression
name|usedRe
decl_stmt|;
comment|// the one actually used
name|void
...
argument_list|)
block|{
comment|// origRe in the private will be an empty QRegularExpression,
comment|// and therefore this validator will match any string.
block|}
end_constructor
begin_comment
comment|/*!     Constructs a validator with a \a parent object that     accepts all strings that match the regular expression \a re. */
end_comment
begin_constructor
DECL|function|QRegularExpressionValidator
name|QRegularExpressionValidator
...
argument_list|)
expr_stmt|;
comment|// We want a validator with an empty QRegularExpression to match anything;
comment|// since we're going to do an exact match (by using d->usedRe), first check if the rx is empty
comment|// (and, if so, accept the input).
if|if
condition|(
name|d
operator|->
...
end_function
begin_comment
comment|/*!     \property QRegularExpressionValidator::regularExpression     \brief the regular expression used for validation      By default, this property contains a regular expression with an empty     pattern (which therefore matches any string). */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!     \internal      Sets \a re as the regular expression. It wraps the regexp that's actually used     between \\A and \\z, therefore forcing an exact match. */
end_comment
begin_function
DECL|function|setRegularExpression
name|void
...
name|re
expr_stmt|;
comment|// copies also the pattern options
name|usedRe
operator|.
name|setPattern
argument_list|(
