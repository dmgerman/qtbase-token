...
end_function
begin_comment
comment|/*!   \class QWindowsVistaStyle   \brief The QWindowsVistaStyle class provides a look and feel suitable for applications on Microsoft Windows Vista.   \since 4.3   \ingroup appearance    \warning This style is only available on the Windows Vista platform   because it makes use of Windows Vista's style engine.    \sa QMacStyle, QWindowsXPStyle, QPlastiqueStyle, QCleanlooksStyle, QMotifStyle */
end_comment
begin_comment
...
end_function
begin_comment
comment|/*! \internal    Helperfunction to paint the current transition state between two   animation frames.    The result is a blended image consisting of ((alpha)*_primaryImage)   + ((1-alpha)*_secondaryImage)  */
end_comment
begin_function
...
end_function
begin_comment
comment|/*! \internal   Paints a transition state. The result will be a mix between the   initial and final state of the transition, depending on the time   difference between _startTime and current time. */
end_comment
begin_function
DECL|function|paint
name|void
...
end_function
begin_comment
comment|/*! \internal   Paints a pulse. The result will be a mix between the primary and   secondary pulse images depending on the time difference between   _startTime and current time. */
end_comment
begin_function
DECL|function|paint
name|void
...
condition|)
block|{
comment|/* all widgets that supports state transitions : */
if|if
condition|(
ifndef|#
directive|ifndef
...
argument_list|)
expr_stmt|;
comment|// Note that the widget pointer is intentionally 0
else|else
comment|// this ensures that we do not recurse in the animation logic above
name|anim
operator|->
name|paint
argument_list|(
...
condition|)
block|{
comment|//Combo boxes are special cased to avoid cleartype issues
name|startImage
operator|.
name|fill
argument_list|(
...
condition|)
block|{
comment|//This is a workaround for the direct3d engine as it currently has some issues with grabWindow
name|startImage
operator|.
name|fill
argument_list|(
...
end_function
begin_comment
comment|/*  * We need to set the windows explorer theme explicitly on a native widget  * in order to get Vista-style item view themes  */
end_comment
begin_function
