...
end_function
begin_comment
comment|/*!     Creates a QOpenGLFramebufferObjectFormat object for specifying     the format of an OpenGL framebuffer object.      By default the format specifies a non-multisample framebuffer object with no     depth/stencil attachments, texture target \c GL_TEXTURE_2D, and internal format \c GL_RGBA8.     On OpenGL/ES systems, the default internal format is \c GL_RGBA.      \sa samples(), attachment(), internalTextureFormat() */
end_comment
begin_constructor
...
end_function
begin_comment
comment|/*!     Returns \c true if mipmapping is enabled.      \sa setMipmap() */
end_comment
begin_function
...
condition|)
return|return;
comment|// Fall back to using a normal non-msaa FBO if we don't have support for MSAA
if|if
condition|(
operator|!
name|funcs
...
name|internalFormat
decl_stmt|;
comment|// ES requires a sized format. The older desktop extension does not. Correct the format on ES.
if|if
condition|(
name|ctx
operator|->
...
name|qWarning
argument_list|(
literal|"Multiple render targets not supported, ignoring extra color attachment request"
argument_list|)
expr_stmt|;
return|return;
block|}
...
end_function
begin_comment
comment|/*! \overload      Creates and attaches an additional texture or renderbuffer of size \a width and \a height.      When \a internalFormat is not \c 0, it specifies the internal format of the texture or     renderbuffer. Otherwise a default of GL_RGBA or GL_RGBA8 is used.      \since 5.6  */
end_comment
begin_function
DECL|function|addColorAttachment
name|void
...
end_function
begin_comment
comment|/*!     \fn bool QOpenGLFramebufferObject::bind()      Switches rendering from the default, windowing system provided     framebuffer to this framebuffer object.     Returns \c true upon success, false otherwise.      \note If takeTexture() was called, a new texture is created and associated     with the framebuffer object. This is potentially expensive and changes the     context state (the currently bound texture).      \sa release() */
end_comment
begin_function
DECL|function|bind
name|bool
...
condition|)
block|{
comment|// Create new textures to replace the ones stolen via takeTexture().
for|for
control|(
name|int
name|i
...
end_function
begin_comment
comment|/*!     \fn bool QOpenGLFramebufferObject::release()      Switches rendering back to the default, windowing system provided     framebuffer.     Returns \c true upon success, false otherwise.      \sa bind() */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!     \fn GLuint QOpenGLFramebufferObject::texture() const      Returns the texture id for the texture attached as the default     rendering target in this framebuffer object. This texture id can     be bound as a normal texture in your own OpenGL code.      If a multisample framebuffer object is used then the value returned     from this function will be invalid.      When multiple textures are attached, the return value is the ID of     the first one.      \sa takeTexture(), textures() */
end_comment
begin_function
...
end_function
begin_comment
comment|/*! \overload     Returns the texture id for the texture attached to the color attachment of    index \a colorAttachmentIndex of this framebuffer object. The ownership of    the texture is transferred to the caller.     When \a colorAttachmentIndex is \c 0, the behavior is identical to the    parameter-less variant of this function.     If the framebuffer object is currently bound, an implicit release()    will be done. During the next call to bind() a new texture will be    created.     If a multisample framebuffer object is used, then there is no    texture and the return value from this function will be invalid.    Similarly, incomplete framebuffer objects will also return 0.     \since 5.6  */
end_comment
begin_function
DECL|function|takeTexture
name|GLuint
...
end_function
begin_comment
comment|/*!     \return the size of the color and depth/stencil attachments attached to     this framebuffer object. */
end_comment
begin_function
DECL|function|size
name|QSize
...
end_function
begin_comment
comment|/*!     \return the sizes of all color attachments attached to this framebuffer     object.      \since 5.6 */
end_comment
begin_function
DECL|function|sizes
name|QVector
...
comment|// Without GL_UNSIGNED_INT_8_8_8_8_REV, GL_BGRA only makes sense on little endian.
specifier|const
name|bool
name|has_bgra_ext
init|=
...
argument_list|)
decl_stmt|;
comment|// Blacklist PowerVR Rogue G6200 as it has problems with its BGRA support.
specifier|const
name|bool
name|blackListed
init|=
...
parameter_list|)
block|{
comment|// We assume OpenGL 1.2+ or ES 3.0+ here.
name|QImage
name|img
argument_list|(
name|size
...
end_function
begin_comment
comment|/*!     \fn QImage QOpenGLFramebufferObject::toImage() const     \overload      Returns the contents of this framebuffer object as a QImage. This method flips     the image from OpenGL coordinates to raster coordinates. */
end_comment
begin_comment
comment|// ### Qt 6: Remove this method and make it a default argument instead.
end_comment
begin_function
DECL|function|toImage
name|QImage
...
end_function
begin_comment
comment|/*! \overload      Returns the contents of the color attachment of index \a     colorAttachmentIndex of this framebuffer object as a QImage. This method     flips the image from OpenGL coordinates to raster coordinates when \a     flipped is set to \c true.      \note This overload is only fully functional when multiple render targets are     supported by the OpenGL implementation. When that is not the case, only one     color attachment will be set up.      \since 5.6 */
end_comment
begin_function
DECL|function|toImage
name|QImage
...
name|qWarning
argument_list|(
literal|"QOpenGLFramebufferObject::toImage() called without a current context"
argument_list|)
expr_stmt|;
return|return
name|QImage
...
name|qWarning
argument_list|(
literal|"QOpenGLFramebufferObject::toImage() called for missing color attachment"
argument_list|)
expr_stmt|;
return|return
name|QImage
...
end_function
begin_comment
comment|/*!     \fn bool QOpenGLFramebufferObject::bindDefault()      Switches rendering back to the default, windowing system provided     framebuffer.     Returns \c true upon success, false otherwise.      \sa bind(), release() */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!     \fn bool QOpenGLFramebufferObject::hasOpenGLFramebufferObjects()      Returns \c true if the OpenGL \c{GL_EXT_framebuffer_object} extension     is present on this system; otherwise returns \c false. */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!     Returns \c true if the framebuffer object is currently bound to the current context,     otherwise false is returned. */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!     \fn bool QOpenGLFramebufferObject::hasOpenGLFramebufferBlit()      Returns \c true if the OpenGL \c{GL_EXT_framebuffer_blit} extension     is present on this system; otherwise returns \c false.      \sa blitFramebuffer() */
end_comment
begin_function
...
argument_list|)
expr_stmt|;
comment|// sets both READ and DRAW
break|break;
case|case
...
end_function
begin_comment
comment|/*!    \overload      Convenience overload to blit between two framebuffer objects and     to restore the previous framebuffer binding. Equivalent to calling     blitFramebuffer(target, targetRect, source, sourceRect, buffers, filter,     readColorAttachmentIndex, drawColorAttachmentIndex,     RestoreFrameBufferBinding). */
end_comment
begin_function
DECL|function|blitFramebuffer
name|void
