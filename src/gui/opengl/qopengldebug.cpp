...
end_comment
begin_comment
comment|// When using OpenGL ES 2.0, all the necessary GL_KHR_debug constants are
end_comment
begin_comment
comment|// provided in qopengles2ext.h. Unfortunately, newer versions of that file
end_comment
begin_comment
comment|// suffix everything with _KHR which causes extra headache when the goal is
end_comment
begin_comment
comment|// to have a single piece of code that builds in all our target
end_comment
begin_comment
comment|// environments. Therefore, try to detect this and use our custom defines
end_comment
begin_comment
comment|// instead, which we anyway need for OS X.
end_comment
begin_if
if|#
directive|if
...
argument_list|)
expr_stmt|;
comment|// Windows' Desktop GL doesn't allow resolution of "basic GL entry points"
comment|// through wglGetProcAddress
if|#
directive|if
name|defined
argument_list|(
