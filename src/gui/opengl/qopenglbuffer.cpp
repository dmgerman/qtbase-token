...
end_function
begin_comment
comment|/*!     Maps the range specified by \a offset and \a count of the contents     of this buffer into the application's memory space and returns a     pointer to it.  Returns null if memory mapping is not possible.     The \a access parameter specifies a combination of access flags.      It is assumed that create() has been called on this buffer and that     it has been bound to the current context.      \note This function is not available on OpenGL ES 2.0 and earlier.      \sa unmap(), create(), bind()  */
end_comment
begin_function
...
name|qWarning
argument_list|(
literal|"QOpenGLBuffer::mapRange(): buffer not created"
argument_list|)
expr_stmt|;
endif|#
directive|endif
...
end_function
begin_comment
comment|/*!     Unmaps the buffer after it was mapped into the application's     memory space with a previous call to map().  Returns \c true if     the unmap succeeded; false otherwise.      It is assumed that this buffer has been bound to the current context,     and that it was previously mapped with map().      \note This function is only supported under OpenGL ES 2.0 and     earlier if the \c{GL_OES_mapbuffer} extension is present.      \sa map() */
end_comment
begin_function
DECL|function|unmap
name|bool
