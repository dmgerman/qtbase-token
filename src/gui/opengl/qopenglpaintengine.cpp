...
include|#
directive|include
file|<private/qtriangulator_p.h>
end_include
begin_include
include|#
directive|include
...
specifier|extern
name|bool
name|qt_applefontsmoothing_enabled
decl_stmt|;
end_decl_stmt
begin_endif
endif|#
...
argument_list|)
expr_stmt|;
comment|//     qDebug("QOpenGL2PaintEngineExPrivate::updateBrushTexture()");
name|Qt
operator|::
name|BrushStyle
name|style
...
argument_list|)
expr_stmt|;
comment|//ctx->d_func()->bindTexture(texImage, GL_TEXTURE_2D, GL_RGBA, QOpenGLContext::InternalBindOption);
name|updateTextureFilter
argument_list|(
name|GL_TEXTURE_2D
argument_list|,
...
literal|0
decl_stmt|;
comment|//ctx->d_func()->bindTexture(currentBrushPixmap, GL_TEXTURE_2D, GL_RGBA,
comment|//                     QOpenGLContext::InternalBindOption |
comment|//                     QOpenGLContext::CanFlipNativePixmapBindOption);
name|updateTextureFilter
argument_list|(
name|GL_TEXTURE_2D
argument_list|,
...
parameter_list|()
block|{
comment|//     qDebug("QOpenGL2PaintEngineExPrivate::updateBrushUniforms()");
name|Qt
operator|::
name|BrushStyle
name|style
...
name|qWarning
argument_list|(
literal|"QOpenGL2PaintEngineEx: Unimplemented fill style"
argument_list|)
expr_stmt|;
specifier|const
name|QPointF
...
parameter_list|()
block|{
comment|//     qDebug("QOpenGL2PaintEngineExPrivate::updateMatrix()");
specifier|const
name|QTransform
modifier|&
name|transform
...
comment|//     qDebug()<< "QOpenGL2PaintEngineExPrivate::updateCompositionMode() - Setting GL composition mode for "<< q->state()->composition_mode;
switch|switch
condition|(
name|q
operator|->
...
struct|struct
name|QOpenGL2PEVectorPathCache
block|{
ifdef|#
directive|ifdef
...
parameter_list|()
block|{
comment|//    qDebug("QOpenGL2PaintEngineEx::opacityChanged()");
name|Q_D
argument_list|(
name|QOpenGL2PaintEngineEx
argument_list|)
...
parameter_list|()
block|{
comment|//     qDebug("QOpenGL2PaintEngineEx::compositionModeChanged()");
name|Q_D
argument_list|(
name|QOpenGL2PaintEngineEx
argument_list|)
...
literal|true
expr_stmt|;
comment|//    qDebug("QOpenGL2PaintEngineEx::renderHintsChanged() not implemented!");
block|}
end_function
begin_function
...
literal|0
decl_stmt|;
comment|//        ctx->d_func()->bindTexture(pixmap, GL_TEXTURE_2D, GL_RGBA, bindOptions);
name|GLfloat
name|top
init|=
name|texture
...
literal|0
decl_stmt|;
comment|//ctx->d_func()->bindTexture(image, GL_TEXTURE_2D, GL_RGBA, bindOptions);
name|GLuint
name|id
init|=
name|texture
...
specifier|static
name|bool
name|fontSmoothingApproximately
parameter_list|(
name|qreal
name|target
parameter_list|)
...
argument_list|)
expr_stmt|;
comment|// Text always causes src pixels to be transparent
comment|// prepareForDraw() have set the opacity on the current shader, so the opacity state can now be reset.
if|if
condition|(
name|compMode
operator|==
...
literal|0
decl_stmt|;
comment|//ctx->d_func()->bindTexture(pixmap, GL_TEXTURE_2D, GL_RGBA,
comment|//                     QOpenGLContext::InternalBindOption
comment|//                     | QOpenGLContext::CanFlipNativePixmapBindOption);
if|if
condition|(
...
parameter_list|)
block|{
comment|//     qDebug("QOpenGL2PaintEngineEx::clip()");
name|Q_D
argument_list|(
name|QOpenGL2PaintEngineEx
argument_list|)
...
condition|)
block|{
comment|//QWidgetPrivate *widgetPrivate = qt_widget_private(static_cast<QWidget *>(currentClipDevice)->window());
comment|//useSystemClip = widgetPrivate->extra&& widgetPrivate->extra->inRenderWithPainter;
name|useSystemClip
operator|=
literal|true
expr_stmt|;
...
parameter_list|)
block|{
comment|//     qDebug("QOpenGL2PaintEngineEx::setState()");
name|Q_D
argument_list|(
name|QOpenGL2PaintEngineEx
argument_list|)
