...
parameter_list|)
block|{
comment|// There is no way to allocate mutable storage for compressed textures in in
comment|// versions older than OpenGL 3.1 and OpenGL ES 3.0, because the older specs
comment|// do not mandate accepting null data pointers for glCompressedTexImage*D,
comment|// unlike glTexImage*D (which in turn does not accept compressed formats).
if|if
condition|(
name|isCompressedFormat
argument_list|(
...
name|qWarning
argument_list|(
literal|"Cannot set compressed data for non-compressed format 0x%x"
argument_list|,
name|format
argument_list|)
expr_stmt|;
...
argument_list|()
decl_stmt|;
comment|// was allocateStorage() a no-op?
switch|switch
condition|(
name|target
condition|)
...
end_function
begin_comment
comment|/*!     If \a enabled is \c true, enables automatic mipmap generation for this texture object     to occur whenever the level 0 mipmap data is set via setData().      The automatic mipmap generation is enabled by default.      \note Mipmap generation is not supported for compressed textures with OpenGL ES 2.0.      \sa isAutoMipMapGenerationEnabled(), generateMipMaps() */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!     Generates mipmaps for this texture object from mipmap level 0. If you are     using a texture target and filtering option that requires mipmaps and you     have disabled automatic mipmap generation then you need to call this function     or the overload to create the mipmap chain.      \note Mipmap generation is not supported for compressed textures with OpenGL     ES 2.0.      \sa setAutoMipMapGenerationEnabled(), setMipLevels(), mipLevels() */
end_comment
begin_function
