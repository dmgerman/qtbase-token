...
condition|)
block|{
comment|// deleting the base class is ok, as the derived classes don't have a destructor
operator|delete
name|backends
index|[
name|i
...
name|X
parameter_list|)
value|QOpenGLVersionFunctionsBackend::OpenGL_##X
define|#
directive|define
name|CREATE_BACKEND
parameter_list|(
...
parameter_list|)
define|\
value|case VERSION_ENUM(X): \         backends[VERSION_ENUM(X)] = new QOpenGLFunctions_##X##Backend(context); \         break;
name|QT_OPENGL_VERSIONS
argument_list|(
argument|CREATE_BACKEND
argument_list|)
case|case
name|QOpenGLVersionFunctionsBackend
operator|::
...
expr_stmt|;
block|}
comment|// the storage keeps one ref
operator|++
name|backends
index|[
name|v
...
name|QT_OPENGL_IMPLEMENT
argument_list|(
argument|QOpenGLFunctions_1_4_CoreBackend
argument_list|,
argument|QT_OPENGL_1_4_FUNCTIONS
argument_list|)
end_macro
begin_macro
name|QT_OPENGL_IMPLEMENT
