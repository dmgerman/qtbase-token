...
struct|struct
name|QVersionDirectivePosition
block|{
...
argument_list|)
decl_stmt|;
comment|// According to the GLSL spec the #version directive must not be
comment|// preceded by anything but whitespace and comments.
comment|// In order to not get confused by #version directives within a
comment|// multiline comment, we need to run a minimal preprocessor first.
enum|enum
block|{
name|Normal
block|,
name|CommentStarting
block|,
...
block|}
block|}
comment|// Search for #version directive
name|int
name|splitPosition
init|=
literal|0
...
argument_list|)
expr_stmt|;
comment|// This method breaks the shader code into two parts:
comment|// 1. Up to and including an optional #version directive.
comment|// 2. The rest.
comment|// If a #version directive exists, qualifierDefines and redefineHighp
comment|// are inserted after. Otherwise they are inserted right at the start.
comment|// In both cases a #line directive is appended in order to compensate
comment|// for line number changes in case of compiler errors.
if|if
condition|(
name|d
operator|->
...
condition|)
block|{
comment|// Append source up to #version directive
name|sourceChunks
operator|.
name|append
argument_list|(
...
name|qWarning
argument_list|(
literal|"QOpenGLShader::link[%ls]: %ls"
argument_list|,
name|qUtf16Printable
argument_list|(
name|name
