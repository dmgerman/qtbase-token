...
end_macro
begin_comment
comment|/*!     \class QOpenGLShaderProgram     \brief The QOpenGLShaderProgram class allows OpenGL shader programs to be linked and used.     \since 5.0     \ingroup painting-3D      \section1 Introduction      This class supports shader programs written in the OpenGL Shading     Language (GLSL) and in the OpenGL/ES Shading Language (GLSL/ES).      QOpenGLShader and QOpenGLShaderProgram shelter the programmer from the details of     compiling and linking vertex and fragment shaders.      The following example creates a vertex shader program using the     supplied source \c{code}.  Once compiled and linked, the shader     program is activated in the current QOpenGLContext by calling     QOpenGLShaderProgram::bind():      \snippet doc/src/snippets/code/src_gui_qopenglshaderprogram.cpp 0      \section1 Writing portable shaders      Shader programs can be difficult to reuse across OpenGL implementations     because of varying levels of support for standard vertex attributes and     uniform variables.  In particular, GLSL/ES lacks all of the     standard variables that are present on desktop OpenGL systems:     \c{gl_Vertex}, \c{gl_Normal}, \c{gl_Color}, and so on.  Desktop OpenGL     lacks the variable qualifiers \c{highp}, \c{mediump}, and \c{lowp}.      The QOpenGLShaderProgram class makes the process of writing portable shaders     easier by prefixing all shader programs with the following lines on     desktop OpenGL:      \code     #define highp     #define mediump     #define lowp     \endcode      This makes it possible to run most GLSL/ES shader programs     on desktop systems.  The programmer should restrict themselves     to just features that are present in GLSL/ES, and avoid     standard variable names that only work on the desktop.      \section1 Simple shader example      \snippet doc/src/snippets/code/src_gui_qopenglshaderprogram.cpp 1      With the above shader program active, we can draw a green triangle     as follows:      \snippet doc/src/snippets/code/src_gui_qopenglshaderprogram.cpp 2      \section1 Binary shaders and programs      Binary shaders may be specified using \c{glShaderBinary()} on     the return value from QOpenGLShader::shaderId().  The QOpenGLShader instance     containing the binary can then be added to the shader program with     addShader() and linked in the usual fashion with link().      Binary programs may be specified using \c{glProgramBinaryOES()}     on the return value from programId().  Then the application should     call link(), which will notice that the program has already been     specified and linked, allowing other operations to be performed     on the shader program.      \sa QOpenGLShader */
end_comment
begin_comment
...
directive|if
literal|0
if|else if (shaderType == QOpenGLShader::Geometry)         shader = glfuncs->glCreateShader(GL_GEOMETRY_SHADER_EXT);
endif|#
directive|endif
else|else
name|shader
...
directive|if
literal|0
block|if (glProgramParameteriEXT) {         foreach (QOpenGLShader *shader, d->shaders) {             if (shader->shaderType()& QOpenGLShader::Geometry) {                 glProgramParameteriEXT(program, GL_GEOMETRY_INPUT_TYPE_EXT,                                        d->geometryInputType);                 glProgramParameteriEXT(program, GL_GEOMETRY_OUTPUT_TYPE_EXT,                                        d->geometryOutputType);                 glProgramParameteriEXT(program, GL_GEOMETRY_VERTICES_OUT_EXT,                                        d->geometryVertexCount);                 break;             }         }     }
endif|#
directive|endif
name|d
operator|->
...
end_function
begin_comment
comment|// We have to repack matrices from qreal to GLfloat.
end_comment
begin_define
DECL|macro|setUniformMatrix
define|#
directive|define
name|setUniformMatrix
parameter_list|(
name|func
parameter_list|,
name|location
...
parameter_list|)
define|\
value|if (location == -1) \         return; \     if (sizeof(qreal) == sizeof(GLfloat)) { \         func(location, 1, GL_FALSE, \              reinterpret_cast<const GLfloat *>(value.constData())); \     } else { \         GLfloat mat[cols * rows]; \         const qreal *data = value.constData(); \         for (int i = 0; i< cols * rows; ++i) \             mat[i] = data[i]; \         func(location, 1, GL_FALSE, mat); \     }
end_define
begin_define
DECL|macro|setUniformGenericMatrix
define|#
directive|define
name|setUniformGenericMatrix
parameter_list|(
name|colfunc
parameter_list|,
name|location
...
parameter_list|)
define|\
value|if (location == -1) \         return; \     if (sizeof(qreal) == sizeof(GLfloat)) { \         const GLfloat *data = reinterpret_cast<const GLfloat *> \             (value.constData());  \         colfunc(location, cols, data); \     } else { \         GLfloat mat[cols * rows]; \         const qreal *data = value.constData(); \         for (int i = 0; i< cols * rows; ++i) \             mat[i] = data[i]; \         colfunc(location, cols, mat); \     }
end_define
begin_comment
...
literal|0
decl_stmt|;
comment|//    glGetIntegerv(GL_MAX_GEOMETRY_OUTPUT_VERTICES_EXT,&n);
return|return
name|n
return|;
block|}
...
name|qWarning
argument_list|(
literal|"QOpenGLShaderProgram::setGeometryOutputVertexCount: count: %d higher than maximum: %d"
argument_list|,
name|count
argument_list|,
name|max
...
directive|if
literal|0
block|bool resolved = qt_resolve_glsl_extensions(const_cast<QOpenGLContext *>(context));     if (!resolved)         return false;      if ((type& Geometry)&& !QByteArray((const char *) glGetString(GL_EXTENSIONS)).contains("GL_EXT_geometry_shader4"))         return false;
endif|#
directive|endif
return|return
literal|true
