...
define|#
directive|define
name|QT_OPENGL_IMPLEMENT_WITH_FLAGS
parameter_list|(
name|CLASS
parameter_list|,
name|FUNCTIONS
parameter_list|)
define|\
value|void CLASS::init(QOpenGLContext *context) \ { \     const int flags[] = { FUNCTIONS(QT_OPENGL_FLAGS) 0 }; \     const char *names = FUNCTIONS(QT_OPENGL_FUNCTION_NAMES); \     const char *name = names; \     for (int i = 0; i< FUNCTIONS(QT_OPENGL_COUNT_FUNCTIONS); ++i) { \         functions[i] = ::getProcAddress(context, name, flags[i]); \         name += strlen(name) + 1; \     } \ }
