...
end_endif
begin_comment
comment|/*!     \enum QOpenGLFunctions::PlatformGLType     This enum defines the type of the underlying GL implementation.      \value DesktopGL Desktop OpenGL     \value GLES2 OpenGL ES 2.0 or higher     \value GLES1 OpenGL ES 1.x      \since 5.3  */
end_comment
begin_comment
comment|/*!   \fn QOpenGLFunctions::isES()    On platforms where the OpenGL implementation is dynamically loaded   this function returns true if the underlying GL implementation is   Open GL ES.    On platforms that do not use runtime loading of the GL the return   value is based on Qt's compile-time configuration and will never   change during runtime.    \sa platformGLType()    \since 5.3   */
end_comment
begin_comment
comment|/*!   Returns the underlying GL implementation type.    On platforms where the OpenGL implementation is not dynamically   loaded, the return value is determined during compile time and never   changes.    Platforms that use dynamic GL loading (e.g. Windows) cannot rely on   compile-time defines for differentiating between desktop and ES   OpenGL code. Instead, they rely on this function to query, during   runtime, the type of the loaded graphics library.    \since 5.3  */
end_comment
begin_function
