...
directive|endif
comment|// defined(Q_WS_QWS) || defined(Q_WS_QPA) || defined(Q_OS_SYMBIAN)
if|#
directive|if
name|defined
argument_list|(
...
parameter_list|)
block|{
comment|// we don't match using the xpoint, xres and yres parameters, only the id
for|for
control|(
name|int
name|i
...
condition|)
block|{
comment|// if styleName for searching first if possible
if|if
condition|(
operator|!
name|styleName
...
end_if
begin_comment
comment|// class with virtual destructor, derived in qfontdatabase_s60.cpp
end_comment
begin_class
...
directive|else
comment|// ..in case of defined(Q_OS_SYMBIAN)&& !defined(QT_NO_FREETYPE)
name|f
operator|->
name|fontFilename
operator|=
...
argument_list|)
expr_stmt|;
comment|//for (int i = 0; i< writingSystems.count(); ++i)
comment|//    qDebug()<< QFontDatabase::writingSystemName(writingSystems.at(i));
block|}
block|}
name|QString
name|family
...
name|FM_DEBUG
argument_list|(
literal|"            required encoding_id not available"
argument_list|)
expr_stmt|;
return|return
name|encoding
...
name|FM_DEBUG
argument_list|(
literal|"            PreferBitmap and/or OpenGL set, skipping Freetype"
argument_list|)
expr_stmt|;
block|}
else|else
...
return|;
block|}
comment|// FT not available, find an XLFD font, trying the default encoding first
name|encoding
operator|=
name|size
operator|->
...
condition|)
block|{
comment|// does it support the requested script?
name|bool
name|supportsScript
init|=
literal|false
...
expr_stmt|;
block|}
comment|// find the first encoding that supports the requested script
for|for
control|(
name|int
name|ws
...
name|FM_DEBUG
argument_list|(
literal|"          size %3d does not support the script we want"
argument_list|,
name|style
operator|->
name|pixelSizes
...
name|FM_DEBUG
argument_list|(
literal|"          foundry doesn't support the script we want"
argument_list|)
expr_stmt|;
continue|continue;
block|}
...
end_function
begin_comment
comment|/*!     Returns a list of the point sizes available for the font that has     family \a family and style \a style. The list may be empty.      \sa smoothSizes(), standardSizes() */
end_comment
begin_function
...
name|Q_WS_WIN
argument_list|)
comment|// windows and macosx are always smoothly scalable
name|Q_UNUSED
argument_list|(
name|family
argument_list|)
...
end_function
begin_comment
comment|/*!     Returns the point sizes of a font that has family \a family and     style \a style that will look attractive. The list may be empty.     For non-scalable fonts and bitmap scalable fonts, this function     is equivalent to pointSizes().    \sa pointSizes(), standardSizes() */
end_comment
begin_function
