...
end_function
begin_comment
comment|/*!     Converts the plain text string \a plain to a HTML string with     HTML metacharacters \c{<}, \c{>}, \c{&}, and \c{"} replaced by HTML     entities.      Example:      \snippet doc/src/snippets/code/src_gui_text_qtextdocument.cpp 0      This function is defined in the \c<QTextDocument> header file.      \sa convertFromPlainText(), mightBeRichText() */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!     Replaces the entire contents of the document with the given     HTML-formatted text in the \a html string.      The HTML formatting is respected as much as possible; for example,     "<b>bold</b> text" will produce text where the first word has a font     weight that gives it a bold appearance: "\bold{bold} text".      \note It is the responsibility of the caller to make sure that the     text is correctly decoded when a QString containing HTML is created     and passed to setHtml().      \sa setPlainText(), {Supported HTML Subset} */
end_comment
begin_ifndef
ifndef|#
directive|ifndef
...
directive|if
literal|0
comment|// ### Qt5: reenable
ifndef|#
directive|ifndef
name|QT_NO_TEXTEDIT
block|else if (QTextEdit *edit = qobject_cast<QTextEdit *>(parent())) {         QUrl resolvedName = edit->d_func()->resolveUrl(name);         r = edit->loadResource(type, resolvedName);     }
endif|#
directive|endif
ifndef|#
directive|ifndef
