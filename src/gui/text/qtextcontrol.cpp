...
expr_stmt|;
comment|// ###
comment|//        if (d->cursor.hasSelection())
comment|//            d->cursorOn&= (QGuiApplication::style()->styleHint(QStyle::SH_BlinkCursorWhenTextSelected)
comment|//                            != 0);
name|d
operator|->
name|repaintCursor
...
directive|if
literal|0
comment|// ### !defined(QT_NO_IM)
block|Q_Q(QTextControl);      QTextLayout *layout = cursor.block().layout();     if (contextObject&& layout&& !layout->preeditAreaText().isEmpty()) {         QInputContext *ctx = inputContext();         int cursorPos = q->hitTest(pos, Qt::FuzzyHit) - cursor.position();          if (cursorPos< 0 || cursorPos> layout->preeditAreaText().length()) {             cursorPos = -1;
comment|// don't send move events outside the preedit area
block|if (eventType == QEvent::MouseMove)                 return true;         }         if (ctx) {             QMouseEvent ev(eventType, contextObject->mapFromGlobal(globalPos), globalPos,                            button, buttons, modifiers);             ctx->mouseHandler(cursorPos,&ev);             e->setAccepted(ev.isAccepted());         }         if (!layout->preeditAreaText().isEmpty())             return true;     }
else|#
directive|else
name|Q_UNUSED
argument_list|(
...
directive|if
literal|0
comment|// ###ndef QT_NO_DESKTOPSERVICES
block|if (openExternalLinks)         QDesktopServices::openUrl(href);     else
endif|#
directive|endif
emit|emit
name|q_func
...
literal|0
comment|// ###
block|QStyleOption opt;             opt.palette = ctx.palette;             QStyleHintReturnVariant ret;             QStyle *style = QGuiApplication::style();             if (widget)                 style = widget->style();             style->styleHint(QStyle::SH_TextControl_FocusIndicatorTextCharFormat,&opt, widget,&ret);             selection.format = qvariant_cast<QTextFormat>(ret.variant).toCharFormat();
endif|#
directive|endif
block|}
else|else
...
literal|0
comment|// ###
block|QInputContext *ctx = contextObject->inputContext();     if (!ctx&& contextObject->parentWidget())         ctx = contextObject->parentWidget()->inputContext();     return ctx;
else|#
directive|else
return|return
literal|0
