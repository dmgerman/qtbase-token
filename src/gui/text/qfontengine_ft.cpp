...
operator|::
name|HintStyle
name|ftInitialDefaultHintStyle
init|=
ifdef|#
directive|ifdef
name|Q_OS_WIN
...
block|{
comment|// Select the shortest bitmap strike whose height is larger than the desired height
for|for
control|(
name|int
name|i
...
block|}
block|}
comment|// According to freetype documentation we must use FT_Select_Size
comment|// to make sure we can select the desired bitmap strike index
if|if
condition|(
name|FT_Select_Size
argument_list|(
...
name|blue
decl_stmt|;
comment|// intra-pixel filter used by the legacy filter (adopted from _ft_lcd_filter_legacy)
name|red
operator|=
operator|(
name|r
...
parameter_list|)
block|{
comment|// false if exceeds QFontEngineFT::Glyph metrics
return|return
call|(
name|short
call|)
...
condition|)
block|{
comment|// Certain fonts don't have no-break space and tab,
