...
end_function
begin_comment
comment|/*!     \since 4.8      Returns the requested font style name, it will be used to match the     font with irregular styles (that can't be normalized in other style     properties). It depends on system font support, thus only works for     OS X and X11 so far. On Windows irregular styles will be added     as separate font families so there is no need for this.      \sa setFamily(), setStyle() */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!     Returns the hash value for \a font. If specified, \a seed is used     to initialize the hash.      \relates QFont     \since 5.3 */
end_comment
begin_function
name|uint
name|qHash
...
condition|)
block|{
comment|// Continue writing 8 bits for versions< 5.4 so that we don't write too much,
comment|// even though we need 16 to store styleStrategy, so there is some data loss.
if|if
condition|(
name|s
operator|.
...
name|FC_DEBUG
argument_list|(
literal|"QFontCache::clear: engineData %p still has refcount %d"
argument_list|,
name|data
argument_list|,
name|data
...
expr_stmt|;
name|bool
name|mightHaveEnginesLeftForCleanup
decl_stmt|;
do|do
block|{
name|mightHaveEnginesLeftForCleanup
...
name|FC_DEBUG
argument_list|(
literal|"QFontCache::clear: engine %p still has refcount %d"
argument_list|,
name|engine
argument_list|,
name|engine
...
name|FC_DEBUG
argument_list|(
literal|"   QFontCache already contains engine data %p for key=(%g %g %d %d %d)"
argument_list|,
name|engineDataCache
operator|.
name|value
...
name|FC_DEBUG
argument_list|(
literal|"QFontCache: inserting new engine %p, refcount %d"
argument_list|,
name|engine
argument_list|,
name|engine
...
name|FC_DEBUG
argument_list|(
literal|"   QFontCache already contains engine %p for key=(%g %g %d %d %d)"
argument_list|,
name|engineCache
operator|.
name|value
...
name|data
decl_stmt|;
comment|// get rid of all occurrences
name|it
operator|=
name|engineCache
operator|.
...
block|}
block|}
comment|// and delete the last occurrence
name|Q_ASSERT
argument_list|(
name|fontEngine
operator|->
