...
argument_list|)
member_init_list|,
name|m_minLeftBearing
argument_list|(
name|kBearingNotInitialized
argument_list|)
member_init_list|,
name|m_minRightBearing
argument_list|(
name|kBearingNotInitialized
argument_list|)
block|{
...
argument_list|()
expr_stmt|;
comment|// Initializes both (see below)
return|return
name|m_minLeftBearing
return|;
block|}
...
define|#
directive|define
name|kMinLeftSideBearingOffset
value|12
end_define
begin_define
...
define|#
directive|define
name|kMinRightSideBearingOffset
value|14
end_define
begin_function
...
condition|)
block|{
comment|// Try the 'hhea' font table first, which covers the entire font
name|QByteArray
name|hheaTable
init|=
name|getSfntTable
...
argument_list|)
decl_stmt|;
comment|// The table data is expressed as FUnits, meaning we have to take the number
comment|// of units per em into account. Since pixelSize already has taken DPI into
comment|// account we can use that directly instead of the point size.
name|int
name|unitsPerEm
init|=
name|emSquareSize
...
name|unitsPerEm
decl_stmt|;
comment|// Some fonts on OS X (such as Gurmukhi Sangam MN, Khmer MN, Lao Sangam MN, etc.), have
comment|// invalid values for their NBSPACE left bearing, causing the 'hhea' minimum bearings to
comment|// be way off. We detect this by assuming that the minimum bearsings are within a certain
comment|// range of the em square size.
specifier|static
specifier|const
name|int
name|largestValidBearing
init|=
literal|4
operator|*
name|unitsPerEm
...
expr_stmt|;
block|}
comment|// Fallback in case of missing 'hhea' table (bitmap fonts e.g.) or broken 'hhea' values
if|if
condition|(
name|m_minLeftBearing
operator|==
...
condition|)
block|{
comment|// To balance performance and correctness we only look at a subset of the
comment|// possible glyphs in the font, based on which characters are more likely
comment|// to have a left or right bearing.
specifier|static
specifier|const
name|ushort
name|characterSubset
index|[]
init|=
block|{
literal|'('
...
block|}
decl_stmt|;
comment|// The font may have minimum bearings larger than 0, so we have to start at the max
name|m_minLeftBearing
operator|=
name|m_minRightBearing
operator|=
...
argument_list|)
decl_stmt|;
comment|// Glyphs with no contours shouldn't contribute to bearings
if|if
condition|(
operator|!
name|glyphMetrics
...
argument_list|()
operator|<<
literal|"Failed to compute left/right minimum bearings for"
operator|<<
name|fontDef
operator|.
name|family
...
condition|)
block|{
comment|// defer obtaining the fallback families until loadEngine(1)
name|m_fallbackFamilies
operator|<<
name|QString
argument_list|()
