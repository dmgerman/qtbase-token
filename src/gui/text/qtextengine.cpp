...
end_ifdef
begin_comment
comment|/*     Adds an inter character justification opportunity after the number or letter     character and a space justification opportunity after the space character. */
end_comment
begin_function
...
name|glyph_pos
expr_stmt|;
comment|// justification opportunity at the end of cluster
if|if
condition|(
name|Q_LIKELY
argument_list|(
...
case|:
comment|// same as default but inter character justification takes precedence
name|spaceAs
operator|=
name|Justification_Arabic_Space
expr_stmt|;
...
name|Script_Han
case|:
comment|// same as default but inter character justification is the only option
name|spaceAs
operator|=
name|Justification_Character
expr_stmt|;
...
argument_list|)
expr_stmt|;
comment|// high part never changes in simple casing
name|uc
index|[
name|i
index|]
...
comment|// k * 3 entries, array[k] == index in string, array[k + 1] == index in glyphs, array[k + 2] == engine index
name|QVector
argument_list|<
name|uint
argument_list|>
...
condition|)
block|{
comment|// ask the font engine to find out which glyphs (as an index in the specific font)
comment|// to use for the text in one item.
name|QGlyphLayout
name|initialGlyphs
init|=
...
argument_list|)
expr_stmt|;
comment|// ### props.language = hb_language_get_default_for_script(props.script);
for|for
control|(
name|int
name|k
...
expr_stmt|;
comment|// ###
comment|// Ligatures are incompatible with custom letter spacing, so when a letter spacing is set,
comment|// we disable them for writing systems where they are purely cosmetic.
name|bool
...
name|Q_OS_DARWIN
argument_list|)
comment|// What's behind QFontEngine::FaceData::user_data isn't compatible between different font engines
comment|// - specifically functions in hb-coretext.cc would run into undefined behavior with data
comment|// from non-CoreText engine. The other shapers works with that engine just fine.
if|if
condition|(
name|actualFontEngine
operator|->
...
name|char
modifier|*
name|s_shaper_list_without_coretext
index|[]
init|=
block|{
literal|"graphite2"
...
argument_list|)
decl_stmt|;
comment|// ensure we have enough space for shaped glyphs and metrics
if|if
condition|(
name|Q_UNLIKELY
argument_list|(
...
return|;
block|}
comment|// fetch the shaped glyphs and metrics
name|QGlyphLayout
name|g
init|=
name|availableGlyphs
...
literal|true
expr_stmt|;
comment|// fix up clusters so that the cluster indices will be monotonic
comment|// and thus we never return out-of-order indices
while|while
condition|(
name|last_cluster
operator|++
...
name|cluster
expr_stmt|;
comment|// hide characters that should normally be invisible
switch|switch
condition|(
name|string
index|[
...
condition|)
block|{
comment|// U+00AD [SOFT HYPHEN] is a default ignorable codepoint,
comment|// so we replace its glyph and metrics with ones for
comment|// U+002D [HYPHEN-MINUS] and make it visible if it appears at line-break
name|g
operator|.
name|glyphs
index|[
...
expr_stmt|;
comment|// ensure we are not asserting in HB_HeuristicSetGlyphAttributes()
name|entire_shaper_item
operator|.
name|num_glyphs
operator|=
...
control|)
block|{
comment|// Workaround wrong log_clusters for surrogates (i.e. QTBUG-39875)
if|if
condition|(
name|shaper_item
operator|.
...
expr_stmt|;
block|}
comment|// replace with shared copies
for|for
control|(
name|int
name|i
...
literal|true
expr_stmt|;
comment|// emulate grapheme cluster
name|attributes
index|[
name|i
index|]
...
name|end
expr_stmt|;
comment|// the last item in the last line -> insert eol position
if|if
condition|(
name|si
operator|.
