...
expr_stmt|;
block|}
comment|// adjust to cell boundaries
if|if
condition|(
name|m
operator|<=
...
end_function
begin_comment
comment|/*!     Clears the current selection by setting the anchor to the cursor position.      Note that it does \bold{not} delete the text of the selection.      \sa removeSelectedText() hasSelection() */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!     \overload      Convenience method for inserting the image with the given \a name at the     current position().      \snippet doc/src/snippets/code/src_gui_text_qtextcursor.cpp 1 */
end_comment
begin_function
DECL|function|insertImage
name|void
...
end_function
begin_comment
comment|/*!     Like beginEditBlock() indicates the start of a block of editing operations     that should appear as a single operation for undo/redo. However unlike     beginEditBlock() it does not start a new block but reverses the previous call to     endEditBlock() and therefore makes following operations part of the previous edit block created.      For example:      \snippet doc/src/snippets/code/src_gui_text_qtextcursor.cpp 3      The call to undo() will cause all three insertions to be undone.      \sa beginEditBlock(), endEditBlock()  */
end_comment
begin_function
