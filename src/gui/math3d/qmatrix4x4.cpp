...
condition|)
block|{
comment|// We can express this projection as a translate and scale
comment|// which will be more efficient to modify with further
comment|// transformations than producing a "General" matrix.
name|translate
argument_list|(
name|QVector3D
argument_list|(
...
end_endif
begin_comment
comment|/*!     Flips between right-handed and left-handed coordinate systems     by multiplying the y and z co-ordinates by -1.  This is normally     used to create a left-handed orthographic view without scaling     the viewport as ortho() does.      \sa ortho() */
end_comment
begin_function
...
parameter_list|()
block|{
comment|// If the last element is not 1, then it can never be special.
if|if
condition|(
name|m
index|[
...
return|return;
block|}
comment|// If the upper three elements m12, m13, and m21 are not all zero,
comment|// or the lower elements below the diagonal are not all zero, then
comment|// the matrix can never be special.
if|if
condition|(
name|m
index|[
...
return|return;
block|}
comment|// Determine what we have in the remaining regions of the matrix.
name|bool
name|identityAlongDiagonal
init|=
operator|(
name|m
index|[
...
operator|)
decl_stmt|;
comment|// Now determine the special matrix type.
if|if
condition|(
name|translationPresent
operator|&&
