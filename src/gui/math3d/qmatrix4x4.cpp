...
end_comment
begin_comment
comment|/*!     \fn QMatrix4x4 qGenericMatrixToMatrix4x4(const QGenericMatrix<N, M, float>& matrix)     \relates QMatrix4x4     \obsolete      Returns a 4x4 matrix constructed from the left-most 4 columns and     top-most 4 rows of \a matrix.  If \a matrix has less than 4 columns     or rows, the remaining elements are filled with elements from the     identity matrix. */
end_comment
begin_comment
...
end_comment
begin_comment
comment|/*!     \fn bool QMatrix4x4::isAffine() const     \since 5.5      Returns \c true if this matrix is affine matrix; false otherwise.      An affine matrix is a 4x4 matrix with row 3 equal to (0, 0, 0, 1),     e.g. no projective coefficients.      \sa isIdentity() */
end_comment
begin_comment
...
end_endif
begin_comment
comment|/*!     \fn void QMatrix4x4::viewport(const QRectF&rect)     \overload      Sets up viewport transform for viewport bounded by \a rect and with near and far set     to 0 and 1 respectively. */
end_comment
begin_comment
comment|/*!     Multiplies this matrix by another that performs the scale and bias     transformation used by OpenGL to transform from normalized device     coordinates (NDC) to viewport (window) coordinates. That is it maps     points from the cube ranging over [-1, 1] in each dimension to the     viewport with it's near-lower-left corner at (\a left, \a bottom, \a nearPlane)     and with size (\a width, \a height, \a farPlane - \a nearPlane).      This matches the transform used by the fixed function OpenGL viewport     transform controlled by the functions glViewport() and glDepthRange().  */
end_comment
begin_function
