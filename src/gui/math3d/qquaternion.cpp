...
end_function
begin_comment
comment|/*!     \since 5.5      Constructs the quaternion using specified forward direction \a direction     and upward direction \a up.     If the upward direction was not specified or the forward and upward     vectors are collinear, a new orthonormal upward direction will be generated.      \sa fromAxes(), rotationTo() */
end_comment
begin_function
...
condition|)
block|{
comment|// collinear or invalid up vector; derive shortest arc to new direction
return|return
name|QQuaternion
operator|::
name|rotationTo
...
end_function
begin_comment
comment|/*!     \since 5.5      Returns the shortest arc quaternion to rotate from the direction described by the vector \a from     to the direction described by the vector \a to.      \sa fromDirection() */
end_comment
begin_function
...
parameter_list|)
block|{
comment|// Based on Stan Melax's article in Game Programming Gems
specifier|const
name|QVector3D
name|v0
argument_list|(
...
literal|1.0f
decl_stmt|;
comment|// if dest vector is close to the inverse of source vector, ANY axis of rotation is valid
if|if
condition|(
name|qFuzzyIsNull
argument_list|(
...
argument_list|()
expr_stmt|;
comment|// same as QQuaternion::fromAxisAndAngle(axis, 180.0f)
return|return
name|QQuaternion
argument_list|(
literal|0.0f
