...
define|\
value|while (buffer< end) { \             const typename Simd::Float32x4 v_index_local = Simd::v_sub(Simd::v_sqrt(Simd::v_max(v_min, det_vec.v)), b_vec.v); \             const typename Simd::Float32x4 v_index = Simd::v_add(Simd::v_mul(v_index_local, v_table_size_minus_one), v_half); \             typename Simd::Vect_buffer_i index_vec;
...
define|\
value|det_vec.v = Simd::v_add(Simd::v_add(det_vec.v, delta_det4_vec.v), v_delta_delta_det6); \             delta_det4_vec.v = Simd::v_add(delta_det4_vec.v, v_delta_delta_det16); \             b_vec.v = Simd::v_add(b_vec.v, v_delta_b4); \             for (int i = 0; i< 4; ++i) \                 *buffer++ = data->gradient.colorTable[index_vec.i[i]]; \         }
if|if
condition|(
name|data
operator|->
