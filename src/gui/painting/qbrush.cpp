...
case|:
block|{
comment|// Note this produces false negatives if the textures have identical data,
comment|// but does not share the same data in memory. Since equality is likely to
comment|// be used to avoid iterating over the data for a texture update, this should
comment|// still be better than doing an accurate comparison.
specifier|const
name|QPixmap
modifier|*
name|us
...
name|them
condition|)
comment|// both images or both pixmaps
return|return
literal|true
return|;
comment|// Only raster QPixmaps use the same cachekeys as QImages.
if|if
condition|(
name|us
operator|&&
