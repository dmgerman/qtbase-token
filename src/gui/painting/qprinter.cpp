...
end_comment
begin_comment
comment|/*   \enum QPrinter::PrinterOption    This enum describes various printer options that appear in the   printer setup dialog. It is used to enable and disable these   options in the setup dialog.    \value PrintToFile Describes if print to file should be enabled.   \value PrintSelection Describes if printing selections should be enabled.   \value PrintPageRange Describes if printing page ranges (from, to) should   be enabled   \value PrintCurrentPage if Print Current Page option should be enabled    \sa setOptionEnabled(), isOptionEnabled() */
end_comment
begin_comment
...
name|QLatin1String
argument_list|(
literal|"untitled_printer_document"
argument_list|)
argument_list|)
expr_stmt|;
block|}
...
name|bool
name|qt_compat_QPrinter_printSetup
parameter_list|(
name|QPrinter
modifier|*
name|printer
...
name|bool
name|qt_compat_QPrinter_pageSetup
parameter_list|(
name|QPrinter
modifier|*
name|p
...
return|;
block|}
comment|/*!     Executes a page setup dialog so that the user can configure the type of     page used for printing. Returns true if the contents of the dialog are     accepted; returns false if the dialog is canceled. */
...
return|;
block|}
comment|/*!     Executes a print setup dialog so that the user can configure the printing     process. Returns true if the contents of the dialog are accepted; returns     false if the dialog is canceled. */
...
directive|endif
comment|// Q_WS_MAC
comment|/*!     Use QPrintDialog instead.      \oldcode         if (printer->setup(parent))             ...     \newcode         QPrintDialog dialog(printer, parent);         if (dialog.exec())             ...     \endcode */
DECL|function|setup
name|bool
name|QPrinter
operator|::
...
empty_stmt|;
block|}
comment|/*!     Use QPrintDialog::minPage() instead. */
...
return|;
block|}
comment|/*!     Use QPrintDialog::maxPage() instead. */
...
return|;
block|}
comment|/*!     Use QPrintDialog::setMinMax() instead. */
...
expr_stmt|;
block|}
comment|/*!     Returns true if the printer is set up to collate copies of printed documents;     otherwise returns false.      Use QPrintDialog::isOptionEnabled(QPrintDialog::PrintCollateCopies)     instead.      \sa collateCopies() */
...
return|;
block|}
comment|/*!     Use QPrintDialog::setOption(QPrintDialog::PrintCollateCopies)     or QPrintDialog::setOptions(QPrintDialog::options()& ~QPrintDialog::PrintCollateCopies) instead, depending on \a     enable. */
...
expr_stmt|;
block|}
comment|/*!     Use QPrintDialog instead. */
