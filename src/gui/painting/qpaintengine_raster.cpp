...
include|#
directive|include
file|<private/qabstractfontengine_p.h>
end_include
begin_elif
elif|#
directive|elif
...
specifier|static
name|void
name|drawLine_midpoint_dashed_i
parameter_list|(
name|int
name|x1
parameter_list|,
...
end_function_decl
begin_comment
comment|// static void drawLine_midpoint_f(qreal x1, qreal y1, qreal x2, qreal y2,
end_comment
begin_comment
comment|//                                 ProcessSpans span_func, QSpanData *data,
end_comment
begin_comment
comment|//                                 LineDrawMode style, const QRect&devRect);
end_comment
begin_function_decl
specifier|static
name|void
...
condition|)
block|{
comment|// this calls the float version
name|QPaintEngineEx
operator|::
name|drawPolygon
argument_list|(
...
index|]
decl_stmt|;
comment|// Use fast path for 0 width /  trivial pens.
name|QIntRect
name|devRect
decl_stmt|;
name|devRect
...
argument_list|)
decl_stmt|;
comment|// Draw all the line segments.
for|for
control|(
name|int
name|i
...
block|}
block|}
comment|// Polygons are implicitly closed.
if|if
condition|(
name|needs_closing
condition|)
...
argument_list|()
decl_stmt|;
comment|// We assert here because this function is called from drawRects
comment|// and drawPolygon and they already do ensurePen(), so we skip that
comment|// here to avoid duplicate checks..
name|Q_ASSERT
argument_list|(
name|s
operator|->
...
end_function
begin_comment
comment|// assumes that rect has positive width and height
end_comment
begin_function
...
literal|false
expr_stmt|;
comment|// don't try to cache huge fonts
specifier|const
name|qreal
name|pixelSize
init|=
...
literal|false
expr_stmt|;
comment|// ### Remove the TestFontEngine and Box engine crap, in these
comment|// ### cases we should delegate painting to the font engine
comment|// ### directly...
if|#
directive|if
name|defined
...
argument_list|()
decl_stmt|;
comment|// qDebug()<< "type"<< fontEngineType<< s->matrix.type();
if|if
condition|(
operator|(
name|fontEngineType
...
index|]
decl_stmt|;
comment|// spans must be sorted on y (primary) and x (secondary)
if|if
condition|(
name|y
operator|<
...
argument_list|()
operator|<<
literal|" - QRasterPaintEngine::drawLine()"
expr_stmt|;
endif|#
directive|endif
name|Q_D
...
end_function
begin_comment
comment|/*!     Draws the given color \a spans with the specified \a color. The \a     count parameter specifies the number of spans.      The default implementation does nothing; reimplement this function     to draw the given color \a spans with the specified \a color. Note     that this function \e must be reimplemented if the framebuffer is     not memory-mapped.      \sa drawBufferSpan() */
end_comment
begin_if
if|#
directive|if
...
name|qFatal
argument_list|(
literal|"QRasterPaintEngine::drawBufferSpan must be reimplemented on "
literal|"a non memory-mapped device"
argument_list|)
expr_stmt|;
block|}
...
continue|continue;
block|}
comment|// we need to merge the two
comment|// find required length
name|int
name|max
init|=
name|qMax
...
name|rasterPoolBase
operator|=
comment|// We make use of setjmp and longjmp in qgrayraster.c which requires
comment|// 16-byte alignment, hence we hardcode this requirement here..
operator|(
name|unsigned
name|char
operator|*
...
name|rasterPoolOnStack
condition|)
comment|// initially on the stack
name|free
argument_list|(
name|rasterPoolBase
argument_list|)
...
end_comment
begin_comment
comment|/*!     \class QCustomRasterPaintDevice     \preliminary     \ingroup qws     \since 4.2      \brief The QCustomRasterPaintDevice class is provided to activate     hardware accelerated paint engines in Qt for Embedded Linux.      Note that this class is only available in \l{Qt for Embedded Linux}.      In \l{Qt for Embedded Linux}, painting is a pure software     implementation. But starting with Qt 4.2, it is     possible to add an accelerated graphics driver to take advantage     of available hardware resources.      Hardware acceleration is accomplished by creating a custom screen     driver, accelerating the copying from memory to the screen, and     implementing a custom paint engine accelerating the various     painting operations. Then a custom paint device (derived from the     QCustomRasterPaintDevice class) and a custom window surface     (derived from QWSWindowSurface) must be implemented to make     \l{Qt for Embedded Linux} aware of the accelerated driver.      See the \l {Adding an Accelerated Graphics Driver to Qt for Embedded Linux}     documentation for details.      \sa QRasterPaintEngine, QPaintDevice */
end_comment
begin_comment
comment|/*!     \fn QCustomRasterPaintDevice::QCustomRasterPaintDevice(QWidget *widget)      Constructs a custom raster based paint device for the given     top-level \a widget. */
end_comment
begin_comment
comment|/*!     \fn int QCustomRasterPaintDevice::bytesPerLine() const      Returns the number of bytes per line in the framebuffer. Note that     this number might be larger than the framebuffer width. */
end_comment
begin_comment
comment|/*!     \fn int QCustomRasterPaintDevice::devType() const     \internal */
end_comment
begin_comment
comment|/*!     \fn QImage::Format QCustomRasterPaintDevice::format() const      Returns the format of the device's memory buffet.      The default format is QImage::Format_ARGB32_Premultiplied. The     only other valid format is QImage::Format_RGB16. */
end_comment
begin_comment
comment|/*!     \fn void * QCustomRasterPaintDevice::memory () const      Returns a pointer to the paint device's memory buffer, or 0 if no     such buffer exists. */
end_comment
begin_comment
comment|/*!     \fn int QCustomRasterPaintDevice::metric ( PaintDeviceMetric m ) const     \reimp */
end_comment
begin_comment
comment|/*!     \fn QSize QCustomRasterPaintDevice::size () const     \internal */
end_comment
begin_constructor
...
argument_list|)
expr_stmt|;
comment|//         qDebug()<< "processed "<< processed<< "clipped"<< clipped-cspans
...
end_endif
begin_comment
comment|/*!     \internal      Draws a line using the floating point midpoint algorithm. The line     \a line is already in device coords at this point. */
end_comment
begin_function
...
condition|)
block|{
comment|// specialcase horizontal lines
if|if
condition|(
name|y1
operator|>=
...
condition|)
block|{
comment|// specialcase vertical lines
if|if
condition|(
name|x1
operator|>=
...
operator|--
expr_stmt|;
comment|// hw: create spans directly instead to possibly avoid clipping
if|if
condition|(
name|len
operator|>
...
condition|)
block|{
comment|/* if x is the major axis: */
if|if
condition|(
name|x2
operator|<
...
condition|)
block|{
comment|/* if coordinates are out of order */
name|qt_swap_int
argument_list|(
name|x1
argument_list|,
...
name|x2
expr_stmt|;
comment|// In the loops below we increment before call the span function so
comment|// we need to stop one pixel before
name|x2
operator|=
name|qMin
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// completely clipped, so abort
if|if
condition|(
name|x2
operator|<=
...
condition|)
block|{
comment|// 315 -> 360 and 135 -> 180 (unit circle degrees)
name|y2
operator|=
name|qMin
argument_list|(
...
block|{
comment|// 0-45 and 180->225 (unit circle degrees)
name|y1
operator|=
name|qMin
argument_list|(
...
block|{
comment|// if y is the major axis:
if|if
condition|(
name|y2
operator|<
...
condition|)
block|{
comment|// 90 -> 135 and 270 -> 315 (unit circle degrees)
name|x2
operator|=
name|qMin
argument_list|(
...
block|{
comment|// 45 -> 90 and 225 -> 270 (unit circle degrees)
name|x1
operator|=
name|qMin
argument_list|(
...
argument_list|()
operator|<<
literal|"   - drawLine_midpoint_dashed_i"
operator|<<
name|x1
operator|<<
name|y1
...
argument_list|)
expr_stmt|;
comment|// pattern must be reversed if coordinates are out of order
name|int
name|reverseLength
init|=
operator|-
...
argument_list|)
decl_stmt|;
comment|// adjust pattern for offset
name|offset_pattern
argument_list|(
operator|*
name|patternOffset
...
operator|--
expr_stmt|;
comment|// adjust pattern for starting offset
name|offset_pattern
argument_list|(
name|start
operator|-
