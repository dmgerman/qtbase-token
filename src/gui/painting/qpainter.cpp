...
name|thread
argument_list|()
comment|// pixmaps cannot be targets unless threaded pixmaps are supported
operator|&&
operator|(
name|devType
operator|!=
...
argument_list|)
operator|)
comment|// framebuffer objects and such cannot be targets unless threaded GL is supported
operator|&&
operator|(
name|devType
operator|!=
...
argument_list|)
operator|)
comment|// widgets cannot be targets except for QGLWidget
operator|&&
operator|(
name|devType
operator|!=
...
end_function
begin_comment
comment|/*!     \fn void QPainter::rotate(qreal angle)      Rotates the coordinate system clockwise. The given \a angle parameter is in degrees.      \sa setWorldTransform(), {QPainter#Coordinate Transformations}{Coordinate Transformations} */
end_comment
begin_function
...
comment|// the text above it, but limit it to stay within descent.
name|qreal
name|adjustedUnderlineOffset
init|=
name|std
operator|::
name|ceil
