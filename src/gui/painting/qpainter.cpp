...
argument_list|)
expr_stmt|;
comment|// Someone either called QPainter::setRedirected in the widget's paint event
comment|// right before this painter was created (or begin was called) or
comment|// sent a paint event directly to the widget.
if|if
condition|(
operator|!
name|widget
...
literal|false
return|;
comment|// Check if we're attempting to paint outside a paint event.
if|if
condition|(
operator|!
name|sp
...
name|qWarning
argument_list|(
literal|"QPainter::begin: Widget painting can only begin as a result of a paintEvent"
argument_list|)
expr_stmt|;
return|return
literal|false
...
condition|)
block|{
comment|// Override the default pen's cosmetic state if the
comment|// NonCosmeticDefaultPen render hint is used.
name|QPen
name|oldPen
init|=
name|newState
...
argument_list|)
expr_stmt|;
comment|// Restore the state pen back to its default to preserve visible
comment|// state.
name|newState
operator|->
name|pen
...
end_function
begin_comment
comment|/*!     Initializes the painters pen, background and font to the same as     the given \a widget. This function is called automatically when the     painter is opened on a QWidget.      \sa begin(), {QPainter#Settings}{Settings} */
end_comment
begin_function
...
name|widget
argument_list|,
literal|"QPainter::initFrom(const QWidget *widget)"
argument_list|,
literal|"Widget cannot be 0"
argument_list|)
expr_stmt|;
name|Q_D
argument_list|(
...
decl_stmt|;
comment|// We know for sure that redirection is broken when the widget is inside
comment|// its paint event, so it's safe to use our hard-coded redirection. However,
comment|// there IS one particular case we still need to support, and that's
comment|// when people call QPainter::setRedirected in the widget's paint event right
comment|// before any painter is created (or QPainter::begin is called). In that
comment|// particular case our hard-coded redirection is restored and the redirection
comment|// is retrieved from QPainter::redirected (as before).
if|if
condition|(
name|pd
operator|->
...
name|qWarning
argument_list|(
literal|"QPainter::begin: Widget painting can only begin as a "
literal|"result of a paintEvent"
argument_list|)
expr_stmt|;
name|qt_cleanup_painter_state
argument_list|(
...
return|;
block|}
comment|// Adjust offset for alien widgets painting outside the paint event.
if|if
condition|(
operator|!
name|inPaintEvent
...
end_function
begin_comment
comment|/*!     Draws the given painter \a path using the current pen for outline     and the current brush for filling.      \table 100%     \row     \o \inlineimage qpainter-path.png     \o     \snippet doc/src/snippets/code/src_gui_painting_qpainter.cpp 5     \endtable      \sa {painting/painterpaths}{the Painter Paths     example},{painting/deform}{the Vector Deformation example} */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!     \fn void QPainter::drawLine(const QLineF&line)      Draws a line defined by \a line.      \table 100%     \row     \o \inlineimage qpainter-line.png     \o     \snippet doc/src/snippets/code/src_gui_painting_qpainter.cpp 6     \endtable      \sa drawLines(), drawPolyline(), {Coordinate System} */
end_comment
begin_comment
...
end_comment
begin_comment
comment|/*!     \fn void QPainter::drawRect(const QRectF&rectangle)      Draws the current \a rectangle with the current pen and brush.      A filled rectangle has a size of \a{rectangle}.size(). A stroked     rectangle has a size of \a{rectangle}.size() plus the pen width.      \table 100%     \row     \o \inlineimage qpainter-rectangle.png     \o     \snippet doc/src/snippets/code/src_gui_painting_qpainter.cpp 7     \endtable      \sa drawRects(), drawPolygon(), {Coordinate System} */
end_comment
begin_comment
...
end_function
begin_comment
comment|/*!     \overload      Sets the painter's pen to have the given \a style, width 0 and     black color. */
end_comment
begin_function
DECL|function|setPen
name|void
...
condition|)
return|return;
comment|// QPen(Qt::NoPen) is to avoid creating QPenData, including its brush (from the color)
comment|// Note that this works well as long as QPen(Qt::NoPen) returns a black, zero-width pen
name|d
operator|->
name|state
operator|->
...
end_comment
begin_comment
comment|/*!     \fn void QPainter::drawEllipse(const QRectF&rectangle)      Draws the ellipse defined by the given \a rectangle.      A filled ellipse has a size of \a{rectangle}.\l     {QRect::size()}{size()}. A stroked ellipse has a size of     \a{rectangle}.\l {QRect::size()}{size()} plus the pen width.      \table 100%     \row     \o \inlineimage qpainter-ellipse.png     \o     \snippet doc/src/snippets/code/src_gui_painting_qpainter.cpp 9     \endtable      \sa drawPie(), {Coordinate System} */
end_comment
begin_function
...
end_ifdef
begin_comment
comment|/*!     \fn void QPainter::drawLineSegments(const QPolygon&polygon, int     index, int count)      Draws \a count separate lines from points defined by the \a     polygon, starting at \a{polygon}\e{[index]} (\a index defaults to     0). If \a count is -1 (the default) all points until the end of     the array are used.      Use drawLines() combined with QPolygon::constData() instead.      \oldcode         QPainter painter(this);         painter.drawLineSegments(polygon, index, count);     \newcode         int lineCount = (count == -1) ?  (polygon.size() - index) / 2  : count;          QPainter painter(this);         painter.drawLines(polygon.constData() + index * 2, lineCount);     \endcode */
end_comment
begin_function
...
name|printf
argument_list|(
literal|"QPainter::drawLineSegments(), count=%d\n"
argument_list|,
name|a
operator|.
name|size
...
condition|)
block|{
comment|// FALCON: Use QVectorPath
name|QVector
argument_list|<
name|QLineF
argument_list|>
...
end_comment
begin_comment
comment|/*!     Draws the polygon defined by the first \a pointCount points in the     array \a points using the current pen and brush.      \table 100%     \row     \o \inlineimage qpainter-polygon.png     \o     \snippet doc/src/snippets/code/src_gui_painting_qpainter.cpp 14     \endtable      The first point is implicitly connected to the last point, and the     polygon is filled with the current brush().      If \a fillRule is Qt::WindingFill, the polygon is filled using the     winding fill algorithm.  If \a fillRule is Qt::OddEvenFill, the     polygon is filled using the odd-even fill algorithm. See     \l{Qt::FillRule} for a more detailed description of these fill     rules.      \sa  drawConvexPolygon(), drawPolyline(), {Coordinate System} */
end_comment
begin_function
...
end_comment
begin_comment
comment|/*!     \fn void QPainter::drawConvexPolygon(const QPointF *points, int pointCount)      Draws the convex polygon defined by the first \a pointCount points     in the array \a points using the current pen.      \table 100%     \row     \o \inlineimage qpainter-polygon.png     \o     \snippet doc/src/snippets/code/src_gui_painting_qpainter.cpp 15     \endtable      The first point is implicitly connected to the last point, and the     polygon is filled with the current brush().  If the supplied     polygon is not convex, i.e. it contains at least one angle larger     than 180 degrees, the results are undefined.      On some platforms (e.g. X11), the drawConvexPolygon() function can     be faster than the drawPolygon() function.      \sa drawPolygon(), drawPolyline(), {Coordinate System} */
end_comment
begin_comment
...
expr_stmt|;
block|}
comment|// If we don't have an extended paint engine, or if the painter is projected,
comment|// we go through standard code path
if|if
condition|(
name|d
operator|->
...
end_comment
begin_comment
comment|/*!     \fn void QPainter::eraseRect(const QRectF&rectangle)      Erases the area inside the given \a rectangle. Equivalent to     calling     \snippet doc/src/snippets/code/src_gui_painting_qpainter.cpp 19      \sa fillRect() */
end_comment
begin_function
...
end_ifdef
begin_comment
comment|/*!     \obsolete      Use the worldTransform() combined with QTransform::dx() instead.      \oldcode         QPainter painter(this);         qreal x = painter.translationX();     \newcode         QPainter painter(this);         qreal x = painter.worldTransform().dx();     \endcode */
end_comment
begin_function
...
name|qWarning
argument_list|(
literal|"QPainter::translationX: Painter not active"
argument_list|)
expr_stmt|;
return|return
literal|0.0
...
end_function
begin_comment
comment|/*!     \obsolete      Use the worldTransform() combined with QTransform::dy() instead.      \oldcode         QPainter painter(this);         qreal y = painter.translationY();     \newcode         QPainter painter(this);         qreal y = painter.worldTransform().dy();     \endcode */
end_comment
begin_function
...
name|qWarning
argument_list|(
literal|"QPainter::translationY: Painter not active"
argument_list|)
expr_stmt|;
return|return
literal|0.0
...
end_function
begin_comment
comment|/*!     \fn void QPainter::map(int x, int y, int *rx, int *ry) const      \internal      Sets (\a{rx}, \a{ry}) to the point that results from applying the     painter's current transformation on the point (\a{x}, \a{y}). */
end_comment
begin_function
DECL|function|map
name|void
...
end_function
begin_comment
comment|/*!     \fn QPoint QPainter::xForm(const QPoint&point) const      Use combinedTransform() instead. */
end_comment
begin_function
DECL|function|xForm
name|QPoint
...
name|qWarning
argument_list|(
literal|"QPainter::xForm: Painter not active"
argument_list|)
expr_stmt|;
return|return
name|QPoint
...
end_function
begin_comment
comment|/*!     \fn QRect QPainter::xForm(const QRect&rectangle) const     \overload      Use combinedTransform() instead of this function and call     mapRect() on the result to obtain a QRect. */
end_comment
begin_function
DECL|function|xForm
name|QRect
...
end_function
begin_comment
comment|/*!     \fn QPolygon QPainter::xForm(const QPolygon&polygon) const     \overload      Use combinedTransform() instead. */
end_comment
begin_function
DECL|function|xForm
name|QPolygon
...
end_function
begin_comment
comment|/*!     \fn QPolygon QPainter::xForm(const QPolygon&polygon, int index, int count) const     \overload      Use combinedTransform() combined with QPolygon::mid() instead.      \oldcode         QPainter painter(this);         QPolygon transformed = painter.xForm(polygon, index, count)     \newcode         QPainter painter(this);         QPolygon transformed = polygon.mid(index, count) * painter.combinedTransform();     \endcode */
end_comment
begin_function
DECL|function|xForm
name|QPolygon
...
end_function
begin_comment
comment|/*!     \fn QPoint QPainter::xFormDev(const QPoint&point) const     \overload     \obsolete      Use combinedTransform() combined with QTransform::inverted() instead.      \oldcode         QPainter painter(this);         QPoint transformed = painter.xFormDev(point);     \newcode         QPainter painter(this);         QPoint transformed = point * painter.combinedTransform().inverted();     \endcode */
end_comment
begin_function
...
name|qWarning
argument_list|(
literal|"QPainter::xFormDev: Painter not active"
argument_list|)
expr_stmt|;
return|return
name|QPoint
...
end_function
begin_comment
comment|/*!     \fn QRect QPainter::xFormDev(const QRect&rectangle) const     \overload     \obsolete      Use combinedTransform() combined with QTransform::inverted() instead.      \oldcode         QPainter painter(this);         QRect transformed = painter.xFormDev(rectangle);     \newcode         QPainter painter(this);         QRegion region = QRegion(rectangle) * painter.combinedTransform().inverted();         QRect transformed = region.boundingRect();     \endcode */
end_comment
begin_function
DECL|function|xFormDev
name|QRect
...
end_function
begin_comment
comment|/*!     \overload      \fn QPoint QPainter::xFormDev(const QPolygon&polygon) const     \obsolete      Use  combinedTransform() combined with QTransform::inverted() instead.      \oldcode         QPainter painter(this);         QPolygon transformed = painter.xFormDev(rectangle);     \newcode         QPainter painter(this);         QPolygon transformed = polygon * painter.combinedTransform().inverted();     \endcode */
end_comment
begin_function
DECL|function|xFormDev
name|QPolygon
...
end_function
begin_comment
comment|/*!     \fn QPolygon QPainter::xFormDev(const QPolygon&polygon, int index, int count) const     \overload     \obsolete      Use combinedTransform() combined with QPolygon::mid() and QTransform::inverted() instead.      \oldcode         QPainter painter(this);         QPolygon transformed = painter.xFormDev(polygon, index, count);     \newcode         QPainter painter(this);         QPolygon transformed = polygon.mid(index, count) * painter.combinedTransform().inverted();     \endcode */
end_comment
begin_function
DECL|function|xFormDev
name|QPolygon
...
name|qWarning
argument_list|(
literal|"QPainter::drawCubicBezier: Cubic Bezier needs 4 control "
literal|"points"
argument_list|)
expr_stmt|;
return|return;
...
argument_list|)
member_init_list|,
name|internalWidgetRedirectionIndex
argument_list|(
operator|-
literal|1
argument_list|)
...
argument_list|(
argument|QPaintDeviceRedirection
argument_list|)
block|}
struct|;
end_struct
...
name|QPaintDeviceRedirection
argument_list|>
name|QPaintDeviceRedirectionList
typedef|;
end_typedef
begin_macro
name|Q_GLOBAL_STATIC
argument_list|(
argument|QPaintDeviceRedirectionList
argument_list|,
argument|globalRedirections
argument_list|)
end_macro
begin_macro
name|Q_GLOBAL_STATIC
...
argument|QMutex
argument_list|,
argument|globalRedirectionsMutex
argument_list|)
end_macro
begin_macro
name|Q_GLOBAL_STATIC
...
argument|QAtomicInt
argument_list|,
argument|globalRedirectionAtomic
argument_list|)
end_macro
begin_comment
...
expr_stmt|;
name|bool
name|hadInternalWidgetRedirection
init|=
literal|false
decl_stmt|;
if|if
...
argument_list|()
decl_stmt|;
comment|// This is the case when the widget is in a paint event.
if|if
condition|(
name|widgetPrivate
operator|->
...
condition|)
block|{
comment|// Remove internal redirection and put it back into the global redirection list.
name|QPoint
name|oldOffset
decl_stmt|;
name|QPaintDevice
...
argument_list|)
expr_stmt|;
comment|// Restore the internal widget redirection, i.e. remove it from the global
comment|// redirection list and put it back into QWidgetPrivate. The index is only set when
comment|// someone call QPainter::setRedirected in a widget's paint event and we internally
comment|// have a redirection set (typically set in QWidgetPrivate::drawWidget).
if|if
condition|(
name|internalWidgetRedirectionIndex
operator|>=
...
argument_list|)
block|{
comment|// ignore the missing mutex, since we could be called from
comment|// a destructor, and destructors shall not throw
block|}
name|QMutexLocker
name|locker
argument_list|(
...
argument_list|)
block|{
comment|// do nothing - code below is safe with redirections being 0.
block|}
if|if
condition|(
name|redirections
...
condition|)
block|{
comment|// do the rounding manually to work around inconsistencies
comment|// in the paint engines when drawing on floating point offsets
specifier|const
name|qreal
name|scale
init|=
...
name|qWarning
argument_list|(
literal|"QPainter: bitBlt only works when source is of type pixmap"
argument_list|)
expr_stmt|;
block|}
block|}
