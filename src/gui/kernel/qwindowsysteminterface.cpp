...
condition|)
block|{
comment|// Check if the shortcut is overridden by some object in the event delivery path (typically the focus object).
comment|// If so, we should not look up the shortcut in the shortcut map, but instead deliver the event as a regular
comment|// key event, so that the target that accepted the shortcut override event can handle it. Note that we only
comment|// do this if the shortcut map hasn't found a partial shortcut match yet. If it has, the shortcut can not be
comment|// overridden.
name|QWindowSystemInterfacePrivate
operator|::
name|KeyEvent
modifier|*
name|shortcutOverrideEvent
init|=
operator|new
name|QWindowSystemInterfacePrivate
operator|::
...
decl_stmt|;
block|{
comment|// FIXME: Template handleWindowSystemEvent to support both sync and async delivery
...
block|}
block|}
comment|// The shortcut event is dispatched as a QShortcutEvent, not a QKeyEvent, but we use
comment|// the QKeyEvent as a container for the various properties that the shortcut map needs
comment|// to inspect to determine if a shortcut matched the keys that were pressed.
name|QKeyEvent
name|keyEvent
argument_list|(
name|QEvent
...
directive|ifndef
name|QT_NO_SHORTCUT
comment|// FIXME: This method should not allow targeting a specific object, but should
comment|// instead forward the event to a window, which then takes care of normal event
comment|// propagation. We need to fix a lot of tests before we can refactor this (the
comment|// window needs to be exposed and active and have a focus object), so we leave
comment|// it as is for now. See QTBUG-48577.
name|QGuiApplicationPrivate
operator|::
...
condition|)
block|{
comment|// Try sending as QKeyEvent::ShortcutOverride first
...
return|;
block|}
comment|// Then as QShortcutEvent
return|return
name|shortcutMap
operator|.
name|tryShortcut
