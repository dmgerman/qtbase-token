...
end_function
begin_comment
comment|/*! \internal     Uses nextState(QKeyEvent) to check for a grabbed shortcut.      If so, it is dispatched using dispatchEvent().      Returns true if a shortcut handled the event.      \sa nextState, dispatchEvent */
end_comment
begin_function
...
name|NoMatch
case|:
comment|// In the case of going from a partial match to no match we handled the
comment|// event, since we already stated that we did for the partial match. But
comment|// in the normal case of directly going to no match we say we didn't.
return|return
name|previousState
operator|==
name|QKeySequence
...
name|PartialMatch
case|:
comment|// For a partial match we don't know yet if we will handle the shortcut
comment|// but we need to say we did, so that we get the follow-up key-presses.
return|return
literal|true
return|;
case|case
