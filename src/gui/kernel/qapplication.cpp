...
begin_comment
DECL|member|graphics_system
comment|// default graphics system
end_comment
begin_decl_stmt
...
begin_comment
DECL|member|graphics_system_name
comment|// graphics system id - for delayed initialization
end_comment
begin_decl_stmt
...
argument_list|()
expr_stmt|;
comment|// the environment variable has the lowest precedence of runtime graphicssystem switches
if|if
condition|(
name|graphics_system_name
operator|.
...
argument_list|>
argument_list|(
literal|"libmeegotouchcore"
argument_list|)
argument_list|)
decl_stmt|;
name|bool
name|linksWithMeeGoGraphicsSystemHelper
init|=
name|dl_iterate_phdr
argument_list|(
name|qt_matchLibraryName
...
argument_list|>
argument_list|(
literal|"libQtMeeGoGraphicsSystemHelper"
argument_list|)
argument_list|)
decl_stmt|;
if|if
...
name|qWarning
argument_list|(
literal|"Running non-meego graphics system enabled  MeeGo touch, forcing native graphicssystem\n"
argument_list|)
expr_stmt|;
name|graphics_system_name
operator|=
...
name|Q_WS_QPA
argument_list|)
comment|// initialize the graphics system - on X11 this is initialized inside
comment|// qt_init() in qapplication_x11.cpp because of several reasons.
comment|// On QWS, the graphics system is set by the QScreen plugin.
comment|// We don't use graphics systems in Qt QPA
name|graphics_system
operator|=
...
end_comment
begin_comment
comment|/*!     \fn bool QApplication::isRightToLeft()      Returns true if the application's layout direction is     Qt::RightToLeft; otherwise returns false.      \sa layoutDirection(), isLeftToRight() */
end_comment
begin_comment
comment|/*!     \fn bool QApplication::isLeftToRight()      Returns true if the application's layout direction is     Qt::LeftToRight; otherwise returns false.      \sa layoutDirection(), isRightToLeft() */
end_comment
begin_comment
comment|/*!     \property QApplication::layoutDirection     \brief the default layout direction for this application      On system start-up, the default layout direction depends on the     application's language.      \sa QWidget::layoutDirection, isLeftToRight(), isRightToLeft()  */
end_comment
begin_function
...
end_endif
begin_comment
comment|/*!     \fn QCursor *QApplication::overrideCursor()      Returns the active application override cursor.      This function returns 0 if no application cursor has been defined (i.e. the     internal cursor stack is empty).      \sa setOverrideCursor(), restoreOverrideCursor() */
end_comment
begin_ifndef
ifndef|#
directive|ifndef
...
end_endif
begin_comment
comment|/*!     \fn void QApplication::setOverrideCursor(const QCursor&cursor, bool replace)      Use changeOverrideCursor(\a cursor) (if \a replace is true) or     setOverrideCursor(\a cursor) (if \a replace is false). */
end_comment
begin_comment
