...
end_destructor
begin_comment
comment|/*!     \fn Qt::KeyboardModifiers QInputEvent::modifiers() const      Returns the keyboard modifier flags that existed immediately     before the event occurred.      \sa QGuiApplication::keyboardModifiers() */
end_comment
begin_comment
...
end_constructor
begin_comment
comment|/*!     \since 5.6      Constructs a mouse event object.      The \a type parameter must be QEvent::MouseButtonPress,     QEvent::MouseButtonRelease, QEvent::MouseButtonDblClick,     or QEvent::MouseMove.      The points \a localPos, \a windowPos and \a screenPos specify the     mouse cursor's position relative to the receiving widget or item,     window, and screen, respectively.      The \a button that caused the event is given as a value from the     \l Qt::MouseButton enum. If the event \a type is \l MouseMove,     the appropriate button for this event is Qt::NoButton. \a buttons     is the state of all buttons at the time of the event, \a modifiers     is the state of all keyboard modifiers.      The source of the event is specified by \a source.  */
end_comment
begin_constructor
DECL|function|QMouseEvent
name|QMouseEvent
...
end_comment
begin_comment
comment|/*!     \fn QPointF QMouseEvent::screenPos() const      \since 5.0      Returns the position of the mouse cursor as a QPointF, relative to the     screen that received the event.      \sa x(), y(), pos(), localPos(), windowPos() */
end_comment
begin_comment
...
end_comment
begin_comment
comment|/*!     \fn QString QKeyEvent::text() const      Returns the Unicode text that this key generated.      Return values when modifier keys such as     Shift, Control, Alt, and Meta are pressed     differ among platforms and could return an empty string.      \note \l key() will always return a valid value,     independent of modifier keys.      \sa Qt::WA_KeyCompression */
end_comment
begin_comment
comment|/*!     Returns the keyboard modifier flags that existed immediately     after the event occurred.      \warning This function cannot always be trusted. The user can     confuse it by pressing both \uicontrol{Shift} keys simultaneously and     releasing one of them, for example.      \sa QGuiApplication::keyboardModifiers() */
end_comment
begin_function
...
end_destructor
begin_comment
comment|/*!    \class QIconDragEvent    \brief The QIconDragEvent class indicates that a main icon drag has begun.     \inmodule QtGui     \ingroup events     Icon drag events are sent to widgets when the main icon of a window    has been dragged away. On OS X, this happens when the proxy    icon of a window is dragged off the title bar.     It is normal to begin using drag and drop in response to this    event.     \sa {Drag and Drop}, QMimeData, QDrag */
end_comment
begin_comment
...
end_destructor
begin_comment
comment|/*!     Returns the button that caused the event.      Note that the returned value is always Qt::NoButton for \l TabletMove,     \l TabletEnterProximity and \l TabletLeaveProximity events.      \sa buttons(), Qt::MouseButton */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!     Returns the button state when the event was generated. The button state is     a combination of buttons from the \l Qt::MouseButton enum using the OR     operator. For \l TabletMove events, this is all buttons that are pressed     down. For \l TabletPress events this includes the button that caused the     event. For \l TabletRelease events this excludes the button that caused the     event.      \sa button(), Qt::MouseButton */
end_comment
begin_function
...
end_comment
begin_comment
comment|/*!     \fn qreal QTabletEvent::rotation() const      Returns the rotation of the current device in degress. This is usually     given by a 4D Mouse. If the device does not support rotation this value is     always 0.0.  */
end_comment
begin_comment
...
end_comment
begin_comment
comment|/*!     Constructs a native gesture event of type \a type.      The points \a localPos, \a windowPos and \a screenPos specify the     gesture position relative to the receiving widget or item,     window, and screen, respectively.      \a realValue is the OS X event parameter, \a sequenceId and \a intValue are the Windows event parameters. */
end_comment
begin_constructor
...
end_constructor
begin_comment
comment|/*!     \fn QNativeGestureEvent::gestureType() const     \since 5.2      Returns the gesture type. */
end_comment
begin_comment
comment|/*!     \fn QNativeGestureEvent::value() const     \since 5.2      Returns the gesture value. The value should be interpreted based on the     gesture type. For example, a Zoom gesture provides a scale factor while a Rotate     gesture provides a rotation delta.      \sa QNativeGestureEvent, gestureType() */
end_comment
begin_comment
comment|/*!     \fn QPoint QNativeGestureEvent::globalPos() const     \since 5.2      Returns the position of the gesture as a QPointF in screen coordinates */
end_comment
begin_comment
comment|/*!     \fn QPoint QNativeGestureEvent::pos() const     \since 5.2      Returns the position of the mouse cursor, relative to the widget     or item that received the event. */
end_comment
begin_comment
comment|/*!     \fn QPointF QNativeGestureEvent::localPos() const     \since 5.2      Returns the position of the gesture as a QPointF, relative to the     widget or item that received the event. */
end_comment
begin_comment
comment|/*!     \fn QPointF QNativeGestureEvent::screenPos() const     \since 5.2      Returns the position of the gesture as a QPointF in screen coordinates. */
end_comment
begin_comment
...
end_ifndef
begin_comment
comment|/*!     \internal     \class QToolBarChangeEvent     \brief The QToolBarChangeEvent class provides an event that is     sent whenever a the toolbar button is clicked on OS X.      \ingroup events     \inmodule QtGui      The QToolBarChangeEvent is sent when the toolbar button is clicked. On Mac     OS X, this is the long oblong button on the right side of the window     title bar. The default implementation is to toggle the appearance (hidden or     shown) of the associated toolbars for the window. */
end_comment
begin_comment
...
name|d
operator|<<
literal|"QInputMethodQueryEvent(queries="
operator|<<
name|showbase
operator|<<
name|hex
...
case|:
return|return
literal|"QWindowStateChangeEvent"
return|;
case|case
name|QEvent
operator|::
...
case|:
return|return
literal|"QScreenOrientationChangeEvent"
return|;
case|case
name|QEvent
operator|::
...
case|:
return|return
literal|"QGraphicsSceneMouseEvent"
return|;
case|case
name|QEvent
operator|::
...
name|dbg
operator|<<
literal|"QApplicationStateChangeEvent("
expr_stmt|;
name|QtDebugUtils
operator|::
name|formatQEnum
...
name|dbg
operator|<<
literal|"QPlatformSurfaceEvent(surfaceEventType="
expr_stmt|;
switch|switch
condition|(
cast|static_cast
...
name|dbg
operator|<<
literal|"SurfaceAboutToBeDestroyed"
expr_stmt|;
break|break;
block|}
name|dbg
...
end_comment
begin_comment
comment|/*!     \class QShortcutEvent     \brief The QShortcutEvent class provides an event which is generated when     the user presses a key combination.      \ingroup events     \inmodule QtGui      Normally you do not need to use this class directly; QShortcut     provides a higher-level interface to handle shortcut keys.      \sa QShortcut */
end_comment
begin_comment
...
end_function
begin_comment
comment|/*!     Creates a new QApplicationStateChangeEvent.     \a applicationState is the new state. */
end_comment
begin_constructor
...
end_constructor
begin_comment
comment|/*!     Returns the state of the application. */
end_comment
begin_function
