...
end_ifdef
begin_comment
comment|/*!     Use QMouseEvent(\a type, \a pos, \a button, \c buttons, \c     modifiers) instead, where \c buttons is \a state&     Qt::MouseButtonMask and \c modifiers is \a state&     Qt::KeyButtonMask. */
end_comment
begin_constructor
DECL|function|QMouseEvent
name|QMouseEvent
...
end_constructor
begin_comment
comment|/*!     Use QMouseEvent(\a type, \a pos, \a globalPos, \a button,     \c buttons, \c modifiers) instead, where     \c buttons is \a state& Qt::MouseButtonMask and     \c modifiers is \a state& Qt::KeyButtonMask. */
end_comment
begin_constructor
DECL|function|QMouseEvent
name|QMouseEvent
...
end_ifdef
begin_comment
comment|/*!     Use one of the other constructors instead. */
end_comment
begin_constructor
DECL|function|QWheelEvent
name|QWheelEvent
...
end_ifdef
begin_comment
comment|/*!     \fn QKeyEvent::QKeyEvent(Type type, int key, int ascii,                              int modifiers, const QString&text,                              bool autorep, ushort count)      Use one of the other constructors instead. */
end_comment
begin_comment
comment|/*!     \fn int QKeyEvent::ascii() const      Use text() instead. */
end_comment
begin_comment
comment|/*!     \fn Qt::ButtonState QKeyEvent::state() const      Use QInputEvent::modifiers() instead. */
end_comment
begin_comment
comment|/*!     \fn Qt::ButtonState QKeyEvent::stateAfter() const      Use modifiers() instead. */
end_comment
begin_endif
endif|#
directive|endif
...
end_ifdef
begin_comment
comment|/*!     \enum QFocusEvent::Reason     \compat      Use Qt::FocusReason instead.      \value Mouse  Same as Qt::MouseFocusReason.     \value Tab  Same as Qt::TabFocusReason.     \value Backtab  Same as Qt::BacktabFocusReason.     \value MenuBar  Same as Qt::MenuBarFocusReason.     \value ActiveWindow  Same as Qt::ActiveWindowFocusReason     \value Other  Same as Qt::OtherFocusReason     \value Popup  Same as Qt::PopupFocusReason     \value Shortcut  Same as Qt::ShortcutFocusReason */
end_comment
begin_endif
endif|#
directive|endif
...
end_ifdef
begin_comment
comment|/*!     Constructs a paint event object with both a \a paintRegion and a     \a paintRect, both of which represent the area of the widget that     needs to be updated.  */
end_comment
begin_constructor
DECL|function|QPaintEvent
name|QPaintEvent
...
end_ifdef
begin_comment
comment|/*!     Constructs a context menu event with the given \a reason for the     position specified by \a pos in widget coordinates and \a globalPos     in global screen coordinates. \a dummy is ignored. */
end_comment
begin_constructor
DECL|function|QContextMenuEvent
name|QContextMenuEvent
...
end_ifdef
begin_comment
comment|/*!     Constructs a context menu event with the given \a reason for the     position specified by \a pos in widget coordinates. \a dummy is     ignored. */
end_comment
begin_constructor
DECL|function|QContextMenuEvent
name|QContextMenuEvent
...
end_function
begin_comment
comment|/*!     \compat     Returns a string describing one of the available data types for     this drag. Common examples are "text/plain" and "image/gif".     If \a n is less than zero or greater than the number of available     data types, format() returns 0.      This function is provided mainly for debugging. Most drop targets     will use provides().      \sa data() provides() */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!     \compat     Returns true if this event provides format \a mimeType; otherwise     returns false.      \sa data() */
end_comment
begin_function
...
end_ifdef
begin_comment
comment|/*!     Use dropAction() instead.      The table below shows the correspondance between the return type     of action() and the return type of dropAction().      \table     \header \i Old enum value   \i New enum value     \row    \i QDropEvent::Copy \i Qt::CopyAction     \row    \i QDropEvent::Move \i Qt::MoveAction     \row    \i QDropEvent::Link \i Qt::LinkAction     \row    \i other            \i Qt::CopyAction     \endtable */
end_comment
begin_function
...
end_destructor
begin_comment
comment|/*!   \fn QByteArray QDropEvent::data(const char* f) const    \obsolete    The encoded data is in \a f.   Use QDropEvent::encodedData(). */
end_comment
begin_comment
...
argument_list|)
decl_stmt|;
comment|// Duplicate here allows the file handle to be valid after S60 app construction is complete.
...
name|dup
decl_stmt|;
comment|// Duplicate here means that the opened QFile will continue to be valid beyond the lifetime of this QFileOpenEvent.
comment|// It also allows openFile to be used in threads other than the thread in which the QFileOpenEvent was created.
if|if
condition|(
name|dup
operator|.
...
end_ifdef
begin_comment
comment|/*!     \class QMenubarUpdatedEvent     \internal     Event sent by QMenuBar to tell Q3Workspace to update itself. */
end_comment
begin_comment
comment|/*! \internal  */
end_comment
...
end_constructor
begin_comment
comment|/*!     \fn QMenuBar *QMenubarUpdatedEvent::menuBar()     \internal */
end_comment
begin_comment
comment|/*!     \fn bool operator==(QKeyEvent *e, QKeySequence::StandardKey key)      \relates QKeyEvent      Returns true if \a key is currently bound to the key combination     specified by \a e.      Equivalent to \c {e->matches(key)}. */
end_comment
begin_comment
comment|/*!     \fn bool operator==(QKeySequence::StandardKey key, QKeyEvent *e)      \relates QKeyEvent      Returns true if \a key is currently bound to the key combination     specified by \a e.      Equivalent to \c {e->matches(key)}. */
end_comment
begin_comment
comment|/*!     \internal      \class QKeyEventEx     \ingroup events      \brief The QKeyEventEx class provides more extended information about a keyevent.      This class is for internal use only, and exists to aid the shortcut system on     various platforms to get all the information it needs. */
end_comment
begin_endif
endif|#
directive|endif
...
end_function
begin_comment
comment|/*!     Returns true if this touch point is the primary touch point. The primary touch point is the     point for which the windowing system generates mouse events. */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!     \enum QScreenOrientationChangeEvent::Orientation      This enum describes the orientations that a device can have.      \value Portrait The device is in a position where its top edge is pointing up.      \value Landscape The device is rotated clockwise 90 degrees, so that its left edge is pointing up.      \value PortraitInverted The device is rotated 180 degrees, so that its bottom edge is pointing up.      \value LandscapeInverted The device is counterclockwise 90 degrees, so that its right edge is pointing up.      \sa QScreenOrientationChangeEvent::orientation()     \sa QScreenOrientationChangeEvent::orientationInDegrees() */
end_comment
begin_comment
comment|/*!     Creates a new QScreenOrientationChangeEvent     \a screenOrientationInDegrees is the new screen orientation, expressed in degrees.     The orientation must be expressed in steps of 90 degrees. */
end_comment
begin_constructor
...
parameter_list|(
name|qint32
name|screenOrientationInDegrees
parameter_list|)
member_init_list|:
name|QEvent
argument_list|(
...
literal|90
decl_stmt|;
comment|// flip around the negative coords to correct order.
if|if
condition|(
name|screenOrientationInDegrees
operator|<
...
end_function
begin_comment
comment|/*!     Returns the screen orientation in degrees.     The orientation is expressed in steps of 90 degrees and depends on the previous value of the orientation.     This is intended to allow for smooth animations from one orientation to the other. */
end_comment
begin_function
