...
name|newScreen
operator|<<
literal|"): Attempt to set a screen on a child window."
expr_stmt|;
return|return;
block|}
if|if
...
argument_list|)
expr_stmt|;
comment|// Handling of the -qwindowgeometry, -geometry command line arguments
specifier|static
name|bool
...
end_function
begin_comment
comment|/*!     Returns the ratio between physical pixels and device-independent pixels     for the window. This value is dependent on the screen the window is on,     and may change when the window is moved.      Common values are 1.0 on normal displays and 2.0 on Apple "retina" displays.      \note For windows not backed by a platform window, meaning that create() was not     called, the function will fall back to QGuiApplication::devicePixelRatio() which in     turn returns the highest screen device pixel ratio found on the system.      \sa QScreen::devicePixelRatio(), QGuiApplication::devicePixelRatio() */
end_comment
begin_function
...
argument_list|)
expr_stmt|;
comment|// If there is no platform window use the app global devicePixelRatio,
comment|// which is the the highest devicePixelRatio found on the system
comment|// screens, and will be correct for single-display systems (a very common case).
if|if
condition|(
operator|!
name|d
...
name|parent
operator|<<
literal|"must be a top level window."
expr_stmt|;
return|return;
block|}
name|d
...
end_function
begin_comment
comment|/*   This is equivalent to QPlatformWindow::screenForGeometry, but in platform   independent coordinates. The duplication is unfortunate, but there is a   chicken and egg problem here: we cannot convert to native coordinates   before we know which screen we are on. */
end_comment
begin_function
...
literal|0
expr_stmt|;
comment|// We flush before deleting the platform window so that any pending activation
comment|// events for the window will be delivered.
...
name|platformWindow
expr_stmt|;
comment|// Then we flush again so that if the platform plugin sent any deactivation
comment|// events as a result of being destroyed, we can pick that up by looking at
comment|// QGuiApplicationPrivate::focus_window, which will be up to date.
name|QWindowSystemInterface
operator|::
name|flushWindowSystemEvents
argument_list|()
...
literal|false
expr_stmt|;
comment|// Ensure Qt doesn't refer to a destroyed QWindow if the platform plugin
comment|// didn't reset the window activation or other states as part of setVisible
comment|// or its destruction. We make a best guess of transferring to the parent
comment|// window, as this is what most window managers will do. We go through the
comment|// QWindowSystemInterface so that the proper signals and events are sent
comment|// as a result of the reset.
if|if
condition|(
name|QGuiApplicationPrivate
operator|::
...
argument_list|)
expr_stmt|;
comment|// FIXME: Handle these two though QPA like the others. Unfortunately both
comment|// processMouseEvent and processTabletEvent in QGuiApplication have conditions
comment|// that make sending the event though QPA not feasable right now.
if|if
condition|(
name|QGuiApplicationPrivate
operator|::
