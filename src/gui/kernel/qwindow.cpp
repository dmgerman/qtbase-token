...
argument_list|)
expr_stmt|;
comment|// The child may have had deferred creation due to this window not being created
comment|// at the time setVisible was called, so we re-apply the visible state, which
comment|// may result in creating the child, and emitting the appropriate signals.
if|if
condition|(
name|childWindow
operator|->
...
condition|)
block|{
comment|// Visibility hasn't changed, and the platform window is in sync
return|return;
block|}
if|if
condition|(
...
condition|)
block|{
comment|// If we have a parent window, but the parent hasn't been created yet, we
comment|// can defer creation until the parent is created or we're re-parented.
if|if
condition|(
name|parent
argument_list|()
...
argument_list|)
expr_stmt|;
comment|// If we were set visible, but not created because we were a child, and we're now
comment|// re-parented into a created parent, or to being a top level, we need re-apply the
comment|// visibility state, which will also create.
if|if
condition|(
name|isVisible
argument_list|()
