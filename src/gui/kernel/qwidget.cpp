...
end_function
begin_comment
comment|/*!   This function sets the input context \a context   on this widget.    Qt takes ownership of the given input \a context.    \sa inputContext() */
end_comment
begin_function
...
condition|)
block|{
comment|// Don't waste GPU mem for unnecessary large egl surface until resized by application
name|data
operator|.
name|crect
operator|=
...
condition|)
block|{
comment|// Okay, we are about to destroy the top-level window that owns
comment|// the backing store. Make sure we delete the backing store right away
comment|// before the window handle is invalid. This is important because
comment|// the backing store will delete its window surface, which may or may
comment|// not have a reference to this widget that will be used later to
comment|// notify the window it no longer has a surface.
name|d
operator|->
name|extra
operator|->
...
directive|ifdef
name|QT3_SUPPORT
comment|/*!     \overload      Sets the widget's GUI style to \a style using the QStyleFactory. */
DECL|function|setStyle
name|QStyle
modifier|*
name|QWidget
...
condition|)
block|{
comment|// ### This is really just a work-around. Layout shouldn't be
comment|// asking for minimum sizes bigger than the screen.
if|if
condition|(
name|minw
operator|>
...
directive|ifdef
name|Q_WS_QWS
comment|// temporary fix for 4.3.x.
comment|// Should move the embedded spesific contraints in setMinimumSize_helper into QLayout
name|int
name|tmpW
init|=
name|w
...
directive|ifdef
name|QT3_SUPPORT
comment|/*!     Returns the color role used for painting the widget's background.      Use QPalette(backgroundRole(()) instead. */
...
return|;
block|}
comment|/*!     \fn void QWidget::setBackgroundMode(Qt::BackgroundMode     widgetBackground, Qt::BackgroundMode paletteBackground)      Sets the color role used for painting the widget's background to     background mode \a widgetBackground. The \a paletteBackground mode     parameter is ignored. */
...
expr_stmt|;
block|}
comment|/*!     The widget mapper is no longer part of the public API. */
...
directive|ifdef
name|QT3_SUPPORT
comment|/*!     Use visibleRegion() instead. */
...
name|hints
expr_stmt|;
comment|// Optimization to update input context only it has already been created.
if|if
condition|(
name|d
operator|->
...
name|Q_WS_QWS
argument_list|)
comment|/*!     \fn bool QWidget::qwsEvent(QWSEvent *event)      This special event handler can be reimplemented in a subclass to     receive native Qt for Embedded Linux events which are passed in the     \a event parameter.      In your reimplementation of this function, if you want to stop the     event being handled by Qt, return true. If you return false, this     native event is passed back to Qt, which translates the event into     a Qt event and sends it to the widget.      \warning This function is not portable.      \sa QApplication::qwsEventFilter() */
...
directive|ifdef
name|QT3_SUPPORT
comment|/*!     Clear the rectangle at point (\a x, \a y) of width \a w and height     \a h.      \warning This is best done in a paintEvent(). */
...
block|}
block|}
comment|/*!     \overload      Clear the given region, \a rgn.      Drawing may only take place in a QPaintEvent. Overload     paintEvent() to do your erasing and call update() to schedule a     replaint whenever necessary. See also QPainter. */
DECL|function|erase
name|void
name|QWidget
operator|::
...
expr_stmt|;
block|}
comment|/*!     Closes the widget.      Use the no-argument overload instead. */
DECL|function|close
name|bool
name|QWidget
operator|::
...
expr_stmt|;
block|}
comment|/*!     Return's the widget's icon.      Use windowIcon() instead. */
DECL|function|icon
specifier|const
name|QPixmap
modifier|*
...
directive|ifdef
name|Q_WS_QWS
comment|// Release the region for this window from qws if the widget has
comment|// been shown before the attribute was set.
if|if
condition|(
name|QWSWindowSurface
modifier|*
...
name|CAknAppUiBase
operator|::
name|EAppUiOrientationAutomatic
block|}
decl_stmt|;
name|CAknAppUiBase
operator|::
...
argument_list|)
expr_stmt|;
comment|// and optimization to update input context only it has already been created.
if|if
condition|(
name|d
operator|->
...
name|Q_WS_WIN
argument_list|)
comment|/*!     Returns the window system handle of the widget, for low-level     access. Using this function is not portable.      An HDC acquired with getDC() has to be released with releaseDC().      \warning Using this function is not portable. */
DECL|function|getDC
name|HDC
name|QWidget
operator|::
...
return|;
block|}
comment|/*!     Releases the HDC \a hdc acquired by a previous call to getDC().      \warning Using this function is not portable. */
...
argument_list|)
expr_stmt|;
comment|// If its the widgets own dc, it will be released elsewhere. If
comment|// its a different HDC we release it and issue a warning if it
comment|// fails.
if|if
condition|(
name|hdc
...
name|qErrnoWarning
argument_list|(
literal|"QWidget::releaseDC(): failed to release HDC"
argument_list|)
expr_stmt|;
block|}
else|#
directive|else
comment|/*!     Returns the window system handle of the widget, for low-level     access. Using this function is not portable.      The HANDLE type varies with platform; see \c qwindowdefs.h for     details. */
...
directive|endif
comment|/*!     \preliminary     \since 4.2     \obsolete      Sets the window surface to be the \a surface specified.     The QWidget takes will ownership of the \a surface.     widget itself is deleted. */
...
directive|endif
block|}
comment|/*!     \preliminary     \since 4.2      Returns the QWindowSurface this widget will be drawn into. */
...
expr_stmt|;
block|}
comment|/*! \fn const QX11Info&QWidget::x11Info() const     Returns information about the configuration of the X display used to display     the widget.      \warning This function is only available on X11. */
