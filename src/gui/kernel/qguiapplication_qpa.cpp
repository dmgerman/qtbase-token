...
directive|if
literal|0
block|if (app_do_modal&& !qt_try_modal(mouseWindow, QEvent::Wheel) ) {          qDebug()<< "modal blocked wheel event"<< mouseWindow;          return;      }      QPoint p = mouseWindow->mapFromGlobal(globalPoint);      QWidget *w = mouseWindow->childAt(p);      if (w) {          mouseWidget = w;          p = mouseWidget->mapFromGlobal(globalPoint);      }       QWheelEvent ev(p, globalPoint, e->delta, buttons, QGuiApplication::keyboardModifiers(),                    e->orient);      QGuiApplication::sendSpontaneousEvent(mouseWidget,&ev);
endif|#
directive|endif
block|}
end_function
...
directive|if
literal|0
block|if (self->inPopupMode()) {         QWidget *popupW = qApp->activePopupWidget();         focusW = popupW->focusWidget() ? popupW->focusWidget() : popupW;     }     if (!focusW)         focusW = QGuiApplication::focusWidget();
endif|#
directive|endif
if|if
condition|(
...
directive|if
literal|0
block|if (!focusW)         focusW = QGuiApplication::activeWindow();
endif|#
directive|endif
...
directive|if
literal|0
block|if (app_do_modal&& !qt_try_modal(focusW, e->keyType))         return;
endif|#
directive|endif
if|if
condition|(
