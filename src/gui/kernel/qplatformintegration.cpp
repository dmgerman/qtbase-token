...
specifier|const
block|{
comment|// Leave popup-windows as is
if|if
condition|(
name|flags
operator|&
...
end_function
begin_comment
comment|/*!   Just removes the screen, call destroyScreen instead.    \sa destroyScreen() */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!   Should be called by the implementation whenever a screen is removed.    This removes the screen from QGuiApplication::screens(), and deletes it.    Failing to call this and manually deleting the QPlatformScreen instead may   lead to a crash due to a pure virtual call. */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!   Should be called whenever the primary screen changes.    When the screen specified as primary changes, this method will notify   QGuiApplication and emit the QGuiApplication::primaryScreenChanged signal.  */
end_comment
begin_function
...
end_ifndef
begin_comment
comment|/*!   Platform integration function for querying the OpenGL implementation type.    Used only when dynamic OpenGL implementation loading is enabled.    Subclasses should reimplement this function and return a value based on   the OpenGL implementation they have chosen to load.    \note The return value does not indicate or limit the types of   contexts that can be created by a given implementation. For example   a desktop OpenGL implementation may be capable of creating OpenGL   ES-compatible contexts too.    \sa QOpenGLContext::openGLModuleType(), QOpenGLContext::isOpenGLES()    \since 5.3  */
end_comment
begin_function
...
end_endif
begin_comment
comment|/*!     \since 5.5      Platform integration function for setting the application icon.      \sa QGuiApplication::setWindowIcon() */
end_comment
begin_function
