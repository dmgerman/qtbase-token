...
literal|"QShortcut"
argument_list|,
literal|"Microphone Volume Down"
argument_list|)
block|}
block|,
block|{
...
end_comment
begin_comment
comment|/*!     \fn QKeySequence&QKeySequence::operator=(QKeySequence&&other)      Move-assigns \a other to this QKeySequence instance.      \since 5.2 */
end_comment
begin_comment
...
literal|4
argument_list|,
literal|"Change docs and ctor impl below"
argument_list|)
expr_stmt|;
end_expr_stmt
begin_comment
...
end_function
begin_comment
comment|/*!     Returns \c true if the key sequence is empty; otherwise returns     false. */
end_comment
begin_function
...
comment|// if we have MaxKeyCount keys before the end.
while|while
condition|(
name|keyseq
operator|.
...
condition|)
block|{
comment|// On OS X the order (by default) is Meta, Alt, Shift, Control.
...
end_function
begin_comment
comment|/*!     \internal     Returns the text representation of the key \a key, which can be used i.e.     when the sequence is serialized. This does not take modifiers into account     (see encodeString() for a version that does).      This static method is used by encodeString() and by the D-Bus menu exporter. */
end_comment
begin_function
...
end_comment
begin_comment
comment|/*!     \fn bool QKeySequence::operator!=(const QKeySequence&other) const      Returns \c true if this key sequence is not equal to the \a other     key sequence; otherwise returns \c false. */
end_comment
begin_comment
comment|/*!     Returns \c true if this key sequence is equal to the \a other     key sequence; otherwise returns \c false.  */
end_comment
begin_function
DECL|function|operator ==
name|bool
...
end_function
begin_comment
comment|/*!     \since 5.6      Calculates the hash value of \a key, using     \a seed to seed the calculation. */
end_comment
begin_function
name|uint
name|qHash
...
end_function
begin_comment
comment|/*!     Provides an arbitrary comparison of this key sequence and     \a other key sequence. All that is guaranteed is that the     operator returns \c false if both key sequences are equal and     that (ks1 \< ks2) == !( ks2 \< ks1) if the key sequences     are not equal.      This function is useful in some circumstances, for example     if you want to use QKeySequence objects as keys in a QMap.      \sa operator==(), operator!=(), operator>(), operator<=(), operator>=() */
end_comment
begin_function
DECL|function|operator <
name|bool
...
end_function
begin_comment
comment|/*!     \fn bool QKeySequence::operator> (const QKeySequence&other) const      Returns \c true if this key sequence is larger than the \a other key     sequence; otherwise returns \c false.      \sa operator==(), operator!=(), operator<(), operator<=(), operator>=() */
end_comment
begin_comment
comment|/*!     \fn bool QKeySequence::operator<= (const QKeySequence&other) const      Returns \c true if this key sequence is smaller or equal to the     \a other key sequence; otherwise returns \c false.      \sa operator==(), operator!=(), operator<(), operator>(), operator>=() */
end_comment
begin_comment
comment|/*!     \fn bool QKeySequence::operator>= (const QKeySequence&other) const      Returns \c true if this key sequence is larger or equal to the     \a other key sequence; otherwise returns \c false.      \sa operator==(), operator!=(), operator<(), operator>(), operator<=() */
end_comment
begin_comment
comment|/*!     \internal */
end_comment
...
end_function
begin_comment
comment|/*!     \since 4.1      Return a string representation of the key sequence,     based on \a format.      For example, the value Qt::CTRL+Qt::Key_O results in "Ctrl+O".     If the key sequence has multiple key codes, each is separated     by commas in the string returned, such as "Alt+X, Ctrl+Y, Z".     The strings, "Ctrl", "Shift", etc. are translated using     QObject::tr() in the "QShortcut" context.      If the key sequence has no keys, an empty string is returned.      On OS X, the string returned resembles the sequence that is     shown in the menu bar.      \sa fromString() */
end_comment
begin_function
...
literal|4
argument_list|,
literal|"Forgot to adapt QDataStream&operator<<(QDataStream&s, const QKeySequence&keysequence) to new QKeySequence::MaxKeyCount"
argument_list|)
expr_stmt|;
specifier|const
name|bool
...
name|qWarning
argument_list|(
literal|"Premature EOF while reading QKeySequence"
argument_list|)
expr_stmt|;
return|return
name|s
