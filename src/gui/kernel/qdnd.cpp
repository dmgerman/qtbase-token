...
argument_list|()
operator|<<
literal|"setting cursor shape to"
operator|<<
name|cursorShape
expr_stmt|;
name|qApp
...
end_function
begin_comment
comment|/*!   Called from startDrag() in QPlatformDrag implementations that do not need   the desktop-oriented stuff provided by the event filter (e.g. because their   drag is not based on mouse events). Instead, they will manage everything on   their own, will not rely on move/drop/cancel, and will call stopDrag() to stop   the event loop when the drag is over.  */
end_comment
begin_function
