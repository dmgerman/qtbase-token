...
name|qWarning
argument_list|(
literal|"QLayout \"%s\" added to %s \"%s\", which already has a layout"
argument_list|,
name|QObject
operator|::
name|objectName
...
end_constructor
begin_comment
comment|/*!     Constructs a new child QLayout called \a name, and places it     inside \a parentLayout by using the default placement defined by     addItem().      If \a spacing is -1, this QLayout inherits \a parentLayout's     spacing(), otherwise the value of \a spacing is used. */
end_comment
begin_constructor
DECL|function|QLayout
name|QLayout
...
end_constructor
begin_comment
comment|/*!     Constructs a new child QLayout called \a name. If \a spacing is     -1, this QLayout inherits its parent's spacing(); otherwise the     value of \a spacing is used.      This layout has to be inserted into another layout before geometry     management will work. */
end_comment
begin_constructor
DECL|function|QLayout
name|QLayout
...
end_constructor
begin_comment
comment|/*!     Automatically adding widgets is deprecated. Use addWidget() or     addLayout() instead. */
end_comment
begin_function
...
condition|)
block|{
comment|//SizeGrip is handled by the dialog itself.
block|}
else|else
endif|#
directive|endif
...
end_ifdef
begin_comment
comment|/*!     Removes and deletes all items in this layout. */
end_comment
begin_function
...
argument_list|)
expr_stmt|;
comment|// layout will not change min/max size
name|QWidget
modifier|*
name|parent
init|=
...
end_function
begin_comment
comment|/*!     \enum QLayout::SizeConstraint      The possible values are:      \value SetDefaultConstraint The main widget's minimum size is set                     to minimumSize(), unless the widget already has                     a minimum size.      \value SetFixedSize The main widget's size is set to sizeHint(); it                     cannot be resized at all.     \value SetMinimumSize  The main widget's minimum size is set to                     minimumSize(); it cannot be smaller.      \value SetMaximumSize  The main widget's maximum size is set to                     maximumSize(); it cannot be larger.      \value SetMinAndMaxSize  The main widget's minimum size is set to                     minimumSize() and its maximum size is set to                     maximumSize().      \value SetNoConstraint  The widget is not constrained.      \omitvalue Auto     \omitvalue FreeResize     \omitvalue Minimum     \omitvalue Fixed      \sa setSizeConstraint() */
end_comment
begin_comment
...
parameter_list|)
block|{
comment|/*         The control type is a flag type, with values 0x1, 0x2, 0x4, 0x8, 0x10,         etc. In memory, we pack it onto the available bits (CTSize) in         setControlType(), and unpack it here.          Example:              0x00000001 maps to 0x00000000             0x00000002 maps to 0x00000200             0x00000004 maps to 0x00000400             0x00000008 maps to 0x00000600             etc.     */
name|int
name|i
init|=
literal|0
