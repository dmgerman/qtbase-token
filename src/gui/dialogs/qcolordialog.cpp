...
directive|endif
comment|// In S60, due to small screen and different screen layouts need to re-arrange the widgets.
...
argument_list|()
operator|->
name|colorDialogSetCurrentColor
argument_list|(
name|this
argument_list|,
name|color
...
end_comment
begin_comment
comment|/*     For Symbian color dialogs */
end_comment
begin_ifdef
ifdef|#
directive|ifdef
...
end_endif
begin_comment
comment|/*!     \since 4.5      Pops up a modal color dialog with the given window \a title (or "Select Color" if none is     specified), lets the user choose a color, and returns that color. The color is initially set     to \a initial. The dialog is a child of \a parent. It returns an invalid (see     QColor::isValid()) color if the user cancels the dialog.      The \a options argument allows you to customize the dialog.      On Symbian, this static function will use the native color dialog and not a QColorDialog.     On Symbian the parameters \a title and \a parent has no relevance and the     \a options parameter is only used to define if the native color dialog is     used or not. */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!     Pops up a modal color dialog, lets the user choose a color, and     returns that color. The color is initially set to \a initial. The     dialog is a child of \a parent. It returns an invalid (see     QColor::isValid()) color if the user cancels the dialog.      On Symbian, this static function will use the native     color dialog and not a QColorDialog. */
end_comment
begin_function
DECL|function|getColor
name|QColor
