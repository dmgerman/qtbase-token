...
argument_list|)
expr_stmt|;
comment|// Set focus on the panel's focus item.
if|if
condition|(
name|QGraphicsItem
modifier|*
...
end_comment
begin_comment
comment|/*!     Renders the \a source rect from scene into \a target, using \a painter. This     function is useful for capturing the contents of the scene onto a paint     device, such as a QImage (e.g., to take a screenshot), or for printing     with QPrinter. For example:      \snippet doc/src/snippets/code/src_gui_graphicsview_qgraphicsscene.cpp 1      If \a source is a null rect, this function will use sceneRect() to     determine what to render. If \a target is a null rect, the dimensions of \a     painter's paint device will be used.      The source rect contents will be transformed according to \a     aspectRatioMode to fit into the target rect. By default, the aspect ratio     is kept, and \a source is scaled to fit in \a target.      \sa QGraphicsView::render() */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!     Calculates and returns the bounding rect of all items on the scene. This     function works by iterating over all items, and because if this, it can     be slow for large scenes.      \sa sceneRect() */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!     Returns a list of all items in the scene in descending stacking order.      \sa addItem(), removeItem(), {QGraphicsItem#Sorting}{Sorting} */
end_comment
begin_function
DECL|function|items
name|QList
...
end_function
begin_comment
comment|/*!     \obsolete      Returns all visible items at position \a pos in the scene. The items are     listed in descending stacking order (i.e., the first item in the list is the     top-most item, and the last item is the bottom-most item).      This function is deprecated and returns incorrect results if the scene     contains items that ignore transformations. Use the overload that takes     a QTransform instead.      \sa itemAt(), {QGraphicsItem#Sorting}{Sorting} */
end_comment
begin_function
DECL|function|items
name|QList
...
end_comment
begin_comment
comment|/*!     \fn QList<QGraphicsItem *> QGraphicsScene::items(qreal x, qreal y, qreal w, qreal h, Qt::ItemSelectionMode mode, Qt::SortOrder order, const QTransform&deviceTransform) const     \overload     \since 4.6      \brief Returns all visible items that, depending on \a mode, are     either inside or intersect with the rectangle defined by \a x, \a y,     \a w and \a h, in a list sorted using \a order.      \a deviceTransform is the transformation that applies to the view, and needs to     be provided if the scene contains items that ignore transformations. */
end_comment
begin_comment
comment|/*!     \fn QList<QGraphicsItem *> QGraphicsScene::items(const QPolygonF&polygon, Qt::ItemSelectionMode mode) const     \overload     \obsolete      Returns all visible items that, depending on \a mode, are either inside or     intersect with the polygon \a polygon.      The default value for \a mode is Qt::IntersectsItemShape; all items whose     exact shape intersects with or is contained by \a polygon are returned.      This function is deprecated and returns incorrect results if the scene     contains items that ignore transformations. Use the overload that takes     a QTransform instead.      \sa itemAt(), {QGraphicsItem#Sorting}{Sorting} */
end_comment
begin_function
DECL|function|items
name|QList
...
end_function
begin_comment
comment|/*!     \fn QList<QGraphicsItem *> QGraphicsScene::items(const QPainterPath&path, Qt::ItemSelectionMode mode) const     \overload     \obsolete      Returns all visible items that, depending on \a path, are either inside or     intersect with the path \a path.      The default value for \a mode is Qt::IntersectsItemShape; all items whose     exact shape intersects with or is contained by \a path are returned.      This function is deprecated and returns incorrect results if the scene     contains items that ignore transformations. Use the overload that takes     a QTransform instead.      \sa itemAt(), {QGraphicsItem#Sorting}{Sorting} */
end_comment
begin_function
DECL|function|items
name|QList
...
end_function
begin_comment
comment|/*!     \overload     \obsolete      Returns the topmost visible item at the specified \a position, or 0 if     there are no items at this position.      This function is deprecated and returns incorrect results if the scene     contains items that ignore transformations. Use the overload that takes     a QTransform instead.      \sa items(), collidingItems(), {QGraphicsItem#Sorting}{Sorting} */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!     \since 4.6      Returns the topmost visible item at the specified \a position, or 0     if there are no items at this position.      \a deviceTransform is the transformation that applies to the view, and needs to     be provided if the scene contains items that ignore transformations.      \sa items(), collidingItems(), {QGraphicsItem#Sorting}{Sorting} */
end_comment
begin_function
DECL|function|itemAt
name|QGraphicsItem
...
end_function
begin_comment
comment|/*!     \obsolete     \overload      Sets the selection area to \a path.      This function is deprecated and leads to incorrect results if the scene     contains items that ignore transformations. Use the overload that takes     a QTransform instead. */
end_comment
begin_function
DECL|function|setSelectionArea
name|void
...
end_function
begin_comment
comment|/*!     \obsolete     \overload     \since 4.3      Sets the selection area to \a path using \a mode to determine if items are     included in the selection area.      \sa clearSelection(), selectionArea() */
end_comment
begin_function
DECL|function|setSelectionArea
name|void
...
end_function
begin_comment
comment|/*!     This event handler, for event \a event, can be reimplemented in a subclass     to receive drag move events for the scene.      \sa QGraphicsItem::dragMoveEvent(), dragEnterEvent(), dragLeaveEvent(),     dropEvent() */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!     This event handler, for event \a hoverEvent, can be reimplemented in a     subclass to receive hover enter events. The default implementation     forwards the event to the topmost item that accepts hover events at the     scene position from the event.      \sa QGraphicsItem::hoverEvent(), QGraphicsItem::setAcceptHoverEvents() */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!     This event handler, for event \a mouseEvent, can be reimplemented     in a subclass to receive mouse press events for the scene.      The default implementation depends on the state of the scene. If     there is a mouse grabber item, then the event is sent to the mouse     grabber. Otherwise, it is forwarded to the topmost item that     accepts mouse events at the scene position from the event, and     that item promptly becomes the mouse grabber item.      If there is no item at the given position on the scene, the     selection area is reset, any focus item loses its input focus, and     the event is then ignored.      \sa QGraphicsItem::mousePressEvent(),     QGraphicsItem::setAcceptedMouseButtons() */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!     This event handler, for event \a wheelEvent, can be reimplemented in a     subclass to receive mouse wheel events for the scene.      By default, the event is delivered to the topmost visible item under the     cursor. If ignored, the event propagates to the item beneath, and again     until the event is accepted, or it reaches the scene. If no items accept     the event, it is ignored.      \sa QGraphicsItem::wheelEvent() */
end_comment
begin_function
