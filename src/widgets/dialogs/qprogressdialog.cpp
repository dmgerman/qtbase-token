...
function_decl|;
name|void
name|ensureSizeIsAtLeastSizeHint
parameter_list|()
function_decl|;
name|void
...
argument_list|()
expr_stmt|;
comment|// until after ensureSizeIsAtLeastSizeHint()
else|else
name|c
operator|->
name|setParent
...
end_function
begin_comment
comment|/*!   \property QProgressDialog::value   \brief the current amount of progress made.    For the progress dialog to work as expected, you should initially set   this property to QProgressDialog::minimum() and finally set it to   QProgressDialog::maximum(); you can call setValue() any number of times   in-between.    \warning If the progress dialog is modal     (see QProgressDialog::QProgressDialog()),     setValue() calls QApplication::processEvents(), so take care that     this does not cause undesirable re-entrancy in your code. For example,     don't use a QProgressDialog inside a paintEvent()!    \sa minimum, maximum */
end_comment
begin_function
...
operator|==
literal|0
comment|/* for compat with Qt< 5.4 */
operator|)
operator|||
name|progress
operator|==
