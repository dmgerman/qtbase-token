...
parameter_list|)
block|{
comment|// since it might expand to a runtime function call (to
comment|// qFlagLocations()), we cannot store the result of SIGNAL() in a
comment|// character array and expect it to be statically initialized. To
comment|// avoid the relocations caused by a char pointer table, use a
...
literal|0
decl_stmt|;
comment|/*         If there is no default button and the Next or Finish button         is enabled, give focus directly to it as a convenience to the         user. This is the normal case on OS X.          Otherwise, give the focus to the new page's first child that         can handle it. If there is no such child, give the focus to         Next or Finish.     */
if|if
condition|(
operator|(
name|opts
...
name|NoButton
case|:
comment|//case QWizard::NStandardButtons:
comment|//case QWizard::NButtons:
empty_stmt|;
block|}
name|Q_UNREACHABLE
argument_list|()
...
name|indexOfMethod
argument_list|(
literal|"defaultBackgroundPixmapForQWizard()"
argument_list|)
decl_stmt|;
if|if
condition|(
...
end_comment
begin_comment
comment|/*!     \enum QWizard::WizardStyle      This enum specifies the different looks supported by QWizard.      \value ClassicStyle  Classic Windows look     \value ModernStyle  Modern Windows look     \value MacStyle  OS X look     \value AeroStyle  Windows Aero look      \omitvalue NStyles      \sa setWizardStyle(), WizardOption, {Wizard Look and Feel} */
end_comment
begin_comment
...
end_function
begin_comment
comment|/*!     \fn bool QWizard::hasVisitedPage(int id) const      Returns \c true if the page history contains page \a id; otherwise,     returns \c false.      Pressing \uicontrol Back marks the current page as "unvisited" again.      \sa visitedPages() */
end_comment
begin_function
...
condition|)
block|{
comment|//Send a resizeevent since the antiflicker widget probably needs a new size
comment|//because of the backbutton in the window title
name|QResizeEvent
name|ev
argument_list|(
name|geometry
...
end_function
begin_comment
comment|/*!     Returns \c true if the given \a option is enabled; otherwise, returns     false.      \sa options, setOption(), setWizardStyle() */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!     Returns the pixmap set for role \a which.      By default, the only pixmap that is set is the BackgroundPixmap on     OS X.      \sa QWizardPage::pixmap(), {Elements of a Wizard Page} */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!     This virtual function is called by QWizard::validateCurrentPage()     when the user clicks \uicontrol Next or \uicontrol Finish to perform some     last-minute validation. If it returns \c true, the next page is shown     (or the wizard finishes); otherwise, the current page stays up.      The default implementation returns \c true.      When possible, it is usually better style to disable the \uicontrol     Next or \uicontrol Finish button (by specifying \l{mandatory fields} or     reimplementing isComplete()) than to reimplement validatePage().      \sa QWizard::validateCurrentPage(), isComplete() */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!     Returns \c true if this page is a commit page; otherwise returns \c false.      \sa setCommitPage() */
end_comment
begin_function
