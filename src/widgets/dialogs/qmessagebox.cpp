...
end_comment
begin_comment
comment|/*!     Constructs a message box with no text and no buttons. \a parent is     passed to the QDialog constructor.      On OS X, if you want your message box to appear     as a Qt::Sheet of its \a parent, set the message box's     \l{setWindowModality()} {window modality} to Qt::WindowModal or use open().     Otherwise, the message box will be a standard dialog.  */
end_comment
begin_constructor
...
end_function
begin_comment
comment|/*!     \since 4.2      Returns the button that is activated when escape is pressed.      By default, QMessageBox attempts to automatically detect an     escape button as follows:      \list 1     \li If there is only one button, it is made the escape button.     \li If there is a \l Cancel button, it is made the escape button.     \li On OS X only, if there is exactly one button with the role        QMessageBox::RejectRole, it is made the escape button.     \endlist      When an escape button could not be automatically detected, pressing     \uicontrol Esc has no effect.      \sa addButton() */
end_comment
begin_function
...
end_function
begin_comment
comment|/*! \since 5.2      Sets the checkbox \a cb on the message dialog. The message box takes ownership of the checkbox.     The argument \a cb can be 0 to remove an existing checkbox from the message box.      \sa checkBox() */
end_comment
begin_function
...
end_function
begin_comment
comment|/*! \since 5.2      Returns the checkbox shown on the dialog. This is 0 if no checkbox is set.     \sa setCheckBox() */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!     Displays a simple about box with title \a title and text \a     text. The about box's parent is \a parent.      about() looks for a suitable icon in four locations:      \list 1     \li It prefers \l{QWidget::windowIcon()}{parent->icon()}     if that exists.     \li If not, it tries the top-level widget containing \a parent.     \li If that fails, it tries the \l{QApplication::activeWindow()}{active window.}     \li As a last resort it uses the Information icon.     \endlist      The about box has a single button labelled "OK". On OS X, the     about box is popped up as a modeless window; on other platforms,     it is currently application modal.      \sa QWidget::windowIcon(), QApplication::activeWindow() */
end_comment
begin_function
DECL|function|about
name|void
...
literal|"comply with the terms of the GNU LGPL version 3.</p>"
...
literal|"<p>Copyright (C) %1 The Qt Company Ltd and other "
literal|"contributors.</p>"
literal|"<p>Qt and the Qt logo are trademarks of The Qt Company Ltd.</p>"
literal|"<p>Qt is The Qt Company Ltd product developed as an open source "
literal|"project. See<a href=\"http://%3/\">%3</a> for more information.</p>"
argument_list|)
operator|.
name|arg
argument_list|(
...
end_function
begin_comment
comment|/*!     \since 4.2      This function shadows QWidget::setWindowTitle().      Sets the title of the message box to \a title. On OS X,     the window title is ignored (as required by the OS X     Guidelines). */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!     \since 4.2      This function shadows QWidget::setWindowModality().      Sets the modality of the message box to \a windowModality.      On OS X, if the modality is set to Qt::WindowModal and the message box     has a parent, then the message box will be a Qt::Sheet, otherwise the     message box will be a standard dialog. */
end_comment
begin_function
