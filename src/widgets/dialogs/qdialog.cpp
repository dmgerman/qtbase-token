...
end_function_decl
begin_comment
comment|//is defined in qguifunctions_wce.cpp
end_comment
begin_elif
elif|#
directive|elif
...
argument_list|)
expr_stmt|;
comment|// also sets WA_NoSystemBackground
name|setAttribute
argument_list|(
name|Qt
operator|::
...
argument_list|)
expr_stmt|;
comment|// restore system background attribute
block|}
endif|#
directive|endif
block|}
...
block|}
block|}
comment|// TODO is Symbian, non-S60 behaviour required?
endif|#
directive|endif
return|return
name|result
...
argument_list|)
expr_stmt|;
comment|//On Windows Mobile we create an empty menu to hide the current menu
ifdef|#
directive|ifdef
name|Q_WS_WINCE_WM
ifndef|#
...
argument_list|()
condition|)
comment|//dialog has already been positioned
return|return;
endif|#
directive|endif
name|QPoint
...
block|{
comment|// naive way to deduce screen orientation
if|if
condition|(
name|S60
operator|->
...
directive|else
comment|// TODO - check positioning requirement for Symbian, non-s60
return|return
literal|false
return|;
endif|#
...
name|Q_WS_S60
argument_list|)
comment|// if size is not fixed, try to adjust it according to S60 layoutting
if|if
condition|(
name|minimumSize
argument_list|()
...
condition|)
block|{
comment|// In S60, dialogs are always the width of screen (in portrait, regardless of current layout)
return|return
name|QSize
argument_list|(
name|qMin
