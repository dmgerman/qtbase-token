...
parameter_list|)
block|{
comment|// Grab the mouse automatically for current window when any button is pressed,
comment|// unless there is an active mousegrabber or mouse is being grabbed for a popup.
if|if
condition|(
name|e
operator|->
...
condition|)
block|{
comment|// No buttons remain pressed, so release the grab unless grab has been acquired
comment|// for some other reason in the meantime.
if|if
condition|(
name|autoGrabber
operator|&&
...
block|{
comment|// Some buttons are pressed, grab mouse input for current window,
comment|// unless there is already an active grab.
if|if
condition|(
operator|!
name|autoGrabber
