...
name|setProperty
argument_list|(
literal|"_q_macAlwaysShowToolWindow"
argument_list|,
name|QVariant
operator|::
name|fromValue
...
condition|)
block|{
comment|// \forall Qt::GestureType type : ungrabGesture(type) (inlined)
for|for
control|(
name|auto
name|it
...
argument_list|()
decl_stmt|;
comment|// The context must be current when destroying the backing store as it may attempt to
comment|// release resources like textures and shader programs. The window may not be suitable
comment|// anymore as there will often not be a platform window underneath at this stage. Fall
comment|// back to a QOffscreenSurface in this case.
name|QScopedPointer
argument_list|<
...
comment|// make the context current here. This is taken care of by deleteBackingStore().
name|extra
operator|->
name|topextra
operator|->
...
specifier|const
name|bool
name|useStyleSheetPropagationInWidgetStyles
init|=
name|QCoreApplication
operator|::
name|testAttribute
...
comment|// Same check as in the no effects case below.
if|if
condition|(
name|backingStore
operator|&&
...
argument_list|)
expr_stmt|;
comment|// ### The QWidget property is deprecated, but the XCB window function is not.
comment|// It should remain available for the rare application that needs it.
if|if
condition|(
name|QWindow
modifier|*
...
end_function
begin_comment
comment|/*!     \fn void QWidget::windowIconTextChanged(const QString&iconText)      This signal is emitted when the window's icon text has changed, with the     new \a iconText as an argument.      \since 5.2     \obsolete      This signal is deprecated. */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!     \property QWidget::windowIcon     \brief the widget's icon      This property only makes sense for windows. If no icon     has been set, windowIcon() returns the application icon     (QApplication::windowIcon()).      \sa windowTitle */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!     \property QWidget::windowIconText     \brief the text to be displayed on the icon of a minimized window      This property only makes sense for windows. If no icon     text has been set, this accessor returns an empty string.     It is only implemented on the X11 platform, and only certain     window managers use this window property.      \obsolete     This property is deprecated.      \sa windowIcon, windowTitle */
end_comment
begin_function
...
decl_stmt|;
comment|/* If we are about to wrap the focus chain, give the platform      * implementation a chance to alter the wrapping behavior.  This is      * especially needed when the window is embedded in a window created by      * another process.      */
if|if
condition|(
name|wrappingOccurred
condition|)
...
block|}
block|}
comment|// Check for an active window container
if|if
condition|(
name|QWindow
modifier|*
...
argument_list|)
decl_stmt|;
comment|// Sanity check bailing out when large variations of screen sizes occur due to
comment|// high DPI scaling or different levels of DPI awareness.
if|if
condition|(
name|restoredScreenWidth
condition|)
...
block|{
comment|// Saved by Qt 5.3 and earlier, try to prevent too large windows
comment|// unless the size will be adapted by maximized or fullscreen.
if|if
condition|(
operator|!
name|maximized
...
argument_list|()
expr_stmt|;
comment|//create toplevels but not children of non-visible parents
name|QWidget
modifier|*
name|pw
init|=
...
end_function
begin_comment
comment|/*! \internal      Implemented in QWidgetPrivate so that QMdiSubWindowPrivate can reimplement it. */
end_comment
begin_function
DECL|function|setWindowFlags
name|void
...
condition|)
block|{
comment|// set the textureChildSeen flag up the whole parent chain
name|QWidgetPrivate
operator|::
name|get
argument_list|(
...
directive|ifndef
name|QT_NO_OPENGL
comment|//renderToTexture widgets also need to know when their top-level window changes
if|if
condition|(
name|d
operator|->
...
condition|)
block|{
comment|// see ::paintEngine for details
name|paintEngine
argument_list|()
expr_stmt|;
if|if
...
end_function
begin_comment
comment|/*!   \property QWidget::toolTipDuration   \brief the widget's tooltip duration   \since 5.2    Specifies how long time the tooltip will be displayed, in milliseconds.   If the value is -1 (default) the duration is calculated depending on the length of the tooltip.    \sa toolTip */
end_comment
begin_function
...
parameter_list|)
block|{
comment|// if you modify this function, check the inlined version in ~QWidget, too
name|Q_D
argument_list|(
name|QWidget
argument_list|)
...
end_function
begin_comment
comment|// Do not call QWindow::mapToGlobal() until QPlatformWindow is properly showing.
end_comment
begin_function
...
struct|struct
name|MapToGlobalTransformResult
block|{
...
end_function
begin_comment
comment|/*!     \fn void QWidget::activateWindow()      Sets the top-level widget containing this widget to be the active     window.      An active window is a visible top-level window that has the     keyboard input focus.      This function performs the same operation as clicking the mouse on     the title bar of a top-level window. On X11, the result depends on     the Window Manager. If you want to ensure that the window is     stacked on top as well you should also call raise(). Note that the     window must be visible, otherwise activateWindow() has no effect.      On Windows, if you are calling this when the application is not     currently the active one then it will not make it the active     window.  It will change the color of the taskbar entry to indicate     that the window has changed in some way. This is because Microsoft     does not allow an application to interrupt what the user is currently     doing in another application.      \sa isActiveWindow(), window(), show(), QWindowsWindowFunctions::setWindowActivationBehavior() */
end_comment
begin_function
