...
end_ifdef
begin_comment
comment|/*!     \obsolete     Constructs a new QGridLayout with \a nRows rows, \a nCols columns     and parent widget, \a  parent. \a parent may not be 0. The grid     layout is called \a name.      \a margin is the number of pixels between the edge of the widget     and its managed children. \a space is the default number of pixels     between cells. If \a space is -1, the value of \a margin is used. */
end_comment
begin_constructor
DECL|function|QGridLayout
name|QGridLayout
...
end_ifdef
begin_comment
comment|/*!     \compat      Searches for widget \a w in this layout (not including child     layouts). If \a w is found, it sets \c{*}\a{row} and     \c{*}\a{column} to the row and column that the widget     occupies and returns true; otherwise returns false.      If the widget spans multiple rows/columns, the top-left cell     is returned.      Use indexOf() and getItemPosition() instead. */
end_comment
begin_function
...
end_ifdef
begin_comment
comment|/*!   \obsolete     Expands this grid so that it will have \a nRows rows and \a nCols     columns. Will not shrink the grid. You should not need to call     this function because QGridLayout expands automatically as new     items are inserted. */
end_comment
begin_function
DECL|function|expand
name|void
