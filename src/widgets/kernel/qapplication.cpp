...
begin_comment
DECL|member|keyboard_input_time
comment|// keyboard input interval
end_comment
begin_ifndef
ifndef|#
directive|ifndef
...
end_ifdef
begin_comment
comment|// The screens are a bit too small to for your thumb when using only 4 pixels drag distance.
end_comment
begin_decl_stmt
...
begin_comment
DECL|variable|drag_distance
comment|//XXX move to qplatformdefs.h
end_comment
begin_else
else|#
directive|else
...
directive|if
literal|0
block|for (int i = 0; i< qt_modal_stack->size(); ++i) {         QWidget *modalWidget = qt_modal_stack->at(i);          {
comment|// check if the active modal widget is our widget or a parent of our widget
block|QWidget *w = widget;             while (w) {                 if (w == modalWidget)                     return false;                 w = w->parentWidget();             }
ifdef|#
directive|ifdef
name|Q_WS_WIN
block|if ((widget->testAttribute(Qt::WA_WState_Created) || widget->data->winid)&& (modalWidget->testAttribute(Qt::WA_WState_Created) || modalWidget->data->winid)&& IsChild(modalWidget->data->winid, widget->data->winid))                 return false;
endif|#
directive|endif
block|}          Qt::WindowModality windowModality = modalWidget->windowModality();         if (windowModality == Qt::NonModal) {
...
argument_list|)
expr_stmt|;
comment|// next lines are for compatibility with Qt<= 3.0.x: old
comment|// QAccel was listening on toplevel widgets
if|if
condition|(
operator|!
name|res
...
directive|endif
comment|//QT3_SUPPORT&& !QT_NO_SHORTCUT
case|case
name|QEvent
operator|::
...
argument_list|)
expr_stmt|;
comment|// only static members being used.
if|if
condition|(
name|QApplicationPrivate
operator|::
...
argument_list|)
decl_stmt|;
comment|// fallback to default X Input Method.
if|if
condition|(
operator|!
name|qic
...
argument_list|()
decl_stmt|;
comment|// Try hbim and coefep first, then try others.
if|if
condition|(
name|keys
operator|.
