...
begin_comment
DECL|member|overrides_native_style
comment|// whether native QApplication style is
end_comment
begin_comment
comment|// overridden, i.e. not native
end_comment
begin_ifndef
ifndef|#
directive|ifndef
...
name|qt_init
argument_list|(
argument|QApplicationPrivate *priv
argument_list|,
argument|int type
argument_list|)
block|{
...
argument_list|()
condition|)
comment|// Qt::WA_DeleteOnClose may cause deletion.
return|return
literal|false
return|;
if|if
...
argument_list|)
expr_stmt|;
comment|// QTBUG-37126, Active X controls may set the focus on native child widgets.
if|if
condition|(
name|wnd
operator|&&
...
condition|)
block|{
comment|// Guard against QGuiApplicationPrivate::lastCursorPosition initialized to qInf(), qInf().
specifier|const
name|QPoint
name|globalPos
init|=
...
condition|)
block|{
comment|// wheel_widget may forward the wheel event to a delegate widget,
...
directive|ifdef
name|Q_OS_MAC
comment|// Enable touch events on enter, disable on leave.
typedef|typedef
name|void
function_decl|(
modifier|*
name|RegisterTouchWindowFn
function_decl|)
parameter_list|(
name|QWindow
modifier|*
...
condition|)
break|break;
comment|// find out whether this widget (or its proxy) already has focus
name|QWidget
modifier|*
name|f
init|=
...
name|focus_proxy
expr_stmt|;
comment|// if it has, stop here.
comment|// otherwise a click on the focused widget would remove its focus if ClickFocus isn't set
if|if
condition|(
name|f
operator|->
