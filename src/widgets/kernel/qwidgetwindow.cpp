...
condition|)
block|{
comment|// Handing show events to widgets would cause them to be triggered twice
case|case
name|QEvent
operator|::
name|Show
...
name|UpdateRequest
case|:
comment|// This is not the same as an UpdateRequest for a QWidget. That just
comment|// syncs the backing store while here we also must mark as dirty.
name|m_widget
operator|->
name|repaint
argument_list|()
...
name|Q_OS_IOS
argument_list|)
comment|// Cocoa tracks popups
comment|// Ignore all enter/leave events from QPA if we are not on the first-level context menu.
comment|// This prevents duplicated events on most platforms. Fake events will be delivered in
comment|// QWidgetWindow::handleMouseEvent(QMouseEvent *). Make an exception whether the widget
comment|// is already under mouse - let the mouse leave.
if|if
condition|(
name|QApplicationPrivate
operator|::
...
condition|)
block|{
comment|// If active popup menu is not the first-level popup menu then we must emulate enter/leave events,
comment|// because first-level popup menu grabs the mouse and enter/leave events are delivered only to it
comment|// by QPA. Make an exception for first-level popup menu when the mouse button is pressed on widget.
if|if
condition|(
name|underMouse
operator|!=
...
name|Q_UNUSED
argument_list|(
argument|contextMenuTrigger
argument_list|)
name|Q_UNUSED
argument_list|(
argument|oldOpenPopupCount
argument_list|)
block|}
endif|#
directive|endif
...
specifier|static
name|void
name|sendScreenChangeRecursively
argument_list|(
...
operator|)
block|{
comment|// Send an event recursively to the widget and its children.
name|sendScreenChangeRecursively
argument_list|(
name|m_widget
argument_list|)
...
argument_list|()
decl_stmt|;
comment|// Use proper popup window for wheel event. Some QPA sends the wheel
comment|// event to the root menu, so redirect it to the proper popup window.
name|QWidget
modifier|*
name|activePopupWidget
init|=
