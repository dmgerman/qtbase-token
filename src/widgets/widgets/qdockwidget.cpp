...
name|QMainWindowLayout
modifier|*
name|qt_mainwindow_layout_from_dock
parameter_list|(
specifier|const
name|QDockWidget
modifier|*
...
end_destructor
begin_comment
comment|/*! \internal     Returns true if the dock widget managed by this layout should have a native     window decoration or if Qt needs to draw it.  */
end_comment
begin_function
...
end_function
begin_comment
comment|/*! \internal     Returns true if the window manager can draw natively the windows decoration     of a dock widget  */
end_comment
begin_function
...
end_function
begin_comment
comment|/*! \internal    Returns true if the dock widget managed by this layout should have a native    window decoration or if Qt needs to draw it. The \a floating parameter    overrides the floating current state of the dock widget.  */
end_comment
begin_function
DECL|function|nativeWindowDeco
name|bool
...
argument_list|)
decl_stmt|;
comment|// If we are in a floating tab, init from the parent because the attributes and the geometry
comment|// of the title bar should be taken from the floating window.
name|option
operator|->
name|initFrom
argument_list|(
...
end_function
begin_comment
comment|/*! \internal     Initialize the drag state structure and remember the position of the click.     This is called when the mouse is pressed, but the dock is not yet dragged out.      \a nca specify that the event comes from NonClientAreaMouseButtonPress  */
end_comment
begin_function
...
end_function
begin_comment
comment|/*! \internal     Actually start the drag and detach the dockwidget.     The \a group parameter is true when we should potentially drag a group of     tabbed widgets, and false if the dock widget should always be dragged     alone.  */
end_comment
begin_function
...
end_function
begin_comment
comment|/*! \internal     Ends the drag end drop operation of the QDockWidget.     The \a abort parameter specifies that it ends because of programmatic state     reset rather than mouse release event.  */
end_comment
begin_function
...
condition|)
block|{
comment|// This QDockWidget will now stay in the floating state.
if|if
condition|(
name|state
operator|->
...
block|{
comment|// The tab was not plugged back in the QMainWindow but the QDockWidget cannot
comment|// stay floating, revert to the previous state.
name|mwLayout
operator|->
name|revert
argument_list|(
...
end_function
begin_comment
comment|/*! \internal     Called when the QDockWidget or the QDockWidgetGroupWindow is moved  */
end_comment
begin_function
