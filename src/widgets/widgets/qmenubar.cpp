...
comment|// We need to install an avent filter on each parent up to the parent that is
comment|// also a window (for shortcuts)
name|QWidget
modifier|*
name|newWindow
init|=
...
name|newParents
decl_stmt|;
comment|// Remove event filters on ex-parents, keep them on still-parents
comment|// The parents are always ordered in the vector
foreach|foreach
control|(
specifier|const
name|QPointer
...
block|}
block|}
comment|// At this point, newParent is the next one to be added to newParents
while|while
condition|(
name|newParent
operator|&&
...
condition|)
block|{
comment|//install event filters all the way up to (excluding) the window
name|newParents
operator|.
name|append
argument_list|(
...
condition|)
block|{
comment|// Install the event filter on the window
name|newParents
operator|.
name|append
argument_list|(
