...
end_function
begin_comment
comment|/*!     Removes the central widget from this main window.      The ownership of the removed widget is passed to the caller.      \since Qt 5.2 */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!     \since 5.6     Resizes the dock widgets in the list \a docks to the corresponding size in     pixels from the list \a sizes. If \a orientation is Qt::Horizontal, adjusts     the width, otherwise adjusts the height of the dock widgets.     The sizes will be adjusted such that the maximum and the minimum sizes are     respected and the QMainWindow itself will not be resized.     Any additional/missing space is distributed amongst the widgets according     to the relative weight of the sizes.      Example:     \code     resizeDocks({blueWidget, yellowWidget}, {20 , 40}, Qt::Horizontal);     \endcode     If the blue and the yellow widget are nested on the same level they will be     resized such that the yellowWidget is twice as big as the blueWidget      If some widgets are grouped in tabs, only one widget per group should be     specified. Widgets not in the list might be changed to repect the constraints. */
end_comment
begin_function
...
argument_list|)
decl_stmt|;
comment|// filter to find out if we own this QDockWidget
if|if
condition|(
name|dockWidget
operator|->
