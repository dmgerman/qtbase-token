...
comment|// Uncommend, to show debugging information for the keymapper
comment|//#define DEBUG_KEYMAPPER
...
argument_list|)
comment|// remap return or action key to select key for windows mobile.
comment|// will be changed to a table remapping function in the next version (4.6/7).
if|if
condition|(
name|keyCode
operator|==
...
end_if
begin_comment
comment|// Use the KeyTbl to resolve a Qt::Key out of the virtual keys.
end_comment
begin_comment
comment|// In case it is not resolvable, continue using the virtual key itself.
end_comment
begin_function
...
end_function
begin_comment
comment|// QETWidget class is only for accessing the sendSpontaneousEvent function in QApplication
end_comment
begin_class
...
argument_list|)
expr_stmt|;
comment|// Strange, but the compiler complains on q not being referenced, even if it is..
name|bool
name|k0
init|=
literal|false
...
literal|0x80000000
decl_stmt|;
comment|// A multi-character key not found by our look-ahead
if|if
condition|(
name|msgType
operator|==
...
expr_stmt|;
block|}
comment|// Input method characters not found by our look-ahead
elseif|else
if|if
condition|(
name|msgType
...
block|}
block|}
comment|// Return true, if a QKeyEvent was sent to a widget
return|return
name|k0
operator|||
name|k1
...
end_function
begin_comment
comment|// QKeyMapper (Windows) implementation -------------------------------------------------[ start ]---
end_comment
begin_function
...
argument_list|()
expr_stmt|;
comment|// Emulate windows behavior
return|return
name|e
operator|.
name|isAccepted
