...
end_function
begin_comment
comment|/*!    \since 5.1    This functions returns the current rubber band area (in viewport coordinates) if the user    is currently doing an itemselection with rubber band. When the user is not using the    rubber band this functions returns (a null) QRectF().     Notice that part of this QRect can be outise the visual viewport. It can e.g    contain negative values.     \sa rubberBandSelectionMode, rubberBandChanged() */
end_comment
begin_function
