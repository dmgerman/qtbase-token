...
end_function
begin_comment
comment|/*!     \fn QGraphicsItem *QGraphicsScene::itemAt(const QPointF&position) const     \overload     \obsolete      Returns the topmost visible item at the specified \a position, or 0 if     there are no items at this position.      This function is deprecated and returns incorrect results if the scene     contains items that ignore transformations. Use the overload that takes     a QTransform instead.      \sa items(), collidingItems(), {QGraphicsItem#Sorting}{Sorting} */
end_comment
begin_comment
