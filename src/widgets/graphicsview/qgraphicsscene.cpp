...
name|ReplaceSelection
case|:
comment|// Deselect all items outside path.
foreach|foreach
control|(
name|QGraphicsItem
modifier|*
...
argument_list|()
decl_stmt|;
comment|// FIXME: iterate over hash directly?
for|for
control|(
name|Qt
operator|::
...
condition|)
block|{
comment|// We cannot simply use !drawItem here. If we did it is possible
comment|// to enter the outter if statement with drawItem == false and minimumRenderSize> 0
comment|// and finally end up inside this inner if, even though none of the above two
comment|// conditions are met. In that case we should not return from this function
comment|// but call draw() instead.
if|if
condition|(
operator|!
name|itemHasChildren
