...
specifier|auto
modifier|&
name|constDeviceData
init|=
name|deviceData
decl_stmt|;
comment|// avoid detach
for|for
control|(
specifier|const
specifier|auto
...
argument_list|()
decl_stmt|;
comment|// FIXME: iterate over the map directly?
for|for
control|(
name|Qt
operator|::
...
end_function
begin_comment
comment|/*!   Sets this item's parent item to \a newParent. If this item already   has a parent, it is first removed from the previous parent. If \a   newParent is 0, this item will become a top-level item.    Note that this implicitly adds this graphics item to the scene of   the parent. You should not \l{QGraphicsScene::addItem()}{add} the   item to the scene yourself.    The behavior when calling this function on an item that is an ancestor of   \a newParent is undefined.    \sa parentItem(), childItems() */
end_comment
begin_function
...
condition|)
block|{
comment|// Item children containtment changes. Propagate the ancestor flag to all children.
name|d_ptr
operator|->
name|updateAncestorFlag
argument_list|(
...
condition|)
block|{
comment|// unlink own widgets from focus chain
name|QGraphicsWidget
modifier|*
name|beforeMe
init|=
...
condition|)
block|{
comment|// link up own widgets to focus chain
name|QGraphicsWidget
modifier|*
name|beforeMe
init|=
...
condition|)
block|{
comment|// Hiding the focus item or the closest non-panel ancestor of the focus item
name|QGraphicsItem
modifier|*
name|focusItem
init|=
...
end_comment
begin_comment
comment|/*!     Returns the type of an item as an int. All standard graphicsitem classes     are associated with a unique value; see QGraphicsItem::Type. This type     information is used by qgraphicsitem_cast() to distinguish between types.      The default implementation (in QGraphicsItem) returns UserType.      To enable use of qgraphicsitem_cast() with a custom item, reimplement this     function and declare a Type enum value equal to your custom item's type.     Custom items must return a value larger than or equal to UserType (65536).      For example:      \snippet code/src_gui_graphicsview_qgraphicsitem.cpp 1      \sa UserType */
end_comment
begin_function
DECL|function|type
name|int
...
end_function
begin_comment
comment|// FIXME: Qt 6: Make this QDebug operator<<(QDebug debug, const QGraphicsItem *item)
end_comment
begin_function
...
end_function
begin_comment
comment|// FIXME: Qt 6: Make this QDebug operator<<(QDebug debug, const QGraphicsObject *item)
end_comment
begin_function
DECL|function|operator <<
name|QDebug
...
name|str
operator|=
literal|"ItemContainsChildrenInShape"
expr_stmt|;
break|break;
block|}
name|debug
