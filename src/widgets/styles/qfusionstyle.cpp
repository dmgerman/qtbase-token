...
argument_list|()
decl_stmt|;
comment|// a scrollbar is transient when the the scrollbar itself and
comment|// its sibling are both inactive (ie. not pressed/hovered/moved)
name|bool
name|transient
init|=
operator|!
...
condition|)
block|{
comment|// if the scrollbar is transient or its attributes, geometry or
comment|// state has changed, the opacity is reset back to 100% opaque
name|opacity
operator|=
literal|1.0
expr_stmt|;
...
condition|)
block|{
comment|// the scrollbar was already fading out while the
comment|// state changed -> restart the fade out animation
name|anim
operator|->
name|setCurrentTime
argument_list|(
...
condition|)
block|{
comment|// once a scrollbar was active (hovered/pressed), it retains
comment|// the active look even if it's no longer active while fading out
if|if
condition|(
name|oldActiveControls
condition|)
...
condition|)
block|{
comment|// Start expand animation only once and when entering
name|anim
operator|=
operator|new
name|QScrollbarStyleAnimation
...
block|{
comment|// Keep expanded state after the animation ends, and when fading out
name|expandScale
operator|=
name|maxExpandScale
expr_stmt|;
...
literal|0
decl_stmt|;
comment|// Is drawn with 3 pixels width in drawComplexControl, independently from PM_SpinBoxFrameWidth
name|int
name|y
init|=
name|fw
