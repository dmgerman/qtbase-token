...
end_macro
begin_comment
comment|/*!     \class QCommonStyle     \brief The QCommonStyle class encapsulates the common Look and Feel of a GUI.      \ingroup appearance      This abstract class implements some of the widget's look and feel     that is common to all GUI styles provided and shipped as part of     Qt.      Since QCommonStyle inherits QStyle, all of its functions are fully documented     in the QStyle documentation.     \omit     , although the     extra functions that QCommonStyle provides, e.g.     drawComplexControl(), drawControl(), drawPrimitive(),     hitTestComplexControl(), subControlRect(), sizeFromContents(), and     subElementRect() are documented here.     \endomit      \sa QStyle, QMotifStyle, QWindowsStyle */
end_comment
begin_comment
...
block|}
decl_stmt|;
comment|// static const int pts5[] = {                // inner fill
comment|//    4,2, 7,2, 9,4, 9,7, 7,9, 4,9, 2,7, 2,4 };
comment|//QPolygon a;
if|if
condition|(
name|lv
...
argument_list|)
expr_stmt|;
comment|//                a.setPoints(INTARRLEN(pts5), pts5);
comment|//                a.translate(x, y);
comment|//        QColor fillColor = isDown() ? g.background() : g.base();
comment|//        p->setPen(fillColor);
comment|//        p->setBrush(fillColor);
comment|//        p->drawPolygon(a);
if|if
condition|(
name|opt
operator|->
