...
argument_list|)
expr_stmt|;
comment|// QCleanlooksStyle will alter the palette, hence we do
comment|// not want to polish the palette unless we are using it as
comment|// the fallback
if|if
condition|(
operator|!
name|d
...
comment|// have to rely on Cleanlooks for a fallback
name|QStyleOptionTitleBar
name|copyOpt
init|=
operator|*
...
name|palette
decl_stmt|;
comment|// Bg color is closer to the window selection than
comment|// the base selection color
name|GdkColor
name|gdkBg
init|=
name|style
...
condition|)
block|{
comment|// Reserve space for outside focus rect
name|QStyleOptionSlider
name|sliderCopy
init|=
operator|*
...
argument_list|)
expr_stmt|;
comment|// Cleanlooks assumes a check column of 20 pixels so we need to
comment|// expand it a bit
name|gint
name|checkSize
decl_stmt|;
name|d
