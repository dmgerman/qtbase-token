...
end_ifndef
begin_comment
comment|/*!     Sets the specified \a menu to be the context menu for the system tray icon.      The menu will pop up when the user requests the context menu for the system     tray icon by clicking the mouse button.      On OS X, this is currenly converted to a NSMenu, so the     aboutToHide() signal is not emitted.      \note The system tray icon does not take ownership of the menu. You must     ensure that it is deleted at the appropriate time by, for example, creating     the menu with a suitable parent object. */
end_comment
begin_function
...
end_comment
begin_comment
comment|/*!     \fn void QSystemTrayIcon::messageClicked()      This signal is emitted when the message displayed using showMessage()     was clicked by the user.      Currently this signal is not sent on OS X.      \note We follow Microsoft Windows XP/Vista behavior, so the     signal is also emitted when the user clicks on a tray icon with     a balloon message displayed.      \sa activated() */
end_comment
begin_comment
...
condition|)
return|return;
comment|// The platform menu already exists.
comment|// The recursion depth is the same as menu depth, so should not
comment|// be higher than 3 levels.
name|QListIterator
argument_list|<
name|QAction
modifier|*
...
expr_stmt|;
block|}
comment|// This menu should be processed *after* its children, otherwise
comment|// setMenu() is not called on respective QPlatformMenuItems.
name|QPlatformMenu
modifier|*
name|platformMenu
init|=
