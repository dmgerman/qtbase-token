...
block|{
comment|// clearRegion() was called on XEMBED_EMBEDDED_NOTIFY, so we hope that got done by now.
comment|// Grab the tray background pixmap, before rendering the icon for the first time.
if|if
condition|(
name|background
operator|.
...
expr_stmt|;
block|}
comment|// Then paint over the icon area with the background before compositing the icon on top.
name|painter
operator|.
name|drawPixmap
argument_list|(
