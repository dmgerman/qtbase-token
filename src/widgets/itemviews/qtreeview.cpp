...
end_function
begin_comment
comment|/*!     \since 4.2     \property QTreeView::animated     \brief whether animations are enabled      If this property is true the treeview will animate expandsion     and collasping of branches. If this property is false, the treeview     will expand or collapse branches immediately without showing     the animation.      By default, this property is false. */
end_comment
begin_function
...
condition|)
block|{
comment|//we need to paint the whole items (including the decoration) so that when the user
comment|//moves the mouse over those elements they are updated
if|if
condition|(
name|oldBranch
operator|>=
...
expr_stmt|;
block|}
comment|//now that we have the visible indexes, we can try to find those which are selected
name|QItemViewPaintPairs
name|ret
decl_stmt|;
for|for
