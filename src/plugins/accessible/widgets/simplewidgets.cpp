...
argument_list|)
return|;
comment|// FIXME: sub buttons when SplitButton
comment|//    QStyleOptionToolButton opt;
comment|//    opt.init(widget());
comment|//    QRect subrect = widget()->style()->subControlRect(QStyle::CC_ToolButton,&opt,
comment|//                                                      QStyle::SC_ToolButtonMenu, toolButton());
comment|//    if (child == ButtonExecute)
comment|//        subrect = QRect(0, 0, subrect.x(), widget()->height());
comment|//    QPoint ntl = widget()->mapToGlobal(subrect.topLeft());
comment|//    subrect.moveTopLeft(ntl);
comment|//    return subrect;
block|}
end_function
begin_comment
