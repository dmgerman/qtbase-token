...
modifier|*
argument_list|>
name|VisibleBoundsChangedHandler
typedef|;
end_typedef
begin_endif
endif|#
...
end_function
begin_comment
comment|// Some keys like modifiers, caps lock etc. should not be automatically repeated if the key is held down
end_comment
begin_function
...
function_decl|(
name|__stdcall
name|IApplicationView2
operator|::
modifier|*
name|ApplicationView2CallbackRemover
function_decl|)
parameter_list|(
name|EventRegistrationToken
parameter_list|)
...
name|IApplicationView2
operator|::
name|remove_VisibleBoundsChanged
index|]
argument_list|)
expr_stmt|;
name|Q_ASSERT_SUCCEEDED
...
name|S_OK
return|;
comment|// If the key was pressed before trigger a key release before the next key press
name|QWindowSystemInterface
operator|::
...
literal|false
argument_list|,
comment|// The final key release does not have autoRepeat set on Windows
operator|!
name|status
operator|.
name|RepeatCount
...
argument_list|)
expr_stmt|;
comment|// Fall-through for pen to generate tablet event
if|if
condition|(
name|pointerDeviceType
operator|!=
...
name|RETURN_OK_IF_FAILED
argument_list|(
literal|"Failed to get window visible bounds"
argument_list|)
argument_list|;
name|d
operator|->
