...
argument_list|)
expr_stmt|;
comment|// Fill the window with solid black. Note that the LSB of the pixel value
comment|// 0x00000000 just happens to be 0x00, so if and when this root window's
comment|// alpha blending mode is changed from None to Source-Over, it will then
comment|// be interpreted as transparent.
name|errno
operator|=
literal|0
expr_stmt|;
...
argument_list|()
expr_stmt|;
comment|// Don't post yet. This will be lazily done from QQnxScreen upon first posting of
comment|// a child window. Doing it now pre-emptively would create a flicker if one of
comment|// the QWindow's about to be created sets its Qt::WA_TranslucentBackground flag
comment|// and immediately triggers the buffer re-creation in makeTranslucent().
block|}
end_constructor
begin_function
...
name|qFatal
argument_list|(
literal|"QQnxRootWindow: failed to destroy window buffer, errno=%d"
argument_list|,
name|errno
argument_list|)
expr_stmt|;
...
expr_stmt|;
block|}
comment|// Install an alpha channel on the root window.
comment|//
comment|// This is necessary in order to avoid interfering with any particular
comment|// toplevel widget's QQnxWindow window instance from showing transparent
comment|// if it desires.
name|errno
operator|=
literal|0
expr_stmt|;
