...
argument_list|)
expr_stmt|;
comment|// setWindowState() does not get called when the platform window hasn't been created yet, so
comment|// make sure to apply the inital window state here
name|setWindowState
argument_list|(
name|window
operator|->
...
condition|)
block|{
comment|// WindowMinimized is not supported - navigator does not have an API to minimize a window
