...
condition|)
block|{
comment|// Qt only gives us position inside the block, so if we move to the
comment|// same position in another block, the Android keyboard will believe
comment|// we have not changed position, and be terribly confused.
if|if
condition|(
name|selStart
operator|==
