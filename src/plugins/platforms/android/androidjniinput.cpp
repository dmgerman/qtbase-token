...
specifier|static
name|bool
name|m_softwareKeyboardVisible
init|=
literal|false
decl_stmt|;
...
name|globalPosF
decl_stmt|;
comment|// Galaxy Note with plain Android:
comment|// 0 1 0    stylus press
comment|// 2 1 0    stylus drag
comment|// 1 1 0    stylus release
comment|// 0 1 2    stylus press with side-button held
comment|// 2 1 2    stylus drag with side-button held
comment|// 1 1 2    stylus release with side-button held
comment|// Galaxy Note 4 with Samsung firmware:
comment|// 0 1 0    stylus press
comment|// 2 1 0    stylus drag
comment|// 1 1 0    stylus release
comment|// 211 1 2  stylus press with side-button held
comment|// 213 1 2  stylus drag with side-button held
comment|// 212 1 2  stylus release with side-button held
comment|// when action == ACTION_UP (1) it's a release; otherwise we say which button is pressed
name|Qt
operator|::
name|MouseButtons
name|buttons
...
literal|212
case|:
comment|// stylus release while side-button held on Galaxy Note 4
name|buttons
operator|=
name|Qt
operator|::
...
break|break;
default|default:
comment|// action is press or drag
if|if
condition|(
name|buttonState
operator|==
...
block|,
block|{
literal|"isTabletEventSupported"
block|,
literal|"()Z"
block|,
operator|(
...
block|,
block|{
literal|"keyboardGeometryChanged"
block|,
literal|"(IIII)V"
block|,
operator|(
