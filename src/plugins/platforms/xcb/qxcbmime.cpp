...
argument_list|)
expr_stmt|;
comment|// mimeAtomToString() converts "text/x-moz-url" to "text/uri-list",
comment|// so QXcbConnection::atomName() has to be used.
if|if
condition|(
name|atomName
operator|==
...
condition|)
block|{
comment|// Return URLs also as plain text.
operator|*
name|data
operator|=
name|QInternalMimeData
...
return|;
block|}
comment|// If data contains UTF16 text, convert it to a string.
comment|// Firefox uses UTF16 without BOM for text/x-moz-url, "text/html",
comment|// Google Chrome uses UTF16 without BOM for "text/x-moz-url",
comment|// UTF16 with BOM for "text/html".
if|if
condition|(
operator|(
name|format
...
expr_stmt|;
block|}
comment|// We expect "text/x-moz-url" as<url><space><title>.
comment|// The atomName variable is not used because mimeAtomToString()
comment|// converts "text/x-moz-url" to "text/uri-list".
if|if
condition|(
operator|!
name|list
