...
operator|::
name|NoModifier
comment|// Fall-back to raw Key_*, for non-latin1 kb layouts
block|}
decl_stmt|;
end_decl_stmt
begin_function
...
argument_list|)
decl_stmt|;
comment|// on old X servers xkb_config can be 0 even if config_reply indicates a succesfull read
if|if
condition|(
operator|!
name|xkb_config
...
condition|)
return|return;
comment|// ### TODO some X servers don't set _XKB_RULES_NAMES at all, in these cases it is filled
comment|// with gibberish, we would need to do some kind of sanity check
name|char
modifier|*
name|names
index|[
...
literal|true
expr_stmt|;
comment|// set xkb context object
if|if
condition|(
operator|!
name|xkb_context
...
return|return;
block|}
comment|// log only critical errors, we do our own error logging from printKeymapError()
...
expr_stmt|;
block|}
comment|// update xkb keymap object
...
condition|)
block|{
comment|// last fallback is to used hard-coded keymap name, see DEFAULT_XKB_* in xkbcommon.pri
name|qWarning
argument_list|()
operator|<<
literal|"Qt: Could not determine keyboard configuration data"
literal|" from X server, will use hard-coded keymap configuration."
expr_stmt|;
name|clearXKBConfig
argument_list|()
expr_stmt|;
...
name|qWarning
argument_list|(
literal|"Qt: Failed to create xkb state!"
argument_list|)
expr_stmt|;
name|m_config
operator|=
...
return|return;
block|}
comment|// update xkb state object
...
name|xkbMask
decl_stmt|;
comment|// set modifiers in depressed if they don't appear in any of the final masks
name|depressed
operator||=
operator|~
operator|(
...
argument_list|)
decl_stmt|;
comment|// bits 13 and 14 report the state keyboard group
if|if
condition|(
operator|(
name|newState
...
argument_list|)
decl_stmt|;
comment|// if layout can produce any of these latin letters (chosen
comment|// arbitrarily) then it must be a latin key based layout
if|if
condition|(
name|sym
operator|==
...
argument_list|)
decl_stmt|;
comment|// Look at user layouts in the order in which they are defined in system
comment|// settings to find a latin keysym.
for|for
control|(
name|layout
operator|=
...
block|}
block|}
comment|// If user layouts don't contain any layout that results in a latin key, we query a
comment|// key from "US" layout, this allows for latin-key-based shorcuts to work even when
comment|// users have only one (non-latin) layout set.
name|xkb_mod_mask_t
name|latchedMods
init|=
name|xkb_state_serialize_mods
...
condition|)
block|{
comment|// print message about failure to compile US keymap only once,
comment|// no need to do this on every key press.
name|printFailure
operator|=
literal|false
expr_stmt|;
name|printKeymapError
argument_list|(
literal|"Qt: Failed to compile US keymap, shortcut handling with "
literal|"non-Latin keyboard layouts may not be fully functional!"
argument_list|)
expr_stmt|;
block|}
block|}
...
name|qWarning
argument_list|(
literal|"QXcbKeyboard: failed to create a state for US keymap!"
argument_list|)
expr_stmt|;
block|}
block|}
...
name|sym
return|;
comment|// Check for uniqueness, consider the following setup:
comment|// setxkbmap -layout us,ru,us -variant dvorak,, -option 'grp:ctrl_alt_toggle' (set 'ru' as active).
comment|// In this setup, the user would expect to trigger a ctrl+q shortcut by pressing ctrl+<physical x key>,
comment|// because "US dvorak" is higher up in the layout settings list. This check verifies that an obtained
comment|// 'sym' can not be acquired by any other layout higher up in the user's layout list. If it can be acquired
comment|// then the obtained key is not unique. This prevents ctrl+<physical q key> from generating a ctrl+q
comment|// shortcut in the above described setup. We don't want ctrl+<physical x key> and ctrl+<physical q key> to
comment|// generate the same shortcut event in this case.
specifier|const
name|xcb_keycode_t
name|minKeycode
init|=
...
argument_list|()
decl_stmt|;
comment|// handle shortcuts for level three and above
name|xkb_layout_index_t
name|layoutIndex
init|=
name|xkb_state_key_get_layout
...
condition|)
continue|continue;
comment|// add a latin key as a fall back key
name|sym
operator|=
name|lookupLatinKeysym
argument_list|(
...
condition|)
continue|continue;
comment|// catch only more specific shortcuts, i.e. Ctrl+Shift+= also generates Ctrl++ and +,
comment|// but Ctrl++ is more specific than +, so we should skip the last one
name|bool
name|ambiguous
init|=
literal|false
...
parameter_list|()
block|{
comment|// The core protocol does not provide a convenient way to determine the mapping
comment|// of modifier bits. Clients must retrieve and search the modifier map to determine
comment|// the keycodes bound to each modifier, and then retrieve and search the keyboard
comment|// mapping to determine the keysyms bound to the keycodes. They must repeat this
comment|// process for all modifiers whenever any part of the modifier mapping is changed.
name|memset
argument_list|(
operator|&
name|rmod_masks
...
name|qWarning
argument_list|(
literal|"Qt: failed to get modifier mapping"
argument_list|)
expr_stmt|;
name|free
argument_list|(
