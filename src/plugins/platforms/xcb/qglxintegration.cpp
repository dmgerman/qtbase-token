...
argument_list|)
expr_stmt|;
comment|// Used for creating the temporary window
name|visualInfo
operator|=
...
expr_stmt|;
block|}
comment|// If we have OpenGL 3.2 or newer we also need to query the profile in use
if|if
condition|(
name|m_format
operator|.
...
condition|)
block|{
comment|// nVidia drivers have a bug where querying GL_CONTEXT_PROFILE_MASK always returns 0.
comment|// In this case let's assume that we got the profile that we asked for since nvidia implements
comment|// both Core and Compatibility profiles
if|if
condition|(
name|versionString
operator|.
