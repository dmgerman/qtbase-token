...
end_comment
begin_endif
unit|static inline bool isTransient(const QWidget *w) {     return ((w->windowType() == Qt::Dialog              || w->windowType() == Qt::Sheet              || w->windowType() == Qt::Tool              || w->windowType() == Qt::SplashScreen              || w->windowType() == Qt::ToolTip              || w->windowType() == Qt::Drawer              || w->windowType() == Qt::Popup)&& !w->testAttribute(Qt::WA_X11BypassTransientForHint)); }
endif|#
directive|endif
end_endif
begin_constructor
...
directive|if
literal|0
block|if (tlw&& isTransient(tlw)&& tlw->parentWidget()) {
comment|// ICCCM 4.1.2.6
block|QWidget *p = tlw->parentWidget()->window();         xcb_window_t parentWindow = p->winId();         Q_XCB_CALL(xcb_change_property(xcb_connection(), XCB_PROP_MODE_REPLACE, m_window,                                        XCB_ATOM_WM_TRANSIENT_FOR, XCB_ATOM_WINDOW, 32,                                        1,&parentWindow));      }
endif|#
directive|endif
...
condition|)
block|{
comment|// TODO: QWindow::isMinimized() or similar
if|if
condition|(
name|window
argument_list|()
