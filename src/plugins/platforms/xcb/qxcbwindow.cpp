...
comment|// the data of the static QByteArray below.
if|if
condition|(
name|tp
operator|.
...
name|qDebug
argument_list|(
literal|"XmbTextListToTextProperty result code %d"
argument_list|,
name|errCode
argument_list|)
expr_stmt|;
...
end_comment
begin_comment
comment|// TODO move this into a utility function in QWindow or QGuiApplication
end_comment
begin_function
...
argument_list|()
operator|<<
literal|"Failed to use requested visual id."
expr_stmt|;
block|}
if|if
condition|(
...
argument_list|()
operator|<<
literal|"Falling back to using screens root_visual."
expr_stmt|;
name|visual
operator|=
name|platformScreen
...
argument_list|)
decl_stmt|;
comment|// _NET_FRAME_EXTENTS format is left, right, top, bottom
name|m_frameMargins
operator|=
name|QMargins
argument_list|(
...
block|}
block|}
comment|// _NET_FRAME_EXTENTS property is not available, so
comment|// walk up the window tree to get the frame parent
name|xcb_window_t
name|window
init|=
name|m_window
...
condition|)
block|{
comment|// Unset mousePressWindow when it (or one of its parents) is unmapped
while|while
condition|(
name|w
condition|)
...
literal|false
expr_stmt|;
comment|// Hiding a modal window doesn't send an enter event to its transient parent when the
comment|// mouse is already over the parent window, so the enter event must be emulated.
if|if
condition|(
name|window
argument_list|()
...
condition|)
block|{
comment|// Get the cursor position at modal window screen
specifier|const
name|QPoint
name|nativePos
init|=
...
argument_list|)
decl_stmt|;
comment|// Find the top level window at cursor position.
comment|// Don't use QGuiApplication::topLevelAt(): search only the virtual siblings of this window's screen
name|QWindow
modifier|*
name|enterWindow
init|=
...
condition|)
block|{
comment|// Find the child window at cursor position, otherwise use the top level window
if|if
condition|(
name|QWindow
modifier|*
...
name|m_window
decl_stmt|;
comment|// If timestamp == 0, then it means that the window should not be
comment|// initially activated. Don't update global user time for this
...
argument_list|)
expr_stmt|;
comment|// Send the synthetic expose event on resize only when the window is shrinked,
comment|// because the "XCB_GRAVITY_NORTH_WEST" flag doesn't send it automatically.
if|if
condition|(
operator|!
...
parameter_list|)
block|{
comment|/* Checking for XCB_NOTIFY_MODE_GRAB and XCB_NOTIFY_DETAIL_ANCESTOR prevents unwanted      * enter/leave events on AwesomeWM on mouse button press. It also ignores duplicated      * enter/leave events on Alt+Tab switching on some WMs with XInput2 events.      * Without XInput2 events the (Un)grabAncestor cannot be checked when mouse button is      * not pressed, otherwise (e.g. on Alt+Tab) it can igonre important enter/leave events.     */
if|if
condition|(
name|conn
condition|)
...
name|mode
operator|==
name|XCB_NOTIFY_MODE_GRAB
operator|&&
name|detail
operator|==
name|XCB_NOTIFY_DETAIL_ANCESTOR
operator|)
operator|||
operator|(
name|mode
...
argument_list|)
decl_stmt|;
comment|// "mousePressWindow" can be NULL i.e. if a window will be grabbed or unmapped, so set it again here.
comment|// Unset "mousePressWindow" when mouse button isn't pressed - in some cases the release event won't arrive.
specifier|const
name|bool
...
expr_stmt|;
block|}
comment|// Handlers for plain xcb events. Used only when XI 2.2 or newer is not available.
DECL|function|handleButtonPressEvent
name|void
name|QXcbWindow
operator|::
...
return|;
block|}
comment|// With XI 2.2+ press/release/motion comes here instead of the above handlers.
...
argument_list|()
operator|<<
literal|"Unrecognized XI2 mouse event"
operator|<<
name|ev
operator|->
name|evtype
...
block|}
block|}
comment|// With XI 2.2+ enter/leave comes here and are blocked in plain xcb events
...
argument_list|)
decl_stmt|;
comment|// Compare the window with current mouse grabber to prevent deliver events to any other windows.
comment|// If leave event occurs and the window is under mouse - allow to deliver the leave event.
name|QXcbWindow
modifier|*
name|mouseGrabber
init|=
...
name|lcQpaXInput
argument_list|,
literal|"XI2 mouse enter %d,%d, mode %d, detail %d, time %d"
argument_list|,
name|event_x
argument_list|,
name|event_y
...
name|lcQpaXInput
argument_list|,
literal|"XI2 mouse leave, mode %d, detail %d, time %d"
argument_list|,
name|ev
operator|->
name|mode
