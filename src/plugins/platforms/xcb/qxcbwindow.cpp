...
directive|if
literal|0
block|QWidget *widget = window()->widget();     if (!widget)         return;      QWindowSurface *surface = widget->windowSurface();     if (surface) {         QRect rect(event->x, event->y, event->width, event->height);          surface->flush(widget, rect, QPoint());     }
endif|#
directive|endif
block|}
end_function
