...
expr_stmt|;
comment|// Known screen removed -> delete it
name|m_screens
operator|.
name|removeOne
argument_list|(
...
expr_stmt|;
block|}
comment|// else ignore disabled screens
block|}
elseif|else
if|if
condition|(
...
condition|)
block|{
comment|// Screen has been disabled -> remove
if|if
condition|(
name|output
operator|.
...
block|{
comment|// Just update existing screen
name|screen
operator|->
name|updateGeometry
argument_list|(
...
comment|// TODO: add a proper mechanism for updating primary screen
if|if
condition|(
operator|!
name|wasPrimary
...
argument_list|)
decl_stmt|;
comment|// TODO: RRGetScreenResources has to be called on each X display at least once before
comment|// RRGetScreenResourcesCurrent can be used - we can't know if we are the first application
comment|// to do so or not, so we always call the slower version here. Ideally we should share some
comment|// global flag (an atom on root window maybe) that at least other Qt apps would understand
comment|// and could call RRGetScreenResourcesCurrent here, speeding up start.
...
block|}
block|}
comment|// Push the screens to QApplication
name|QXcbIntegration
modifier|*
name|integration
init|=
...
name|lcQpaXInput
argument_list|,
literal|"xcb: pressed mouse button %d, button state %X"
argument_list|,
name|event
operator|->
name|detail
...
name|lcQpaXInput
argument_list|,
literal|"xcb: released mouse button %d, button state %X"
argument_list|,
name|event
operator|->
name|detail
...
name|qDebug
argument_list|(
literal|"Using threaded event reader with xcb_poll_for_queued_event"
argument_list|)
expr_stmt|;
endif|#
directive|endif
...
condition|)
block|{
comment|// compress multiple motion notify events in a row
comment|// to avoid swamping the event queue
name|xcb_generic_event_t
modifier|*
name|next
init|=
