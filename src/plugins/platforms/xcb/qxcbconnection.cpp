...
name|QXcbConnection
operator|::
name|createScreenWithFabricatedName
parameter_list|(
name|int
name|screenNumber
parameter_list|,
...
argument_list|)
decl_stmt|;
comment|// make sure the primary screen appears first since it is used by QGuiApplication::primaryScreen()
if|if
condition|(
...
comment|// Now iterate the individual outputs (e.g. DVI-I-1, VGA-1, etc.)
comment|// and make a QScreen instance for each.
name|xcb_screen_t
modifier|*
name|xcbScreen
init|=
...
condition|)
block|{
comment|// This happens on VNC for example.  But there is actually a screen anyway.
ifdef|#
directive|ifdef
name|Q_XCB_DEBUG
name|qDebug
