...
condition|)
block|{
comment|// other "Genius" tablets
comment|// WALTOP International Corp. Slim Tablet
name|isTablet
operator|=
literal|true
expr_stmt|;
...
condition|)
block|{
comment|// We want both mouse and touch through XI2 if touch is supported (>= 2.2).
comment|// The plain xcb press and motion events will not be delivered after this.
name|bitMask
operator||=
...
name|XI_MotionMask
expr_stmt|;
comment|// There is a check for enter/leave events in plain xcb enter/leave event handler
name|bitMask
operator||=
name|XI_EnterMask
expr_stmt|;
...
name|lcQpaXInput
argument_list|,
literal|"XInput 2.2: Selecting press/release/motion events in addition to touch"
argument_list|)
expr_stmt|;
block|}
name|XIEventMask
...
name|lcQpaXInput
argument_list|,
literal|"XInput 2.2: failed to select pointer/touch events, window %x, result %d"
argument_list|,
name|window
argument_list|,
name|result
...
condition|)
block|{
comment|// Only when XI2 mouse events are not enabled, otherwise motion and release are selected already.
name|QVector
argument_list|<
name|XIEventMask
argument_list|>
...
name|sourceid
expr_stmt|;
comment|// use the actual device id instead of the master
break|break;
block|}
case|case
name|XI_Enter
...
case|:
name|xi2HandleDeviceChangedEvent
argument_list|(
name|xiEvent
argument_list|)
expr_stmt|;
...
name|lcQpaXInput
argument_list|,
literal|"XI2 touch event type %d seq %d detail %d pos %6.1f, %6.1f root pos %6.1f, %6.1f on window %x"
argument_list|,
name|event
operator|->
name|event_type
...
condition|)
block|{
comment|// Find the closest axis.
comment|// 0 corresponds to the Y axis, vci->max to the X axis.
comment|// Flipping over the Y axis and rotating by 180 degrees
comment|// don't change the result, so normalize value to range
comment|// [0, vci->max] first.
name|value
operator|=
name|qAbs
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// Touches must be accepted when we are grabbing touch events. Otherwise the entire sequence
comment|// will get replayed when the grab ends.
if|if
condition|(
name|m_xiGrab
condition|)
block|{
comment|// XIAllowTouchEvents deadlocks with libXi< 1.7.4 (this has nothing to do with the XI2 versions like 2.2)
comment|// http://lists.x.org/archives/xorg-devel/2014-July/043059.html
ifndef|#
directive|ifndef
name|LIBXI_MAJOR
specifier|static
name|bool
name|allowTouchWarningShown
init|=
literal|false
decl_stmt|;
if|if
...
name|qWarning
argument_list|(
literal|"Skipping XIAllowTouchEvents() because it was not possible to detect libXi version at build time."
literal|" Minimum libXi version required is 1.7.4."
literal|" Expect issues with touch behavior."
argument_list|)
expr_stmt|;
block|}
elif|#
...
name|qWarning
argument_list|(
literal|"Skipping XIAllowTouchEvents() due to not having libXi>= 1.7.4."
literal|" libXi version at build time was %d.%d.%d."
literal|" Expect issues with touch behavior."
argument_list|,
name|LIBXI_MAJOR
argument_list|,
...
name|lcQpaXInput
argument_list|,
literal|"XInput 2.2: failed to ungrab events for device %d (result %d)"
argument_list|,
name|id
argument_list|,
name|result
...
name|lcQpaXInput
argument_list|,
literal|"XInput 2.2: failed to grab events for device %d on window %x (result %d)"
argument_list|,
name|id
argument_list|,
name|w
...
argument_list|)
decl_stmt|;
comment|// ### If a slave device changes (XIDeviceChange), we should probably run setup on it again.
if|if
condition|(
name|xiEvent
operator|->
...
directive|ifdef
name|XCB_USE_XINPUT21
comment|// This code handles broken scrolling device drivers that reset absolute positions
comment|// when they are made active. Whenever a new slave device is made active the
comment|// primary pointer sends a DeviceChanged event with XISlaveSwitch, and the new
comment|// active slave in sourceid.
name|QHash
argument_list|<
name|int
argument_list|,
...
name|lcQpaXInputDevices
argument_list|,
literal|"scrolling device %d no longer present"
argument_list|,
name|xiEvent
operator|->
name|sourceid
...
name|lcQpaXInput
argument_list|,
literal|"scrolling device %d moved from (%f, %f) to (%f, %f)"
argument_list|,
name|scrollingDevice
operator|.
name|deviceId
...
end_function
begin_function
DECL|function|tabletDataForDevice
name|QXcbConnection
operator|::
name|TabletData
modifier|*
