...
argument_list|)
expr_stmt|;
comment|// Convert the image to the native byte order.
name|xcb_image_t
modifier|*
...
argument_list|()
operator|<<
literal|"QXcbBackingStore::flush() wrong devicePixelRatio for backingstore image"
operator|<<
name|dpr
operator|<<
name|imageDpr
...
name|dpr
decl_stmt|;
comment|//because we multiply with the DPR later
name|QRegion
name|clipped
init|=
name|region
