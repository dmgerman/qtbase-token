...
condition|)
block|{
comment|// If workArea->value_len> 4, the remaining ones seem to be for WM's virtual desktops
comment|// (don't mess with QXcbVirtualDesktop which represents an X screen).
...
literal|nullptr
return|;
comment|// Try to find a RGB visual rather than e.g. BGR or GBR
for|for
control|(
specifier|const
name|xcb_visualtype_t
...
name|candidate
return|;
comment|// Did not find anything we like, just grab the first one and hope for the best
return|return
name|candidates
operator|.
name|first
...
argument_list|)
expr_stmt|;
comment|// TODO: Send an event to the QScreen instance that the screen changed its name
block|}
end_function
begin_function
