...
condition|)
block|{
comment|// Bit 7 indicates touch event, else tablet pen.
name|source
operator|=
name|Qt
operator|::
...
parameter_list|)
block|{
comment|// This is a workaround against some touchpads that send WM_HSCROLL instead of WM_MOUSEHWHEEL.
comment|// We could also handle vertical scroll here but there's no reason to, there's no bug for vertical
comment|// (broken vertical scroll would have been noticed long time ago), so lets keep the change small
comment|// and minimize the chance for regressions.
name|int
name|delta
init|=
literal|0
...
name|qWarning
argument_list|(
literal|"Unable to initialize touch handling."
argument_list|)
expr_stmt|;
return|return
literal|true
...
name|deltaX
init|=
name|GID_DIRECTMANIPULATION_DELTA_X
argument_list|(
name|gi
operator|.
name|ullArguments
...
name|deltaY
init|=
name|GID_DIRECTMANIPULATION_DELTA_Y
argument_list|(
name|gi
operator|.
name|ullArguments
argument_list|)
decl_stmt|;
comment|//Touch points are taken from the whole screen so map the position to the screen
specifier|const
name|QPoint
name|globalPosition
init|=
...
argument_list|)
expr_stmt|;
comment|// handle window focusing in/out
if|if
condition|(
name|window
operator|!=
