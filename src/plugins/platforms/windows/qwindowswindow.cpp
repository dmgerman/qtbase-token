...
end_function
begin_comment
comment|/*!     \brief Sets custom margins to be added to the default margins determined by     the windows style in the handling of the WM_NCCALCSIZE message.      This is currently used to give the Aero-style QWizard a smaller top margin.     The property can be set using QPlatformNativeInterface::setWindowProperty() or,     before platform window creation, by setting a dynamic property     on the QWindow (see QWindowsIntegration::createPlatformWindow()).      Note: The function uses (unscaled) device pixels since the QWizard also     uses AdjustWindowRect() and using device independent pixels would introduce     rounding errors. */
end_comment
begin_function
