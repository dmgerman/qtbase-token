...
destructor_decl|;
name|bool
name|ensureStaticOpenGLContext
parameter_list|()
function_decl|;
...
expr_stmt|;
block|}
comment|// Determine suitable scale factor, don't mix Windows and Qt scaling
if|if
condition|(
name|dpiAwareness
operator|!=
...
argument_list|)
decl_stmt|;
comment|// angle, desktop, software
specifier|const
name|bool
name|angleRequested
init|=
...
block|{
comment|// If opengl32.dll seems to be OpenGL 2.x capable, or desktop OpenGL is requested, use it.
if|if
condition|(
operator|!
name|softwareRequested
...
argument_list|()
expr_stmt|;
comment|// If failed and desktop OpenGL is not explicitly requested, try ANGLE.
if|if
condition|(
operator|!
name|ctx
...
argument_list|)
expr_stmt|;
comment|// If software was explicitly requested but failed, try the regular one.
if|if
condition|(
operator|!
name|ctx
