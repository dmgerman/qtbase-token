...
name|qFatal
argument_list|(
literal|"GetGlyphOutline() not supported under Windows CE. Please try using freetype font-rendering, by "
literal|"passing the command line argument -platform windows:fontengine=freetype to the application."
argument_list|)
expr_stmt|;
return|return
name|GDI_ERROR
...
end_define
begin_comment
comment|/* Shell stock icon IDs    SHGetStockIconInfo() is not available on CE, but we're using these    constants in code that is built on CE as well */
end_comment
begin_enum
enum|enum
block|{
...
end_ifndef
begin_comment
comment|// Value is wrong, but doesn't matter, not used at runtime
end_comment
begin_define
