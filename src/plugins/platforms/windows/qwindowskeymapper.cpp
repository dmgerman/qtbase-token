...
index|]
decl_stmt|;
comment|// While key combinations containing alt and ctrl might trigger the third assignment of a key
comment|// (for example "alt+ctrl+q" causes '@' on a German layout), ToUnicode often does not return the
comment|// wanted character if only the ctrl modifier is used. Thus we unset this modifier temporarily
comment|// if it is not used together with alt.
specifier|const
name|unsigned
name|char
name|controlState
...
directive|ifndef
name|QT_NO_SHORTCUT
comment|// Is Qt interested in the context menu key?
if|if
condition|(
name|modifiers
operator|==
