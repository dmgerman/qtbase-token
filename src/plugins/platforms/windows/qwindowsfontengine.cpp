...
name|qErrnoWarning
argument_list|(
literal|"%s: CreateFontIndirect failed for family '%s'"
argument_list|,
name|__FUNCTION__
argument_list|,
name|qPrintable
...
condition|)
block|{
comment|// If metrics requested, retrieve first using GGO_METRICS, because the returned
comment|// values are incorrect for OpenType PS fonts if obtained at the same time as the
comment|// glyph paths themselves (ie. with GGO_NATIVE as the format).
name|uint
name|format
init|=
name|GGO_METRICS
...
decl_stmt|;
name|bool
name|clearTypeTemporarilyDisabled
init|=
operator|(
name|m_fontEngineData
operator|->
