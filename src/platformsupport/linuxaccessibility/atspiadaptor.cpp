...
include|#
directive|include
file|"../accessibility/qaccessiblebridgeutils_p.h"
end_include
begin_include
include|#
directive|include
...
argument_list|()
decl_stmt|;
comment|// The id is always in the range [INT_MAX+1, UINT_MAX]
if|if
condition|(
operator|(
name|int
...
argument_list|()
operator|<<
literal|"No accessible object found for id: "
operator|<<
name|id
expr_stmt|;
return|return
...
argument_list|()
operator|<<
literal|"Sending TextCaretMoved from object that does not implement text interface: "
operator|<<
name|iface
expr_stmt|;
return|return;
...
condition|)
block|{
comment|// Combo Box with AT-SPI likes to be special
comment|// It requires a name-change to update caches and then selection-changed
name|QString
name|path
init|=
name|pathForInterface
...
argument_list|()
operator|<<
literal|"ValueChanged event and no ValueInterface or ComboBox: "
operator|<<
name|iface
expr_stmt|;
block|}
...
break|break;
block|}
comment|// For now we ignore these events
case|case
name|QAccessible
operator|::
name|TableModelChanged
case|:
comment|// For tables, setting manages_descendants should
comment|// indicate to the client that it cannot cache these
comment|// interfaces.
case|case
name|QAccessible
operator|::
...
argument_list|,
argument|QAccessibleBridgeUtils::effectiveActionNames(interface)
argument_list|)
block|{
name|QSpiAction
name|action
...
name|ia2Value
decl_stmt|;
comment|// IAccessible2: http://www.linuxfoundation.org/collaborate/workgroups/accessibility/iaccessible2/textattributes
comment|// ATK attribute names: https://git.gnome.org/browse/orca/tree/src/orca/text_attribute_names.py
comment|// ATK attribute values: https://developer.gnome.org/atk/unstable/AtkText.html#AtkTextAttribute
comment|// https://bugzilla.gnome.org/show_bug.cgi?id=744553 "ATK docs provide no guidance for allowed values of some text attributes"
comment|// specifically for "weight", "invalid", "language" and value range for colors
if|if
condition|(
name|ia2Name
operator|==
...
argument_list|()
operator|<<
literal|"Unknown text-align attribute value \""
operator|<<
name|value
operator|<<
literal|"\" cannot be translated to AT-SPI."
expr_stmt|;
block|}
block|}
block|}
...
argument_list|()
operator|<<
literal|"Unknown font-style attribute value \""
operator|<<
name|value
operator|<<
literal|"\" cannot be translated to AT-SPI."
...
argument_list|()
operator|<<
literal|"Unknown text-underline-type attribute value \""
operator|<<
name|value
operator|<<
literal|"\" cannot be translated to AT-SPI."
...
argument_list|)
condition|)
comment|// Orca seems to accept all IAccessible2 values except for "normal"
comment|// (on which it produces traceback and fails to read any following text attributes),
comment|// but that is the default value, so omit it anyway
name|value
operator|=
name|QString
argument_list|()
...
argument_list|()
operator|<<
literal|"Unknown text-position attribute value \""
operator|<<
name|value
operator|<<
literal|"\" cannot be translated to AT-SPI."
...
condition|)
block|{
comment|// IAccessible2 docs refer to XSL, which specifies "tb" is shorthand for "tb-rl"; so at least give a hint about the horizontal direction (ATK does not support vertical direction in this attribute (yet))
name|value
operator|=
name|QStringLiteral
argument_list|(
...
argument_list|()
operator|<<
literal|"writing-mode attribute value \"tb\" translated only w.r.t. horizontal direction; vertical direction ignored"
expr_stmt|;
block|}
else|else
block|{
...
argument_list|()
operator|<<
literal|"Unknown writing-mode attribute value \""
operator|<<
name|value
operator|<<
literal|"\" cannot be translated to AT-SPI."
...
condition|)
block|{
comment|// OK - ATK has no docs on the format of the value, IAccessible2 has reasonable format - leave it at that now
block|}
elseif|else
if|if
condition|(
...
condition|)
block|{
comment|// OK - ATK docs are vague but suggest they support the same range of values as IAccessible2
block|}
else|else
block|{
comment|// attribute we know nothing about
name|name
operator|=
name|QString
argument_list|()
...
argument_list|()
operator|<<
literal|"AtSpiAdaptor::valueInterface: Could not convert to double: "
operator|<<
name|function
expr_stmt|;
block|}
