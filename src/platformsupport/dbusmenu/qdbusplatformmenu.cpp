...
parameter_list|)
block|{
comment|// We need to check contains because otherwise QHash would insert
comment|// a default-constructed nullptr value into menuItemsByID
if|if
condition|(
name|menuItemsByID
operator|.
...
condition|)
block|{
comment|// disconnect from the signals we connected to in syncSubMenu()
specifier|const
name|QDBusPlatformMenu
modifier|*
name|menu
...
parameter_list|)
block|{
comment|// The adaptor is only connected to the propertiesUpdated signal of the top-level
comment|// menu, so the submenus should transfer their signals to their parents.
name|connect
argument_list|(
name|menu
argument_list|,
...
argument_list|)
decl_stmt|;
comment|// if a submenu was added to this item, we need to connect to its signals
if|if
condition|(
name|item
operator|->
