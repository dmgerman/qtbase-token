begin_unit
begin_comment
comment|/**************************************************************************** ** ** Copyright (C) 2016 The Qt Company Ltd. ** Contact: https://www.qt.io/licensing/ ** ** This file is part of the plugins of the Qt Toolkit. ** ** $QT_BEGIN_LICENSE:LGPL$ ** Commercial License Usage ** Licensees holding valid commercial Qt licenses may use this file in ** accordance with the commercial license agreement provided with the ** Software or, alternatively, in accordance with the terms contained in ** a written agreement between you and The Qt Company. For licensing terms ** and conditions see https://www.qt.io/terms-conditions. For further ** information use the contact form at https://www.qt.io/contact-us. ** ** GNU Lesser General Public License Usage ** Alternatively, this file may be used under the terms of the GNU Lesser ** General Public License version 3 as published by the Free Software ** Foundation and appearing in the file LICENSE.LGPL3 included in the ** packaging of this file. Please review the following information to ** ensure the GNU Lesser General Public License version 3 requirements ** will be met: https://www.gnu.org/licenses/lgpl-3.0.html. ** ** GNU General Public License Usage ** Alternatively, this file may be used under the terms of the GNU ** General Public License version 2.0 or (at your option) the GNU General ** Public license version 3 or any later version approved by the KDE Free ** Qt Foundation. The licenses are as published by the Free Software ** Foundation and appearing in the file LICENSE.GPL2 and LICENSE.GPL3 ** included in the packaging of this file. Please review the following ** information to ensure the GNU General Public License requirements will ** be met: https://www.gnu.org/licenses/gpl-2.0.html and ** https://www.gnu.org/licenses/gpl-3.0.html. ** ** $QT_END_LICENSE$ ** ****************************************************************************/
end_comment
begin_include
include|#
directive|include
file|"qgenericunixthemes_p.h"
end_include
begin_include
include|#
directive|include
file|"../../services/genericunix/qgenericunixservices_p.h"
end_include
begin_include
include|#
directive|include
file|"qpa/qplatformtheme_p.h"
end_include
begin_include
include|#
directive|include
file|<QtGui/QPalette>
end_include
begin_include
include|#
directive|include
file|<QtGui/QFont>
end_include
begin_include
include|#
directive|include
file|<QtGui/QGuiApplication>
end_include
begin_include
include|#
directive|include
file|<QtCore/QDir>
end_include
begin_include
include|#
directive|include
file|<QtCore/QFileInfo>
end_include
begin_include
include|#
directive|include
file|<QtCore/QFile>
end_include
begin_include
include|#
directive|include
file|<QtCore/QDebug>
end_include
begin_include
include|#
directive|include
file|<QtCore/QHash>
end_include
begin_include
include|#
directive|include
file|<QtCore/QLoggingCategory>
end_include
begin_include
include|#
directive|include
file|<QtCore/QSettings>
end_include
begin_include
include|#
directive|include
file|<QtCore/QVariant>
end_include
begin_include
include|#
directive|include
file|<QtCore/QStandardPaths>
end_include
begin_include
include|#
directive|include
file|<QtCore/QStringList>
end_include
begin_include
include|#
directive|include
file|<private/qguiapplication_p.h>
end_include
begin_include
include|#
directive|include
file|<qpa/qplatformintegration.h>
end_include
begin_include
include|#
directive|include
file|<qpa/qplatformservices.h>
end_include
begin_include
include|#
directive|include
file|<qpa/qplatformdialoghelper.h>
end_include
begin_ifndef
ifndef|#
directive|ifndef
name|QT_NO_DBUS
end_ifndef
begin_include
include|#
directive|include
file|"QtPlatformSupport/private/qdbusplatformmenu_p.h"
end_include
begin_include
include|#
directive|include
file|"QtPlatformSupport/private/qdbusmenubar_p.h"
end_include
begin_endif
endif|#
directive|endif
end_endif
begin_if
if|#
directive|if
operator|!
name|defined
argument_list|(
name|QT_NO_DBUS
argument_list|)
operator|&&
operator|!
name|defined
argument_list|(
name|QT_NO_SYSTEMTRAYICON
argument_list|)
end_if
begin_include
include|#
directive|include
file|"QtPlatformSupport/private/qdbustrayicon_p.h"
end_include
begin_endif
endif|#
directive|endif
end_endif
begin_include
include|#
directive|include
file|<algorithm>
end_include
begin_decl_stmt
name|QT_BEGIN_NAMESPACE
name|Q_DECLARE_LOGGING_CATEGORY
argument_list|(
name|qLcTray
argument_list|)
DECL|function|ResourceHelper
name|ResourceHelper
operator|::
name|ResourceHelper
argument_list|()
argument_list|{
name|std
operator|::
name|fill
argument_list|(
name|palettes
argument_list|,
name|palettes
operator|+
name|QPlatformTheme
operator|::
name|NPalettes
argument_list|,
cast|static_cast
argument_list|<
name|QPalette
operator|*
argument_list|>
argument_list|(
literal|0
argument_list|)
argument_list|)
argument_list|;
name|std
operator|::
name|fill
argument_list|(
name|fonts
argument_list|,
name|fonts
operator|+
name|QPlatformTheme
operator|::
name|NFonts
argument_list|,
cast|static_cast
argument_list|<
name|QFont
operator|*
argument_list|>
argument_list|(
literal|0
argument_list|)
argument_list|)
argument_list|; }
DECL|function|clear
name|void
name|ResourceHelper
operator|::
name|clear
argument_list|()
argument_list|{
name|qDeleteAll
argument_list|(
name|palettes
argument_list|,
name|palettes
operator|+
name|QPlatformTheme
operator|::
name|NPalettes
argument_list|)
argument_list|;
name|qDeleteAll
argument_list|(
name|fonts
argument_list|,
name|fonts
operator|+
name|QPlatformTheme
operator|::
name|NFonts
argument_list|)
argument_list|;
name|std
operator|::
name|fill
argument_list|(
name|palettes
argument_list|,
name|palettes
operator|+
name|QPlatformTheme
operator|::
name|NPalettes
argument_list|,
cast|static_cast
argument_list|<
name|QPalette
operator|*
argument_list|>
argument_list|(
literal|0
argument_list|)
argument_list|)
argument_list|;
name|std
operator|::
name|fill
argument_list|(
name|fonts
argument_list|,
name|fonts
operator|+
name|QPlatformTheme
operator|::
name|NFonts
argument_list|,
cast|static_cast
argument_list|<
name|QFont
operator|*
argument_list|>
argument_list|(
literal|0
argument_list|)
argument_list|)
argument_list|; }
comment|/*!     \class QGenericX11ThemeQKdeTheme     \brief QGenericX11Theme is a generic theme implementation for X11.     \since 5.0     \internal     \ingroup qpa */
decl|const
DECL|member|name
name|char
modifier|*
name|QGenericUnixTheme
operator|::
name|name
init|=
literal|"generic"
decl_stmt|;
end_decl_stmt
begin_comment
comment|// Default system font, corresponding to the value returned by 4.8 for
end_comment
begin_comment
comment|// XRender/FontConfig which we can now assume as default.
end_comment
begin_decl_stmt
DECL|variable|defaultSystemFontNameC
specifier|static
specifier|const
name|char
name|defaultSystemFontNameC
index|[]
init|=
literal|"Sans Serif"
decl_stmt|;
end_decl_stmt
begin_enum
DECL|enumerator|defaultSystemFontSize
enum|enum
block|{
name|defaultSystemFontSize
init|=
literal|9
block|}
enum|;
end_enum
begin_if
if|#
directive|if
operator|!
name|defined
argument_list|(
name|QT_NO_DBUS
argument_list|)
operator|&&
operator|!
name|defined
argument_list|(
name|QT_NO_SYSTEMTRAYICON
argument_list|)
end_if
begin_function
DECL|function|isDBusTrayAvailable
specifier|static
name|bool
name|isDBusTrayAvailable
parameter_list|()
block|{
specifier|static
name|bool
name|dbusTrayAvailable
init|=
literal|false
decl_stmt|;
specifier|static
name|bool
name|dbusTrayAvailableKnown
init|=
literal|false
decl_stmt|;
if|if
condition|(
operator|!
name|dbusTrayAvailableKnown
condition|)
block|{
name|QDBusMenuConnection
name|conn
decl_stmt|;
if|if
condition|(
name|conn
operator|.
name|isStatusNotifierHostRegistered
argument_list|()
condition|)
name|dbusTrayAvailable
operator|=
literal|true
expr_stmt|;
name|dbusTrayAvailableKnown
operator|=
literal|true
expr_stmt|;
name|qCDebug
argument_list|(
name|qLcTray
argument_list|)
operator|<<
literal|"D-Bus tray available:"
operator|<<
name|dbusTrayAvailable
expr_stmt|;
block|}
return|return
name|dbusTrayAvailable
return|;
block|}
end_function
begin_endif
endif|#
directive|endif
end_endif
begin_ifndef
ifndef|#
directive|ifndef
name|QT_NO_DBUS
end_ifndef
begin_function
DECL|function|checkDBusGlobalMenuAvailable
specifier|static
name|bool
name|checkDBusGlobalMenuAvailable
parameter_list|()
block|{
name|QDBusConnection
name|connection
init|=
name|QDBusConnection
operator|::
name|sessionBus
argument_list|()
decl_stmt|;
name|QString
name|registrarService
init|=
name|QStringLiteral
argument_list|(
literal|"com.canonical.AppMenu.Registrar"
argument_list|)
decl_stmt|;
return|return
name|connection
operator|.
name|interface
argument_list|()
operator|->
name|isServiceRegistered
argument_list|(
name|registrarService
argument_list|)
return|;
block|}
end_function
begin_function
DECL|function|isDBusGlobalMenuAvailable
specifier|static
name|bool
name|isDBusGlobalMenuAvailable
parameter_list|()
block|{
specifier|static
name|bool
name|dbusGlobalMenuAvailable
init|=
name|checkDBusGlobalMenuAvailable
argument_list|()
decl_stmt|;
return|return
name|dbusGlobalMenuAvailable
return|;
block|}
end_function
begin_endif
endif|#
directive|endif
end_endif
begin_class
DECL|class|QGenericUnixThemePrivate
class|class
name|QGenericUnixThemePrivate
super|:
specifier|public
name|QPlatformThemePrivate
block|{
public|public:
DECL|function|QGenericUnixThemePrivate
name|QGenericUnixThemePrivate
parameter_list|()
member_init_list|:
name|QPlatformThemePrivate
argument_list|()
member_init_list|,
name|systemFont
argument_list|(
name|QLatin1String
argument_list|(
name|defaultSystemFontNameC
argument_list|)
argument_list|,
name|defaultSystemFontSize
argument_list|)
member_init_list|,
name|fixedFont
argument_list|(
name|QStringLiteral
argument_list|(
literal|"monospace"
argument_list|)
argument_list|,
name|systemFont
operator|.
name|pointSize
argument_list|()
argument_list|)
block|{
name|fixedFont
operator|.
name|setStyleHint
argument_list|(
name|QFont
operator|::
name|TypeWriter
argument_list|)
expr_stmt|;
block|}
DECL|member|systemFont
specifier|const
name|QFont
name|systemFont
decl_stmt|;
DECL|member|fixedFont
name|QFont
name|fixedFont
decl_stmt|;
block|}
class|;
end_class
begin_constructor
DECL|function|QGenericUnixTheme
name|QGenericUnixTheme
operator|::
name|QGenericUnixTheme
parameter_list|()
member_init_list|:
name|QPlatformTheme
argument_list|(
operator|new
name|QGenericUnixThemePrivate
argument_list|()
argument_list|)
block|{ }
end_constructor
begin_function
DECL|function|font
specifier|const
name|QFont
modifier|*
name|QGenericUnixTheme
operator|::
name|font
parameter_list|(
name|Font
name|type
parameter_list|)
specifier|const
block|{
name|Q_D
argument_list|(
specifier|const
name|QGenericUnixTheme
argument_list|)
expr_stmt|;
switch|switch
condition|(
name|type
condition|)
block|{
case|case
name|QPlatformTheme
operator|::
name|SystemFont
case|:
return|return
operator|&
name|d
operator|->
name|systemFont
return|;
case|case
name|QPlatformTheme
operator|::
name|FixedFont
case|:
return|return
operator|&
name|d
operator|->
name|fixedFont
return|;
default|default:
return|return
literal|0
return|;
block|}
block|}
end_function
begin_comment
comment|// Helper to return the icon theme paths from XDG.
end_comment
begin_function
DECL|function|xdgIconThemePaths
name|QStringList
name|QGenericUnixTheme
operator|::
name|xdgIconThemePaths
parameter_list|()
block|{
name|QStringList
name|paths
decl_stmt|;
comment|// Add home directory first in search path
specifier|const
name|QFileInfo
name|homeIconDir
argument_list|(
name|QDir
operator|::
name|homePath
argument_list|()
operator|+
name|QLatin1String
argument_list|(
literal|"/.icons"
argument_list|)
argument_list|)
decl_stmt|;
if|if
condition|(
name|homeIconDir
operator|.
name|isDir
argument_list|()
condition|)
name|paths
operator|.
name|prepend
argument_list|(
name|homeIconDir
operator|.
name|absoluteFilePath
argument_list|()
argument_list|)
expr_stmt|;
name|QString
name|xdgDirString
init|=
name|QFile
operator|::
name|decodeName
argument_list|(
name|qgetenv
argument_list|(
literal|"XDG_DATA_DIRS"
argument_list|)
argument_list|)
decl_stmt|;
if|if
condition|(
name|xdgDirString
operator|.
name|isEmpty
argument_list|()
condition|)
name|xdgDirString
operator|=
name|QLatin1String
argument_list|(
literal|"/usr/local/share/:/usr/share/"
argument_list|)
expr_stmt|;
specifier|const
specifier|auto
name|xdgDirs
init|=
name|xdgDirString
operator|.
name|splitRef
argument_list|(
name|QLatin1Char
argument_list|(
literal|':'
argument_list|)
argument_list|)
decl_stmt|;
for|for
control|(
specifier|const
name|QStringRef
modifier|&
name|xdgDir
range|:
name|xdgDirs
control|)
block|{
specifier|const
name|QFileInfo
name|xdgIconsDir
argument_list|(
name|xdgDir
operator|+
name|QLatin1String
argument_list|(
literal|"/icons"
argument_list|)
argument_list|)
decl_stmt|;
if|if
condition|(
name|xdgIconsDir
operator|.
name|isDir
argument_list|()
condition|)
name|paths
operator|.
name|append
argument_list|(
name|xdgIconsDir
operator|.
name|absoluteFilePath
argument_list|()
argument_list|)
expr_stmt|;
block|}
specifier|const
name|QFileInfo
name|pixmapsIconsDir
argument_list|(
name|QStringLiteral
argument_list|(
literal|"/usr/share/pixmaps"
argument_list|)
argument_list|)
decl_stmt|;
if|if
condition|(
name|pixmapsIconsDir
operator|.
name|isDir
argument_list|()
condition|)
name|paths
operator|.
name|append
argument_list|(
name|pixmapsIconsDir
operator|.
name|absoluteFilePath
argument_list|()
argument_list|)
expr_stmt|;
return|return
name|paths
return|;
block|}
end_function
begin_ifndef
ifndef|#
directive|ifndef
name|QT_NO_DBUS
end_ifndef
begin_function
DECL|function|createPlatformMenuBar
name|QPlatformMenuBar
modifier|*
name|QGenericUnixTheme
operator|::
name|createPlatformMenuBar
parameter_list|()
specifier|const
block|{
if|if
condition|(
name|isDBusGlobalMenuAvailable
argument_list|()
condition|)
return|return
operator|new
name|QDBusMenuBar
argument_list|()
return|;
return|return
literal|nullptr
return|;
block|}
end_function
begin_endif
endif|#
directive|endif
end_endif
begin_if
if|#
directive|if
operator|!
name|defined
argument_list|(
name|QT_NO_DBUS
argument_list|)
operator|&&
operator|!
name|defined
argument_list|(
name|QT_NO_SYSTEMTRAYICON
argument_list|)
end_if
begin_function
DECL|function|createPlatformSystemTrayIcon
name|QPlatformSystemTrayIcon
modifier|*
name|QGenericUnixTheme
operator|::
name|createPlatformSystemTrayIcon
parameter_list|()
specifier|const
block|{
if|if
condition|(
name|isDBusTrayAvailable
argument_list|()
condition|)
return|return
operator|new
name|QDBusTrayIcon
argument_list|()
return|;
return|return
name|Q_NULLPTR
return|;
block|}
end_function
begin_endif
endif|#
directive|endif
end_endif
begin_function
DECL|function|themeHint
name|QVariant
name|QGenericUnixTheme
operator|::
name|themeHint
parameter_list|(
name|ThemeHint
name|hint
parameter_list|)
specifier|const
block|{
switch|switch
condition|(
name|hint
condition|)
block|{
case|case
name|QPlatformTheme
operator|::
name|SystemIconFallbackThemeName
case|:
return|return
name|QVariant
argument_list|(
name|QString
argument_list|(
name|QStringLiteral
argument_list|(
literal|"hicolor"
argument_list|)
argument_list|)
argument_list|)
return|;
case|case
name|QPlatformTheme
operator|::
name|IconThemeSearchPaths
case|:
return|return
name|xdgIconThemePaths
argument_list|()
return|;
case|case
name|QPlatformTheme
operator|::
name|DialogButtonBoxButtonsHaveIcons
case|:
return|return
name|QVariant
argument_list|(
literal|true
argument_list|)
return|;
case|case
name|QPlatformTheme
operator|::
name|StyleNames
case|:
block|{
name|QStringList
name|styleNames
decl_stmt|;
name|styleNames
operator|<<
name|QStringLiteral
argument_list|(
literal|"Fusion"
argument_list|)
operator|<<
name|QStringLiteral
argument_list|(
literal|"Windows"
argument_list|)
expr_stmt|;
return|return
name|QVariant
argument_list|(
name|styleNames
argument_list|)
return|;
block|}
case|case
name|QPlatformTheme
operator|::
name|KeyboardScheme
case|:
return|return
name|QVariant
argument_list|(
name|int
argument_list|(
name|X11KeyboardScheme
argument_list|)
argument_list|)
return|;
default|default:
break|break;
block|}
return|return
name|QPlatformTheme
operator|::
name|themeHint
argument_list|(
name|hint
argument_list|)
return|;
block|}
end_function
begin_ifndef
ifndef|#
directive|ifndef
name|QT_NO_SETTINGS
end_ifndef
begin_class
DECL|class|QKdeThemePrivate
class|class
name|QKdeThemePrivate
super|:
specifier|public
name|QPlatformThemePrivate
block|{
public|public:
DECL|function|QKdeThemePrivate
name|QKdeThemePrivate
parameter_list|(
specifier|const
name|QStringList
modifier|&
name|kdeDirs
parameter_list|,
name|int
name|kdeVersion
parameter_list|)
member_init_list|:
name|kdeDirs
argument_list|(
name|kdeDirs
argument_list|)
member_init_list|,
name|kdeVersion
argument_list|(
name|kdeVersion
argument_list|)
member_init_list|,
name|toolButtonStyle
argument_list|(
name|Qt
operator|::
name|ToolButtonTextBesideIcon
argument_list|)
member_init_list|,
name|toolBarIconSize
argument_list|(
literal|0
argument_list|)
member_init_list|,
name|singleClick
argument_list|(
literal|true
argument_list|)
member_init_list|,
name|wheelScrollLines
argument_list|(
literal|3
argument_list|)
block|{ }
DECL|function|kdeGlobals
specifier|static
name|QString
name|kdeGlobals
parameter_list|(
specifier|const
name|QString
modifier|&
name|kdeDir
parameter_list|,
name|int
name|kdeVersion
parameter_list|)
block|{
if|if
condition|(
name|kdeVersion
operator|>
literal|4
condition|)
return|return
name|kdeDir
operator|+
name|QLatin1String
argument_list|(
literal|"/kdeglobals"
argument_list|)
return|;
return|return
name|kdeDir
operator|+
name|QLatin1String
argument_list|(
literal|"/share/config/kdeglobals"
argument_list|)
return|;
block|}
name|void
name|refresh
parameter_list|()
function_decl|;
specifier|static
name|QVariant
name|readKdeSetting
parameter_list|(
specifier|const
name|QString
modifier|&
name|key
parameter_list|,
specifier|const
name|QStringList
modifier|&
name|kdeDirs
parameter_list|,
name|int
name|kdeVersion
parameter_list|,
name|QHash
argument_list|<
name|QString
argument_list|,
name|QSettings
modifier|*
argument_list|>
modifier|&
name|kdeSettings
parameter_list|)
function_decl|;
specifier|static
name|void
name|readKdeSystemPalette
parameter_list|(
specifier|const
name|QStringList
modifier|&
name|kdeDirs
parameter_list|,
name|int
name|kdeVersion
parameter_list|,
name|QHash
argument_list|<
name|QString
argument_list|,
name|QSettings
modifier|*
argument_list|>
modifier|&
name|kdeSettings
parameter_list|,
name|QPalette
modifier|*
name|pal
parameter_list|)
function_decl|;
specifier|static
name|QFont
modifier|*
name|kdeFont
parameter_list|(
specifier|const
name|QVariant
modifier|&
name|fontValue
parameter_list|)
function_decl|;
specifier|static
name|QStringList
name|kdeIconThemeSearchPaths
parameter_list|(
specifier|const
name|QStringList
modifier|&
name|kdeDirs
parameter_list|)
function_decl|;
DECL|member|kdeDirs
specifier|const
name|QStringList
name|kdeDirs
decl_stmt|;
DECL|member|kdeVersion
specifier|const
name|int
name|kdeVersion
decl_stmt|;
DECL|member|resources
name|ResourceHelper
name|resources
decl_stmt|;
DECL|member|iconThemeName
name|QString
name|iconThemeName
decl_stmt|;
DECL|member|iconFallbackThemeName
name|QString
name|iconFallbackThemeName
decl_stmt|;
DECL|member|styleNames
name|QStringList
name|styleNames
decl_stmt|;
DECL|member|toolButtonStyle
name|int
name|toolButtonStyle
decl_stmt|;
DECL|member|toolBarIconSize
name|int
name|toolBarIconSize
decl_stmt|;
DECL|member|singleClick
name|bool
name|singleClick
decl_stmt|;
DECL|member|wheelScrollLines
name|int
name|wheelScrollLines
decl_stmt|;
block|}
class|;
end_class
begin_function
DECL|function|refresh
name|void
name|QKdeThemePrivate
operator|::
name|refresh
parameter_list|()
block|{
name|resources
operator|.
name|clear
argument_list|()
expr_stmt|;
name|toolButtonStyle
operator|=
name|Qt
operator|::
name|ToolButtonTextBesideIcon
expr_stmt|;
name|toolBarIconSize
operator|=
literal|0
expr_stmt|;
name|styleNames
operator|.
name|clear
argument_list|()
expr_stmt|;
if|if
condition|(
name|kdeVersion
operator|>=
literal|5
condition|)
name|styleNames
operator|<<
name|QStringLiteral
argument_list|(
literal|"breeze"
argument_list|)
expr_stmt|;
name|styleNames
operator|<<
name|QStringLiteral
argument_list|(
literal|"Oxygen"
argument_list|)
operator|<<
name|QStringLiteral
argument_list|(
literal|"fusion"
argument_list|)
operator|<<
name|QStringLiteral
argument_list|(
literal|"windows"
argument_list|)
expr_stmt|;
if|if
condition|(
name|kdeVersion
operator|>=
literal|5
condition|)
name|iconFallbackThemeName
operator|=
name|iconThemeName
operator|=
name|QStringLiteral
argument_list|(
literal|"breeze"
argument_list|)
expr_stmt|;
else|else
name|iconFallbackThemeName
operator|=
name|iconThemeName
operator|=
name|QStringLiteral
argument_list|(
literal|"oxygen"
argument_list|)
expr_stmt|;
name|QHash
argument_list|<
name|QString
argument_list|,
name|QSettings
modifier|*
argument_list|>
name|kdeSettings
decl_stmt|;
name|QPalette
name|systemPalette
init|=
name|QPalette
argument_list|()
decl_stmt|;
name|readKdeSystemPalette
argument_list|(
name|kdeDirs
argument_list|,
name|kdeVersion
argument_list|,
name|kdeSettings
argument_list|,
operator|&
name|systemPalette
argument_list|)
expr_stmt|;
name|resources
operator|.
name|palettes
index|[
name|QPlatformTheme
operator|::
name|SystemPalette
index|]
operator|=
operator|new
name|QPalette
argument_list|(
name|systemPalette
argument_list|)
expr_stmt|;
comment|//## TODO tooltip color
specifier|const
name|QVariant
name|styleValue
init|=
name|readKdeSetting
argument_list|(
name|QStringLiteral
argument_list|(
literal|"widgetStyle"
argument_list|)
argument_list|,
name|kdeDirs
argument_list|,
name|kdeVersion
argument_list|,
name|kdeSettings
argument_list|)
decl_stmt|;
if|if
condition|(
name|styleValue
operator|.
name|isValid
argument_list|()
condition|)
block|{
specifier|const
name|QString
name|style
init|=
name|styleValue
operator|.
name|toString
argument_list|()
decl_stmt|;
if|if
condition|(
name|style
operator|!=
name|styleNames
operator|.
name|front
argument_list|()
condition|)
name|styleNames
operator|.
name|push_front
argument_list|(
name|style
argument_list|)
expr_stmt|;
block|}
specifier|const
name|QVariant
name|singleClickValue
init|=
name|readKdeSetting
argument_list|(
name|QStringLiteral
argument_list|(
literal|"KDE/SingleClick"
argument_list|)
argument_list|,
name|kdeDirs
argument_list|,
name|kdeVersion
argument_list|,
name|kdeSettings
argument_list|)
decl_stmt|;
if|if
condition|(
name|singleClickValue
operator|.
name|isValid
argument_list|()
condition|)
name|singleClick
operator|=
name|singleClickValue
operator|.
name|toBool
argument_list|()
expr_stmt|;
specifier|const
name|QVariant
name|themeValue
init|=
name|readKdeSetting
argument_list|(
name|QStringLiteral
argument_list|(
literal|"Icons/Theme"
argument_list|)
argument_list|,
name|kdeDirs
argument_list|,
name|kdeVersion
argument_list|,
name|kdeSettings
argument_list|)
decl_stmt|;
if|if
condition|(
name|themeValue
operator|.
name|isValid
argument_list|()
condition|)
name|iconThemeName
operator|=
name|themeValue
operator|.
name|toString
argument_list|()
expr_stmt|;
specifier|const
name|QVariant
name|toolBarIconSizeValue
init|=
name|readKdeSetting
argument_list|(
name|QStringLiteral
argument_list|(
literal|"ToolbarIcons/Size"
argument_list|)
argument_list|,
name|kdeDirs
argument_list|,
name|kdeVersion
argument_list|,
name|kdeSettings
argument_list|)
decl_stmt|;
if|if
condition|(
name|toolBarIconSizeValue
operator|.
name|isValid
argument_list|()
condition|)
name|toolBarIconSize
operator|=
name|toolBarIconSizeValue
operator|.
name|toInt
argument_list|()
expr_stmt|;
specifier|const
name|QVariant
name|toolbarStyleValue
init|=
name|readKdeSetting
argument_list|(
name|QStringLiteral
argument_list|(
literal|"Toolbar style/ToolButtonStyle"
argument_list|)
argument_list|,
name|kdeDirs
argument_list|,
name|kdeVersion
argument_list|,
name|kdeSettings
argument_list|)
decl_stmt|;
if|if
condition|(
name|toolbarStyleValue
operator|.
name|isValid
argument_list|()
condition|)
block|{
specifier|const
name|QString
name|toolBarStyle
init|=
name|toolbarStyleValue
operator|.
name|toString
argument_list|()
decl_stmt|;
if|if
condition|(
name|toolBarStyle
operator|==
name|QLatin1String
argument_list|(
literal|"TextBesideIcon"
argument_list|)
condition|)
name|toolButtonStyle
operator|=
name|Qt
operator|::
name|ToolButtonTextBesideIcon
expr_stmt|;
elseif|else
if|if
condition|(
name|toolBarStyle
operator|==
name|QLatin1String
argument_list|(
literal|"TextOnly"
argument_list|)
condition|)
name|toolButtonStyle
operator|=
name|Qt
operator|::
name|ToolButtonTextOnly
expr_stmt|;
elseif|else
if|if
condition|(
name|toolBarStyle
operator|==
name|QLatin1String
argument_list|(
literal|"TextUnderIcon"
argument_list|)
condition|)
name|toolButtonStyle
operator|=
name|Qt
operator|::
name|ToolButtonTextUnderIcon
expr_stmt|;
block|}
specifier|const
name|QVariant
name|wheelScrollLinesValue
init|=
name|readKdeSetting
argument_list|(
name|QStringLiteral
argument_list|(
literal|"KDE/WheelScrollLines"
argument_list|)
argument_list|,
name|kdeDirs
argument_list|,
name|kdeVersion
argument_list|,
name|kdeSettings
argument_list|)
decl_stmt|;
if|if
condition|(
name|wheelScrollLinesValue
operator|.
name|isValid
argument_list|()
condition|)
name|wheelScrollLines
operator|=
name|wheelScrollLinesValue
operator|.
name|toInt
argument_list|()
expr_stmt|;
comment|// Read system font, ignore 'smallestReadableFont'
if|if
condition|(
name|QFont
modifier|*
name|systemFont
init|=
name|kdeFont
argument_list|(
name|readKdeSetting
argument_list|(
name|QStringLiteral
argument_list|(
literal|"font"
argument_list|)
argument_list|,
name|kdeDirs
argument_list|,
name|kdeVersion
argument_list|,
name|kdeSettings
argument_list|)
argument_list|)
condition|)
name|resources
operator|.
name|fonts
index|[
name|QPlatformTheme
operator|::
name|SystemFont
index|]
operator|=
name|systemFont
expr_stmt|;
else|else
name|resources
operator|.
name|fonts
index|[
name|QPlatformTheme
operator|::
name|SystemFont
index|]
operator|=
operator|new
name|QFont
argument_list|(
name|QLatin1String
argument_list|(
name|defaultSystemFontNameC
argument_list|)
argument_list|,
name|defaultSystemFontSize
argument_list|)
expr_stmt|;
if|if
condition|(
name|QFont
modifier|*
name|fixedFont
init|=
name|kdeFont
argument_list|(
name|readKdeSetting
argument_list|(
name|QStringLiteral
argument_list|(
literal|"fixed"
argument_list|)
argument_list|,
name|kdeDirs
argument_list|,
name|kdeVersion
argument_list|,
name|kdeSettings
argument_list|)
argument_list|)
condition|)
block|{
name|resources
operator|.
name|fonts
index|[
name|QPlatformTheme
operator|::
name|FixedFont
index|]
operator|=
name|fixedFont
expr_stmt|;
block|}
else|else
block|{
name|fixedFont
operator|=
operator|new
name|QFont
argument_list|(
name|QLatin1String
argument_list|(
name|defaultSystemFontNameC
argument_list|)
argument_list|,
name|defaultSystemFontSize
argument_list|)
expr_stmt|;
name|fixedFont
operator|->
name|setStyleHint
argument_list|(
name|QFont
operator|::
name|TypeWriter
argument_list|)
expr_stmt|;
name|resources
operator|.
name|fonts
index|[
name|QPlatformTheme
operator|::
name|FixedFont
index|]
operator|=
name|fixedFont
expr_stmt|;
block|}
name|qDeleteAll
argument_list|(
name|kdeSettings
argument_list|)
expr_stmt|;
block|}
end_function
begin_function
DECL|function|readKdeSetting
name|QVariant
name|QKdeThemePrivate
operator|::
name|readKdeSetting
parameter_list|(
specifier|const
name|QString
modifier|&
name|key
parameter_list|,
specifier|const
name|QStringList
modifier|&
name|kdeDirs
parameter_list|,
name|int
name|kdeVersion
parameter_list|,
name|QHash
argument_list|<
name|QString
argument_list|,
name|QSettings
modifier|*
argument_list|>
modifier|&
name|kdeSettings
parameter_list|)
block|{
foreach|foreach
control|(
specifier|const
name|QString
modifier|&
name|kdeDir
decl|,
name|kdeDirs
control|)
block|{
name|QSettings
modifier|*
name|settings
init|=
name|kdeSettings
operator|.
name|value
argument_list|(
name|kdeDir
argument_list|)
decl_stmt|;
if|if
condition|(
operator|!
name|settings
condition|)
block|{
specifier|const
name|QString
name|kdeGlobalsPath
init|=
name|kdeGlobals
argument_list|(
name|kdeDir
argument_list|,
name|kdeVersion
argument_list|)
decl_stmt|;
if|if
condition|(
name|QFileInfo
argument_list|(
name|kdeGlobalsPath
argument_list|)
operator|.
name|isReadable
argument_list|()
condition|)
block|{
name|settings
operator|=
operator|new
name|QSettings
argument_list|(
name|kdeGlobalsPath
argument_list|,
name|QSettings
operator|::
name|IniFormat
argument_list|)
expr_stmt|;
name|kdeSettings
operator|.
name|insert
argument_list|(
name|kdeDir
argument_list|,
name|settings
argument_list|)
expr_stmt|;
block|}
block|}
if|if
condition|(
name|settings
condition|)
block|{
specifier|const
name|QVariant
name|value
init|=
name|settings
operator|->
name|value
argument_list|(
name|key
argument_list|)
decl_stmt|;
if|if
condition|(
name|value
operator|.
name|isValid
argument_list|()
condition|)
return|return
name|value
return|;
block|}
block|}
return|return
name|QVariant
argument_list|()
return|;
block|}
end_function
begin_comment
comment|// Reads the color from the KDE configuration, and store it in the
end_comment
begin_comment
comment|// palette with the given color role if found.
end_comment
begin_function
DECL|function|kdeColor
specifier|static
specifier|inline
name|bool
name|kdeColor
parameter_list|(
name|QPalette
modifier|*
name|pal
parameter_list|,
name|QPalette
operator|::
name|ColorRole
name|role
parameter_list|,
specifier|const
name|QVariant
modifier|&
name|value
parameter_list|)
block|{
if|if
condition|(
operator|!
name|value
operator|.
name|isValid
argument_list|()
condition|)
return|return
literal|false
return|;
specifier|const
name|QStringList
name|values
init|=
name|value
operator|.
name|toStringList
argument_list|()
decl_stmt|;
if|if
condition|(
name|values
operator|.
name|size
argument_list|()
operator|!=
literal|3
condition|)
return|return
literal|false
return|;
name|pal
operator|->
name|setBrush
argument_list|(
name|role
argument_list|,
name|QColor
argument_list|(
name|values
operator|.
name|at
argument_list|(
literal|0
argument_list|)
operator|.
name|toInt
argument_list|()
argument_list|,
name|values
operator|.
name|at
argument_list|(
literal|1
argument_list|)
operator|.
name|toInt
argument_list|()
argument_list|,
name|values
operator|.
name|at
argument_list|(
literal|2
argument_list|)
operator|.
name|toInt
argument_list|()
argument_list|)
argument_list|)
expr_stmt|;
return|return
literal|true
return|;
block|}
end_function
begin_function
DECL|function|readKdeSystemPalette
name|void
name|QKdeThemePrivate
operator|::
name|readKdeSystemPalette
parameter_list|(
specifier|const
name|QStringList
modifier|&
name|kdeDirs
parameter_list|,
name|int
name|kdeVersion
parameter_list|,
name|QHash
argument_list|<
name|QString
argument_list|,
name|QSettings
modifier|*
argument_list|>
modifier|&
name|kdeSettings
parameter_list|,
name|QPalette
modifier|*
name|pal
parameter_list|)
block|{
if|if
condition|(
operator|!
name|kdeColor
argument_list|(
name|pal
argument_list|,
name|QPalette
operator|::
name|Button
argument_list|,
name|readKdeSetting
argument_list|(
name|QStringLiteral
argument_list|(
literal|"Colors:Button/BackgroundNormal"
argument_list|)
argument_list|,
name|kdeDirs
argument_list|,
name|kdeVersion
argument_list|,
name|kdeSettings
argument_list|)
argument_list|)
condition|)
block|{
comment|// kcolorscheme.cpp: SetDefaultColors
specifier|const
name|QColor
name|defaultWindowBackground
argument_list|(
literal|214
argument_list|,
literal|210
argument_list|,
literal|208
argument_list|)
decl_stmt|;
specifier|const
name|QColor
name|defaultButtonBackground
argument_list|(
literal|223
argument_list|,
literal|220
argument_list|,
literal|217
argument_list|)
decl_stmt|;
operator|*
name|pal
operator|=
name|QPalette
argument_list|(
name|defaultButtonBackground
argument_list|,
name|defaultWindowBackground
argument_list|)
expr_stmt|;
return|return;
block|}
name|kdeColor
argument_list|(
name|pal
argument_list|,
name|QPalette
operator|::
name|Window
argument_list|,
name|readKdeSetting
argument_list|(
name|QStringLiteral
argument_list|(
literal|"Colors:Window/BackgroundNormal"
argument_list|)
argument_list|,
name|kdeDirs
argument_list|,
name|kdeVersion
argument_list|,
name|kdeSettings
argument_list|)
argument_list|)
expr_stmt|;
name|kdeColor
argument_list|(
name|pal
argument_list|,
name|QPalette
operator|::
name|Text
argument_list|,
name|readKdeSetting
argument_list|(
name|QStringLiteral
argument_list|(
literal|"Colors:View/ForegroundNormal"
argument_list|)
argument_list|,
name|kdeDirs
argument_list|,
name|kdeVersion
argument_list|,
name|kdeSettings
argument_list|)
argument_list|)
expr_stmt|;
name|kdeColor
argument_list|(
name|pal
argument_list|,
name|QPalette
operator|::
name|WindowText
argument_list|,
name|readKdeSetting
argument_list|(
name|QStringLiteral
argument_list|(
literal|"Colors:Window/ForegroundNormal"
argument_list|)
argument_list|,
name|kdeDirs
argument_list|,
name|kdeVersion
argument_list|,
name|kdeSettings
argument_list|)
argument_list|)
expr_stmt|;
name|kdeColor
argument_list|(
name|pal
argument_list|,
name|QPalette
operator|::
name|Base
argument_list|,
name|readKdeSetting
argument_list|(
name|QStringLiteral
argument_list|(
literal|"Colors:View/BackgroundNormal"
argument_list|)
argument_list|,
name|kdeDirs
argument_list|,
name|kdeVersion
argument_list|,
name|kdeSettings
argument_list|)
argument_list|)
expr_stmt|;
name|kdeColor
argument_list|(
name|pal
argument_list|,
name|QPalette
operator|::
name|Highlight
argument_list|,
name|readKdeSetting
argument_list|(
name|QStringLiteral
argument_list|(
literal|"Colors:Selection/BackgroundNormal"
argument_list|)
argument_list|,
name|kdeDirs
argument_list|,
name|kdeVersion
argument_list|,
name|kdeSettings
argument_list|)
argument_list|)
expr_stmt|;
name|kdeColor
argument_list|(
name|pal
argument_list|,
name|QPalette
operator|::
name|HighlightedText
argument_list|,
name|readKdeSetting
argument_list|(
name|QStringLiteral
argument_list|(
literal|"Colors:Selection/ForegroundNormal"
argument_list|)
argument_list|,
name|kdeDirs
argument_list|,
name|kdeVersion
argument_list|,
name|kdeSettings
argument_list|)
argument_list|)
expr_stmt|;
name|kdeColor
argument_list|(
name|pal
argument_list|,
name|QPalette
operator|::
name|AlternateBase
argument_list|,
name|readKdeSetting
argument_list|(
name|QStringLiteral
argument_list|(
literal|"Colors:View/BackgroundAlternate"
argument_list|)
argument_list|,
name|kdeDirs
argument_list|,
name|kdeVersion
argument_list|,
name|kdeSettings
argument_list|)
argument_list|)
expr_stmt|;
name|kdeColor
argument_list|(
name|pal
argument_list|,
name|QPalette
operator|::
name|ButtonText
argument_list|,
name|readKdeSetting
argument_list|(
name|QStringLiteral
argument_list|(
literal|"Colors:Button/ForegroundNormal"
argument_list|)
argument_list|,
name|kdeDirs
argument_list|,
name|kdeVersion
argument_list|,
name|kdeSettings
argument_list|)
argument_list|)
expr_stmt|;
name|kdeColor
argument_list|(
name|pal
argument_list|,
name|QPalette
operator|::
name|Link
argument_list|,
name|readKdeSetting
argument_list|(
name|QStringLiteral
argument_list|(
literal|"Colors:View/ForegroundLink"
argument_list|)
argument_list|,
name|kdeDirs
argument_list|,
name|kdeVersion
argument_list|,
name|kdeSettings
argument_list|)
argument_list|)
expr_stmt|;
name|kdeColor
argument_list|(
name|pal
argument_list|,
name|QPalette
operator|::
name|LinkVisited
argument_list|,
name|readKdeSetting
argument_list|(
name|QStringLiteral
argument_list|(
literal|"Colors:View/ForegroundVisited"
argument_list|)
argument_list|,
name|kdeDirs
argument_list|,
name|kdeVersion
argument_list|,
name|kdeSettings
argument_list|)
argument_list|)
expr_stmt|;
name|kdeColor
argument_list|(
name|pal
argument_list|,
name|QPalette
operator|::
name|ToolTipBase
argument_list|,
name|readKdeSetting
argument_list|(
name|QStringLiteral
argument_list|(
literal|"Colors:Tooltip/BackgroundNormal"
argument_list|)
argument_list|,
name|kdeDirs
argument_list|,
name|kdeVersion
argument_list|,
name|kdeSettings
argument_list|)
argument_list|)
expr_stmt|;
name|kdeColor
argument_list|(
name|pal
argument_list|,
name|QPalette
operator|::
name|ToolTipText
argument_list|,
name|readKdeSetting
argument_list|(
name|QStringLiteral
argument_list|(
literal|"Colors:Tooltip/ForegroundNormal"
argument_list|)
argument_list|,
name|kdeDirs
argument_list|,
name|kdeVersion
argument_list|,
name|kdeSettings
argument_list|)
argument_list|)
expr_stmt|;
comment|// The above code sets _all_ color roles to "normal" colors. In KDE, the disabled
comment|// color roles are calculated by applying various effects described in kdeglobals.
comment|// We use a bit simpler approach here, similar logic than in qt_palette_from_color().
specifier|const
name|QColor
name|button
init|=
name|pal
operator|->
name|color
argument_list|(
name|QPalette
operator|::
name|Button
argument_list|)
decl_stmt|;
name|int
name|h
decl_stmt|,
name|s
decl_stmt|,
name|v
decl_stmt|;
name|button
operator|.
name|getHsv
argument_list|(
operator|&
name|h
argument_list|,
operator|&
name|s
argument_list|,
operator|&
name|v
argument_list|)
expr_stmt|;
specifier|const
name|QBrush
name|whiteBrush
init|=
name|QBrush
argument_list|(
name|Qt
operator|::
name|white
argument_list|)
decl_stmt|;
specifier|const
name|QBrush
name|buttonBrush
init|=
name|QBrush
argument_list|(
name|button
argument_list|)
decl_stmt|;
specifier|const
name|QBrush
name|buttonBrushDark
init|=
name|QBrush
argument_list|(
name|button
operator|.
name|darker
argument_list|(
name|v
operator|>
literal|128
condition|?
literal|200
else|:
literal|50
argument_list|)
argument_list|)
decl_stmt|;
specifier|const
name|QBrush
name|buttonBrushDark150
init|=
name|QBrush
argument_list|(
name|button
operator|.
name|darker
argument_list|(
name|v
operator|>
literal|128
condition|?
literal|150
else|:
literal|75
argument_list|)
argument_list|)
decl_stmt|;
specifier|const
name|QBrush
name|buttonBrushLight150
init|=
name|QBrush
argument_list|(
name|button
operator|.
name|lighter
argument_list|(
name|v
operator|>
literal|128
condition|?
literal|150
else|:
literal|75
argument_list|)
argument_list|)
decl_stmt|;
specifier|const
name|QBrush
name|buttonBrushLight
init|=
name|QBrush
argument_list|(
name|button
operator|.
name|lighter
argument_list|(
name|v
operator|>
literal|128
condition|?
literal|200
else|:
literal|50
argument_list|)
argument_list|)
decl_stmt|;
name|pal
operator|->
name|setBrush
argument_list|(
name|QPalette
operator|::
name|Disabled
argument_list|,
name|QPalette
operator|::
name|WindowText
argument_list|,
name|buttonBrushDark
argument_list|)
expr_stmt|;
name|pal
operator|->
name|setBrush
argument_list|(
name|QPalette
operator|::
name|Disabled
argument_list|,
name|QPalette
operator|::
name|ButtonText
argument_list|,
name|buttonBrushDark
argument_list|)
expr_stmt|;
name|pal
operator|->
name|setBrush
argument_list|(
name|QPalette
operator|::
name|Disabled
argument_list|,
name|QPalette
operator|::
name|Button
argument_list|,
name|buttonBrush
argument_list|)
expr_stmt|;
name|pal
operator|->
name|setBrush
argument_list|(
name|QPalette
operator|::
name|Disabled
argument_list|,
name|QPalette
operator|::
name|Text
argument_list|,
name|buttonBrushDark
argument_list|)
expr_stmt|;
name|pal
operator|->
name|setBrush
argument_list|(
name|QPalette
operator|::
name|Disabled
argument_list|,
name|QPalette
operator|::
name|BrightText
argument_list|,
name|whiteBrush
argument_list|)
expr_stmt|;
name|pal
operator|->
name|setBrush
argument_list|(
name|QPalette
operator|::
name|Disabled
argument_list|,
name|QPalette
operator|::
name|Base
argument_list|,
name|buttonBrush
argument_list|)
expr_stmt|;
name|pal
operator|->
name|setBrush
argument_list|(
name|QPalette
operator|::
name|Disabled
argument_list|,
name|QPalette
operator|::
name|Window
argument_list|,
name|buttonBrush
argument_list|)
expr_stmt|;
name|pal
operator|->
name|setBrush
argument_list|(
name|QPalette
operator|::
name|Disabled
argument_list|,
name|QPalette
operator|::
name|Highlight
argument_list|,
name|buttonBrushDark150
argument_list|)
expr_stmt|;
name|pal
operator|->
name|setBrush
argument_list|(
name|QPalette
operator|::
name|Disabled
argument_list|,
name|QPalette
operator|::
name|HighlightedText
argument_list|,
name|buttonBrushLight150
argument_list|)
expr_stmt|;
comment|// set calculated colors for all groups
name|pal
operator|->
name|setBrush
argument_list|(
name|QPalette
operator|::
name|Light
argument_list|,
name|buttonBrushLight
argument_list|)
expr_stmt|;
name|pal
operator|->
name|setBrush
argument_list|(
name|QPalette
operator|::
name|Midlight
argument_list|,
name|buttonBrushLight150
argument_list|)
expr_stmt|;
name|pal
operator|->
name|setBrush
argument_list|(
name|QPalette
operator|::
name|Mid
argument_list|,
name|buttonBrushDark150
argument_list|)
expr_stmt|;
name|pal
operator|->
name|setBrush
argument_list|(
name|QPalette
operator|::
name|Dark
argument_list|,
name|buttonBrushDark
argument_list|)
expr_stmt|;
block|}
end_function
begin_comment
comment|/*!     \class QKdeTheme     \brief QKdeTheme is a theme implementation for the KDE desktop (version 4 or higher).     \since 5.0     \internal     \ingroup qpa */
end_comment
begin_decl_stmt
DECL|member|name
specifier|const
name|char
modifier|*
name|QKdeTheme
operator|::
name|name
init|=
literal|"kde"
decl_stmt|;
end_decl_stmt
begin_constructor
DECL|function|QKdeTheme
name|QKdeTheme
operator|::
name|QKdeTheme
parameter_list|(
specifier|const
name|QStringList
modifier|&
name|kdeDirs
parameter_list|,
name|int
name|kdeVersion
parameter_list|)
member_init_list|:
name|QPlatformTheme
argument_list|(
operator|new
name|QKdeThemePrivate
argument_list|(
name|kdeDirs
argument_list|,
name|kdeVersion
argument_list|)
argument_list|)
block|{
name|d_func
argument_list|()
operator|->
name|refresh
argument_list|()
expr_stmt|;
block|}
end_constructor
begin_function
DECL|function|kdeFont
name|QFont
modifier|*
name|QKdeThemePrivate
operator|::
name|kdeFont
parameter_list|(
specifier|const
name|QVariant
modifier|&
name|fontValue
parameter_list|)
block|{
if|if
condition|(
name|fontValue
operator|.
name|isValid
argument_list|()
condition|)
block|{
comment|// Read font value: Might be a QStringList as KDE stores fonts without quotes.
comment|// Also retrieve the family for the constructor since we cannot use the
comment|// default constructor of QFont, which accesses QGuiApplication::systemFont()
comment|// causing recursion.
name|QString
name|fontDescription
decl_stmt|;
name|QString
name|fontFamily
decl_stmt|;
if|if
condition|(
name|fontValue
operator|.
name|type
argument_list|()
operator|==
name|QVariant
operator|::
name|StringList
condition|)
block|{
specifier|const
name|QStringList
name|list
init|=
name|fontValue
operator|.
name|toStringList
argument_list|()
decl_stmt|;
if|if
condition|(
operator|!
name|list
operator|.
name|isEmpty
argument_list|()
condition|)
block|{
name|fontFamily
operator|=
name|list
operator|.
name|first
argument_list|()
expr_stmt|;
name|fontDescription
operator|=
name|list
operator|.
name|join
argument_list|(
name|QLatin1Char
argument_list|(
literal|','
argument_list|)
argument_list|)
expr_stmt|;
block|}
block|}
else|else
block|{
name|fontDescription
operator|=
name|fontFamily
operator|=
name|fontValue
operator|.
name|toString
argument_list|()
expr_stmt|;
block|}
if|if
condition|(
operator|!
name|fontDescription
operator|.
name|isEmpty
argument_list|()
condition|)
block|{
name|QFont
name|font
argument_list|(
name|fontFamily
argument_list|)
decl_stmt|;
if|if
condition|(
name|font
operator|.
name|fromString
argument_list|(
name|fontDescription
argument_list|)
condition|)
return|return
operator|new
name|QFont
argument_list|(
name|font
argument_list|)
return|;
block|}
block|}
return|return
literal|0
return|;
block|}
end_function
begin_function
DECL|function|kdeIconThemeSearchPaths
name|QStringList
name|QKdeThemePrivate
operator|::
name|kdeIconThemeSearchPaths
parameter_list|(
specifier|const
name|QStringList
modifier|&
name|kdeDirs
parameter_list|)
block|{
name|QStringList
name|paths
init|=
name|QGenericUnixTheme
operator|::
name|xdgIconThemePaths
argument_list|()
decl_stmt|;
specifier|const
name|QString
name|iconPath
init|=
name|QStringLiteral
argument_list|(
literal|"/share/icons"
argument_list|)
decl_stmt|;
foreach|foreach
control|(
specifier|const
name|QString
modifier|&
name|candidate
decl|,
name|kdeDirs
control|)
block|{
specifier|const
name|QFileInfo
name|fi
argument_list|(
name|candidate
operator|+
name|iconPath
argument_list|)
decl_stmt|;
if|if
condition|(
name|fi
operator|.
name|isDir
argument_list|()
condition|)
name|paths
operator|.
name|append
argument_list|(
name|fi
operator|.
name|absoluteFilePath
argument_list|()
argument_list|)
expr_stmt|;
block|}
return|return
name|paths
return|;
block|}
end_function
begin_function
DECL|function|themeHint
name|QVariant
name|QKdeTheme
operator|::
name|themeHint
parameter_list|(
name|QPlatformTheme
operator|::
name|ThemeHint
name|hint
parameter_list|)
specifier|const
block|{
name|Q_D
argument_list|(
specifier|const
name|QKdeTheme
argument_list|)
expr_stmt|;
switch|switch
condition|(
name|hint
condition|)
block|{
case|case
name|QPlatformTheme
operator|::
name|UseFullScreenForPopupMenu
case|:
return|return
name|QVariant
argument_list|(
literal|true
argument_list|)
return|;
case|case
name|QPlatformTheme
operator|::
name|DialogButtonBoxButtonsHaveIcons
case|:
return|return
name|QVariant
argument_list|(
literal|true
argument_list|)
return|;
case|case
name|QPlatformTheme
operator|::
name|DialogButtonBoxLayout
case|:
return|return
name|QVariant
argument_list|(
name|QPlatformDialogHelper
operator|::
name|KdeLayout
argument_list|)
return|;
case|case
name|QPlatformTheme
operator|::
name|ToolButtonStyle
case|:
return|return
name|QVariant
argument_list|(
name|d
operator|->
name|toolButtonStyle
argument_list|)
return|;
case|case
name|QPlatformTheme
operator|::
name|ToolBarIconSize
case|:
return|return
name|QVariant
argument_list|(
name|d
operator|->
name|toolBarIconSize
argument_list|)
return|;
case|case
name|QPlatformTheme
operator|::
name|SystemIconThemeName
case|:
return|return
name|QVariant
argument_list|(
name|d
operator|->
name|iconThemeName
argument_list|)
return|;
case|case
name|QPlatformTheme
operator|::
name|SystemIconFallbackThemeName
case|:
return|return
name|QVariant
argument_list|(
name|d
operator|->
name|iconFallbackThemeName
argument_list|)
return|;
case|case
name|QPlatformTheme
operator|::
name|IconThemeSearchPaths
case|:
return|return
name|QVariant
argument_list|(
name|d
operator|->
name|kdeIconThemeSearchPaths
argument_list|(
name|d
operator|->
name|kdeDirs
argument_list|)
argument_list|)
return|;
case|case
name|QPlatformTheme
operator|::
name|StyleNames
case|:
return|return
name|QVariant
argument_list|(
name|d
operator|->
name|styleNames
argument_list|)
return|;
case|case
name|QPlatformTheme
operator|::
name|KeyboardScheme
case|:
return|return
name|QVariant
argument_list|(
name|int
argument_list|(
name|KdeKeyboardScheme
argument_list|)
argument_list|)
return|;
case|case
name|QPlatformTheme
operator|::
name|ItemViewActivateItemOnSingleClick
case|:
return|return
name|QVariant
argument_list|(
name|d
operator|->
name|singleClick
argument_list|)
return|;
case|case
name|QPlatformTheme
operator|::
name|WheelScrollLines
case|:
return|return
name|QVariant
argument_list|(
name|d
operator|->
name|wheelScrollLines
argument_list|)
return|;
default|default:
break|break;
block|}
return|return
name|QPlatformTheme
operator|::
name|themeHint
argument_list|(
name|hint
argument_list|)
return|;
block|}
end_function
begin_function
DECL|function|palette
specifier|const
name|QPalette
modifier|*
name|QKdeTheme
operator|::
name|palette
parameter_list|(
name|Palette
name|type
parameter_list|)
specifier|const
block|{
name|Q_D
argument_list|(
specifier|const
name|QKdeTheme
argument_list|)
expr_stmt|;
return|return
name|d
operator|->
name|resources
operator|.
name|palettes
index|[
name|type
index|]
return|;
block|}
end_function
begin_function
DECL|function|font
specifier|const
name|QFont
modifier|*
name|QKdeTheme
operator|::
name|font
parameter_list|(
name|Font
name|type
parameter_list|)
specifier|const
block|{
name|Q_D
argument_list|(
specifier|const
name|QKdeTheme
argument_list|)
expr_stmt|;
return|return
name|d
operator|->
name|resources
operator|.
name|fonts
index|[
name|type
index|]
return|;
block|}
end_function
begin_function
DECL|function|createKdeTheme
name|QPlatformTheme
modifier|*
name|QKdeTheme
operator|::
name|createKdeTheme
parameter_list|()
block|{
specifier|const
name|QByteArray
name|kdeVersionBA
init|=
name|qgetenv
argument_list|(
literal|"KDE_SESSION_VERSION"
argument_list|)
decl_stmt|;
specifier|const
name|int
name|kdeVersion
init|=
name|kdeVersionBA
operator|.
name|toInt
argument_list|()
decl_stmt|;
if|if
condition|(
name|kdeVersion
operator|<
literal|4
condition|)
return|return
literal|0
return|;
if|if
condition|(
name|kdeVersion
operator|>
literal|4
condition|)
comment|// Plasma 5 follows XDG spec
comment|// but uses the same config file format:
return|return
operator|new
name|QKdeTheme
argument_list|(
name|QStandardPaths
operator|::
name|standardLocations
argument_list|(
name|QStandardPaths
operator|::
name|GenericConfigLocation
argument_list|)
argument_list|,
name|kdeVersion
argument_list|)
return|;
comment|// Determine KDE prefixes in the following priority order:
comment|// - KDEHOME and KDEDIRS environment variables
comment|// - ~/.kde(<version>)
comment|// - read prefixes from /etc/kde<version>rc
comment|// - fallback to /etc/kde<version>
name|QStringList
name|kdeDirs
decl_stmt|;
specifier|const
name|QString
name|kdeHomePathVar
init|=
name|QFile
operator|::
name|decodeName
argument_list|(
name|qgetenv
argument_list|(
literal|"KDEHOME"
argument_list|)
argument_list|)
decl_stmt|;
if|if
condition|(
operator|!
name|kdeHomePathVar
operator|.
name|isEmpty
argument_list|()
condition|)
name|kdeDirs
operator|+=
name|kdeHomePathVar
expr_stmt|;
specifier|const
name|QString
name|kdeDirsVar
init|=
name|QFile
operator|::
name|decodeName
argument_list|(
name|qgetenv
argument_list|(
literal|"KDEDIRS"
argument_list|)
argument_list|)
decl_stmt|;
if|if
condition|(
operator|!
name|kdeDirsVar
operator|.
name|isEmpty
argument_list|()
condition|)
name|kdeDirs
operator|+=
name|kdeDirsVar
operator|.
name|split
argument_list|(
name|QLatin1Char
argument_list|(
literal|':'
argument_list|)
argument_list|,
name|QString
operator|::
name|SkipEmptyParts
argument_list|)
expr_stmt|;
specifier|const
name|QString
name|kdeVersionHomePath
init|=
name|QDir
operator|::
name|homePath
argument_list|()
operator|+
name|QLatin1String
argument_list|(
literal|"/.kde"
argument_list|)
operator|+
name|QLatin1String
argument_list|(
name|kdeVersionBA
argument_list|)
decl_stmt|;
if|if
condition|(
name|QFileInfo
argument_list|(
name|kdeVersionHomePath
argument_list|)
operator|.
name|isDir
argument_list|()
condition|)
name|kdeDirs
operator|+=
name|kdeVersionHomePath
expr_stmt|;
specifier|const
name|QString
name|kdeHomePath
init|=
name|QDir
operator|::
name|homePath
argument_list|()
operator|+
name|QLatin1String
argument_list|(
literal|"/.kde"
argument_list|)
decl_stmt|;
if|if
condition|(
name|QFileInfo
argument_list|(
name|kdeHomePath
argument_list|)
operator|.
name|isDir
argument_list|()
condition|)
name|kdeDirs
operator|+=
name|kdeHomePath
expr_stmt|;
specifier|const
name|QString
name|kdeRcPath
init|=
name|QLatin1String
argument_list|(
literal|"/etc/kde"
argument_list|)
operator|+
name|QLatin1String
argument_list|(
name|kdeVersionBA
argument_list|)
operator|+
name|QLatin1String
argument_list|(
literal|"rc"
argument_list|)
decl_stmt|;
if|if
condition|(
name|QFileInfo
argument_list|(
name|kdeRcPath
argument_list|)
operator|.
name|isReadable
argument_list|()
condition|)
block|{
name|QSettings
name|kdeSettings
argument_list|(
name|kdeRcPath
argument_list|,
name|QSettings
operator|::
name|IniFormat
argument_list|)
decl_stmt|;
name|kdeSettings
operator|.
name|beginGroup
argument_list|(
name|QStringLiteral
argument_list|(
literal|"Directories-default"
argument_list|)
argument_list|)
expr_stmt|;
name|kdeDirs
operator|+=
name|kdeSettings
operator|.
name|value
argument_list|(
name|QStringLiteral
argument_list|(
literal|"prefixes"
argument_list|)
argument_list|)
operator|.
name|toStringList
argument_list|()
expr_stmt|;
block|}
specifier|const
name|QString
name|kdeVersionPrefix
init|=
name|QLatin1String
argument_list|(
literal|"/etc/kde"
argument_list|)
operator|+
name|QLatin1String
argument_list|(
name|kdeVersionBA
argument_list|)
decl_stmt|;
if|if
condition|(
name|QFileInfo
argument_list|(
name|kdeVersionPrefix
argument_list|)
operator|.
name|isDir
argument_list|()
condition|)
name|kdeDirs
operator|+=
name|kdeVersionPrefix
expr_stmt|;
name|kdeDirs
operator|.
name|removeDuplicates
argument_list|()
expr_stmt|;
if|if
condition|(
name|kdeDirs
operator|.
name|isEmpty
argument_list|()
condition|)
block|{
name|qWarning
argument_list|(
literal|"Unable to determine KDE dirs"
argument_list|)
expr_stmt|;
return|return
literal|0
return|;
block|}
return|return
operator|new
name|QKdeTheme
argument_list|(
name|kdeDirs
argument_list|,
name|kdeVersion
argument_list|)
return|;
block|}
end_function
begin_ifndef
ifndef|#
directive|ifndef
name|QT_NO_DBUS
end_ifndef
begin_function
DECL|function|createPlatformMenuBar
name|QPlatformMenuBar
modifier|*
name|QKdeTheme
operator|::
name|createPlatformMenuBar
parameter_list|()
specifier|const
block|{
if|if
condition|(
name|isDBusGlobalMenuAvailable
argument_list|()
condition|)
return|return
operator|new
name|QDBusMenuBar
argument_list|()
return|;
return|return
literal|nullptr
return|;
block|}
end_function
begin_endif
endif|#
directive|endif
end_endif
begin_if
if|#
directive|if
operator|!
name|defined
argument_list|(
name|QT_NO_DBUS
argument_list|)
operator|&&
operator|!
name|defined
argument_list|(
name|QT_NO_SYSTEMTRAYICON
argument_list|)
end_if
begin_function
DECL|function|createPlatformSystemTrayIcon
name|QPlatformSystemTrayIcon
modifier|*
name|QKdeTheme
operator|::
name|createPlatformSystemTrayIcon
parameter_list|()
specifier|const
block|{
if|if
condition|(
name|isDBusTrayAvailable
argument_list|()
condition|)
return|return
operator|new
name|QDBusTrayIcon
argument_list|()
return|;
return|return
name|Q_NULLPTR
return|;
block|}
end_function
begin_endif
endif|#
directive|endif
end_endif
begin_endif
endif|#
directive|endif
end_endif
begin_comment
comment|// QT_NO_SETTINGS
end_comment
begin_comment
comment|/*!     \class QGnomeTheme     \brief QGnomeTheme is a theme implementation for the Gnome desktop.     \since 5.0     \internal     \ingroup qpa */
end_comment
begin_decl_stmt
DECL|member|name
specifier|const
name|char
modifier|*
name|QGnomeTheme
operator|::
name|name
init|=
literal|"gnome"
decl_stmt|;
end_decl_stmt
begin_class
DECL|class|QGnomeThemePrivate
class|class
name|QGnomeThemePrivate
super|:
specifier|public
name|QPlatformThemePrivate
block|{
public|public:
DECL|function|QGnomeThemePrivate
name|QGnomeThemePrivate
parameter_list|()
member_init_list|:
name|systemFont
argument_list|(
name|Q_NULLPTR
argument_list|)
member_init_list|,
name|fixedFont
argument_list|(
name|Q_NULLPTR
argument_list|)
block|{}
DECL|function|~QGnomeThemePrivate
name|~
name|QGnomeThemePrivate
parameter_list|()
block|{
operator|delete
name|systemFont
expr_stmt|;
operator|delete
name|fixedFont
expr_stmt|;
block|}
DECL|function|configureFonts
name|void
name|configureFonts
parameter_list|(
specifier|const
name|QString
modifier|&
name|gtkFontName
parameter_list|)
specifier|const
block|{
name|Q_ASSERT
argument_list|(
operator|!
name|systemFont
argument_list|)
expr_stmt|;
specifier|const
name|int
name|split
init|=
name|gtkFontName
operator|.
name|lastIndexOf
argument_list|(
name|QChar
operator|::
name|Space
argument_list|)
decl_stmt|;
name|float
name|size
init|=
name|gtkFontName
operator|.
name|midRef
argument_list|(
name|split
operator|+
literal|1
argument_list|)
operator|.
name|toFloat
argument_list|()
decl_stmt|;
name|QString
name|fontName
init|=
name|gtkFontName
operator|.
name|left
argument_list|(
name|split
argument_list|)
decl_stmt|;
name|systemFont
operator|=
operator|new
name|QFont
argument_list|(
name|fontName
argument_list|,
name|size
argument_list|)
expr_stmt|;
name|fixedFont
operator|=
operator|new
name|QFont
argument_list|(
name|QLatin1String
argument_list|(
literal|"monospace"
argument_list|)
argument_list|,
name|systemFont
operator|->
name|pointSize
argument_list|()
argument_list|)
expr_stmt|;
name|fixedFont
operator|->
name|setStyleHint
argument_list|(
name|QFont
operator|::
name|TypeWriter
argument_list|)
expr_stmt|;
block|}
DECL|member|systemFont
specifier|mutable
name|QFont
modifier|*
name|systemFont
decl_stmt|;
DECL|member|fixedFont
specifier|mutable
name|QFont
modifier|*
name|fixedFont
decl_stmt|;
block|}
class|;
end_class
begin_constructor
DECL|function|QGnomeTheme
name|QGnomeTheme
operator|::
name|QGnomeTheme
parameter_list|()
member_init_list|:
name|QPlatformTheme
argument_list|(
operator|new
name|QGnomeThemePrivate
argument_list|()
argument_list|)
block|{ }
end_constructor
begin_function
DECL|function|themeHint
name|QVariant
name|QGnomeTheme
operator|::
name|themeHint
parameter_list|(
name|QPlatformTheme
operator|::
name|ThemeHint
name|hint
parameter_list|)
specifier|const
block|{
switch|switch
condition|(
name|hint
condition|)
block|{
case|case
name|QPlatformTheme
operator|::
name|DialogButtonBoxButtonsHaveIcons
case|:
return|return
name|QVariant
argument_list|(
literal|true
argument_list|)
return|;
case|case
name|QPlatformTheme
operator|::
name|DialogButtonBoxLayout
case|:
return|return
name|QVariant
argument_list|(
name|QPlatformDialogHelper
operator|::
name|GnomeLayout
argument_list|)
return|;
case|case
name|QPlatformTheme
operator|::
name|SystemIconThemeName
case|:
return|return
name|QVariant
argument_list|(
name|QStringLiteral
argument_list|(
literal|"Adwaita"
argument_list|)
argument_list|)
return|;
case|case
name|QPlatformTheme
operator|::
name|SystemIconFallbackThemeName
case|:
return|return
name|QVariant
argument_list|(
name|QStringLiteral
argument_list|(
literal|"gnome"
argument_list|)
argument_list|)
return|;
case|case
name|QPlatformTheme
operator|::
name|IconThemeSearchPaths
case|:
return|return
name|QVariant
argument_list|(
name|QGenericUnixTheme
operator|::
name|xdgIconThemePaths
argument_list|()
argument_list|)
return|;
case|case
name|QPlatformTheme
operator|::
name|StyleNames
case|:
block|{
name|QStringList
name|styleNames
decl_stmt|;
name|styleNames
operator|<<
name|QStringLiteral
argument_list|(
literal|"fusion"
argument_list|)
operator|<<
name|QStringLiteral
argument_list|(
literal|"windows"
argument_list|)
expr_stmt|;
return|return
name|QVariant
argument_list|(
name|styleNames
argument_list|)
return|;
block|}
case|case
name|QPlatformTheme
operator|::
name|KeyboardScheme
case|:
return|return
name|QVariant
argument_list|(
name|int
argument_list|(
name|GnomeKeyboardScheme
argument_list|)
argument_list|)
return|;
case|case
name|QPlatformTheme
operator|::
name|PasswordMaskCharacter
case|:
return|return
name|QVariant
argument_list|(
name|QChar
argument_list|(
literal|0x2022
argument_list|)
argument_list|)
return|;
default|default:
break|break;
block|}
return|return
name|QPlatformTheme
operator|::
name|themeHint
argument_list|(
name|hint
argument_list|)
return|;
block|}
end_function
begin_function
DECL|function|font
specifier|const
name|QFont
modifier|*
name|QGnomeTheme
operator|::
name|font
parameter_list|(
name|Font
name|type
parameter_list|)
specifier|const
block|{
name|Q_D
argument_list|(
specifier|const
name|QGnomeTheme
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|d
operator|->
name|systemFont
condition|)
name|d
operator|->
name|configureFonts
argument_list|(
name|gtkFontName
argument_list|()
argument_list|)
expr_stmt|;
switch|switch
condition|(
name|type
condition|)
block|{
case|case
name|QPlatformTheme
operator|::
name|SystemFont
case|:
return|return
name|d
operator|->
name|systemFont
return|;
case|case
name|QPlatformTheme
operator|::
name|FixedFont
case|:
return|return
name|d
operator|->
name|fixedFont
return|;
default|default:
return|return
literal|0
return|;
block|}
block|}
end_function
begin_function
DECL|function|gtkFontName
name|QString
name|QGnomeTheme
operator|::
name|gtkFontName
parameter_list|()
specifier|const
block|{
return|return
name|QStringLiteral
argument_list|(
literal|"%1 %2"
argument_list|)
operator|.
name|arg
argument_list|(
name|QLatin1String
argument_list|(
name|defaultSystemFontNameC
argument_list|)
argument_list|)
operator|.
name|arg
argument_list|(
name|defaultSystemFontSize
argument_list|)
return|;
block|}
end_function
begin_ifndef
ifndef|#
directive|ifndef
name|QT_NO_DBUS
end_ifndef
begin_function
DECL|function|createPlatformMenuBar
name|QPlatformMenuBar
modifier|*
name|QGnomeTheme
operator|::
name|createPlatformMenuBar
parameter_list|()
specifier|const
block|{
if|if
condition|(
name|isDBusGlobalMenuAvailable
argument_list|()
condition|)
return|return
operator|new
name|QDBusMenuBar
argument_list|()
return|;
return|return
literal|nullptr
return|;
block|}
end_function
begin_endif
endif|#
directive|endif
end_endif
begin_if
if|#
directive|if
operator|!
name|defined
argument_list|(
name|QT_NO_DBUS
argument_list|)
operator|&&
operator|!
name|defined
argument_list|(
name|QT_NO_SYSTEMTRAYICON
argument_list|)
end_if
begin_function
DECL|function|createPlatformSystemTrayIcon
name|QPlatformSystemTrayIcon
modifier|*
name|QGnomeTheme
operator|::
name|createPlatformSystemTrayIcon
parameter_list|()
specifier|const
block|{
if|if
condition|(
name|isDBusTrayAvailable
argument_list|()
condition|)
return|return
operator|new
name|QDBusTrayIcon
argument_list|()
return|;
return|return
name|Q_NULLPTR
return|;
block|}
end_function
begin_endif
endif|#
directive|endif
end_endif
begin_function
DECL|function|standardButtonText
name|QString
name|QGnomeTheme
operator|::
name|standardButtonText
parameter_list|(
name|int
name|button
parameter_list|)
specifier|const
block|{
switch|switch
condition|(
name|button
condition|)
block|{
case|case
name|QPlatformDialogHelper
operator|::
name|Ok
case|:
return|return
name|QCoreApplication
operator|::
name|translate
argument_list|(
literal|"QGnomeTheme"
argument_list|,
literal|"&OK"
argument_list|)
return|;
case|case
name|QPlatformDialogHelper
operator|::
name|Save
case|:
return|return
name|QCoreApplication
operator|::
name|translate
argument_list|(
literal|"QGnomeTheme"
argument_list|,
literal|"&Save"
argument_list|)
return|;
case|case
name|QPlatformDialogHelper
operator|::
name|Cancel
case|:
return|return
name|QCoreApplication
operator|::
name|translate
argument_list|(
literal|"QGnomeTheme"
argument_list|,
literal|"&Cancel"
argument_list|)
return|;
case|case
name|QPlatformDialogHelper
operator|::
name|Close
case|:
return|return
name|QCoreApplication
operator|::
name|translate
argument_list|(
literal|"QGnomeTheme"
argument_list|,
literal|"&Close"
argument_list|)
return|;
case|case
name|QPlatformDialogHelper
operator|::
name|Discard
case|:
return|return
name|QCoreApplication
operator|::
name|translate
argument_list|(
literal|"QGnomeTheme"
argument_list|,
literal|"Close without Saving"
argument_list|)
return|;
default|default:
break|break;
block|}
return|return
name|QPlatformTheme
operator|::
name|standardButtonText
argument_list|(
name|button
argument_list|)
return|;
block|}
end_function
begin_comment
comment|/*!     \brief Creates a UNIX theme according to the detected desktop environment. */
end_comment
begin_function
DECL|function|createUnixTheme
name|QPlatformTheme
modifier|*
name|QGenericUnixTheme
operator|::
name|createUnixTheme
parameter_list|(
specifier|const
name|QString
modifier|&
name|name
parameter_list|)
block|{
if|if
condition|(
name|name
operator|==
name|QLatin1String
argument_list|(
name|QGenericUnixTheme
operator|::
name|name
argument_list|)
condition|)
return|return
operator|new
name|QGenericUnixTheme
return|;
ifndef|#
directive|ifndef
name|QT_NO_SETTINGS
if|if
condition|(
name|name
operator|==
name|QLatin1String
argument_list|(
name|QKdeTheme
operator|::
name|name
argument_list|)
condition|)
if|if
condition|(
name|QPlatformTheme
modifier|*
name|kdeTheme
init|=
name|QKdeTheme
operator|::
name|createKdeTheme
argument_list|()
condition|)
return|return
name|kdeTheme
return|;
endif|#
directive|endif
if|if
condition|(
name|name
operator|==
name|QLatin1String
argument_list|(
name|QGnomeTheme
operator|::
name|name
argument_list|)
condition|)
return|return
operator|new
name|QGnomeTheme
return|;
return|return
name|Q_NULLPTR
return|;
block|}
end_function
begin_function
DECL|function|themeNames
name|QStringList
name|QGenericUnixTheme
operator|::
name|themeNames
parameter_list|()
block|{
name|QStringList
name|result
decl_stmt|;
if|if
condition|(
name|QGuiApplication
operator|::
name|desktopSettingsAware
argument_list|()
condition|)
block|{
specifier|const
name|QByteArray
name|desktopEnvironment
init|=
name|QGuiApplicationPrivate
operator|::
name|platformIntegration
argument_list|()
operator|->
name|services
argument_list|()
operator|->
name|desktopEnvironment
argument_list|()
decl_stmt|;
name|QList
argument_list|<
name|QByteArray
argument_list|>
name|gtkBasedEnvironments
decl_stmt|;
name|gtkBasedEnvironments
operator|<<
literal|"GNOME"
operator|<<
literal|"X-CINNAMON"
operator|<<
literal|"UNITY"
operator|<<
literal|"MATE"
operator|<<
literal|"XFCE"
operator|<<
literal|"LXDE"
expr_stmt|;
name|QList
argument_list|<
name|QByteArray
argument_list|>
name|desktopNames
init|=
name|desktopEnvironment
operator|.
name|split
argument_list|(
literal|':'
argument_list|)
decl_stmt|;
name|Q_FOREACH
argument_list|(
argument|const QByteArray&desktopName
argument_list|,
argument|desktopNames
argument_list|)
block|{
if|if
condition|(
name|desktopEnvironment
operator|==
literal|"KDE"
condition|)
block|{
ifndef|#
directive|ifndef
name|QT_NO_SETTINGS
name|result
operator|.
name|push_back
argument_list|(
name|QLatin1String
argument_list|(
name|QKdeTheme
operator|::
name|name
argument_list|)
argument_list|)
expr_stmt|;
endif|#
directive|endif
block|}
elseif|else
if|if
condition|(
name|gtkBasedEnvironments
operator|.
name|contains
argument_list|(
name|desktopName
argument_list|)
condition|)
block|{
comment|// prefer the GTK3 theme implementation with native dialogs etc.
name|result
operator|.
name|push_back
argument_list|(
name|QStringLiteral
argument_list|(
literal|"gtk3"
argument_list|)
argument_list|)
expr_stmt|;
comment|// fallback to the generic Gnome theme if loading the GTK3 theme fails
name|result
operator|.
name|push_back
argument_list|(
name|QLatin1String
argument_list|(
name|QGnomeTheme
operator|::
name|name
argument_list|)
argument_list|)
expr_stmt|;
block|}
block|}
specifier|const
name|QString
name|session
init|=
name|QString
operator|::
name|fromLocal8Bit
argument_list|(
name|qgetenv
argument_list|(
literal|"DESKTOP_SESSION"
argument_list|)
argument_list|)
decl_stmt|;
if|if
condition|(
operator|!
name|session
operator|.
name|isEmpty
argument_list|()
operator|&&
name|session
operator|!=
name|QLatin1String
argument_list|(
literal|"default"
argument_list|)
operator|&&
operator|!
name|result
operator|.
name|contains
argument_list|(
name|session
argument_list|)
condition|)
name|result
operator|.
name|push_back
argument_list|(
name|session
argument_list|)
expr_stmt|;
block|}
comment|// desktopSettingsAware
name|result
operator|.
name|append
argument_list|(
name|QLatin1String
argument_list|(
name|QGenericUnixTheme
operator|::
name|name
argument_list|)
argument_list|)
expr_stmt|;
return|return
name|result
return|;
block|}
end_function
begin_macro
name|QT_END_NAMESPACE
end_macro
end_unit
