...
name|m_properties
decl_stmt|;
comment|// take a copy: setProperty() below modifies m_properties
for|for
control|(
specifier|const
specifier|auto
...
end_comment
begin_comment
comment|/*!     \fn bool QPrinter::setPageMargins(const QMarginsF&margins, QPageLayout::Unit units)     \since 5.3      Set the page margins to \a margins in the given \a units.  If \a units are     not provided then the current units are used.      If in Full Page mode then no check is performed on the \a margins set,     otherwise the \a margins must fall within the printable area for the page     size on the current printer.      To obtain the current page margins use pageLayout().pageMargins().      Returns \c true if the page margins was successfully set to \a margins.      \sa pageLayout(), setPageLayout() */
end_comment
begin_comment
comment|/*!     \fn bool QPrinter::setPageMargins(const QMarginsF&margins)      Set the page margins to \a margins using the current units.     Returns \c true if the page margins were set successfully.      \sa pageLayout(), setPageLayout() */
end_comment
begin_comment
...
end_function
begin_comment
comment|/*!   \obsolete   Returns the number of copies to be printed. The default value is 1.    On Windows, OS X and X11 systems that support CUPS, this will always   return 1 as these operating systems can internally handle the number   of copies.    On X11, this value will return the number of times the application is   required to print in order to match the number specified in the printer setup   dialog. This has been done since some printer drivers are not capable of   buffering up the copies and in those cases the application must make an   explicit call to the print code for each copy.    Use copyCount() in conjunction with supportsMultipleCopies() instead.    \sa setNumCopies(), actualNumCopies() */
end_comment
begin_function
