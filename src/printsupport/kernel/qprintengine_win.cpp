...
end_struct
begin_comment
comment|// Return a list of printer paper sizes in millimeters with the corresponding dmPaperSize value
end_comment
begin_function
...
condition|)
block|{
comment|// If they are not equal, then there seems to be a problem with the driver
if|if
condition|(
name|paperNameCount
operator|!=
...
end_function
begin_comment
comment|// Find the best-matching printer paper for size in millimeters.
end_comment
begin_function
...
argument_list|)
expr_stmt|;
comment|// Our sizes may not match the paper name's size exactly
comment|// So we treat it as custom so we know the paper size is correct
name|d
operator|->
name|has_custom_paper_size
operator|=
