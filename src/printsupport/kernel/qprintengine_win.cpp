...
directive|if
literal|0
block|if (!fallBack) {         QFontEngineWin *fe = static_cast<QFontEngineWin *>(ti.fontEngine);
comment|// Try selecting the font to see if we get a substitution font
block|SelectObject(d->hdc, fe->hfont);          if (GetDeviceCaps(d->hdc, TECHNOLOGY) != DT_CHARSTREAM) {             wchar_t n[64];             GetTextFace(d->hdc, 64, n);             fallBack = QString::fromWCharArray(n)                     != QString::fromWCharArray(fe->logfont.lfFaceName);         }     }
endif|#
directive|endif
if|if
condition|(
...
decl_stmt|;
comment|//  ### TODO
comment|//    QFontEngineWin *winfe = (fe->type() == QFontEngine::Win) ? static_cast<QFontEngineWin *>(fe) : 0;
name|HFONT
name|hfont
decl_stmt|;
name|bool
...
decl_stmt|;
comment|//    if (winfe) {
comment|//        hfont = winfe->hfont;
comment|//        ttf = winfe->ttf;
comment|//    } else {
name|hfont
operator|=
operator|(
