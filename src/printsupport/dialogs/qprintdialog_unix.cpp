...
block|{
comment|// This case happens when CUPS server-side page range is enabled
comment|// Setting the range to the printer occurs below
name|p
operator|->
name|setPrintRange
argument_list|(
...
expr_stmt|;
block|}
comment|// server-side page range, since we set the page range on the printer to 0-0/AllPages above,
comment|// we need to take the values directly from the widget as q->fromPage() will return 0
if|if
condition|(
operator|!
name|q
...
condition|)
block|{
comment|// If we can do CUPS server side pages selection,
comment|// display the page range widgets
name|options
operator|.
name|gbPrintRange
operator|->
...
block|}
block|}
comment|// PDF printer not added to the dialog yet, we'll handle those cases in QUnixPrintWidgetPrivate::updateWidget
if|if
condition|(
name|propertiesDialog
condition|)
