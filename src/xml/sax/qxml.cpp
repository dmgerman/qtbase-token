...
namespace|namespace
block|{
comment|// work around missing std::stack::clear()
template|template
parameter_list|<
name|typename
name|Container
...
end_comment
begin_comment
comment|/*!     \fn bool QXmlContentHandler::endElement(const QString& namespaceURI, const QString& localName, const QString& qName)      The reader calls this function when it has parsed an end element     tag with the qualified name \a qName, the local name \a localName     and the namespace URI \a namespaceURI.      If this function returns \c false the reader stops parsing and     reports an error. The reader uses the function errorString() to     get the error message.      \sa startElement(), {Namespace Support via Features} */
end_comment
begin_comment
...
end_comment
begin_comment
comment|/*!     \fn QString QXmlContentHandler::errorString() const      The reader calls this function to get an error string, e.g. if any     of the handler functions returns \c false. */
end_comment
begin_comment
...
end_comment
begin_comment
comment|/*! \fn bool QXmlErrorHandler::fatalError(const QXmlParseException& exception)  A reader must use this function to report a non-recoverable error. Details of the error are stored in \a exception.  If this function returns \c true the reader might try to go on parsing and reporting further errors, but no regular parsing events are reported. */
end_comment
begin_comment
comment|/*!     \fn QString QXmlErrorHandler::errorString() const      The reader calls this function to get an error string if any of     the handler functions returns \c false. */
end_comment
begin_comment
...
end_comment
begin_comment
comment|/*!     \fn QString QXmlDTDHandler::errorString() const      The reader calls this function to get an error string if any of     the handler functions returns \c false. */
end_comment
begin_comment
...
end_comment
begin_comment
comment|/*!     \fn QString QXmlEntityResolver::errorString() const      The reader calls this function to get an error string if any of     the handler functions returns \c false. */
end_comment
begin_comment
...
end_comment
begin_comment
comment|/*!     \fn bool QXmlLexicalHandler::endDTD()      The reader calls this function to report the end of a DTD     declaration, if any.      If this function returns \c false the reader stops parsing and     reports an error. The reader uses the function errorString() to     get the error message.      \sa startDTD() */
end_comment
begin_comment
...
end_comment
begin_comment
comment|/*!     \fn bool QXmlLexicalHandler::endEntity(const QString& name)      The reader calls this function to report the end of an entity     called \a name.      For every startEntity() call, there is a corresponding endEntity()     call. The calls to startEntity() and endEntity() are properly     nested.      If this function returns \c false the reader stops parsing and     reports an error. The reader uses the function errorString() to     get the error message.      \sa startEntity(), QXmlContentHandler::skippedEntity(), QXmlSimpleReader::setFeature() */
end_comment
begin_comment
...
end_comment
begin_comment
comment|/*!     \fn bool QXmlLexicalHandler::endCDATA()      The reader calls this function to report the end of a CDATA     section.      If this function returns \c false the reader stops parsing and reports     an error. The reader uses the function errorString() to get the error     message.      \sa startCDATA(), QXmlContentHandler::characters() */
end_comment
begin_comment
...
end_comment
begin_comment
comment|/*!     \fn QString QXmlLexicalHandler::errorString() const      The reader calls this function to get an error string if any of     the handler functions returns \c false. */
end_comment
begin_comment
...
end_comment
begin_comment
comment|/*!     \fn QString QXmlDeclHandler::errorString() const      The reader calls this function to get an error string if any of     the handler functions returns \c false. */
end_comment
begin_comment
...
end_comment
begin_comment
comment|/*!     \fn bool QXmlReader::hasFeature(const QString& name) const      Returns \c true if the reader has the feature called \a name;     otherwise returns \c false.      \sa feature(), setFeature() */
end_comment
begin_comment
...
end_comment
begin_comment
comment|/*!     \fn bool QXmlReader::hasProperty(const QString& name) const      Returns \c true if the reader has the property \a name; otherwise     returns \c false.      \sa property(), setProperty() */
end_comment
begin_comment
...
end_comment
begin_comment
comment|/*!     \fn bool QXmlReader::parse(const QXmlInputSource *input)      Reads an XML document from \a input and parses it. Returns \c true if     the parsing was successful; otherwise returns \c false. */
end_comment
begin_comment
...
end_function
begin_comment
comment|/*!     Turns on the feature \a name if \a enable is true; otherwise turns it off.      The \a name parameter must be one of the following strings:     \table     \header \li Feature \li Default \li Notes     \row \li \e http://xml.org/sax/features/namespaces          \li true          \li If enabled, namespaces are reported to the content handler.     \row \li \e http://xml.org/sax/features/namespace-prefixes          \li false          \li If enabled, the original prefixed names             and attributes used for namespace declarations are             reported.     \row \li \e http://qt-project.org/xml/features/report-whitespace-only-CharData          \li true          \li If enabled, CharData that consist of             only whitespace characters are reported             using QXmlContentHandler::characters(). If disabled, whitespace is silently             discarded.     \row \li \e http://qt-project.org/xml/features/report-start-end-entity          \li false          \li If enabled, the parser reports             QXmlContentHandler::startEntity() and             QXmlContentHandler::endEntity() events, so character data             might be reported in chunks.             If disabled, the parser does not report these events, but             silently substitutes the entities, and reports the character             data in one chunk.     \endtable      \sa feature(), hasFeature(), {SAX2 Features} */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!     Reads an XML document from \a input and parses it in one pass (non-incrementally).     Returns \c true if the parsing was successful; otherwise returns \c false. */
end_comment
begin_function
DECL|function|parse
name|bool
...
name|fromLatin1
argument_list|(
literal|"DTD parsing exceeded recursion limit of %1."
argument_list|)
operator|.
name|arg
argument_list|(
...
block|{
name|QString
name|entityNameBuffer
decl_stmt|;
comment|// For every entity, check how many times all entity names were referenced in its value.
for|for
control|(
name|QMap
argument_list|<
...
argument_list|()
decl_stmt|;
comment|// Don't check the same entities twice.
if|if
condition|(
operator|!
name|literalEntitySizes
...
condition|)
block|{
comment|// The amount of characters that weren't entity names, but literals, like 'X'.
name|QString
name|leftOvers
init|=
name|entities
...
argument_list|)
decl_stmt|;
comment|// How many times was entityName referenced by toSearch?
for|for
control|(
name|QMap
argument_list|<
...
argument_list|)
expr_stmt|;
comment|// The entityName we're currently trying to find was matched in this string; increase our count.
operator|++
name|referencesToOtherEntities
index|[
name|toSearch
...
argument_list|)
decl_stmt|;
comment|// The total size of an entity's value is the expanded size of all of its referenced entities, plus its literal size.
operator|*
name|expandedIt
operator|+=
name|expandedSizes
...
name|fromLatin1
argument_list|(
literal|"The XML entity \"%1\" expands to a string that is too large to process (%2 characters> %3)."
argument_list|)
operator|.
name|arg
argument_list|(
...
end_function
begin_comment
comment|/*   This private function inserts and reports an entity substitution. The   substituted string is \a data and the name of the entity reference is \a   name. If \a inLiteral is true, the entity is IncludedInLiteral (i.e., " and '   must be quoted. Otherwise they are not quoted.    This function returns \c false on error. */
end_comment
begin_function
...
end_function
begin_comment
comment|/*   This private function moves the cursor to the next non-whitespace character.   This function does not move the cursor if the actual cursor position is a   non-whitespace charcter.    Returns \c false when you use incremental parsing and this function reaches EOF   with reading only whitespace characters. In this case it also poplulates the   parseStack with useful information. In all other cases, this function returns   true. */
end_comment
begin_function
...
end_function
begin_comment
comment|/*   Returns \c true if a entity with the name \a e exists,   otherwise returns \c false. */
end_comment
begin_function
