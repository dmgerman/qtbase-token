...
function_decl|;
name|bool
name|isPartiallyExpandedEntityValueTooLarge
parameter_list|(
name|QString
modifier|*
name|errorMessage
...
condition|)
block|{
comment|// The entityName we're currently trying to find
comment|// was matched in this string; increase our count.
operator|++
name|referencedEntityCounts
index|[
name|entityName
...
name|fromLatin1
argument_list|(
literal|"The XML entity \"%1\""
literal|"expands too a string that is too large to process when "
literal|"referencing \"%2\" %3 times."
argument_list|)
operator|.
name|arg
argument_list|(
...
condition|)
block|{
comment|// The entity at entityName is entityValue.size() characters
comment|// long in its unexpanded form, and was mentioned timesReferenced times,
comment|// resulting in a string that would be greater than 1024 characters.
name|reportParseError
argument_list|(
name|errorMessage
argument_list|)
