...
end_function
begin_comment
comment|/*!   Pop the current DITA tag off the stack, and write the   appropriate end tag to the DITA XML file.  */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!   Returns "xml" for this subclass of class Generator.  */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!   Writes the \c {#include ...} required to include the class   or namespace in a compilation.  */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!   Renamed from highlightedCode() in the html generator. Writes   the \a markedCode to the current XML stream.  */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!   Collect all the nodes in the tree according to their type or subtype.    type: Class   type: Namespace    subtype: Example   subtype: External page   subtype: Group   subtype: Header file   subtype: Module   subtype: Page   subtype: QML basic type   subtype: QML class   subtype: QML module  */
end_comment
begin_function
...
name|Namespace
case|:
comment|//qDebug()<< "NODE: Namespace"<< "TITLE:"<< child->name()
comment|//<< "FILE:"<< fileName(child);
if|if
condition|(
operator|!
name|isDuplicate
...
name|Class
case|:
comment|//qDebug()<< "NODE: Class"<< "TITLE:"<< child->name()
comment|//<< "FILE:"<< fileName(child);
if|if
condition|(
operator|!
...
name|Example
case|:
comment|//qDebug()<< "FAKE NODE: Example"<< "TITLE:"<< child->title()
comment|//<< "FILE:"<< fileName(child);
if|if
condition|(
operator|!
...
name|HeaderFile
case|:
comment|//qDebug()<< "FAKE NODE: Header file"<< "TITLE:"<< child->title()
comment|//<< "FILE:"<< fileName(child);
if|if
condition|(
operator|!
...
name|Image
case|:
comment|//qDebug()<< "FAKE NODE: Image";
break|break;
case|case
name|Node
operator|::
name|Group
case|:
comment|//qDebug()<< "FAKE NODE: Group"<< "TITLE:"<< child->title()
comment|//<< "FILE:"<< fileName(child);
if|if
condition|(
operator|!
...
name|Module
case|:
comment|//qDebug()<< "FAKE NODE: Module"<< "TITLE:"<< child->title()
comment|//<< "FILE:"<< fileName(child);
if|if
condition|(
operator|!
...
name|Page
case|:
comment|//qDebug()<< "FAKE NODE: Page"<< "PAGE TYPE:"<< child->pageTypeString()
comment|//<< "TITLE:"<< child->title()
comment|//<< "FILE:"<< fileName(child);
if|if
condition|(
operator|!
...
name|ExternalPage
case|:
comment|//qDebug()<< "FAKE NODE: External page"<< "TITLE:"<< child->title()
comment|//<< "FILE:"<< fileName(child);
if|if
condition|(
operator|!
...
name|QmlClass
case|:
comment|//qDebug()<< "FAKE NODE: QML class"<< "TITLE:"<< child->title()<< "FILE:"
comment|//<< fileName(child);
if|if
condition|(
operator|!
name|isDuplicate
...
name|QmlPropertyGroup
case|:
comment|//qDebug()<< "FAKE NODE: QML property group";
break|break;
case|case
name|Node
operator|::
name|QmlBasicType
case|:
comment|//qDebug()<< "FAKE NODE: QML basic type"<< "TITLE:"<< child->title()
comment|//<< "FILE:"<< fileName(child);
if|if
condition|(
operator|!
...
name|QmlModule
case|:
comment|//qDebug()<< "FAKE NODE: QML module"<< "TITLE:"<< child->title()
comment|//<< "FILE:"<< fileName(child);
if|if
condition|(
operator|!
...
name|Collision
case|:
comment|//qDebug()<< "FAKE NODE: Collision";
break|break;
default|default:
break|break;
block|}
...
expr_stmt|;
comment|//    doctype = "<!DOCTYPE cxxAPIMap PUBLIC \"-//NOKIA//DTD DITA C++ API Map Reference Type v0.6.0//EN\" \"dtd/cxxAPIMap.dtd\">";
name|xmlWriter
argument_list|()
operator|.
name|writeDTD
...
argument_list|)
expr_stmt|;
comment|//xmlWriter().writeAttribute("id","Qt-DITA-Map");
comment|//xmlWriter().writeAttribute("title","Qt DITA Map");
name|writeStartTag
argument_list|(
name|DT_topicmeta
argument_list|)
...
directive|if
literal|0
block|for (unsigned i=0; i<Node::LastType; ++i) {         if (nodeTypeMaps[i]&& nodeTypeMaps[i]->size()> 0)             qDebug()<< "NODE TYPE:"<< Node::nodeTypeString(i)<< nodeTypeMaps[i]->size();     }     for (unsigned i=1; i<Node::LastSubtype; ++i) {         if (nodeSubtypeMaps[i]&& nodeSubtypeMaps[i]->size()> 0)             qDebug()<< "NODE SUBTYPE:"<< Node::nodeSubtypeString(i)<< nodeSubtypeMaps[i]->size();     }     for (unsigned i=1; i<Node::OnBeyondZebra; ++i) {         if (pageTypeMaps[i]&& pageTypeMaps[i]->size()> 0)             qDebug()<< "PAGE TYPE:"<< Node::pageTypeString(i)<< pageTypeMaps[i]->size();     }
endif|#
directive|endif
name|beginSubPage
argument_list|(
