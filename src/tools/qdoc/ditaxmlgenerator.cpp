...
argument_list|)
decl_stmt|;
comment|//if (myLink.isEmpty())
comment|//myLink = getCollisionLink(atom);
if|if
condition|(
name|myLink
operator|.
...
end_function
begin_comment
comment|/*!   Generate the DITA XML file for a group, module, or QML module.  */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!   Writes an XML file header to the current XML stream. This   depends on which kind of DITA XML file is being generated,   which is determined by the \a node type and subtype and the   \a subpage flag.  */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!   Constructs a file name appropriate for the \a node and returns   it. If the \a node is not a not an external page, an image, or   a ditamap, call fileName() in the base class, Generator.  */
end_comment
begin_function
...
directive|if
literal|0
block|if (!(*i).text().isEmpty()) {                         writeStartTag(DT_apiDesc);                         generateText((*i).text(), en, marker);                         writeEndTag();
comment|//</apiDesc>
block|}
endif|#
directive|endif
