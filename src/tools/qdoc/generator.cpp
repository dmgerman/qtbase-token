...
name|tr
argument_list|(
literal|"HTML file already exists; overwriting %1"
argument_list|)
operator|.
name|arg
argument_list|(
...
argument_list|()
expr_stmt|;
comment|/*           To avoid file name conflicts in the html directory,           we prepend a prefix (by default, "qml-") and an optional suffix           to the file name. The suffix, if one exists, is appended to the           module name.         */
if|if
condition|(
operator|!
name|node
...
expr_stmt|;
block|}
comment|// Why not add "-group" for group pages?
block|}
else|else
block|{
specifier|const
...
name|fdl
decl_stmt|;
comment|/*       If the useSubdir parameter is set, then the output is       being sent to subdirectories of the output directory.       Prepend the subdirectory name + '/' to the result.      */
if|if
condition|(
name|useSubdir
condition|)
...
end_function
begin_comment
comment|/*!   Generate the body of the documentation from the qdoc comment   found with the entity represented by the \a node.  */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!   Generates a bold line that says:   "The signal is private, not emitted by the user.   The function is public so the user can pass it to connect()."  */
end_comment
begin_function
...
argument_list|)
operator|<<
literal|"This is a private signal. It can be used in signal connections but cannot be emitted by the user."
operator|<<
name|Atom
operator|::
name|ParaRight
...
end_function
begin_comment
comment|/*!   Generate the documentation for \a relative. i.e. \a relative   is the node that reporesentas the entity where a qdoc comment   was found, and \a text represents the qdoc comment.  */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!     If the node is an overloaded signal, and a node with an example on how to connect to it  */
end_comment
begin_function
...
condition|)
return|return;
comment|// Compute a friendly name for the object of that instance.
comment|// e.g:  "QAbstractSocket" -> "abstractSocket"
name|QString
name|objectName
init|=
name|node
...
expr_stmt|;
block|}
comment|// We have an overloaded signal, show an example
name|QString
name|code
init|=
literal|"connect("
...
argument_list|)
operator|<<
literal|" is overloaded in this class. "
literal|"To connect to this one using the function pointer syntax, you must "
literal|"specify the signal type in a static cast, as shown in this example:"
operator|<<
name|Atom
argument_list|(
name|Atom
...
parameter_list|)
block|{
comment|// Prefix is applied to QML and JS types
if|if
condition|(
name|node
operator|->
...
parameter_list|)
block|{
comment|// Suffix is applied to QML and JS types, as
comment|// well as module pages.
if|if
condition|(
name|node
operator|->
