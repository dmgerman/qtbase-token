...
end_comment
begin_endif
unit|const Node* QDocDatabase::findFunctionNode(const QString& target,                                            const Node* relative,                                            Node::Genus genus) {     return forest_.findFunctionNode(target, relative, genus); }
endif|#
directive|endif
end_endif
begin_comment
...
condition|)
block|{
comment|// Allow multiple (major) versions of QML/JS modules
if|if
condition|(
name|n
operator|->
...
end_function
begin_comment
comment|/*!   Finds all the collection nodes with the same name   and genus as \a c and merges their members into the   members list of \a c.    For QML and JS modules, the merge is done only if   the module identifier matches between the nodes, to avoid   merging modules with different (major) versions.  */
end_comment
begin_function
DECL|function|mergeCollections
name|void
