...
directive|if
literal|0
block|n = t->findDocNodeByTitle(target);         if (n)             return n;
endif|#
directive|endif
name|relative
operator|=
...
end_function
begin_comment
comment|/*!   Find a node of the specified \a type and \a subtype that is   reached with the specified \a path qualified with the name   of one of the open namespaces (might not be any open ones).   If the node is found in an open namespace, prefix \a path   with the name of the open namespace and "::" and return a   pointer to the node. Othewrwise return 0.    This function only searches in the current primary tree.  */
end_comment
begin_function
DECL|function|findNodeInOpenNamespace
name|Node
...
end_if
begin_comment
unit|void getAllGroups(CNMM& t);     void getAllModules(CNMM& t);     void getAllQmlModules(CNMM& t);
comment|/*!   For each tree in the forest, get the group map from the tree.   Insert each pair from the group map into the collection node   multimap \a t.  */
end_comment
begin_comment
unit|void QDocForest::getAllGroups(CNMM& t) {     foreach (Tree* t, searchOrder()) {         const GroupMap& gm = t->groups();         if (!gm.isEmpty()) {             GroupMap::const_iterator i = gm.begin();             while (i != gm.end()) {                 t.insert(i.key(), i.value());                 ++i;             }         }     } }
comment|/*!   For each tree in the forest, get the module map from the tree.   Insert each pair from the module map into the collection node   multimap \a t.  */
end_comment
begin_comment
unit|void QDocForest::getAllModules(CNMM& t) {     foreach (Tree* t, searchOrder()) {         const ModuleMap& mm = t->modules();         if (!mm.isEmpty()) {             ModuleMap::const_iterator i = mm.begin();             while (i != mm.end()) {                 t.insert(i.key(), i.value());                 ++i;             }         }     } }
comment|/*!   For each tree in the forest, get the QML module map from the   tree. Insert each pair from the QML module map into the   collection node multimap \a t.  */
end_comment
begin_endif
unit|void QDocForest::getAllQmlModules(CNMM& t) {     foreach (Tree* t, searchOrder()) {         const QmlModuleMap& qmm = t->groups();         if (!qmm.isEmpty()) {             QmlModuleMap::const_iterator i = qmm.begin();             while (i != qmm.end()) {                 t.insert(i.key(), i.value());                 ++i;             }         }     } }
endif|#
directive|endif
end_endif
end_unit
