...
end_function
begin_comment
comment|/*!   Looks up the QML module named \a name. If it isn't there,   create it. Then append \a node to the QML module's member   list. The parent of \a node is not changed by this function.   Returns a pointer to the QML module node.  */
end_comment
begin_function
...
directive|if
literal|0
block|else {                     qDebug()<< "Unable to resolve QML base type:"<< qcn->qmlBaseName()<< "for QML type:"<< qcn->name();                 }
endif|#
directive|endif
block|}
block|}
