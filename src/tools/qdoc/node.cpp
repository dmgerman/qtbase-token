...
end_comment
begin_endif
unit|Node* InnerNode::findChildNode(const QString& name, bool qml) const {     NodeList nodes = childMap.values(name);     if (!nodes.isEmpty()) {         for (int i=0; i<nodes.size(); ++i) {             Node* node = nodes.at(i);             if (!qml) {                 if (!node->isQmlNode())                     return node;             }             else if (node->isQmlNode())                 return node;         }     }     if (qml&& isQmlType()) {         for (int i=0; i<children_.size(); ++i) {             Node* node = children_.at(i);             if (node->isQmlPropertyGroup()) {                 node = static_cast<InnerNode*>(node)->findChildNode(name);                 if (node)                     return node;             }         }     }     return primaryFunctionMap.value(name); }
endif|#
directive|endif
end_endif
begin_comment
comment|/*!   This function is like findChildNode(), but if a node   with the specified \a name is found but it is not of the   specified \a type, 0 is returned.    This function is not recursive and therefore can't handle   collisions. If it finds a collision node named \a name, it   will return that node. But it might not find the collision   node because it looks up \a name in the child map, not the   list.  */
end_comment
begin_function
DECL|function|findChildNode
name|Node
...
end_comment
begin_comment
unit|void InnerNode::findNodes(const QString& name, NodeList& n) {     n.clear();     Node* node = 0;     NodeList nodes = childMap.values(name);
comment|/*<sigh> If this node's child map contains no nodes named       name, then if this node is a QML class, search each of its       property group nodes for a node named name. If a match is       found, append it to the output list and return immediately.      */
end_comment
begin_comment
unit|if (nodes.isEmpty()) {         if (isQmlType()) {             for (int i=0; i<children_.size(); ++i) {                 node = children_.at(i);                 if (node->isQmlPropertyGroup()) {                     node = static_cast<InnerNode*>(node)->findChildNode(name);                     if (node) {                         n.append(node);                         return;                     }                 }             }         }     }     else {
...
end_comment
begin_endif
unit|for (int i=0; i<nodes.size(); ++i) {             node = nodes.at(i);             if (!node->isQmlPropertyGroup())                 n.append(node);             else {                 node = static_cast<InnerNode*>(node)->findChildNode(name);                 if (node)                     n.append(node);             }         }     }     if (!n.isEmpty())         return;     node = primaryFunctionMap.value(name);     if (node)         n.append(node); }
endif|#
directive|endif
end_endif
begin_comment
...
end_constructor
begin_comment
comment|/*! \fn QString DocNode::title() const   Returns the document node's title. This is used for the page title. */
end_comment
begin_comment
