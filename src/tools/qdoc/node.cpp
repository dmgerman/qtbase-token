...
expr_stmt|;
block|}
comment|/*           Most \l commands that link to functions don't include           the parameter declarations in the function signature,           so if the \l is meant to go to a function that does           have parameters, the algorithm above won't find it.           Therefore we must return the pointer to the function           in the primary function map in the cases where the           parameters should have been specified in the \l command.           But if the primary function is marked internal, search           the secondary list to find one that is not marked internal.         */
if|if
condition|(
operator|!
name|fn
...
end_function
begin_comment
comment|/*!   This is where we should set the overload numbers, including   the related non-members.  */
end_comment
begin_function
...
expr_stmt|;
block|}
comment|/*       Add the related non-members here.      */
if|if
condition|(
operator|!
name|related_
