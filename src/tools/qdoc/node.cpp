...
end_function
begin_comment
comment|/*!   The QML module map contains an entry for each QML module   identifier. A QML module identifier is constucted from the   QML module name and the module's major version number, like   this: \e {<module-name><major-version>}    If the QML module map does not contain the module identifier   \a qmid, insert the QML module node \a fn mapped to \a qmid.  */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!   Insert the QML type node \a qcn into the static QML module   member map. The key is \a qmid + "::" + qcn->name().  */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!   Lookup the QML type node identified by the Qml module id   \a qmid and QML type \a name, and return a pointer to the   node. The key is \a qmid + "::" + qcn->name().  */
end_comment
begin_function
