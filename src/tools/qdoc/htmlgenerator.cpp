...
end_function
begin_comment
comment|/*!   Generate targets for any \keyword commands that were seen   in the qdoc comment for the \a node.  */
end_comment
begin_function
...
argument_list|)
expr_stmt|;
comment|// Disabled: keywords always link to the top of the QDoc
comment|// comment they appear in, and do not use a dedicated anchor.
if|#
directive|if
literal|0
block|if (!node->doc().isEmpty()) {         const QList<Atom*>& keywords = node->doc().keywords();         foreach (Atom* a, keywords) {             out()<< QLatin1String("<a name=\"")<< Doc::canonicalTitle(a->string())<< QLatin1String("\"></a>");         }     }
endif|#
directive|endif
block|}
end_function
...
end_function
begin_comment
comment|/*!   Output the module's Quality Assurance page.  */
end_comment
begin_function
...
name|title
init|=
literal|"Quality Assurance Page for "
operator|+
name|defaultModuleName
argument_list|()
decl_stmt|;
...
name|t
init|=
literal|"Quality assurance information for checking the "
operator|+
name|defaultModuleName
argument_list|()
operator|+
...
name|t
operator|=
literal|"Intermodule Link Counts"
expr_stmt|;
name|QString
name|ref
init|=
...
argument_list|()
operator|<<
literal|"<table class=\"valuelist\"><tr valign=\"top\" "
operator|<<
literal|"class=\"even\"><th class=\"tblConst\">Destination Module</th>"
operator|<<
literal|"<th class=\"tblval\">Link Count</th></tr>\n"
expr_stmt|;
name|QString
name|fileName
decl_stmt|;
...
literal|"</a>"
operator|<<
literal|"</tt></td><td class=\"topAlign\"><tt>"
operator|<<
name|counts
operator|.
name|at
...
name|t
operator|=
literal|"The Optimal \"depends\" Variable"
expr_stmt|;
name|out
argument_list|()
operator|<<
...
name|t
operator|=
literal|"Consider replacing the depends variable in "
operator|+
name|defaultModuleName
argument_list|()
operator|.
...
argument_list|()
operator|+
literal|".qdocconf with this one, if the two are not identical:"
expr_stmt|;
name|out
argument_list|()
operator|<<
...
end_function
begin_comment
comment|/*!   This function writes an html file containing a list of   links to links that originate in the current module and   go to targets in the specified \a module. The \a marker   is used for the same thing the marker is always used for.  */
end_comment
begin_function
...
control|)
block|{
comment|// e.g.:<a name="link-8421"></a><a href="layout.html">Layout Management</a>
name|out
argument_list|()
operator|<<
...
name|fileName
operator|=
literal|"aaa-links-to-broken-links.html"
expr_stmt|;
name|beginSubPage
argument_list|(
name|node
...
block|else if (atom->string() == QLatin1String("relatedinline")) {             const DocumentNode *dn = static_cast<const DocumentNode *>(relative);             if (dn&& !dn->members().isEmpty()) {
...
expr_stmt|;
block|}
comment|/*               mws saw this on 17/10/2014.               Is this correct? Setting node to 0 means the               following test always fails. Did we decide to               no longer warn about linking to obsolete things?              */
name|node
operator|=
literal|0
expr_stmt|;
...
name|navigationbar
decl_stmt|;
comment|// Set list item types based on the navigation bar type
name|Atom
operator|::
name|AtomType
name|itemLeft
...
argument_list|()
operator|<<
literal|"</tr></table><table class=\"buildversion\"><tr>\n"
operator|<<
literal|"<td id=\"buildversion\" width=\"100%\" align=\"right\">"
operator|<<
name|buildversion
operator|<<
literal|"</td>\n"
...
end_function
begin_comment
comment|/*!   Generates a separate file where certain members of the QML   type \a qcn are listed. The \a marker is used to generate   the section lists, which are then traversed and output here.    Note that this function currently only handles correctly the   case where \a status is \c {CodeMarker::Obsolete}.  */
end_comment
begin_function
...
argument_list|()
operator|<<
literal|"<p><b>The following members of QML type "
operator|<<
literal|"<a href=\""
operator|<<
name|linkForNode
...
end_function
begin_comment
comment|/*!   This function finds the common prefix of the names of all   the classes in the class map \a nmm and then generates a   compact list of the class names alphabetized on the part   of the name not including the common prefix. You can tell   the function to use \a comonPrefix as the common prefix,   but normally you let it figure it out itself by looking at   the name of the first and last classes in the class map   \a nmm.  */
end_comment
begin_function
...
argument_list|()
decl_stmt|;
comment|/*       Divide the data into 37 paragraphs: 0, ..., 9, A, ..., Z,       underscore (_). QAccel will fall in paragraph 10 (A) and       QXtWidget in paragraph 33 (X). This is the only place where we       assume that NumParagraphs is 37. Each paragraph is a NodeMultiMap.     */
name|NodeMultiMap
name|paragraph
index|[
name|NumParagraphs
...
block|{
comment|/*           \generatelist {selector} is only allowed in a           comment where the topic is \group, \module,           \qmlmodule, or \jsmodule         */
if|if
condition|(
operator|!
name|relative
...
name|tr
argument_list|(
literal|"\\generatelist {%1} is only allowed in \\group, "
literal|"\\module, \\qmlmodule, and \\jsmodule comments."
argument_list|)
operator|.
name|arg
argument_list|(
...
argument_list|()
operator|<<
literal|"</td><td class=\"memItemRight bottomAlign\">[see note below]"
expr_stmt|;
block|}
block|}
if|if
...
name|Compat
case|:
comment|// Porting to Qt 4 no longer supported
break|break;
default|default:
name|Generator
operator|::
...
literal|"<tr valign=\"top\" class=\"odd\" id=\"%1\">\n"
literal|"<td class=\"%2\"><p>\n"
literal|"<a name=\"%3\"></a>"
argument_list|)
decl_stmt|;
name|QString
...
name|text
operator|<<
literal|" is instantiated by Javascript Type "
expr_stmt|;
name|text
operator|<<
name|Atom
...
continue|continue;
block|}
comment|// attributes that are always written for the element
name|usedAttributes
operator|.
name|clear
argument_list|()
...
expr_stmt|;
comment|// description
comment|// Add words from module name as tags
comment|// QtQuickControls -> qt,quick,controls
comment|// QtOpenGL -> qt,opengl
name|QRegExp
name|re
argument_list|(
literal|"([A-Z]+[a-z0-9]*(3D|GL)?)"
argument_list|)
decl_stmt|;
name|int
name|pos
...
argument_list|)
expr_stmt|;
comment|// Clean up tags, exclude invalid and common words
name|QSet
argument_list|<
name|QString
argument_list|>
...
argument_list|()
decl_stmt|;
comment|// open .qml, .cpp and .h files with a
comment|// basename matching the example (project) name
comment|// QMap key indicates the priority -
comment|// the lowest value will be the top-most file
if|if
condition|(
operator|(
name|fileInfo
...
expr_stmt|;
block|}
comment|// main.qml takes precedence over main.cpp
elseif|else
if|if
condition|(
name|fileName
...
end_function
begin_comment
comment|/*!   Generates bold Note lines that explain how function \a fn   is associated with each of its associated properties.  */
end_comment
begin_function
