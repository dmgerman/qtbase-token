...
name|QmlType
expr_stmt|;
comment|// Legacy alias for 'qmltype'
name|typeHash
index|[
literal|"qmltype"
index|]
...
condition|)
block|{
comment|// Add all group members for 'group:name' selector
if|if
condition|(
name|node
operator|->
...
block|{
comment|// Accept only doc nodes with subtypes contained in the selector's
comment|// mask.
specifier|const
name|DocumentNode
modifier|*
...
condition|)
block|{
comment|/*                   Don't visit QML/JS property group nodes,                   but visit their children, which are all                   QML/JS property nodes.                    This is probably not correct anymore,                   because The Qml/Js Property Group is                   an actual documented thing.                  */
specifier|const
name|Aggregate
modifier|*
name|inner
...
end_function
begin_comment
comment|/*     Write subsections for all members, compatibility members and obsolete members. */
end_comment
begin_function
...
block|}
block|}
comment|// No contents/nextpage links found, write all nodes unsorted
if|if
condition|(
operator|!
name|contentsFound
...
argument_list|)
expr_stmt|;
comment|// The list of files to write is the union of generated files and
comment|// other files (images and extras) included in the project
name|QSet
argument_list|<
name|QString
argument_list|>
