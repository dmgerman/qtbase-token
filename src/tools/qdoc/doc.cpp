...
name|tr
argument_list|(
literal|"Check parameter in '[ ]' of '\\l' command: '%1', "
literal|"possible misspelling, or unrecognized module name"
argument_list|)
operator|.
name|arg
argument_list|(
...
directive|if
literal|0
block|QString filePath = Config::findFile(location(),                                         sourceFiles,                                         sourceDirs,                                         fileName,                                         userFriendlyFilePath);
endif|#
directive|endif
if|if
condition|(
...
end_function
begin_comment
comment|/*!   Gets an argument that is enclosed in brackets and returns it   without the enclosing brackets. On entry, the current character   is the left bracket. On exit, the current character is the one   that comes after the right bracket.  */
end_comment
begin_function
