...
end_decl_stmt
begin_comment
comment|/*     This is used for fuzzy matching only, which in turn is only used     for Qt Jambi. */
end_comment
begin_function
...
name|QRegExp
argument_list|(
literal|"\\bu(int|short|char|long)\\b"
argument_list|)
argument_list|,
literal|"unsigned \\1"
argument_list|)
...
condition|)
block|{
comment|// we assume that the path for the associated enum
comment|// is the same as for the flag typedef
name|QStringList
name|path
init|=
name|regExp
...
condition|)
block|{
comment|// remove needless (and needful) class prefixes
name|QRegExp
name|regExp
argument_list|(
literal|"[A-Za-z0-9_]+::"
...
end_function
begin_comment
comment|/*!   Parse the C++ header file identified by \a filePath   and add the parsed contents to the big \a tree. The   \a location is used for reporting errors.  */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!   Get ready to parse the C++ cpp file identified by \a filePath   and add its parsed contents to the big \a tree. \a location is   used for reporting errors.    Call matchDocsAndStuff() to do all the parsing and tree building.  */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!   This is called after all the source files (i.e., not the   header files) have been parsed. It traverses the tree to   resolve property links, normalize overload signatures, and   do other housekeeping of the tree.  */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!   This function searches the \a tree to find a FunctionNode   for a function with the signature \a synopsis. If the   \a relative node is provided, the search begins there. If   \a fuzzy is true, base classes are searched. The function   node is returned, if found.  */
end_comment
begin_function
...
argument_list|)
expr_stmt|;
comment|/*             This is necessary because Roberto's parser resolves typedefs.         */
if|if
condition|(
operator|!
name|func
...
literal|0
return|;
comment|/*                     There's only one function with the correct number                     of parameters. That must be the one.                 */
if|if
condition|(
name|candidates
operator|.
...
expr_stmt|;
block|}
comment|/*                     There are several functions with the correct                     parameter count, but only one has the correct                     parameter names.                 */
if|if
condition|(
name|candidates
operator|.
...
expr_stmt|;
block|}
comment|/*                     There are several functions with the correct                     parameter count, but only one has the correct                     types, loosely compared.                 */
if|if
condition|(
name|candidates
operator|.
...
end_function
begin_comment
comment|/*!   Process the topic \a command in context \a doc with argument \a arg.  */
end_comment
begin_function
...
condition|)
block|{
comment|/*           The command was neither "fn" nor "macro" .          */
comment|// ### split(QLatin1Char(' ')) hack is there to support header file syntax
name|QStringList
name|paths
init|=
name|arg
...
condition|)
block|{
comment|// If the command is "\service", then we need to tag the
comment|// class with the actual service name.
name|QStringList
name|args
init|=
name|arg
...
directive|if
literal|0
comment|/*           This code apparently does nothing. After further           investigation to verify it is useless, it will           be removed.          */
block|if (command == COMMAND_CLASS) {             if (paths.size()> 1) {                 if (!paths[1].endsWith(".h")) {                     ClassNode* cnode = static_cast<ClassNode*>(node);                     cnode->setQmlElement(paths[1]);                 }             }         }
endif|#
directive|endif
return|return
name|node
...
end_function
begin_comment
comment|/*!   Process the topic \a command group with arguments \a args.    Currently, this function is called only for \e{qmlproperty}   and \e{qmlattachedproperty}.  */
end_comment
begin_function
