...
name|tr
argument_list|(
literal|"Invalid use of '\\%1'; not allowed in a '\\%2'"
argument_list|)
operator|.
name|arg
argument_list|(
...
name|tr
argument_list|(
literal|"Invalid use of '\\%1' (already a member of '%2')"
argument_list|)
operator|.
name|arg
argument_list|(
...
end_function
begin_comment
comment|/*!   Parse the next function parameter, if there is one, and   append it to parameter vector \a pvect. Return true if   a parameter is parsed and appended to \a pvect.   Otherwise return false.  */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!   If the current token is any of several function modifiers,   return that token value after reading the next token. If it   is not one of the function modieifer tokens, return -1 but   don\t read the next token.  */
end_comment
begin_function
...
argument_list|()
expr_stmt|;
comment|// A left paren was seen. Parse the parameters
name|bool
name|isQPrivateSignal
init|=
literal|false
...
do|;
block|}
comment|// The parameters must end with a right paren
if|if
condition|(
operator|!
name|match
...
argument_list|)
decl_stmt|;
comment|// look for 0 indicating pure virtual
if|if
condition|(
name|match
argument_list|(
...
name|PureVirtual
expr_stmt|;
comment|// look for colon indicating ctors which must be skipped
if|if
condition|(
name|match
argument_list|(
...
expr_stmt|;
block|}
comment|// If no ';' expect a body, which must be skipped.
name|bool
name|body_expected
init|=
literal|false
...
end_function
begin_comment
comment|/*!   This function uses a Tokenizer to parse the \a parameters of a   function into the parameter vector \a {pvect}.  */
end_comment
begin_function
