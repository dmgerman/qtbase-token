...
end_if
begin_endif
unit|if (result)         qDebug()<< "FOUND:"<< path<< Node::nodeTypeString(type)<< Node::nodeSubtypeString(subtype);     else         qDebug()<< "NOT FOUND:"<< path<< Node::nodeTypeString(type)<< Node::nodeSubtypeString(subtype);
endif|#
directive|endif
end_endif
begin_comment
...
end_if
begin_comment
unit|const Node* Tree::findNodeXXX(const QStringList& path, bool qml) const {     const Node* current = root();     do {         const Node* node = current;         int i;         int start_idx = 0;
comment|/*           If the path contains one or two double colons ("::"),           check first to see if the first two path strings refer           to a QML element. If yes, that reference identifies a           QML class node.         */
end_comment
begin_endif
unit|if (qml&& path.size()>= 2) {             QmlClassNode* qcn = QmlClassNode::moduleMap.value(path[0]+ "::" +path[1]);             if (qcn) {                 node = qcn;                 if (path.size() == 2)                     return node;                 start_idx = 2;             }         }          for (i = start_idx; i< path.size(); ++i) {             if (node == 0 || !node->isInnerNode())                 break;              const Node* next = static_cast<const InnerNode*>(node)->findChildNodeByName(path.at(i), qml);             node = next;         }         if (node&& i == path.size()) {             if (node->subType() != Node::QmlPropertyGroup) {                 if (node->subType() == Node::Collision) {                     node = node->applyModuleIdentifier(start);                 }                 return node;             }         }         current = current->parent();     } while (current);      return 0; }
endif|#
directive|endif
end_endif
end_unit
