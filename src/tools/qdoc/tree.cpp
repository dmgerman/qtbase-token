...
end_destructor
begin_comment
comment|/*!   This function simply calls the const version of itself and   returns the result.  */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!   This code in this function was extracted from the other   version of findNode() that has the same signature without   the last bool parameter \a qml. This function is called   only by that other findNode(). It can be called a second   time if the first call returns null. If \a qml is false,   the search will only match a node that is not a QML node.   If \a qml is true, the search will only match a node that   is a QML node.  */
end_comment
begin_function
DECL|function|findNode
specifier|const
...
literal|0
decl_stmt|;
comment|/*           If the path contains one or two double colons ("::"),           check first to see if the first two path strings refer           to a QML element. If yes, that reference identifies a           QML class node.         */
if|if
condition|(
name|qml
operator|&&
...
end_function
begin_comment
comment|/*!   Find the node with the specified \a path name of the   specified \a type.  */
end_comment
begin_function
DECL|function|findNode
name|Node
...
end_function
begin_comment
comment|/*!   This function just calls the const version of itself and returns   a pointer to the QML class node, or null.  */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!   This function adds the \a node to the QML \a module. The QML   module can be listed anywhere using the \e{annotated list}   command.  */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!   Returns the QML module map.  */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!   For each node in the QML module map, add the node to the   appropriate QML module node.  */
end_comment
begin_function
