...
end_destructor
begin_comment
comment|/*!   Returns the location of thre nearest comment above the \a offset.  */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!   Finds the nearest unused qdoc comment above the QML entity   represented by the \a node and processes the qdoc commands   in that comment. The proceesed documentation is stored in   the \a node.    If a qdoc comment is found about \a location, true is returned.   If a comment is not found there, false is returned.  */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!   Begin the visit of the object \a definition, recording it in a tree   structure.  Increment the object nesting level, which is used to   test whether we are at the public API level. The public level is   level 1. */
end_comment
begin_function
DECL|function|visit
name|bool
...
specifier|static
name|QString
name|reconstituteFieldMemberExpression
parameter_list|(
name|EN
modifier|*
name|en
...
argument_list|()
operator|<<
literal|"  There is an expression"
operator|<<
name|en
operator|->
name|kind
...
argument_list|()
operator|<<
literal|"    But it wasn't a recognized expression kind"
expr_stmt|;
block|}
block|}
return|return
...
directive|if
literal|0
block|if (qmlClass) {                 QString name = member->name->asString();                 QmlPropGroupNode *qmlPropGroup = new QmlPropGroupNode(qmlClass, name, false);                 if (member->isDefaultMember)                     qmlPropGroup->setDefault();                 QmlPropertyNode *qmlPropNode = new QmlPropertyNode(qmlPropGroup, name, type, false);                 qmlPropNode->setWritable(!member->isReadonlyMember);                 applyDocumentation(member->firstSourceLocation(), qmlPropGroup);             }
endif|#
directive|endif
block|}
break|break;
...
end_function
begin_comment
comment|/*!   Begin the visit of the signal handler declaration \a sb, but only   if the nesting level is 1.  */
end_comment
begin_function
DECL|function|visit
name|bool
