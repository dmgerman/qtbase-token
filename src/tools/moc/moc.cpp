...
condition|)
block|{
comment|// We can only handle substitutions that result in a single symbol
return|return
name|it
operator|->
name|symbols
...
name|used
decl_stmt|;
comment|// Process substitution chain until no replacement exists
name|QByteArray
name|substitution
init|=
name|getSubstitution
...
name|result
decl_stmt|;
comment|// A word can contain multiple components separated by '*'
name|int
name|startIndex
init|=
literal|0
...
name|result
decl_stmt|;
comment|// Parse multiple tokens in this name independently
name|int
name|startIndex
init|=
literal|0
...
argument_list|)
decl_stmt|;
comment|// Parse the interior type independently
name|QByteArray
name|parameter
init|=
name|typeName
...
block|{
comment|// Something is broken; return the input unmodified
return|return
name|typeName
return|;
block|}
