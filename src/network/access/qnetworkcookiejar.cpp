...
include|#
directive|include
file|"qnetworkcookiejartlds_p.h"
end_include
begin_include
include|#
directive|include
...
name|now
decl_stmt|;
comment|// validate the cookie& set the defaults if unset
if|if
condition|(
name|cookie
operator|.
...
comment|//        else if (!isParentPath(pathAndFileName, cookie.path())) {
comment|//            continue;           // not accepted
comment|//        }
if|if
condition|(
name|cookie
...
comment|// redundant; the "leading dot" rule has been relaxed anyway, see above
comment|// we remove the leading dot for this check
if|if
condition|(
name|QNetworkCookieJarPrivate
operator|::
...
name|it
control|)
comment|// does this cookie already exist?
if|if
condition|(
name|cookie
operator|.
