...
argument_list|)
member_init_list|,
comment|// 64K initial window size according to SPDY standard
...
condition|)
block|{
comment|// happens when called from the SPDY protocol handler
name|inflateStrm
operator|=
operator|new
name|z_stream
...
specifier|const
block|{
comment|// We're in the process of redirecting - if the HTTP status code says so and
comment|// followRedirect is switched on
return|return
operator|(
name|QHttpNetworkReply
operator|::
