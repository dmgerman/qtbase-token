...
end_comment
begin_comment
comment|/*!     Returns true if this cookie is equal to \a other. This function     only returns true if all fields of the cookie are the same.      However, in some contexts, two cookies of the same name could be     considered equal.      \sa operator!=() */
end_comment
begin_function
DECL|function|operator ==
name|bool
...
argument_list|)
expr_stmt|;
comment|// parse the first part, before the equal sign
for|for
control|(
name|i
operator|=
...
literal|'='
condition|)
comment|// no equal sign, we found format (1)
return|return
name|qMakePair
argument_list|(
name|first
...
argument_list|)
expr_stmt|;
comment|// arbitrary but works for most cases
name|i
operator|=
name|nextNonWhitespace
argument_list|(
...
condition|)
block|{
comment|// a quote, we found format (3), where:
...
block|{
comment|// no quote, we found format (2)
name|position
operator|=
name|i
expr_stmt|;
...
argument_list|)
expr_stmt|;
comment|// turns into empty-but-not-null
return|return
name|qMakePair
argument_list|(
name|first
