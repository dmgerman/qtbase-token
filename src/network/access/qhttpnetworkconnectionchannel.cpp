...
condition|)
block|{
comment|// important: At the point of this readyRead(), the byteDatas list must be empty,
...
condition|)
block|{
comment|// expand a chunk if possible
comment|// If expand() failed we can just return, it had already called connection->emitReplyError()
return|return;
block|}
endif|#
directive|endif
...
condition|)
block|{
comment|// important: At the point of this readyRead(), inflated must be cleared,
comment|// else implicit sharing will trigger memcpy when the user is reading data!
emit|emit
name|reply
operator|->
