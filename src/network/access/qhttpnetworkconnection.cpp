...
block|}
block|}
comment|// If the location url is invalid/empty, we emit ProtocolUnknownError
if|if
condition|(
operator|!
name|rUrl
...
argument_list|)
expr_stmt|;
comment|// Check redirect url protocol
name|QString
name|scheme
init|=
name|rUrl
...
argument_list|()
decl_stmt|;
comment|// Check if we're doing an unsecure redirect (https -> http)
if|if
condition|(
name|previousUrlScheme
operator|==
...
directive|ifndef
name|QT_NO_SSL
comment|// is the reply inside the SPDY pipeline of this channel already?
name|QMultiMap
argument_list|<
name|int
argument_list|,
...
condition|)
block|{
comment|// we choose the reply to emit the proxyAuth signal from somewhat arbitrarily,
comment|// but that does not matter because the signal will ultimately be emitted
comment|// by the QNetworkAccessManager.
name|Q_ASSERT
argument_list|(
name|chan
operator|->
