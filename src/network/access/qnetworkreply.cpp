...
name|config
decl_stmt|;
comment|// determine if we support this extension
name|int
name|id
init|=
name|metaObject
...
name|indexOfMethod
argument_list|(
literal|"setSslConfigurationImplementation(QSslConfiguration)"
argument_list|)
decl_stmt|;
if|if
condition|(
...
parameter_list|)
block|{
comment|// do this cryptic trick, because we could not add a virtual method to this class later on
comment|// since that breaks binary compatibility
name|int
name|id
init|=
name|metaObject
...
name|indexOfMethod
argument_list|(
literal|"ignoreSslErrorsImplementation(QList<QSslError>)"
argument_list|)
decl_stmt|;
if|if
condition|(
