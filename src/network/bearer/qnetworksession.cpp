...
end_function
begin_comment
comment|/*!     Invalidates all open sessions against the network interface and therefore stops the      underlying network interface. This function always changes the session's state() flag to     \l Disconnected.      On Symbian platform, a 'NetworkControl' capability is required for     full interface-level stop (without the capability, only the current session is stopped).      \sa open(), close() */
end_comment
begin_function
DECL|function|stop
name|void
...
parameter_list|()
block|{
comment|// Needed on at least Symbian platform: the roaming must be explicitly
...
name|qWarning
argument_list|(
literal|"QNetworkSessionPrivate::nativeSession called in wrong thread"
argument_list|)
expr_stmt|;
return|return
name|s
