...
end_comment
begin_comment
comment|/*!     Returns true if this is a null certificate (i.e., a certificate     with no contents); otherwise returns false.      By default, QSslCertificate constructs a null certificate.      \sa isValid(), clear() */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!     Returns true if this certificate is valid; otherwise returns     false.      Note: Currently, this function checks that the current     data-time is within the date-time range during which the     certificate is considered valid, and checks that the     certificate is not in a blacklist of fraudulent certificates.      \sa isNull() */
end_comment
begin_function
...
argument_list|)
expr_stmt|;
comment|// Special case - if the prefix ends up being nothing, use "." instead and
comment|// chop off the first two characters from the glob'ed paths.
name|int
name|startIndex
init|=
literal|0
...
end_function
begin_comment
comment|/*!     Verifies a certificate chain. If a hostName is specified then the certificate is     also checked to see if it is valid for the specified hostName.     Note that the first certificate in the list should be the leaf certificate of     the chain to be verified.  */
end_comment
begin_function
...
argument_list|)
return|;
comment|// This is used for unknown info so we get the OID as text
name|char
name|buf
index|[
literal|80
