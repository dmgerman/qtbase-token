...
name|unsigned
name|int
name|q_ssl_psk_client_callback
parameter_list|(
name|SSL
modifier|*
name|ssl
...
operator|>=
literal|0x10001000L
comment|// Choosing Tlsv1_1OrLater or TlsV1_2OrLater on OpenSSL< 1.0.1
comment|// will cause an error in QSslContext::fromConfiguration, meaning
comment|// we will never get here.
elseif|else
if|if
condition|(
name|protocol
...
operator|>=
literal|0x10001000L
comment|// Save a pointer to this object into the SSL structure.
if|if
condition|(
name|q_SSLeay
argument_list|()
...
name|OPENSSL_NO_PSK
argument_list|)
comment|// Set the client callback for PSK
if|if
condition|(
name|q_SSLeay
argument_list|()
...
name|qWarning
argument_list|(
literal|"Random number generator not seeded, disabling SSL support"
argument_list|)
expr_stmt|;
return|return
literal|false
...
condition|)
block|{
comment|// Unconditionally exclude ADH and AECDH ciphers since they offer no MITM protection
if|if
condition|(
operator|!
name|ciph
...
comment|// set the list of supported ECs, but not the list
comment|// of *default* ECs. OpenSSL doesn't like forcing an EC for the wrong
comment|// ciphersuite, so don't try it -- leave the empty list to mean
comment|// "the implementation will choose the most suitable one".
...
end_ifndef
begin_comment
comment|// Apple implementation in qsslsocket_mac_shared.cpp
end_comment
begin_function
...
return|;
block|}
comment|// store peer certificate chain
name|storePeerCertificates
argument_list|()
expr_stmt|;
...
name|authenticator
decl_stmt|;
comment|// Fill in some read-only fields (for the user)
if|if
condition|(
name|hint
condition|)
...
argument_list|)
expr_stmt|;
comment|// it's NUL terminated, but do not include the NUL
name|authenticator
operator|.
name|d
operator|->
...
literal|1
expr_stmt|;
comment|// needs to be NUL terminated
name|authenticator
operator|.
name|d
operator|->
...
argument_list|)
expr_stmt|;
comment|// Let the client provide the remaining bits...
name|Q_Q
argument_list|(
name|QSslSocket
argument_list|)
...
argument_list|)
emit|;
comment|// No PSK set? Return now to make the handshake fail
if|if
condition|(
name|authenticator
operator|.
...
literal|0
return|;
comment|// Copy data back into OpenSSL
specifier|const
name|int
name|identityLength
init|=
...
condition|)
block|{
comment|// we could not agree -> be conservative and use HTTP/1.1
name|configuration
operator|.
