...
end_function
begin_comment
comment|/*!     Initializes QSslSocket with the native socket descriptor \a     socketDescriptor. Returns \c true if \a socketDescriptor is accepted     as a valid socket descriptor; otherwise returns \c false.     The socket is opened in the mode specified by \a openMode, and     enters the socket state specified by \a state.      \note It is not possible to initialize two sockets with the same     native socket descriptor.      \sa socketDescriptor() */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!     \since 5.4     Returns the version number of the SSL library in use at compile     time. If no SSL support is available then this will return an     undefined value.      \sa sslLibraryVersionNumber() */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!     \since 5.4     Returns the version string of the SSL library in use at compile     time. If no SSL support is available then this will return an     empty value.      \sa sslLibraryVersionString() */
end_comment
begin_function
...
parameter_list|)
block|{
comment|// this function is called from QAbstractSocket::bind
if|if
condition|(
operator|!
name|initialized
