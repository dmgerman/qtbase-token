...
specifier|static
name|SSLContextRef
name|qt_createSecureTransportContext
parameter_list|(
name|QSslSocket
operator|::
name|SslMode
...
name|kSSLClientSide
decl_stmt|;
comment|// We never use kSSLDatagramType, so it's kSSLStreamType unconditionally.
name|context
operator|=
...
argument_list|)
operator|<<
literal|"SSLNewContext failed with error:"
operator|<<
name|errCode
expr_stmt|;
block|}
...
specifier|static
name|void
name|qt_releaseSecureTransportContext
parameter_list|(
name|SSLContextRef
name|context
parameter_list|)
...
directive|endif
comment|// QT_MAC_PLATFORM_...
specifier|const
name|OSStatus
name|errCode
init|=
name|SSLDisposeContext
argument_list|(
name|context
argument_list|)
decl_stmt|;
...
argument_list|)
operator|<<
literal|"SSLDisposeContext failed with error:"
operator|<<
name|errCode
expr_stmt|;
block|}
...
parameter_list|)
block|{
comment|// This function works with (now) deprecated API that does not even exist on
comment|// iOS but is the only API we have on OS X below 10.8
comment|// Without SSLSetProtocolVersionMin/Max functions it's quite difficult
comment|// to have the required result:
comment|// If we use SSLSetProtocolVersion - any constant except the ones with 'Only' suffix -
comment|// allows a negotiation and we can not set the lower limit.
comment|// SSLSetProtocolVersionEnabled supports only a limited subset of constants, if you believe their docs:
comment|// kSSLProtocol2
comment|// kSSLProtocol3
comment|// kTLSProtocol1
comment|// kSSLProtocolAll
comment|// Here we can only have a look into the SecureTransport's code and hope that what we see there
comment|// and what we have on 10.7 is similar:
comment|// SSLSetProtocoLVersionEnabled actually accepts other constants also,
comment|// called twice with two different protocols it sets a range,
comment|// called once with a protocol (when all protocols were disabled)
comment|// - only this protocol is enabled (without a lower limit negotiation).
name|Q_ASSERT
argument_list|(
name|context
argument_list|)
...
if|if
condition|(
name|SSLSetProtocolVersionEnabled
argument_list|(
name|context
argument_list|,
name|kSSLProtocolAll
argument_list|,
literal|false
argument_list|)
operator|!=
...
name|err
operator|=
name|SSLSetProtocolVersion
argument_list|(
name|context
argument_list|,
name|kSSLProtocol3Only
argument_list|)
expr_stmt|;
block|}
elseif|else
...
argument|QMutex
argument_list|,
argument|qt_securetransport_mutex
argument_list|,
...
name|QLatin1String
argument_list|(
literal|"ECDHE-RSA-AES256-GCM-SHA384"
argument_list|)
expr_stmt|;
break|break;
default|default:
