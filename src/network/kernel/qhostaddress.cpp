...
literal|true
expr_stmt|;
comment|//create mapped address, except for a_ == 0 (any)
name|a6_64
operator|.
name|c
index|[
...
end_function
begin_comment
comment|/// parses v4-mapped addresses or the AnyIPv6 address and stores in \a a;
end_comment
begin_comment
comment|/// returns true if the address was one of those
end_comment
begin_function
...
end_comment
begin_comment
comment|/*!  Constructs a null host address object, i.e. an address which is not valid for any host or interface.      \sa clear() */
end_comment
begin_constructor
...
end_function
begin_comment
comment|/*!     \since 4.1      Sets the IPv6 scope ID of the address to \a id. If the address protocol is     not IPv6, this function does nothing. The scope ID may be set as an     interface name (such as "eth0" or "en1") or as an integer representing the     interface index. If \a id is an interface name, QtNetwork will convert to     an interface index using QNetworkInterface::interfaceIndexFromName() before     calling the operating system networking functions.      \sa scopeId(), QNetworkInterface, QNetworkInterface::interfaceFromName */
end_comment
begin_function
...
argument_list|)
decl_stmt|;
comment|// 1 for localhost, 0 for any
return|return
name|d
operator|->
name|a6_64
...
end_function
begin_comment
comment|/*!     \since 5.6      Returns \c true if the address is an IPv4 or IPv6 multicast address, \c     false otherwise. */
end_comment
begin_function
...
literal|true
return|;
comment|// 224.0.0.0-239.255.255.255 (including v4-mapped IPv6 addresses)
if|if
condition|(
name|d
operator|->
...
name|d
operator|<<
literal|"QHostAddress(QHostAddress::Any)"
expr_stmt|;
else|else
name|d
operator|<<
