...
name|returnValue
parameter_list|)
value|do { \     if (d->socketState != (state1)&& d->socketState != (state2)&& d->socketState != (state3)) { \         qWarning(""#function" was called" \                  " not in "#state1" or "#state2); \         return (returnValue); \     } } while (0)
end_define
begin_define
...
end_function
begin_comment
comment|/*!     \internal      Adjusts the incoming \a address family to match the currently bound address     (if any). This function will convert v4-mapped IPv6 addresses to IPv4 and     vice-versa. All other address types and values will be left unchanged.  */
end_comment
begin_function
...
condition|)
block|{
comment|// convert to IPv6 v4-mapped address. This always works
return|return
name|QHostAddress
argument_list|(
name|address
...
condition|)
block|{
comment|// convert to IPv4 if the source is a v4-mapped address
name|quint32
name|ip4
init|=
name|address
