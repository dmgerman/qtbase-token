...
name|IP_HOPLIMIT
value|21
comment|// Receive packet hop limit.
endif|#
directive|endif
if|#
directive|if
...
block|}
block|}
comment|// get the pointer to sendmsg and recvmsg
name|DWORD
name|bytesReturned
decl_stmt|;
name|GUID
...
literal|false
return|;
comment|// handle non-setsockopt options
switch|switch
condition|(
name|opt
condition|)
...
condition|)
block|{
comment|// MSDN says getsockopt with SO_ERROR clears the error, but it's not actually cleared
comment|// and this can affect all subsequent WSAConnect attempts, so clear it now.
specifier|const
name|int
name|val
init|=
...
condition|)
block|{
comment|// The default may change in future, so set it explicitly
name|int
name|ipv6only
init|=
literal|0
...
end_ifdef
begin_comment
comment|// Windows CE has no support for sendmsg or recvmsg. We set it to null here to simplify the code below.
end_comment
begin_expr_stmt
...
name|align
decl_stmt|;
comment|// only to ensure alignment
block|}
union|;
name|WSAMSG
name|msg
...
condition|)
block|{
comment|// get the ancillary data
name|WSACMSGHDR
modifier|*
name|cmsgptr
decl_stmt|;
