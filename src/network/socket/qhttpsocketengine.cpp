...
return|return;
block|}
comment|// Still in handshake mode. Wait until we've got a full response.
name|bool
name|done
init|=
literal|false
...
argument_list|()
expr_stmt|;
comment|// we parsed the proxy protocol response. from now on direct socket reading will be done
name|int
name|statusCode
init|=
name|responseHeader
