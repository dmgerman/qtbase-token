...
block|{
comment|// Discard unwanted data if opened in WriteOnly mode
...
name|qDebug
argument_list|(
literal|"QAbstractSocketPrivate::readFromSocket() about to discard %lld bytes"
argument_list|,
name|bytesToRead
argument_list|)
expr_stmt|;
...
end_function
begin_comment
comment|/*!     \internal      Sets the socket error state to \c errorCode and \a errorString. */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!     \internal      Sets the socket error state to \c errorCode and \a errorString,     and emits the QAbstractSocket::error() signal. */
end_comment
begin_function
...
name|written
return|;
comment|// written = actually written + what has been buffered
block|}
elseif|else
if|if
condition|(
