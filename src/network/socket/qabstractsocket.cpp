...
end_comment
begin_comment
comment|/*!     \enum QAbstractSocket::NetworkLayerProtocol      This enum describes the network layer protocol values used in Qt.      \value IPv4Protocol IPv4     \value IPv6Protocol IPv6     \value UnknownNetworkLayerProtocol Other than IPv4 and IPv6      \sa QHostAddress::protocol() */
end_comment
begin_comment
...
end_destructor
begin_comment
comment|/*!     Returns true if the socket is valid and ready for use; otherwise     returns false.      \bold{Note:} The socket's state must be ConnectedState before reading and     writing can occur.      \sa state() */
end_comment
begin_function
...
directive|ifndef
name|QT_NO_OPENSSL
comment|// Manual polymorphism; this function is not virtual, but has an overload
comment|// in QSslSocket.
if|if
condition|(
name|QSslSocket
modifier|*
...
end_function
begin_comment
comment|/*! \reimp       Returns true if no more data is currently      available for reading; otherwise returns false.       This function is most commonly used when reading data from the      socket in a loop. For example:       \snippet doc/src/snippets/code/src_network_socket_qabstractsocket.cpp 2       \sa bytesAvailable(), readyRead()  */
end_comment
begin_function
DECL|function|atEnd
name|bool
...
argument_list|)
return|;
comment|// short cut for a char read if we have something in the buffer
if|if
condition|(
name|maxSize
operator|==
...
name|qDebug
argument_list|(
literal|"QAbstractSocket::readData(%p '%c (0x%.2x)', 1) == 1 [char buffer]"
argument_list|,
name|data
argument_list|,
name|isprint
...
return|;
block|}
comment|// Special case for an Unbuffered QTcpSocket
comment|// Re-filling the buffer.
if|if
condition|(
name|d
operator|->
...
condition|)
block|{
comment|// Our buffer is empty and a read() was requested for a byte amount that is smaller
comment|// than the readBufferMaxSize. This means that we should fill our buffer since we want
comment|// such small reads come from the buffer and not always go to the costly socket engine read()
name|qint64
name|bytesToRead
init|=
name|d
...
block|}
block|}
comment|// First try to satisfy the read from the buffer
name|qint64
name|bytesToRead
init|=
name|qMin
...
condition|)
block|{
comment|// we read some data from buffer.
comment|// Just return, readyRead will be emitted again
if|#
directive|if
name|defined
argument_list|(
...
name|qDebug
argument_list|(
literal|"QAbstractSocket::readData(%p '%c (0x%.2x)', %lli) == %lli [buffer]"
argument_list|,
name|data
argument_list|,
name|isprint
...
return|;
block|}
comment|// This code path is for Unbuffered QTcpSocket or for connected UDP
if|if
condition|(
operator|!
name|d
...
name|qDebug
argument_list|(
literal|"QAbstractSocket::readData(%p \"%s\", %lli) == %lld [unreachable]"
argument_list|,
name|data
argument_list|,
name|qt_prettyDebug
...
directive|ifndef
name|QT_NO_OPENSSL
comment|// Manual polymorphism; setReadBufferSize() isn't virtual, but QSslSocket overloads
comment|// it.
if|if
condition|(
name|QSslSocket
