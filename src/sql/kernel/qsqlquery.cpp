...
end_function
begin_comment
comment|/*!     Returns the value of field \a index in the current record.      The fields are numbered from left to right using the text of the     \c SELECT statement, e.g. in      \snippet doc/src/snippets/code/src_sql_kernel_qsqlquery.cpp 0      field 0 is \c forename and field 1 is \c     surname. Using \c{SELECT *} is not recommended because the order     of the fields in the query is undefined.      An invalid QVariant is returned if field \a index does not     exist, if the query is inactive, or if the query is positioned on     an invalid record.      \sa previous() next() first() last() seek() isActive() isValid() */
end_comment
begin_function
DECL|function|value
name|QVariant
...
end_function
begin_comment
comment|/*!   Returns a map of the bound values.    With named binding, the bound values can be examined in the   following ways:    \snippet doc/src/snippets/sqldatabase/sqldatabase.cpp 14    With positional binding, the code becomes:    \snippet doc/src/snippets/sqldatabase/sqldatabase.cpp 15    \sa boundValue() bindValue() addBindValue() */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!   \fn bool QSqlQuery::prev()    Use previous() instead. */
end_comment
begin_comment
