...
name|qWarning
argument_list|(
literal|"QSqlQuery::isNull: unknown field name '%s'"
argument_list|,
name|qPrintable
argument_list|(
name|name
...
end_function
begin_comment
comment|/*!   Returns the size of the result (number of rows returned), or -1 if   the size cannot be determined or if the database does not support   reporting information about query sizes. Note that for non-\c SELECT   statements (isSelect() returns \c false), size() will return -1. If the   query is not active (isActive() returns \c false), -1 is returned.    To determine the number of rows affected by a non-\c SELECT   statement, use numRowsAffected().    \sa isActive(), numRowsAffected(), QSqlDriver::hasFeature() */
end_comment
begin_function
DECL|function|size
name|int
...
end_function
begin_comment
comment|/*!   Returns \c true if the query is currently positioned on a valid   record; otherwise returns \c false. */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!   Returns \c true if the current query is a \c SELECT statement;   otherwise returns \c false. */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!   Returns \c true if you can only scroll forward through a result set;   otherwise returns \c false.    \sa setForwardOnly(), next() */
end_comment
begin_function
...
end_function
begin_comment
comment|/*!   Executes a previously prepared SQL query. Returns \c true if the query   executed successfully; otherwise returns \c false.    Note that the last error for this query is reset when exec() is   called.    \sa prepare(), bindValue(), addBindValue(), boundValue(), boundValues() */
end_comment
begin_function
DECL|function|exec
name|bool
...
argument_list|()
operator|<<
literal|"Executed prepared query ("
operator|<<
name|t
operator|.
name|elapsed
