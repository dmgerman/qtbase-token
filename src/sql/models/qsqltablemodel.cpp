...
name|it
expr_stmt|;
comment|// rows must be accounted for
if|if
condition|(
name|it
operator|.
...
end_function
begin_comment
comment|/*!     Sets the data for the item \a index for the role \a role to \a     value.      For edit strategy OnFieldChange, an index may receive a change     only if no other index has a cached change. Changes are     submitted immediately. However, rows that have not yet been     inserted in the database may be freely changed and and are not     submitted automatically.      For OnRowChange, the first change to a row will cause cached     operations on other rows to be submitted. If submitting fails,     the new change is rejected.      Submitted changes are not reverted upon failure.      Returns true if the value could be set or false on error, for     example if \a index is out of bounds.      \sa editStrategy(), data(), submit(), submitAll(), revertRow() */
end_comment
begin_function
