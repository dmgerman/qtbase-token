...
argument_list|)
decl_stmt|;
comment|// cannot revert a committed change
if|if
condition|(
name|r
operator|.
...
literal|false
return|;
comment|// clear the submitted flags so revertAll can do its job
for|for
control|(
name|QSqlTableModelPrivate
operator|::
...
expr_stmt|;
block|}
comment|// all changes have been committed
comment|// clean up inserted rows
name|QSqlTableModelPrivate
operator|::
name|CacheMap
operator|::
