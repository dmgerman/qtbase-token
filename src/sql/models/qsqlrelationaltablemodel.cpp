...
argument_list|)
expr_stmt|;
comment|// If there are duplicate field names they must be aliased
if|if
condition|(
name|fieldNames
operator|.
...
condition|)
block|{
comment|// this needs fixing!! the below if is borken.
...
end_function
begin_comment
comment|/*! \enum QSqlRelationalTableModel::JoinMode      \value InnerJoin - Inner join mode, return rows when there is at least one match in both tables.     \value LeftJoin - Left join mode, returns all rows from the left table (table_name1), even if there are no matches in the right table (table_name2).      \see QSqlRelationalTableModel::setJoinMode     \since 4.8 */
end_comment
begin_comment
comment|/*!     Sets the SQL join mode to show or hide rows with NULL foreign keys.     In InnerJoin mode (the default) these rows will not be showed: use the     LeftJoin mode if you want to show them.      \see QSqlRelationalTableModel::JoinMode     \since 4.8 */
end_comment
begin_function
