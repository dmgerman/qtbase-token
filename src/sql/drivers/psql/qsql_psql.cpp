...
argument_list|)
decl_stmt|;
comment|// Most recent sequence value obtained from nextval
if|if
condition|(
name|qry
operator|.
...
parameter_list|()
block|{
comment|// standard_conforming_strings option introduced in 8.2
comment|// http://www.postgresql.org/docs/8.2/static/runtime-config-compatible.html
if|if
condition|(
name|pro
operator|<
...
specifier|inline
name|void
name|assignSpecialPsqlFloatValue
parameter_list|(
name|FloatType
name|val
parameter_list|,
...
condition|)
block|{
comment|// we force the value to be considered with a timezone information, and we force it to be UTC
comment|// this is safe since postgresql stores only the UTC value and not the timezone offset (only used
comment|// while parsing), so we have correct behavior in both case of with timezone and without tz
name|r
operator|=
name|QLatin1String
argument_list|(
...
name|QLatin1String
argument_list|(
literal|"yyyy-MM-ddThh:mm:ss.zzz"
argument_list|)
argument_list|)
operator|+
name|QLatin1Char
